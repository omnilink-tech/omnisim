...
class CaptureState:
    ...
    def __init__(self, supervisor: Supervisor, basic_step_ms: int):
        ...
        self.camera_fov = 0.0
        # NOTE: cam.enable() currently segfaults Webots in --batch render
        # paths. Skip it; screenshots fall back to Supervisor.exportImage.
        # We still read the Camera node's declared resolution for sim_state.
        try:
        ...
        if cam is not None:
            try:
                ...
                pass
        if cam is not None and os.environ.get("OMNISIM_CAPTURE_ENABLE_CAMERA") == "1":
            try:
...
def axis_angle_to_target(position: list[float], target: list[float]) -> list[float]:
    ...
    package. Webots default forward is +X.
    """
    import math
...
def cmd_screenshot(state: CaptureState, args: dict) -> dict:
    ...
    if source == "camera":
        if state.camera is None:
            raise CommandError("camera device not available; pass source='viewport' to use exportImage")
        ok = state.camera.saveImage(path, quality)
        # Camera.saveImage returns 0 on success, -1 on failure.
        if ok == -1:
    ...
    if source == "viewport":
        ...
        return {"path": path, "source": "viewport"}
