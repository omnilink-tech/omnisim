...
Alternatives: Jackal (smaller), Husarion ROSbot, OmniQuad (quadruped, rough terrain).
...
* Outdoor precedent: `samples/demos/worlds/showcase/husky_rocks_traverse.omniworld`
  (Husky + `ElevationGrid` hill + `husky_terrain_nav` controller),
  `environments/desert_ruins.omniworld`, `forest.omniworld`.
