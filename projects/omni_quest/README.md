...
> **Camera obstacle avoidance ✅.** `omni_quest_offroad.wbt` adds rougher rolling
...
`omni_quest_world.wbt` is the showcase — a flat grass field densely populated for
a vibrant scene:
...
live: **File → Open World → `omni_quest_world.wbt`**.*

### Cross-platform fleet

`omni_quest_fleet.wbt` proves the navigation algorithm is **platform-agnostic**: a
...
`omni_quest_swarm.wbt` turns the whole fleet into navigators: **four robots
...
`omni_quest_intersection.wbt` solves the perpendicular crossing the right way —
...
& "$env:OMNISIM_HOME\msys64\mingw64\bin\webots-bin.exe" `
  "$env:OMNISIM_HOME\projects\omni_quest\worlds\omni_quest_waypoints.wbt" `
...
In the GUI: open `worlds/omni_quest_waypoints.wbt` from OmniSim
...
├── worlds/
│   └── omni_quest_waypoints.wbt     # M1: Husky + grass field + 4 GPS posts
└── controllers/
    └── omni_quest_nav/
