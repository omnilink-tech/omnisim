...
|---|---|---|
| [`battlebots/worlds/battlebox_duel.omniworld`](battlebots/worlds/battlebox_duel.omniworld) | RED vertical-spinner vs BLUE wedge (lightweight reference, ~10 kg) | BLUE wins by chassis_dead at t=122 s |
| [`battlebots/worlds/battlebox_royal_rumble.omniworld`](battlebots/worlds/battlebox_royal_rumble.omniworld) | All 5 BattleBot weapon archetypes free-for-all | (legacy match_director scorecard) |
| [`battlebots/worlds/battlebox_husky_proving.omniworld`](battlebots/worlds/battlebox_husky_proving.omniworld) | 2 Huskies in the BattleBox arena (Phase-1 validation) | n/a — arena test |
| [`battlebots/worlds/gravedigger_vs_biteforce.omniworld`](battlebots/worlds/gravedigger_vs_biteforce.omniworld) | Tombstone tribute (horizontal bar) vs Bite Force tribute (vertical disc) — both heavyweight | mutual weapon destruction, draw at t=240 s |
| [`battlebots/worlds/hydra_vs_gravedigger.omniworld`](battlebots/worlds/hydra_vs_gravedigger.omniworld) | Hydra tribute (pneumatic flipper, `--weapon-mode pulse`) vs Gravedigger | Hydra breaks Gravedigger's bar at t=3.7 s, draw on wheel count at t=240 s |
| [`battlebots/worlds/huge_vs_bitebot.omniworld`](battlebots/worlds/huge_vs_bitebot.omniworld) | HUGE tribute (giant wheels + overhead bar) vs BiteBot | draw at timeout, HUGE chassis untouched (wheels did their job) |
...
| [`battlebots/protos/BattleBot.proto`](battlebots/protos/BattleBot.proto) | Parameterised 4WD chassis with a `weaponType` slot: `wedge`, `vertical_spinner`, `horizontal_spinner`, `hammer`, `flipper`. Only used by `battlebox_royal_rumble.omniworld` and the legacy worlds; the headliner matchups (Gravedigger, Hydra, HUGE, BiteBot) are inlined directly into the world so the damage director can detach parts via `Supervisor.parent.remove()`. |
...
| [`battlebots/controllers/match_director/`](battlebots/controllers/match_director) | Legacy judge-scoring referee — 3-min clock, KO/OOTA detection, BattleBots-style judges' scorecard (damage / aggression / control). Still wired into `battlebox_royal_rumble.omniworld`. |
...
"$OMNISIM_BIN" --mode=realtime \
    projects/robot_combat/battlebots/worlds/battlebox_duel.omniworld
```
...
```bash
"$OMNISIM_BIN" --mode=realtime \
    projects/robot_combat/battlebots/worlds/gravedigger_vs_biteforce.omniworld
"$OMNISIM_BIN" --mode=realtime \
    projects/robot_combat/battlebots/worlds/hydra_vs_gravedigger.omniworld
"$OMNISIM_BIN" --mode=realtime \
    projects/robot_combat/battlebots/worlds/huge_vs_bitebot.omniworld
```
...
1. Open one of the existing matchup worlds as a template (e.g.
   `gravedigger_vs_biteforce.omniworld`).
