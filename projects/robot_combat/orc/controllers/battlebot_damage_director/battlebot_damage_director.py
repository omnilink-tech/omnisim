...
customData schema (on this supervisor node):
  {
    ...
    # ORC "queen defense" mode (optional). When both keys are present,
    # the match-end rule changes from "last bot standing wins" to
    # "team whose queen survives wins". A queen counts as down via the
    # same immobilization criteria as any other fighter (chassis HP,
    # lost wheels, OOTA). Other fighters can still be eliminated
    # individually but their team only loses when the queen falls.
...
def main():
    ...
    # "Queen defense" config: when both `teams` and `queens` are
    # present, replace the last-bot-standing rule with "team with a
    # surviving queen wins". teams is unused for direct logic but
    # echoed back in write_state() so brains can see team rosters.
    teams_cfg = cfg.get("teams") if isinstance(cfg.get("teams"), dict) else {}
    queens_cfg = cfg.get("queens") if isinstance(cfg.get("queens"), dict) else {}
    queens_mode = bool(teams_cfg and queens_cfg)
    if queens_mode:
        sys.stderr.write(
            f"[damage_director] queens mode: teams={list(teams_cfg.keys())} "
            f"queens={queens_cfg}\n"
        )
    ...
    def write_state():
        ...
        if queens_mode:
            st["teams"] = teams_cfg
            st["queens"] = queens_cfg
            st["queens_alive"] = {
                team: not any(
                    f.name == q and f.immobile for f in fighters
                )
                for team, q in queens_cfg.items()
            }
        try:
    ...
    while sup.step(step_ms) != -1:
        ...
        if not match_over:
            ...
            # Match-end decision.
            if queens_mode:
                # Team with a surviving queen wins. Other fighters can be
                # eliminated individually but their team only loses when
                # the queen falls.
                queen_status = {}
                for team_name, qn in queens_cfg.items():
                    qf = next((f for f in fighters if f.name == qn), None)
                    # Missing queen node = treat as down.
                    queen_status[team_name] = (qf is None) or qf.immobile
                dead_teams = [t for t, dead in queen_status.items() if dead]
                alive_teams = [t for t, dead in queen_status.items() if not dead]
                if dead_teams:
                    match_over = True
                    if len(alive_teams) == 1:
                        winner = alive_teams[0]
                        win_reason = f"queen_killed:{dead_teams[0]}"
                    else:
                        winner = "draw"
                        win_reason = "all_queens_killed"
                    sys.stderr.write(
                    ...
                    match_over = True
                    # Timeout tiebreak: most surviving roster members (incl. queen).
                    team_survivors = {
                        t: sum(1 for f in fighters
                               if f.name in teams_cfg.get(t, [])
                               and not f.immobile)
                        for t in teams_cfg
                    }
                    best = max(team_survivors, key=lambda n: team_survivors[n])
                    if team_survivors[best] == min(team_survivors.values()):
                        winner = "draw"
                        win_reason = "timeout_tie"
                    else:
                        winner = best
                        win_reason = "timeout_team_survivor_count"
                    sys.stderr.write(
                        f"[damage_director] MATCH OVER at t={sim_t:.2f}s "
