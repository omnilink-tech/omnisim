...
Wired into the world: husky_maze.omniworld's Husky has supervisor=TRUE and
...
# --- Maze layout (matches husky_maze.omniworld header comments) -------------------
...
# Default 6070 / 6071 is the single-husky world contract every husky_maze_*
# world relies on — none of them set controllerArgs, so both ports must be
# correct with no arguments at all.
# A world with more than one husky overrides per-instance via Webots
# controllerArgs:
#   controller "husky_omnilink_bridge"
#   controllerArgs ["--port" "6080" "--eye-port" "6081"]
# (No world in the tree currently does this; the multiplexing path is kept
# because it costs nothing and a second husky is a one-line world edit.)
...
class BridgeState:
    def __init__(self):
        ...
        #                      | {kind: "turn", seq, commanded, remaining,
        #                         last_yaw, speed, deadline}
        #                      | {kind: "goto_cell", col, row, x, y, speed, deadline}
        self.task = None
        # Turn bookkeeping. `turn_seq` stamps each turn command so a reply
        # can only ever quote the measurement belonging to ITS OWN command;
        # `last_turn` is where the physics loop deposits the settled,
        # UNWRAPPED signed rotation when a turn finishes. A reply that
        # cannot match a measurement to its seq reports achieved: null --
        # it never falls back to the commanded angle.
        self.turn_seq = 0
        self.last_turn = None
    ...
    def snapshot(self):
        with self.lock:
            ...
            return {
                ...
                # hint. The vision-only protocol on husky_maze_blind.omniworld
...
def _turn_achieved(state: "BridgeState", seq: int, commanded: float):
    """Signed, UNWRAPPED rotation actually delivered by turn `seq`.

    Returns ``(achieved_rad_or_None, source)``. ``None`` means the bridge
    could not attribute a measurement to THIS command -- a later turn
    superseded it, or its accumulator is gone. None is the honest answer
    there; the commanded angle is never substituted for a measurement.
    """
    with state.lock:
        done = dict(state.last_turn) if state.last_turn else None
        t = state.task
        live = (dict(t) if (t is not None and t.get("kind") == "turn"
                            and t.get("seq") == seq) else None)
    if done is not None and done.get("seq") == seq:
        return float(done["achieved"]), "settled"
    if live is not None and "remaining" in live:
        # Still turning: the caller's wait budget expired before the turn
        # did. `remaining` is decremented by MEASURED yaw deltas, so this
        # is a real partial reading -- labelled in_flight so no caller
        # mistakes it for a final value.
        return float(commanded) - float(live["remaining"]), "in_flight"
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/protocol":
                self._json(200, {
                    ...
                    "instance": {"name": "husky_omnilink_bridge", "robot_id": "husky"},
                    "extensions": [],
                })
                return
            if self.path == "/state":
            ...
            if self.path.startswith("/solid"):
                ...
                # rotation of any DEF'd Solid in the scene. This is the
                # ground-truth read: it reports where something actually
                # IS, independent of what any agent claims it did or how
                # the robot got there. Operators use it to check world
                # state without screenshotting the GUI, and tools that
                # perform side-effects use it to verify rather than trust
                # the actuator's "command accepted" return.
                # See agents/AGENT_PATTERNS.md §2 / §2b — results graded
                # through this endpoint stay quotable even when a defect
                # is later found in the locomotion layer beneath them.
                ...
                if node is None:
                    self._json(404, {
                        ...
                        "hint": "names are case-sensitive, and only nodes given an explicit DEF in the .wbt are addressable. The husky_maze worlds DEF just HUSKY (plus SUN / SUN_MARKER); the maze markers are not DEF'd, so query them through /scan on the husky_eye sidecar instead.",
                    })
                    return
                try:
            ...
            if self.path == "/mission":
                self._json(200, {
                    ...
                    "hint": (
                        ...
                        "...] }. The bridge verifies every claimed cell "
                        "against the trail it recorded from the physics body "
                        "pose, and refuses the claim with 409 if any cell was "
                        "never actually reached. Drive there before you claim "
                        "it."
                    ),
                })
                return
        ...
        def do_POST(self):
            ...
            try:
                ...
                import traceback
                print(f"[husky_omnilink_bridge] HTTP request failed: {exc!r}\n{traceback.format_exc()}")
        ...
        def _route_post(self):
            ...
            if self.path == "/admin/teleport_solid":
                # Teleport any DEF'd Solid via the supervisor. This is
                # OPERATOR test-setup tooling: it exists to author
                # "something moved" ground truth between runs (relocate an
                # object, then check the agent notices). It moves scene
                # objects only.
                # NOT to be confused with the wedge-recovery teleport that
                # used to snap a STUCK ROBOT to its target instead of
                # reporting the fault — that was a defect and was removed
                # in b18bd7a3. Keep the distinction when reading run notes:
                # "after teleport" has historically meant both.
                # Body: {"def": "SOME_SOLID", "x": 1.0, "y": 2.0, "z": 0.20}.
                try:
                    body = self._read_json()
                except Exception as exc:
                ...
                if not def_name:
                    self._json(400, {"error": "'def' required",
                                     "example": "{\"def\":\"HUSKY\",\"x\":1,\"y\":2,\"z\":0.2}"})
                    return
                sup = getattr(state, "supervisor", None)
                if sup is None:
                    self._json(503, {"error": "supervisor not initialised"})
            ...
            if action == "complete_mission":
                ...
                if "corners" in title_lower and current_cell != (0, 10):
                    reasons.append({
                        "kind": "return_to_start_required",
                        "current_cell": list(current_cell),
                        "required_final_cell": [0, 10],
                        "hint": "the corners tour is incomplete until the robot returns to its start cell",
                    })
            ...
            if action == "drive_forward":
                ...
                speed_fraction = finite_number(body.get("speed", 0.5), "speed")
                speed = clamp(
                    speed_fraction * MAX_LINEAR_M_S,
                    0.05, MAX_LINEAR_M_S,
                )
                ...
                with state.lock:
                    ...
                    state.fault = None
                if body.get("wait"):
                    # PROTOCOL.md 5.4.1. This tool's description has always
                    # promised "returns when within 0.15 m or after 30 s" --
                    # a blocking contract the bridge did not honour, so a
                    # model that believed the description issued its next
                    # command into a still-moving robot. Now it is true.
                    r = _wait_for_task_idle(state, GOTO_TIMEOUT_S + 5.0)
                    # Signed progress along the ORIGINAL heading axis, which
                    # is what was commanded -- not Euclidean drift.
                    achieved = (r["x"] - sx) * hx + (r["y"] - sy) * hy
                    self._ok({"commanded": distance, "achieved": achieved,
                              "error": achieved - distance, "unit": "m",
                              "settled": r["fault"] is None,
                              "fault": r["fault"],
                              "speed_m_s": speed,
                              "final_pose": {"x": r["x"], "y": r["y"],
                                             "yaw": r["yaw"]}})
                    return
                self._ok({"commanded": distance, "achieved": None,
                          "speed_m_s": speed, "unit": "m",
                          "note": ("NOT complete -- returns on acceptance. "
                                   "Pass wait=true for the achieved distance, "
                                   "or poll get_state until mode is idle.")})
                return

            if action == "drive_to_waypoint":
                # Continuous-space "drive to (x, y)" — for worlds where the
                # husky navigates arbitrary world coordinates rather than a
                # fixed cell grid. Reuses the existing goto_cell per-tick
                # controller (two-phase turn-then-drive with a heading-error
                # gate) because that's exactly the right control law for
                # "go to a point"; the only difference from goto_cell is
                # that there's no cell-grid validation or snap-to-grid
                # post-step. Synchronous-wait mirrors goto_cell's wait=true
                # contract.
                # NOTE: no world in the tree currently uses this path — the
                # husky_maze_* worlds are all cell-grid (goto_cell /
                # execute_path). It is kept as the open-world counterpart to
                # goto_cell; the defaults below were tuned against a ~30 m
                # warehouse floor, so re-check them against your own layout.
                try:
                    tx = float(body.get("x"))
                    ty = float(body.get("y"))
                ...
                # 2 m maze cell hops; an open-floor waypoint can be 20+ m of
                # diagonal traversal at the husky's ~0.5 m/s average
                # (turn-then-drive loses momentum repeatedly). 90 s covers a
                # ~30 m floor end to end; scale it with your world.
                ...
                if body.get("wait"):
                    ...
                    # Waypoint-mode tolerance is deliberately more permissive
                    # than the cell-grid GOTO_REACH_TOL_M (0.18 m): open-floor
                    # navigation wants "in the area", not cm accuracy.
                    # Sizing rule, if you need to retune: keep it below half
                    # the narrowest aisle the husky must pass (so it can't
                    # settle into a neighbour), and below the range at which
                    # the camera can still resolve whatever it came to look
                    # at (FoV at 1 m gives a ~1.4 m visible patch). Caller
                    # can pass `arrival_tolerance_m` for tighter or looser.
                    # If we got within tolerance, treat http_wait_timeout as
                    ...
                    # but in waypoint mode, getting within arrival_tol of
                    # the target IS reaching it.
            ...
            if action == "turn":
                ...
                if not math.isfinite(angle):
                    self._json(400, {"error": "angle must be a finite number"})
                    return
                speed = clamp(
                    float(body.get("speed", 0.6)) * MAX_ANGULAR_R_S,
                    0.0, MAX_ANGULAR_R_S,
                )
                # ACCUMULATED signed rotation, never an absolute target yaw.
                # wrap_pi(yaw + angle) drops a full turn back onto the
                # current yaw, so turn(2*pi) asked the husky to rotate to
                # where it already stood: it never moved, and the reply said
                # achieved -0.000000, error 0.000000, settled true. Half
                # turns were worse -- +pi and -pi wrap to the SAME target,
                # so turn(3.5) rotated -2.783 (the short way round, the
                # opposite sign) and still reported error 0.000000, because
                # the error was wrapped too and therefore could not express
                # an under-rotation past half a turn. `remaining` is
                # decremented by the per-tick MEASURED delta instead: it has
                # no antipode, and it lets a command exceed +/-pi.
                with state.lock:
                    state.turn_seq += 1
                    seq = state.turn_seq
                    state.task = {
                        ...
                        "last_yaw": state.yaw,
                        "start_yaw": state.yaw,
                        # Context only -- the loop drives off `remaining`.
                ...
                if body.get("wait"):
                    r = _wait_for_task_idle(state, GOTO_TIMEOUT_S + 5.0)
                    achieved, measured_from = _turn_achieved(state, seq, angle)
                    self._ok({"commanded": angle,
                              "achieved": achieved,
                              # UNWRAPPED. An under-rotation of more than
                              # half a turn has to be expressible.
                              "error": (None if achieved is None
                                        else achieved - angle),
                              "unit": "rad",
                              "settled": (measured_from == "settled"
                                          and r["fault"] is None),
                              "fault": r["fault"],
                              ...
                              "final_pose": {"x": r["x"], "y": r["y"],
                                             "yaw": r["yaw"]}})
                    return
                self._ok({"commanded": angle, "achieved": None, "seq": seq,
                          "speed_rad_s": speed, "unit": "rad",
                          "note": ("NOT complete -- returns on acceptance. "
                                   "Pass wait=true for the achieved angle, "
                                   "or poll get_state until mode is idle.")})
                return
...
def _twist_for_task(task, x, y, yaw, sim_time, v_lin, v_ang):
    ...
    if kind == "turn":
        # Accumulate the signed rotation actually delivered and drive the
        # RESIDUAL to zero. Wrapping the per-tick delta is safe (one tick of
        # a <=3.5 rad/s pivot is a few milliradians); wrapping the running
        # TOTAL is what turned turn(2*pi) into a silent no-op. The
        # accumulator also picks up rotation the stuck-escape manoeuvre
        # contributes, which is correct -- it is rotation that happened.
        prev_yaw = task.get("last_yaw")
        if prev_yaw is None:
            # Plan built before the accumulator existed; adopt the current
            # yaw as the datum rather than inventing progress.
            task["last_yaw"] = yaw
            task.setdefault("remaining", 0.0)
        else:
            task["remaining"] = task["remaining"] - wrap_pi(yaw - prev_yaw)
            task["last_yaw"] = yaw
        # Deadline is checked AFTER accumulating so the final measured
        # delta still lands in `remaining` on the tick that aborts.
        if sim_time > task["deadline"]:
...
def main():
    ...
    # Stash the supervisor so the /solid and /admin/teleport_solid
    # endpoints can read and write DEF'd Solid positions on demand —
    # the ground-truth path that lets a caller verify world state
    # instead of trusting an actuator's acknowledgement.
    ...
    while robot.step(time_step) != -1:
        ...
        if task is not None:
            ...
            # `task` is a COPY of state.task, so the turn controller's
            # accumulated progress has to be written back or every tick
            # would restart from the full commanded angle.
            if task.get("kind") == "turn":
                with state.lock:
                    live = state.task
                    if (live is not None and live.get("kind") == "turn"
                            and live.get("seq") == task.get("seq")):
                        live["remaining"] = task["remaining"]
                        live["last_yaw"] = task["last_yaw"]
            if done:
                with state.lock:
                    if task.get("kind") == "turn":
                        # Publish the settled, UNWRAPPED measurement for
                        # the HTTP reply. achieved = commanded - what is
                        # left of it; both sides are measured, neither is
                        # the argument echoed back.
                        state.last_turn = {
                            "seq": task.get("seq"),
                            "commanded": float(task.get("commanded", 0.0)),
                            "achieved": (float(task.get("commanded", 0.0))
                                         - float(task.get("remaining", 0.0))),
                            "fault": fault,
                            "sim_time": state.sim_time,
                        }
