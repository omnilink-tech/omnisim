...
# Default 6071 + tracking DEF "HUSKY" is the single-husky world contract
# husky_maze_visual / husky_maze_blind rely on — neither sets
# controllerArgs, so both must be correct with no arguments at all.
...
# (No world in the tree currently does this; kept because it costs
# nothing and pairs with the same option on husky_omnilink_bridge.)
...
def _analyze_bgra(bgra_bytes: bytes, w: int, h: int) -> dict:
    ...
    # Six-colour marker counters. The maze worlds use red/green/blue
    # cylinder markers, plus a yellow finial on the goal post — so four
    # of the six fire in the tree as it stands. magenta/cyan are kept
    # because the classifier's else-if chain is order-sensitive (see
    # below): the secondary-colour branches exist to catch pixels that
    # would otherwise mis-bin into a primary, so removing them would
    # change how red/green/blue count, not just drop two keys.
    # The point of classifying in the controller at all is that a tool
    # can return a structured marker response without sending pixels
    # to the LLM.
    ...
    for y in range(h):
        ...
        for x in range(w):
            ...
            # were tuned against emissive marker geometry — an
            # emissiveColor pushes channels well past these gates under
            # any lighting, so they are deliberately loose. Re-tune if
            # you add markers that rely on diffuse shading instead.
            tag = None
    ...
    # matter at 0.002 threshold.
    # Where 0.002 comes from: the smallest marker that must still be
    # detectable is a 0.30 m emissive cube at a 4 m vantage, which
    # occupies ~290 px of a 76,800 px frame (~0.38%) — so the floor sits
    # roughly half that, leaving headroom without admitting noise. The
    # maze's 0.50 m cylinder markers at ~1 m fill 5-10%, i.e. far above
    # it. Re-derive from the smallest-and-furthest marker in your world
    # if you change marker geometry or camera resolution.
    marker = None
    n_marker = 0
    centroid = None
    ...
    # against husky_maze_blind.omniworld: a roughcast wall ~1 m away gives
