...
"""Generate go2_ghost.urdf: the Go2 URDF with every <inertial> and
...
projects/robots/boston_dynamics/spot/urdf/make_ghost_urdf.py).

Run after make_go2_urdf.py (it reads the generated go2.urdf).
"""
    ...
    for tag in ("inertial", "collision"):
        for el in link.findall(tag):
            ...
            if tag == "inertial":
                ...
                n_coll += 1
root.set("name", root.get("name", "go2") + "_ghost")
