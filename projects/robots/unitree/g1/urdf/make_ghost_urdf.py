...
"""Generate g1_23dof_ghost.urdf: the deploy G1 URDF with every <inertial>
and <collision> stripped, leaving only <visual> + <joint>. The OmniSim URDF
importer then emits NO Physics on any link (no inertial -> no physics block,
no collision -> no synthetic physics), so the robot imports as a fully
...
motor.setPosition(). This is the gait-model GHOST body.
"""
import xml.etree.ElementTree as ET
from pathlib import Path

HERE = Path(__file__).resolve().parent
SRC = HERE / "g1_23dof_omnisim.urdf"
DST = HERE / "g1_23dof_ghost.urdf"
    ...
    for tag in ("inertial", "collision"):
        for el in link.findall(tag):
            ...
            if tag == "inertial":
                ...
                n_coll += 1
root.set("name", root.get("name", "g1") + "_ghost")
