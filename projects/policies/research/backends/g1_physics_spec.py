...
⚠ SCOPE -- READ THIS BEFORE QUOTING A NUMBER FROM HERE (measured 2026-08-15).
"the trainer and the deploy" above means the RESEARCH lane and
``g1_deploy_launch.py``. It does NOT cover the shipped flagship humanoid path,
which is a second, self-consistent lane that never imports this module:
...
``g1_walk_recipe.py`` -- the current flagship trainer, reached via
``run_g1_decent_walker.ps1`` -> ``run_walk_rl.sh`` -- contains no import of this
spec. That is NOT a train/deploy mismatch: the export sits above the mode
branch, so probe, train and deploy all take 200/30/2.0 and train==deploy still
holds within that lane. But the two lanes disagree with each other, and
``tests/test_g1_physics_spec_conformance.py`` enforces this spec only on the
lane that already obeys it -- so nothing detects further drift.

Do not "fix" the disagreement by editing one side: 200/30/2.0 is what the
shipped champion was trained AND deploys at, and 100/5/1.0 is what the research
lane and the conformance test are built on. Reconciling them is a decision
about the training recipe, not a constant edit.
