...
class Sb3CpuBackend:
    ...
    def train(self, robot, config, out_dir):
        ...
        # The SpotEnv lives in projects/policies/research/envs/spot_env.py. For
        # non-Spot robots, a similar gym env should be plugged in.
        sys.path.insert(0, str(REPO_ROOT / "projects" / "rl" / "envs"))
        from spot_env import SpotEnv

        # Spot is the only robot the sb3 backend supports today (the gym
        # env subprocess is Spot-specific). Other robots can train via
        # mjx by pointing to their own URDF + RobotSpec.
        if robot.name != "spot":
            raise NotImplementedError(
                f"sb3 backend currently wraps SpotEnv; {robot.name!r} "
        ...
        # Reward weights -> env vars (consumed by spot_rl_agent.py).
        env_vars = {
            "SPOT_R_LIN_VEL_WT": str(config.r_lin_vel_wt),
            "SPOT_R_ANG_VEL_WT": str(config.r_ang_vel_wt),
            "SPOT_R_VZ_WT": str(config.r_vz_wt),
            "SPOT_R_ROLLPITCH_WT": str(config.r_rollpitch_wt),
            "SPOT_R_ANG_XY_WT": str(config.r_ang_xy_wt),
            "SPOT_R_ACTION_RATE_WT": str(config.r_action_rate_wt),
            "SPOT_R_JOINT_ACC_WT": str(config.r_joint_acc_wt),
            "SPOT_R_ALIVE_BONUS": str(config.r_alive_bonus),
        }
