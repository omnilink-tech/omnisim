...
class RobotSpec:
    ...
    trained by populating this. OmniQuad's spec is in `omniquad_robot_spec.py`.
    """
    ...
    recipes (see `obs_recipes.py` / generic deploy controller):
      'legged_locomotion' : 9 + 3*nu + 4 D (OmniQuad/ANYmal/quadrupeds)
    ...
    nominal pose" behavior). For trot, ~1.5-2.0 Hz works well for OmniQuad."""
