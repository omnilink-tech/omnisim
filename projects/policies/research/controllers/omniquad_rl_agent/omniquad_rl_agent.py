...
"""OmniQuad RL agent — OmniSim-side controller for OmniQuadEnv.

This controller binds a TCP server on `--port` and exchanges fixed-size
numpy float32 packets with the OmniQuadEnv gym wrapper. The protocol:
...
OBS_DIM = int(os.environ.get("OMNIQUAD_OBS_DIM", "50"))
ACT_DIM = 12
...
def parse_args() -> argparse.Namespace:
    ...
    # OMNIQUAD_RL_PORT env var overrides --port (so OmniQuadEnv can pick a unique
    # port per worker without editing the world file).
    env_port = os.environ.get("OMNIQUAD_RL_PORT")
    if env_port:
        try:
...
def main() -> int:
    args = parse_args()
    sys.stderr.write(f"[omniquad_rl_agent] resolved port = {args.port} "
                     f"(env OMNIQUAD_RL_PORT={os.environ.get('OMNIQUAD_RL_PORT','<unset>')})\n")
    sys.stderr.flush()
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            sys.stderr.write(f"[omniquad_rl_agent] missing motor {name}\n")
            return 1
        motors.append(m)
        try:
    ...
    if self_node is None:
        sys.stderr.write("[omniquad_rl_agent] no supervisor handle\n")
        return 1
    try:
    ...
    srv.listen(1)
    sys.stderr.write(f"[omniquad_rl_agent] waiting for trainer on {args.host}:{args.port}\n")
    sys.stderr.flush()
    conn, addr = srv.accept()
    conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
    sys.stderr.write(f"[omniquad_rl_agent] trainer connected from {addr}\n")
    sys.stderr.flush()
    ...
    R_LIN_VEL_WT     = _env_float("OMNIQUAD_R_LIN_VEL_WT", 1.0)
    ...
    R_LIN_VEL_SCALE  = _env_float("OMNIQUAD_R_LIN_VEL_SCALE", 0.25)
    R_ANG_VEL_WT     = _env_float("OMNIQUAD_R_ANG_VEL_WT", 0.5)
    R_VZ_WT          = _env_float("OMNIQUAD_R_VZ_WT", -1.0)
    R_ROLLPITCH_WT   = _env_float("OMNIQUAD_R_ROLLPITCH_WT", -0.1)
    R_ANG_XY_WT      = _env_float("OMNIQUAD_R_ANG_XY_WT", -0.02)
    R_ACTION_RATE_WT = _env_float("OMNIQUAD_R_ACTION_RATE_WT", -0.005)
    R_JOINT_ACC_WT   = _env_float("OMNIQUAD_R_JOINT_ACC_WT", -2.5e-7)
    R_ALIVE_BONUS    = _env_float("OMNIQUAD_R_ALIVE_BONUS", 0.05)
    ...
    R_UPRIGHT_WT     = _env_float("OMNIQUAD_R_UPRIGHT_WT", 0.0)
    R_UPRIGHT_SIGMA  = _env_float("OMNIQUAD_R_UPRIGHT_SIGMA", 0.15)
    ...
    R_VX_BONUS_WT    = _env_float("OMNIQUAD_R_VX_BONUS_WT", 0.0)
    R_VX_DEADBAND    = _env_float("OMNIQUAD_R_VX_DEADBAND", 0.05)  # m/s
    ...
    # 0 = uncapped (legacy).
    R_VX_BONUS_CAP   = _env_float("OMNIQUAD_R_VX_BONUS_CAP", 0.0)  # m/s
    ...
    R_CLIP_LO        = _env_float("OMNIQUAD_R_CLIP_LO", -10.0)
    R_CLIP_HI        = _env_float("OMNIQUAD_R_CLIP_HI", +10.0)
    ...
    OMNIQUAD_FIXED_VX    = _env_float("OMNIQUAD_FIXED_VX", float("nan"))
    OMNIQUAD_FIXED_WZ    = _env_float("OMNIQUAD_FIXED_WZ", float("nan"))
    # NEW: CPG trotting prior. Default freq 0 disables the CPG (recovers
    # the legacy "residual on nominal pose" behavior).
    CPG_FREQ_HZ      = _env_float("OMNIQUAD_CPG_FREQ_HZ", 0.0)
    CPG_HIP_Y_AMP    = _env_float("OMNIQUAD_CPG_HIP_Y_AMP", 0.20)
    CPG_KNEE_AMP     = _env_float("OMNIQUAD_CPG_KNEE_AMP", 0.30)
    ...
    ACTION_SCALE_RUN = _env_float("OMNIQUAD_ACTION_SCALE", float(ACTION_SCALE))
    ...
    # is the priority.
    R_TERM_PENALTY   = _env_float("OMNIQUAD_R_TERM_PENALTY", -1.0)
    ...
    R_JOINT_LIMIT_WT = _env_float("OMNIQUAD_R_JOINT_LIMIT_WT", 0.0)
    ...
    R_HEADING_WT     = _env_float("OMNIQUAD_R_HEADING_WT", 0.0)   # rad^2  (try -2.0 .. -6.0)
    R_LATERAL_WT     = _env_float("OMNIQUAD_R_LATERAL_WT", 0.0)   # m^2    (try -1.0 .. -4.0)
    sys.stderr.write(
        f"[omniquad_rl_agent] rewards: lin={R_LIN_VEL_WT} ang={R_ANG_VEL_WT} "
        f"vz={R_VZ_WT} alive={R_ALIVE_BONUS} vx_bonus={R_VX_BONUS_WT} "
        f"heading={R_HEADING_WT} lateral={R_LATERAL_WT} "
        f"clip=[{R_CLIP_LO},{R_CLIP_HI}] fixed_vx={OMNIQUAD_FIXED_VX} fixed_wz={OMNIQUAD_FIXED_WZ} "
    ...
    def reset_episode() -> None:
        ...
        # If the env vars OMNIQUAD_FIXED_VX / OMNIQUAD_FIXED_WZ are finite, the
        # command is pinned to those instead (curriculum-friendly).
        if math.isfinite(OMNIQUAD_FIXED_VX):
            vel_command[0] = OMNIQUAD_FIXED_VX
        else:
            vel_command[0] = 0.2 + 0.6 * np.random.random()
        vel_command[1] = 0.0
        if math.isfinite(OMNIQUAD_FIXED_WZ):
            vel_command[2] = OMNIQUAD_FIXED_WZ
        else:
    ...
    def build_obs_reward_done() -> tuple[np.ndarray, float, bool, dict]:
        ...
        # obs proven insufficient (see project_omniquad_walker_standard).
        if OBS_DIM >= 50:
    ...
    while robot.step(step_ms) != -1:
        ...
        try:
            ...
            sys.stderr.write("[omniquad_rl_agent] trainer disconnected; exiting\n")
            break
        ...
        try:
            ...
            sys.stderr.write("[omniquad_rl_agent] trainer disconnected mid-cmd; exiting\n")
            break
        tag = cmd[0]
        if tag == 2:  # QUIT
            sys.stderr.write("[omniquad_rl_agent] QUIT received\n")
            break
        if tag == 1 or done:  # RESET (or auto-reset on done)
            ...
            if episode_step > 0 and (episode_step >= MAX_EPISODE_STEPS
                ...
                sys.stderr.write(
                    f"[omniquad_rl_agent] ep end: steps={episode_step} "
