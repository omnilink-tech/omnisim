...
"""Kinematic GHOST controller for Spot -- "the ideal we optimise toward".

The Spot port of g1_ghost: drives a SECOND, physics-free Spot (visual-only
URDF, staticBase TRUE) with the PURE trot model reference
(projects/policies/control/gait/spot_trot_gait) -- no RL, no physics. The ghost trots
beside the real robot (DEF SPOT_REAL) with its leg cycle locked to the
real robot's FORWARD PROGRESS; the visible gap between them IS the RL
correction + tracking error.

Reads the same SPOT_GAIT_* env as the real controller so model params
match. Hologram look via SPOT_GHOST_ALPHA / SPOT_GHOST_TINT. With no real
robot (or SPOT_GHOST_SELF_WALK=1) it trots forward across the floor on its
own clock. SPOT_GHOST_LOG=<path> writes per-second telemetry. (Parity with
...
def main() -> int:
    gp = stg.GaitParams(
        vx=_genv("SPOT_GAIT_VX", 0.4),
        freq=_genv("SPOT_GAIT_FREQ", 1.4),
        duty=_genv("SPOT_GAIT_DUTY", 0.6),
        step_height=_genv("SPOT_GAIT_STEP_H", 0.06),
        body_height=_genv("SPOT_GAIT_BODY_H", 0.55),
        x0=_genv("SPOT_GAIT_X0", 0.0),
        ramp_s=_genv("SPOT_GAIT_RAMP_S", 1.0))
    Y_OFFSET = _genv("SPOT_GHOST_Y", 1.4)
    Z_BASE = _genv("SPOT_GHOST_Z", gp.body_height)
    nominal = stg.standing_pose(gp)

    # WALK<->STOP schedule (mirrors spot_walk_deploy.py so the ghost stops
    ...
    WALK_FOR_S = _genv("SPOT_WALK_FOR_S", 0.0)
    STAND_FOR_S = _genv("SPOT_STAND_FOR_S", 5.0)
    STAND_AT_S = _genv("SPOT_STAND_AT_S", 0.0)
    MODE_BLEND_S = _genv("SPOT_MODE_BLEND_S", 1.0)
    # The real controller settles SPOT_SETTLE_S at the standing pose before its
    ...
    SETTLE_S = _genv("SPOT_SETTLE_S", 1.5)
    ...
    for jn in JOINT_ORDER:
        ...
        if m is not None:
            motors[jn] = m
    sys.stderr.write(f"[spot_ghost] {len(motors)}/12 motors found\n")
    ...
    real = None
    try:
        real = robot.getFromDef("SPOT_REAL")
    except Exception:
        real = None
    ...
    # tracking error. The trot model is Spot's feasible-ghost generator (gait is
    ...
    #
    # SPOT_GHOST_TRACK_REAL=1 restores the legacy behavior of pinning the ghost's
    ...
    # comparison only.
    track_real = (os.environ.get("SPOT_GHOST_TRACK_REAL", "") == "1"
    ...
    _glog_path = os.environ.get("SPOT_GHOST_LOG")
    _glog = open(_glog_path, "w", buffering=1) if _glog_path else None

    alpha = _genv("SPOT_GHOST_ALPHA", 0.6)
    if alpha > 0.0:
        tint_s = os.environ.get("SPOT_GHOST_TINT", "0.62,0.82,1.0")
        try:
            tint = tuple(float(x) for x in tint_s.split(","))
        except ValueError:
            tint = (0.62, 0.82, 1.0)
        n_shapes = _ghostify(self_node, alpha, tint)
        sys.stderr.write(f"[spot_ghost] ghostified {n_shapes} shapes "
                         f"(transparency={alpha})\n")
        if _glog is not None:
            _glog.write(f"ghostified={n_shapes} alpha={alpha}\n")

    sys.stderr.write(f"[spot_ghost] ideal-reference trot ghost running "
