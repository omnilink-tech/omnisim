...
runs OmniQuad, ANYmal, a manipulator, anything with a `robot_spec.json` +
`policy.onnx` pair.
...
class _RobotSpecLite:
    ...
    def __init__(self, d):
        ...
        # actuator instead of the scalar motor_pid. Heavy robots use this
        # to clamp arms+back+neck at stiff hold so deploy matches training.
...
def _cpg_offsets(t, n_act, joint_names, freq_hz, hip_y_amp, knee_amp, leg_phase):
    ...
    omniquad_rl_agent.cpg_base_offsets so the policy sees the same
...
def _read_velocity_command():
    vx = float(os.environ.get("OMNISIM_VX") or os.environ.get("OMNIQUAD_VX", 0.5))
...
def main() -> int:
    ...
    # nondeterministic order; one thread keeps the same checkpoint bit-stable
    # across runs and machines at zero cost for a net this size.
    ...
    trace_env = (os.environ.get("OMNISIM_DEPLOY_TRACE")
                 or os.environ.get("OMNIQUAD_DEPLOY_TRACE"))
