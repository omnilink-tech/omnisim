...
"""Kinematic GHOST for the Spot GET-UP -- "the ideal the robot shadows".

A physics-free translucent Spot (spot_ghost.urdf, staticBase) that REPLAYS the
MPPI-generated, verifier-certified get-up reference (shadowing/ghosts/
spot_getup_ghost.npz) beside the real RL-stabilised robot (DEF SPOT_REAL). The
...
real controller's schedule via SPOT_SETTLE_S, and (like the real deploy) ramps to
the crisp trot stand after the get-up ends. Hologram look: SPOT_GHOST_ALPHA/TINT.
"""
...
from projects.policies.control.gait import spot_trot_gait as _stg  # noqa: E402
...
def _ghostify(node, transparency, tint, depth=0):
    """Make every Shape under `node` translucent + tinted (spot_ghost recipe:
...
def main():
    ...
    SETTLE_S = _genv("SPOT_SETTLE_S", 1.0)
    FADE_S = _genv("SPOT_GETUP_HOLD_FADE_S", 1.2)
    ...
    for jn in JOINT_ORDER:
        ...
        if m is not None:
            motors[jn] = m
    sys.stderr.write(f"[spot_getup_ghost] {len(motors)}/12 motors, ref {N} steps\n")
    ...
    if alpha > 0.0:
        try:
            tint = tuple(float(x) for x in
                         os.environ.get("SPOT_GHOST_TINT", "0.62,0.82,1.0").split(","))
        except ValueError:
            tint = (0.62, 0.82, 1.0)
        ns = _ghostify(self_node, alpha, tint)
        sys.stderr.write(f"[spot_getup_ghost] ghostified {ns} shapes\n")
