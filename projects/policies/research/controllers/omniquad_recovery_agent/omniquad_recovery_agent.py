...
"""OmniQuad RL recovery-training agent — OmniSim-side TCP controller.

Trains a residual policy on top of the orientation-aware scripted
righting maneuver (`projects/policies/control/omniquad_recovery.py`). Episode
...
Protocol mirrors omniquad_residual_agent: fixed-size float32 packets,
...
def main() -> int:
    args = parse_args()
    sys.stderr.write(f"[omniquad_recovery_agent] resolved port = {args.port}\n")
    sys.stderr.flush()
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            sys.stderr.write(f"[omniquad_recovery_agent] missing motor {name}\n")
            return 1
        try:
    ...
    apply_realistic_limits(motors,
                           max_torque_nm=_env_float("OMNIQUAD_MAX_TORQUE_NM", 30.0),
                           max_vel_rad_s=_env_float("OMNIQUAD_MAX_VEL_RAD_S", 6.0))
    motor_bank = RateLimitedMotorBank(
        motors, step_dt,
        max_vel_rad_s=_env_float("OMNIQUAD_TARGET_RATE_RAD_S", 4.0))

    self_node = robot.getSelf()
    if self_node is None:
        sys.stderr.write("[omniquad_recovery_agent] no supervisor handle\n")
        return 1
    ...
    R_UPRIGHT_BONUS  = _env_float("OMNIQUAD_REC_UPRIGHT_BONUS", 500.0)
    R_PROGRESS_WT    = _env_float("OMNIQUAD_REC_PROGRESS_WT", 0.0)
    R_GRAV_WT        = _env_float("OMNIQUAD_REC_GRAV_WT", 0.02)
    R_LEVEL_WT       = _env_float("OMNIQUAD_REC_LEVEL_WT", 0.0)
    R_ACTION_WT      = _env_float("OMNIQUAD_REC_ACTION_WT", -0.001)
    R_ALIVE          = _env_float("OMNIQUAD_REC_ALIVE", -0.50)
    R_CLIP_LO        = _env_float("OMNIQUAD_REC_CLIP_LO", -1.0)
    R_CLIP_HI        = _env_float("OMNIQUAD_REC_CLIP_HI", 1.0)
    # Curriculum env vars — restrict episode initial orientations.
    # OMNIQUAD_REC_INIT can be a comma-separated subset of
    # {face_plant, left_side, right_side, on_back}; default = all.
    init_set_env = os.environ.get(
        "OMNIQUAD_REC_INIT", "face_plant,left_side,right_side,on_back")
    INIT_SET = [s.strip() for s in init_set_env.split(",") if s.strip()]

    BZ_UPRIGHT = 0.55
    ROLL_UPRIGHT = 0.30
    PITCH_UPRIGHT = 0.30
    MAX_EPISODE_STEPS = int(_env_float("OMNIQUAD_REC_MAX_STEPS", 400))
    ...
    srv.listen(1)
    sys.stderr.write(f"[omniquad_recovery_agent] waiting on {args.host}:{args.port}\n")
    sys.stderr.flush()
    conn, addr = srv.accept()
    conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
    sys.stderr.write(f"[omniquad_recovery_agent] trainer connected from {addr}\n")
    sys.stderr.flush()
    ...
    def random_fallen_pose():
        ...
        initial pose. Mix is determined by the OMNIQUAD_REC_INIT
    ...
    while robot.step(step_ms) != -1:
        ...
        try:
            ...
            sys.stderr.write("[omniquad_recovery_agent] trainer disconnected\n")
            break
