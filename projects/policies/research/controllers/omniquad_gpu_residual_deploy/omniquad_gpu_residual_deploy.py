...
"""OmniQuad deploy controller for the GPU mjwarp residual policy.
...
ONNX exported from that trainer can be run in OmniSim:
  ...
  * ±0.15 rad JOINT-SPACE residual on top of gait→IK joint
    ...
    `omniquad_residual_deploy` controller uses.
...
def main() -> int:
    ...
    say(f"[omniquad_gpu_residual_deploy] policy: {policy_path}\n")
    sess = None
    if policy_path.exists():
        try:
            ...
            say("[omniquad_gpu_residual_deploy] ONNX loaded\n")
    ...
    gait = GaitParams(
        neutral_front_x=_env_float("OMNIQUAD_GAIT_FRONT_X", 0.322),
        neutral_rear_x=_env_float("OMNIQUAD_GAIT_REAR_X", -0.274),
        neutral_lateral_y=_env_float("OMNIQUAD_GAIT_LATERAL_Y", 0.344),
        ground_z=_env_float("OMNIQUAD_GAIT_GROUND_Z", -0.57),
        step_height=_env_float("OMNIQUAD_GAIT_STEP_HEIGHT", 0.06))
    vx = _env_float("OMNIQUAD_VX", 0.5)
    vy = _env_float("OMNIQUAD_VY", 0.0)
    wz = _env_float("OMNIQUAD_WZ", 0.0)
    say(f"[omniquad_gpu_residual_deploy] command vx={vx} vy={vy} wz={wz}\n")
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            say(f"[omniquad_gpu_residual_deploy] missing motor {name}\n")
            return 1
        try:
    ...
    # Re-enable with OMNIQUAD_TARGET_RATE_RAD_S for hardware-realism studies.
    ...
    if os.environ.get("OMNIQUAD_DEPLOY_TRACE") or os.environ.get(
        ...
        trace_path = Path(r"C:\tmp\husky_trace\omniquad_gpu_deploy.csv")
        trace_path.parent.mkdir(parents=True, exist_ok=True)
        trace_file = open(trace_path, "w", buffering=1)
        trace_file.write("t_ms,bx,by,bz,roll,pitch,yaw,vx\n")
        say(f"[omniquad_gpu_residual_deploy] trace -> {trace_path}\n")
    ...
    # active when the user command wz is 0. OMNIQUAD_HEADING_HOLD=0 disables.
    ...
    while robot.step(step_ms) != -1:
        ...
        if os.environ.get("OMNIQUAD_GPU_OBS_DEBUG") and sim_ms % 320 == 0:
        ...
        if sess is not None:
            try:
                ...
                say(f"[omniquad_gpu_residual_deploy] inference failed: {e}\n")
