...
"""omniquad_getup_mimic -- the real OmniSim/Newton OmniQuad SHADOWING its MPPI get-up ghost,
...
can't hold open-loop, so the RL residual (gpu_mjwarp_omniquad_getup_trainer) supplies the
...
The obs MIRRORS gpu_mjwarp_omniquad_getup_trainer._build_obs_t EXACTLY (49-dim):
  [vlin_world(3), vang_BODY(3), proj_g(3), q-ref_legs(12), qd(12), last_action(12),
   phase(4)=[b, b_ahead(+0.2s), z_err, pitch_err]]

Env: OMNIQUAD_GETUP_REF / OMNIQUAD_GETUP_POLICY / OMNIQUAD_GETUP_RES_SCALE / OMNIQUAD_GETUP_LOG /
     OMNIQUAD_SETTLE_S (lying-settle seconds) / OMNIQUAD_GETUP_LOOP.
"""
...
from projects.policies.control.gait import omniquad_trot_gait as _stg  # noqa: E402  (proven-stable stand)

_DBG = str(Path(__file__).resolve().parent / "omniquad_getup_mimic.dbg")
...
def main():
    ...
    sys.stderr.write(f"[omniquad_getup_mimic] ghost {ref_path}: {N} steps, base_z "
    ...
    for leg, j in JOINT_ORDER:
        ...
        if m is None:
            sys.stderr.write(f"[omniquad_getup_mimic] MISSING motor {leg}_{j}_motor\n")
            return 1
        motors[(leg, j)] = m
    ...
    res_scale = _envf("OMNIQUAD_GETUP_RES_SCALE", 0.15)
    loop = os.environ.get("OMNIQUAD_GETUP_LOOP", "0") != "0"
    settle = max(1, int(_envf("OMNIQUAD_SETTLE_S", 1.5) / step_dt))

    sess = None
    pol = os.environ.get("OMNIQUAD_GETUP_POLICY")
    if pol and os.path.exists(pol):
        try:
            import onnxruntime as ort
            sess = ort.InferenceSession(pol, providers=["CPUExecutionProvider"])
            sys.stderr.write(f"[omniquad_getup_mimic] RL policy: {pol}\n")
        except Exception as e:
            sys.stderr.write(f"[omniquad_getup_mimic] FATAL: policy EXISTS but failed to load ({e}) -- refusing to run the bare feedforward and report it as a policy result\n")
        ...
        sys.stderr.write("[omniquad_getup_mimic] FEEDFORWARD (no policy) -- expect it to sag\n")
    ...
    log_f = None
    log_path = os.environ.get("OMNIQUAD_GETUP_LOG")
    if log_path:
        try:
    ...
    # OMNIQUAD_GETUP_HOLD_FADE_S: the reference's final pose IS the stable trot stand
    ...
    hold_fade_s = _envf("OMNIQUAD_GETUP_HOLD_FADE_S", 1.0)
    ...
    while robot.step(step_ms) != -1:
        ...
        if sim_ms >= next_log:
            ...
            sys.stderr.write(f"[omniquad_getup_mimic] t={sim_ms/1000:4.1f}s [{ph:6s}] "
