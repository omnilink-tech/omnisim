...
"""Spot RL deploy controller — loads an ONNX policy and runs it.

A drop-in replacement for spot_simple_pose / spot_rl_agent that:
  - reads the ONNX policy from a path passed via env var SPOT_POLICY_ONNX
    (default: projects/policies/research/inference/policies/spot_ppo_main/policy.onnx)
...
ACTION_SCALE = 0.15  # must match the value in spot_rl_agent.py at train time

_DEPLOY_LOOP = bool(os.environ.get("OMNISIM_DEPLOY_LOOP") or os.environ.get("SPOT_DEPLOY_LOOP"))
...
def _pitch_trim_vec(pitch_trim: float) -> np.ndarray:
    ...
    wrong and Spot tips. Resolved from the sidecar/env in main()."""
...
# When SPOT_CPG_FREQ_HZ > 0, joint target becomes:
...
# (spot_walk_v12, spot_newton_v*, ...) was TRAINED against -- it must stay
...
_GAIT_MODE = (os.environ.get("OMNISIM_CPG_GAIT") or os.environ.get("SPOT_CPG_GAIT")
...
def find_policy_path() -> Path:
    ...
    candidates = []
    # Webots' batch mode drops SPOT_-prefixed env vars before they reach
    # the controller subprocess for some --batch/--mode=fast combos
    # (same failure that hit SPOT_DEPLOY_TRACE). OMNISIM_-prefixed vars
    # survive, so check both -- without the OMNISIM_ fallback every
    # headless eval silently loaded the spot_ppo_main default instead of
    # the policy under test, making four different trained policies all
    # produce byte-identical "falls at 0.688 s" traces.
    env_path = os.environ.get("SPOT_POLICY_ONNX") or os.environ.get("OMNISIM_POLICY_ONNX")
    if env_path:
    ...
    # Default walker: spot_walk_v12_200k is the verified-stable ODE walker
    # (upright 100%, never falls, trots ~0.5 m/s) -- the current shipped
    # Spot solution. Tried before the legacy spot_ppo_main slot so the
    # deploy world "just works" with no env var.
    candidates.append(repo_root / "projects" / "rl" / "inference" / "policies" / "spot_walk_v12_200k" / "policy.onnx")
    # Legacy canonical slot -- eval_policy.py copies here.
    candidates.append(repo_root / "projects" / "rl" / "inference" / "policies" / "spot_ppo_main" / "policy.onnx")
...
def _load_sidecar(policy_path: Path) -> dict:
    ...
    try:
        ...
        sys.stderr.write(f"[spot_rl_deploy] sidecar parse failed ({_e}); using legacy defaults\n")
        return {}
...
def main() -> int:
    ...
    # SPOT_DEPLOY_LOG is dropped by Webots batch mode; OMNISIM_DEPLOY_LOG
    # survives. Check both so the side log captures reliably in headless
    # --batch runs (where SPOT_-prefixed env vars never reach us).
    _side_log_path = os.environ.get("SPOT_DEPLOY_LOG") or os.environ.get("OMNISIM_DEPLOY_LOG")
    _side_log = None
    if _side_log_path:
        try:
    ...
    def _say(msg: str) -> None:
        ...
        if _side_log is not None:
            try:
                ...
                pass
    _say("[spot_rl_deploy] starting\n")

    policy_path = find_policy_path()
    _say(f"[spot_rl_deploy] policy: {policy_path}\n")
    if not policy_path.exists():
        sys.stderr.write(f"[spot_rl_deploy] ERROR: policy not found at {policy_path}\n")
        sys.stderr.write("  Set SPOT_POLICY_ONNX env var, or export a policy via\n")
    ...
    _pt_env = os.environ.get("SPOT_PITCH_TRIM") or os.environ.get("OMNISIM_PITCH_TRIM")
    ...
    _say(f"[spot_rl_deploy] pitch_trim={pitch_trim} (sidecar+env)\n")
    # SPOT_KP env var overrides the sidecar kp. Used by the Newton demo
    # where the actuator is softer than ODE's internal PD and the
    # sidecar's ODE-tuned kp=20 leaves the legs collapsing under
    # gravity. Setting SPOT_KP=200 (10x stiffer) gives Newton's
    # POSITION_VELOCITY actuator enough force authority to hold the
    # standing pose against the chassis weight.
    # SPOT_KP is dropped by Webots batch mode; OMNISIM_KP survives. Check
    ...
    _kp_override = os.environ.get("SPOT_KP") or os.environ.get("OMNISIM_KP")
    if _kp_override:
        try:
            ...
            pass
    _say(f"[spot_rl_deploy] sidecar motor_pid={motor_pid_arg} "
    ...
    try:
        ...
        if _detected in (49, 50):
            OBS_DIM = _detected
            _say(f"[spot_rl_deploy] policy obs_dim auto-detected = {OBS_DIM}\n")
    except Exception:
        pass
    _say(f"[spot_rl_deploy] loaded policy from {policy_path}\n")

    robot = _Robot()
    step_ms = int(robot.getBasicTimeStep())
    step_dt = step_ms / 1000.0
    _say(f"[spot_rl_deploy] robot ok, step_ms={step_ms}\n")
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            sys.stderr.write(f"[spot_rl_deploy] missing motor {name}\n")
            return 1
        motors.append(m)
        try:
    ...
    if self_node is None:
        sys.stderr.write("[spot_rl_deploy] no supervisor handle\n")
        return 1

    _say("[spot_rl_deploy] motors/sensors registered, applying nominal pose\n")
    ...
    for _ in range(20):
        if robot.step(step_ms) == -1:
            settled_ok = False
            break
    _say(f"[spot_rl_deploy] settle complete (ok={settled_ok})\n")
    ...
    # SPOT_-prefixed env vars are dropped by Webots batch mode before they
    ...
    def _cpg(spot_key, omni_key, sidecar_key):
        v = os.environ.get(spot_key) or os.environ.get(omni_key)
    ...
    cpg_freq = _cpg("SPOT_CPG_FREQ_HZ", "OMNISIM_CPG_FREQ_HZ", "cpg_freq_hz")
    cpg_hip_amp = _cpg("SPOT_CPG_HIP_Y_AMP", "OMNISIM_CPG_HIP_Y_AMP", "cpg_hip_y_amp")
    cpg_knee_amp = _cpg("SPOT_CPG_KNEE_AMP", "OMNISIM_CPG_KNEE_AMP", "cpg_knee_amp")
    _say(
        f"[spot_rl_deploy] cpg: freq={cpg_freq}Hz hip={cpg_hip_amp} "
    ...
    # mode, SPOT_ is convenient from a shell.
    _go = os.environ.get("OMNISIM_GAIT_ONLY") or os.environ.get("SPOT_GAIT_ONLY")
    gait_only = bool(_go) and _go.strip() not in ("", "0", "false", "False")
    if gait_only:
        _say("[spot_rl_deploy] GAIT-ONLY mode: policy action zeroed\n")
    ...
    # vx, vy, wz.
    vx = float(os.environ.get("SPOT_VX", "0.5"))
    vy = float(os.environ.get("SPOT_VY", "0.0"))
    wz = float(os.environ.get("SPOT_WZ", "0.0"))
    try:
        repo_root = next(_p for _p in Path(__file__).resolve().parents if (_p / "projects" / "policies").is_dir() or (_p / "AGENTS.md").exists() or (_p / ".git").exists())
    ...
    _say(f"[spot_rl_deploy] vel command = ({vx:.2f}, {vy:.2f}, {wz:.2f})\n")
    sim_time = 0.0
    sim_ms = 0

    # Optional body trace at 10 Hz, opt-in via SPOT_DEPLOY_TRACE env var
    # (or OMNISIM_DEPLOY_TRACE -- the Newton-demo harness sets that one
    # because Webots' batch mode drops SPOT_-prefixed vars on the floor
    ...
    if os.environ.get("SPOT_DEPLOY_TRACE") or os.environ.get("OMNISIM_DEPLOY_TRACE"):
        try:
            ...
            _say("[spot_rl_deploy] trace opened\n")
        except Exception as _e:
            _say(f"[spot_rl_deploy] trace open failed: {_e}\n")
            trace = None
    else:
        _say("[spot_rl_deploy] SPOT_DEPLOY_TRACE unset, trace disabled\n")
    next_trace_ms = 0
    ...
    if _snap:
        try:
            os.makedirs(r"C:\tmp\spot_snap", exist_ok=True)
            _say("[spot_rl_deploy] frame snapshots ON -> C:\\tmp\\spot_snap\n")
        except Exception:
            _snap = False
    ...
    if os.environ.get("OMNISIM_DEPLOY_LINKLOG"):
        ...
        try:
            ...
            _linklog = open(r"C:\tmp\spot_links.csv", "w", buffering=1)
            hdr = "t_ms," + ",".join(
                f"{(n.getDef() or 'L%d' % i)}_{ax}" for i, n in enumerate(_link_nodes) for ax in "xyz")
            _linklog.write(hdr + "\n")
            _say(f"[spot_rl_deploy] LINKLOG on: {len(_link_nodes)} solids tracked\n")
        except Exception as _le:
            _say(f"[spot_rl_deploy] linklog setup failed: {_le}\n")
            _linklog = None
    _linklog_next_ms = 0
    ...
    _say("[spot_rl_deploy] entering main loop\n")
    _dbg_step = 0
    prev_body_pos = None
    prev_body_R = None
    ...
    # [-0.3, +0.3]) and steers back to straight. Set SPOT_HEADING_LOCK=0 to
    # disable and let the user-supplied wz command pass through unchanged.
    _heading_lock = (os.environ.get("SPOT_HEADING_LOCK", "1").strip() != "0")
    _heading_kp = float(os.environ.get("SPOT_HEADING_KP", "1.0"))
    _heading_clip = float(os.environ.get("SPOT_HEADING_CLIP", "0.25"))
    ...
    while robot.step(step_ms) != -1:
        ...
        if _snap and sim_ms >= _snap_next_ms:
            try:
                robot.exportImage(rf"C:\tmp\spot_snap\f{_snap_n:04d}.jpg", 80)
                _snap_n += 1
                _snap_next_ms = sim_ms + _snap_period
            except Exception as _se:
                _say(f"[spot_rl_deploy] exportImage failed: {_se}\n")
                _snap = False
        ...
        if _DEPLOY_LOOP:
            ...
            if abs(_roll) > 1.0 or abs(_pitch) > 1.0 or pos[2] < 0.30:
                ...
                _say("[spot_rl_deploy] fell -> reset (demo loop)\n")
                continue
        ...
        if _DEPLOY_DBG and _dbg_step < 600 and _dbg_step % 10 == 0:
            ...
            try:
                with open(r"C:\tmp\spot_dbg_steps.txt", "a") as _df:
