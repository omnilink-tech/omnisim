...
"""Spot WALK deploy controller -- the G1 walk-deploy recipe ported.

Runs the foot-space TROT MODEL (projects/policies/control/gait/spot_trot_gait.py) as the
baseline and adds the ONNX residual trained by
gpu_mjwarp_spot_walk_trainer.py. The obs/action contract mirrors that
trainer EXACTLY:
  ...
  * HEADING HOLD through the command channel: the trainer steers the
    ...
    (SPOT_HEADING_HOLD=0 disables).
  * +-SPOT_ACT_SCALE rad joint-space residual on the model targets.
...
Gait params are env-configurable (SPOT_GAIT_*) so one controller deploys
any policy from that trainer -- set them to the TRAINING values.

Deploy env (set by scripts/dev/run_spot_walk_deploy.ps1):
...
# Gait reference: trot by default; SPOT_GAIT_MODULE selects another (e.g.
# projects.policies.control.gait.spot_crawl_gait for the hill crawl) -- MUST match training.
_GM = os.environ.get("SPOT_GAIT_MODULE")
if _GM:
    import importlib
...
def find_policy_path() -> Path:
    ...
    return (_REPO / "projects" / "rl" / "inference" / "policies"
            / "gpu_spot_walk_main" / "policy.onnx")
...
def main() -> int:
    ...
    gp = stg.GaitParams(
        vx=_env_float("SPOT_GAIT_VX", 0.4),
        freq=_env_float("SPOT_GAIT_FREQ", 1.4),
        duty=_env_float("SPOT_GAIT_DUTY", 0.6),
        step_height=_env_float("SPOT_GAIT_STEP_H", 0.06),
        body_height=_env_float("SPOT_GAIT_BODY_H", 0.55),
        x0=_env_float("SPOT_GAIT_X0", 0.0),
        ramp_s=_env_float("SPOT_GAIT_RAMP_S", 1.0))
    res_scale = _env_float("SPOT_ACT_SCALE", 0.15)
    ...
    hill_la = _env_float("SPOT_HILL_LOOKAHEAD", 0.0)
    _hill = os.environ.get("SPOT_HILL_GHOST")
    if _hill and os.path.exists(_hill):
        ...
        sys.stderr.write(f"[spot_walk_deploy] HILL obs ON from {_hill} "
                         f"(crest z {hill_bz.max():.2f})\n")
    say(f"[spot_walk_deploy] gait vx={gp.vx} freq={gp.freq} duty={gp.duty} "
    ...
    if policy_path.exists():
        try:
            ...
            say(f"[spot_walk_deploy] ONNX loaded: {policy_path}\n")
        except Exception as e:
            say(f"[spot_walk_deploy] ONNX load failed ({e}); ZERO residual\n")
    else:
        say(f"[spot_walk_deploy] policy not found at {policy_path}; "
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            say(f"[spot_walk_deploy] missing motor {leg}_{joint}_motor\n")
            return 1
        motors.append(m)
        try:
    ...
    wz_user = _env_float("SPOT_WZ", 0.0)
    ...
    HOLD_WZ_MAX = _env_float("SPOT_HOLD_WZ_MAX", 0.3)
    yaw_ref = None
    lat_ref = 0.0
    wz_cmd = wz_user
    ...
    #   settle  -> walk SPOT_WALK_FOR_S -> stand SPOT_STAND_FOR_S -> repeat,
    #   _w ramped over SPOT_MODE_BLEND_S so steps decelerate/accelerate.
    # A single-stop window (G1-style) is available via SPOT_STAND_AT_S>0.
    VX_NOMINAL = _env_float("SPOT_GAIT_VX", 0.4)
    VX_CMD_MAX = _env_float("SPOT_VX_CMD_MAX", 0.0)      # >0 = VC policy (obs+1)
    WALK_FOR_S = _env_float("SPOT_WALK_FOR_S", 0.0)      # 0 = no repeating cycle
    STAND_FOR_S = _env_float("SPOT_STAND_FOR_S", 5.0)
    STAND_AT_S = _env_float("SPOT_STAND_AT_S", 0.0)      # 0 = no single window
    MODE_BLEND_S = _env_float("SPOT_MODE_BLEND_S", 1.0)
    _w = 1.0
    ...
    while robot.step(step_ms) != -1:
        ...
        obs = np.clip(np.nan_to_num(obs, nan=0.0, posinf=10.0, neginf=-10.0),
                      -10.0, 10.0)
        if os.environ.get("SPOT_OBS_DEBUG") and sim_ms % 320 == 0:
        ...
        if sess is not None:
            try:
                ...
                say(f"[spot_walk_deploy] inference failed: {e}\n")
