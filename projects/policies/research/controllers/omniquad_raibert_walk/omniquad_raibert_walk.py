...
"""OmniQuad Raibert-trot controller — the MODELED gait base layer.

Drives the velocity-feedback trot from projects/policies/control/omniquad_raibert.py:
...
Env knobs:
  OMNIQUAD_VX / OMNIQUAD_VY / OMNIQUAD_WZ      commanded body velocity (default 0.4/0/0)
  OMNIQUAD_RAIBERT_FREQ_HZ             trot cadence (default 2.0)
  OMNIQUAD_RAIBERT_H0                  body height over feet (default 0.52)
  OMNIQUAD_RAIBERT_STEP_H              swing apex (default 0.07)
  OMNIQUAD_RAIBERT_KV                  placement velocity gain (default derived)
  OMNIQUAD_HEADING_LOCK=1              hold spawn heading via wz placement
  OMNIQUAD_HEADING_KP / _CLIP / _LAT2YAW   lock gains (1.0 / 0.3 / 0.1)
  OMNIQUAD_DEPLOY_TRACE=1              chassis trace CSV for verify_straight_walk
  OMNIQUAD_DEPLOY_LOG=<path>           mirror stderr
"""
...
from projects.policies.control.omniquad_raibert import (  # noqa: E402
...
def main() -> int:
    ...
    vx = _env_float("OMNIQUAD_VX", 0.4)
    vy = _env_float("OMNIQUAD_VY", 0.0)
    wz = _env_float("OMNIQUAD_WZ", 0.0)

    params = RaibertParams(
        freq_hz=_env_float("OMNIQUAD_RAIBERT_FREQ_HZ", 2.0),
        duty=_env_float("OMNIQUAD_RAIBERT_DUTY", 0.5),
        h0=_env_float("OMNIQUAD_RAIBERT_H0", 0.52),
        step_height=_env_float("OMNIQUAD_RAIBERT_STEP_H", 0.07),
        k_v=_env_float("OMNIQUAD_RAIBERT_KV", 0.0),
        k_height=_env_float("OMNIQUAD_RAIBERT_KH", 0.6),
        k_level=_env_float("OMNIQUAD_RAIBERT_KL", 0.5),
        lead_s=_env_float("OMNIQUAD_RAIBERT_LEAD", 0.12),
    )
    gait = RaibertGait(params)

    heading_lock = (os.environ.get("OMNIQUAD_HEADING_LOCK", "1").strip() != "0")
    heading_kp = _env_float("OMNIQUAD_HEADING_KP", 1.0)
    heading_clip = _env_float("OMNIQUAD_HEADING_CLIP", 0.3)
    heading_lat2yaw = _env_float("OMNIQUAD_HEADING_LAT2YAW", 0.1)

    say(f"[omniquad_raibert_walk] cmd vx={vx} vy={vy} wz={wz} "
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            say(f"[omniquad_raibert_walk] missing motor {leg}_{joint}_motor\n")
            return 1
        motors.append(m)
        try:
    ...
    while robot.step(step_ms) != -1:
        ...
        # OMNIQUAD_RAIBERT_TAU must equal the deployed KD/KE ratio.
        ...
        tau = _env_float("OMNIQUAD_RAIBERT_TAU", 0.12)
        tau_stance = _env_float("OMNIQUAD_RAIBERT_TAU_STANCE", tau)
