...
"""omniquad_crouch_mimic -- the real, OmniSim/Newton OmniQuad SHADOWING its MPPI-generated
crouch-recover ghost (shadowing-pipeline Stage D / deploy).

The ghost (projects/policies/research/shadowing/ghosts/omniquad_crouch_ghost.npz) was DISCOVERED by the
MPPI ghost_generator over OmniQuad's real dynamics and CERTIFIED dynamically feasible
(open-loop sustainable) by ghost_verifier -- see projects/policies/research/tools/generate_omniquad_crouch.py.
This controller replays that certified, feasible POSITION-TARGET trajectory on the
URDF-imported OmniQuad in OmniSim Newton, so the robot physically shadows it:
...
should already track it. OMNIQUAD_CROUCH_POLICY=<onnx> adds an RL residual (Component 3) for
robustness / domain-gap; the obs then mirrors a tracking trainer.

Env:
  OMNIQUAD_CROUCH_REF=<npz>     ghost (default: shadowing/ghosts/omniquad_crouch_ghost.npz)
  OMNIQUAD_CROUCH_POLICY=<onnx> optional RL residual (else feedforward)
  OMNIQUAD_CROUCH_RES_SCALE     residual authority (default 0.15)
  OMNIQUAD_CROUCH_LOOP=1        repeat the crouch cycle (default: play once then hold)
  OMNIQUAD_CROUCH_LOG=<csv>     per-second telemetry (t, base_z, ref_z, tilt, joint_sim)
  OMNIQUAD_SETTLE_S             settle seconds at the stand before the motion (default 1.0)
"""
...
def main():
    ...
    ref_path = os.environ.get("OMNIQUAD_CROUCH_REF") or str(
        _REPO / "projects/policies/research/shadowing/ghosts/omniquad_crouch_ghost.npz")
    ...
    sys.stderr.write(f"[omniquad_crouch_mimic] ghost {ref_path} -- {n_ref} steps @ "
    ...
    for leg, j in JOINT_ORDER:
        ...
        if m is None:
            sys.stderr.write(f"[omniquad_crouch_mimic] MISSING motor {leg}_{j}_motor\n")
            return 1
        motors[(leg, j)] = m
        try:
    ...
    res_scale = _envf("OMNIQUAD_CROUCH_RES_SCALE", 0.15)
    loop = os.environ.get("OMNIQUAD_CROUCH_LOOP", "0") != "0"
    settle = max(1, int(_envf("OMNIQUAD_SETTLE_S", 1.0) / step_dt))

    # Optional RL residual (Component 3). Feedforward when absent.
    sess = None
    pol = os.environ.get("OMNIQUAD_CROUCH_POLICY")
    if pol and os.path.exists(pol):
        try:
            import onnxruntime as ort
            sess = ort.InferenceSession(pol, providers=["CPUExecutionProvider"])
            sys.stderr.write(f"[omniquad_crouch_mimic] RL residual: {pol}\n")
        except Exception as e:
            sys.stderr.write(f"[omniquad_crouch_mimic] FATAL: policy EXISTS but failed to load ({e}) -- refusing to run the bare FEEDFORWARD and report it as a policy result\n")
        ...
        sys.stderr.write("[omniquad_crouch_mimic] FEEDFORWARD (no policy) -- shadowing the "
    ...
    log_f = None
    log_path = os.environ.get("OMNIQUAD_CROUCH_LOG")
    if log_path:
        try:
    ...
    while robot.step(step_ms) != -1:
        ...
        if self_node is not None and sim_ms >= next_log:
            ...
            sys.stderr.write(f"[omniquad_crouch_mimic] t={sim_ms/1000:4.1f}s [{phase:6s}] "
