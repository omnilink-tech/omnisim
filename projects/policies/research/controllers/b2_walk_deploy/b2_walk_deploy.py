...
"""Unitree B2 WALK deploy controller -- the OmniQuad walk-deploy recipe ported.

Runs the foot-space TROT MODEL (projects/policies/control/gait/b2_trot_gait.py) as the
baseline and adds the ONNX residual trained by
gpu_mjwarp_b2_walk_trainer.py. The obs/action contract mirrors that
trainer EXACTLY:
  ...
  * HEADING HOLD through the command channel (B2_HEADING_HOLD=0 disables).
  * +-B2_ACT_SCALE rad joint-space residual on the model targets.
...
B2 motors are imported as "<joint>_motor" with joint names
FL/FR/RL/RR_{hip,thigh,calf}_joint. Gait params are env-configurable
(B2_GAIT_*) so one controller deploys any policy from that trainer -- set
them to the TRAINING values.

Deploy env (set by scripts/dev/run_b2_walk_deploy.ps1):
    OMNISIM_NEWTON_TARGET_KE=1400 KD=35 <-> trainer MJCF kp/kv (MATCHED)
    OMNISIM_NEWTON_SUBSTEPS=8           <-> trainer 8 x 2 ms decimation
    OMNISIM_NEWTON_MJWARP=1             <-> trainer engine
(This block previously read "KE=80 KD=2.0 (MATCHED)" -- copy-pasted from go2 and
WRONG. The gains are baked into research/training/mjcf/b2_newton.xml: kp=1400/kv=35,
which run_b2_walk_deploy.ps1 + run_quad_rough_track.ps1 both correctly set.)
"""
...
# Gait reference: trot by default; B2_GAIT_MODULE selects another (e.g.
# projects.policies.control.gait.b2_crawl_gait for the statically-stable hill crawl) -- MUST
# match the policy's training --gait-module.
_GM = os.environ.get("B2_GAIT_MODULE")
if _GM:
    import importlib
...
# B2 URDF joint names (controller order FL,FR,RL,RR x hip,thigh,calf). The
...
ACT_DIM = 12
NJ = 12
# Match gpu_mjwarp_b2_walk_trainer JOINT_LIMITS (B2 MJCF ranges).
...
def find_policy_path() -> Path:
    p = os.environ.get("B2_POLICY_ONNX") or os.environ.get(
        "OMNISIM_POLICY_ONNX")
    if p:
        return Path(p)
    return (_REPO / "projects" / "rl" / "inference" / "policies"
            / "gpu_b2_walk_main" / "policy.onnx")


def main() -> int:
    side_log_path = os.environ.get("B2_DEPLOY_LOG") or os.environ.get(
        "OMNISIM_DEPLOY_LOG")
    ...
    gp = stg.GaitParams(
        vx=_env_float("B2_GAIT_VX", 0.5),
        freq=_env_float("B2_GAIT_FREQ", 1.3),
        duty=_env_float("B2_GAIT_DUTY", 0.6),
        step_height=_env_float("B2_GAIT_STEP_H", 0.08),
        body_height=_env_float("B2_GAIT_BODY_H", 0.50),
        x0=_env_float("B2_GAIT_X0", 0.0),
        ramp_s=_env_float("B2_GAIT_RAMP_S", 1.0))
    res_scale = _env_float("B2_ACT_SCALE", 0.15)
    nominal = stg.standing_pose(gp).astype(np.float32)
    say(f"[b2_walk_deploy] gait vx={gp.vx} freq={gp.freq} duty={gp.duty} "
    ...
    # When B2_HILL_GHOST points at a hill ghost npz, the obs gains 2 dims --
    # target base pitch + height error vs the ghost at the robot's forward x.
    hill_bx = hill_bz = hill_bp = None
    hill_la = _env_float("B2_HILL_LOOKAHEAD", 0.0)   # MUST match training --hill-lookahead
    _hill = os.environ.get("B2_HILL_GHOST")
    if _hill and os.path.exists(_hill):
        ...
        say(f"[b2_walk_deploy] HILL obs ON from {_hill} "
            f"(crest z {hill_bz.max():.2f}, |pitch|max {np.degrees(np.abs(hill_bp).max()):.0f}deg)\n")
    ...
    if policy_path.exists():
        try:
            ...
            say(f"[b2_walk_deploy] ONNX loaded: {policy_path}\n")
            ...
            say(f"[b2_walk_deploy] FATAL: ONNX policy exists but failed to load ({e}).\n")
            say(f"[b2_walk_deploy] path: {policy_path}\n")
            say("[b2_walk_deploy] refusing to run with ZERO residual and report it as a "
                "policy result. Install onnxruntime for the CONTROLLER interpreter "
                "(the python that runs this file), not just the engine's.\n")
            raise SystemExit(2)
    else:
        say(f"[b2_walk_deploy] policy not found at {policy_path}; "
    ...
    for jn in JOINT_NAMES:
        ...
        if m is None:
            say(f"[b2_walk_deploy] missing motor {jn}_motor\n")
            return 1
        motors.append(m)
        try:
    ...
    # B2 hip/thigh effort 200 Nm, calf 320 Nm -- the Newton backend already
    ...
    apply_realistic_limits(motors,
                           max_torque_nm=_env_float("B2_MAX_TORQUE_NM", 350.0),
                           max_vel_rad_s=_env_float("B2_MAX_VEL_RAD_S", 25.0))
    # No slew by default: the trainer applies targets instantly each tick.
    motor_bank = RateLimitedMotorBank(
        motors, step_dt,
        max_vel_rad_s=_env_float("B2_TARGET_RATE_RAD_S", 1e6))
    ...
    trace_file = None
    if os.environ.get("B2_DEPLOY_TRACE") or os.environ.get(
            "OMNISIM_DEPLOY_TRACE"):
        trace_path = Path(r"C:\tmp\husky_trace\b2_walk_deploy.csv")
        trace_path.parent.mkdir(parents=True, exist_ok=True)
        trace_file = open(trace_path, "w", buffering=1)
        trace_file.write("t_ms,bx,by,bz,roll,pitch,yaw,vx\n")
        say(f"[b2_walk_deploy] trace -> {trace_path}\n")

    # Settle at the standing pose (the trainer's rest-start state).
    settle = max(1, int(_env_float("B2_SETTLE_S", 1.5) / step_dt))
    for _ in range(settle):
        if robot.step(step_ms) == -1:
            return 0

    wz_user = _env_float("B2_WZ", 0.0)
    HOLD = (os.environ.get("B2_HEADING_HOLD", "1").strip() != "0")
    HOLD_KP_YAW = _env_float("B2_HOLD_KP_YAW", 1.0)
    HOLD_KP_LAT2YAW = _env_float("B2_HOLD_KP_LAT2YAW", 0.3)
    HOLD_WZ_MAX = _env_float("B2_HOLD_WZ_MAX", 0.3)
    yaw_ref = None
    lat_ref = 0.0
    wz_cmd = wz_user

    # ── Velocity conditioning + the WALK<->STOP schedule (the OmniQuad port) ──
    VX_NOMINAL = _env_float("B2_GAIT_VX", 0.4)
    VX_CMD_MAX = _env_float("B2_VX_CMD_MAX", 0.0)      # >0 = VC policy (obs+1)
    WALK_FOR_S = _env_float("B2_WALK_FOR_S", 0.0)      # 0 = no repeating cycle
    STAND_FOR_S = _env_float("B2_STAND_FOR_S", 5.0)
    STAND_AT_S = _env_float("B2_STAND_AT_S", 0.0)      # 0 = no single window
    MODE_BLEND_S = _env_float("B2_MODE_BLEND_S", 1.0)
    _w = 1.0
    _ctrl_t0 = None
    ...
    while robot.step(step_ms) != -1:
        ...
        obs = np.clip(np.nan_to_num(obs, nan=0.0, posinf=10.0, neginf=-10.0),
                      -10.0, 10.0)
        if os.environ.get("B2_OBS_DEBUG") and sim_ms % 320 == 0:
        ...
        if sess is not None:
            try:
                ...
                say(f"[b2_walk_deploy] inference failed: {e}\n")
