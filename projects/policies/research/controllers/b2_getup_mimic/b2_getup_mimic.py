...
"""b2_getup_mimic -- the real OmniSim/Newton OmniQuad SHADOWING its MPPI get-up ghost,
...
def main():
    ...
    if pol and os.path.exists(pol):
        try:
            ...
            _dbg(f"[b2_getup_mimic] FATAL: policy EXISTS but failed to load ({e}) -- refusing to run the bare feedforward and report it as a policy result\n")
    ...
    if os.path.exists(walk_pol):
        try:
            ...
            _dbg(f"[b2_getup_mimic] FATAL: walk-hold policy EXISTS but failed to load ({e}) -- refusing to run\n")
            # FATAL, deliberately: the policy file EXISTS and would not load. That is a
            # broken environment (classically: onnxruntime missing from the CONTROLLER
            # interpreter -- a DIFFERENT python than the engine embeds), not a mode.
            # Degrading here keeps the robot moving on the bare reference and still exits
