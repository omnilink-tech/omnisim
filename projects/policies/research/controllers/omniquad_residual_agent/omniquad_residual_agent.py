...
"""OmniQuad residual-RL training agent — OmniSim-side controller.

Wraps the model walker (gait + IK + balance) with a TCP protocol the
trainer drives. Same wire protocol as omniquad_rl_agent but the action
space is interpreted differently:

  Action vector (12 dims, ±1 each, scaled by OMNIQUAD_RES_SCALE = 0.03 m):
...
from projects.policies.control.omniquad_gait import GaitParams, foot_targets  # noqa: E402
...
def main() -> int:
    args = parse_args()
    sys.stderr.write(f"[omniquad_residual_agent] resolved port = {args.port}\n")
    sys.stderr.flush()
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            sys.stderr.write(f"[omniquad_residual_agent] missing motor {name}\n")
            return 1
    ...
    if self_node is None:
        sys.stderr.write("[omniquad_residual_agent] no supervisor handle\n")
        return 1
    ...
    # drags the body BACKWARD; OMNIQUAD_GAIT_STEP_HEIGHT=0.09 restores forward
    ...
    # moment that tips OmniQuad pitch-forward from the standing pose
    ...
    # knobs as omniquad_residual_deploy's hold, so the TRAINING base equals the
    ...
    R_VX_WT = _env_float("OMNIQUAD_RES_R_VX_WT", 2.0)        # exp tracking
    R_VX_SCALE = _env_float("OMNIQUAD_RES_R_VX_SCALE", 0.10)
    R_HEADING_WT = _env_float("OMNIQUAD_RES_R_HEADING_WT", -1.0)  # rad²
    R_LATERAL_WT = _env_float("OMNIQUAD_RES_R_LATERAL_WT", -0.5)  # m²
    R_ACTION_WT = _env_float("OMNIQUAD_RES_R_ACTION_WT", -0.05)   # ||a||²
    R_ALIVE = _env_float("OMNIQUAD_RES_R_ALIVE", 0.05)
    R_TERM = _env_float("OMNIQUAD_RES_R_TERM", -10.0)
    R_CLIP_HI = _env_float("OMNIQUAD_RES_R_CLIP_HI", 5.0)
    R_CLIP_LO = _env_float("OMNIQUAD_RES_R_CLIP_LO", -10.0)
    ...
    R_UPRIGHT_BONUS_WT = _env_float("OMNIQUAD_RES_R_UPRIGHT_BONUS_WT", 0.0)
    R_UPRIGHT_BONUS_SCALE = _env_float("OMNIQUAD_RES_R_UPRIGHT_BONUS_SCALE", 0.10)
    ...
    PERTURB_FORCE_TICKS = int(_env_float("OMNIQUAD_PERTURB_FORCE_TICKS", 2.0))
    OMNIQUAD_MASS_KG = _env_float("OMNIQUAD_MASS_KG", 32.0)
    ...
    srv.listen(1)
    sys.stderr.write(f"[omniquad_residual_agent] waiting on {args.host}:{args.port}\n")
    sys.stderr.flush()
    conn, addr = srv.accept()
    conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
    sys.stderr.write(f"[omniquad_residual_agent] trainer connected from {addr}\n")
    sys.stderr.flush()
    ...
    while robot.step(step_ms) != -1:
        ...
        if PERTURB_ENABLED and sim_time >= next_perturb_t:
            try:
                ...
                force_mag = (OMNIQUAD_MASS_KG * dv) / duration_s
        ...
        try:
            ...
            sys.stderr.write("[omniquad_residual_agent] trainer disconnected\n")
            break
