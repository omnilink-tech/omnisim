...
"""OmniQuad residual-RL deploy controller.

Runs the model walker (gait + IK + balance) and applies an ONNX
residual policy on top: per-leg foot-position offsets (12 dims, ±1
each, scaled to ±3 cm).

Reads the ONNX policy from OMNIQUAD_POLICY_ONNX (default:
projects/policies/research/inference/policies/omniquad_residual_main/policy.onnx).

Writes a body-pose trace CSV (same format as omniquad_rl_deploy and
omniquad_model_walk) when OMNIQUAD_DEPLOY_TRACE is set, so verify_straight_walk
...
from projects.policies.control.omniquad_kinematics import inverse_kinematics  # noqa: E402
...
def main() -> int:
    ...
    say(f"[omniquad_residual_deploy] policy: {policy_path}\n")
    ...
    if use_policy:
        try:
            ...
            say("[omniquad_residual_deploy] ONNX loaded\n")
    ...
    # deploys with OMNIQUAD_GAIT_STEP_HEIGHT=0.09 (see omniquad-residual-rl.md).
    _gd = GaitParams()
    gait = GaitParams(
        step_height=_env_float("OMNIQUAD_GAIT_STEP_HEIGHT", _gd.step_height),
        ground_z=_env_float("OMNIQUAD_GAIT_GROUND_Z", _gd.ground_z),
    )
    ...
    say(f"[omniquad_residual_deploy] command vx={vx} vy={vy} wz={wz}\n")
    ...
    # docs/developer/omniquad-residual-rl.md. Tunable; default ON.
    HOLD = (os.environ.get("OMNIQUAD_HEADING_HOLD", "1").strip() != "0")
    HOLD_KP_YAW = _env_float("OMNIQUAD_HOLD_KP_YAW", 2.0)
    HOLD_KD_YAW = _env_float("OMNIQUAD_HOLD_KD_YAW", 0.4)
    ...
    HOLD_KP_LAT = _env_float("OMNIQUAD_HOLD_KP_LAT", 0.6)
    ...
    HOLD_KP_LAT2YAW = _env_float("OMNIQUAD_HOLD_KP_LAT2YAW", 0.18)
    HOLD_WZ_MAX = _env_float("OMNIQUAD_HOLD_WZ_MAX", 0.35)
    HOLD_VY_MAX = _env_float("OMNIQUAD_HOLD_VY_MAX", 0.12)
    say(f"[omniquad_residual_deploy] heading_hold={'on' if HOLD else 'off'} "
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            say(f"[omniquad_residual_deploy] missing motor {name}\n")
            return 1
        try:
            if hasattr(m, "setControlPID"):
                m.setControlPID(_env_float("OMNIQUAD_MOTOR_KP", 20.0), 0.0,
                                _env_float("OMNIQUAD_MOTOR_KD", 0.3))
        except Exception:
            pass
        motors.append(m)
        try:
    ...
    # the ground and back upright. See projects/policies/control/omniquad_recovery
    ...
    say(f"[omniquad_residual_deploy] recovery={'on' if recovery_enabled else 'off'} "
    ...
    if os.environ.get("OMNIQUAD_DEPLOY_TRACE") or os.environ.get(
        ...
        say(f"[omniquad_residual_deploy] trace -> {trace_path}\n")
    ...
    # omniquad_residual_agent: every PERTURB_INTERVAL_S past PERTURB_START_S,
    # add a random horizontal velocity impulse to the chassis. Default
    # DV_MAX=0 disables; OMNIQUAD_PERTURB_DV_MAX=0.4 reproduces what the
    # policy was trained to handle.
    #
    # If OMNIQUAD_PERTURB_THROW_CUBE=1 (default 1 when perturbation enabled),
    ...
    # cube is cosmetic — the actual force on OmniQuad is the impulse — but
    # it sells the visual: "something was thrown at the robot."
    PERTURB_DV_MIN = _env_float("OMNIQUAD_PERTURB_DV_MIN", 0.0)
    PERTURB_DV_MAX = _env_float("OMNIQUAD_PERTURB_DV_MAX", 0.0)
    PERTURB_INTERVAL_S = _env_float("OMNIQUAD_PERTURB_INTERVAL_S", 3.0)
    PERTURB_START_S = _env_float("OMNIQUAD_PERTURB_START_S", 2.0)
    PERTURB_ENABLED = (PERTURB_DV_MAX > 0.0)
    PERTURB_THROW_CUBE = (os.environ.get("OMNIQUAD_PERTURB_THROW_CUBE", "1").strip() != "0")
    PERTURB_CUBE_DISTANCE = _env_float("OMNIQUAD_PERTURB_CUBE_DISTANCE", 1.5)  # m
    PERTURB_CUBE_SPEED = _env_float("OMNIQUAD_PERTURB_CUBE_SPEED", 5.0)        # m/s
    PERTURB_CUBE_SIZE = _env_float("OMNIQUAD_PERTURB_CUBE_SIZE", 0.12)         # m
    PERTURB_CUBE_MASS = _env_float("OMNIQUAD_PERTURB_CUBE_MASS", 1.0)          # kg
    ...
    # interpenetrating OmniQuad's chassis; auto-removal cleans up before
    # the stuck cube interferes with the next perturbation cycle.
    # 1.5s is well past collision time (~50ms) and well before the next
    # perturbation fires.
    PERTURB_CUBE_LIFETIME_S = _env_float("OMNIQUAD_PERTURB_CUBE_LIFETIME_S", 1.5)
    next_perturb_t = PERTURB_START_S
    _root_children = None
    if PERTURB_ENABLED:
        say(f"[omniquad_residual_deploy] perturbations: dv in "
        ...
        if PERTURB_THROW_CUBE:
            try:
                ...
                say(f"[omniquad_residual_deploy] cannot access root.children: {e}; "
    ...
    # OMNIQUAD_DEBUG_JOINTS=1 prints a once-per-second summary of joint
    # angle range + max joint velocity since the last print. Used to
    # diagnose whether Newton is enforcing the URDF velocity_limit
    # (20 rad/s for OmniQuad) under external impacts. With armature=0 in
    # OmNewtonBackend, the joint inertia is just the limb inertia
    # (small), so external forces can whip joints faster than the
    # motor's own velocity limit would allow.
    DEBUG_JOINTS = (os.environ.get("OMNIQUAD_DEBUG_JOINTS", "0").strip() != "0")
    JOINT_VEL_LIMIT = _env_float("OMNIQUAD_JOINT_VEL_LIMIT", 20.0)  # rad/s
    _prev_q: list = [None] * len(motors)
    _joint_vel_max = 0.0
    _joint_q_max = -1e9
    _joint_q_min = 1e9
    _last_debug_t = 0.0
    # OMNIQUAD_DEBUG_JOINTS_CSV=<path> emits one row per tick with all 12
    # joint angles + 12 velocities. Used for offline anomaly detection.
    _joints_csv = None
    _joints_csv_path = os.environ.get("OMNIQUAD_DEBUG_JOINTS_CSV", "").strip()
    if _joints_csv_path:
        try:
            ...
            say(f"[omniquad_residual_deploy] joint CSV -> {_joints_csv_path}\n")
        except Exception as e:
            say(f"[omniquad_residual_deploy] joint CSV open failed: {e}\n")
            _joints_csv = None
    if DEBUG_JOINTS:
        say(f"[omniquad_residual_deploy] joint diag ON "
    ...
    while robot.step(step_ms) != -1:
        ...
        if DEBUG_JOINTS or _joints_csv is not None:
            ...
            if sim_t - _last_debug_t >= 1.0:
                ...
                say(f"[omniquad_residual_deploy] t={sim_t:5.1f}s  "
        ...
        if PERTURB_ENABLED and self_node is not None and sim_t >= next_perturb_t:
            try:
                ...
                if PERTURB_THROW_CUBE and _root_children is not None:
                    _cube_id += 1
                    # Aim at where OmniQuad will be when the cube arrives,
                    # not where OmniQuad is now. flight_time = distance/speed.
                    ...
                    try:
                        ...
                        say(f"[omniquad_residual_deploy] cube spawn failed: {e}\n")
            except Exception:
                pass
        ...
        if recovery_enabled:
            ...
            if rec_action == RecoveryAction.RIGHTING:
                if not getattr(rec, "_logged_start", False):
                    say(f"[omniquad_residual_deploy] fall at t={sim_t:.1f}s "
                    ...
                    say(f"[omniquad_residual_deploy]   t={sim_t:.1f}s "
            ...
            if getattr(rec, "_logged_start", False):
                actually_upright = (bz > 0.55 and abs(roll) < 0.40
                                    and abs(pitch) < 0.40)
                if actually_upright:
                    say(f"[omniquad_residual_deploy] recovered at t={sim_t:.1f}s "
                        f"(bz={bz:.2f} roll={roll:+.2f} pitch={pitch:+.2f}); "
                        f"resuming gait\n")
                    ...
                    say(f"[omniquad_residual_deploy] recovery timed out at "
                        f"t={sim_t:.1f}s "
                        f"(bz={bz:.2f} roll={roll:+.2f} pitch={pitch:+.2f}); "
        ...
        if sess is not None:
            try:
                ...
                say(f"[omniquad_residual_deploy] inference failed: {e}\n")
