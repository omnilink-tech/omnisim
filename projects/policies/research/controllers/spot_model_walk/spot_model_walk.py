...
"""Model-only Spot walker — no policy, no RL, no neural network.

Hand-coded controller that drives Spot at a commanded body velocity by:
  1. The gait engine (`spot_gait`) emits 4 foot targets in body frame.
  2. The inverse kinematics (`spot_kinematics`) converts each foot
...
in subsequent phases. By itself it should already walk Spot straight at
the commanded vx with near-zero drift on flat ground.

Environment overrides:
  SPOT_VX=0.5   commanded forward velocity, m/s
  SPOT_VY=0.0   commanded lateral velocity, m/s
  SPOT_WZ=0.0   commanded yaw rate, rad/s
  SPOT_GAIT_FREQ_HZ=2.0     gait cycle frequency
  SPOT_GAIT_DUTY=0.5        stance duty factor
  SPOT_GAIT_STEP_HEIGHT=0.05  swing arc peak, m
  SPOT_GAIT_GROUND_Z=-0.56  foot z in body frame when planted, m
  SPOT_GAIT_LATERAL_Y=0.30  foot's lateral neutral, m from body center
  SPOT_GAIT_FRONT_X=0.30    front feet's forward neutral, m
  SPOT_GAIT_REAR_X=-0.28    rear feet's backward neutral, m

  SPOT_DEPLOY_TRACE=1       write a body-pose trace CSV (matches the RL
                            deploy controller's format so verify_straight
                            _walk.py works against this controller too).
  SPOT_DEPLOY_LOG=<path>    mirror controller stderr to this file.
"""
...
# sits at projects/policies/research/controllers/spot_model_walk/spot_model_walk.py)
...
from projects.policies.control.spot_kinematics import (  # noqa: E402
...
def _gait_from_env() -> GaitParams:
    ...
    defaults = GaitParams()
    freq = _env_float("SPOT_GAIT_FREQ_HZ", 1.0 / defaults.period_s)
    period = 1.0 / max(freq, 0.1)
    return GaitParams(
        period_s=period,
        duty=_env_float("SPOT_GAIT_DUTY", defaults.duty),
        step_height=_env_float("SPOT_GAIT_STEP_HEIGHT", defaults.step_height),
        ground_z=_env_float("SPOT_GAIT_GROUND_Z", defaults.ground_z),
        neutral_lateral_y=_env_float("SPOT_GAIT_LATERAL_Y",
                                     defaults.neutral_lateral_y),
        neutral_front_x=_env_float("SPOT_GAIT_FRONT_X", defaults.neutral_front_x),
        neutral_rear_x=_env_float("SPOT_GAIT_REAR_X", defaults.neutral_rear_x),
    )
...
def main() -> int:
    ...
    say("[spot_model_walk] starting (model-only, no policy)\n")
    gait = _gait_from_env()
    ...
    balance_enabled = (os.environ.get("SPOT_BALANCE", "1").strip() != "0")
    ...
    # Spot pitch-forward from the standing pose within ~25 ticks.
    ...
    # SPOT_PITCH_TRIM): the trot has a small structural lateral thrust on
    ...
    vy += _env_float("SPOT_VY_TRIM", 0.0)
    wz = _env_float("SPOT_WZ", 0.0)
    # Closed-loop heading lock: if SPOT_WZ command is 0, override wz
    ...
    heading_lock = (os.environ.get("SPOT_HEADING_LOCK", "0").strip() != "0")
    heading_kp = _env_float("SPOT_HEADING_KP", 0.5)
    heading_clip = _env_float("SPOT_HEADING_CLIP", 0.10)
    heading_lat2yaw = _env_float("SPOT_HEADING_LAT2YAW", 0.0)
    say(f"[spot_model_walk] gait: period={gait.period_s:.3f}s duty={gait.duty} "
    ...
    say(f"[spot_model_walk] command: vx={vx} vy={vy} wz={wz}\n")
    ...
    for leg, joint in JOINT_ORDER:
        ...
        if m is None:
            say(f"[spot_model_walk] ERROR: missing motor {name}\n")
            return 1
        # Match the agent's PD: kp=20, kd=0.3. The neutral foot
        # geometry in `spot_gait.GaitParams` is set to the FK of the
        # agent's nominal pose so the resulting joint targets reproduce
        # the standing pose Spot is stable at under these gains. Stiff
    ...
    # Realistic actuator constraints from the Spot URDF (80 Nm /
    ...
    # projects/policies/control/spot_recovery.py for the orientation classes
    ...
    say(f"[spot_model_walk] recovery={'on' if recovery_enabled else 'off'} "
    ...
    if os.environ.get("SPOT_DEPLOY_TRACE") or os.environ.get(
        ...
        say(f"[spot_model_walk] trace -> {trace_path}\n")
    ...
    def stance_targets() -> list:
        ...
        from projects.policies.control.spot_gait import neutral_foot_positions
        ...
        for i, (leg, _joint) in enumerate(JOINT_ORDER):
            ...
            if q is None:
                say(f"[spot_model_walk] WARN: nominal IK unreachable for {leg}\n")
                continue
            out[i + 0] = q.hip_x
    ...
    say(f"[spot_model_walk] gait engaged (balance={'on' if balance_enabled else 'off'}, "
    ...
    while robot.step(step_ms) != -1:
        ...
        if recovery_enabled:
            ...
            if rec_action == RecoveryAction.RIGHTING:
                if not getattr(rec, "_logged_start", False):
                    say(f"[spot_model_walk] fall at t={sim_t:.1f}s "
                    ...
                    say(f"[spot_model_walk]   t={sim_t:.1f}s "
            ...
            if getattr(rec, "_logged_start", False):
                ...
                actually_upright = (bz > 0.55 and abs(roll) < 0.40
                                    and abs(pitch) < 0.40)
                if actually_upright:
                    say(f"[spot_model_walk] recovered at t={sim_t:.1f}s "
                        f"(bz={bz:.2f} roll={roll:+.2f} pitch={pitch:+.2f}); "
                        f"resuming gait\n")
                    ...
                    say(f"[spot_model_walk] recovery timed out at t={sim_t:.1f}s "
                        f"(bz={bz:.2f} roll={roll:+.2f} pitch={pitch:+.2f}); "
        ...
        # SPOT_MODEL_STAND_ONLY=1: just hold the agent's nominal stand
        # pose (0.30, 0.30, -0.60) -- a sanity check that the controller
        # plumbing is sound. Independent of gait/IK.
        if os.environ.get("SPOT_MODEL_STAND_ONLY") == "1":
            stand_vec = []
        ...
        # (steer-to-centreline, SPOT_HEADING_LAT2YAW, default 0 = legacy)
