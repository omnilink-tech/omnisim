...
ISOLATED from the in-engine MPC (src/omnisim/physics/WbNewtonBackend.cpp, owned by
...
Idea (the deterministic analog of our RL residual, but PLANNED not learned, zero-gap):
  ...
  python projects/policies/research/mpc/quad_mpc_offline.py --robot spot --secs 6
...
ROBOTS = {
    ...
    "spot": ("spot_newton_fixed2.xml", "projects.policies.control.gait.spot_trot_gait",
...
# yaw control -> dead straight; Spot/B2 (heavier) are straighter WITHOUT it (the
# yaw-correcting residual perturbs their balance). Tuned empirically.
YAW_W = {"spot": (0.0, 0.0), "go2": (12.0, 4.0), "b2": (0.0, 0.0)}
...
def build_maps(mjm, mujoco):
    ...
    for j in range(mjm.njnt):
        ...
        # knee = the Y-hinge whose UPPER bound is negative (true for spot/go2/b2);
...
def main():
    ...
    ap.add_argument("--robot", choices=list(ROBOTS), default="spot")
