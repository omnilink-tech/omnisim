...
weights), then calls its main(). Mirrors foot_redesign/walk_exp.py.
  ...
  python projects/policies/research/mpc/quad_morph/walk_exp.py \
      --base omniquad --foot boxwide --terrain rough --secs 8 [--stance 1.0] [--yaw 0] [--wz 0]
  ...
  --yaw/--wz override the MPC heading weights for THIS robot (stock omniquad=0/0, go2=12/4)
"""
...
def mjcf_path(base, foot, terrain):
    if foot == "orig":
        if terrain == "rough":
            return MJCF / {"omniquad": "omniquad_newton_fixed2_rough.xml"}[base]
        return MJCF / {"omniquad": "omniquad_newton_fixed2.xml",
...
def main():
    ...
    ap.add_argument("--base", choices=list(BASE), default="omniquad")
