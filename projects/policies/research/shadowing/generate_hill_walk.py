...
robot-agnostic over Spot / B2 / Go2.
...
force-limited closed-loop traversal (dynamic feasibility with feedback).
  ...
  python projects/policies/research/shadowing/generate_hill_walk.py --robot spot
"""
...
ROBOTS = {
    ...
    "spot": dict(
        gait="projects.policies.control.gait.spot_trot_gait",
        mjcf=os.path.join(REPO, "projects/policies/research/training/mjcf/spot_newton_fixed2.xml"),
...
def setup_generator(robot, grade, vx, ramp_run=1.8, crest=1.0, fillet=0.4,
    ...
    (Spot's anonymous interleaved dump needs the override). Returns
    ...
    if cfg["anon"]:
        from projects.policies.research.tools.generate_spot_crouch import _classify, JL_LO, JL_HI, LOWER_LEG_BODIES
