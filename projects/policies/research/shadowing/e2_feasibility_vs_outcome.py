...
from projects.policies.research.shadowing.feasibility_certificate import (  # noqa: E402
    certify, GHOSTS, GO2, B2, OMNIQUAD, G1F, G1SIT, GH,
)
from projects.policies.research.shadowing.feasibility_certificate import (  # noqa: E402
...
OUTCOMES = {
    ...
    # NOTE: OmniQuad walk DEPLOYS (+450.8 m, 0 falls, _scratch/omniquad_walk_deploy.log),
    # but the committed ghost library has no faithful OmniQuad *walk* ghost and OmniQuad's
    ...
    # meaningless). OmniQuad's deploy success is represented in this study by its real
    # library ghosts (omniquad crouch / omniquad getup), which ARE faithfully certifiable.
    "b2 walk": dict(
        deployed=True,
        outcome="DEPLOYS: walks +95 m straight, 0 falls",
        evidence="rl-current-state.md OmniQuad row (B2 +95 m); _scratch/b2_walk_deploy.log launches & steps",
    ),
    "g1 walk": dict(
        deployed=False,
    ...
    "omniquad getup": dict(
        deployed=None,
        outcome="get-up reference (B2 rise SOLVED in deploy per memory; omniquad rise not on record)",
        evidence="project_b2_getup_shadowing (RISE solved); no committed omniquad getup deploy log",
    ),
    "b2 getup": dict(
        deployed=True,
...
def _cases():
    return [
        ...
        ("omniquad crouch",             GH("omniquad_crouch_ghost.npz"), OMNIQUAD, "auto"),
        ("omniquad getup",              GH("omniquad_getup_ghost.npz"),  OMNIQUAD, "getup"),
        ("b2 getup",                GH("b2_getup_ghost.npz"),    B2,   "getup"),
        ("omniquad jump",               GH("omniquad_jump_ghost.npz"),   OMNIQUAD, "jump"),
        ("b2 hill6",                GH("b2_hill6_ghost.npz"),    B2,   "hill"),
        ("omniquad hill8",              GH("omniquad_hill8_ghost.npz"),  OMNIQUAD, "hill"),
...
def analysis(rows):
    ...
    if fp:
        ...
        print("         -> g1 walk (byte-matched model, drift compounds on the biped), b2/omniquad hill")
    ...
    print("     omniquad getup deploy verdicts come from memory notes, not a regenerable log).")
