...
"""PPO training for OmniQuad under NEWTON physics (instead of the default ODE).

Equivalent to train_omniquad_walk.py but points OmniQuadEnv at the Newton
training world and enables the Newton-side opt-ins:

  - OMNIQUAD_TRAIN_WORLD          -> projects/policies/worlds/omniquad_rl_newton.omniworld
  ...
  - OMNISIM_NEWTON_WRAPPER_USES_OWN_SHAPE -> 1 (chassis bounding box on the
    URDFRobot wrapper, since OmniQuad's legs hang below the chassis envelope)
...
PD. Deploy through the omniquad_newton_demo world (or any world with
`physicsBackend "newton"`).

Run from repo root:
    python projects/policies/research/training/train_omniquad_newton.py --envs 4 --steps 300000 \\
        --run-name omniquad_newton_v1
"""
...
def main() -> int:
    ...
    # OmniQuadEnv subprocesses inherit OMNISIM_SESSION and derive disjoint
    ...
    # project_omniquad_newton_standing memory.
    ...
    # value. See project_omniquad_newton_walking memory.
    env.setdefault("OMNIQUAD_PITCH_TRIM", "0.20")
    ...
    #     having to walk fast)
    env.setdefault("OMNIQUAD_R_LIN_VEL_WT", "1.0")
    env.setdefault("OMNIQUAD_R_ALIVE_BONUS", "1.0")
    env.setdefault("OMNIQUAD_FIXED_VX", "0.1")
    ...
    env.setdefault("OMNIQUAD_R_JOINT_LIMIT_WT", "0.0")
    env.setdefault("OMNIQUAD_R_VX_BONUS_WT", "0.0")
    # CPG trot prior MUST match deploy. The deploy sidecar (OMNIQUAD spec)
    ...
    # learns a residual ON the trot, exactly as the ODE omniquad_walk_v12
    # recipe did.
