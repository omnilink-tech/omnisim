...
Run from repo root:
    OMNIQUAD_R_VX_BONUS_WT=12.0 OMNIQUAD_R_VZ_WT=-0.2 ... \
        python projects/policies/research/training/continue_training.py \
            --from projects/policies/research/training/runs/omniquad_walk_v11/checkpoints/omniquad_30000_steps.zip \
            --run-name omniquad_walk_v12 \
            --steps 100000 \
...
def main() -> int:
    ...
    for k in sorted(os.environ):
        if k.startswith("OMNIQUAD_R_") or k.startswith("OMNIQUAD_CPG_") or k.startswith("OMNIQUAD_FIXED_"):
    ...
    final = run_dir / "omniquad_final.zip"
