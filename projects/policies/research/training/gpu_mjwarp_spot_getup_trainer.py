...
class BatchedSpotWalkEnv:
    ...
    def __init__(self, n, mjcf, device="cuda:0", reward_cfg=None, sim_dt=0.0,
        ...
        if self.track:
            ...
            # reference base VERTICAL velocity (finite-diff) -- a launch-velocity
            # reward (rw_launchvel) uses it so an explosive motion (leap) is
            # incentivised to MATCH the ref's push-off impulse, not just its
            # position (which the policy lags during a fast launch).
            self.ref_vz = torch.tensor((np.gradient(rq[:, 2]) / DT).astype(np.float32),
                                       device=self.tdev)
    ...
    def step(self, action_t):
        ...
        if self.track:
            ...
            reward = r_alive + r_up + r_track + r_z + r_act + r_rate
            if r.get("rw_launchvel", 0.0) > 0.0:
                # match the ref's base vertical velocity -> push the launch impulse
                vzerr = self.qvel_t[:, 2] - self.ref_vz[ei]
                reward = reward + r.get("rw_launchvel") * torch.exp(
                    -(vzerr * vzerr) / r.get("launchvel_sigma", 0.5))
            # FALL for a get-up: flipped over only. The standing-height floor
            # (BZ_FAIL) must NOT apply -- the start (lying) IS below it.
...
def main():
    ...
    p.add_argument("--rw-launchvel", type=float, default=0.0,
                   help="reward matching the ref's base VERTICAL velocity (push the launch "
                        "impulse for explosive motions like a leap). 0=off.")
    p.add_argument("--launchvel-sigma", type=float, default=0.5)
    ...
    reward_cfg = dict(alive=args.alive, upright=args.upright,
                      ...
                      rw_z=args.rw_z, z_sigma=args.z_sigma,
                      rw_launchvel=args.rw_launchvel, launchvel_sigma=args.launchvel_sigma,
                      gait_params=dict(
