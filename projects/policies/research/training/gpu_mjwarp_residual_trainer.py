...
Per step, for each of the N parallel envs:
    ...
    3. Foot target + residual → joint angles via the vectorized
       analytic IK (`omniquad_kinematics_np.inverse_kinematics_batched`).
...
`docs/developer/omniquad-residual-rl.md` and proven on SB3+Webots.
...
from projects.policies.control.omniquad_kinematics_np import (  # noqa: E402
...
class BatchedQuadResidualEnv:
    """Batched OmniQuad env using gait+IK model layer + foot-offset residual."""
    ...
    def _derive_joint_order(mjm, mjd):
        ...
        for j in range(1, mjm.njnt):
            ...
            # Axis test FIRST: the widened omniquad.urdf opens hip_x to +/-1.5
...
def main():
    ...
    if not Path(args.mjcf).exists():
        raise SystemExit(
            f"MJCF not found: {args.mjcf}. Run the OmniQuad Newton export "
            f"first (see projects/policies/research/training/omniquad_native.py).")
