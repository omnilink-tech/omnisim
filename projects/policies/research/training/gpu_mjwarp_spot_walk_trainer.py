...
"""GPU mujoco_warp trainer for the Spot WALKING policy -- the G1 recipe.

THE SPOT PORT OF gpu_mjwarp_g1_walk_trainer.py (the canonical sim-to-deploy
template). Architecture: an open-loop FOOT-SPACE TROT reference
(projects/policies/control/gait/spot_trot_gait.py -- stance slides at exactly -vx,
quintic swing, duty>0.5 four-foot overlap, IK-realized, stride ramp from
standing) + a 12-DOF residual policy trained with PPO on mujoco_warp
against the EXACT deploy-matched MJCF (kp=500/kv=60 actuators ==
OMNISIM_NEWTON_TARGET_KE=500/KD=60).
...
Usage (chunked warm-starts, ~400 iters x 4096 envs per chunk):
    python projects/policies/research/training/gpu_mjwarp_spot_walk_trainer.py \\
        --envs 4096 --iters 400 --rollout 12 \\
        --seed-gait-pose --rest-start-frac 0.25 --dr-init-vx-bias 0.3 \\
        --rw-sched -5 --rw-slip -0.5 --vel-l1 -0.3 \\
        --save projects/policies/research/training/runs/gpu_spot_walk1/policy.pt
"""
...
from projects.policies.research.shadowing.hill_terrain import HillProfile, RoughProfile  # noqa: E402


class HillRef:
    """GPU lookups for shadowing a hill-walk ghost: target base pitch + height
    as a function of forward x (from the ghost's own base trajectory), and the
    ground height h(x) (from the HillProfile) so foot rewards + the fall test are
    ground-relative. Also injects the hill geoms into the source MJCF so the
    trainer's physics sees the slope."""

    def __init__(self, ghost_npz, src_mjcf, out_mjcf, device):
        import numpy as _np
        g = _np.load(ghost_npz, allow_pickle=True)
        self.hp = HillProfile(grade_deg=float(g["hill_grade_deg"]),
                              approach=float(g["hill_approach"]),
                              ramp_run=float(g["hill_ramp_run"]),
                              crest=float(g["hill_crest"]),
                              fillet=float(g["hill_fillet"]))
        self.hp.inject_mjcf(src_mjcf, out_mjcf)
        self.mjcf = out_mjcf
        base = g["base"]
        x = base[:, 0].astype(_np.float32)
        z = base[:, 2].astype(_np.float32)
        # base pitch in the trainer's convention: asin(2(qw*qy - qz*qx))
        pitch = _np.arcsin(_np.clip(2 * (base[:, 3] * base[:, 5] - base[:, 6] * base[:, 4]),
                                    -1, 1)).astype(_np.float32)
        order = _np.argsort(x)
        self.bx = torch.tensor(x[order], device=device)
        self.bz = torch.tensor(z[order], device=device)
        self.bp = torch.tensor(pitch[order], device=device)
        # dense ground-height grid h(x) for per-foot lookups
        gx = _np.arange(-0.5, self.hp.x_end + 1.5, 0.02, dtype=_np.float32)
        gh = _np.array([self.hp.height(float(v)) for v in gx], dtype=_np.float32)
        self.gx = torch.tensor(gx, device=device)
        self.gh = torch.tensor(gh, device=device)
        self.x_min = float(x.min())
        self.x_max = float(self.hp.x_end + 1.0)
    ...
    def target_pitch(self, xq):
        return self._interp(xq, self.bx, self.bp)

    def target_z(self, xq):
        return self._interp(xq, self.bx, self.bz)

    def ground_h(self, xq):
        return self._interp(xq, self.gx, self.gh)


class RoughRef:
    """GPU ground_h lookups for BLIND terrain-curriculum RL. Wraps a
    RoughProfile (random full-width bump bars), injects its box geoms into the
    source MJCF so the trainer physics sees the bumps, and exposes ground_h(xq)
    so the fall-test + foot rewards are ground-relative. NO ghost, NO obs change
    (blind) — the policy just learns to walk the trot over the bumps."""
    ...
    def ground_h(self, xq):
        return HillRef._interp(xq, self.gx, self.gh)
...
# MJCF joint ranges (deploy dump). Controller order FL,FR,RL,RR x (hx,hy,kn).
JOINT_LIMITS_LO = np.array([-1.50, -0.50, -1.20] * 4, dtype=np.float32)
JOINT_LIMITS_HI = np.array([+1.50, +3.13, -0.01] * 4, dtype=np.float32)

LOWER_LEG_BODIES = (4, 7, 10, 13)   # FL, FR, RL, RR (deploy MJCF dump order)
...
class BatchedSpotWalkEnv:
    ...
    def __init__(self, n, mjcf, device="cuda:0", reward_cfg=None, sim_dt=0.0,
                 dr_cfg=None, hill_ghost=None, rough_cfg=None):
        import warp as wp
        import mujoco
        ...
        # HILL mode: inject the up-over-down hill into the deploy MJCF and shadow
        # the ghost's base pitch/height (vs the flat "stay level" reference).
        self.hill = None
        self.tdev = torch.device("cuda:0" if "cuda" in str(self.device).lower() else "cpu")
        if hill_ghost:
            hill_mjcf = str(Path(mjcf).with_name(Path(mjcf).stem + "_hill.xml"))
            self.hill = HillRef(hill_ghost, mjcf, hill_mjcf, self.tdev)
            mjcf = self.hill.mjcf
            print(f"[hill] injected {len(self.hill.hp._boxes())} boxes; shadowing ghost "
                  f"pitch/height (crest {self.hill.hp.rise:.2f} m) -> {mjcf}")

        # ROUGH-TERRAIN mode (blind terrain-curriculum RL): inject random bump
        # bars into the MJCF and make rewards/fall-test ground-relative. No ghost,
        # no obs change. Mutually exclusive with --hill-ghost.
        self.terrain = None
        if rough_cfg and not hill_ghost:
            amp = float(rough_cfg.get("amp", 0.06))
            rough_mjcf = str(Path(mjcf).with_name(Path(mjcf).stem + "_rough.xml"))
            self.terrain = RoughRef(mjcf, rough_mjcf, self.tdev, amp=amp,
        ...
        with wp.ScopedDevice(self.device):
            ...
            import os as _njos
            _njmax = int(_njos.environ.get("SPOT_NJMAX", "256"))
            _nconmax = int(_njos.environ.get("SPOT_NCONMAX", "256"))
        ...
        for j in range(self.mjm.njnt):
            ...
            # Axis test FIRST (hip_x is the only X hinge); among Y hinges
            # the knee has the strongly negative lower range.
            joint = ("hip_x" if abs(ax[0]) > 0.5 else
                     "knee" if rng_j[0] < -0.9 else "hip_y")
        ...
        # +2 hill obs (target base pitch, height error vs the ghost) appended last
        self.obs_dim = OBS_DIM + (1 if self.vx_cond else 0) + (2 if self.hill else 0)
        ...
        self.hill_flat_frac = float(self.r.get("hill_flat_frac", 0.25))
        self.hill_lookahead = float(self.r.get("hill_lookahead", 0.0))
        ...
        # ── GHOST-TUBE EARLY TERMINATION (research lever, default OFF) ──
        # Prune off-manifold exploration: end the episode the moment the robot
        # leaves a neighbourhood ("tube") of the ghost, instead of waiting for
        # the gross fall (roll/pitch 0.8 rad = 46 deg). DeepMimic-style ET:
        # by default NO extra penalty (tube_term=0) -- the only effect is that
        # the policy never collects experience in states the deploy walk will
        # never visit, so PPO spends its samples on the reachable manifold.
        #   tube_q  : joint-space RMS tracking error threshold vs the ghost (rad)
        #   tube_rp : |roll| / |pitch_err| threshold (rad)  -- tighter than fall
        #   tube_h  : |base height - target| threshold (m)
        #   tube_term: reward added on a tube hit (0 = pure ET)
        self.tube_q = float(self.r.get("tube_q", 0.0))
        self.tube_rp = float(self.r.get("tube_rp", 0.0))
        self.tube_h = float(self.r.get("tube_h", 0.0))
        # tube_prog: forward-progress LAG tube (m). Integrates the velocity
        # deficit (vx_target - vx) after warmup; fires when the robot has
        # fallen this far behind where the striding ghost should be. This is
        # the term that catches the STAND-STILL degenerate (a frozen robot
        # accrues only ~0.19 rad joint error -- indistinguishable from healthy
        # walk -- but its base-position lag grows without bound). The H2O /
        # OPT-Mimic base-position tracking term.
        self.tube_prog = float(self.r.get("tube_prog", 0.0))
        self.tube_term = float(self.r.get("tube_term", 0.0))
        self.tube_warmup = int(self.r.get("tube_warmup", 0))  # grace steps
        self.tube_on = (self.tube_q > 0 or self.tube_rp > 0 or
                        self.tube_h > 0 or self.tube_prog > 0)
        # diagnostics: count tube vs fall terminations over a run
        self._n_tube_term = 0
        self._n_fall_term = 0

        # Seed pose: standing pose, feet on the ground (+1 cm settle gap).
        self.spawn_z = self.gp.body_height + 0.01
        ...
        self.prog_lag_t = torch.zeros(n, device=self.tdev)   # progress-tube lag (m)
    ...
    def _sample_vx_cmd(self, m):
        ...
        walk). Spot's deploy showed the walk gets DILUTED by too much non-
        nominal experience (the +119m base walk weakened + drifted before
        reconverging), so concentrate HARDER on the two milestone speeds than
        the G1's 30/28/42 mix and keep a thin uniform tail for generalisation:
          ~42% exactly vx_target (preserve the strong base walk),
          ~33% exactly 0        (a solid stand),
          ~25% uniform[0,vx_cmd_max].
        """
        u = torch.rand(m, device=self.tdev)
        vc = torch.rand(m, device=self.tdev) * self.vx_cmd_max
        vc = torch.where(u < 0.42, torch.full_like(vc, self.vx_target), vc)
        vc = torch.where((u >= 0.42) & (u < 0.75), torch.zeros_like(vc), vc)
        return vc
    ...
    def _reset_envs(self, env_mask=None):
        ...
        # HILL: spread spawn x ALONG the traverse (so up-ramp/crest/down-ramp are
        # all trained directly), each from a STABLE 4-foot-planted stance
        # (QS_PHASE, standing pose) pitched + raised to the ghost pose at that x,
        # with the stride ramping in. (A random-phase/mid-swing spawn is laterally
        # unstable on a slope and roll-falls in ~1 s before it can learn -- the
        # mid-ramp diagnostic showed exactly this.) A fraction start at x=0 (the
        # deploy launch).
        if self.hill is not None:
            x0 = torch.rand(m, device=self.tdev) * self.hill.hp.x_end
            flat = torch.rand(m, device=self.tdev) < self.hill_flat_frac
            x0 = torch.where(flat, torch.zeros_like(x0), x0)
            tp = self.hill.target_pitch(x0)
            tz = self.hill.target_z(x0)
            zj = (torch.rand(m, device=self.tdev) * 2 - 1) * 0.02
            self.qpos_t[idx, 0] = x0
            self.qpos_t[idx, 1] = (torch.rand(m, device=self.tdev) * 2 - 1) * self._init_xy_band
            self.qpos_t[idx, 2] = tz + zj
            self.qpos_t[idx, 3] = torch.cos(tp * 0.5)
            self.qpos_t[idx, 4] = 0.0
            self.qpos_t[idx, 5] = torch.sin(tp * 0.5)
            self.qpos_t[idx, 6] = 0.0
            self.qvel_t[idx] = 0.0
            # planted standing pose (+ small jitter), gait clock at QS_PHASE
            bi = idx.unsqueeze(1).expand(-1, NJ)
            ci = self.qpos_idx_t.unsqueeze(0).expand(m, -1)
            jit = (torch.rand(m, NJ, device=self.tdev) * 2 - 1) * 0.03
            self.qpos_t[bi, ci] = self.nominal_t.unsqueeze(0) + jit
            self.phase_t[idx] = stg.QS_PHASE
            self._ramp_t0[idx] = 0.0

        # Refresh body world poses (xpos/xquat) for the just-teleported reset
        # rows so the slip-tracker foot baseline sees the new pose. These are a
        # pure function of qpos via forward KINEMATICS -- they do NOT need the
        # collision/constraint solve that dominates a full mjw.forward. Using
        # mjw.kinematics is bit-identical for xpos/xquat but ~15x cheaper; the
        # full forward ran every step (some env almost always resets) and was a
        # large share of train time. Nothing reads cvel/contacts/sensordata
        # before the next mjw.step recomputes the full pipeline. (The G1 walk
        # trainer's reset uses the same patch -- ~2.5x throughput.)
        with self.wp.ScopedDevice(self.device):
    ...
    def _build_obs_t(self):
        ...
        if self.hill is not None:
            # SLOPE COMMAND: where the robot should be pitched + its height error
            # vs the ghost at its current forward x (matches the deploy obs).
            xq = self.qpos_t[:, 0]
            tp = self.hill.target_pitch(xq + self.hill_lookahead)
            hz_err = self.hill.target_z(xq) - self.qpos_t[:, 2]
            obs = torch.cat([obs, tp.unsqueeze(1), hz_err.unsqueeze(1)], dim=1)
    ...
    def step(self, action_t):
        ...
        # HILL: shadow the ghost's base pitch/height vs flat "stay level". The
        # pitch ERROR (pitch - target) is what must stay ~0; the world-x speed
        # target is scaled by cos(slope) so climbing slower isn't penalised.
        if self.hill is not None:
            xq = self.qpos_t[:, 0]
            tgt_pitch = self.hill.target_pitch(xq)
            tgt_z = self.hill.target_z(xq)
            grnd = self.hill.ground_h(xq)
            pitch_err = pitch - tgt_pitch
            _vtgt = (self.vx_cmd_t if self.vx_cond else self.vx_target) * torch.cos(tgt_pitch)
        elif self.terrain is not None:
            # BLIND rough terrain: ground-relative height + fall-test so climbing
            # a bump is not penalised; stay level (no target pitch); normal speed.
            xq = self.qpos_t[:, 0]
            grnd = self.terrain.ground_h(xq)
            tgt_pitch = torch.zeros_like(pitch)
            tgt_z = grnd + r.get("z_ref", self.gp.body_height - 0.02)
            pitch_err = pitch
            _vtgt = self.vx_cmd_t if self.vx_cond else self.vx_target
        else:
            tgt_pitch = torch.zeros_like(pitch)
            tgt_z = r.get("z_ref", self.gp.body_height - 0.02)
            grnd = torch.zeros_like(bz)
            pitch_err = pitch
            _vtgt = self.vx_cmd_t if self.vx_cond else self.vx_target
        upright = torch.clamp(1.0 - roll * roll - pitch_err * pitch_err, min=0.0)
        ...
        r_height = r.get("height", -10.0) * (bz - tgt_z) ** 2
        r_act = r.get("act", -0.005) * (action_t * action_t).sum(dim=1)
        r_rate = r.get("act_rate", -0.01) * ((action_t - prev_action_t) ** 2).sum(dim=1)
        # HILL: persistent forward-PROGRESS reward (linear in vx, capped) so the
        # policy keeps CLIMBING instead of parking at the ramp base where the
        # gaussian speed reward + alive bonus make standing locally optimal.
        if self.hill is not None and r.get("fwd_prog", 0.0):
            r_fwd = r.get("fwd_prog", 0.0) * torch.minimum(
                torch.clamp(vx, min=0.0), _vtgt * 1.5)
        else:
            r_fwd = torch.zeros_like(vx)
        reward = r_alive + r_up + r_vel + r_lat + r_yaw + r_height + r_act + r_rate + r_fwd

        if self.rw_sched != 0.0 or self.rw_slip != 0.0:
            feet = self._foot_pos_t()                      # (n,4,3)
            foot_z = feet[:, :, 2]
            foot_xy = feet[:, :, 0:2]
            if self.hill is not None:                      # height ABOVE local ground
                foot_z = foot_z - self.hill.ground_h(
        ...
        # HILL: pitch fail is relative to the commanded slope pitch, and the
        # body-height fail is relative to the LOCAL ground (else a collapse onto
        # an elevated ramp reads as "still standing").
        fall = (torch.abs(roll) > ROLL_FAIL) | (torch.abs(pitch_err) > PITCH_FAIL) | \
               ((bz - grnd) < BZ_FAIL)
        reward = reward + fall.float() * r.get("term", -1.0)

        # GHOST-TUBE EARLY TERMINATION: leave the neighbourhood of the ghost
        # -> end the episode (pruning off-manifold states the deploy walk never
        # visits). Only fires after a short warmup grace and never double-counts
        # a row that already fell this step.
        if self.tube_on:
            past_warmup = self.ep_step_t >= self.tube_warmup
            tube = torch.zeros_like(fall)
            if self.tube_q > 0:
                q_now = self.qpos_t.index_select(1, self.qpos_idx_t)
                jerr = torch.sqrt(((q_now - baseline) ** 2).mean(dim=1) + 1e-12)
                tube = tube | (jerr > self.tube_q)
            if self.tube_rp > 0:
                tube = tube | (torch.abs(roll) > self.tube_rp) | \
                       (torch.abs(pitch_err) > self.tube_rp)
            if self.tube_h > 0:
                tube = tube | (torch.abs(bz - tgt_z) > self.tube_h)
            tube = tube & past_warmup & (~fall)
            reward = reward + tube.float() * self.tube_term
            self._n_tube_term += int(tube.sum().item())
            self._n_fall_term += int(fall.sum().item())
            fall = fall | tube
...
def main():
    ...
    p.add_argument("--mjcf", default=r"C:\tmp\spot_newton_fixed2.xml")
    p.add_argument("--lr", type=float, default=3e-4)
    p.add_argument("--save",
                   default=str(REPO / "projects/policies/research/training/runs/gpu_spot_walk/policy.pt"))
    ...
    p.add_argument("--res-scale", type=float, default=RES_SCALE,
                   help="residual rad. Deploy MUST set SPOT_ACT_SCALE to match.")
    p.add_argument("--seed-gait-pose", action="store_true")
    p.add_argument("--rest-start-frac", type=float, default=0.0)
    p.add_argument("--hill-ghost", default=None,
                   help="path to a hill ghost npz (projects/policies/research/shadowing/ghosts/"
                        "<robot>_hill_ghost.npz): injects the up-over-down hill, "
                        "spawns along the traverse, and shadows the ghost's base "
                        "pitch/height (obs +2). Absent = flat walk (unchanged).")
    p.add_argument("--hill-flat-frac", type=float, default=0.25,
                   help="fraction of resets that start on the flat at x=0 (the "
                        "deploy launch); the rest spread along the hill.")
    p.add_argument("--fwd-prog", type=float, default=0.0,
                   help="HILL: linear forward-progress reward (try 4.0) so the "
                        "policy commits to climbing instead of parking at the ramp.")
    p.add_argument("--hill-lookahead", type=float, default=0.0,
                   help="HILL: read the target pitch this many metres AHEAD so the "
                        "robot pitches up BEFORE hitting the ramp.")
    p.add_argument("--gait-module", default=None,
                   help="swap the gait reference, e.g. projects.policies.control.gait.spot_crawl_gait "
                        "(statically-stable crawl for hill climbing). Default = trot.")
    ...
    # GHOST-TUBE EARLY TERMINATION (research lever; default OFF = unchanged).
    p.add_argument("--tube-q", type=float, default=0.0,
                   help="end episode if joint-space RMS error vs the ghost "
                        "exceeds this (rad). 0 = off. Try 0.30.")
    p.add_argument("--tube-rp", type=float, default=0.0,
                   help="end episode if |roll| or |pitch_err| exceeds this "
                        "(rad), tighter than the 0.8 fall gate. Try 0.4.")
    p.add_argument("--tube-h", type=float, default=0.0,
                   help="end episode if |base height - target| exceeds this "
                        "(m). 0 = off. Try 0.12.")
    p.add_argument("--tube-term", type=float, default=0.0,
                   help="reward added on a tube hit (0 = pure DeepMimic-style "
                        "ET, recommended). A negative value punishes drift.")
    p.add_argument("--tube-warmup", type=int, default=0,
                   help="grace steps after reset before the tube can fire.")
    p.add_argument("--seed", type=int, default=0,
                   help="torch seed (policy init + action sampling). Also the "
                        "default DR seed if --dr-seed is left 0.")
    p.add_argument("--ckpt-every", type=int, default=0,
                   help="save a checkpoint every N iters as ckpt_<steps>.pt "
                        "next to --save (for sample-efficiency curves).")
    p.add_argument("--eval-curve", type=int, default=0,
                   help="if >0, build a held-out eval env with this many envs "
                        "and log a FAIR (fall-only, rest-start, deterministic) "
                        "survival/distance eval to curve.jsonl every "
                        "--ckpt-every iters. For sample-efficiency A/B curves.")
    # gait model knobs (deploy mirrors via SPOT_GAIT_*)
    ...
    # ROUGH-TERRAIN curriculum (blind): inject random bump bars into the training
    # MJCF, ground-relative reward/fall-test, NO obs change. Ramp --rough-amp
    # across stages (finetune each from the previous via --init-from).
    p.add_argument("--rough-amp", type=float, default=0.0,
                   help="max bump height (m). >0 enables blind rough-terrain "
                        "training. Curriculum: 0.04 -> 0.08 -> 0.12. Mutually "
                        "exclusive with --hill-ghost.")
    p.add_argument("--rough-seed", type=int, default=0,
                   help="terrain RNG seed (vary across stages for new layouts).")
    p.add_argument("--rough-x-start", type=float, default=2.0,
                   help="flat approach length before the first bump (m).")
    p.add_argument("--rough-x-end", type=float, default=40.0,
                   help="bump-field extent in +x (m).")
    args = p.parse_args()

    if args.gait_module:
        global stg
        import importlib
        stg = importlib.import_module(args.gait_module)
        print(f"[gait-module] {args.gait_module} (duty={stg.GaitParams().duty} "
              f"freq={stg.GaitParams().freq})")
    ...
    reward_cfg = dict(alive=args.alive, upright=args.upright,
                      ...
                      hill_flat_frac=args.hill_flat_frac,
                      fwd_prog=args.fwd_prog,
                      hill_lookahead=args.hill_lookahead,
                      ...
                      tube_q=args.tube_q, tube_rp=args.tube_rp,
                      tube_h=args.tube_h, tube_term=args.tube_term,
                      tube_warmup=args.tube_warmup,
    ...
    if args.no_dr:
        ...
        dr_cfg = dict(
            ...
            seed=args.dr_seed or args.seed)
        print(f"[DR] {dr_cfg}")
    ...
    env = BatchedSpotWalkEnv(args.envs, args.mjcf, reward_cfg=reward_cfg,
                             ...
                             hill_ghost=args.hill_ghost, rough_cfg=rough_cfg)
    N = args.envs
    ...
    if args.eval or args.bare:
        ...
        # HILL eval = the real deploy test: ALL envs start on the flat at x=0 and
        # must traverse the whole hill (raise --max-ep so the episode spans it).
        if env.hill is not None:
            env.hill_flat_frac = 1.0
            print(f"[hill-eval] all envs start at x=0; ghost crest x_end="
                  f"{env.hill.hp.x_end:.1f} m, full traverse ~{env.hill.bx[-1].item():.1f} m")
        obs = env.reset()
    ...
    # ── Held-out sample-efficiency eval (built ONCE; run at ckpt cadence) ──
    # A fixed, FAIR protocol identical across arms: all envs start from the
    # deploy rest-launch, FALL-ONLY termination (tube OFF by construction),
    # deterministic actions. Logs survival@6s/@12s, first-fall, distance, speed
    # vs total env-steps to curve.jsonl so baseline vs tube curves are
    # directly comparable at equal sample budget.
    eval_env = None
    curve_path = Path(args.save).with_name("curve.jsonl")
    if args.eval_curve > 0:
        import json as _json
        ev_rcfg = dict(reward_cfg)
        ev_rcfg["rest_start_frac"] = 1.0
        ev_rcfg["seed_gait"] = False
        for _k in ("tube_q", "tube_rp", "tube_h", "tube_term", "tube_warmup"):
            ev_rcfg[_k] = 0          # eval is always fall-only -> fair
        ev_dr = dict(mass_scale=0, friction_scale=0, actuator_kp_scale=0,
                     actuator_kv_scale=0, gravity_scale=0, push_prob=0,
                     push_vmax=0, obs_noise=0, action_latency_max=0,
                     init_q_band=0.02, init_xy_band=0.0, init_z_band=0.0,
                     seed=12345)
        eval_env = BatchedSpotWalkEnv(args.eval_curve, args.mjcf,
                                      reward_cfg=ev_rcfg, sim_dt=args.sim_dt,
                                      dr_cfg=ev_dr, hill_ghost=args.hill_ghost)
        curve_path.write_text("")

        def run_eval(total_steps, ev_steps=900):
            ee = eval_env
            o = ee.reset()
            ff = torch.zeros(ee.n, dtype=torch.int32, device=tdev)
            dist = torch.zeros(ee.n, device=tdev)
            vx_sum = torch.zeros(ee.n, device=tdev)
            alive_steps = torch.zeros(ee.n, device=tdev)
            for st in range(ev_steps):
                with torch.no_grad():
                    mu, _, _ = ac(o)
                alive = (ff == 0)
                o, _, done, _ = ee.step(mu)
                sa = (alive & (~done)).float()
                vx = ee.qvel_t[:, 0]
                dist += vx * DT * sa
                vx_sum += vx * sa
                alive_steps += sa
                newly = done & (ff == 0)
                ff = torch.where(newly, torch.full_like(ff, st + 1), ff)
            ffn = ff.cpu().numpy()
            never = (ffn == 0)
            ff_t = ffn.copy().astype(np.float64)
            ff_t[never] = ev_steps                 # survived the whole horizon
            surv6 = float((ff_t >= 375).mean())    # 6 s  (375*0.016)
            surv12 = float((ff_t >= 750).mean())   # 12 s
            mean_vx = (vx_sum / torch.clamp(alive_steps, min=1.0)).mean().item()
            rec = dict(steps=int(total_steps),
                       first_fall_s=float(ff_t.mean() * DT),
                       surv6=surv6, surv12=surv12,
                       dist_m=float(dist.mean().item()),
                       speed=float(mean_vx))
            with open(curve_path, "a") as fh:
                fh.write(_json.dumps(rec) + "\n")
            print(f"  [eval@{total_steps:,}] surv6={surv6:.0%} surv12={surv12:.0%} "
                  f"first_fall={rec['first_fall_s']:.2f}s dist={rec['dist_m']:.2f}m "
                  f"speed={rec['speed']:.3f}", flush=True)
    ...
    for it in range(1, args.iters + 1):
        ...
        if it % 5 == 0 or it == 1:
            ...
            tube_msg = ""
            if env.tube_on:
                tt, ft = env._n_tube_term, env._n_fall_term
                frac = tt / max(tt + ft, 1)
                tube_msg = f"  tube/fall={tt}/{ft} (tube {frac:.0%})"
            print(f"it {it:4d}  ep_rew/step~{rew_buf.mean().item():+.3f}  "
                  f"meanV {val_buf.mean().item():+.2f}  steps {total_steps:,}  "
                  f"{fps:,.0f} env-steps/s{tube_msg}", flush=True)

        if args.ckpt_every > 0 and (it % args.ckpt_every == 0):
            ck = Path(args.save).with_name(f"ckpt_{total_steps}.pt")
            torch.save(ac.state_dict(), ck)
        if eval_env is not None and (it == 1 or
                                     (args.ckpt_every > 0 and it % args.ckpt_every == 0)):
            run_eval(total_steps)
