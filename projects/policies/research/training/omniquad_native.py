...
"""Build the deploy-faithful OmniQuad as a NATIVE Newton articulation.
...
ignored, the robot runs on ctrl=0). The OmniSim deploy (OmNewtonBackend)
instead builds NATIVE Newton revolute joints with target_ke/target_kd in
POSITION_VELOCITY mode and drives them via control.joint_target_q. So to
...
exported deploy MJCF (C:/tmp/omniquad_newton_fixed.xml), so the dynamics match
...
# Gains MUST match the deploy backend (OmNewtonBackend default
...
def build_omniquad_native(mu=1.0, ke=KE, kd=KD):
    ...
    b.add_ground_plane()
    b.add_articulation(joint_indices, label="omniquad")
    return b.finalize()
