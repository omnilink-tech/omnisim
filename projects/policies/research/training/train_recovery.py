...
"""Tiny PPO trainer for the OmniQuad recovery residual policy.

Same shape as `train_residual.py` but on the OmniQuadRecoveryEnv:
...
def main() -> int:
    ...
    from omniquad_recovery_env import OmniQuadRecoveryEnv

    def make_env(eid):
        return lambda: OmniQuadRecoveryEnv(env_id=eid, verbose=False)
    ...
    final = run_dir / "omniquad_rec_final.zip"
