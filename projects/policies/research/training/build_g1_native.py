...
add_mjcf): the OmniSim deploy (OmNewtonBackend) builds NATIVE Newton revolute
joints with target_ke/target_kd in POSITION_VELOCITY mode and drives them via
control.joint_target_q; add_mjcf's MuJoCo actuators ignore that control path
(same reasoning as omniquad_native.py). We import the G1 legs URDF (carries the
...
def build_g1_native(mu: float = 1.0, ke: float = KE, kd: float = KD,
    ...
    what the real g1_stand_deploy.omniworld actually uses). Pass None to faithfully
