# IMPROVED-SHADOW (C: human 3D) run. Warm-start from the shape champion
# (gpu_g1_walk26_shape_c8) and adapt to the improved shadow: the gait reference
# now carries the measured human FRONTAL (hip ab/adduction) + TRANSVERSE (hip
# rotation) plane, and --rw-frontal-track pulls the actual hip-roll/yaw onto it
# (instead of the ~30deg splay). Everything else is IDENTICAL to the shape
# recipe so the comparison is apples-to-apples. See docs/developer/g1-improved-shadow.md.
#
# Usage: shadowC_run_driver.ps1 [-Chunks 4] [-ItersPerChunk 1000] [-Front 1.0]
param([int]$Chunks = 4, [int]$ItersPerChunk = 1000, [double]$Front = 1.0)
...
# THE upgrade: human frontal + transverse shadow, and track it.
...
for ($c = 1; $c -le $Chunks; $c++) {
    ...
    $cmd = "python -u `"$trainer`" --envs 4096 --iters $ItersPerChunk --rollout 12 --max-ep 1875 --lr $lr --ent-coef $ent --log-std-clamp $cl --dr-seed $(140 + $c) $arch $gait $shadow $dr $rw $init --mjcf `"$mjcf`" --save `"$save`""
    Invoke-Expression "$cmd *> `"$root\_scratch\shadowC_c$c.train.log`""
    if ($LASTEXITCODE -ne 0) { "TRAIN FAILED chunk $c" | Add-Content "$root\_scratch\shadowC_run_curve.csv"; break }
    "chunk $c done" | Add-Content "$root\_scratch\shadowC_run_curve.csv"
}
"DONE shadowC chunks=$Chunks" | Add-Content "$root\_scratch\shadowC_run_curve.csv"
