...
Run from repo root:
    ...
    python projects/policies/research/training/train_robot.py --robot spot --backend sb3 \
        --envs 4 --steps 500000
...
def main() -> int:
    ...
    parser.add_argument("--robot", default="spot",
    ...
    config = TrainingConfig(
        ...
        **extra_cfg,
        ...
        r_lin_vel_wt=float(os.environ.get("SPOT_R_LIN_VEL_WT", 1.0)),
        r_ang_vel_wt=float(os.environ.get("SPOT_R_ANG_VEL_WT", 0.5)),
        r_vz_wt=float(os.environ.get("SPOT_R_VZ_WT", -1.0)),
        r_rollpitch_wt=float(os.environ.get("SPOT_R_ROLLPITCH_WT", -0.1)),
        r_ang_xy_wt=float(os.environ.get("SPOT_R_ANG_XY_WT", -0.02)),
        r_action_rate_wt=float(os.environ.get("SPOT_R_ACTION_RATE_WT", -0.005)),
        r_joint_acc_wt=float(os.environ.get("SPOT_R_JOINT_ACC_WT", -2.5e-7)),
        r_alive_bonus=float(os.environ.get("SPOT_R_ALIVE_BONUS", 0.05)),
        r_vx_bonus_wt=float(os.environ.get("SPOT_R_VX_BONUS_WT", 0.0)),
        r_vx_deadband=float(os.environ.get("SPOT_R_VX_DEADBAND", 0.05)),
    )
