...
"""GPU mujoco_warp trainer for the Unitree B2 WALKING policy.

THE GO2 PORT OF gpu_mjwarp_spot_walk_trainer.py (itself the Spot port of the
canonical G1 sim-to-deploy template). Identical architecture -- an open-loop
FOOT-SPACE TROT reference (projects/policies/control/gait/b2_trot_gait.py) + a 12-DOF
...
gait-param defaults differ from the Spot trainer. Go2 deploy-matched
actuators: kp/kv set by the KE/KD sweep -- Go2's
hip/thigh motors are 23.7 Nm vs Spot's 80, so the servo is softer).
...
class RoughRef:
    """Blind rough-terrain ground_h: inject random bump bars into the MJCF and
    expose ground_h(xq) so reward/fall-test are ground-relative. No ghost, no
    obs change."""
...
JOINT_LIMITS_LO = np.array([-0.87, -0.94, -2.82] * 4, dtype=np.float32)
JOINT_LIMITS_HI = np.array([+0.87, +3.1316, -0.43] * 4, dtype=np.float32)

LOWER_LEG_BODIES = (4, 7, 10, 13)   # FL,FR,RL,RR calf bodies (verify vs MJCF dump)
...
class BatchedSpotWalkEnv:
    ...
    def __init__(self, n, mjcf, device="cuda:0", reward_cfg=None, sim_dt=0.0,
        ...
        self.spawn_z = self.gp.body_height + 0.04
    ...
    def _reset_envs(self, env_mask=None):
        ...
        # xpos/xquat for the just-teleported rows via forward KINEMATICS only
        # (bit-identical for body poses, ~2.5x cheaper than a full mjw.forward;
        # the Spot trainer's reset uses the same patch -- nothing reads
        # contacts/cvel before the next mjw.step recomputes the full pipeline).
        with self.wp.ScopedDevice(self.device):
    ...
    def _build_obs_t(self):
        ...
        if self.hill is not None:
            # SLOPE COMMAND: where the robot should be pitched (read LOOK-AHEAD so
            # it can prepare before the ramp) + its height error vs the ghost at
            # its current forward x (matches the deploy obs).
            xq = self.qpos_t[:, 0]
...
def main():
    ...
    p.add_argument("--mjcf", default=r"C:\tmp\b2_newton.xml")
    p.add_argument("--lr", type=float, default=3e-4)
    p.add_argument("--save",
                   default=str(REPO / "projects/policies/research/training/runs/gpu_b2_walk/policy.pt"))
    ...
    p.add_argument("--hill-lookahead", type=float, default=0.0,
                   help="HILL: read the target pitch this many metres AHEAD of the "
                        "robot (try 0.4) so it pitches up BEFORE hitting the ramp.")
    p.add_argument("--gait-module", default=None,
                   help="swap the gait reference, e.g. projects.policies.control.gait.b2_crawl_gait "
    ...
    p.add_argument("--vx-target", type=float, default=0.5)
    ...
    p.add_argument("--gait-step-height", type=float, default=0.08)
    p.add_argument("--gait-body-h", type=float, default=0.50)
