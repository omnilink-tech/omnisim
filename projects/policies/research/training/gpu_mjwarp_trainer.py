...
validate the trainer end-to-end at GPU speed. Porting to the real Spot
model = export Spot's MJCF the same way + match the controller's nominal
...
def classify_joints(mjm):
    ...
    matches the EXACT Spot (or quad) model: nominal stance, which joints
...
class BatchedQuadEnv:
    def __init__(self, n, mjcf, fixed_vx=0.5, device="cuda:0", reward_cfg=None,
        ...
        # quad and the real Spot; joint order = controller JOINT_ORDER)
