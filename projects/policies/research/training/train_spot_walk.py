...
"""PPO training for Spot — WALK-focused reward shaping.

Identical to train_spot.py but reads reward weights from env vars so
this variant can push harder on velocity tracking and remove the alive
bonus that creates a "stand still" local optimum.

Run from repo root:
    python projects/policies/research/training/train_spot_walk.py --envs 4 --steps 500000 \
        --run-name spot_walk

Env vars consumed by the controller (spot_rl_agent.py):
    SPOT_R_LIN_VEL_WT    forward velocity tracking weight (default 1.0)
    SPOT_R_ALIVE_BONUS   per-tick alive bonus (default 0.05)
    SPOT_R_VZ_WT         vertical velocity penalty (default -1.0)
...
def main() -> int:
    ...
    env["SPOT_R_LIN_VEL_WT"] = "2.5"
    env["SPOT_R_ALIVE_BONUS"] = "0.0"
    # Default args to train_spot.py:
    cmd = [
        sys.executable,
