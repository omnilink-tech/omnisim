...
"""Continue a Spot PPO run under NEWTON physics (warm-start from a ckpt).
...
injected by train_spot_newton.py for fresh runs. This wrapper injects the
...
weights / curriculum (SPOT_R_*, SPOT_FIXED_*) are left to the environment
...
SPOT_FIXED_VX for a speed phase.

Run from repo root, e.g. (balance phase):
    SPOT_R_ROLLPITCH_WT=-0.25 SPOT_FIXED_VX=0.1 \\
      python projects/policies/research/training/continue_spot_newton.py \\
        --from projects/policies/research/training/runs/spot_newton_v21_mu2/checkpoints/spot_60000_steps.zip \\
        --run-name spot_newton_v23_balance --steps 150000 --envs 2 \\
