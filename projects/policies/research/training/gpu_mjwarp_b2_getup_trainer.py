...
class BatchedSpotWalkEnv:
    ...
    def __init__(self, n, mjcf, device="cuda:0", reward_cfg=None, sim_dt=0.0,
        ...
        with wp.ScopedDevice(self.device):
            ...
            import os as _njos
            _njmax = int(_njos.environ.get("B2_NJMAX", "256"))
            _nconmax = int(_njos.environ.get("B2_NCONMAX", "256"))
        ...
        # ── SHADOWING reference (get-up): RSI + tracking target from a CERTIFIED
        # ghost (projects/policies/research/shadowing/ghosts/spot_getup_ghost.npz). The policy
        # learns the FEEDBACK to reproduce the (feasible but open-loop-sagging)
        # MPPI get-up. Reference resampled from the ghost control_dt onto DT. ──
        self.track_ref = self.r.get("track_ref", None)
        self.track = self.track_ref is not None
        self.lookahead_steps = int(round(0.2 / DT))
        self.rsi_start_frac = float(self.r.get("rsi_start_frac", 0.5))
        if self.track:
            _gd = np.load(self.track_ref, allow_pickle=True)
            _gq, _gqv, _gc = _gd["q"], _gd["qvel"], _gd["ctrl"]
            _gdt = float(_gd["dt"])
            _nout = int(round((_gq.shape[0] - 1) * _gdt / DT)) + 1
            _t = np.arange(_nout) * DT
            _src = np.arange(_gq.shape[0]) * _gdt
            _rs = lambda arr: np.stack(
                [np.interp(_t, _src, arr[:, c]) for c in range(arr.shape[1])], axis=1)
            rq = _rs(_gq).astype(np.float32)        # (nout, nq) full qpos
            rqv = _rs(_gqv).astype(np.float32)       # (nout, nv)
            rc = _rs(_gc).astype(np.float32)         # (nout, 12) controller-order targets
            qn = np.linalg.norm(rq[:, 3:7], axis=1, keepdims=True)
            rq[:, 3:7] /= np.clip(qn, 1e-6, None)    # renormalise lerp'd base quat
            self.ref_n = _nout
            self.ref_q = torch.tensor(rq, device=self.tdev)
            self.ref_qvel = torch.tensor(rqv, device=self.tdev)
            self.ref_legs = torch.tensor(rc, device=self.tdev)         # baseline target
            self.ref_z = torch.tensor(rq[:, 2], device=self.tdev)
            _pw, _px, _py, _pz = rq[:, 3], rq[:, 4], rq[:, 5], rq[:, 6]
            self.ref_pitch = torch.tensor(
                np.arcsin(np.clip(2 * (_pw * _py - _pz * _px), -1, 1)).astype(np.float32),
                device=self.tdev)
            self.obs_dim = 49   # vlin3+vang3+pg3+(q-ref)12+qd12+last12+phase4
            print(f"[track] get-up reference: {self.ref_n} steps @ {DT*1000:.0f}ms, "
                  f"base_z {float(self.ref_z[0]):.3f}->{float(self.ref_z.max()):.3f}, obs={self.obs_dim}")
    ...
    def _reset_envs(self, env_mask=None):
        ...
        m = idx.shape[0]
        if self.track:
            # RSI: start at a sampled reference step -- rsi_start_frac of envs at
            # step 0 (the FULL get-up from lying), the rest spread along the ref
            # for sample efficiency. qpos/qvel come from the CERTIFIED ghost, so
            # every start is feasible + on-distribution. Early-return (own tail).
            start0 = torch.rand(m, device=self.tdev) < self.rsi_start_frac
            ridx = (torch.rand(m, device=self.tdev) * self.ref_n).long().clamp(0, self.ref_n - 1)
            ridx = torch.where(start0, torch.zeros_like(ridx), ridx)
            self.qpos_t[idx] = self.ref_q[ridx]
            self.qvel_t[idx] = self.ref_qvel[ridx]
            self.ep_step_t[idx] = ridx.to(self.ep_step_t.dtype)
    ...
    def _build_obs_t(self):
        ...
        if self.track:
            # TRACKING obs: q-ref, qd, last action, + phase(b, b_ahead, z_err, pitch_err).
            ei = torch.clamp(self.ep_step_t, max=self.ref_n - 1).long()
            q_c = q - self.ref_legs[ei]
            b = ei.to(torch.float32) / max(1, self.ref_n - 1)
            ei_a = torch.clamp(self.ep_step_t + self.lookahead_steps,
                               max=self.ref_n - 1).long()
            b_a = ei_a.to(torch.float32) / max(1, self.ref_n - 1)
            pitch = torch.asin(torch.clamp(2 * (w * y - z * x), -1.0, 1.0))
            z_err = qp[:, 2] - self.ref_z[ei]
            pitch_err = pitch - self.ref_pitch[ei]
            phase = torch.stack([b, b_a, z_err, pitch_err], dim=1)
            obs = torch.cat([vlin, vang, pg, q_c, qd, self.last_action_t, phase], dim=1)
        else:
    ...
    def _baseline_targets_t(self):
        """The baseline the residual rides on. TRACK mode: the certified ghost's
        position targets at this episode step (clamped, holds the stand at the
        end). Else the trot-model gait."""
        if self.track:
            ei = torch.clamp(self.ep_step_t, max=self.ref_n - 1).long()
            return self.ref_legs[ei], None
    ...
    def step(self, action_t):
        ...
        if self.track:
            # IMITATION: track the certified get-up reference (joints + base z).
            ei = torch.clamp(self.ep_step_t, max=self.ref_n - 1).long()
            q_now = self.qpos_t.index_select(1, self.qpos_idx_t)
            jerr = ((q_now - self.ref_legs[ei]) ** 2).mean(dim=1)
            r_track = r.get("rw_track", 3.0) * torch.exp(-jerr / r.get("track_sigma", 0.05))
            zerr = bz - self.ref_z[ei]
            r_z = r.get("rw_z", 4.0) * torch.exp(-(zerr * zerr) / r.get("z_sigma", 0.01))
            # HOLD-DAMP: penalise base roll/pitch angular velocity (wx^2+wy^2),
            # ramped IN only past the stand keyframe (b>=0.40) so the dynamic
            # rise is free to rotate but the policy must ARRIVE SETTLED and stay
            # quiet at the stand. The deploy topples because the rise is
            # ballistic -- it reaches stand height with angular momentum it
            # can't damp; this teaches it to land calm so the static hold holds.
            b_phase = ei.to(torch.float32) / max(1, self.ref_n - 1)
            hold_w = torch.clamp((b_phase - 0.40) / 0.10, 0.0, 1.0)
            wxy2 = self.qvel_t[:, 3] ** 2 + self.qvel_t[:, 4] ** 2
            r_damp = r.get("hold_damp", -0.4) * hold_w * wxy2
            reward = r_alive + r_up + r_track + r_z + r_act + r_rate + r_damp
            # FALL for a get-up: flipped over (huge tilt) during the rise. The
            # standing-height floor must NOT apply early -- the start (lying) IS
            # below it. BUT once past the stand keyframe (at_stand) we TIGHTEN
            # it: tilting past ~29deg or sinking below 0.30 m now terminates, so
            # the policy is FORCED to learn a tight, settled hold (the loose
            # 80deg gate let it tilt to 79deg at the stand for free -> it never
            # learned to stay up, and the deploy toppled right after arriving).
            at_stand = (ei.to(torch.float32) / max(1, self.ref_n - 1)) >= 0.45
            fall = ((torch.abs(roll) > 1.4) | (torch.abs(pitch) > 1.4) | (bz < 0.05)
                    | (at_stand & ((torch.abs(roll) > 0.5) | (torch.abs(pitch) > 0.5)
                                   | (bz < 0.30))))
        else:
            ...
            z_ref = r.get("z_ref", self.gp.body_height - 0.02)
...
def main():
    ...
    p.add_argument("--mjcf", default=str(REPO / "projects/robots/unitree/b2/urdf/b2_planner.mjcf.xml"))
    p.add_argument("--lr", type=float, default=3e-4)
    p.add_argument("--save",
                   default=str(REPO / "projects/policies/research/training/runs/gpu_b2_getup/policy.pt"))
    ...
    # ── SHADOWING get-up tracking (Component 3) ──
    p.add_argument("--track-ref", default=None,
                   help="ghost .npz to SHADOW (reference tracking). Enables RSI + the "
                        "imitation reward + the (q-ref, phase) obs (obs=49). "
                        "e.g. projects/policies/research/shadowing/ghosts/spot_getup_ghost.npz")
    p.add_argument("--rsi-start-frac", type=float, default=0.5,
                   help="fraction of resets that start at ref step 0 (full get-up "
                        "from lying); the rest spread along the reference (RSI).")
    p.add_argument("--rw-track", type=float, default=3.0, help="joint-imitation reward weight")
    p.add_argument("--track-sigma", type=float, default=0.05, help="joint-imitation gaussian width")
    p.add_argument("--rw-z", type=float, default=4.0, help="base-height tracking reward weight")
    p.add_argument("--z-sigma", type=float, default=0.01, help="base-height gaussian width")
    p.add_argument("--hold-damp", type=float, default=-0.4,
                   help="penalty on base roll/pitch angular vel (wx^2+wy^2), "
                        "ramped in past the stand keyframe so the policy arrives "
                        "settled and the static deploy hold holds")
    ...
    reward_cfg = dict(alive=args.alive, upright=args.upright,
                      ...
                      track_ref=args.track_ref, rsi_start_frac=args.rsi_start_frac,
                      rw_track=args.rw_track, track_sigma=args.track_sigma,
                      rw_z=args.rw_z, z_sigma=args.z_sigma, hold_damp=args.hold_damp,
                      gait_params=dict(
