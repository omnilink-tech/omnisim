...
Run:
    python projects/policies/research/training/newton_solver_trainer.py \
        ...
        --init-from projects/policies/research/training/runs/cpu_omniquad/policy_cpu.pt \
        --save projects/policies/research/training/runs/newton_omniquad/policy.pt
...
sys.path.insert(0, str(Path(__file__).resolve().parent))  # so workers can import omniquad_native
...
# default CPG (sidecar values for the OmniQuad)
CPG_FREQ = 2.6
CPG_HIPY = 0.16
CPG_KNEE = 0.22
...
class NewtonEnv:
    """One faithful OmniQuad env on Newton SolverMuJoCo (use_mujoco_cpu=True)."""

    def __init__(self, mjcf, cfg):
        import warp as wp
        import newton
        from omniquad_native import build_omniquad_native
        ...
        self.model = build_omniquad_native(mu=cfg.get("mu", 1.0),
...
def main():
    ...
    p.add_argument("--save", default=str(REPO / "projects/policies/research/training/runs/newton_omniquad/policy.pt"))
