...
"""PPO residual-RL training for Spot under NEWTON physics.

Same recipe as train_residual.py (model walker + 12-dim foot-offset
residual policy), but routes SpotResidualEnv at the Newton training
world and enables the Newton-side opt-ins from train_spot_newton.py
that produced the working "Spot stands and walks open-loop" baseline.

Opt-ins:
  SPOT_TRAIN_WORLD               -> spot_residual_train_newton.wbt
...
def main() -> int:
    ...
    env["SPOT_TRAIN_WORLD"] = str(
        repo / "projects" / "rl" / "worlds" / "spot_residual_train_newton.wbt"
    )
