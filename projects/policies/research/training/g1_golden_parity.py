...
# inertia the native C++ backend feeds it (OmSolid.cpp addBody +
# OmBasicJoint.cpp addJointRevolute), then World.finalize().
# ===========================================================================
def build_deploy_model(urdf_path: Optional[Path] = None):
    ...
    Mirrors the native backend's drive of ``omnisim_newton_runtime.World``:
      ...
      * FK the URDF kinematic tree at the REST configuration (all joints = 0)
        ...
        inertia tensor, since OMNISIM_URDF_USE_INERTIA=1 -- OmSolid.cpp reads it
    ...
    # ---- Fixed-joint rollup (Webots OmSolidMerger / rolledUpMass) ---------
    ...
    # into its parent: OmSolid.cpp passes ``mass = rolledUpMass(s)`` (own mass +
    ...
    # what the rebuild-gated OmSolid.cpp change does. Off (default) passes no
    ...
    # ---- add_joint_revolute per revolute joint (mirror OmBasicJoint) ----
    n_rev = 0
    for J in joints:
        ...
        # fixed-merged parent stays faithful (mirrors OmBasicJoint's leader
        # re-expression).
...
def rollout(model, actions, seed, ticks, substeps=None):
    ...
    control.joint_target_q = nominal + ACT_SCALE * action (clamped to limits).
...
def run_trajectory(ticks: int) -> int:
    ...
    print("  * P1-vs-P2 (trainer vs deploy, both driven via newton control.joint_target_q)")
