...
checkpoint (typically the ODE-trained spot_walk_v12).

Wraps continue_training.py with the Newton-side opt-ins:

  SPOT_TRAIN_WORLD                 -> spot_rl_newton.wbt
  OMNISIM_URDF_USE_INERTIA         -> 1 (real per-link inertia tensors)
  OMNISIM_NEWTON_FORCE_MUJOCO      -> 1 (MuJoCo CPU solver under Newton)
  OMNISIM_NEWTON_WRAPPER_USES_OWN_SHAPE -> 1 (Spot chassis bounding box)

Default reward + CPG knobs are tuned for a sane "keep walking" curriculum
on the warm-started policy:

  SPOT_R_LIN_VEL_WT      = 2.0   (track commanded vx without dominating)
  SPOT_R_ALIVE_BONUS     = 0.5   (positive every step the body is upright
                                  -- defeats the "freeze for -10/step is
                                  worse than +0/step" local optimum the
                                  fresh-init 100k run got stuck in)
  SPOT_R_VZ_WT           = -0.2  (mild vertical-velocity penalty)
  SPOT_R_ANG_XY_WT       = -0.2  (mild yaw-rate penalty)
  SPOT_R_ROLLPITCH_WT    = -0.2  (mild roll/pitch penalty)
  SPOT_FIXED_VX          = 0.3   (slower-than-train_spot_walk curriculum:
...
Run from repo root:
    python projects/policies/research/training/continue_training_newton.py \\
        --from projects/policies/research/training/runs/spot_walk_v12/checkpoints/spot_140000_steps.zip \\
        --run-name spot_newton_v2 \\
...
def main() -> int:
    ...
    # MuJoCo CPU is the stable solver. XPBD on GPU NaN's on Spot under
    ...
    # CPG MUST match the warm-start policy's training. spot_walk_v12
    ...
    env.setdefault("SPOT_R_LIN_VEL_WT", "1.5")
    env.setdefault("SPOT_R_ALIVE_BONUS", "0.5")
    env.setdefault("SPOT_R_VZ_WT", "-0.2")
    env.setdefault("SPOT_R_ANG_XY_WT", "-0.1")
    env.setdefault("SPOT_R_ROLLPITCH_WT", "-0.2")
    env.setdefault("SPOT_FIXED_VX", "0.3")
