...
"""PPO residual-RL training for Spot push-recovery under NEWTON physics.
...
Default knobs (env-overridable):
  SPOT_PERTURB_DV_MAX=0.40    max Δv (m/s) applied per perturbation
                              0.4 m/s × ~32 kg = ~13 Ns ≈ small cube hit
  SPOT_PERTURB_DV_MIN=0.10    floor so every perturbation is noticeable
  SPOT_PERTURB_INTERVAL_S=2.5 seconds between perturbations
  SPOT_PERTURB_START_S=2.0    grace period before first perturbation
  SPOT_RES_R_UPRIGHT_BONUS_WT=1.0  weight on exp(-(r²+p²)/0.1) term

Run from repo root:
    python projects/policies/research/training/train_residual_perturb_newton.py \\
        --run-name spot_residual_perturb_v1 --steps 50000
"""
...
def main() -> int:
    ...
    env["SPOT_TRAIN_WORLD"] = str(
        repo / "projects" / "rl" / "worlds" / "spot_residual_train_perturb_newton.wbt"
    )
    ...
    env.setdefault("SPOT_PERTURB_DV_MIN", "0.10")
    env.setdefault("SPOT_PERTURB_DV_MAX", "0.40")
    env.setdefault("SPOT_PERTURB_INTERVAL_S", "2.5")
    env.setdefault("SPOT_PERTURB_START_S", "2.0")
    ...
    env.setdefault("SPOT_RES_R_UPRIGHT_BONUS_WT", "1.0")
    env.setdefault("SPOT_RES_R_UPRIGHT_BONUS_SCALE", "0.10")
    ...
    env.setdefault("SPOT_RES_R_VX_WT", "1.0")
    env.setdefault("SPOT_RES_R_HEADING_WT", "-1.0")
    env.setdefault("SPOT_RES_R_LATERAL_WT", "-0.5")
