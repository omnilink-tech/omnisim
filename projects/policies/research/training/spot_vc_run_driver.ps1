# VELOCITY-CONDITIONED Spot run: warm from gpu_spot_walk_main (the deployable
...
# "stop-in-the-middle" milestone, ported to Spot (see vc_run_driver.ps1).
#
...
# Writes spot_vc_c*.train.log for inspection.
# Usage: spot_vc_run_driver.ps1 [-Chunks 6] [-ItersPerChunk 500]
param([int]$Chunks = 6, [int]$ItersPerChunk = 500)
$ErrorActionPreference = "Continue"
$root    = (Resolve-Path "$PSScriptRoot\..\..\..").Path
$env:OMNISIM_HOME = $root
$trainer = "$root\projects\policies\research\training\gpu_mjwarp_spot_walk_trainer.py"
$mjcf    = "C:\tmp\spot_newton_fixed2.xml"
$runtag  = "gpu_spot_vc"
$warm    = "$root\projects\policies\research\inference\policies\gpu_spot_walk_main\policy.pt"
...
for ($c = 1; $c -le $Chunks; $c++) {
    ...
    Write-Host "[spot-vc] chunk $c -> $save (lr=$lr)"
    Invoke-Expression "$cmd *> `"$root\_scratch\spot_vc_c$c.train.log`""
    if ($LASTEXITCODE -ne 0) { Write-Host "TRAIN FAILED chunk $c"; break }
}
Write-Host "DONE spot vc chunks=$Chunks"
