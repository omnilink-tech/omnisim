...
"""OmniQuadResidualEnv — Gymnasium env for residual-RL on OmniQuad.

Mirrors the OmniQuadEnv TCP protocol but with a smaller observation/action
...
The OmniSim world (omniquad_residual_train.omniworld) launches the
omniquad_residual_agent controller.
"""
...
WORLD_PATH = REPO_ROOT / "projects" / "rl" / "worlds" / "omniquad_residual_train.omniworld"
...
BASE_PORT = 7300  # disjoint from OmniQuadEnv's 7100 to avoid collision


class OmniQuadResidualEnv(gym.Env):
    ...
    def __init__(
        ...
        # 200-port slot (OmniQuadEnv agents use base+0..29, recovery base+60..89,
        ...
        # World resolution mirrors OmniQuadEnv: ctor arg > OMNIQUAD_TRAIN_WORLD env > default.
        # Lets the Newton launcher swap in omniquad_residual_train_newton.omniworld
    ...
    def _start_sim(self) -> None:
        ...
        env["OMNISIM_LOG_PATH"] = str(sdir / f"omniquad_res_{self.env_id}.log")
        ...
        stdout_path = sdir / f"omniquad_res_{self.env_id}.stdout.log"
        ...
        if last_err is not None:
            raise RuntimeError(f"OmniQuadResidualEnv {self.env_id}: Popen failed: {last_err}")
    ...
    def _connect(self) -> None:
        ...
        while time.time() < deadline:
            try:
                ...
                last_err = e
                time.sleep(0.2)
        raise RuntimeError(f"OmniQuadResidualEnv {self.env_id}: connect failed: {last_err}")
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.proc is None or self.proc.poll() is not None:
            ...
            # tunable via OMNIQUAD_ENV_STARTUP_S env var.
