...
"""OmniQuadEnv — Gymnasium environment wrapping an OmniSim subprocess.

Each instance launches a headless OmniSim process running
the omniquad_rl_agent controller, which exchanges fixed-size observation
and action packets over TCP. See omniquad_rl_agent.py for the protocol.

Usage:
    from omniquad_env import OmniQuadEnv
    env = OmniQuadEnv(env_id=0)
    obs, info = env.reset()
...
class OmniQuadEnv(gym.Env):
    ...
    def __init__(
        ...
        #   2. OMNIQUAD_TRAIN_WORLD env var (lets the launcher pick Newton
        #      vs ODE without code changes; subproc envs inherit the
        #      variable from the parent so all parallel envs get the
        #      same world)
        #   3. WORLD_PATH default (omniquad_rl.omniworld, ODE)
        if world_path:
            self.world_path = Path(world_path)
        elif os.environ.get("OMNIQUAD_TRAIN_WORLD"):
            self.world_path = Path(os.environ["OMNIQUAD_TRAIN_WORLD"])
        else:
    ...
    def _kill_port_squatter_DISABLED(self) -> None:
        ...
        for line in out.splitlines():
            if "LISTENING" in line and wanted in line:
                ...
                pid = parts[-1]
                if self.verbose:
                    print(f"[OmniQuadEnv {self.env_id}] killing port {self.port} squatter pid={pid}")
                try:
                    _sp.run(
    ...
    def _start_sim(self) -> None:
        ...
        env["OMNISIM_LOG_PATH"] = str(sdir / f"omniquad_rl_{self.env_id}.log")
        env["OMNIQUAD_RL_PORT"] = str(self.port)  # belt+suspenders
        ...
        stdout_path = sdir / f"omniquad_rl_{self.env_id}.stdout.log"
        self._stdout_log = open(stdout_path, "w")
        if self.verbose:
            print(f"[OmniQuadEnv {self.env_id}] launching {cmd[0]} world={self.world_path.name} port={self.port}")
        ...
        if last_err is not None:
            raise RuntimeError(
                f"OmniQuadEnv {self.env_id}: subprocess.Popen failed after retries: {last_err}"
            )
    ...
    def _connect(self) -> None:
        ...
        while time.time() < deadline:
            try:
                ...
                self.sock = s
                if self.verbose:
                    print(f"[OmniQuadEnv {self.env_id}] connected to {self.host}:{self.port}")
                return
            except (ConnectionRefusedError, socket.timeout, OSError) as e:
                last_err = e
                time.sleep(0.2)
        raise RuntimeError(f"OmniQuadEnv {self.env_id}: could not connect to {self.host}:{self.port}: {last_err}")
...
def make_omniquad_env(env_id: int = 0, **kwargs):
    """Factory for SubprocVecEnv compatibility."""
    def _thunk():
        return OmniQuadEnv(env_id=env_id, **kwargs)
    return _thunk
