...
baseline, no gait). Mirrors the shape of omniquad_residual_env but adapted
...
ACT_SCALE = 0.3      # rad per action unit. For G1 the policy carries most of
                     ...
                     # OmniQuad used 0.15 because its gait-IK baseline already walks.
SPAWN_Z = 0.78
BZ_TARGET = 0.70
BZ_FAIL = 0.45
ROLL_FAIL = 0.8
...
class G1StandEnv(gym.Env):
    ...
    def _balance_baseline(self) -> np.ndarray:
        ...
        Mirrors OmniQuad's analytic baseline (NOMINAL pose + body-pose PD).
