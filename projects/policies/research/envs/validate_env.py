...
"""Smoke test for OmniQuadEnv: random-action rollout, sanity-check obs/reward.

Launches one OmniQuadEnv, runs N random-action steps, prints stats.
"""
...
def main() -> int:
    print("[validate] launching OmniQuadEnv (verbose)…")
    env = OmniQuadEnv(env_id=0, verbose=True)
    t0 = time.time()
    obs, info = env.reset()
    t1 = time.time()
