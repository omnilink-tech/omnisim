# Run the OmniQuad Newton walking deploy headless and report straight-walk metrics.
#
# Recipe (2026-06-11, post self-collision fix): the W6 "OmniQuad deploy collapse"
...
#   - OMNIQUAD_GAIT_STEP_HEIGHT=0.09 (the default 0.05 swing arc grazes the
#     ground and drags the body BACKWARD)
# Policy: a residual ONNX trained through this exact stack via
#   python projects/policies/research/training/train_residual_newton.py (KE/KD/OMNIQUAD_GAIT_*
#   are honored from the environment).
#
# VERIFIED STATE 2026-06-11 (evening): with the omniquad_gait yaw-sweep fix +
...
# the env port-collision fix in omniquad_residual_env).
#
# Usage:  powershell -File scripts/dev/run_omniquad_walk_newton.ps1 [-Duration 30] [-Policy <onnx>]
param(
    [int]$Duration = 30,
    [string]$Policy = ""
)
...
$env:OMNIQUAD_GAIT_STEP_HEIGHT              = "0.09"
...
$env:OMNIQUAD_HEADING_LOCK                  = "1"
$env:OMNIQUAD_HEADING_KP                    = "1.0"
$env:OMNIQUAD_HEADING_CLIP                  = "0.20"
$env:OMNIQUAD_HEADING_LAT2YAW               = "0.10"
...
$env:OMNIQUAD_HOLD_KP_YAW                   = "1.0"
$env:OMNIQUAD_HOLD_KD_YAW                   = "0"
$env:OMNIQUAD_HOLD_KP_LAT2YAW               = "0.10"
$env:OMNIQUAD_HOLD_WZ_MAX                   = "0.20"
$env:OMNIQUAD_HOLD_KP_LAT                   = "0"
$env:OMNIQUAD_HOLD_VY_MAX                   = "0"

if ($Policy -eq "") {
    python "$root\projects\policies\research\tools\verify_straight_walk.py" --no-policy `
        --world "$root\projects\policies\research\worlds\omniquad_model_walk_demo.omniworld" --duration $Duration
} else {
    python "$root\projects\policies\research\tools\verify_straight_walk.py" --policy $Policy `
        --world "$root\projects\policies\research\worlds\omniquad_residual_deploy.omniworld" --duration $Duration
}
