...
# The bracketed engine tag is named after the emitting C++ class, and those
# classes are being renamed Wb* -> Om*, so the same lines will read
# "[OmNewtonBackend] step N ..." once the rename lands.  ONE pattern feeds all
# three matchers below (match / split-lookahead / prefix test) so they can never
# drift apart, and both prefixes are accepted permanently: this tool must work on
# either side of the rename and on log files captured before it.
_STEP_PREFIX_PAT = r"INFO: \[(?:Wb|Om)NewtonBackend\] step "
_STEP_PREFIX_RE = re.compile(_STEP_PREFIX_PAT)
_STEP_LINE_RE = re.compile(_STEP_PREFIX_PAT + r"(\d+) .*")
_STEP_SPLIT_RE = re.compile(r"(?=" + _STEP_PREFIX_PAT + r")")
...
def main() -> int:
    ...
    # Each "INFO: [OmNewtonBackend] step ..." can span multiple physical
    # lines due to Webots' Qt log line-wrapping. Re-flow.
    lines = _STEP_SPLIT_RE.split(text)
    steps = []
    for chunk in lines:
        if not _STEP_PREFIX_RE.match(chunk):
            continue
        # Join wrapped lines
