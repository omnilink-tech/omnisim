...
"""Faithful, fast OmniQuad policy eval on Newton's SolverMuJoCo (the OmniSim
deploy engine) -- NO Webots/GUI, runs in seconds.

Loads the EXACT exported OmniQuad MJCF into a newton.ModelBuilder via add_mjcf,
finalizes, and steps SolverMuJoCo with the SAME obs->ONNX->control loop the
deploy controller (omniquad_rl_deploy.py) runs in OmniSim:
  ...
  control.joint_target_q[qd-indexed] = target
...
Run:
    python projects/policies/research/tools/_eval_omniquad_solver.py \
        --policy projects/policies/research/inference/policies/gpu_omniquad/policy.onnx \
        --mjcf C:/tmp/omniquad_newton_fixed.xml --vx 0.5 --secs 12
...
def main():
    ...
    p.add_argument("--policy", default=str(REPO / "projects/policies/research/inference/policies/gpu_omniquad/policy.onnx"))
