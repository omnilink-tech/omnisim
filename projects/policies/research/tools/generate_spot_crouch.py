...
"""FULL shadowing Component 1 (MPPI ghost generator) on a NON-GAIT Spot motion.

The Spot WALK ghost uses the analytic trot model as its feasible generator (gait is feasible
by construction). This script exercises the REAL Component-1 path -- the MPPI ghost_generator
DISCOVERING a feasible motion over Spot's actual dynamics -- on a motion the trot model can't
...
Spot's deploy MJCF dump (spot_newton_fixed2.xml) has ANONYMOUS joints/bodies and INTERLEAVED
...
OVERRIDE the Spot mappings on the instance (pos actuators, their joint qpos addrs, synthetic
...
Run: python projects/policies/research/tools/generate_spot_crouch.py [--secs 5 --crouch-z 0.40]
"""
...
def main():
    ...
    ap.add_argument("--out", default=str(_REPO / "_scratch" / "spot_crouch_ghost.npz"))
    a = ap.parse_args()

    gen = GhostGenerator(a.mjcf, sim_dt=0.004, control_dt=0.02)

    # --- OVERRIDE the Spot mappings on the instance (anonymous interleaved dump) ---
    ...
    gen.fixed_base = False                                          # Spot has a free base
    print(f"[spot-crouch] overrode mappings: {len(gen.pos_act)} pos actuators, "
    ...
    print(f"[spot-crouch] settled stand: base_z={init[2]:.3f}  tilt={_tilt(init):.1f}deg")
    ...
    bz = g["base"][:, 2]
    print(f"[spot-crouch] generated {g['q'].shape[0]} steps; base_z "
    ...
    #     with the correct Spot actuators -> did it stay upright? (the decision gate).
    ...
    print("=" * 72)
    print("SPOT CROUCH-RECOVER GHOST -- MPPI-GENERATED + CERTIFIED (shadowing C1+C2)")
    print("=" * 72)
