...
"""Analyze per-tick joint state CSV from omniquad_residual_deploy for unrealistic
motion patterns. Pair with OMNIQUAD_DEBUG_JOINTS_CSV=<path> during a deploy run.

Six categories flagged:
  1. Out-of-URDF-range joint positions (the post-step clamp in
     OmNewtonBackend should make this 0).
...
DEFAULT_CSV = Path.home() / "AppData/Local/Temp/omniquad_joints.csv"

# URDF limits — read from the OmNewtonBackend startup log lines like
# `hinge joint X ... lim=[lo, hi]`. Defaults below match the widened
# omniquad.urdf (post-2026-05-24). For the narrow omniquad.classic.urdf, pass
# --urdf classic.
URDF_RANGE_WIDENED = {}
URDF_RANGE_CLASSIC = {}
...
def main():
    ...
    p.add_argument("--csv", type=Path, default=DEFAULT_CSV,
                   help="Path to OMNIQUAD_DEBUG_JOINTS_CSV output.")
