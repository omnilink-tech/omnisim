...
"""Certify the Spot walk/STOP/walk ghost is DYNAMICALLY FEASIBLE (Shadowing Component 2).

Honest gap this closes: the Spot ghost reference is the analytic foot-space TROT MODEL
...
shadowing sense and was never run through ghost_verifier. This script does it properly:
  ...
  1. GENERATE a feasible-BY-CONSTRUCTION ghost: forward-roll the trot walk/stop/walk
     ...
     reference) open-loop over the DEPLOY-MATCHED MuJoCo model (spot_newton_fixed2.xml,
...
Run: python projects/policies/research/tools/verify_spot_walk_ghost.py [--walk 5 --stand 5 --secs 20]
"""
...
def _classify(m, d):
    ...
    Same classification as gpu_mjwarp_spot_walk_trainer (anonymous joint_N in the dump):
...
def main():
    ...
    ap.add_argument("--out", default=str(_REPO / "_scratch" / "spot_walk_ghost.npz"))
    a = ap.parse_args()
    ...
    print("=" * 74)
    print("SPOT WALK/STOP/WALK GHOST -- DYNAMIC FEASIBILITY CERTIFICATE (shadowing C2)")
    print("=" * 74)
