...
"""Deploy a OmniQuad policy headless on the ODE walk demo and report straight-line metrics.

Reads the chassis trace CSV the deploy controller writes
(`C:/tmp/husky_trace/omniquad_deploy.csv`) and reports:
  - forward distance (+x)
...
TRACE = Path(r"C:\tmp\husky_trace\omniquad_deploy.csv")
...
def run_deploy(policy: Path, duration_s: float, vx: float,
    ...
    env["OMNIQUAD_VX"] = str(vx); env["OMNIQUAD_VY"] = "0.0"; env["OMNIQUAD_WZ"] = "0.0"
...
def main() -> int:
    ...
    p.add_argument("--policy", type=Path, required=False,
                   help="ONNX policy (used by omniquad_walk_demo.omniworld's deploy "
                        "controller). Pass --no-policy to verify a model-only "
                        "controller -- in that case --world is required and "
                        "the OMNIQUAD_POLICY_ONNX env var is ignored by the world.")
    ...
    p.add_argument("--world", type=Path, default=WORLD,
                   help="Webots world (.wbt) to run. Defaults to "
                        "projects/policies/worlds/omniquad_walk_demo.omniworld.")
