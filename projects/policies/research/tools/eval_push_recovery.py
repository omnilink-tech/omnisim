...
Runs N episodes through SpotResidualEnv with periodic horizontal Δv
impulses injected by spot_residual_agent. For each episode, reports
...
Example:
    ...
    # Trained policy
    python projects/policies/research/tools/eval_push_recovery.py \\
        --policy projects/policies/research/inference/policies/spot_residual_perturb_main/policy.onnx \\
...
def main() -> int:
    ...
    p.add_argument("--world", type=Path,
                   ...
                   / "spot_residual_train_perturb_newton.wbt")
    p.add_argument("--seed", type=int, default=42)
    args = p.parse_args()

    # Configure the Newton + perturbation env vars BEFORE SpotResidualEnv
    # launches Webots — they're inherited by the spawned process.
    os.environ["SPOT_TRAIN_WORLD"] = str(args.world)
    ...
    os.environ["SPOT_PERTURB_DV_MIN"] = str(args.dv_min)
    os.environ["SPOT_PERTURB_DV_MAX"] = str(args.dv_max)
    os.environ["SPOT_PERTURB_INTERVAL_S"] = str(args.interval_s)
    os.environ["SPOT_PERTURB_START_S"] = str(args.start_s)
    ...
    env = SpotResidualEnv(env_id=0, verbose=False)
