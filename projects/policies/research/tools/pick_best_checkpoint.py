...
Run from repo root:
    OMNIQUAD_CPG_FREQ_HZ=1.75 OMNIQUAD_CPG_HIP_Y_AMP=0.20 OMNIQUAD_CPG_KNEE_AMP=0.30 \
        python projects/policies/research/tools/pick_best_checkpoint.py \
            --run omniquad_walk_v9 \
            --promote-to projects/policies/research/inference/policies/omniquad_walk_main
...
def write_sidecar_for(onnx_path: Path):
    """Write robot_spec.json next to the ONNX using the canonical OmniQuad spec
    ...
    spec = dataclasses.replace(
        OMNIQUAD,
        cpg_freq_hz=float(os.environ.get("OMNIQUAD_CPG_FREQ_HZ", OMNIQUAD.cpg_freq_hz)),
        cpg_hip_y_amp=float(os.environ.get("OMNIQUAD_CPG_HIP_Y_AMP", OMNIQUAD.cpg_hip_y_amp)),
        cpg_knee_amp=float(os.environ.get("OMNIQUAD_CPG_KNEE_AMP", OMNIQUAD.cpg_knee_amp)),
    )
