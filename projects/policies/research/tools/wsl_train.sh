...
# Train OmniQuad via MJX inside WSL Ubuntu + deploy ONNX back to Windows OmniSim.
#
...
#   STEPS=10000000  ENVS=1024  RUN_NAME=omniquad_mjx_v1  bash wsl_train.sh
set -e
...
ENVS=${ENVS:-1024}
RUN_NAME=${RUN_NAME:-omniquad_mjx_gpu_$(date +%Y%m%d_%H%M%S)}
...
echo "[wsl_train] reward weights: lin_vel=$OMNIQUAD_R_LIN_VEL_WT alive=$OMNIQUAD_R_ALIVE_BONUS"
echo ''
...
echo "  .\\msys64\\mingw64\\bin\\omnisim-bin.exe projects\\policies\\research\\worlds\\rl_deploy.omniworld"
