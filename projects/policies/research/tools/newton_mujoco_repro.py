...
def main():
    ...
    def reapply_control():
        a = control.joint_target_pos.numpy()
        for di, tgt in target_by_qdidx.items():
            if di < len(a):
                a[di] = tgt
        control.joint_target_pos.assign(a)

    # Two-"episode" test: simulate, RESET (mirroring WbNewtonBackend's
    ...
    def run_episode(label, n=40):
        ...
        for step in range(1, n + 1):
            ...
            tpa = control.joint_target_pos.numpy()
            for di, tgt in target_by_qdidx.items():
                if di < len(tpa):
                    tpa[di] = tgt
            control.joint_target_pos.assign(tpa)
            a.clear_forces()
    ...
    def do_reset():
        # Mirror WbNewtonBackend.reset_joints_to_defaults: restore standing
