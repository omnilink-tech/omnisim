...
def eval_in_env(policy_path: Path, duration_s: float, vx_target: float,
    ...
    os.environ["OMNIQUAD_FIXED_VX"] = f"{vx_target}"
    os.environ["OMNIQUAD_FIXED_WZ"] = f"{wz_target}"

    env = OmniQuadEnv(env_id=42, verbose=verbose)
    obs, _ = env.reset()
...
def eval_in_deploy(policy_path: Path, duration_s: float,
    ...
    csv_path = trace_dir / "omniquad_deploy.csv"
    if csv_path.exists():
        csv_path.unlink()
