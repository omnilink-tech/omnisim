...
"""Patch the OmniQuad URDF so it's suitable for RL training.
...
Run from repo root:
    python projects/policies/research/tools/patch_omniquad_urdf_for_rl.py
...
URDF_PATH = pathlib.Path("projects/robots/omnisim/omniquad/urdf/omniquad.urdf")
...
def patch_lower_leg_collision(content: str) -> str:
    ...
    for leg in LEGS:
        ...
        old = (
            r'<collision>\s*'
            r'<geometry>\s*'
            r'<mesh filename="package://omniquad/meshes/'
