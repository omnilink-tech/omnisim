...
For each checkpoint .zip in a run dir, this script:
  ...
  2. Launches the deploy world (`omniquad_rl_deploy.omniworld`) headlessly with that
     ONNX as the policy
...
Run from repo root (Windows PowerShell or Git Bash):
    python projects/policies/research/tools/pick_best_omnisim.py \\
        --run omniquad_omnisim_v1 \\
        --duration 30 \\
        --stride 2 \\
        --promote-to projects/policies/research/inference/policies/omniquad_omnisim_main
...
def write_sidecar_for(onnx_path: Path):
    ...
    OMNIQUAD runtime spec, so the deploy controller picks up the same
...
def run_one_deploy(onnx_path: Path, duration_s: float, vx_target: float) -> dict:
    ...
    if TRACE_PATH.exists():
        TRACE_PATH.unlink()
    env = os.environ.copy()
    env["OMNIQUAD_POLICY_ONNX"] = str(onnx_path)
    env["OMNISIM_POLICY_ONNX"] = str(onnx_path)
    env["OMNIQUAD_DEPLOY_TRACE"] = "1"
    env["OMNIQUAD_VX"] = str(vx_target)
    env["OMNIQUAD_VY"] = "0.0"
    env["OMNIQUAD_WZ"] = "0.0"
    cmd = [
