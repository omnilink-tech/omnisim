...
After training + ONNX export, this script:
  ...
  2. Launches the omniquad_rl_deploy world headlessly with OMNIQUAD_SIM_DURATION_S
  3. Reads the body's trajectory CSV from C:\\tmp\\husky_trace\\omniquad_deploy_*.log
  4. Prints summary: how long did the policy keep OmniQuad upright? Did it walk?
...
def main() -> int:
    ...
    try:
        Path(r"C:\tmp\husky_trace\omniquad_deploy.csv").unlink(missing_ok=True)
    except Exception:
        pass
    ...
    csv = Path(r"C:\tmp\husky_trace\omniquad_deploy.csv")
    if csv.exists():
