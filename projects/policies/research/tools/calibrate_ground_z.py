...
"""Measure Spot's settled body Z in MuJoCo and recommend gait `ground_z`.
...
This tool drops Spot from STAND_Z=0.7 with the controller's nominal
...
Usage:
  ...
  # backends._urdf_to_mjcf.load_or_convert with the canonical Spot
  ...
  python projects/policies/research/tools/calibrate_ground_z.py \
      --mjcf C:/tmp/spot_newton_fixed.xml

Once the recommended value is known, pass it to the trainer via the
new --ground-z arg (or edit GaitParams default in spot_gait.py).
"""
...
from projects.policies.research.backends.spot_robot_spec import (  # noqa: E402
    JOINT_NAMES,
    JOINT_LIMITS_LO,
    JOINT_LIMITS_HI,
    NOMINAL_POSE,
...
def main():
    ...
    src = args.mjcf if args.mjcf else f"URDF: {SPOT.urdf_path}"
    print("Spot ground_z calibration")
    print("-" * 60)
