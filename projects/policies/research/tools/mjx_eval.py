...
Usage (from repo root, inside the WSL venv):
    /root/venv-gpu/bin/python projects/policies/research/tools/mjx_eval.py \
        --checkpoint projects/policies/research/training/runs/spot_mjx_v3/spot_final.msgpack \
        --duration 10 --vx 0.5
"""
...
def main() -> int:
    ...
    def reset_one(rng):
        ...
        qpos = d.qpos.at[0].set(float(SPOT.spawn_translation[0]))
        qpos = qpos.at[1].set(float(SPOT.spawn_translation[1]))
        qpos = qpos.at[2].set(float(SPOT.spawn_translation[2]))
        ...
        for i, q in enumerate(SPOT.nominal_pose):
    ...
    nominal = jnp.asarray(SPOT.nominal_pose)
    ...
    def step_batch(states, actions):
        ...
        targets = nominal + float(SPOT.action_scale) * actions
    ...
    print(f"[eval] final body z (across envs): {bz_final:+.3f} (spawn z = {SPOT.spawn_translation[2]})")
