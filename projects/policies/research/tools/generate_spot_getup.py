...
"""FULL shadowing Component 1 on a HARD non-gait motion: Spot GET-UP from the ground.
...
Pipeline here = Component 1 (+2): drop Spot into a folded LYING pose and settle (trunk
...
Spot dump has anonymous interleaved actuators + no foot/_pos names -> we OVERRIDE the
GhostGenerator's mappings on the instance (no edits to the shared generator).

Run: python projects/policies/research/tools/generate_spot_getup.py [--secs 4 --fold-knee -1.1 ...]
"""
...
def main():
    ...
    ap.add_argument("--out", default=str(_REPO / "_scratch" / "spot_getup_ghost.npz"))
    a = ap.parse_args()
    ...
    # ── verify: open-loop replay of the generated targets (correct Spot actuators) ──
    ...
    print("=" * 72)
    print("SPOT GET-UP GHOST -- MPPI-GENERATED (shadowing C1) + verify")
    print("=" * 72)
