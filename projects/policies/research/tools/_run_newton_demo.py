...
"""Launch the Spot Newton demo headless and report what Newton did.

Pulls the trained policy from spot_omnisim_main (the ODE-validated walker
...
actually loads Spot's articulation (the multi-parent bug fix) and steps
without crashing.
...
WORLD = REPO_ROOT / "projects" / "rl" / "worlds" / "spot_newton_demo.wbt"
OMNISIM_BIN = REPO_ROOT / "msys64" / "mingw64" / "bin" / "omnisim-bin.exe"
OMNISIM_GUI = REPO_ROOT / "msys64" / "mingw64" / "bin" / "webots.exe"
POLICY = REPO_ROOT / "projects" / "rl" / "inference" / "policies" / "spot_omnisim_main" / "policy.onnx"
...
def main() -> int:
    ...
    # Make WbUrdfImporter emit each URDF link's <inertia> tensor into
    ...
    # leg-segment mesh-AABB fix below is what gives Spot proper ground
    ...
    env["SPOT_DEPLOY_LOG"] = str(LOG_DIR / "spot_controller.log")
