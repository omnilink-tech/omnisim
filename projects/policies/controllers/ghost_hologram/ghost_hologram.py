...
# THE GHOST STORE. Ghost luts used to live in this controller's own directory (which was
# named `g1_ghost` -- so the H1's ghost lived inside a folder named for the G1). They now live
# in ONE robot-partitioned store, `projects/policies/ghosts/<robot>/`, and this controller is
# just the robot-agnostic player. Worlds pass a BARE lut filename as a controllerArg, so the
# resolver below has to find it wherever it now lives.
_GHOST_STORE = _REPO / "projects" / "policies" / "ghosts"


def _resolve_lut(a):
    """A lut reference -> a real path. Accepts, in order: absolute; repo-relative;
    relative-to-this-controller (the pre-move convention); or a BARE FILENAME, which is
    looked up across every robot in the ghost store."""
    if os.path.isabs(a) and os.path.exists(a):
        return a
    for base in (_REPO, Path(__file__).resolve().parent):
        p = base / a
        if p.exists():
            return str(p)
    hits = sorted(_GHOST_STORE.glob(f"*/{os.path.basename(a)}")) + \
        sorted(_GHOST_STORE.glob(f"*/*/{os.path.basename(a)}"))   # */dance/*
    if hits:
        return str(hits[0])
    return a                                    # let the loader raise with the original name
...
# world can replay a specific lut without relying on env. Env still wins if GHOST_LUT is set.
# (G1_GHOST_LUT is the legacy spelling of GHOST_LUT -- the var was named for one robot too.)
if os.environ.get("G1_GHOST_LUT") and not os.environ.get("GHOST_LUT"):
    os.environ["GHOST_LUT"] = os.environ["G1_GHOST_LUT"]
if not os.environ.get("GHOST_LUT"):
    for _a in sys.argv[1:]:
        if isinstance(_a, str) and _a.endswith(".json"):
            os.environ["GHOST_LUT"] = _resolve_lut(_a)
            break
os.environ.setdefault("G1_GHOST_LUT", os.environ.get("GHOST_LUT", ""))   # legacy readers
...
def _ghostify(node, transparency, tint, depth=0, emissive=None, glow=0.0,
              rough=None, metal=None):
    """Recursively restyle every Shape under `node` so the ghost reads as a
    hologram rather than a second solid robot. Walks the scene tree via
    supervisor field access (children / endPoint); sets PBRAppearance
    transparency / baseColor / emissiveColor / emissiveIntensity / roughness /
    metalness in place. Returns #shapes hit.

    ⚠️ TRANSPARENCY FLICKERS (owner, 2026-07-12: "the shadow is flickering while
    moving around the real robot"). A humanoid is ~30 meshes that INTERSECT each
    other at every joint (thigh into pelvis, hand into forearm). Translucent
    surfaces do not write depth, so the renderer sorts them per-object and per
    frame -- as the ghost moves, the draw order of two intersecting translucent
    meshes SWAPS, and the overlap region flips which mesh is in front. That is
    the flicker, and no alpha value fixes it: it is inherent to sorted
    transparency on self-intersecting geometry.

    The fix is a SOLID ghost (transparency=0, the default now) that reads as a
    hologram through COLOUR instead of alpha: a pale tint plus an emissive glow.
    Depth-writing geometry has one unambiguous draw order, so it cannot flicker.
    G1_GHOST_ALPHA>0 restores the old translucent look (and its flicker)."""
    ...
    if type_name == "Shape":
        try:
            ...
            if app is not None:
                ...
                # HOLOGRAM WITHOUT ALPHA: self-lit so the ghost stays legible in
                # shadow and against the floor, and keeps a distinct read from
                # the real robot even though both are now opaque.
                if emissive is not None:
                    ef = app.getField("emissiveColor")
                    if ef is not None:
                        ef.setSFColor(list(emissive))
                    gf = app.getField("emissiveIntensity")
                    if gf is not None and glow > 0.0:
                        gf.setSFFloat(glow)
                if rough is not None:
                    rf = app.getField("roughness")
                    if rf is not None:
                        rf.setSFFloat(rough)
                if metal is not None:
                    mf = app.getField("metalness")
                    if mf is not None:
                        mf.setSFFloat(metal)
                n += 1
        except Exception:
...
def main() -> int:
    ...
    # HOLOGRAM LOOK -- SOLID, AND THAT IS NOT NEGOTIABLE (2026-07-12).
    #
    # THE MEASUREMENT, from the owner watching the live demo:
    #   alpha=0.6 (textured)      -> "the shadow is flickering while moving"
    #   alpha=0.0 (solid)         -> no flicker, but "just solid blue"
    #   alpha=0.45 (emissive)     -> "now it is flickering AGAIN"
    # The third case REFUTES the tempting theory that a uniform self-lit colour
    # makes the sort order invisible. It does not: ANY transparency on this model
    # flickers. Translucent surfaces skip the depth write, so the renderer sorts
    # them per object per frame, and a humanoid's meshes INTERSECT at every joint
    # -- the order of two overlapping meshes keeps swapping as the ghost moves.
    # No colour, alpha value, or emission setting fixes that; only depth-writing
    # (opaque) geometry has one unambiguous draw order. DO NOT re-litigate this
    # with another alpha value -- it has now been tried three times.
    #
    # So the hologram must be built WITHOUT alpha: a glowing cyan self-lit body
    # with a polished sheen that catches the sky IBL. It reads as a projection of
    # the robot rather than a second robot, and it cannot flicker.
    alpha = _genv("G1_GHOST_ALPHA", 0.0)
    tint_s = os.environ.get("G1_GHOST_TINT", "0.10,0.42,0.72")
    try:
        tint = tuple(float(x) for x in tint_s.split(","))
    except ValueError:
        tint = (0.10, 0.42, 0.72)
    emis_s = os.environ.get("G1_GHOST_EMISSIVE", "0.05,0.32,0.55")
    try:
        emissive = tuple(float(x) for x in emis_s.split(","))
    except ValueError:
        emissive = (0.05, 0.32, 0.55)
    glow = _genv("G1_GHOST_GLOW", 1.3)
    # A POLISHED, self-lit surface: low roughness + high metalness makes the ghost
    # pick up the sky IBL, so its form is read from moving highlights instead of
    # from see-through-ness. That is what stops an opaque ghost from looking like
    # flat blue paint (owner's complaint about the first solid build).
    n_shapes = _ghostify(self_node, alpha, tint, 0, emissive, glow,
                         _genv("G1_GHOST_ROUGH", 0.25), _genv("G1_GHOST_METAL", 0.75))
    sys.stderr.write(f"[g1_ghost] ghostified {n_shapes} shapes "
                     f"(alpha={alpha} emissive={emissive} glow={glow}: "
                     f"{'SOLID' if alpha <= 0 else 'EMISSIVE HOLOGRAM -- order-independent, no sort flicker'})\n")
    if _glog is not None:
        _glog.write(f"ghostified={n_shapes} alpha={alpha} tint={tint} emissive={emissive}\n")
