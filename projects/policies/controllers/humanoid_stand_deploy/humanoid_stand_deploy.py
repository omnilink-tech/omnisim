...
def main() -> int:
    ...
    for jn in joint_order:
        ...
        if jn in arm_hold:
            try:
                # Respect each URDF motor's declared capability.  Asking every
                # waist/arm joint for 120 Nm produced a page of warnings and did
                # not grant extra authority (G1 arms are 25 Nm, waist 88 Nm).
                m.setAvailableTorque(min(120.0, float(m.getMaxTorque())))
            except Exception:
                pass
            try:
