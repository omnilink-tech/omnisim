...
# ⚠️ EXPERIMENTAL 2x-SPEED VARIANT (2026-07-12) -- NOT the shipping demo (that is
# run_box_grasp.sh). Same suction physics, but the whole course runs on the retrained
# mid-clock pair (wr_navmid_stab dual-role walk+carry on ghost_walk_mid_v1, vx 0.2475 =
# 0.55x-retimed ghost_official_full_v3) at HARNESS_LAM0=0.70 (LESS crane assist than the
# 0.90 baseline -- the level the policy EARNED via HARNESS_GRAD_SURV graduation).
# MEASURED (8 full-course runs, 2026-07-12): delivery seated at 173-240 s vs 347 s slow
# (~2x); pick latches ~75% (touch-servo retries, GR_ENG_RETRY=5); given a pick, the box
# ends ON the stand ~50% -- the press-approach landing scatter at this speed
# (x 9.02-9.66, y -1.42..-1.80) exceeds the 0.61x0.44 slab. End-to-end ~50% vs the slow
# demo's 2/2. Known open fixes: wider delivery table (needs top ~0.9x0.6, but the carry
# approach line then crosses it) or a slow-approach hybrid (BATON per-zone speed).
# Original physics notes:
# SUCTION DELIVERY -- the G1 picks the box up with a SUCTION GRIPPER and the
...
COURSE="${COURSE:-walkto,3.1,-1.02;walkto,4.3,-0.98;stand,10;turn,14,40;stand,2;carryto,5.0,0.2;carryto,7.8,-1.42;carryto,7.9,-1.78;carryto,9.3,-1.62;stand,4;walkto,11.0,-2.6;stand,2;turn,20,70;stand,2;walkto,10.8,0.5;walkto,10.7,1.9;stand,0}"
...
bash projects/policies/training/run_walk_rl.sh "$DUR" "$TAG" deploy "$GUI" \
  ...
  GHOST_LUT_JSON=$G/ghost_walk_mid_v1_lut.json GHOST_RESIDUAL=0.100 \
  RES_POLICY=$R/wr_navmid_stab.pt \
  WHOLE_BODY=1 REF_OBS=1 REF_OBS_K=3 \
  "BATON_SPECIALISTS=carry|$R/wr_navmid_stab.pt|$G/ghost_walk_mid_v1_lut.json|0.2475;stand|$R/wr_stander.pt|$G/ghost_stand_v1_lut.json|0" \
  "BATON_COURSE=$COURSE" \
  BATON_COLD_HIDDEN=1 BATON_MORPH_TICKS=30 BATON_STAND_PIVOT=0 \
  "BATON_MODE_FILE=$MODEF" \
  PHYS_GRASP=1 GR_PICK_SEG=2 GR_PLACE_SEG=9 HAND_TRACK=1 \
  GR_SUCTION=1 GR_SUCTION_R=0.05 GR_SUCTION_DROP=0.01 GR_CLOSE_TICKS=100 \
  GR_ENG_PITCH=-0.77 GR_ENG_ELB=1.02 GR_ENG_RETRY=5 \
  GR_DROP_X=9.10 GR_DROP_Y=-1.70 GR_DROP_R=0.14 GR_DROP_DWELL=45 \
  GR_LOWER_TICKS=100 \
  GR_REL_X=9.10 GR_REL_Y=-1.68 GR_REL_RTOL=0.08 GR_REL_VMAX=0.12 GR_REL_PITCH=-0.65 GR_REL_ELB=1.18 \
  GR_PIN_F=30 GR_REL_ZMAX=0.66 GR_PAD_X=0.05 GR_PAD_Y=0.07 \
  ...
  HARNESS_KYAW=150 \
  HARNESS_LAM0=0.70 HARNESS_KP=600 HARNESS_KD=60 HARNESS_FY=400 HARNESS_FY_HEADING=1 HARNESS_ATT_HEADING=1 HARNESS_KZ=2000 HARNESS_DZ=150 \
  BATON_ARRIVE_R=0.50 VX_START=0.2475 VX_MAX=0.2475 VX_CURR_ITERS=1 POLICY_ARCH=lstm PPO_HID=256 ${EXTRA:-}
