...
"""Analytic forward + inverse kinematics for Unitree B2 legs.

THE GO2 PORT OF projects/policies/control/omniquad_kinematics.py -- identical 3-link
...
projects/robots/unitree/go2/urdf/b2.urdf (==unitree_ros go2_description):
...
Same closed-form IK math as omniquad_kinematics; `python go2_kinematics.py`
...
# ── Geometry (meters) -- from b2.urdf ───────────────────────────────────
L1 = 0.35             # thigh: thigh joint -> calf joint, z component magnitude
L2 = 0.35             # calf:  calf joint  -> foot tip
KNEE_X = 0.0          # B2 calf is inline with the thigh (no forward offset)
HIPY_OFFSET = 0.11973 # lateral offset of the thigh joint from the hip joint
...
def inverse_kinematics(
    ...
    Closed-form (same derivation as omniquad_kinematics.inverse_kinematics):
    ...
    if abs(cos_arg) > 1.0:
        return None
    # Go2's knee bends backward (γ < 0, valid range [-2.82,-0.43]): pick -acos.
