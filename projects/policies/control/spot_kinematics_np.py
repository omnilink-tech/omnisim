...
"""Vectorized numpy IK + FK for Spot — N envs at once.

Same closed-form analytic solution as `spot_kinematics.py` but every
...
Leg geometry constants are imported from `spot_kinematics` so the
...
def _selftest() -> None:
    ...
    from .spot_kinematics import inverse_kinematics, forward_kinematics
    ...
    for n in range(N):
        for li, leg in enumerate(LEG_ORDER):
            ...
            from .spot_kinematics import JointAngles
