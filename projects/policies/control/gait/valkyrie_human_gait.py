...
"""Human-like parametric gait model for the NASA Valkyrie (legs+torso).

Valkyrie is a heavy (~130 kg) 6-DOF-per-leg biped (hipYaw, hipRoll, hipPitch,
kneePitch, anklePitch, ankleRoll -- same topology as G1, camelCase names),
taller than G1 (~1.17 m leg reach). This is the G1 foot-space gait shadow
re-calibrated to Valkyrie's kinematics (see the constants block) and scaled to
its size + mass: slower cadence, smaller bob, and (because it HAS ankle-roll)
the LIPM lateral weight-transfer mode with ankle counter-rotation.
...
# -- Calibrated kinematics (Valkyrie; projects/policies/control/gait/tools/urdf_leg_calib.py,
# frozen 2026-06-24). Thigh 0.431 m, shank 0.406 m with a tiny backward shank
# offset; pitch axes +y so signs are all -1 (same family as G1). The stand
# cross-check matches to within the SHANK_OFF (0.025 rad). --
L1 = 0.43096          # thigh: hip_pitch anchor -> knee anchor, sagittal plane
L2 = 0.40640          # shank: knee anchor -> ankle_pitch anchor
THIGH_OFF = +0.00026  # thigh ~straight down at q=0
SHANK_OFF = -0.02546  # shank leans ~1.5 deg back relative to the thigh at q=0
...
HIP_DROP = 0.24620    # pelvis origin -> hip_pitch anchor, vertical drop
SOLE_DROP = 0.08800   # ankle_pitch anchor -> foot sole, foot flat

# Joint order matches the walk trainer / deploy controller LEGS_JOINTS (the
# gait's internal 13-slot layout; the ghost/trainer map slots to Valkyrie's
# camelCase joint names -- leftHipPitch, leftAnkleRoll, ...).
NJ = 13
...
class GaitParams:
    ...
    vx: float = 0.35           # forward speed (Valkyrie: heavy, walk slow)
    freq: float = 1.0          # per-leg cycle frequency (slow cadence, heavy robot)
    duty: float = 0.6          # stance fraction of the cycle (human ~0.6)
    step_height: float = 0.06  # swing foot apex clearance
    pelvis_height: float = 1.13   # MEAN pelvis z (reach 1.172; moderate-bent stance)
    bob: float = 0.018         # small vertical bob (heavy robot -> rigid arc)
    ...
    lateral: str = "lipm"      # Valkyrie HAS ankle-roll -> LIPM weight transfer
    #                            with ankle counter-rotation (feet stay flat)
    yaw: str = "human"         # natural hip internal/external rotation
    com_height: float = 0.95   # CoM height above the ankle for the LIPM omega
    step_width: float = 0.20   # nominal lateral foot separation (m): hip_y=0.102
    lat_hip_amp: float = 0.07  # (A) peak hip-roll for LIPM weight transfer (rad,
...
WINTER_STAND = (-0.30, 0.58, -0.305)  # hip, knee, ankle (flat foot incl. SHANK_OFF)
...
# ---------------------------------------------------------------------------
# Self-test (ANALYTIC FK). Valkyrie has no MuJoCo MJCF in-tree (its URDF will
...
LIM_LO = np.array([-2.42, -0.467, -0.414, -0.083, -0.93, -0.40, -2.42, -0.467, -0.414, -0.083, -0.93, -0.40, -1.33])
LIM_HI = np.array([ 1.62,  0.55,   1.10,   2.057,  0.65,  0.40,  1.62,  0.55,   1.10,   2.057,  0.65,  0.40,  1.18])


def calibrate():
    print("Valkyrie gait constants are frozen from the URDF. Re-derive with:")
    print("  python projects/policies/control/gait/tools/urdf_leg_calib.py valkyrie")
...
def _self_test():
    ...
    for dx in np.linspace(-0.30, 0.30, 13):
        for dz in np.linspace(0.62, 0.82, 9):
    ...
    for tag, pm in (("ik", GaitParams()),
                    ...
                    ("3D", GaitParams(style="winter", lateral="lipm", yaw="human"))):
    ...
    pc = GaitParams(style="winter", lateral="lipm", yaw="human")
