...
"""Foot-space parametric trot gait model for Boston Dynamics Spot (12-DOF).

THE SPOT PORT OF projects/policies/control/gait/g1_human_gait.py -- same architecture,
quadruped trot instead of biped walk. The gait is PLANNED IN FOOT SPACE
and realized through the closed-form 3-DOF leg IK
(projects/policies/control/spot_kinematics.py math, vectorized here):
...
`python spot_trot_gait.py` self-tests both against each other and the
...
# ── Geometry (meters) -- from spot.urdf via spot_kinematics.py ──────────
...
# creep order FL -> RR -> FR -> RL. duty>=0.75 => always >=3 feet planted =>
# statically stable on inclines (the diagonal trot rolls when pitched on a slope).
...
class GaitParams:
    ...
    freq: float = 1.2          # per-leg cycle frequency (crawl; keeps stride/steer margin)
    duty: float = 0.85         # stance fraction (>=0.75 = at most one foot swings)
    step_height: float = 0.06  # swing apex clearance (0.08 over-bends Spot's knee)
...
QS_PHASE = 0.05 * 2.0 * math.pi   # crawl: 0.05 sits in an all-feet-down window
...
def _leg_ik(leg, tx, ty, tz):
    ...
    Same math as spot_kinematics.inverse_kinematics; clamps the acos
...
def _yaw_sweep(leg_phi, p: GaitParams, wz):
    ...
    straight gait). Same mechanism as spot_gait's wz fix (c3c7aff9)."""
    if wz == 0.0:
        return 0.0
...
def _self_test():
    ...
    from projects.policies.control.spot_kinematics import (
    ...
    # The CRAWL's long stride leaves less steering range than the trot; the
    # hill walks straight (heading-hold caps wz ~0.2), so test that band.
    for wz in (-0.2, 0.2):
    ...
    wzv = torch.full((64,), 0.15)
    legs_wz_t, _ = targets_torch(torch.tensor(phases, dtype=torch.float32), p,
                                 wz_t=wzv)
    legs_wz_np = np.stack([targets_np(ph, p, wz=0.15)[0] for ph in phases])
    ...
    mjcf = r"C:\tmp\spot_newton_fixed2.xml"
    try:
        import mujoco
