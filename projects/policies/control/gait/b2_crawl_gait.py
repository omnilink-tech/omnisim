...
"""Foot-space parametric CRAWL (static-walk) gait model for Unitree B2 (12-DOF).

The trot gait's twin, re-phased for STATIC STABILITY on inclines: legs lift one
at a time (0.25-cycle apart) at high duty (0.85) so >=3 feet are always planted
-- the diagonal trot's 2-foot support phases roll-fall when pitched on a slope.
Same foot-space machinery + IK as b2_trot_gait.py; only the phase offsets, duty,
freq, step-height and QS_PHASE differ. Used by the hill-walk tracker/deploy via
--gait-module / B2_GAIT_MODULE. Original docstring follows.

Foot-space parametric trot gait model for Unitree B2 (12-DOF).

THE GO2 PORT OF projects/policies/control/gait/omniquad_trot_gait.py -- identical architecture
(quadruped trot planned in FOOT SPACE, realized through the closed-form 3-DOF
leg IK of projects/policies/control/b2_kinematics.py), only the geometry constants
...
# ── Geometry (meters) -- from b2.urdf via b2_kinematics.py ─────────────
L1 = 0.35               # thigh: thigh joint -> calf joint, z magnitude
L2 = 0.35               # calf:  calf joint  -> foot tip
KNEE_X = 0.0            # B2 calf is inline with the thigh (no offset)
HIPY_OFFSET = 0.11973   # lateral offset of the thigh plane from the hip joint
...
# CRAWL (static walk) phasing: lift ONE leg at a time, 0.25 cycle apart, in the
# creep order LF -> RH -> RF -> LH (FL, RR, FR, RL). With duty>=0.75 only one
# foot swings at a time => always >=3 feet planted => statically stable, which
# the diagonal trot is NOT when pitched on an incline. (Name kept TROT_OFFSET so
# the shared targets_np/torch bodies are unchanged.)
TROT_OFFSET = {"FL": 0.0, "RR": 0.25, "FR": 0.5, "RL": 0.75}
...
class GaitParams:
    ...
    B2-scaled defaults: tall body (0.50 m), long 0.35 m links, bigger
    swing clearance, slower cadence than Go2 for the long heavy legs.
    """
    vx: float = 0.35           # forward speed the reference encodes (slower crawl)
    freq: float = 1.0          # per-leg cycle frequency (deliberate crawl cadence)
    duty: float = 0.85         # stance fraction (>=0.75 = at most one foot swings)
    step_height: float = 0.10  # swing apex clearance (clears the rising ramp)
    body_height: float = 0.50  # body z above ground; feet at -body_height.
    lateral_y: float = 0.26    # WIDE stance for roll stability on the slope
    #                            (vs 0.192 nominal; bigger lateral support base)
    x_front: float = 0.3285    # stance x of the FRONT feet (under the hips)
    x_rear: float = -0.3285    # stance x of the REAR feet
    ...
    def step_len(self) -> float:
        # Stance slide is EXACTLY -vx: L = vx * duty / freq (see omniquad port).
        return self.vx * self.duty / self.freq


# Gait clock phase (rad) at which ALL FOUR feet are in stance. For the crawl
# (duty 0.85, 0.25-spaced offsets) the all-down windows are [0,0.10) etc.; 0.05
# sits in the first one.
...
def _leg_ik(leg, tx, ty, tz):
    ...
    Same math as b2_kinematics.inverse_kinematics; clamps the acos args."""
...
def _yaw_sweep(leg_phi, p: GaitParams, wz):
    """Tangential stance sweep for yaw-rate steering (see omniquad port)."""
    if wz == 0.0:
        return 0.0
...
def speed_scale(legs, swings, nominal, s):
    ...
    output toward the STANDING pose by s = vx_cmd / vx_nominal (see omniquad port).
...
def _self_test():
    ...
    from projects.policies.control.b2_kinematics import (
    ...
    #    hip [-0.87,0.87], thigh [-0.94,4.69], calf [-2.82,-0.43].
    lo = np.array([-0.87, -0.94, -2.82] * 4)
    hi = np.array([+0.87, +4.69, -0.43] * 4)
