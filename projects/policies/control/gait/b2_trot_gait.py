...
class GaitParams:
    ...
    """
    vx: float = 0.5            # forward speed the reference encodes
    freq: float = 1.3          # per-leg cycle frequency (long legs step slower)
    duty: float = 0.6          # stance fraction (>0.5 = 4-foot overlap)
    step_height: float = 0.08  # swing apex clearance
    body_height: float = 0.50  # body z above ground; feet at -body_height.
    lateral_y: float = 0.192   # stance |y| of each foot (abduction ~0:
    #                            0.072 hip mount + 0.11973 thigh offset)
...
# Gait clock phase (rad) at which ALL FOUR feet are in stance (see omniquad port).
