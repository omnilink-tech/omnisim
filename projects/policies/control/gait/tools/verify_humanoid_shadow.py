...
The URDFs (H1 bad mesh path, h1 .dae) will not load in MuJoCo, so this
...
Usage: python projects/policies/control/gait/tools/verify_humanoid_shadow.py g1|h1|h1
"""
import math
import os
import sys
...
def emit_mjcf(robot):
    c = CFG[robot]
    links, joints, children, root_link = parse_urdf(_urdf_path(robot, c["urdf"]))
    ...
    def body_xml(link, indent):
        ...
        for ch in children.get(link, []):
            ...
            # 'foot'/'ankle_roll'. h1's 'leftFoot' (camelCase) matches neither,
