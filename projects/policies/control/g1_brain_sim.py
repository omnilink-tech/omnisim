...
The research instrument: runs the canonical brain (projects/policies/control/g1_brain.py)
against the deploy-matched plant (g1_full_kp100.mjcf.xml, kp=100/kd=5, 16 ms control,
4x4 ms substeps -- verified identical to the trainer) in PLAIN MuJoCo. Seconds per
run, headless, scriptable -- so the brain's feedback gains can be tuned without ever
launching OmniSim. OmniSim is only for milestone validation.

See docs/developer/g1-deterministic-brain.md for the architecture + research log.

Usage
-----
  python projects/policies/control/g1_brain_sim.py                        # default brain, 20 s
  python projects/policies/control/g1_brain_sim.py --controller openloop  # zero-gain = ghost-in-physics baseline
  python projects/policies/control/g1_brain_sim.py --seconds 30 --render  # watch it (needs a display)
  python projects/policies/control/g1_brain_sim.py --set k_cp_sag=0.6 k_ank_r=2.0   # override BrainConfig gains
  python projects/policies/control/g1_brain_sim.py --sweep k_cp_lat=0.6,0.9,1.2,1.6  # 1-D gain sweep
  python projects/policies/control/g1_brain_sim.py --quiet --sweep freq=1.3,1.8,2.4
...
from projects.policies.control.g1_brain import (  # noqa: E402
...
def _zero_gain(cfg: BrainConfig) -> BrainConfig:
    ...
    Generic across BrainConfig revisions: a 'gain' is any float field named k_*/kd_*
    plus the adaptive-phase tilt gain. Clamps/limits/style/vx/freq/com_height are
    left untouched (they shape the feedforward, not the feedback)."""
    for f in fields(cfg):
        if f.type not in ("float", float):
            continue
        if f.name.startswith(("k_", "kd_")) or f.name == "phase_gate_tilt":
