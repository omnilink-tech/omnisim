...
# equivalent in docs). Headless-verified 2026-07-17 on machine 9722d23d12a3:
# 0.120 m/s over 15.04 s, zero backward samples, max |yaw| 0.148 rad. This is
# still the weight-bearing lam=0.9 balance harness, not free-standing walking.
...
if ($git) {
  $bashCandidates += (Join-Path (Split-Path -Parent (Split-Path -Parent $git.Source)) "bin\bash.exe")
}
$bash = $bashCandidates | Where-Object { $_ -and (Test-Path -LiteralPath $_) } | Select-Object -First 1
if (-not $bash) { throw "Git Bash was not found. WSL bash cannot launch the Windows OmniSim binary." }
...
if (-not $python) { throw "Native python.exe was not found. Set up the OmniSim controller Python first." }
$env:OMNISIM_PYTHON = $python
$env:PATH = "$(Split-Path -Parent $python);$env:PATH"

$RepoFwd = $Repo -replace "\\","/"
$RunMode = if ($Headless) { "headless" } else { "gui" }
# Use a login shell so Git for Windows provisions /usr/bin (dirname, mkdir,
# date, sed, tail) even when the parent PowerShell has a restricted PATH.  The
# "$@" trampoline retains every env assignment as one literal argument.
& $bash -lc '"$@"' -- "projects/policies/training/run_walk_rl.sh" $Duration flagshipdemo deploy $RunMode `
  ...
  GHOST_LUT_JSON=$RepoFwd/projects/policies/ghosts/g1/ghost_official_full_v3_lut.json `
  G1_GHOST_LUT=$RepoFwd/projects/policies/ghosts/g1/ghost_official_full_v3_lut.json `
