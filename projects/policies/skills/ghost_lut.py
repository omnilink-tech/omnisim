...
``projects/policies/ghosts/g1/ghost_*_lut.json``. Historically the
...
# Identity + attestation (schema 2). `robot`/`joints` are REQUIRED; the rest are the richer
# metadata the quadruped lut pioneered in research/ and that the flagship G1 luts never had:
# a structured provenance dict, the builder's own gate results, and the stamped validator
# verdict. Generalizing meant bringing that schema BACK to the flagship, not forking it.
_IDENTITY_KEYS = {"robot", "joints", "joint_order", "provenance", "gates", "validator",
                  "gait", "ffdq_lut", "ffdq_kp"}

_KNOWN_KEYS = set(_SCALARS) | {c.name for c in CHANNELS} | _COMPANION_KEYS | _IDENTITY_KEYS
...
class GhostLut:
    ...
    # ---- identity: a ghost must say WHAT ROBOT IT IS FOR, in its own joint names --------
    @property
    def robot(self) -> str:
        return str(self.raw.get("robot", "") or "").strip().lower()

    @property
    def joints(self) -> list[str]:
        return list(self.raw.get("joints", []) or [])

    @property
    def validator(self) -> dict:
        """The stamped ghost_validator verdict, if any: {verdict, date, gates{}}.

        The verdict used to live only as PROSE in a skill manifest ("validator": "PASS"),
        i.e. a human retyping a result next to an artifact that could change underneath it.
        `ghost_validator.py --stamp` writes it INTO the lut instead.
        """
        return dict(self.raw.get("validator", {}) or {})
    ...
    def validate(self, strict: bool = False) -> list[Issue]:
        ...
        # ---- IDENTITY (schema 2) ------------------------------------------------------
        # A ghost that does not declare its robot and its joint NAMES cannot be checked
        # against a model, cannot be corridor-mapped by role, and cannot be blended safely
        # -- every downstream tool has to GUESS, and the guess it made was always "G1,
        # positional". That guess is how an H1 ghost got limit-checked against a G1 URDF
        # and how a 12-wide quadruped lut looked exactly like a 12-wide humanoid one.
        if not self.robot:
            issues.append(Issue("error", "no 'robot' key -- a ghost must declare what it is FOR"))
        if not self.joints:
            issues.append(Issue("error", "no 'joints' key -- a ghost must name its own leg "
                                         "joints (never rely on the positional fallback)"))
        # Structured provenance / a stamped verdict are RECOMMENDED, not fabricated: the
        # legacy luts carry free-text `source` only, and inventing a provenance dict for them
        # would be worse than admitting it is missing.
        if not raw.get("provenance"):
            issues.append(Issue("warn", "no structured 'provenance' (free-text 'source' only)"))
        if not self.validator:
            issues.append(Issue("warn", "no stamped 'validator' verdict "
                                        "(run ghost_validator.py --stamp)"))
        ...
        for chan in CHANNELS:
            ...
            # (3) leg DOF must match the DECLARED joint list -- not a hardcoded robot count.
            #     (This used to read "neither 12 (G1) nor 10 (H1)", which is the whole bug in
            #     miniature: the schema enumerated the robots it knew instead of asking the lut
            #     what it was for. A 12-wide Go2 lut sailed through as if it were a G1.)
            if chan.name == "leg_lut" and common is not None and self.joints \
                    and common != len(self.joints):
                issues.append(Issue(
                    "error",
                    f"leg_lut width {common} != len(joints)={len(self.joints)} for "
                    f"robot={self.robot!r} -- the lut's columns and its declared joints disagree"))
...
def ghost_store() -> Path:
    """THE ghost store: projects/policies/ghosts/<robot>/[dance/]*.json

    It used to be `controllers/g1_ghost/` -- a directory named for ONE ROBOT, inside a
    CONTROLLER, that nonetheless held every ghost including the H1's. The sweep below globbed
    only that directory, so it reported "92 luts, 0 errors" while never once looking at a
    quadruped ghost (those lived in research/). One store, partitioned by robot, swept whole.
    """
    return _repo_root() / "projects" / "policies" / "ghosts"


def discover(robot: str | None = None) -> list[Path]:
    """Every ghost lut in the store (optionally for one robot), incl. nested groups (dance/)."""
    root = ghost_store()
    pats = [f"{robot or '*'}/*.json", f"{robot or '*'}/*/*.json"]
    return sorted({p for pat in pats for p in root.glob(pat)})


def robot_of_path(p: Path) -> str:
    """The owning robot = the store partition the lut sits in."""
    try:
        return p.resolve().relative_to(ghost_store().resolve()).parts[0]
