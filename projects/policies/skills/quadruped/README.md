...
| `g1_walk` / `trot` / `turn` | **RL** | shipped for OmniQuad / Go2 / B2; re-hosted Unitree | residual PPO on a model-based gait |
| `get_up` / recover | RL (Shadowing) | OmniQuad/B2 rise solved; durable hold fragile | ghost-tracking pipeline |
...
**Shipped skills:** `go2_walk` and `omniquad_walk` (RL residual PPO on a model-based trot;
