...
⚠️ FROZEN (2026-07-13). Nothing in the repo imports this file; it is kept because it is a REAL
alternative method (from-scratch consensus RL, no ghost), not a stale duplicate of the shipped
recipe. Two things to know before you touch it:
  * Its inline BATON block is a FOSSIL. The live policy-switching runtime is
    `projects/policies/training/baton.py` -- ONE arbiter, shared by the course and schedule
    paths. Do not "fix" BATON here; do not copy it out of here.
  * It predates the ghost-store move and the robot registry, so its paths and its G1-slot
    assumptions are the OLD ones.
If you resurrect this trainer, port it onto baton.py + robot_registry.py first.
