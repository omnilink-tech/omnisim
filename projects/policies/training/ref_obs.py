    ...
    for key in ("pi.0.weight", "enc.weight", "vf.0.weight"):
        if key in sd and key in cur:
            ...
            if new_in > old_in and sd[key].shape[0] == cur[key].shape[0]:
                ...
                sd[key] = w
                expanded = True
    # STRUCTURAL heads the checkpoint predates (EST_HEAD=1 adds est.*; POLICY_HEAD=mlp reshapes pi.*):
    # keep the fresh net's init for anything the checkpoint simply does not have, rather than dying on
    # a missing key. A shape MISMATCH is still fatal -- that is a genuinely incompatible checkpoint.
    fresh = []
    for key, val in cur.items():
        if key not in sd:
            sd[key] = val; fresh.append(key)
        elif sd[key].shape != val.shape:
            raise ValueError("checkpoint %s: %s has shape %s, net expects %s -- incompatible "
                             "(wrong ACT_DIM/PPO_HID/POLICY_HEAD?)" % (path, key, tuple(sd[key].shape),
                                                                       tuple(val.shape)))
    if fresh:
        print("[ref_obs] checkpoint predates %d param(s) -- keeping fresh init: %s"
              % (len(fresh), ", ".join(sorted(fresh)[:6])))
