...
class G1IK:
    def __init__(self, carry=False, carry_pose=None):
        ...
        self.arm_pose = (carry_pose or CARRY_ARM) if carry else ARM_NOMINAL
...
def build(deg, inc, cycle_s, lat, pelvis_z, lift, shift, frames_per_phase, waist_lead=0.0, carry=False,
          solve_com=False, com_frac=1.0, com_ramp="linear", carry_pose=None,
          motion_profile="legacy", turn_arm_swing=0.0):
    ik = G1IK(carry=carry, carry_pose=carry_pose)
    ...
    for ki in range(len(kfs) - 1):
        ...
        for f in range(frames_per_phase):
            ...
            # A humanoid does not snap either its pelvis or swing foot onto a new heading at a
            # constant rate.  The natural profile is minimum-jerk (zero velocity and acceleration
            # at both phase boundaries); it makes the four visible steps read as deliberate weight
            # transfers instead of a turntable animation.  Keep `legacy` for reproducibility of
            # already-trained turners.
            ue = u * u * u * (10.0 + u * (-15.0 + 6.0 * u)) if motion_profile == "natural" else u
            base_yaw = wrap(by0 + wrap(by1 - by0) * ue)
            swing = sw1
            ...
            if swing == "R":
                ...
                Rt_xy = r_from + (r_to - r_from) * ue
                zlift = lift * (math.sin(math.pi * u) ** (2 if motion_profile == "natural" else 1))
                Rt_pos = np.array([Rt_xy[0], Rt_xy[1], zlift]); Rt_yaw = wrap(Ry0 + wrap(Ry1 - Ry0) * ue)
                ...
                Lt_xy = l_from + (l_to - l_from) * ue
                zlift = lift * (math.sin(math.pi * u) ** (2 if motion_profile == "natural" else 1))
                Lt_pos = np.array([Lt_xy[0], Lt_xy[1], zlift]); Lt_yaw = wrap(Ly0 + wrap(Ly1 - Ly0) * ue)
            ...
            base_xy = _wtgt(w0) + (_wtgt(w1) - _wtgt(w0)) * (ue if motion_profile == "natural" else u)
            if solve_com:
                ...
                if com_ramp == "minimum-jerk":
                    us = u * u * u * (10.0 + u * (-15.0 + 6.0 * u))
                else:
            ...
            if turn_arm_swing and not carry:
                # Small counter-swing: the shoulders lead opposite one another through the turn,
                # then return exactly to neutral.  This is deliberately subtle; the feet and COM
                # remain the source of yaw authority.
                _prog = max(0.0, min(1.0, base_yaw / (math.radians(deg) + 1e-9)))
                _as = float(turn_arm_swing) * math.sin(math.pi * _prog)
                _wb[13] += _as
                _wb[18] -= _as
    ...
    nb = len(leg_lut)
    _cp = carry_pose or CARRY_ARM
    _shpL = _cp["left_shoulder_pitch_joint"] if carry else ARM_NOMINAL["left_shoulder_pitch_joint"]
    _shpR = _cp["right_shoulder_pitch_joint"] if carry else ARM_NOMINAL["right_shoulder_pitch_joint"]
    lut = {"robot": "g1", "joints": LEG_JOINTS,
           "nb": nb, "freq": 1.0 / cycle_s, "cycle_s": cycle_s, "vx": 0.0, "seq": True,
           "hold_end": True, "arm_A": 0.0, "leg_lut": leg_lut, "wb_lut": wb_lut,
           "att_lut": att_lut, "root_lut": root_lut,
           "arm_lut": [[_shpL, _shpR]] * nb,
           "wb_joints": WB_JOINTS,
           "source": "build_step_turn_ghost.py: footstep-planned + IK, COM-over-support (achievable by construction)"
                     + ("; natural minimum-jerk step-around profile" if motion_profile == "natural" else "")}
    if carry:   # carry arms: bent elbows + hands-together shoulders (matches ghost_carry_v1_lut; SHRY at train)
        lut["elbow_lut"] = [[_cp["left_elbow_joint"], _cp["right_elbow_joint"]]] * nb
        lut["shroll"] = _cp["left_shoulder_roll_joint"]
        lut["shyaw"] = _cp["left_shoulder_yaw_joint"]
        lut["carry_pose"] = _cp
        lut["carry"] = True
...
def main():
    ...
    ap.add_argument("out", nargs="?", default="projects/policies/ghosts/g1/ghost_step_turn_lut.json")
    ...
    ap.add_argument("--carry-pitch", type=float, default=CARRY_SHPITCH,
                    help="absolute shoulder-pitch target for both carry arms")
    ap.add_argument("--carry-elbow", type=float, default=CARRY_ELBOW,
                    help="absolute elbow target for both carry arms")
    ap.add_argument("--carry-roll", type=float, default=CARRY_SHROLL,
                    help="left shoulder-roll target; right is mirrored")
    ap.add_argument("--carry-yaw", type=float, default=CARRY_SHYAW,
                    help="left shoulder-yaw target; right is mirrored")
    ...
    ap.add_argument("--com-ramp", choices=("linear", "smoothstep", "minimum-jerk"), default="linear",
    ...
    ap.add_argument("--motion-profile", choices=("legacy", "natural"), default="legacy",
                    help="natural uses minimum-jerk pelvis/foot yaw and translation plus a C1 swing-height arc")
    ap.add_argument("--turn-arm-swing", type=float, default=0.0,
                    help="empty-hand natural turn only: subtle anti-phase shoulder-pitch counter-swing (rad)")
    ...
    if a.carry:
        carry_pose = {
            "left_shoulder_pitch_joint": a.carry_pitch,
            "left_shoulder_roll_joint": a.carry_roll,
            "left_shoulder_yaw_joint": a.carry_yaw,
            "left_elbow_joint": a.carry_elbow,
            "left_wrist_roll_joint": 0.0,
            "right_shoulder_pitch_joint": a.carry_pitch,
            "right_shoulder_roll_joint": -a.carry_roll,
            "right_shoulder_yaw_joint": -a.carry_yaw,
            "right_elbow_joint": a.carry_elbow,
            "right_wrist_roll_joint": 0.0,
        }
    lut, ikerr, com = build(a.deg, a.inc, a.cycle_s, a.lat, a.pelvis_z, a.lift,
                            a.shift, a.fpp, a.waist_lead, a.carry,
                            a.solve_com, a.com_frac, a.com_ramp, carry_pose,
                            a.motion_profile, a.turn_arm_swing)
    if a.vby_budget > 0:
