...
Unitree G1: live-verified durable walking on the lambda=0.9 weight-bearing balance harness,
WBMATCH 0.913 vs an owner-approved reference, zero sim-to-deploy gap by construction). This is
not a free-standing humanoid walking claim.
...
| `runs/g1_walk_mlp.npz`, `runs/g1_walk_res.npz` | the evolution-strategy trainer's two flat parameter vectors (`theta` 588 and 144 float32 + a 3-element `meta`, weights only). `g1_walk_res.npz` is the residual `run_walk_rl.sh` loads by default (`RES_POLICY`); `g1_walk_mlp.npz` is the base MLP policy it was trained against. Both are covered by [`runs/PROVENANCE.md`](runs/PROVENANCE.md). |
...
[`projects/policies/ghosts/g1/`](../ghosts/g1/).
...
bash projects/policies/training/run_walk_rl.sh 130 ghost_demo deploy gui \
  ...
  WALK_WORLD=projects/policies/worlds/g1_walk_ghost_wide.omniworld WHOLE_BODY=1 STAND_SEED=1 STAND_Z=0.75 STAND_POSE=unitree \
  PPO_HID=256 GHOST_LUT_JSON=$OMNISIM_HOME/projects/policies/ghosts/g1/ghost_official_full_v3_lut.json \
  G1_GHOST_LUT=$OMNISIM_HOME/projects/policies/ghosts/g1/ghost_official_full_v3_lut.json \
  ...
  RES_POLICY=$OMNISIM_HOME/projects/policies/training/runs/wr_decent_walker.pt
```
...
`WALK_WORLD=projects/policies/worlds/g1_walk_orig.omniworld` and drop the two `G1_GHOST_*` envs. The
...
bash projects/policies/training/run_walk_rl.sh 900 myeval train headless \
  WALK_WORLD=projects/policies/worlds/g1_walk_orig.omniworld WHOLE_BODY=1 STAND_SEED=1 STAND_Z=0.75 STAND_POSE=unitree \
  PPO_NENV=2048 PPO_HID=256 EVAL_ONLY=1 EVAL_H=1500 EVAL_IC=0.02 OBS_NOISE=0.01 MOTOR_RAND=0.03 \
  GHOST_LUT_JSON=$OMNISIM_HOME/projects/policies/ghosts/g1/ghost_unitree_lut.json \
  GHOST_METRIC_JSON=$OMNISIM_HOME/projects/policies/ghosts/g1/ghost_v3c_lut.json \
...
bash projects/policies/training/run_walk_rl.sh 1800 demo deploy gui \
  WALK_WORLD=projects/policies/worlds/g1_walk_ghost2.omniworld ... \
  OMNISIM_INENGINE_PYMOD=projects.policies.training.g1_walk_recipe:g1_walk_recipe_deploy \
