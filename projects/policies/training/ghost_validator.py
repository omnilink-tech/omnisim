...
def _limits_from_model():
    """Joint position/velocity limits from the SPEC's prim URDF (the single source of truth)."""
    try:
        spec = json.load(open(os.path.join(_RT, "projects/policies/research/backends/g1_physics.json")))
        u = spec["urdf"]
        if isinstance(u, dict):
            u = u.get("prim") or u.get("full")
        urdf = u if os.path.isabs(u) else os.path.join(_RT, u)
        import xml.etree.ElementTree as ET
        root = ET.parse(urdf).getroot()
        lim = {}
        for j in root.iter("joint"):
            nm = j.get("name"); li = j.find("limit")
            if nm and li is not None:
                lim[nm] = (float(li.get("lower", -99)), float(li.get("upper", 99)),
                           float(li.get("velocity", 99)))
        return lim
    except Exception as e:
        print(f"  [limits] model unavailable ({e!r}) -- T0 limit checks skipped")
        return {}
...
def validate(path, baseline_path=None):
    ...
    global JOINTS
    if "joints" in d:                       # self-describing lut (H1, future robots)
        JOINTS = list(d["joints"])
    leg = np.asarray(d["leg_lut"], np.float64)
    nb = int(d["nb"]); freq = float(d.get("freq", 1.25))
    dt_bin = (1.0 / freq) / nb
    rep = Report()
    print(f"GHOST VALIDATOR: {os.path.basename(path)}  (nb={nb} freq={freq}Hz "
          f"vx={d.get('vx', '?')})\n  source: {d.get('source', 'MISSING')[:100]}")

    # ── T0: kinematic legality ──────────────────────────────────────────────
    lim = _limits_from_model()
    if lim:
        hard, soft = [], []
        for i, jn in enumerate(JOINTS):
            lo, hi, _ = lim.get(jn, (-99, 99, 99))
    ...
    rep.add("WARN" if hi_e > 0.08 else "ok", "T0.harmonic-jitter",
            f"energy above 8th harmonic: {hi_e * 100:.1f}% (clean recorded gaits: <5%)")
    # L/R symmetry: right half-cycle-shifted should mirror left (pitch-plane joints)
    half = nb // 2
    sym_err = []
    _n = len(JOINTS) // 2
    _pairs = [(i, i + _n, 1.0) for i, jn in enumerate(JOINTS[:_n])
              if any(k in jn for k in ('hip_pitch', 'knee', 'ankle_pitch', 'ankle_joint'))]
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    sys.exit(0 if validate(args.ghost, args.baseline) != "FAIL" else 1)
