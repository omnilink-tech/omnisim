...
def _seed_legs():
    ...
    if os.environ.get("STAND_POSE_LEGS"):
        # explicit 12-value seed (2026-07-11, seq-ghost entry-state parity): the batched seq
        ...
        return np.array([float(v) for v in os.environ["STAND_POSE_LEGS"].split(",")], np.float32)
...
def _walk_recipe_train(world):
    ...
    if _glj:
        ...
        ghost_ffdq_t = None
        if _i("GHOST_FF", 0) and "ffdq_lut" in _gd:
...
def _deploy_setup(world):
    ...
    if (_bp and _bl and os.path.exists(_bp) and os.path.exists(_bl)) or os.environ.get("BATON_SPECIALISTS"):
        try:
            ...
            _reg = {"walk": {
                ...
                "tables": {
                    "glut": world._wr_glut.copy(), "ffdq": getattr(world, "_wr_ffdq", None),
    ...
    if _tck and _tlu and os.path.exists(_tck) and os.path.exists(_tlu):
        try:
            ...
            _t_seqcmd = _t_seqyaw = _t_yawflat = None; _t_seqnb = int(_tgd["nb"]); _t_seqhold = bool(_tgd.get("hold_end", False))
            if _tgd.get("seq") and "root_lut" in _tgd:
                ...
                # TURN-LOOP plateau table: the step-turn ghost is a staircase of 15-deg mini-pivots,
                # each starting/ending on the same feet-together stand (yaw stationary <=> both feet
                # planted, by the ghost's own construction). A partial replay pass may RESTART at any
                # plateau start -- the pose there matches the end-hold the robot is settled in
                # (measured max joint delta 0.067 rad).
                _tg9 = np.abs(np.gradient(_t_seqyaw.astype(np.float64)))
                _tf9 = _tg9 < 1e-4
                for _s9 in (-2, -1, 1, 2):
                    _tf9 = _tf9 & np.roll(_tg9 < 1e-4, _s9)
                _t_yawflat = []
                _in9 = False
                for _b9 in range(len(_tf9)):
                    if _tf9[_b9] and not _in9:
                        _t_yawflat.append((int(_b9), float(np.degrees(_t_seqyaw[_b9]))))
                        _in9 = True
                    elif not _tf9[_b9]:
                        _in9 = False
            world._wr_turnctx = {
...
def g1_walk_recipe_deploy(world):
    ...
    if _course_env and world._wr_baton is not None:
        ...
        if _seg[0] in ("walkto", "carryto"):
            ...
            _arrived9 = math.hypot(_dx9, _dy9) < _f("BATON_ARRIVE_R", 0.55)
            ...
            if _arrived9 and _ci + 1 < len(world._wr_course):
        ...
        if not getattr(world, "_wr_in_turn", False):
            ...
            world._wr_glut = _effc("glut"); world._wr_ffdq = None   # specialist luts carry no ffdq
    ...
    if _i("TURN_MODE", 0) or getattr(world, "_wr_turn_active", False):
        ...
        yaw_obs = yaw
    _av9 = 0.25 * qv[3:6]
    if _i("TURN_OBS_RELATIVE", 0) and getattr(world, "_wr_in_turn", False):
        # ⛔ OOD HEADING BAND (2026-07-10, measured 6/6): the angular-rate obs are WORLD-frame and
        # the turner only ever trained across its ghost's 0->90 deg absolute span -- every shuttle
        # corner-2 fall happened within ~5-25 deg of the robot crossing heading 90 (103/106/113/
        # ~150 deg; an entry at 93 spun immediately), while the 0->90 corner never failed. Rotate
        # the world angular rates by the turn's ENTRY yaw so any corner reads like a turn from
        # heading 0 -- exactly the trained presentation. (Everything else in the obs is already
        # body-frame or routine-relative; yaw_obs is 0 during turns.)
    ...
    if world._wr_whole:                                       # legs + waist + arms, by newton dof
        ...
        if world._wr_gres > 0:                                # legs: ghost feedforward + bounded residual
            ...
            # physics") ─────────────────────────────────────────────────────────────────────────
            # The box is a REAL Newton body (g1_box_grasp.wbt); nothing ever writes its state.
            ...
            if _i("PHYS_GRASP", 0) and getattr(world, "_wr_armnd", None) and getattr(world, "_wr_elbnd", None) \
                ...
                if getattr(world, "_wr_gr", None) is None:
                    ...
                    R._log(world, "PHYS-GRASP armed: box body=%s (friction grip, zero writes)" % (_bxb,))
                    # ── SUCTION GRIPPER (owner 2026-07-11): a contact-free FORCE COUPLING -- the
                    # honest physics of a suction/sticky cup. When engaged, a stiff spring-damper
                    ...
                    # -- so the box is a REAL Newton body that swings and loads the balance, and
                    # robot<->box CONTACT never happens (engage at a standoff): the stack-x-contact
                    # NaN path is bypassed entirely. GR_SUCTION=1 arms it.
                ...
                if _st == "idle" and _ci9 == _i("GR_PICK_SEG", 1) and _el9 >= 0 and \
                    ...
                    # SUCTION engage: hold the reach pose until the cup (hand centroid) is within
                    # GR_SUCTION_R of the box-top anchor, then latch the coupling ON -- no contact
                    # ever needed. Hard timeout -> abort to withdraw (box left in place).
                    _SKe = getattr(world, "_wr_suck", None)
                    if _SKe is not None and _SKe.get("gap", 9.9) < _f("GR_SUCTION_R", 0.08):
                        _SKe["on"] = True
                        _G["state"] = "lift"; _G["t0"] = tt
                        R._log(world, "SUCTION ON at t=%d (gap %.3f m)" % (tt, _SKe["gap"]))
                    ...
                    # consecutive ticks over the drop target while the robot presses against
                    # the delivery stand (the press is the STABLE hover; a stand-settle drags
                    # the box ~20 cm west, measured grS10/grS12). Lower fires from carry.
                ...
                if _SK is not None and _G["state"] in ("open", "withdraw", "done") and _SK.get("on"):
                    ...
                    R._log(world, "SUCTION OFF at t=%d (release)" % tt)
                if _i("GR_SUCTION", 0) and _SK is not None and _SK["qadr"] >= 0 and \
                        _G["state"] in ("reach", "engage", "lift", "hold", "lower", "open", "unstick"):
                    ...
                    try:
                        ...
                        _anc7 = np.array([float(qp[_SK["qadr"]]), float(qp[_SK["qadr"] + 1]),
                                          float(qp[_SK["qadr"] + 2]) + 0.09])
                        _SK["gap"] = float(np.linalg.norm(_cup7 - _anc7))
                        _SK["bxy"] = (float(_anc7[0]), float(_anc7[1]))
                        ...
                        if _gdx8 < 1e8:
                            _dd8 = math.hypot(_anc7[0] - _gdx8, _anc7[1] - _f("GR_DROP_Y", 0.0))
                        ...
                        if _SK["on"]:
                            ...
                            _F7 = _f("GR_SUCTION_KP", 900.0) * (_cup7 - _anc7) \
                            ...
                            _SK["w"] = (_F7, _T7, -_F7, np.cross(_cup7 - _pel7, -_F7))
                ...
                if _G["state"] in _POSES and _G["cur"] is not None:
                    ...
                    if _G["state"] in ("close", "lift", "hold", "lower"):
                        try:
    ...
    if getattr(world, "_wr_hlam", 0.0) > 0 and qp[2] > 0.35:
        ...
        _bstand5 = bool(_bt5) and _bt5.get("target") == "stand"
        ...
        if _i("HARNESS_ATT_HEADING", 0):
            # HEADING-AWARE attitude spring (2026-07-10, the corner-2 wall): _r5/_p5 are BODY
            # roll/pitch (yaw-first Euler), but the raw torques below go to WORLD angular x/y --
            # correct only near heading 0. At heading 90-175 deg the roll correction feeds PITCH
            # and vice versa: the safety net is cross-wired, and a pivoting turner (±0.15 rad
            # weight shifts, ~90 N*m injected cross-axis) toppled sideways 4/4 at the shuttle's
            # far corner while the near-0-heading corner passed every run. Compute the PD in the
            # BODY frame (body-frame rates too) and rotate the torque into world axes. Gated so
            # every demo verified under the old spring stays byte-identical.
            _yb5 = math.atan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z))
            _cb5, _sb5 = math.cos(_yb5), math.sin(_yb5)
            _wxb5 = _cb5 * float(qv[3]) + _sb5 * float(qv[4])
            _wyb5 = -_sb5 * float(qv[3]) + _cb5 * float(qv[4])
            _txb5 = -_f("HARNESS_KP", 600.0) * _r5 - _f("HARNESS_KD", 60.0) * _wxb5
            _tyb5 = -_f("HARNESS_KP", 600.0) * _p5 - _f("HARNESS_KD", 60.0) * _wyb5
            _tx5 = max(-350.0, min(350.0, _lam5 * (_cb5 * _txb5 - _sb5 * _tyb5)))
    ...
    if _t2d > 0.0 and (getattr(world, "_wr_turn_active", False) or not getattr(world, "_wr_turnctx", None)):
        ...
        if _i("TURN_LOOP", 0) and getattr(world, "_wr_in_turn", False) and getattr(world, "_wr_seq_hold", False):
            if not getattr(world, "_wr_turn_done", False):
                ...
                if _rem < -_f("TURN_LOOP_ABORT_DEG", 60.0):
                    ...
                    R._log(world, "TURN-LOOP ABORT: over-rotated to %.1f deg (target %.0f) at t=%d -- end-hold pinned"
                           % (abs(world._wr_turn_acc) * 57.2958, _t2d, tt))
                elif world._wr_phase >= _tp9 - 1e-9:                  # a pass reached the end-hold
                    _acc9 = abs(world._wr_turn_acc) * 57.2958
                    _tol9 = _f("TURN_LOOP_TOL_DEG", 8.0)
                    _ctx9 = getattr(world, "_wr_turnctx", None) or {}
    ...
    if world._wr_gfollow and tt % 2 == 0:
        try:
            ...
            _bt9 = world._wr_baton.get("target") if getattr(world, "_wr_baton", None) is not None else None
            if getattr(world, "_wr_in_turn", False):
