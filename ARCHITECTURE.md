...
│   ├── templates/                Profile-only starter kits (picker, roomba, husky_swarm, mission_control)
│   ├── production/               Full agents with their own runners (8) + shared _lib/
...
│   ├── robots/                   URDF + PROTO robots (Clearpath, Boston Dynamics, Unitree, …)
...
│               ├── omnilink_launcher.wbt         default entry point (stays flat)
│               ├── omnilink_mission_control.wbt  mission-control entry (stays flat)
│               ├── chat/         11 single-robot natural-language demos
│               ├── flagship/     18 multi-robot agent-driven showcases
