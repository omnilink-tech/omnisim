...
- **GitHub Security Advisories** (preferred). On the OmniSim repository, open
  ...
  see.
- **Email**: send the report to `info@omnilink-agents.com`. PGP is not
...
- The OmniSim simulator binary and bundled libraries (`src/omnisim/`,
  `src/controller/`), including the wgpu renderer (`src/omnisim/render/`) and
  the embedded Newton physics runtime
  (`src/omnisim/physics/omnisim_newton_runtime.py`, which the engine executes
  in an embedded CPython interpreter).
...
- Third-party components vendored into or fetched by the build — Newton, NVIDIA
  Warp, MuJoCo / MuJoCo-Warp, wgpu-native, the embedded CPython, Assimp, GLM,
  stb, glad, OIS, OpenVR, FreeType, OpenAL Soft, OpenSSL, the MSYS2/MinGW
  runtime, and Qt. Report those issues to the upstream projects directly. See
  [`NOTICE`](NOTICE) and [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) for
  the list and links.
  ⚠️ Two names that used to be on this list are gone from the product entirely
  and are not "out of scope" so much as absent: **ODE** was deleted on
  2026-08-08 (Newton is the only physics backend) and **WREN** on 2026-08-23
  (wgpu-native is the only renderer). A vulnerability report against either is
  a report against a release no longer current; say which release you tested.
