...
/**********************************************************************************/
/* Description:  OmniSim C programming interface for the Supervisor node          */
/**********************************************************************************/
...
// Batched inverse-kinematics PREVIEW against the live Newton physics model.
// `node` is the end-effector Solid; `targets` is n_targets * [x,y,z] in the
// world frame; `rotations` is NULL or n_targets * [qx,qy,qz,qw]; `tool_offset`
// is NULL or a [x,y,z] TCP offset in the end effector's own frame. The engine
// solves for every 1-DoF (Hinge/Slider) joint of the end effector's robot that
// is registered with the physics backend, clamps the answer to the authored
// joint limits, and measures each target's residual (metres) by forward
// kinematics on exactly the returned angles. NOTHING MOVES: it is a pure read.
// On success returns 0 and fills the out parameters; the buffers stay valid
// until the next call. `angles` is problem-major: n_targets * (*n_joints)
// values, target 0's joints first. `joint_node_ids` are the joints' unique
// node ids (wb_supervisor_node_get_from_id). Non-zero return = no answer:
// -1 no physics backend / world not finalised, -2 the node owns no physics
// body, -3 the robot has no IK-solvable joints, -4 the solver failed (engine
// log has the Python error), -9 the engine did not answer, -10 invalid
// arguments. First use per world compiles a solver kernel (seconds); later
// calls are fast.
