...
producer that reads the authoritative ``[WbNewtonBackend] world finalised
...
def resolved_backend(fp: dict) -> str:
    """Collapse the env_fingerprint log scan to ``newton`` | ``ode`` | ``unknown``.

    A finalise line (any solver) means the Newton backend drove the world. An
    ODE fallback, or a readable log with no finalise line, means ODE. No
    readable log (fresh install, never run) is ``unknown``.
    """
    scan = fp.get("scan") or {}
