...
import {omnisim} from './omnisim.js';
...
// All sibling assets (CSS, dependency JS, wrenjs.* WASM bundle, etc.) are
// resolved relative to wherever this module is served from. OmniSim ships
// the full wwi/ tree locally under resources/web/wwi/, so the streaming
// viewer and HTML5 export work without phoning home to cyberbotics.com.
const WWI_BASE = new URL('.', import.meta.url).href;

/* The following member variables can be set by the application:

omnisimView.showIde               // defines whether the IDE button should be displayed.
omnisimView.showInfo              // defines whether the info button should be displayed.
omnisimView.showPlay              // defines whether the play button should be displayed.
omnisimView.showQuit              // defines whether the quit button should be displayed.
omnisimView.showReload            // defines whether the reload button should be displayed.
omnisimView.showReset             // defines whether the reset button should be displayed.
omnisimView.showRobotWindow       // defines whether the robot window button should be displayed.
omnisimView.showRun               // defines whether the run button should be displayed.
omnisimView.showStep              // defines whether the step button should be displayed.
omnisimView.showTerminal          // defines whether the terminal button should be displayed.
omnisimView.showCustomWindow      // defines whether the custom window button should be displayed.
omnisimView.showWorldSelection    // defines whether the world selection button should be displayed.
*/

export default class OmnisimView extends HTMLElement {
  #hasAnimation;
  #hasProto;
  #hasScene;
  #initialCallbackDone;
  ...
  connectedCallback() {
    ...
    this.toolbarCss.href = WWI_BASE + 'css/toolbar.css';
    ...
    this.progressCss.href = WWI_BASE + 'css/progress.css';
    ...
    script.textContent = `var Module = [];
        ...
        // .data files (e.g. wrenjs.data) live alongside this OmnisimView module.
        if (path.endsWith('.data'))
          return ${JSON.stringify(WWI_BASE)} + path;
  ...
  #init() {
    ...
    promises.push(this.#loadScript(WWI_BASE + 'dependencies/ansi_up.js'));
    promises.push(this.#loadScript(WWI_BASE + 'dependencies/assimpjs.js'));
    promises.push(this.#loadScript(WWI_BASE + 'dependencies/glm-js.min.js'));
    promises.push(this.#loadScript(WWI_BASE + 'dependencies/quaternion.min.js'));
    promises.push(this.#loadScript(WWI_BASE + 'dependencies/libtess.min.js'));
    promises.push(this.#loadScript(WWI_BASE + 'enum.js'));
    promises.push(this.#loadScript(WWI_BASE + 'wrenjs.js'));
  }
  ...
  loadAnimation(scene, animation, play, isMobileDevice, thumbnail, raw) {
    ...
    if (!this.initializationComplete)
      ...
      if (typeof this._view === 'undefined')
        this._view = new omnisim.View(this, isMobileDevice);
      this._view.onready = () => {
    ...
    if (!this.initializationComplete)
      ...
      this._view.onready = () => {
        ...
        if (document.getElementById('robot-window-button') !== null)
          document.getElementsByTagName('omnisim-view')[0].toolbar.loadRobotWindows();
        if (typeof this.onready === 'function')
          this.onready();
      };
...
window.customElements.define('omnisim-view', OmnisimView);
// Deprecated back-compat alias: keep <webots-view> working for existing embeds.
window.customElements.define('webots-view', class extends OmnisimView {});
