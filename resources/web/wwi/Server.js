...
export default class Server {
    ...
    fetch(self.#url)
      ...
      .then(function(data) {
        ...
        self.socket.onclose = (event) => {
          ...
          console.log('Disconnected from the OmniSim server.');
        };
  ...
  onMessage(event) {
    ...
    if (message.indexOf('webots:ws://') === 0 || message.indexOf('webots:wss://') === 0) {
    ...
    else if (message === '.') { // received every 5 seconds when OmniSim is running
      // nothing to do
      ...
      let percent;
      if (document.getElementById('omnisim-progress-bar-percent'))
        percent = document.getElementById('omnisim-progress-bar-percent').value;
      ...
      console.log('Received an unknown message from the OmniSim server socket: "' + message + '"');
  }
}
