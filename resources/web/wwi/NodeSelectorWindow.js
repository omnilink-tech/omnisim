...
// ---------------------------------------------------------------------------
// PROTO library provider -- UPSTREAM INFRASTRUCTURE, not an OmniSim service.
//
// The node selector's PROTO catalogue is fetched at runtime from webots.cloud,
// which is Cyberbotics' web property. That is inherited from upstream and has
// deliberately NOT been repointed yet. Two things to know before changing it:
//
//  1. It is third-party in content as well as in hosting. The endpoint answers
//     with entries whose `url` points into the Webots repository and whose
//     `license` is typically "Copyright Cyberbotics Ltd. Licensed for use only
//     with Webots." -- by their own terms those assets are not licensed for use
//     here, so this list is not an OmniSim node library and should not be
//     presented as one.
//
//  2. Replacing it needs an ENGINE change, not a viewer change. OmniSim already
//     generates a complete local catalogue: resources/proto-list.xml (260
//     entries, omnisim:// urls), built by scripts/packaging/generate_proto_list.py
//     and already consumed by the Qt "Add Node" dialog in OmProtoManager.cpp.
//     The web viewer cannot reach it today -- OmTcpServer::sendTcpRequestReply
//     routes only "resources/web/wwi/" plus a .js/.css/.html catch-all, and
//     OmHttpReply::mimeType has no "xml" entry, so a request for
//     /resources/proto-list.xml is rejected before routing is even considered.
//     Give that file a mime type and a route and this provider can be swapped
//     for the local catalogue.
//
// Until then the dependency is at least explicit and overridable rather than
// buried in a function body: set `window.omnisimProtoLibraryUrl` to a full URL
// before opening the node selector to use your own provider. It must accept a
// POST of {base_types, slot_type?, skip_no_robot_ancestor?} and answer with a
// JSON array of {url, base_type, license, description}.
// ---------------------------------------------------------------------------
const UPSTREAM_PROTO_LIBRARY_HOST = 'webots.cloud';
const PROTO_LIBRARY_PATH = '/ajax/proto/insertable.php';
...
export default class NodeSelectorWindow {
  ...
  constructor(parentNode, rootProto) {
    ...
    // Always iterable. populateWindow() is also reached straight from the filter
    // input's oninput, which can fire before -- or instead of -- a successful fetch.
  ...
  #setupWindow(parentNode) {
    ...
    img.draggable = false;
    img.src = '/resources/web/wwi/images/missing_proto_icon.png';
  ...
  // Resolve the PROTO library endpoint. See the note at the top of this file:
  // the default is upstream infrastructure, and this is the one place to repoint it.
  #protoLibraryUrl() {
    if (typeof window.omnisimProtoLibraryUrl === 'string' && window.omnisimProtoLibraryUrl !== '')
      return window.omnisimProtoLibraryUrl;

    // When the viewer is itself served by the upstream provider, ask that same
    // origin; otherwise fall back to the upstream host.
    let url = window.location.href;
    if (!url.includes(UPSTREAM_PROTO_LIBRARY_HOST))
      url = 'https://' + UPSTREAM_PROTO_LIBRARY_HOST + '/';

    return 'https://' + new URL(url).hostname + PROTO_LIBRARY_PATH;
  }
  ...
  async fetchCompatibleNodes() {
    ...
    // Base nodes come from the local FieldModel and need no network whatsoever,
    // so build them BEFORE the fetch: an unreachable PROTO library must not be
    // able to take the built-in nodes down with it.
    this.baseNodes = new Map();
    ...
    this.protoNodes = new Map();
    const endpoint = this.#protoLibraryUrl();

    return fetch(endpoint, {method: 'post', body: JSON.stringify(content)})
      ...
      .catch(error => {
        // Degrade to the local base nodes instead of breaking the window. Before
        // this, a failed fetch left baseNodes/protoNodes undefined, show() opened
        // an empty panel on an unhandled rejection, and the next populateWindow()
        // -- which the filter input fires on every keystroke -- threw
        // "this.baseNodes is not iterable".
        console.warn('NodeSelectorWindow: PROTO library unreachable at ' + endpoint + ' (' + error +
          '); offering base nodes only.');
      });
  }
  ...
  populateNodeInfo(nodeName) {
    ...
    if (typeof nodeName === 'undefined') {
      ...
      nodeImage.onerror = () => {
        ...
        nodeImage.src = '/resources/web/wwi/images/missing_proto_icon.png';
      };
  ...
  show(parameter, element, callback, parent, mfId, resetButton) {
    ...
    if (window.webotsJSPreventAddNode) {
      alert('New nodes can only be inserted in protos that are registered in the proto library.');
      return;
    }
