...
export default class ImageLoader {
  ...
  static loadTextureData(prefix, url, isHdr, rotation) {
    ...
    if (url.startsWith('omnisim://')) {
      // `omnisim://` is OmniSim's OWN asset scheme, so it must resolve against the
      // OmniSim repository -- not cyberbotics/webots, which does not carry OmniSim's
      // assets (or its tags). Matches scripts/packaging/update_urls.py.
      if (typeof ImageLoader.owner === 'undefined')
        ImageLoader.owner = 'omnilink-tech';
      if (typeof ImageLoader.repository === 'undefined')
        ImageLoader.repository = 'omnisim';
      if (typeof ImageLoader.branch === 'undefined' || ImageLoader.branch === '')
        ImageLoader.branch = 'main';
      url = url.replace('omnisim://', 'https://raw.githubusercontent.com/' + ImageLoader.owner + '/' +
                                      ImageLoader.repository + '/' + ImageLoader.branch + '/');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        if (typeof img.failed === 'undefined') {
          ...
          img.src = '/resources/web/wwi/images/missing_texture.png';
          img.failed = true;
        } else
