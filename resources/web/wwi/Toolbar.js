...
export default class Toolbar {
  ...
  #resizeToolbar() {
    ...
    const viewElement = document.getElementsByClassName('omnisim-view')[0];
    if (viewElement)
  ...
  #createRobotWindows() {
    ...
    this.#view.robots.forEach((robot) => {
      if (robot.window !== '<none>') {
        ...
        if (robot.visible) // if the robot window is visible by default in OmniSim, then show it
  ...
  #checkWindowBoundaries() {
    ...
    resizeObserver.observe(document.getElementsByClassName('omnisim-view')[0]);
  }
  ...
  #createStreamingTimeIndicator() {
    ...
    clock.className = 'omnisim-streaming-time';

    clock.id = 'omnisim-clock';
