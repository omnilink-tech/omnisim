...
export default class Parser {
  ...
  async parse(text, renderer, finalize = true, parentNode, callback) {
    omnisim.currentView.progress.setProgressBar('same', 'Parsing...', 0, 'Parsing object...');
    let xml = null;
    if (window.DOMParser) {
    ...
    return Promise.all(this.#promises).then(() => {
      ...
      omnisim.currentView.progress.setProgressBar('none');

      if (!finalize) {
        setTimeout(() => { omnisim.currentView.progress.setProgressBar('none'); }, 300);
        return;
      }
      ...
      omnisim.currentView.progress.setProgressBar('block', 'Finalizing...', 0, 'Finalizing...');
      ...
      WbWorld.instance.currentView = omnisim.currentView;
      WbWorld.instance.root.finalize();

      WbWorld.instance.readyForUpdates = true;

      omnisim.currentView.w3dScene.resize();
      renderer.render();
      setTimeout(() => { omnisim.currentView.progress.setProgressBar('none'); }, 300);
  ...
  #parseWorldInfo(node) {
    ...
    let omnisimView = document.getElementsByTagName('omnisim-view')[0];
    if (omnisimView && typeof omnisimView.toolbar !== 'undefined') {
      let informationPanel = omnisimView.toolbar.informationPanel;
  ...
  #updatePromiseCounter(info) {
    ...
    omnisim.currentView.progress.setProgressBar('block', 'same', percentage, info);
  }
