...
/*
 * Injects an OmniSim 3D view inside a HTML tag.
 * @class
 * @classdesc
 *   The OmniSim view object displays a 3D view on a web page.
 *   This view represents an OmniSim simulation world that may be
 *   connected to an OmniSim instance running on a remote server.
 * @example
 *   // Example: Initialize from an OmniSim streaming server
 *   const view = new omnisim.View(document.getElementById('myDiv'));
...
// Canonical global is `omnisim`; `webots` is kept as a back-compat alias to the
// same object so existing embedders (window.webots, import {webots}) keep working.
let omnisim = window.omnisim || window.webots || {};

omnisim.View = class View {
  #isWebSocketProtocol;
  #server;
  #w3dDiv;
  constructor(view3D, mobile) {
    omnisim.currentView = this;
    this.onerror = (text) => {
    ...
    this.onresize = () => {
      ...
      const labels = document.getElementsByClassName('omnisim-label');
    ...
    this.view3D = view3D;
    this.view3D.className = view3D.className + ' omnisim-view';
  ...
  open(url, mode, thumbnail, raw) {
    ...
    const initWorld = async() => {
      ...
      if (document.getElementById('omnisim-progress'))
        document.getElementById('omnisim-progress').style.display = 'block';

      if (this.#isWebSocketProtocol) {
        if (this.url.endsWith('.omniworld') || this.url.endsWith('.wbt')) { // url expected form: "wss://localhost:1999/simple/worlds/simple.wbt" or
    ...
    this.#isWebSocketProtocol = this.url.startsWith('ws://') || this.url.startsWith('wss://') ||
      this.url.endsWith('.omniworld') || this.url.endsWith('.wbt');
    ...
    if (mode === 'mjpeg') {
      ...
      this.#w3dDiv.className = 'omnisim-3d-view';
  ...
  updateWorldList(currentWorld, worlds) {
    if (this.broadcast)
      ...
      // where multiple users can connect to the same OmniSim instance.
      return;
    ...
    if (existingCurrentWorld) {
      const omnisimView = document.getElementsByTagName('omnisim-view')[0];
      if (omnisimView && typeof omnisimView.toolbar !== 'undefined' &&
        typeof omnisimView.toolbar.worldSelectionPane !== 'undefined') {
        document.getElementById('world-selection-pane').remove();
        omnisimView.toolbar.createWorldSelectionPane();
      }
    }
  }
  ...
  setLabel(properties) {
    ...
    if (labelElement == null) {
      ...
      labelElement.className = 'omnisim-label';
  ...
  resetSimulation() {
    ...
    this.removeLabels();
    if (document.getElementById('omnisim-clock'))
      document.getElementById('omnisim-clock').innerHTML = omnisim.parseMillisecondsIntoReadableTime(0);
  }
...
omnisim.parseMillisecondsIntoReadableTime = (milliseconds) => {
...
// Expose the canonical global and the back-compat alias on window.
window.omnisim = omnisim;
// Deprecated alias kept for existing embedders that reference window.webots.
const webots = omnisim;
window.webots = webots;

export {omnisim, webots};
