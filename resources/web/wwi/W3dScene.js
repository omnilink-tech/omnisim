...
export default class W3dScene {
  ...
  init(texturePathPrefix = '') {
    omnisim.currentView.prefix = texturePathPrefix;
  ...
  destroyWorld() {
    if (typeof document.getElementsByTagName('omnisim-view')[0] !== 'undefined' &&
      typeof document.getElementsByTagName('omnisim-view')[0].toolbar !== 'undefined') {
      const toolbar = document.getElementsByTagName('omnisim-view')[0].toolbar;
  ...
  async loadRawWorldFile(raw, onLoad, progress) {
    const prefix = omnisim.currentView.prefix;
  ...
  async loadObject(w3dObject, parentId, callback) {
    ...
    const parser = new Parser(omnisim.currentView.prefix);
    parser.prefix = omnisim.currentView.prefix;
    ...
    if (data.startsWith('application/json:')) {
      if (typeof view.time !== 'undefined') { // otherwise ignore late updates until the scene loading is completed
        ...
        if (document.getElementById('omnisim-clock'))
          document.getElementById('omnisim-clock').innerHTML = omnisim.parseMillisecondsIntoReadableTime(frame.time);
