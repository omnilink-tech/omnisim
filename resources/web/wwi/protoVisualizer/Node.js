    ...
    return new Promise((resolve, reject) => {
      ...
      for (const line of lines) {
        if (line.indexOf('EXTERNPROTO') !== -1) { // get only the text after 'EXTERNPROTO'
          ...
          if (address.startsWith('omnisim://'))
            address = 'https://raw.githubusercontent.com/omnilink-tech/omnisim/main/' + address.substring(9); // TODO change to released once a new version is published
          else
...
function combinePaths(url, parentUrl) {
  ...
  if (url.startsWith('omnisim://')) {
    if (parentUrl.startsWith('http://') || parentUrl.startsWith('https://')) {
      ...
      return url.replace('omnisim://', match[0]);
    } else
      return url;
  }
