...
export default class Stream {
  ...
  connect() {
    ...
    this.view.progress.setProgressBar('block', 'Connecting to OmniSim instance...', percent, 'Opening socket...');
  ...
  #onSocketMessage(event) {
    ...
    if (data.startsWith('robot window:')) {
      ...
      if (json.remove) {
        const robots = omnisim.currentView.robots;
        ...
        omnisim.currentView.robots.push({name: json.robot, window: robotWindow, main: json.main, visible: json.visible});
      ...
      this.view.progress.setProgressBar('block', 'OmniSim: ' + message + '...', percent, info + '%');
      ...
      this.view.time = parseFloat(data.substring(data.indexOf(':') + 1).trim());
      if (document.getElementById('omnisim-clock'))
        document.getElementById('omnisim-clock').innerHTML = omnisim.parseMillisecondsIntoReadableTime(this.view.time);
