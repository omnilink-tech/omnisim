...
> Warehouse Foreman, Patrol Squad, and Drone Surveyor all shipped (Drone Surveyor iter 0–2;
> its iter-3 operator-chat LLM run is still open pending an `OMNI_KEY` run).
...
## 1. Warehouse Foreman — multi-agent orchestration *(✅ shipped)*

**Status:** end-to-end working, ground-truth verified, $1.50/hr at Gemini 3 Flash list. Full architecture in [`warehouse_foreman/docs/ARCHITECTURE.md`](production/warehouse_foreman/docs/ARCHITECTURE.md), measured runs in [`warehouse_foreman/docs/RESULTS.md`](production/warehouse_foreman/docs/RESULTS.md), transferable patterns in [`AGENT_PATTERNS.md`](AGENT_PATTERNS.md).
...
- **Picker** — Husky drives to a labelled pallet, snaps a camera frame, identifies it by colour, then physically pushes it to the loading dock. ~90% of Husky Maze v1 (vision branch) with the maze swapped for `warehouse_husky.wbt` and the brief swapped for "find SKU X / colour Y".
...
- New world: extend `warehouse_husky.wbt` with labelled pallet markers and a loading-dock zone (~50 lines of WBT).
...
## 2. Patrol Squad — cross-session memory dividend in action *(✅ shipped)*

**Status:** end-to-end working, dividend verified. Two huskies (north + south), one shared `Patrol Husky` profile, sector-tagged manifests persisted across runner restarts. Diff narration matches the operator's actual teleports. Live results in [`warehouse_patrol/docs/RESULTS.md`](production/warehouse_patrol/docs/RESULTS.md).
...
- `warehouse_husky.wbt` (already crowded with crates that can be subtly moved between sweeps)
...
## 3. Drone Surveyor — vision + autonomy on a different motion model *(iter 0-2 shipped)*

**Status:** world (`chat/omnilink_mavic.wbt`) + bridge (`mavic_omnilink_bridge`) + LLM-free `solve.py` shipped iter 0; iter-1 verified end-to-end (3/3 red markers detected at 0.06 m mean projection error, see [`drone_surveyor/docs/RESULTS.md`](production/drone_surveyor/docs/RESULTS.md)); iter-2 productized agent shipped — full profile + system prompt + 21-tool surface + runner + chat_drive script, smoke-tested against the live bridge. Iter 3 (operator chat → live LLM-driven survey → cost numbers) pending an OMNI_KEY run. Full plan in [`drone_surveyor/docs/PLAN.md`](production/drone_surveyor/docs/PLAN.md).

**Pitch.** Mavic 2 Pro on `chat/omnilink_mavic.wbt`. Operator:
...
- **Rough-terrain (`desert_ruins.wbt`, `moon.wbt`).** Outdoor navigation looks cool but adds little over the maze story we already tell. Maybe later as a "multi-environment Husky" pack.
...
1. **Warehouse Foreman** ✅ shipped 2026-05-02
2. **Patrol Squad** ✅ shipped 2026-05-02 — world + bridge multiplexing + sector-tagged shared profile + sweep/recall/diff pipeline + cross-session dividend live verified
3. **Drone Surveyor** iter 0-2 shipped 2026-05-02 — world + bridge + verified perception (3/3 red markers, 0.06 m mean error) + productized agent (21-tool surface, runner on :51521, chat_drive); iter-3 LLM run pending OMNI_KEY

Patrol Squad and Drone Surveyor are being built in parallel because they don't share the same world or bridge — Patrol works on `warehouse_patrol.wbt` with the husky stack, Drone Surveyor works on `chat/omnilink_mavic.wbt` with a brand-new mavic stack.
