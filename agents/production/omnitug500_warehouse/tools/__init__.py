...
"""OmniTug 500 warehouse-courier agent — tool registry.

Each module exports `SPEC` or `SPECS`. `load_all()` aggregates them. Drop a new
file in to add a tool. Mirrors the husky_maze loader.
"""
    ...
    for info in pkgutil.iter_modules([str(pkg_path)]):
        ...
        for spec in specs:
            if spec.name in registry:
                raise RuntimeError(f"duplicate tool name {spec.name!r} (in {info.name})")
