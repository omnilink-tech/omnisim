...
from _lib.supervision import execute_verified_sequence
...
MISSION_PLAN_MAX_STEPS = int(os.environ.get("CAPTAIN_PLAN_MAX_STEPS", "8"))
_MISSION_PLANS: Dict[str, Dict[str, Any]] = {}
_MISSION_PLAN_LOCK = threading.Lock()


def get_mission_plan(plan_id: str) -> Optional[Dict[str, Any]]:
    """Return a stored plan (read-only copy) for inspection."""
    plan = _MISSION_PLANS.get(plan_id)
    return dict(plan) if plan is not None else None


def claim_mission_plan(
    plan_id: str,
    *,
    rationale: str = "",
) -> "tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]":
    """Bind a verified plan to EXACTLY ONE captain claim. Returns (plan, error).

    `_MISSION_PLANS` is process-lifetime and never pruned, so before this a
    plan_id stayed a valid completion token for every later mission in the
    session: after mission A's plan P verified,
    `complete_mission(rationale="found the red cylinder", plan_id=P)` for an
    unrelated mission B passed the gate and wrote an audit record carrying
    mission A's objective and A's legs. The claim looked evidenced and named
    the wrong work.

    A plan is evidence for one claim. Stamping it on first use makes the
    second attempt say so instead of silently succeeding.
    """
    with _MISSION_PLAN_LOCK:
        plan = _MISSION_PLANS.get(plan_id)
        if plan is None:
            return None, {"error": "unknown_plan_id", "plan_id": plan_id}
        if not plan.get("completed") or not plan.get("verified"):
            return None, {
                "error": "mission_plan_not_verified",
                "plan_id": plan_id,
                "ledger": plan.get("ledger"),
            }
        existing = plan.get("claimed_by")
        if existing is not None:
            return None, {
                "error": "mission_plan_already_claimed",
                "plan_id": plan_id,
                "objective": plan.get("objective"),
                "claimed_by": existing,
                "hint": (
                    "This plan has already backed a completion claim. A plan is "
                    "evidence for the objective it executed and for one claim "
                    "only. If this is a new mission, run execute_mission_plan "
                    "again and claim against the plan_id it returns."
                ),
            }
        plan["claimed_by"] = {
...
def _slim_settings_for_subchat(settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(settings, dict):
        return settings
    slim = dict(settings)
    contract = slim.get("executionContract")
    main_task = slim.get("mainTask")
    if isinstance(contract, str) and contract.strip():
        # Profiles accumulate historical guidance in mainTask. A concise
        # executionContract is the current production policy and must win when
        # it conflicts with older per-primitive recipes.
        slim["mainTask"] = (
            "CURRENT EXECUTION CONTRACT (takes precedence over legacy flow):\n"
            + contract.strip()
            + (
                "\n\nBACKGROUND / FALLBACK GUIDANCE:\n" + main_task
                if isinstance(main_task, str) and main_task.strip()
                else ""
            )
        )
    ...
    while turns < cap:
        ...
        for attempt in range(3):
            try:
                ...
                # /status now performs a live bridge read (that is where
                # mission_complete lives), so 1.0 s is too tight to be honest.
                mid_status = http_get(info["runner_status_url"], timeout=3.0)
        ...
        sub_status = http_get(info["runner_status_url"], timeout=3.0)
...
# ──────────────────────────────────────────────────────────────────────
def _impl_execute_mission_plan(
    steps: Any = None,
    objective: str = "",
    **_: Any,
) -> Dict[str, Any]:
    """Execute a bounded delegation plan and retain its verified ledger."""
    if not objective or not objective.strip():
        return {"error": "objective is required"}
    if not isinstance(steps, list) or not steps:
        return {"error": "steps must be a non-empty list"}
    if len(steps) > MISSION_PLAN_MAX_STEPS:
        return {
            "error": "mission_plan_too_large",
            "maximum_steps": MISSION_PLAN_MAX_STEPS,
        }
    ...
    for index, raw in enumerate(steps):
        if not isinstance(raw, dict):
            return {"error": "each plan step must be an object", "index": index}
        agent = str(raw.get("agent") or "").strip()
        task = str(raw.get("task") or "").strip()
        if not agent or not task:
            return {
                "error": "each plan step requires agent and task",
                "index": index,
            }
        if agent not in SPECIALIST_REGISTRY:
            return {
                ...
                "agent": agent,
                "known": sorted(SPECIALIST_REGISTRY),
            }
        if agent in seen_agents:
            return {
                "error": "duplicate_specialist_mission",
                "agent": agent,
                "first_index": seen_agents[agent],
                "duplicate_index": index,
                "hint": (
                    "Give one specialist a single self-contained mission. "
                    "Its mission_complete flag cannot verify multiple legs."
                ),
            }
        seen_agents[agent] = index
        status = http_get(
            SPECIALIST_REGISTRY[agent]["runner_status_url"],
            timeout=2.0,
        )
        if status.get("error"):
            return {
        ...
        if not isinstance(status.get("mission_complete"), bool):
            # delegate_to_agent's ONLY completion evidence is this flag. When
            # /status cannot report it, no leg can ever verify -- the mission
            # burns its whole turn budget and then fails as "unverified", which
            # reads like the robot failed rather than like the runner is not
            # publishing the flag. Say so before spending 30 turns finding out.
            return {
                "error": "specialist_cannot_report_completion",
                "agent": agent,
                "index": index,
                "mission_complete_source": status.get("mission_complete_source"),
                "hint": (
                    "The specialist's /status has no boolean `mission_complete`. "
                    "That flag lives on the robot bridge; the runner's "
                    "status_snapshot must forward it (see husky_maze_agent's "
                    "_live_mission_complete). Start/repair the bridge, or fix "
                    "the runner, before delegating."
                ),
                "status": status,
            }
        if status.get("mission_complete"):
            return {
                "error": "specialist_not_fresh",
                "agent": agent,
                "index": index,
                "hint": (
                    "The specialist already reports mission_complete=true. "
                    "Reset or reload its mission before using that flag as evidence."
                ),
                "status": status,
            }
        preflight.append({"agent": agent, "status": status})
        try:
            max_turns = int(raw.get("max_turns") or 0)
        except (TypeError, ValueError):
            return {"error": "max_turns must be an integer", "index": index}
        normalized.append(
            {
                "id": str(raw.get("id") or f"delegation_{index + 1}"),
                "agent": agent,
                "task": task,
                "max_turns": max_turns,
            }
    ...
    ledger = execute_verified_sequence(
        normalized,
        lambda step: _impl_delegate_to_agent(
            agent=str(step["agent"]),
            task=str(step["task"]),
            max_turns=int(step.get("max_turns") or 0),
        ),
        verifier=lambda _step, result: (
            isinstance(result, dict)
            and result.get("success") is True
            and result.get("mission_complete") is True
...
SPECS = [
    ...
    ToolSpec(
        name="execute_mission_plan",
        tier=GUARDED,
        surface=ALWAYS,
        description=(
            "Execute and mechanically verify a bounded delegation plan. "
            "Preflights every specialist, rejects stale mission_complete "
            "state and duplicate ownership, executes dependent legs in order, "
            "and stops at the first unverified result. Returns a plan_id and "
            "per-leg ledger. Give each specialist one self-contained mission; "
            "after completed=true, pass plan_id to complete_mission."
        ),
        parameters={
            "type": "object",
            "properties": {
                "objective": {
                    "type": "string",
                    "description": "The operator objective this plan satisfies.",
                },
                "steps": {
                    "type": "array",
                    "items": {
                    ...
                    "description": "Ordered, dependent specialist missions.",
                },
            },
            "required": ["objective", "steps"],
        },
        impl=_impl_execute_mission_plan,
        tags=["delegation", "orchestration", "mission"],
    ),
]
