...
- [`tools/`](tools/) — **11 tools** (3 guarded, 8 safe): `delegate_to_agent`, `execute_mission_plan`, `query_agent_status`, `list_agents`, `complete_mission`, plus `recall` / `save_local_memory` / `search_local_memory` / `list_local_memories` / `forget_local_memory` / `search_knowledge`. *(Count check: `tools.load_all()` — auto-discovered, never hand-kept.)*
