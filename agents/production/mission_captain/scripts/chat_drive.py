...
"""Drive the Mission Captain agent via OmniLink chat() with a local tool loop.

Same architecture as agents/production/husky_maze/scripts/chat_drive.py: the
OmniLink chat() endpoint is one-shot (returns toolCalls but can't reach
loopback URLs to dispatch them), so we run the dispatch loop locally.

The captain's `delegate_to_agent` tool is itself a sub-chat-loop, so a
single captain delegation can take dozens of seconds (the captain's
chat ticks once, but its delegation ticks the sub-agent for a long
time before returning a single result).

Usage:
    OMNI_KEY=olink_... python agents/production/mission_captain/scripts/chat_drive.py \
        "Drive to cell (5, 3) then come back to start."

The Mission Captain runner (mission_captain_agent.py) must already be
running so the captain's profile is on the platform and its tool-callback
server is up. The Husky Maze runner (and Axis runner if used) must be
running too.
"""
...
ENGINE = "g1-engine"
_CAPTAIN_PORT = int(os.environ.get("CAPTAIN_PORT", "51518"))
TOOL_SERVER_URL = f"http://127.0.0.1:{_CAPTAIN_PORT}/tool"
ACTIVITY_URL = f"http://127.0.0.1:{_CAPTAIN_PORT}/activity"
STATUS_URL = f"http://127.0.0.1:{_CAPTAIN_PORT}/status"
MAX_TURNS = int(os.environ.get("CAPTAIN_CHAT_MAX_TURNS", "20"))
...
def execute_tool(name: str, args: dict) -> dict:
    ...
    # Per-tool timeout. delegate_to_agent runs whole sub-missions which can
    # take 5-15 min for multi-cell legs, so we set this generously.
    timeout = int(os.environ.get("CAPTAIN_DRIVE_TOOL_TIMEOUT", "1800"))
    try:
...
def main() -> int:
    ...
    p.add_argument("prompt", nargs="?",
                   default="Drive the husky to cell (5, 3), then return to start.")
    ...
    if profile is None:
        print(f"Error: profile {AGENT_NAME!r} not found. Start mission_captain_agent.py first.")
        return 1
    ...
    pre = fetch_status()
    print(f"[chat] pre-status: {pre.get('captain_complete_calls_this_session')} prior captain claims")
    ...
    while turn < args.max_turns:
        ...
        if not tool_calls:
            # Check if the captain considers itself done.
            st = fetch_status()
            if st.get("captain_complete_calls_this_session", 0) > pre.get("captain_complete_calls_this_session", 0):
                print(f"[chat] captain claimed mission complete after {turn} turns, {total_tools} tool calls")
                return 0
            print("[chat] no tool calls; nudging")
            messages.append({"role": "assistant", "content": text or "(narration)"})
            messages.append({
                "role": "user",
                "content": "Don't just narrate. Call your next tool now (delegate_to_agent or complete_mission).",
            })
            continue
        ...
        st = fetch_status()
        if st.get("captain_complete_calls_this_session", 0) > pre.get("captain_complete_calls_this_session", 0):
            print(f"[chat] captain claimed mission complete in turn {turn}; total tools: {total_tools}")
            return 0
