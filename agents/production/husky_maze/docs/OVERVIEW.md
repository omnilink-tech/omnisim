...
| `projects/samples/demos/worlds/flagship/husky_maze.wbt` | Trivial brief world (drive to SE goal). Both script and agent satisfy it. |
| `projects/samples/demos/worlds/flagship/husky_maze_unknown.wbt` | Same destination but map gated → lidar wall-follow. |
| `projects/samples/demos/worlds/flagship/husky_maze_corners.wbt` | Multi-objective brief. **Agent only.** |
| `projects/samples/demos/worlds/flagship/husky_maze_visual.wbt` | Vision-only marker ID — find the red cylinder by sight, then BFS to it. **Agent only, structurally.** |
| `projects/samples/demos/worlds/flagship/husky_maze_blind.wbt` | Map AND lidar gated. Agent navigates from `scan_surroundings` (sidecar-side perception → ~1.3 KB tags per cardinal). Architectural cost-shape demo: perception lives in the tool, agent does brief interpretation + symbolic navigation. |
| `agents/production/husky_maze/` | The OmniLink agent: profile, prompts, knowledge, long-term memory, 20 tools, runner, standalone solver. |
...
- `tools/` — 20 tools, auto-discovered, each with tier (`safe` / `guarded`) and a single-line `kind`-aware activity-feed classifier.
...
`husky_maze.wbt` (map exposed) and `husky_maze_unknown.wbt` (map gated) have the same destination — drive from cell `(0, 10)` to cell `(10, 0)`. The bridge's `capabilities.map_available` tells the caller which navigation strategy applies.
...
`husky_maze_corners.wbt` ships a multi-objective brief in `WorldInfo.info`:
...
### Layer D — perception-as-tool, agent does navigation logic (`husky_maze_blind.wbt`)

`husky_maze_blind.wbt` is the same maze layout as the visual world but its title triggers the bridge to gate **both** `/maze` and `/lidar`. The husky still has the four-cardinal-camera sidecar, but instead of the agent consuming pixels, the sidecar runs a pure-Python BGRA analyser per frame and the bridge exposes the result as `scan_surroundings` — ~1.3 KB of structured tags (`wall_close`, `marker`, `marker_centroid`, `floor_visible`) per cardinal direction.
...
- Mission-brief interpretation on `husky_maze_corners.wbt`: agent paraphrased the four corners back in turn 2.
- Vision pipeline on `husky_maze_visual.wbt`: agent called `read_camera` in turn 1, described the maze corridor verbatim in turn 2.
...
- **URDF importer doesn't parse `<sensor>` tags** — the `WbUrdfImporter.cpp` (1302 lines) has no sensor-tag handling.
...
For `husky_maze.wbt` with the trivial "drive to (10, 0)" brief, the agent runs ~28 cells in 28 chat turns and 5 minutes wall-clock. The script runs all 72 cells in ~90 seconds with no platform dependency. **Use the script.** The agent layer for this world is a more expensive way to get the same outcome.

For `husky_maze_unknown.wbt` (lidar wall-follow, same destination), the same logic applies. The wall-follower is a 138-step mechanical loop with no decisions an LLM adds value to.

## Where the agent earns its keep

For `husky_maze_corners.wbt` and `husky_maze_visual.wbt`, the agent is doing work no script can do — interpreting natural language, reasoning about visual content, deciding when to claim completion. Even if the chat-loop latency is high, *there is no faster path that produces the same outcome*. The script-only path produces the wrong outcome (`mission_complete=false`).

For `husky_maze_blind.wbt` the honest claim is narrower: the perception layer (deterministic Python in the husky_eye sidecar) is doing the recognition, and a non-LLM script could in principle consume the same `scan_surroundings` tags. What the agent earns its keep on here is brief interpretation (knowing "RED" matters, not green or blue) plus tolerant navigation over imperfect symbolic perception — and it does so at ~100× the per-turn cost of the raw `read_camera` path. The blind world is best read as the **architecture demo** for cost-effective agents on vision-rich worlds, not as a stronger version of layer C.
...
- **20 tools** registered (12 safe, 8 guarded, after the camera + mission additions).
...
- **Real OmniSim Camera in the URDF** — would require writing C++ in `WbUrdfImporter.cpp` to parse `<sensor type="camera">` tags. Removes the sidecar.
...
- **Vision-only worlds with hidden maps** — strip both `try_get_known_map` AND `read_lidar`, only camera + pose. The current `husky_maze_visual.wbt` keeps the map available; the strict-vision-only variant is a one-line gate change.
