...
> ## ⚠️ RETRACTED — do not cite these numbers (banner added 2026-07-19)
>
> **Every run recorded below was measured before commit `b18bd7a3`
> ("husky_maze: wheels-only navigation, no teleport recovery anywhere",
> 2026-05-24), under a regime this project has since declared dishonest.**
> The traces here are dated 2026-04-26 through 2026-04-28.
>
> Two defects invalidate the results, both fixed by `b18bd7a3`:
>
> 1. **Teleport-through-walls recovery.** `goto_cell`, `execute_path` and
>    `drive_to_waypoint` in `husky_omnilink_bridge.py` carried a
>    wedge-recovery **snap** that teleported the husky to the target cell
>    when the wheel controller could not get there. The husky did not
>    always drive the paths reported below — it was sometimes moved
>    through walls. The commit message records the measured scale: *"An
>    honest run (before this commit) clipped 12 walls via the snap"* on the
>    Corners Tour. Any "goal_reached" / "cells visited" / "turns per cell"
>    figure on this page may include teleported hops.
> 2. **`complete_mission` was agent self-report.** It was a log line, not a
>    check — the bridge recorded whatever the agent claimed. It is now a
>    real grader: `claimed_cells` must appear in the bridge's own visited
>    trail, per-world required cells are enforced, and a bad claim returns
>    409 with `reasons[]` while `mission_complete` stays false. Every
>    `mission_complete: true` below is an unverified claim by the agent
>    about itself.
>
> Also unenforced at the time: the bridge did not validate hop legality
> against maze adjacency, so an agent-supplied `prev_cell` could route a
> path through a wall.
>
> The results are kept, unedited, as a record of what was believed and
> when. They are **not** evidence of agent or bridge capability, and no
> number on this page should be quoted anywhere. Re-measurement under the
> post-`b18bd7a3` bridge has not been done; until it is, treat the Husky
> Maze demo as unquantified.
...
curl -X POST http://127.0.0.1:6070/admin/reload \
     ...
     -d '{"world": "projects/samples/demos/worlds/flagship/husky_maze_unknown.omniworld"}'
python agents/production/husky_maze/solve.py
```
...
- **`<sensor type="camera">` in the URDF** — OmniSim's URDF importer (`OmUrdfImporter.cpp`, 1302 lines) has no sensor-tag handling. The Gazebo-style sensor extensions are silently ignored.
...
Live agent run on `husky_maze_visual.omniworld`:
...
The bridge now reads `WorldInfo.info` as a **mission brief** (free-form natural language) and exposes it on `/mission`. A new world `husky_maze_corners.omniworld` ships with a multi-objective brief that the hardcoded `solve.py` cannot satisfy.
...
World: `husky_maze.omniworld` (seed-7).
...
End-to-end run, husky_maze.omniworld, v3:
...
## Direction C — hardcore vision-only world (`husky_maze_blind.omniworld`)

The visual world (`husky_maze_visual.omniworld`) keeps `try_get_known_map`
...
world `husky_maze_blind.omniworld` whose title contains "Blind", which the
...
launch.bat projects\samples\demos\worlds\flagship\husky_maze_blind.omniworld
# or:
curl -X POST http://127.0.0.1:6070/admin/reload \
     -H "Content-Type: application/json" \
     -d "{\"world\": \"projects/samples/demos/worlds/flagship/husky_maze_blind.omniworld\"}"
...
End-to-end run, husky_maze_blind.omniworld, v1, 15-turn budget:
...
#### End-to-end run (60-turn budget on `husky_maze_blind.omniworld`)
...
End-to-end captain delegation, `husky_maze.omniworld`, slim sub-chat path
