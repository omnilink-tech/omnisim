...
"""Unified recall across Husky Maze's memory tiers.

One call queries the curated knowledge folder (bridge contract, maze layout
conventions) AND the agent's local long-term memory (saved BFS plans by world
title, fault patterns, operator overrides), labels each hit by source, and
flags contradictions when both tiers answer. Preferred lookup for any factual
question — agent should call this BEFORE re-deriving things from raw tool
output.

The fusion logic is shared across all production agents in
`_lib.agent_recall`; the agent wires the two backend impls onto the module
globals below at startup and this shim forwards them to the shared body.
"""
...
# Wired by the agent at startup (see <agent>_agent.py): the search_knowledge
# tool's impl and local_memory.search_local_memory_for_recall, respectively.
...
def _impl_recall(
    ...
    **_: Any,
    ...
    return _recall(
        ...
        empty_query_hint="Pass the topic you want to look up, e.g. 'ur5e_01 home pose' or 'IK_MAX_DQ override'.",
    )
