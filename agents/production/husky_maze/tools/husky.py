...
map" property honest in the husky_maze_unknown.omniworld demo: the agent only
...
def _impl_execute_cell_mission(
    waypoints: Any = None,
    rationale: str = "",
    return_to_start: bool = True,
    attempts_per_waypoint: int = 3,
    speed: float = 0.42,
    **_: Any,
) -> Dict[str, Any]:
    """Execute a verified multi-waypoint mission with bounded recovery.

    The model still owns intent interpretation and waypoint ordering. The
    runtime owns the repetitive execution contract: plan from live state,
    retry a partial leg after clearing the active fault, stop at the first
    unverified waypoint, return to the measured start when requested, and
    submit one bridge-verified claim only after every leg settled.
    """
    from _lib.supervision import execute_verified_sequence

    if not isinstance(waypoints, list) or not waypoints:
        return {"error": "waypoints must be a non-empty list of [col,row] pairs"}
    if not rationale or not rationale.strip():
        return {"error": "rationale is required"}
    try:
        attempts = max(1, min(6, int(attempts_per_waypoint)))
        base_speed = max(0.2, min(0.7, float(speed)))
    except (TypeError, ValueError):
        return {"error": "attempts_per_waypoint and speed must be numeric"}
    ...
    for index, raw in enumerate(waypoints):
        try:
            ...
            return {"error": "waypoint must be [col,row]", "index": index}
        if not (0 <= cell[0] <= 10 and 0 <= cell[1] <= 10):
            return {"error": "waypoint_out_of_bounds", "index": index, "cell": cell}
        if not ordered or ordered[-1] != cell:
            ordered.append(cell)
    if len(ordered) > 12:
        return {"error": "too_many_waypoints", "maximum": 12}

    initial = bridge_get("state") or {}
    start_raw = initial.get("current_cell") or {}
    try:
        start_cell = [int(start_raw.get("col")), int(start_raw.get("row"))]
    except (TypeError, ValueError):
        return {"error": "could_not_measure_start_cell", "state": initial}
    if return_to_start and ordered[-1] != start_cell:
        ordered.append(start_cell)

    def _execute(step: Dict[str, Any]) -> Dict[str, Any]:
        cell = step["cell"]
        attempt_log = []
        for attempt in range(1, attempts + 1):
            # Slow down after a wedge: skid-steer overshoot is the dominant
            # cause of goto_cell_timeout near tight corners.
            attempt_speed = max(0.24, base_speed - 0.06 * (attempt - 1))
            result = _impl_drive_to_cell(
                to_col=cell[0],
                to_row=cell[1],
                max_replans=8,
                max_wall_seconds=300.0,
            ...
            attempt_log.append(
                {"attempt": attempt, "speed": attempt_speed, "result": result}
            )
            ...
            bridge_post("action", {"action": "stop"})
        return {
            "completed": False,
            "verified": False,
            "cell": cell,
            "attempts": attempts,
            "attempt_log": attempt_log,
            "reason": "waypoint_recovery_exhausted",
        }

    steps = [
        {"id": f"waypoint_{index + 1}", "cell": cell}
    ...
    claimed = []
    for cell in [start_cell] + ordered:
        if cell not in claimed:
            claimed.append(cell)
    claim = _impl_complete_mission(
        rationale=rationale,
        claimed_cells=claimed,
    )
    verified = bool(
        claim.get("mission_complete") is True and claim.get("verified") is True
    )
    return {
        **({} if verified else {"error": "bridge_refused_mission_claim"}),
        "completed": verified,
        "verified": verified,
        "start_cell": start_cell,
        "waypoints": ordered,
...
def _impl_drive_forward(distance: float = 0.0, speed: float = 0.5, **_: Any) -> Dict[str, Any]:
    # wait=True: the tool description promises a blocking, closed-loop call.
    # It used to post without it, so the bridge returned on acceptance and the
    # description was false -- a model that believed it drove into a robot
    # that was still moving. See PROTOCOL.md 5.4.1.
    return bridge_post("action", {
...
SPECS = [
    ...
    ToolSpec(
        name="execute_cell_mission",
        tier=GUARDED,
        surface=ALWAYS,
        description=(
            "Execute an ordered multi-waypoint mission under one supervised "
            "runtime contract. You interpret the brief and provide waypoints; "
            "the tool plans from live state, retries partial legs at slower "
            "speeds, stops on the first unverified waypoint, optionally "
            "returns to the measured start, and calls bridge-verified "
            "complete_mission only after every leg settles. Preferred for "
            "known-map tours, patrols, inspections, and return-to-start tasks."
        ),
        parameters={
            "type": "object",
            "properties": {
                "waypoints": {
                    ...
                    "description": "Ordered objective cells as [[col,row], ...].",
                },
                "rationale": {
                    "type": "string",
                    "description": "Why these waypoints satisfy the mission brief.",
                },
                "return_to_start": {
                    "type": "boolean",
                    "description": "Append the measured start cell (default true).",
                },
                "attempts_per_waypoint": {
                    "type": "integer",
                    "description": "Bounded recovery attempts per waypoint (default 3).",
                },
                "speed": {
                    "type": "number",
                    "description": "Initial motion speed fraction; retries slow down.",
                },
            },
            "required": ["waypoints", "rationale"],
        },
        impl=_impl_execute_cell_mission,
        tags=["maze", "navigation", "mission", "recovery"],
    ),
    ToolSpec(
        name="plan_path",
        tier=SAFE,
    ...
    ToolSpec(
        ...
        description=(
            ...
            "max_linear_m_s` (default 0.5). BLOCKING and closed-loop on "
            "pose: it returns only once the move has finished, and gives "
            "you `achieved` in metres plus `error` = achieved - commanded. "
            "Report those, not the distance you asked for. Heading is held "
            "constant. Negative distance reverses."
        ),
        parameters={
            "type": "object",
            "properties": {
    ...
    ToolSpec(
        ...
        description=(
            ...
            "(default 0.6). Positive = left turn. BLOCKING and closed-loop "
            "on yaw: it returns only once the turn has finished, and gives "
            "you `achieved` in radians plus `error` = achieved - commanded. "
            "Report those, not the angle you asked for."
        ),
        parameters={
            "type": "object",
            "properties": {
