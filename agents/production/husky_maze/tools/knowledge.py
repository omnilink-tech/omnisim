...
"""Search Husky Maze's curated knowledge folder.

The `knowledge/` folder sits alongside `tools/` under this agent's
directory. It holds source-controlled grounding material: the bridge
HTTP contract, the maze world layout convention, and any other
authoritative reference the agent should consult before reasoning.

`search_knowledge(query)` scans the folder on every call, chunks each
file on heading or blank-line boundaries, scores chunks by keyword hits
(plus a proximity bonus when multiple terms land close together), and
returns the top N matches with file, line range, and text.

The scan/chunk/score implementation is shared across all production
agents in `_lib.agent_knowledge`; this module only binds it to this
agent's `knowledge/` folder and declares the tool spec.
"""
