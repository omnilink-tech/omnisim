...
- **29 tools registered (15 safe, 14 guarded)**: motion + bridge + lidar + camera + map + mission, plus `save_local_memory` / `search_local_memory` / `list_local_memories` / `forget_local_memory` / `recall` / `search_knowledge` for cross-session memory. *(Count check: `tools.load_all()` — the loader auto-discovers every `SPEC`/`SPECS` under `tools/`, so this number is derivable, never hand-kept.)*
...
- Implements primitive motion (`set_velocity`, `drive_forward`, `turn`, `goto_cell`, `drive_to_waypoint`) plus admin actions (`stop`, `reset`). `snap_to_cell` is retained only as a 410 stub — see below
...
- For each cell move, drives there under wheel control: `turn → drive_forward`, or `goto_cell` / `drive_to_waypoint` for the closed-loop version.

> **Teleport-snapping has been removed.** Earlier revisions of this demo re-anchored the Husky to the cell centre after every move (`snap_to_cell`), a concession to the ~0.5 m drift skid-steer pivots accumulate per 90° turn. That is gone: the robot now navigates entirely under wheel control, and `snap_to_cell` returns **410** so older callers fail loudly instead of silently changing behaviour. Any result predating that change is not comparable — see the caveat banners in `docs/RESULTS.md`.
...
- Cell moves are wheel-driven end to end; nothing re-anchors the robot's pose. `complete_mission` checks every `claimed_cells` entry against the trail recorded from the physics body pose and returns 409 if the agent claims a cell it never reached.
