...
1. **`complete_mission` with a one-sentence rationale and `claimed_cells`** — this is how the operator and the bridge know you consider the mission done. The rationale is free text and is only logged — but `claimed_cells` is **checked**: the bridge compares every cell you claim against the trail it recorded from the robot's actual physics pose, and refuses the whole call with a 409 if you claim a cell you never reached. Claiming a cell you did not drive to does not get you a pass, it gets you an error. The tool wrapper enforces the no-fault precondition.
