...
- [x] Wire `husky_maze.omniworld` to the bridge with `supervisor TRUE`
...
- [x] Headless verification of `husky_maze.omniworld` after the controller swap
...
- [x] Generate `husky_maze_unknown.omniworld` (seed=19, different layout)
...
- [x] Generator accepts `--info` for the brief; new world `husky_maze_corners.omniworld` with multi-corner brief.
...
- [x] `husky_maze_visual.omniworld` (seed 100) ships three coloured cylinders + the brief.
...
- [ ] Reuse the bridge for `warehouse_husky.omniworld` (no maze, but same motion primitives + a different world title that hides the map)
