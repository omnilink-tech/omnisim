...
`husky_maze.omniworld` (and `husky_maze_unknown.omniworld`) has:
...
- `husky_maze.omniworld` → title `"Husky Maze"` → `map_available: true`
- `husky_maze_unknown.omniworld` → title `"Husky Maze (Unknown)"` → `map_available: false`
