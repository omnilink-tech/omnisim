...
The maze graph itself is not stored here — it is parsed from `husky_maze.omniworld` on demand by the `get_maze_graph` tool, so the world file remains the single source of truth.
