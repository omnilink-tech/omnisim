...
"""Mission Captain — tool registry. Mirrors husky_maze's loader."""
...
from ._base import ToolSpec  # noqa: F401
    ...
    for info in pkgutil.iter_modules([str(pkg_path)]):
        ...
        for spec in specs:
            if spec.name in registry:
                raise RuntimeError(f"duplicate tool {spec.name!r} in {info.name}")
