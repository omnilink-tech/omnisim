...
"""Local long-term memory for warehouse_foreman (shim).
...
SPECS = [
    ToolSpec(
        ...
        surface=ALWAYS,
        description=(
            "Persist a durable mission pattern into Mission Captain's "
            "local long-term memory. Title format: '<specialists involved>: "
            "<mission-shape> on <world(s)>'. Body: the leg sequence that "
            "worked, with self-contained task strings ready to delegate "
            "next time. Tag with operator-goal keywords + specialist names "
            "+ world titles. Stored under agents/production/mission_captain/"
            "long_term_memory/ with hybrid embedding+BM25 search."
        ),
        parameters={
            "type": "object",
            "properties": {
    ...
    ToolSpec(
        ...
        surface=ALWAYS,
        description=(
            "Search Mission Captain's local long-term memory. Default "
            "mode is hybrid (vector + BM25 + RRF). Call with keywords "
            "from the operator's request to see if a similar mission's "
            "leg sequence is already saved — if yes, lift the plan and "
            "skip decomposition. Prefer `recall` for cross-tier lookups."
        ),
        parameters={
            "type": "object",
            "properties": {
