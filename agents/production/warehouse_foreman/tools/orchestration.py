...
def _impl_delegate_to_agent(
    ...
    **_: Any,
    ...
    # Snapshot the specialist's pre-delegation completion state. We treat
    # "this delegation succeeded" as a transition: either the bridge's
    # mission_complete flag flips false->true, or the runner's session
    # complete-call counter increments. Without a baseline, a stale
    # mission_complete=true from a previous run (the bridge persists
    # the flag until the world is reverted, and the picker has no
    # bridge-side reset on its in-session counter either) would make
    # every delegation immediately "succeed" without doing any work.
    pre_status = http_get(info["runner_status_url"], timeout=2.0)
    pre_complete = bool(pre_status.get("mission_complete")) if isinstance(pre_status, dict) else False
    pre_count = 0
    if isinstance(pre_status, dict):
        for k in ("complete_calls_this_session",
                  "captain_complete_calls_this_session",
                  "foreman_complete_calls_this_session"):
            v = pre_status.get(k)
            if isinstance(v, int):
                pre_count = v
                break
    ...
    def _completed(status: Any) -> bool:
        if not isinstance(status, dict):
            return False
        # New-true from baseline of mission_complete (husky_maze pattern).
        if status.get("mission_complete") and not pre_complete:
            return True
        # Counter increment from baseline (picker / captain / foreman pattern).
        for k in ("complete_calls_this_session",
                  "captain_complete_calls_this_session",
                  "foreman_complete_calls_this_session"):
            v = status.get(k)
            if isinstance(v, int) and v > pre_count:
                return True
        return False

    print(f"  [CAPTAIN] delegating to {agent!r}: {task!r:.120}")
    print(f"  [CAPTAIN] pre-status: complete_count={pre_count} mission_complete={pre_complete}")
    ...
    tool_calls_total = 0
    last_text = ""
    no_tool_streak = 0  # consecutive turns with no tool calls — graceful exit
    ...
    while turns < cap:
        ...
        for attempt in range(3):
            try:
                ...
                last_exc = exc
                # Before retrying, check if the specialist completed
                # mid-flight (counter increment or mission_complete
                # transition from the pre-delegation baseline).
                mid_status = http_get(info["runner_status_url"], timeout=1.0)
                if _completed(mid_status):
                    print(f"  [CAPTAIN] {agent!r} completed during chat retry; success")
                    return {
                        "success": True,
                        "agent": agent,
                        "turns": turns,
        ...
        if response is None:
            ...
            late_complete = _completed(late_status)
            if late_complete:
                return {
                    "success": True,
                    "agent": agent,
        ...
        # the sub-agent's next prompt) can react to a fresh completion
        # signal (counter increment from baseline OR mission_complete
        # transition) without an extra round-trip.
        sub_status = http_get(info["runner_status_url"], timeout=1.0)
        if _completed(sub_status):
            print(f"  [CAPTAIN] {agent!r} reports completion after {turns} turns")
            return {
                "success": True,
                "agent": agent,
                "turns": turns,
        ...
        if not tool_calls:
            no_tool_streak += 1
            # First miss: probably a narration. Nudge once.
            # Two-in-a-row: the specialist genuinely thinks it's done
            # but lacks a complete_mission tool (e.g. the productized
            # Axis agent which just stops calling tools when satisfied).
            # Treat that as a graceful natural completion — the orchestrator
            # exits with success=true if the specialist actually ran some
            # tools at all (otherwise nothing happened and it's a bust).
            if no_tool_streak >= 2:
                ran_anything = tool_calls_total > 0
                print(f"  [CAPTAIN] {agent!r} stopped calling tools after "
                      f"{turns} turns (ran_anything={ran_anything})")
                return {
                    "success": ran_anything,
                    "agent": agent,
                    "turns": turns,
                    "tool_calls": tool_calls_total,
                    "final_text": last_text,
                    # No complete_mission counter increment, so we can't
                    # claim mission_complete=true here. The orchestrator
                    # caller decides whether ran_anything is enough.
                    "mission_complete": False,
                    "sub_status": http_get(info["runner_status_url"], timeout=1.0),
                    "reason": "specialist stopped emitting tool calls — assuming natural exit",
                }
            ...
            messages.append({
                ...
                "content": (
                    ...
                    "task is complete, call complete_mission (or, if you "
                    "do not have a complete_mission tool, simply do not "
                    "emit any more tool calls — the orchestrator will "
                    "treat that as success). If the task can't be done, "
                    "explain why and stop."
                ),
            })
            continue
        # Reset the no-tool streak as soon as the specialist calls anything.
        no_tool_streak = 0
        ...
        for tc in tool_calls:
            ...
            payload = dict(args)
            payload["tool"] = name
            # 600 s ceiling on a single tool dispatch — long enough for
            # the picker's push_pallet_to to finish a full two-segment
            # warehouse traversal (~120 s per segment under contact),
            # short enough that a genuinely-stuck call still surfaces
            # before max_turns. Override via SUB_AGENT_TOOL_TIMEOUT env.
            _tool_timeout = float(os.environ.get("SUB_AGENT_TOOL_TIMEOUT", "600"))
            tool_call_result = http_post(info["tool_callback_url"], payload, timeout=_tool_timeout)
    ...
    return {
        ...
        "tool_calls": tool_calls_total,
        "final_text": last_text,
        "mission_complete": _completed(sub_status),
        "sub_status": sub_status,
