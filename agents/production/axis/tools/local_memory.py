...
"""Local long-term memory for axis (shim).
...
SPECS = [
    ToolSpec(
        ...
        surface=ALWAYS,
        description=(
            "Persist a durable note into Axis's local long-term memory "
            "folder. Use for facts that should survive across sessions: "
            "robot calibrations, operator-authored safety overrides "
            "(e.g. tightened IK_MAX_DQ for a specific robot), recurring "
            "failure signatures, reusable trajectory recipes, deployment "
            "topology. The note is saved as markdown under "
            "agents/axis/long_term_memory/ and indexed with an embedding "
            "(via local Ollama when available; BM25 fallback). One concrete "
            "fact per memory — do not dump whole session logs."
        ),
        parameters={
            "type": "object",
            "properties": {
                ...
                "tags": {"type": "array", "items": {"type": "string"}, "description": "Optional tags: robot id, 'calibration', 'safety', 'trajectory', etc."},
            },
            "required": ["title", "body"],
        },
    ...
    ToolSpec(
        ...
        surface=ALWAYS,
        description=(
            "Search Axis's local long-term memory. Default mode is hybrid — "
            "runs vector cosine (via Ollama embeddings) AND BM25 lexical "
            "ranking, then fuses results via Reciprocal Rank Fusion. This "
            "beats either signal alone on short robot notes where two "
            "calibrations can sit at near-identical cosine distance; BM25 "
            "breaks the tie via distinctive terms (robot ids, joint names), "
            "and vector catches paraphrases BM25 misses. Prefer `recall` "
            "for cross-tier lookups; this is local-memory-only."
        ),
        parameters={
            "type": "object",
            "properties": {
