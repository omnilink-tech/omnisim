...
"""Drive the Patrol Husky agent via OmniLink chat() with a local
tool loop.
...
The Picker runner (warehouse_patrol_agent.py) must already be running
...
ENGINE = "g1-engine"
_PATROL_PORT = int(os.environ.get("PATROL_PORT", "51530"))
TOOL_SERVER_URL = f"http://127.0.0.1:{_PATROL_PORT}/tool"
ACTIVITY_URL = f"http://127.0.0.1:{_PATROL_PORT}/activity"
STATUS_URL = f"http://127.0.0.1:{_PATROL_PORT}/status"
MAX_TURNS = int(os.environ.get("PATROL_CHAT_MAX_TURNS", "12"))
...
def execute_tool(name: str, args: dict) -> dict:
    ...
    timeout = int(os.environ.get("PATROL_DRIVE_TOOL_TIMEOUT", "300"))
    try:
...
def main() -> int:
    # Probe the runner for its sector assignment so the default prompt
    # bakes the right sector_name into the agent's instructions. Falls
    # back to PATROL_SECTOR env or "north" if /status isn't reachable.
    runner_sector = os.environ.get("PATROL_SECTOR", "north").lower()
    try:
        with urllib.request.urlopen(STATUS_URL, timeout=2) as r:
            runner_sector = (json.loads(r.read()).get("sector") or runner_sector).lower()
    except Exception:
        pass

    default_prompt = (
        f"Run a patrol sweep of your assigned sector. Sector: {runner_sector}. "
        f"Workflow: (1) call recall_last_sweep(sector_name='{runner_sector}') to "
        "fetch the prior manifest from local memory; if it returns 'no prior "
        "sweep', that's expected on the first run — proceed to step 2 and "
        "establish a baseline. (2) call sweep_summary(sector_name="
        f"'{runner_sector}') to drive each crate vantage in the sector and "
        "persist the new manifest. (3) if you got a prior manifest in step 1, "
        "call diff_sweeps(prior=<step1.manifest>, current=<step2.manifest>) "
        "and narrate the result. (4) call complete_mission with a one-sentence "
        "summary: either 'baseline established for {sector}' or 'X crates "
        "moved, Y missing, Z unchanged in {sector}'."
    )
    ...
    if profile is None:
        print(f"Error: profile {AGENT_NAME!r} not found. "
              f"Start warehouse_patrol_agent.py first.")
        return 1
