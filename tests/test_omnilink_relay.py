...
try:
    ...
    for module_name in list(sys.modules):
        if module_name == "omnilink" or module_name.startswith("omnilink."):
    ...
    class _Delta:
        ...
        def report(self):
            return f"{self.values['total_units']} units"
    ...
    class UsageMeter:
        ...
        def _rollup(self):
            return self.client.get_usage(limit=1)["rollup"]

        def start(self):
            self.baseline = self._rollup()
            return self.baseline

        def snapshot(self, baseline=None):
            before = baseline or self.baseline or {}
            after = self._rollup()
            input_units = after.get("input_units_24h", 0) - before.get("input_units_24h", 0)
            output_units = after.get("output_units_24h", 0) - before.get("output_units_24h", 0)
            return _Delta({
                "input_units": input_units,
                "output_units": output_units,
                "cached_input_units": after.get("cached_input_units_24h", 0) - before.get("cached_input_units_24h", 0),
                "total_units": input_units + output_units,
                "credits": after.get("credits_24h", 0) - before.get("credits_24h", 0),
            })

    client_mod.OmniLinkClient = _OfflineClient
    client_mod.OmniLinkAPIError = OmniLinkAPIError
    usage_mod.UsageMeter = UsageMeter
    pkg.client = client_mod
    pkg.usage_meter = usage_mod
    sys.modules.update({
        "omnilink": pkg,
        "omnilink.client": client_mod,
        "omnilink.usage_meter": usage_mod,
    })
...
def test_http_error_surfaced_as_error() -> None:
    ...
    err = OmniLinkAPIError(401, {"error": "invalid key"})
    ...
    assert err_events and "401" in str(err_events[0]["text"]), f"missing 401 surface: {err_events}"
...
def test_timed_out_queued_prompt_never_executes_later() -> None:
    """A timed-out request waiting behind another prompt is cancelled.

    This is the regression for the old 90-second HTTP timeout behavior where
    a caller could retry while the original queued motion executed later.
    """
    ...
    class BlockingClient:
        ...
        def chat(self, **_kwargs):
            ...
            return {"ok": True, "text": "done", "toolCalls": []}

    tool = Tool("noop", "n", {"type": "object", "properties": {}}, lambda _a: {"ok": True})
    relay = _make_relay([tool], [])
    client = BlockingClient()
    relay._client = client
    relay.dispatch_async("first", lambda _kind, _payload: None)
    assert first_started.wait(2)

    timed_out = relay.dispatch_sync("second", timeout_s=0.02)
    assert timed_out["error"] == "timeout"
    assert timed_out["cancelled"] is True
    release_first.set()
    relay.close()
    relay._worker.join(timeout=2)
    assert client.calls == 1, "cancelled queued request reached OmniLink later"
    print("[test_timed_out_queued_prompt_never_executes_later] PASS")


def test_transport_retry_reuses_request_id() -> None:
    """A lost response must not create a second logical model/tool turn."""
    class RetryClient:
        def __init__(self):
            self.request_ids = []

        def chat(self, **kwargs):
            self.request_ids.append(kwargs["requestId"])
            if len(self.request_ids) == 1:
                raise OmniLinkAPIError(503, {"error": "temporary"})
            return {"ok": True, "text": "done", "toolCalls": []}
    ...
    client = RetryClient()
    relay._client = client
    assert relay._post_chat([])["text"] == "done"
    relay.close()
    assert len(client.request_ids) == 2
    assert len(set(client.request_ids)) == 1, client.request_ids
    print("[test_transport_retry_reuses_request_id] PASS")


def test_timeout_returns_promptly_when_tool_is_already_in_flight() -> None:
    started = threading.Event()
    release = threading.Event()

    def blocking_action(_args):
        started.set()
        assert release.wait(2)
        return {"accepted": True}

    tool = Tool("move", "m", {"type": "object", "properties": {}}, blocking_action)
    relay = _make_relay([tool], [{
        "ok": True, "text": "", "toolCalls": [{"id": "c1", "name": "move", "arguments": {}}]
    }])

    result_box = {}

    def invoke():
        result_box["value"] = relay.dispatch_sync("move", timeout_s=0.05)

    caller = threading.Thread(target=invoke)
    caller.start()
    assert started.wait(1)
    caller.join(timeout=0.5)
    assert not caller.is_alive(), "timeout waited for the in-flight tool to finish"
    result = result_box["value"]
    assert result["in_flight"] is True
    assert result["do_not_retry"] is True
    assert result["error"] == "timeout_after_actions"
    release.set()
    relay.close()
    relay._worker.join(timeout=2)
    print("[test_timeout_returns_promptly_when_tool_is_already_in_flight] PASS")
...
if __name__ == "__main__":
    ...
    test_timed_out_queued_prompt_never_executes_later()
    test_transport_retry_reuses_request_id()
    test_timeout_returns_promptly_when_tool_is_already_in_flight()
