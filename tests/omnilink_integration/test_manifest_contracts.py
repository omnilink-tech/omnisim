...
AGENTS_DIR = REPO_ROOT / "omnilink-agents"
...
def test_manifest_world_resolves(name: str, path: Path, data: Dict):
    ...
    assert full.suffix == ".wbt", f"{name}: world is not a .wbt file"
...
def _bridge_ports_declared_in_controllers() -> Dict[int, Path]:
    ...
    OmniSim-bridge range 6000–6999 that appears anywhere in the
    controller source (``BRIDGE_PORT = 6060``,
    ...
    if not CONTROLLERS_DIR.exists():
        return result
    pattern = re.compile(r"\b(6[0-9]{3})\b")
