...
def _in_process_smoke() -> int:
    ...
    Verifies that OmCudaBuffer round-trips inside the actual omnisim-bin ABI —
    ...
    if "CUDA smoke PASS" in log_text:
        print("[run_smoke] in-process PASS: OmCudaBuffer round trip verified inside omnisim-bin.")
        return 0
