/*
 * Description:  Test painting with Pen on a rotated and scaled capsule.
 */
...
int main(int argc, char **argv) {
  ...
  color = getPixelColor(4, 40);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (4, 40) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after first paint call", 206, 171, 58,
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels <= 80,
