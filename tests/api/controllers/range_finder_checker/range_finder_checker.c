...
int main(int argc, char **argv) {
  ...
  if (strcmp(argv[1], "range_finder") == 0) {
    ...
    // POST-D1.4 RE-PIN (wren-deletion-runbook F2 -> D1.4): this world's
    // RangeFinder declares a non-planar (cylindrical) projection, which the wgpu
    // sensor path does not implement. The device DECLINES the render (warned once,
    // by name: "the wgpu render is DECLINED for this device and its image stays
    // empty") and the range image stays the honestly-empty buffer -- 0.0 at every
    // pixel, NOT a distance and NOT the +inf miss contract (nothing rendered at
    // all). Through F2 the test passed byte-for-WREN via the WREN fallback; D1.4
    // deleted the fallback, so the decided expectation is the empty buffer.
    //
    // Old (WREN-era) expectations at the same five sample positions, recorded per
    // the standing re-golden rule:
    //   2.0 (collision), INFINITY (no collision), 2.0 (collision),
    //   INFINITY (no collision), 2.0 (collision)
    float v;
    int x, y;
    const float *image = wb_range_finder_get_range_image(range_finder);
    ...
    const double samples_expected_values[5] = {
      0.0,  // DECLINED, empty buffer (was 2.0, collision)
      0.0,  // DECLINED, empty buffer (was INFINITY, no collision)
      0.0,  // DECLINED, empty buffer (was 2.0, collision)
      0.0,  // DECLINED, empty buffer (was INFINITY, no collision)
      0.0,  // DECLINED, empty buffer (was 2.0, collision)
    };
