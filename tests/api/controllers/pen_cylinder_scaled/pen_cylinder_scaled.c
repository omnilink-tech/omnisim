/*
 * Description:  Test painting with Pen on a rotated and scaled cylinder.
...
int main(int argc, char **argv) {
  ...
  ts_assert_boolean_equal(numColorPixels < 51,
                          "The number of pixels painted after the first step should be lower than 51 not %d", numColorPixels);

  color = getPixelColor(26, 40);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (26, 40) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after first paint call", 206, 171,
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 102,
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 102 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(43, 25);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (43, 25) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
    58, color.r, color.g, color.b);
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 112,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 112 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(49, 14);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (49, 14) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
