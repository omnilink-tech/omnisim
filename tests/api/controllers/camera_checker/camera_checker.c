...
int main(int argc, char **argv) {
  ...
  if (strcmp(argv[1], "camera_color_spherical") == 0) {
    ...
    // Post-D1.4 re-pin note: this declaration used to SHADOW the outer
    // `spherical_camera` (a second `WbDeviceTag spherical_camera = ...` here), so
    // the outer variable stayed 0 and the spherical sample block below never ran
    // -- the spherical half of this test was dead even in the WREN era. Fixed
    // together with the re-pin so the spherical camera's decided behaviour is
    // actually asserted.
    spherical_camera = wb_robot_get_device("spherical camera");
  ...
  if (strcmp(argv[1], "camera_color") == 0) {
    ...
    // POST-D1.4 RE-PIN (wren-deletion-runbook F2 -> D1.4): non-planar
    // (cylindrical/spherical) Camera projections are RETIRED-unported on the wgpu
    // arm. The device DECLINES the render (warned once per device, by name:
    // "projection ... is not implemented by the wgpu sensor path (planar pinhole
    // only) -- the wgpu render is DECLINED for this device and its image stays
    // empty") and the image is honestly EMPTY -- every pixel (0, 0, 0). Through F2
    // this test still passed byte-for-WREN via the WREN fallback; D1.4 deleted the
    // fallback, so the decided expectation is now the empty image itself.
    //
    // Old (WREN-era) expectations, recorded per the standing re-golden rule --
    // cylindrical camera at {22,6}/{230,9}/{19,115}/{234,115}/{146,11}/{81,104}:
    //   {207,207,0} yellow, {207,0,0} red, {207,207,0} yellow, {207,0,0} red,
    //   {203,83,124} pink, {173,198,63} green-ish
    // spherical camera at {22,6}/{87,32}/{118,38}/{204,63}/{146,11}/{88,95}:
    //   {175,175,175} gray, {202,93,185} pink, {98,59,131} purple,
    //   {41,62,171} blue, {147,147,147} gray, {202,93,185} pink
    // (the spherical block was additionally dead code before this re-pin -- see
    // the shadowing note at the top of main()).
    const int samples_number = 6;

    {  // 6 cylindrical camera sample pixels -- DECLINED, image stays empty
      int samples_positions[6][2] = {{22, 6}, {230, 9}, {19, 115}, {234, 115}, {146, 11}, {81, 104}};
      int samples_expected_colors[6][3] = {
        {0, 0, 0},  // was {207, 207, 0} yellow
        {0, 0, 0},  // was {207, 0, 0} red
        {0, 0, 0},  // was {207, 207, 0} yellow
        {0, 0, 0},  // was {207, 0, 0} red
        {0, 0, 0},  // was {203, 83, 124} pink
        {0, 0, 0}   // was {173, 198, 63} green-ish
      };
      check_pixel_values(camera, samples_number, samples_positions, samples_expected_colors);
    }

    if (spherical_camera > 0) {  // 6 spherical camera sample pixels -- DECLINED, image stays empty
      int samples_positions[6][2] = {{22, 6}, {87, 32}, {118, 38}, {204, 63}, {146, 11}, {88, 95}};
      int samples_expected_colors[6][3] = {
        {0, 0, 0},  // was {175, 175, 175} gray
        {0, 0, 0},  // was {202, 93, 185} pink
        {0, 0, 0},  // was {98, 59, 131} purple
        {0, 0, 0},  // was {41, 62, 171} blue
        {0, 0, 0},  // was {147, 147, 147} gray
        {0, 0, 0}   // was {202, 93, 185} pink
      };
