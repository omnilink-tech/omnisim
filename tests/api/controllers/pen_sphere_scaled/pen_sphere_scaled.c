/*
 * Description:  Test painting with Pen on a scaled sphere.
 */
...
int main(int argc, char **argv) {
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels <= 140,
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 140 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(21, 30);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (21, 30) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
    58, color.r, color.g, color.b);
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 174,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 174 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(30, 18);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (30, 18) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
