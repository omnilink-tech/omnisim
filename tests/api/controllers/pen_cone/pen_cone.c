/*
 * Description:  Test painting with Pen on a rotated cone.
...
int main(int argc, char **argv) {
  ...
  color = getPixelColor(104, 80);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 10,
    "Pixel (104, 80) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after first paint call", 206, 171,
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 460,
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 460 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(80, 60);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 10,
    "Pixel (80, 60) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
    58, color.r, color.g, color.b);
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 520,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 520 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(19, 51);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 10,
    "Pixel (19, 51) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
