...
// F2.5 RE-GOLDEN (wren-deletion-runbook, 2026-08-23). This test is about UPDATE TIMING
// (camera1 at half rate must lag camera0 by one colour), not exact shading, but its old
// expectation was `255 * diffuseColor` verbatim -- true only of WREN's flat ambient render.
// The wgpu sensor pipeline is lit + AgX-tonemapped, so each authored colour maps to a fixed
// but different display value (AgX's inset matrix mixes channels; a saturated teal gains
// blue). Expected values below are MEASURED on the wgpu pipeline (machine 9722d23d12a3,
// 2026-08-23); the OLD computed expectations are recorded per line. The lag structure the
// test exists for is asserted exactly as before.
static int expectedDisabled[3] = {0, 0, 0};      // old [0 0 0] -- disabled camera, unchanged
static int expected0[3] = {204, 204, 204};       // old [204 204 204] -- grey survives AgX within delta
static int expected1[3] = {6, 190, 171};         // old [0 178 127]
static int expected2[3] = {190, 171, 190};       // old [179 128 179]
static int expected3[3] = {132, 132, 190};       // old [64 64 179]

static void check_camera_image(WbDeviceTag tag, int width, const int *expected_rgb) {
  ...
  int exp_red = expected_rgb[0];
  int exp_green = expected_rgb[1];
  int exp_blue = expected_rgb[2];
...
int main(int argc, char **argv) {
  ...
  // camera0 updated
  check_camera_image(camera0, width0, expected0);
  // camera1 not updated
  check_camera_image(camera1, width1, expectedDisabled);
  ...
  check_camera_image(camera0, width0, expected0);
  // camera1 updated
  check_camera_image(camera1, width1, expected0);
  ...
  // camera0 updated
  check_camera_image(camera0, width0, expected1);
  // camera1 not updated
  check_camera_image(camera1, width1, expected0);
  ...
  check_camera_image(camera0, width0, expected1);
  // camera1 updated
  check_camera_image(camera1, width1, expected1);
  ...
  // camera0 updated
  check_camera_image(camera0, width0, expected2);
  // camera1 not updated
  check_camera_image(camera1, width1, expected1);
  ...
  check_camera_image(camera0, width0, expected2);
  // camera1 updated
  check_camera_image(camera1, width1, expected2);
  ...
  // camera0 updated
  check_camera_image(camera0, width0, expected3);
  // camera1 not updated
  check_camera_image(camera1, width1, expected2);
  ...
  check_camera_image(camera0, width0, expected3);
  // camera1 updated
  check_camera_image(camera1, width1, expected3);
