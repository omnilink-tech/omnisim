/*
 * Description:  Test painting with Pen on a sphere.
 */
...
int main(int argc, char **argv) {
  ...
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 22,
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels <= 100,
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 100 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(28, 28);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (28, 28) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
    58, color.r, color.g, color.b);
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels <= 170,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 170 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(36, 14);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (36, 14) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
