...
int main(int argc, char **argv) {
  if (argc == 1) {  // launched by Webots
    wb_robot_init();
    putenv("OMNISIM_ROBOT_ID=");  // clear environment variables set by the simulator
    putenv("OMNISIM_INSTANCE_PATH=");
    putenv("OMNISIM_ROBOT_NAME=");
    putenv("OMNISIM_CONTROLLER_URL=extern1");  // set the OMNISIM_CONTROLLER_URL
#ifdef _WIN32
    STARTUPINFO si;
