/*
 * Description:  Test painting with Pen on a rotated capsule.
 */
...
int main(int argc, char **argv) {
  ...
  ts_assert_boolean_equal(numColorPixels <= 40,
                          "The number of pixels painted after the first step should be lower than 40 not %d", numColorPixels);

  color = getPixelColor(19, 50);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 10,
    "Pixel (19, 50) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after first paint call", 206, 171,
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels <= 120,
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 120 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(36, 32);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 10,
    "Pixel (36, 32) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
    58, color.r, color.g, color.b);
  ...
  numColorPixels = computeNumColorPixels();
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 179,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 179 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(45, 12);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 10,
    "Pixel (45, 12) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
