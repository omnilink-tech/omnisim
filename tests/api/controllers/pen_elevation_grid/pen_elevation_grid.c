/*
 * Description:  Test painting with Pen on an elevation grid.
 */
...
int main(int argc, char **argv) {
  ...
  color = getPixelColor(14, 8);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 5,
    "Pixel (14, 8) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after first paint call", 206, 171, 58,
  ...
  // wait until motor reach the coorect position
  i = 0;
  do {
    wb_robot_step(TIME_STEP);
  ...
  ts_assert_boolean_equal(
    ...
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 60 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(20, 20);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 5,
    "Pixel (20, 20) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
  ...
  numColorPixels = computeNumColorPixels();
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 126,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 126 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(33, 55);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 5,
    "Pixel (33, 55) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
