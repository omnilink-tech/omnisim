/*
 * Description:  Test painting with Pen on a rotated cylinder.
 *               The same texture is displayed on each face and
...
int main(int argc, char **argv) {
  ...
  ts_assert_boolean_equal(numColorPixels < 24,
                          "The number of pixels painted after the first step should be lower than 24 not %d", numColorPixels);

  color = getPixelColor(14, 55);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 32,
    "Pixel (14, 55) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after first paint call", 206, 171,
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 105,
    "The number of pixels painted after the second step should be greater than before (%d) and lower than 105 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(32, 47);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (32, 47) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after second paint call", 206, 171,
    58, color.r, color.g, color.b);
  ...
  ts_assert_boolean_equal(
    oldValue <= numColorPixels && numColorPixels < 133,
    "The number of pixels painted after the third step should be greater than before (%d) and lower than 133 not %d", oldValue,
    numColorPixels);

  color = getPixelColor(48, 34);
  ts_assert_color_in_delta(
    color.r, color.g, color.b, 206, 171, 58, 15,
    "Pixel (48, 34) should be painted with color [r=%d, g=%d, b=%d] not [r=%d, g=%d, b=%d] after third paint call", 206, 171,
    58, color.r, color.g, color.b);
