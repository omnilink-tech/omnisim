/*
 * Description:  Test Emitters and Receivers with infra-red transmission type.
...
int main(int argc, char **argv) {
  ...
  switch (robotID) {
    ...
    case 3:
      ...
      queueLength = wb_receiver_get_queue_length(communication);
      ts_assert_int_equal(queueLength, 0,
                          "Case 4 - Receiver \"receiver%d\" should receive %d packets not %d packets when a wall is placed "
                          "between the emitter and the receiver",
                          robotID, 0, queueLength);

      channel = wb_receiver_get_channel(communication);
      ts_assert_boolean_equal(channel == channel1,
                              "Case 4 - Receiver \"receiver%d\" should be set to channel %d not %d before changing it", robotID,
                              channel1, channel);

      wb_receiver_set_channel(communication, channel3);
      channel = wb_receiver_get_channel(communication);
      ts_assert_boolean_equal(channel == channel3,
                              "Case 4 - Receiver \"receiver%d\" should be set to channel %d not %d after changing it", robotID,
                              channel3, channel);
  ...
  // communication: e1 -> r3, msgString2
  //                e1 -> r3, msgString3
  switch (robotID) {
    case 0:
      wb_emitter_send(communication, msgString1, strlen(msgString1) + 1);
    ...
    case 3:
      ...
      ts_assert_int_equal(queueLength, 1,
                          "Case 5 - Receiver \"receiver%d\" should receive %d packets not %d packets when a wall is placed "
                          "between the emitter and the receiver",
  ...
  switch (robotID) {
    ...
    case 3: {
      ...
      queueLength = wb_receiver_get_queue_length(communication);
      ts_assert_int_equal(queueLength, 1,
                          "Case 6 - Receiver \"receiver%d\" should receive %d packets not %d packets  when a wall is placed "
                          "between the emitter and the receiver",
                          robotID, 1, queueLength);
