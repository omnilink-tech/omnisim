...
EXTENSIONS_TO_CHECK = [
    '.proto', '.omniworld', '.wbt', '.wbo', '.wbproj', '.forest', '.txt', '.md', '.c', '.h', '.cpp', '.hpp', '.py', '.java', '.m', '.bvh',
    '.motion', '.obj', '.dae'
]
...
class TestLineEnding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.files = []
        for root_path, dir_names, file_names in os.walk(os.path.normpath(os.environ['OMNISIM_HOME'])):
