...
ignoredProtos = []
ignoredProtosFull = [os.path.join(OMNISIM_HOME, os.path.normpath(file)) for file in ignoredProtos]
...
class TestIcons(unittest.TestCase):
    ...
    def setUp(self):
        ...
        protos = []
        for rootPath, dirNames, fileNames in os.walk(os.path.join(OMNISIM_HOME, 'projects')):
        ...
        for proto in protos:
            ...
            # Windows checkout is cp1252 -- and any non-ASCII byte in any PROTO then raises
            # UnicodeDecodeError in setUp, so this guard never runs at all.
