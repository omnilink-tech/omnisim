...
class TestProtos(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for directory in ['projects', 'resources']:
            for rootPath, dirNames, fileNames in os.walk(os.path.join(os.path.normpath(os.environ['OMNISIM_HOME']), directory)):
    ...
    def test_proto_field_comment(self):
        ...
        for protoFile in self.protoFiles:
            # encoding is not optional: Python defaults to the LOCALE codec,
            # which on a Windows checkout is cp1252 -- so any non-ASCII byte in
            # any PROTO raises UnicodeDecodeError here and this guard never runs
            # at all. Two PROTOs carry a U+2B50 star in a comment, and the
            # format's own header line says "utf8". Same defect as the one fixed
            # in test_header_version.py.
            with open(protoFile, 'r', encoding='utf-8') as file:
                content = file.read()
