...
IGNORED_WORLDS = [
    'tests/protos/worlds/backward_compability_enu_flu.omniworld',
    ...
    def setUp(self):
        ...
        for rootPath, dirNames, fileNames in os.walk(OMNISIM_HOME):
            ...
            for fileName in fnmatch.filter(fileNames, '*.proto'):
                ...
                if proto in ignoredProtosFull:
                    continue
                # Same git-tracked guard the world scan below already uses.
                # Without it this graded dev git worktrees under .claude/,
                # gitignored .local-runs/ output and captured benchmark
                # workspaces -- none of them ours to format, and together the
                # dominant source of both false failures and runtime.
                if subprocess.run(['git', 'check-ignore', '-q', proto]).returncode == 0:
                    continue
    ...
    def test_header_version(self):
        ...
        for currentFile in self.files:
            ...
            with open(fileToTest, encoding='utf-8') as file:
                ...
                # DUAL-READ: the header migrated `#VRML_SIM` -> `#OMNISIM`
                # while the tokenizer kept accepting both, so BOTH spellings
                # are correct on disk. This test encoded only the pre-migration
                # one and so demanded that 1,143 correctly-migrated files be
                # reverted -- it was the test that was stale, not the corpus.
                expected = currentFile[1]
                accepted = (expected, expected.replace('#VRML_SIM', '#OMNISIM'))
                self.assertTrue(
                    line.startswith(accepted),
                    msg='Wrong header in file: "%s" (got %r, want one of %r)'
                        % (fileToTest, line[:40], accepted)
                )
