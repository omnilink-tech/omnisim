...
def main(agent_path: Path) -> int:
    ...
    smoke_tool = {
        "name": "__smoke__",
        "description": "Offline runner smoke tool.",
        "parameters": {
            "type": "object",
    ...
    def smoke_dispatch(tool_name: str, args: dict) -> dict:
        if tool_name == "__smoke__":
            return {"echo": args["x"]}
        return mod.dispatch(tool_name, args)

    runner = OmniLinkAgentRunner(
        agent_name="smoke",
        profile_path=agent_path.parent / "profile.json",
        port=0,  # random free port
        dispatch=smoke_dispatch,
        query_tools=[*mod.QUERY_TOOLS, smoke_tool],
    ...
    tool_resp = _post(f"{base}/tool", {"tool": "__smoke__", "x": 1})
    assert tool_resp.get("status") == "ok", tool_resp
    assert tool_resp.get("result") == {"echo": 1}, tool_resp

    code, validation = _post_error(f"{base}/tool", {"tool": "__smoke__"})
    assert code == 400 and validation.get("error") == "missing_field", validation
    code, unknown = _post_error(f"{base}/tool", {"tool": "__smoke_unknown__"})
    assert code == 404 and unknown.get("error") == "unknown_tool", unknown
    assert _post(f"{base}/tool", {"id": "smoke-once", "tool": "__smoke__", "x": 2})["status"] == "ok"
    code, duplicate = _post_error(
        f"{base}/tool", {"id": "smoke-once", "tool": "__smoke__", "x": 2}
    )
    assert code == 409 and duplicate.get("error") == "duplicate_request", duplicate
    ...
    assert activity["entries"][-1]["data"]["tool"] == "__smoke__", activity
