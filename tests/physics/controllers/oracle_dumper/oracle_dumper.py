"""oracle_dumper — physics dual-backend oracle (default-flip-plan.md §3.3, physics half).

A generic supervisor that records the absolute world position of every top-level DEF'd Solid,
once per simulation step, to a CSV. The runner (scripts/dev/physics_oracle.py) runs the SAME world
twice — once forced to ODE (OMNISIM_LEGACY=1) and once on the migration default (Newton when
available) — then diffs the two CSVs to show exactly where, and by how much, Newton diverges from the
ODE reference. Legacy is the oracle that verifies the new backend, which is the whole point of keeping
it as a fallback.
