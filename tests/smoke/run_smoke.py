...
_TEST_WORLD_RE = re.compile(r"(^|/)tests/[^/]+/worlds/[^/]+\.wbt$")


def check_verdicts(active_worlds: list) -> list[str]:
    """Cross-check tests/output.txt against the worlds we asked for.
    ...
    problems = []
    if SUITE_COMPLETE_MARKER not in content:
        problems.append(
            f'no "{SUITE_COMPLETE_MARKER}" marker — the suite died before finishing'
        )
...
def main() -> int:
    ...
    worlds = json.loads(SMOKE_FILE.read_text())
    env = os.environ.copy()
    ...
    # Force the canonical ODE physics path. The smoke worlds (empty,
    # accelerometer, contact_points, template_deterministic) are legacy
    # deterministic-regression tests with ODE-tuned golden values — e.g.
    # contact_points is built on ODE-specific ContactProperties
    # (softCFM/softERP). After Phase D flipped the default physicsBackend
    # to "auto", a Newton-ON build silently routes these onto Newton,
    # whose (valid but different) physics fails the golden assertions.
    # OMNISIM_FORCE_ODE makes the backend registry resolve every Solid to
    # ODE regardless of build config, so the safety net stays green and
    # build-independent (it's a no-op on Newton-OFF builds, where "auto"
    # already falls back to ODE). See WbPhysicsBackendRegistry::resolve.
    ...
    suite_rc = subprocess.run(cmd, cwd=REPO_ROOT, env=env, check=False).returncode
    problems = check_verdicts(active_worlds)
    if suite_rc != 0:
        # Normalize: test_suite.py exits with its raw failure COUNT, which can
        # exceed 255 and wrap to 0 through the shell on POSIX.
        problems.insert(0, f"test_suite.py exited {suite_rc}")
    if problems:
        print("[smoke] FAILED:", file=sys.stderr)
        for p in problems:
            print(f"[smoke] - {p}", file=sys.stderr)
        return 1
    print("[smoke] All smoke worlds passed with explicit verdicts.")
    return 0
