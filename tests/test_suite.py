...
webotsStdOutFilename = os.path.join(testsFolderPath, 'omnisim_stdout.txt')
webotsStdErrFilename = os.path.join(testsFolderPath, 'omnisim_stderr.txt')
...
def setupWebots():
    """Find the OmniSim binary via OMNISIM_HOME (preferred) or legacy WEBOTS_HOME."""
    os.putenv('WEBOTS_TEST_SUITE', 'TRUE')
    os.putenv('WEBOTS_EMPTY_PROJECT_PATH', defaultProjectPath)

    install_root = os.environ.get('OMNISIM_HOME') or os.environ.get('WEBOTS_HOME')

    # Prefer the canonical omnisim-bin; fall back to the legacy webots alias.
    if sys.platform == 'win32':
        if install_root is None:
            sys.exit('Error: neither OMNISIM_HOME nor WEBOTS_HOME is set.')
        binDir = os.path.join(os.path.normpath(install_root), 'msys64', 'mingw64', 'bin')
        candidates = [os.path.join(binDir, 'omnisim-bin.exe'), os.path.join(binDir, 'webots.exe')]
        webotsFullPath = next((c for c in candidates if os.path.isfile(c)), candidates[0])
    elif sys.platform == 'darwin':
        if install_root is None:
            sys.exit('Error: neither OMNISIM_HOME nor WEBOTS_HOME is set.')
        macosDir = os.path.join(os.path.normpath(install_root), 'Contents', 'MacOS')
        candidates = [os.path.join(macosDir, 'omnisim'), os.path.join(macosDir, 'webots')]
        webotsFullPath = next((c for c in candidates if os.path.isfile(c)), candidates[0])
    else:
        base = os.path.normpath(install_root) if install_root is not None else os.path.join('..', '..')
        candidates = [os.path.join(base, 'bin', 'omnisim-bin'), os.path.join(base, 'omnisim-bin'), os.path.join(base, 'webots')]
        webotsFullPath = next((c for c in candidates if os.path.isfile(c)), None)
        if webotsFullPath is None:
            sys.exit('Error: omnisim-bin binary not found')
        webotsFullPath = os.path.normpath(webotsFullPath)

    # Dual-read: the shipped empty world is `empty.omniworld`; a `.wbt` copy from
    # an older install still resolves.
    _emptyBase = (os.path.normpath(install_root) if install_root is not None
                  else os.path.join(testsFolderPath, '..'))
    _emptyDir = os.path.join(_emptyBase, 'resources', 'projects', 'worlds')
    _emptyCandidates = [os.path.join(_emptyDir, 'empty' + ext) for ext in ('.omniworld', '.wbt')]
    webotsEmptyWorldPath = next((c for c in _emptyCandidates if os.path.isfile(c)), _emptyCandidates[0])

    command = Command([webotsFullPath, '--version'])
    command.run()
    if command.returncode != 0:
        raise RuntimeError('Error when getting the OmniSim version: ' + command.output)
    try:
        webotsVersion = command.output.replace('\n', ' ').split(' ')[2].split('.')
    except IndexError:
        raise RuntimeError('Cannot parse OmniSim version: ' + command.output)

    command = Command([webotsFullPath, '--sysinfo'])
    command.run()
    if command.returncode != 0:
        raise RuntimeError('Error when getting the OmniSim information of the system: ' + command.output)
...
def resetOutputFile():
    ...
    file.write(formatString('OmniSim binary: ' + webotsFullPath) + '\n')
    file.write(formatString('OmniSim version: ' + str(webotsVersion)) + '\n')
...
def generateWorldsList(groupName):
    ...
    # generate the list from 'ls worlds/*.wbt'
        ...
        filenames = (glob.glob(os.path.join(testsFolderPath, groupName, 'worlds', '*.omniworld'))
                     + glob.glob(os.path.join(testsFolderPath, groupName, 'worlds', '*.wbt')))
        ...
        for filename in filenames:
            ...
            if (not filename.endswith('_temp.wbt') and
                    not ('GITHUB_ACTIONS' in os.environ and (
                        ...
                        filename.endswith('local_proto_with_texture.omniworld') or
                        (filename.endswith('billboard.omniworld') and sys.platform == 'darwin') or
                        (filename.endswith('billboard.omniworld') and sys.platform == 'win32') or
                        (filename.endswith('robot_window_html.omniworld') and sys.platform == 'linux') or
                        (filename.endswith('robot_window_html.omniworld') and sys.platform == 'win32')
                    ))):
...
def runGroupTest(groupName, firstSimulation, worldsCount, failures):
    ...
    if sys.platform == "darwin":
        # Longer timeout on MacOS because OmniSim takes longer to start there during CI.
        timeoutMinutes = 60
    ...
    if command.isTimeout or command.returncode != 0:
        if command.isTimeout:
            failures += 1
            appendToOutputFile(
                'FAILURE: OmniSim has been terminated by the test suite script\n')
        else:
            failures += 1
            appendToOutputFile(
...
# Run a copy of OmniSim in the background to ensure doing so doesn't cause any tests to fail and start the empty project.
...
# The engine auto-scans [1234, 1294] when 1234 is taken, so a fixed connect to 1234 verifies
# the WRONG process whenever anything else holds that port. Measured 2026-08-23: a wedged
# zombie engine (dead to taskkill, stuck in the GPU driver) held 1234's listener; while its
# accept-backlog had room this connect "succeeded" against the corpse, and once the backlog
# filled every suite run died here with TimeoutError -- either way the background engine this
# loop exists to verify was actually on 1235 and never checked. Scan the same range the
# engine scans, and remember what answered where.
retries = 0
error = None
connectedPort = None
while retries < 10 and connectedPort is None:
    for candidate in range(1234, 1245):
        try:
...
if connectedPort is None:
    raise error
if connectedPort != 1234:
    print(f'Note: background OmniSim reached on port {connectedPort} (1234 is held by another process).')
...
for groupName in testGroups:
    ...
    if groupName == "cache":
        for world in worldsList:
            # restart OmniSim after each "cache" test
