...
def test_world_wrong_extension():
    ...
    diag = classify_line("ERROR: Could not open file: '/tmp/x.txt'. The world file extension must be "
                         "'.omniworld' (or the legacy '.wbt').")
    assert diag["code"] == "WORLD_WRONG_EXTENSION"
    assert diag["source_path"] == "/tmp/x.txt"
    # the pre-migration single-extension wording must still classify
    legacy = classify_line("ERROR: Could not open file: '/tmp/x.txt'. The world file extension must be '.wbt'.")
    assert legacy["code"] == "WORLD_WRONG_EXTENSION"
...
def test_cuda_codes_round_trip_through_tagged_parser():
    # CUDA_* codes are emitted only via OmLog::diagnostic. Verify the tagged
