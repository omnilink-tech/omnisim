...
def _omnisim_home():
    ...
    # retired with the rest of the WEBOTS_* runtime contract. Name it explicitly when only the
    # retired spelling is present -- a bare "OMNISIM_HOME is not set" reads as if nothing were
    # configured, and sends the reader looking in the wrong place.
    home = os.environ.get('OMNISIM_HOME')
    if home:
        return home
    if os.environ.get('WEBOTS_HOME'):
        raise KeyError("WEBOTS_HOME is set but OMNISIM_HOME is not. OmniSim reads only OMNISIM_HOME "
                       "-- the WEBOTS_HOME name was retired; rename it.")
