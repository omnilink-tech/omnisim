...
if sys.platform == 'win32':
    ...
    def _delete_object(object):
        del object
    # OMNISIM_STD*_REDIRECT only -- the launcher writes exactly these names and the legacy
    # WEBOTS_* twins were retired from the runtime contract on both sides at once.
    if os.environ.get('OMNISIM_STDOUT_REDIRECT'):
        _stdout_redirect = StdStreamRedirect(1)
        atexit.register(_delete_object, _stdout_redirect)
    if os.environ.get('OMNISIM_STDERR_REDIRECT'):
