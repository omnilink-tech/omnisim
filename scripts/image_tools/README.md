...
> Note: It may be interesting to clamp the highest values of an HDR image when obtaining saturated results in the OmniSim rendering.
A typical value for the threshold may be 30.

```shell
cd $OMNISIM_HOME/scripts/image_tools
...
### Convert an HDR Equirectangular Map to an OmniSim-Compliant Background
