...
def _live_parent_pids_win() -> dict[int, tuple[int, str]]:
    """{pid: (parent_pid, command_line)} for every omnisim-bin on a Windows
    host, via CIM (wmic is gone from current Windows 11 builds)."""
    script = (
    ...
    try:
        out = subprocess.check_output(
            ["powershell", "-NoProfile", "-Command", script], text=True, errors="replace"
        )
    ...
    for line in out.splitlines():
        ...
        if len(parts) < 2:
            continue
        try:
            info[int(parts[0])] = (int(parts[1]), parts[2] if len(parts) > 2 else "")
        except ValueError:
            continue
    return info
...
def _pid_alive(pid: int) -> bool:
    if sys.platform == "win32":
        try:
            out = subprocess.check_output(
                ["tasklist", "/NH", "/FO", "CSV", "/FI", f"PID eq {pid}"], text=True, errors="replace"
            )
        except (subprocess.CalledProcessError, FileNotFoundError):
            return False
        return f",\"{pid}\"," in out
...
def detect_live_sims() -> list[tuple[int, str]]:
    """Every omnisim-bin on the host, orphan or not (the old meaning of
    `detect_orphan_sim`)."""
    return _detect_sims(orphans_only=False)


def detect_orphan_sim() -> list[tuple[int, str]]:
    """ORPHANS ONLY: omnisim-bin processes whose parent process is gone.

    MEASURED 2026-08-29: this used to return every omnisim-bin on the host
    and the callers' refuse-and-tell message told the operator to kill
    'the specific PID(s) and retry' -- so an agent lane blocked by ANOTHER
    lane's live engine killed it (exit code 1, no diagnostic, mid-run) and
    retried, five times in one evening, each time ending a 15-minute reef
    epoch of the other lane. A live engine whose launcher is still alive is
    somebody's session, not an orphan; the multi-instance contract (AGENTS.md
    section 3e) says running engines coexist. Use `detect_live_sims()` when
    the question really is 'is anything running'."""
    return _detect_sims(orphans_only=True)


def _detect_sims(orphans_only: bool) -> list[tuple[int, str]]:
    ...
    try:
        if sys.platform == "win32":
            ...
            if orphans_only and results:
                parents = _live_parent_pids_win()
                kept = []
                for pid, name in results:
                    ppid, cmd = parents.get(pid, (None, ""))
                    if ppid is not None and _pid_alive(ppid):
                        continue                    # its launcher lives: another session, not an orphan
                    kept.append((pid, (cmd or name)[:120]))
                results = kept
            return results
        else:
            out = subprocess.check_output(
            ...
            if orphans_only and results:
                ...
                for pid, cmd in results:
                    try:
                        ppid = int(open(f"/proc/{pid}/stat").read().split(")")[-1].split()[1])
