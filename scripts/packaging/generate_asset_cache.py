...
# ensure OMNISIM_HOME is set and tag was provided
...
def generate_asset_cache(tag, version_name):
    ...
    for extension in EXTENSIONS:
        assets.extend(Path(OMNISIM_HOME + '/projects').rglob(f'*.{extension}'))
    ...
    with open(f'{folder_name}/log.txt', 'w') as log_file:
        ...
        for asset in assets:
            ...
            remote_url = str(asset).replace(OMNISIM_HOME, rf'https://raw.githubusercontent.com/omnilink-tech/omnisim/{tag}')
    ...
    shutil.move(f'{folder_name}.zip', f'{OMNISIM_HOME}/distribution/{folder_name}.zip')
    shutil.rmtree(folder_name)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        sys.exit('''Missing one or more arguments.
            It should have the form: generate_asset_cache [commit sha or tag] [omnisim version name]''')
    else:
        tag = sys.argv[1]
