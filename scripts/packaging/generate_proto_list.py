...
"""Generate OmniSim proto-list.xml file"""
...
class ProtoInfo:
    ...
    def parse_header(self):
        for line in self.contents.split('\n'):
            ...
            if (clean_line.startswith(('OMNISIM', 'VRML_SIM')) or re.search(r'template language\s*:', clean_line) or
    ...
    # Whitespace and/or '#' comment lines. A PROTO body may legally open with an
    # explanatory comment before its template or its root node, so every gap in the
    # body-opening pattern below must tolerate comments -- not just spaces.
    _GAP = r'(?:\s|#[^\n]*\n)*'

    def parse_body(self):
        # determine the proto_type of the PROTO (ex: for RoadSegment is Road)
        # regex: find the start of the PROTO body (']' then '{'), skip any template
        # statement (%< ... >%) and any 'DEF SOMETHING', and take the node name that
        # follows -- that is the proto_type.
        g = self._GAP
        child_node = re.search(
            r'\]' + g + r'\{' + g + r'(?:\%\<[\s\S]*?\>\%)?' + g +
            r'(?:DEF\s+\S+' + g + r')?([a-zA-Z0-9\_\-\+]+)\s*\{',
            self.contents)
        if child_node is None:
            raise RuntimeError(f'Error, parsing body of {self.name} failed: could not find '
                               f'the root node after the PROTO interface.')
        self.proto_type = child_node.groups()[-1]


def generate_proto_list(current_tag=None, silent=False):
    # ensure OMNISIM_HOME is set
    if 'OMNISIM_HOME' in os.environ:
        OMNISIM_HOME = os.environ['OMNISIM_HOME'].replace('\\', '/')  # use cross-platform forward slashes
    else:
        raise RuntimeError('Error, OMNISIM_HOME variable is not set.')

    # if no argument (branch/release/hash) is provided, assume it's a locally defined 'omnisim://'
    if current_tag:
        prefix = f'https://raw.githubusercontent.com/omnilink-tech/omnisim/{current_tag}/'
    else:
        prefix = 'omnisim://'

    # find all PROTO assets
    assets = []
    assets.extend(Path(OMNISIM_HOME + '/projects').rglob('*.proto'))

    filename = f'{OMNISIM_HOME}/resources/proto-list.xml'
    ...
    base_nodes = [os.path.splitext(os.path.basename(x))[0] for x in glob.glob(f'{OMNISIM_HOME}/resources/nodes/*.wrl')]
    ...
    for key, info in protos.items():
        ...
        ET.SubElement(proto_element, 'url').text = info.path.replace(OMNISIM_HOME + '/', prefix)
