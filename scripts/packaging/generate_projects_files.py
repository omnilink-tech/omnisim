...
class ProjectFilesGenerator:
    def __init__(self):
        ...
        self.omit_in_projects = [os.path.join(OMNISIM_HOME, line) for line in self.omit_in_projects]

        with open(os.path.join(script_dir, "recurse_in_projects.txt")) as f:
            self.recurse_in_projects = f.read().splitlines()
        self.recurse_in_projects = [os.path.join(OMNISIM_HOME, line) for line in self.recurse_in_projects]
        ...
        with open(os.path.join(script_dir, self.exist_in_projects_platform_path)) as f:
            self.exist_in_projects += f.read().splitlines()
        self.exist_in_projects = [os.path.join(OMNISIM_HOME, line) for line in self.exist_in_projects]
    ...
    def check_exist_in_projects(self, files):
        ...
        for file in files:
            if not os.path.exists(os.path.join(OMNISIM_HOME, file)):
...
if __name__ == "__main__":
    for item in generate_projects_files(os.path.join(OMNISIM_HOME, 'projects')):
        print(item)
