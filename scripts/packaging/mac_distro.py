...
"""Generate macOS OmniSim package."""

from generic_distro import OmniSimPackage, remove_force
import json
import os
import re
import shutil
...
def check_rpath(home_path):
    ...
    # - relative to @rpath (= OMNISIM_HOME) and are existing
...
class MacOmniSimPackage(OmniSimPackage):
    def __init__(self, package_name):
        super().__init__(package_name)
        self.bundle_name = package_name + '.app'
        self.package_omnisim_path = os.path.join(self.distribution_path, self.bundle_name)
    ...
    def create_omnisim_bundle(self, include_commit_file):
        ...
        os.rename(os.path.join(self.package_omnisim_path, 'Contents', 'resources'),
                  os.path.join(self.package_omnisim_path, 'Contents', 'Resources'))

        # create Qt symlinks
        path = os.path.join(self.package_omnisim_path, 'Contents', 'Frameworks')
        ...
        # sign the bundle
        os.system(f'codesign --deep -f -s - {self.package_omnisim_path}')
    ...
    def make_dir(self, directory):
        ...
        if directory.startswith('Contents/'):
            destination_dir = os.path.join(self.package_omnisim_path, directory)
        else:
            destination_dir = os.path.join(self.package_omnisim_path, 'Contents', directory)
        if not os.path.isdir(destination_dir):
            os.makedirs(destination_dir)

    def copy_file(self, path):
        if path.startswith('Contents/'):
            destination_dir = os.path.join(self.package_omnisim_path, os.path.dirname(path))
        else:
            destination_dir = os.path.join(self.package_omnisim_path, 'Contents', os.path.dirname(path))
        source_path = os.path.join(self.omnisim_home, path)
        if not os.path.isfile(source_path):
            source_path = os.path.join(self.omnisim_home, 'Contents', path)
            if not os.path.isfile(source_path):
