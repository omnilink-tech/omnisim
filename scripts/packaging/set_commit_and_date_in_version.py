...
if 'OMNISIM_HOME' in os.environ:
    path = os.path.normpath(os.environ['OMNISIM_HOME'])
else:
...
if len(sys.argv) != 2:  # no commit id passed as an argument
    ...
    with open(os.path.join(path, 'src', 'omnisim', 'core', 'WbApplicationInfo.cpp'), 'r') as f:
        date = datetime.datetime.today()
        content = f.read().replace(version, '%s-%s-%d/%d/%d' % (version, commit, date.day, date.month, date.year))
    with open(os.path.join(path, 'src', 'omnisim', 'core', 'WbApplicationInfo.cpp'), 'w') as f:
        f.write(content)
