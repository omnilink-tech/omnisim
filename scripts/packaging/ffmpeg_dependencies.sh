...
# We generate the list of DLL dependencies for ffmpeg.exe that are not already dependencies of omnisim-bin.exe
ldd /mingw64/bin/ffmpeg.exe | grep /mingw64/bin | awk '{print $1;}' | sort -u > ffmpeg.tmp
ldd ${OMNISIM_HOME}/msys64/mingw64/bin/omnisim-bin.exe | grep /mingw64/bin | awk '{print $1;}' | sort -u > webots.tmp
