...
"""Generate Linux OmniSim tarball, Debian and snap packages."""

from generic_distro import OmniSimPackage, remove_force, symlink_force, print_error_message_and_exit
...
class LinuxOmniSimPackage(OmniSimPackage):
    USR_LIB_X68_64 = [
        "libfontconfig.so.1",
        "libfreetype.so.6",
        "libgomp.so.1",
    ...
    def __init__(self, package_name):
        ...
        self.package_omnisim_path = os.path.join(self.distribution_path, 'debian', 'usr', 'local',
        ...
        for f in glob.glob(os.path.join(self.distribution_path,
            ...
            remove_force(f)
        os.makedirs(self.package_omnisim_path)
    ...
    def create_omnisim_bundle(self, include_commit_file):
        ...
        if self.tarball_enabled or self.deb_enabled:
            ...
            package_lib_dir = os.path.join(self.package_omnisim_path, 'lib', 'webots')
            if distro.version() == '22.04' or distro.version() == '24.04':  # Ubuntu 22.04 and 24.04
                ...
                for lib in openssl_libs:
                    shutil.copy(os.path.join(system_lib_path, lib), package_lib_dir)
    ...
    def create_debian_bundle(self):
        ...
        # copy OmniSim application files needed by the debian package
        packaging_files = [['omnisim.mime', 'mime-info'],
                           ['omnisim.keys', 'mime-info'],
                           ['omnisim.png', 'pixmaps'],
                           ['omnisim_doc.png', 'pixmaps'],
                           ['omnisim.applications', 'application-registry'],
                           ['omnisim.desktop', 'applications'],
                           ['omnisim.desktop', 'app-install/desktop']]
        ...
        # Create the launcher symlinks in /usr/local/bin. `omnisim` is the canonical
        # command (it is what omnisim.desktop's Exec= invokes); `webots` is kept as a
        # legacy alias so existing scripts and shells keep working. Both point at the
        # launcher shell shipped inside the install root.
        os.makedirs(os.path.join(self.distribution_path, 'debian', 'usr', 'local', 'bin'))
        launcher = f"/usr/local/{self.application_name_lowercase_and_dashes}/webots"
        for command in (self.application_name_lowercase_and_dashes, 'webots'):
            symlink_force(launcher,
                          os.path.join(self.distribution_path, 'debian', 'usr', 'local', 'bin', command))
        ...
        if distro.version() == '22.04' or distro.version() == '24.04':
            shutil.copy(os.path.join(system_lib_path, 'libzip.so.4'), package_lib_dir)
        ...
        with open(os.path.join(self.distribution_path, 'debian', 'DEBIAN', 'control'), 'w') as f:
            f.write(
                ...
                "Maintainer: OmniLink <https://github.com/omnilink-tech/omnisim>\n"
                "Homepage: https://www.omnilink-agents.com\n"
                "Description: Robot simulator for agentic AI\n"
                " OmniSim is a fast prototyping and simulation software\n"
                " which allows you to model, program and simulate any mobile\n"
                " robot, including wheeled, legged, swimming and flying robots.\n"
                " It is purpose-built as the simulation environment for the\n"
                " OmniLink agentic AI platform, and is free and open source\n"
                " (Apache-2.0), built on the OmniSim engine by Cyberbotics Ltd.\n"
            )
    ...
    def create_tarball_bundle(self):
        ...
        os.chdir(self.package_omnisim_path)
        os.chdir('..')
    ...
    def create_snap_bundle(self):
        ...
        # Everything else in this script installs under /usr/share/<app>, so these
        # paths must use the same root -- they used to be hardcoded to
        # "/usr/share/webots", a directory the build no longer creates.
        app = self.application_name_lowercase_and_dashes
        self.snap_script.write(f"mkdir $DESTDIR/usr/share/{app}/include/libssh\n")
        self.snap_script.write(f"cp -a /usr/include/libssh $DESTDIR/usr/share/{app}/include/libssh/\n")
        self.snap_script.write(f"mkdir $DESTDIR/usr/share/{app}/include/libzip\n")
        self.snap_script.write(f"cp -a /usr/include/zip.h $DESTDIR/usr/share/{app}/include/libzip/\n")
        self.snap_script.write(f"cp /usr/include/zipconf.h $DESTDIR/usr/share/{app}/include/libzip/\n")
        self.snap_script.write(f"cp $OMNISIM_HOME/scripts/packaging/{app}_snap.desktop "
                               f"$DESTDIR/usr/share/{app}/resources/{app}.desktop\n")
        # `omnisim` is the canonical command name the .desktop Exec= line invokes; the
        # launcher shell itself still ships as `webots` (legacy alias, see AGENTS.md).
        self.snap_script.write(f"ln -sf webots $DESTDIR/usr/share/{app}/{app}\n")
    ...
    def copy_file(self, path):
        ...
        if self.tarball_enabled or self.deb_enabled:
            ...
            dst_dir = os.path.join(self.package_omnisim_path, dir_path)
            shutil.copy(os.path.join(self.omnisim_home, path), dst_dir)

        if self.snap_enabled:
            # add copy instruction in snap script
            protected_filename = file_name.replace('$', '\\$').replace('(', '\\(').replace(')', '\\)').replace(' ', '\\ ')
            self.snap_script.write(f"cp -a $OMNISIM_HOME/{os.path.join(dir_path, protected_filename)} "
