...
class WindowsWebotsPackage(WebotsPackage):
    ...
    def create_webots_bundle(self, include_commit_file):
        ...
        self.iss_script.write(
            ...
            # the file-type description, so it must carry the OmniSim brand. The .wbt
            # *extension* is untouched -- that is the world file FORMAT, not branding.
            "\n[Registry]\n"
            ...
            "end;\n\n"
            "\n[Run]\n"
            "Filename: {app}\\msys64\\mingw64\\bin\\webotsw.exe; Description: \"Launch OmniSim\"; Flags: nowait postinstall "
            "skipifsilent\n"
            )
    ...
    def check_newton_runtime_bundle(self):
        ...
        automatically. But if it is ABSENT the installer still builds and silently
        produces a stock install that falls back to ODE -- the exact "silent
        degrade" default-flip-plan.md principle #4 forbids. So warn prominently
        and point at the fix, rather than failing: an intentionally ODE-only /
        no-GPU release is legitimate. Set OMNISIM_REQUIRE_NEWTON_BUNDLE=1 to make
        a missing bundle fatal (for the Newton-capable release matrix)."""
        bin_dir = os.path.join(self.webots_home, 'msys64', 'mingw64', 'bin')
        ...
        msg = ('Newton runtime NOT bundled (missing: ' + ', '.join(missing) + '). '
               'This installer runs ODE on a clean box, not Newton. Run '
