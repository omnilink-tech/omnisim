...
install resolve `physicsBackend "auto"` -> Newton instead of silently falling
back to ODE, by shipping the Newton runtime (CPython + warp + newton + the
MuJoCo-solver + USD deps) *inside* the install tree.

Why this can be done in the packaging layer alone (no C++ change)
----------------------------------------------------------------
`WbNewtonBackend` brings the interpreter up with a bare `Py_InitializeEx(0)` --
no `Py_SetPythonHome`, no `PyConfig`, no `sys.path` edits (verified in
src/omnisim/physics/WbNewtonBackend.cpp) -- then does `import warp` / `import
...
warp (or none) and `auto` silently degrades to ODE.
...
Newton smoke world and checks for the "[WbNewtonBackend]" runtime-up log line
...
# Modules that must import for the runtime to be considered live.
VERIFY_IMPORTS = ["warp", "newton"]
...
def inspect(binary_path, target_dir):
    ...
    print(f"  imports: {dll or '(no python3XX.dll import -- pure-ODE build?)'}")
...
def main(argv=None):
    ...
    if not dll:
        print(f"ERROR: {binary} imports no python3XX.dll -- this looks like a "
              f"pure-ODE build (OMNISIM_WITH_NEWTON=OFF). Nothing to bundle.",
              file=sys.stderr)
        return 1
