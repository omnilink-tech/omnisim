...
"""Uploads the content of 'OMNISIM_HOME/distribution' to a Github release."""
...
optParser = optparse.OptionParser(
    usage="usage: %prog --key=... --repo=<owner>/omnisim --branch=develop --commit=... [--tag=...]")
optParser.add_option("--key", dest="key", default="", help="specifies the repo access key")
optParser.add_option("--repo", dest="repo", default=os.environ.get("OMNISIM_RELEASE_REPO", ""),
                     help="target release repo as <owner>/<name> (default: $OMNISIM_RELEASE_REPO)")
...
if not options.repo:
    sys.exit("error: no target repo. Pass --repo=<owner>/omnisim or set $OMNISIM_RELEASE_REPO.")
...
warningMessage = '\nIt might be unstable, for a stable version of OmniSim, please use the [latest official release]' \
                 '(https://github.com/%s/releases/latest).' % options.repo
if options.tag and not options.tag.startswith('refs/heads/'):
    tagName = options.tag.replace('refs/tags/', '')
    title = tagName
    message = 'This is a nightly build of OmniSim from the "%s" tag.%s' % (tagName, warningMessage)
    if tagName.startswith('nightly_'):
        print('Skipping nightly build tag.')
        sys.exit(0)
else:
    title = 'OmniSim Nightly Build (%d-%d-%d)' % (now.day, now.month, now.year)
    ...
    message = 'This is a nightly build of OmniSim from the following branch(es):\n  - %s\n%s' % (branchLink, warningMessage)
...
for release in repo.get_releases():
    match = re.match(r'OmniSim Nightly Build \((\d*)-(\d*)-(\d*)\)', release.title, re.MULTILINE)
...
for release in repo.get_releases():
    if release.title == title:
        ...
        if 'OMNISIM_HOME' in os.environ:
            rootPath = os.path.normpath(os.environ['OMNISIM_HOME'])
        else:
