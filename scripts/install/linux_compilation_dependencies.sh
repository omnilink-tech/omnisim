...
install_ubuntu_packages() {
    ...
    apt update
    # python3-dev: Python embed headers (the engine embeds CPython for the Newton backend)
    # python-is-python3: controllers are spawned via `python`, which doesn't exist on stock Ubuntu
    # xvfb + x11-utils: the engine needs an X context even for headless runs
    apt install --yes git cmake swig libglu1-mesa-dev libglib2.0-dev libfreeimage3 libfreetype6-dev libxml2-dev libboost-dev libssh-gcrypt-dev libzip-dev libreadline-dev pbzip2 wget zip unzip python3 python3-pip python3-dev python-is-python3 xvfb x11-utils libopenal-dev
