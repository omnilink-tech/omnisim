...
QT_VERSION=6.5.3
# Ubuntu 24.04+ marks the system python "externally managed" (PEP 668) and the
# plain pip install hard-fails under `set -e`; older Ubuntus don't know
# --break-system-packages, so try plain first and fall back.
pip install --no-input aqtinstall 2>/dev/null || pip install --no-input --break-system-packages aqtinstall
...
OMNISIM_HOME="$(cd "$(dirname "${BASH_SOURCE[0]}" )"/../.. && pwd)"

echo Installing Qt in OmniSim
echo Source: $QT_INSTALLATION_PATH
echo Destination: $OMNISIM_HOME

# uninstall any previous installation of Qt from OmniSim
