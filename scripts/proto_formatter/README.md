...
- In the editor, press `Ctrl+Shift+P`.
    ...
    "tasks": [
      {
        ...
        "command": "python",
        "args": [
          "$(OMNISIM_HOME)/scripts/proto_formatter/proto_formatter.py",
          "${file}"
        ],
        "group": {
          "kind": "build",
...
- Replace `$(OMNISIM_HOME)` by the path to your OmniSim installation folder.
- Press `Ctrl+Shift+B` to trigger the formatting in a .proto file.
