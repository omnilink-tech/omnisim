...
- **Read endpoints pause the engine for the duration of their walk** (`518a335e`). Every supervisor read is one engine round-trip, and a round-trip costs ~6 ms against the free-running engine vs **~0.15 ms paused** (measured, 40×) — so `/scene/tree`, `?bounds=1`, `/robots`, `/sim/contacts`, `/robot/<def>/joints|devices`, `/scene/node` and the camera reads take a paused **single-instant snapshot** and restore the prior mode on the way out. Medians on the 298-node 10-Husky stress scene, light session: `/scene/tree` 11.0 s → **0.11 s**, `?bounds=1` 19.5 s → **0.19 s**, `/robots` 15.8 s → **0.14 s**, `/sim/contacts` 11.9 s → **0.13 s**; AgentBench-task-sized worlds all answer in well under 0.1 s. The sim still free-runs *between* calls, and `?wake=1` still advances its settle steps (before the pause — stepping a paused engine deadlocks, and a test tripwire enforces the ordering). ⚠️ What this does **not** fix: a **non-light** session on a multi-robot scene still queues *every* RPC (even `/sim/state`) behind the per-step trackers' own free-run walks — ~28 s flat on that stress scene — so the "load `{"light": true}` on multi-robot scenes" rule stands. Full before/after ledger: [docs/developer/harness-latency-2026-07-31.md](../../docs/developer/harness-latency-2026-07-31.md).
...
>
> **So:** use `spawn` for cameras, markers and visual props, and for staging a scene you then `/world/load` to make real. Do **not** use it for anything that must fall, collide or be picked up, and never treat a spawned floor or wall as a collision surface. `set_pose` is unaffected — it moves a body the solver already knows about. Fix tracked internally as W1.7 — runtime scene mutation, one workstream covering both the spawn and the delete direction.
