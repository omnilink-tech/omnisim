...
| File | Purpose |
|---|---|
| [`omnisim_harness.py`](omnisim_harness.py) | The HTTP service. Run directly or via `python -m omnisim harness`. |
...
- **Light tracking is the DEFAULT (2026-09-02).** A `POST /world/load` that names **neither** `light` **nor** `tracking` now injects the supervisor with `--light` (no per-step contact / joint-limit / grip trackers) and says so in the response: `tracking.mode: "light"`, `tracking.default_applied: true`, a `default` sub-block (the same one `GET /capabilities` -> `limits.tracking_default` serves, built by one function) and `default_note` -- one sentence naming the way back. **The contract is dual: an explicit `light` (either value) or any `tracking` object always wins**; a `tracking` object with no `light` is `partial` mode, never the default. Revert process-wide with `OMNISIM_HARNESS_LIGHT=0` (value-parsed: unset or `1` -> light, `0` -> full); the harness prints the armed default on its startup banner. Why: with the trackers on, the harness was slower than the `run-headless` it replaces -- measured on machine `9722d23d12a3`, fleet arena (309 nodes, CPU `mj_step`, 2026-08-29) 5.2 s vs 4.65 s (2026-09-02 engine; 12.1 s vs 4.1 s on 2026-08-29) to load, `/sim/step 1` 573-606 ms vs 6-35 ms (~17x), 10 steps 2855-3187 ms vs 48-67 ms (~47x); cloth world (2026-08-14) 13.4 s vs 3.1 s to reload -- and an agent that forgot the flag got the slow path. What light drops is unchanged: `/sim/grips` answers empty with `tracking.enabled=false` and the `contact.*` / `grip.*` / `joint.limit_hit` events go quiet (`/sim/contacts` is walked per call and unaffected). New with the flip: the first `/sim/grips` read of a session whose GripTracker is not running emits one `world.warning` `TRACKER_NOT_RUNNING` per load on `/sim/events`, naming whether light was the default or requested. Callers that read the tracker-fed surfaces must now pass `{"light": false}` or a `tracking` object -- audited 2026-09-02: no shipped caller in the tree relied on the old full default (the ROS 2 sidecar, the benchmark lanes, `batch_validate.py` and the capture/cinema services either pass `light` explicitly or never read a tracker-fed surface).
