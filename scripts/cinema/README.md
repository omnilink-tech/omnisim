...
2. Get a starter storyboard:
   ...
   ```bash
   python -m omnisim cinema new --title "Spot RL hero" --subject spot \
     --world projects/policies/worlds/spot_rl_deploy.wbt > spot_hero.json
   ```

3. Render it end-to-end:

   ```bash
   python -m omnisim cinema render spot_hero.json
   ```
...
random framing. A camera primitive like `tracking_side(spot, side='left')`
queries the supervisor for Spot's actual world pose, places the camera
off Spot's heading-relative left flank at a distance scaled to Spot's
body size, and moves the camera with Spot's predicted motion. The
...
{
  "title": "Spot RL hero",
  "world": "projects/policies/worlds/spot_rl_deploy.wbt",
  "subject": "spot",
  "look": "teal_orange",
...
- **Worlds that segfault in the capture pipeline** (currently
  `warehouse_industrial.wbt`) will fail cleanly with a "world load
