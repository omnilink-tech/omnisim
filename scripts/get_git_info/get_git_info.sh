...
then
	echo "$branch" | $(sed -n -e 's/^\* \(.*\)/\1/p' | cut -c 26- | rev | cut -c 2- | rev > $OMNISIM_HOME/resources/branch.txt)
elif [[ $branch == '* (HEAD detached at '* ]]
then
	echo "$branch" | $(sed -n -e 's/^\* \(.*\)/\1/p' | cut -c 19- | rev | cut -c 2- | rev > $OMNISIM_HOME/resources/branch.txt)
elif [[ $branch != fatal* ]]
then
	echo "$branch" | $(sed -n -e 's/^\* \(.*\)/\1/p' > $OMNISIM_HOME/resources/branch.txt)
else
	echo "master" > $OMNISIM_HOME/resources/branch.txt
fi
...
then
	echo "$url" | $(cut -c 16- | rev | cut -c 5- | rev > $OMNISIM_HOME/resources/repo.txt)
elif [[ $url == https://github.com* ]]
then
	echo "$url" | $(cut -c 20- | rev | cut -c 5- | rev > $OMNISIM_HOME/resources/repo.txt)
elif [[ $url == https://*@github.com* ]]
then
	repo=$(echo $url | cut -d/ -f4)/$(echo $url | cut -d/ -f5 | cut -d. -f1)
	echo "$repo" > $OMNISIM_HOME/resources/repo.txt
else
...
if [[ $commit != "" ]]
then
	echo "$commit" > $OMNISIM_HOME/resources/commit.txt
fi
