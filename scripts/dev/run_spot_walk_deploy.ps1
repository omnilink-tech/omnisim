# Run the Spot WALK deploy world headless and report forward progress.
#
# THE G1 WALK RECIPE PORTED TO SPOT (gait-residual RL on the
# stiffness-matched model). The trainer MJCF matches the deploy joint
# drive EXACTLY:
#   C:\tmp\spot_newton_fixed2.xml (kp=500/kv=60) <-> OMNISIM_NEWTON_TARGET_KE=500/KD=60
#
# Gait reference: foot-space trot model (projects/policies/control/gait/spot_trot_gait.py)
...
#
# Usage:  powershell -File scripts/dev/run_spot_walk_deploy.ps1 [-Duration 240] [-Policy <onnx>] [-Bare]
param(
    [int]$Duration = 240,
    [string]$Policy = "",
    [switch]$Bare,
...
$log = "$root\_scratch\spot_walk_deploy.log"
...
if ($Bare) { Remove-Item Env:SPOT_POLICY_ONNX -ErrorAction SilentlyContinue; $env:SPOT_POLICY_ONNX = "$root\__no_policy__" }
else       { $env:SPOT_POLICY_ONNX = $Policy }
...
if ($Bare) { Write-Host "Running spot_walk_deploy.wbt for ${Duration}s wall (BARE gait model)..." }
else       { Write-Host "Running spot_walk_deploy.wbt for ${Duration}s wall (trot model + RL policy)..." }
python -u "$root\scripts\dev\headless_runner.py" $world --duration $Duration 2>&1 | Select-Object -Last 4

Write-Host "`n===== SPOT WALK RESULT ====="
if (Test-Path $log) {
