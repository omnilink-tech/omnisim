...
def insert_externprotos(text: str) -> str:
    """Insert the 3 EXTERNPROTOs after the last existing EXTERNPROTO line, or after the header line."""
    lines = text.split("\n")
    ...
    if last_externproto_idx >= 0:
        ...
        # Insert after the #OMNISIM / legacy #VRML_SIM line (line 0), possibly past any leading comments
        insert_at = 1
...
def find_world_files() -> list[Path]:
    ...
    for root in USER_FACING_ROOTS:
        ...
        if not root_path.exists():
            continue
        for p in [*root_path.rglob("*.omniworld"), *root_path.rglob("*.wbt")]:
