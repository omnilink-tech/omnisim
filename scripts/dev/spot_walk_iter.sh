#!/usr/bin/env bash
# Spot probe runner. Launches OmniSim headless with SPOT_PROBE=<mode> so
# the controller self-quits cleanly, then analyzes the CSV trace.
#
# Usage: scripts/dev/spot_walk_iter.sh <mode> [sim_seconds] [wall_timeout]
...
WORLD="projects/robots/boston_dynamics/spot/worlds/spot.wbt"
TRACE_DIR="C:/tmp/husky_trace"
CSV="${TRACE_DIR}/spot_probe_${MODE}.csv"
WB="./msys64/mingw64/bin/omnisim-bin.exe"

rm -f "$CSV"
mkdir -p "$TRACE_DIR"

SPOT_PROBE="$MODE" SPOT_SIM_DURATION_S="$SIM_DURATION" OMNISIM_HOME="$(pwd)" \
  "$WB" "$WORLD" --batch --mode=fast --no-rendering --minimize \
  --stdout --stderr > /tmp/spot_iter_stdout.txt 2>&1 &
WB_PID=$!
...
if [ ! -f "$CSV" ]; then
  echo "[iter] NO CSV PRODUCED -- check /tmp/spot_iter_stdout.txt"
  tail -20 /tmp/spot_iter_stdout.txt 2>/dev/null
  exit 1
fi
