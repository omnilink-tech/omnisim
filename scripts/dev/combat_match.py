...
Usage: python scripts/dev/combat_match.py <world.wbt> [timer_s] [vel_smooth]
"""
import json
import os
import re
...
def main():
    if len(sys.argv) < 2:
        print("usage: combat_match.py <world.wbt> [timer_s] [vel_smooth]")
        return 2
    world = sys.argv[1]
    timer_s = float(sys.argv[2]) if len(sys.argv) > 2 else 60.0
    vel_smooth = sys.argv[3] if len(sys.argv) > 3 else "5"
    backend = sys.argv[4] if len(sys.argv) > 4 else "newton"   # "ode" forces the ODE A/B baseline
