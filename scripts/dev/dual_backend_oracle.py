...
"""dual_backend_oracle — the unified dual-backend verifier (architectural-baseline.md §2 B1).

ONE command runs a world on legacy (ODE + WREN) and on the new stack (Newton + wgpu) and reports
per-arm divergence — the "use legacy to verify the new architecture" tool the migration plan calls
for (default-flip-plan.md §3.3). It is a thin front-end over the two arm-specific oracles, so each arm
stays the single source of truth for its own launch handling, retries, and tolerances:

  PHYSICS arm  (🟦 Newton/physics session — IMPLEMENTED): ODE vs Newton per-body trajectory
               divergence. Delegates to scripts/dev/physics_oracle.py, which dumps each DEF'd Solid's
               world position per step under both backends and aligns them column-for-column. The
               report mode prints per-body max |Δpos| + first-divergence step; the gate mode asserts
               physical invariants (finite/settled/reproducible) and that Newton was actually active.
...
diffs a trajectory; render diffs an image) and different runtime needs, but a developer debugging a
world wants ONE command that says "here is where Newton diverges from ODE, and here is where wgpu
...
arm's Newton run silently falls back to ODE and the divergence reads ~0 (a false "they match").
...
def run_physics_arm(world, steps, gate):
    argv = [sys.executable, PHYSICS_ORACLE, "--world", world, "--steps", str(steps)]
    if gate:
        # In the unified gate, the physics arm is the Newton-default gate. --require-newton is left to
        # the caller's environment (the pre-push hook adds it); here we keep the portable invariant gate
        # so the unified tool runs on a no-GPU box too (gates whichever backend resolved).
        argv.append("--gate")
    return _run(argv, "PHYSICS (ODE<->Newton)")
...
def run_render_arm(world, gate, strict_render):
    ...
    if os.path.exists(RENDER_ORACLE):
        return _run([sys.executable, RENDER_ORACLE, "--world", world], "RENDER (WREN<->wgpu, per-world)")
