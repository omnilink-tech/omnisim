...
headless and reads the OMNISIM_PROBE_GATE report that WbWorld::finalize() writes: every top-level
...
The probe is AUTHORITATIVE (the real WbSolid gate, not a Python re-derivation that would drift) and
...
Per articulation the probe records:  backend=<ode|newton|auto>  gate=<capable|mesh|joint>
  ...
  - backend == ode             -> ODE. WHY: gate in {mesh, joint} = a Newton CAPABILITY GAP (what the plan
                                  closes); gate == capable = a deliberate user/world pin, NOT a gap.

This v1 measures Newton ELIGIBILITY (what the gate allows). The plan's full metric also requires "matches
ODE within the physics-oracle tolerance" -- that faithful-match layer is a follow-up (it needs the oracle
wired per-world); eligibility is the signal the W1-W3 gap-closing workstreams move.
...
def default_corpus():
    pats = ["projects/robots/**/worlds/*.wbt", "projects/samples/**/worlds/*.wbt"]
    worlds = []
    for p in pats:
        for w in globmod.glob(os.path.join(REPO, p), recursive=True):
            if os.path.basename(w).startswith("."):  # skip .spot_rl_env* scratch worlds
                continue
            worlds.append(w)
