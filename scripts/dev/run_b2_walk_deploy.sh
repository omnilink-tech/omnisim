...
#
# Usage:  bash scripts/dev/run_b2_walk_deploy.sh [duration] [--policy <onnx>] [--bare] [--gui] [--realtime] [--cpu-engine]
#
# NOTE: bash sibling of run_b2_walk_deploy.ps1 (the Windows launcher) --
...
export B2_GAIT_VX="0.5"
export B2_GAIT_FREQ="1.3"
export B2_GAIT_DUTY="0.6"
export B2_GAIT_STEP_H="0.08"
export B2_GAIT_BODY_H="0.50"
export B2_GAIT_RAMP_S="1.0"
export B2_ACT_SCALE="0.15"
export B2_DEPLOY_LOG="$LOG"
export B2_DEPLOY_TRACE="1"

RUN_ARGS=("$WORLD" --duration "$DURATION")
if [ "$GUI" = "1" ]; then RUN_ARGS+=(--gui); fi
if [ "$REALTIME" = "1" ]; then RUN_ARGS+=(--realtime); fi
if [ "$BARE" = "1" ]; then echo "Running b2_walk_deploy.omniworld for ${DURATION}s wall (BARE gait model)..."
else                       echo "Running b2_walk_deploy.omniworld for ${DURATION}s wall (trot model + RL policy)..."; fi
