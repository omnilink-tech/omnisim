...
"""URDF → Webots VRML converter.

Converts a URDF robot description into the equivalent VRML node tree
that the Webots simulator can load directly. Kept in sync with the C++
runtime importer at src/omnisim/vrml/WbUrdfImporter.cpp; the two
...
def _resolve_mesh_path(filename_attr: str, urdf_dir: Path) -> str:
    """Replica of WbUrdfImporter::resolveMeshPath. Returns absolute forward-slash
...
def emit_sensors_for_link(link_name: str, indent: str,
    ...
    """Emit Webots device nodes for sensors attached to this link."""
    if link_name not in sensors_by_link:
        return ""
    out = []
    for s in sensors_by_link[link_name]:
        if s.kind == "imu":
            # Webots has no single IMU node — emit the three companion devices
...
def emit_link_physics_and_bounding(link: Link, indent: str) -> str:
    ...
    if link.inertial is not None:
        ...
        # Webots requires centerOfMass when inertiaMatrix is set. Emit it
...
def emit_robot(robot: UrdfRobot, controller: str = "<none>") -> str:
    """Emit a Webots Robot { ... } node tree from a parsed URDF."""
...
def build_report(robot: UrdfRobot) -> dict:
    ...
    for joint in robot.joints:
        ...
        if not _vectors_close(joint.axis, axis_parent):
            joint_notes.append(
                "Joint axis is rotated by the joint origin before emission because Webots expects the axis in the parent frame."
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Convert URDF to Webots VRML")
