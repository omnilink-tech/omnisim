...
"runCommand" the OmniLink platform's project page can list:
    ...
    python scripts/dev/omnisim_run_agent.py --agent drone_surveyor
...
# ---------------------------------------------------------------------------
# Webots binary discovery
# ---------------------------------------------------------------------------


def find_webots_binary() -> Path:
...
def webots_env() -> dict:
    """Build the env dict Webots expects, with msys2 toolchain on Windows."""
    env = dict(os.environ)
...
def launch_world(spec: AgentSpec, *, headless: bool, duration: Optional[int]) -> Optional[subprocess.Popen]:
    ...
    proc = _spawn(args, env=webots_env())
    if duration:
...
def launch_agent(spec: AgentSpec, *, port_override: Optional[int]) -> Optional[subprocess.Popen]:
    ...
    if not os.environ.get("OMNI_KEY", "").strip():
        ...
        print('  Get one at https://www.omnilink-agents.com/omnilink-api')
        return None
...
def run(spec: AgentSpec, args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not args.no_agent:
            ...
            if agent_proc is None and spec.agent_script is not None and not args.allow_missing_omni_key:
                # Only return non-zero if the user expected the agent to run.
                # If they explicitly asked for --no-agent we don't get here.
                if world_proc is not None:
                    print("  [info]  bridge is up; OMNI_KEY missing so the agent stays off.")
                    print("          run the agent yourself when ready, or set OMNI_KEY and re-run.")
