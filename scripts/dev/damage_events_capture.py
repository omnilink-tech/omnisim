...
def _resolve_binary() -> Path:
    """Same resolution rule as scripts/dev/headless_runner.py."""
    here = Path(__file__).resolve()
    repo_root = here.parents[2]
    # omnisim-bin.exe only -- no webots-bin.exe fallback. No Makefile produces
    # that name any more, so a leftover copy is a stale artefact; falling back
    # to it would silently run a weeks-old binary instead of failing loudly.
    cand = repo_root / "msys64" / "mingw64" / "bin" / "omnisim-bin.exe"
    if cand.is_file():
        return cand
    raise FileNotFoundError("omnisim-bin.exe not found under msys64/mingw64/bin")
