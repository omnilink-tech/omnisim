...
#
# -Robot accepts h1 and g1. "valkyrie" was removed 2026-08-24: the robot
# package went in be41986f8 and took specs/valkyrie.json with it, so the
# switch offered a value the spec check below could only ever reject.
param(
    [Parameter(Mandatory=$true)][ValidateSet("h1","g1")][string]$Robot,
    [int]$Duration = 20,
    [switch]$Gui,
