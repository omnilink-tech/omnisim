...
"""faithful_check — the faithful-match layer of the Newton coverage meter (newton-ode-replacement-plan.md W5.4).

newton_coverage.py measures gate ELIGIBILITY (which backend resolves). This measures FAITHFULNESS: does an
eligible world actually TRACK ODE? For each world it runs the sim twice -- Newton (default) and forced-ODE
(OMNISIM_FORCE_ODE=1) -- with the in-binary OMNISIM_PROBE_TRAJ probe (every step it dumps each articulation
ROOT's world position), then compares the two trajectories per root over time.

HONEST reading: ODE and Newton are different solvers, so even a faithful world drifts a little, and a
genuinely DYNAMIC/chaotic world (a walking/falling robot) diverges by design -- a fidelity fact, not a
Newton bug. So the signal is (a) the EARLY divergence (before chaos amplifies) and (b) the quasi-static
worlds. We report both an early-window max and the full-window max, and classify on the early window.

Run from PowerShell (Newton needs warp). Build omnisim-bin first.

Usage:
    python scripts/dev/faithful_check.py --worlds projects/samples/demos/worlds/showcase/warehouse_husky.wbt ...
    python scripts/dev/faithful_check.py --glob "projects/robots/**/worlds/*.wbt"
"""
...
def _one_run(world, out, force_ode):
    ...
    if force_ode:
        ...
        # plane/mesh exists -- else dynamic props fall through it) + substep contact (XPBD penetrates at
        # impact on a single substep). Caller-set values win. ODE always has its statics, so this only
        # applies to the Newton run -- making the comparison Newton-at-its-best vs ODE.
...
def main():
    ap = argparse.ArgumentParser(description="Newton faithful-match check (W5.4)")
