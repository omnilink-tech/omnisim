...
range (`min..max`), marking each `[ok ]` or `[OUT]`.

EXIT CODES -- the plain and `--json` invocations ALWAYS exit 0, even
when a category reads `[OUT]`. Only `--check` exits 1 on drift. That is
deliberate, not an oversight: `tests/sources/test_naming.py` shells out
to `--json` with `check=True`, and the documented re-snapshot recipe is
a plain `--json` redirect, so a non-zero default would break both. Use
`--check` for a CI fence; read `[OUT]` as advisory everywhere else.

The two fences are also aimed at different things, and the difference
matters when one of them goes red:

  * `expected_max` (here) is a coarse ceiling with headroom. It answers
    "has this category exploded?"
  * `tests/baselines/rename_audit_baseline.json` is the tight,
    one-directional ratchet. It answers "did this commit add even one
    new Webots-named symbol?" -- and it is the real anti-drift gate.

A ceiling that sits *below* the category's true, documented floor is as
useless as no gate at all: it reads `[OUT]` on every run, so nobody can
tell a real regression from the standing red. When a deliberate
reduction lands, lower the matching `expected_max`; when deliberate
*growth* lands (as it did for `webots_home_env`, where the OMNISIM_*
migration kept WEBOTS_* as a working fallback so every reader now names
both), raise it past the new floor and say why.

Naming policy: [AGENTS.md §0](../../AGENTS.md).
"""
...
def _git_source_files() -> list[Path] | None:
    """Every file git tracks *plus* every untracked-but-not-ignored file,
    ...
    gitignored file are absent by construction -- `--exclude-standard`
    honours .gitignore/.git/info/exclude exactly as `ls-files` does.
    ...
    Untracked-but-not-ignored files are included because a rename lands in
    the working tree *before* it is staged: the renamed-away file is gone
    from disk (so `ls-files` skips it via the `is_file()` test in
    `_walk_repo`) while the renamed-to file is not yet tracked. Counting
    only tracked files therefore reports a transient, too-low number
    mid-migration, and a baseline snapshotted at that moment flips the
    ratchet red the instant the rename is staged. Measured on the
    2026-08-16 launcher rename: WEBOTS_HOME read 316 untracked vs 318
    staged, purely because `omnisim_controller.c` was invisible. A clean
    CI checkout has no untracked files, so this term is empty there and
    the gate stays reproducible.
    """
    try:
...
def collect() -> dict[str, Category]:
    cats = {
        ...
        # 200 was the Phase-I.6 ceiling, set 2026-05-25 when this category
        # read 101. It then grew *by design*: the runtime env contract moved
        # to OMNISIM_* with WEBOTS_* kept as a working fallback, so every
        # reader names both (AGENTS.md §10). The count passed 200 long ago
        # and `--check` has been standing-red ever since, which is why the
        # ceiling is raised here rather than lowered. The tight ratchet is
        # the baseline JSON, not this number.
        "webots_home_env": Category(
            "webots_home_env",
            "WEBOTS_HOME env-var reads / writes inside text files",
            expected_min=0, expected_max=400,
        ),
    }
