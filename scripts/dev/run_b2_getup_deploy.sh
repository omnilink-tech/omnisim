...
#
# Usage: bash scripts/dev/run_b2_getup_deploy.sh [duration] [--gui] [--loop] [--policy <onnx>] [--cpu-engine]
#
# NOTE: bash sibling of run_b2_getup_deploy.ps1 (the Windows launcher) --
...
WORLD="$ROOT/projects/policies/research/worlds/b2_getup_deploy.omniworld"
...
export OMNISIM_NEWTON_TARGET_KE="1400"
export OMNISIM_NEWTON_TARGET_KD="35"
export OMNISIM_LOG_PATH="$ROOT/_scratch/omnisim_log_b2_getup.txt"
export B2_GETUP_REF="$ROOT/projects/policies/research/shadowing/ghosts/b2_getup_ghost.npz"
export B2_GETUP_LOG="$LOG"
export B2_GETUP_RES_SCALE="0.15"
export B2_SETTLE_S="1.5"
if [ "$LOOP" = "1" ]; then export B2_GETUP_LOOP="1"; fi
if [ -f "$POLICY" ]; then export B2_GETUP_POLICY="$POLICY"; echo "RL policy: $POLICY"
