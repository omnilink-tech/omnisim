# Deploy the shadowing pipeline END-TO-END on a NON-gait Spot motion: the Spot
...
#
# Pipeline: generate_spot_crouch.py (MPPI, Component 1) -> ghost_verifier (C2,
# certified PASS) -> THIS deploy (Stage D). The crouch ghost lives at
# projects/policies/research/shadowing/ghosts/spot_crouch_ghost.npz.
#
# Usage: run_spot_crouch_deploy.ps1 [-Duration 30] [-Loop] [-Gui] [-Policy <onnx>]
param(
    [int]$Duration = 30,
    [switch]$Loop,
    [switch]$Gui,
...
$world = "$root\projects\policies\research\worlds\spot_crouch_deploy.wbt"
$log   = "$root\_scratch\spot_crouch_deploy.log"
...
$env:OMNISIM_LOG_PATH            = "$root\_scratch\omnisim_log_spot_crouch.txt"
$env:SPOT_CROUCH_REF  = "$root\projects\policies\research\shadowing\ghosts\spot_crouch_ghost.npz"
$env:SPOT_CROUCH_LOG  = $log
$env:SPOT_SETTLE_S    = "1.0"
if ($Loop)   { $env:SPOT_CROUCH_LOOP = "1" } else { Remove-Item Env:SPOT_CROUCH_LOOP -ErrorAction SilentlyContinue }
if ($Policy -ne "") { $env:SPOT_CROUCH_POLICY = $Policy } else { Remove-Item Env:SPOT_CROUCH_POLICY -ErrorAction SilentlyContinue }

if ($Policy -ne "") { Write-Host "Spot crouch deploy (RL residual: $Policy)" }
else                { Write-Host "Spot crouch deploy (FEEDFORWARD -- shadowing the certified ghost)" }
if ($Gui) {
...
Write-Host "`n===== SPOT CROUCH RESULT ====="
if (Test-Path $log) {
