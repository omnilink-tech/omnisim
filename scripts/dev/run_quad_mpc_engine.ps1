...
if ($Orig) { $world = "$root\projects\policies\research\worlds\omniquad_rough_track.omniworld" }
else       { $world = "$root\projects\policies\research\worlds\omniquad_terrain_mpc_bigfoot.omniworld" }
$env:OMNISIM_INENGINE_MPC_LOG = "$root\_scratch\omniquad_terrain_mpc_loco.log"
Remove-Item $env:OMNISIM_INENGINE_MPC_LOG -ErrorAction SilentlyContinue
$log = "$root\_scratch\omniquad_terrain_mpc.log"
New-Item -ItemType Directory -Force -Path "$root\_scratch" | Out-Null
Remove-Item $log -ErrorAction SilentlyContinue

# --- Newton deploy env (matches run_omniquad_walk_deploy.ps1) + collidable terrain ---
...
$env:MPC_LOCO_GAIT_MODULE = "projects.policies.control.gait.omniquad_trot_gait"
...
$env:OMNIQUAD_POLICY_ONNX = "$root\__no_policy__"
...
$env:OMNIQUAD_DEPLOY_LOG = $log; $env:OMNIQUAD_DEPLOY_TRACE = "1"
