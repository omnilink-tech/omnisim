# WATCH OmniQuad walk/STOP/walk beside its gait-model GHOST -- the time-aware
...
# model. BOTH follow the same time-aware schedule -- walk OMNIQUAD_WALK_FOR_S
# seconds, then STAND OMNIQUAD_STAND_FOR_S seconds, repeating. The trot model is
...
# ideal target.
#
# By default it runs the EXISTING straight-walk policy (gpu_omniquad_walk_main, 48-d
...
# Usage:
#   run_omniquad_walk_vc_ghost_demo.ps1 [-Duration 60] [-WalkFor 5] [-StandFor 5]
...
if ($Policy -eq "") {
    # The velocity-conditioned walker (gpu_omniquad_vc3_c1): walks/stops/resumes on
...
$env:OMNIQUAD_WALK_FOR_S  = "$WalkFor"
$env:OMNIQUAD_STAND_FOR_S = "$StandFor"
$env:OMNIQUAD_MODE_BLEND_S = "$BlendS"
$env:OMNIQUAD_VX_CMD_MAX  = "$VcCmdMax"     # 0 = existing 48-d policy; >0 = VC policy
$env:OMNIQUAD_DEPLOY_LOG  = "$root\_scratch\omniquad_walk_vc_ghost_demo.log"
# Hologram look.
$env:OMNIQUAD_GHOST_Y     = "1.4"
$env:OMNIQUAD_GHOST_ALPHA = "0.6"
$env:OMNIQUAD_GHOST_TINT  = "0.62,0.82,1.0"

Write-Host "Opening omniquad_walk_ghost_demo.wbt: real OmniQuad + hologram ghost, walk ${WalkFor}s / STOP ${StandFor}s repeating."
