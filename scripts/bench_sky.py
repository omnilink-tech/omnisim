...
OmBackground.applySkyBoxToWren isn't safe to call twice in the same
...
# Cross-platform binary resolution (Windows msys64 — omnisim-bin.exe first,
# then the legacy webots.exe alias — Linux bin/, macOS bundle). Fall back to
# the legacy Windows path so the error message still prints a helpful
# location on an unbuilt checkout.
WEBOTS_EXE = Path(resolve_omnisim_binary() or _BIN_DIR / "omnisim-bin.exe")
...
def render_shot(src: Path, shot: dict, out_dir: Path) -> Path:
    name = shot["name"]
    bench_world = src.with_name(f"_bench_{src.stem}_{name}{src.suffix}")
