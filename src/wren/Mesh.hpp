...
namespace wren {
  ...
  public:
    ...
    // Vertex and triangle counts for diagnostics. Default reads from
    // the local storage; subclasses that clear those after GPU upload
    // (StaticMesh) override to read from their cache.
    virtual int vertexCount() const { return static_cast<int>(mCoords.size()); }
    virtual int triangleCount() const { return static_cast<int>(mIndices.size() / 3); }
