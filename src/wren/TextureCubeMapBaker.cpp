...
namespace wren {
  namespace texturecubemapbaker {
    ...
    TextureCubeMap *bakeFrom2D(TextureRtt *input2D, ShaderProgram *shader, unsigned int size) {
      // Mirrors bakeDiffuseIrradiance, but the input is a 2D RTT bound
      // to ``WR_GLSL_LAYOUT_UNIFORM_TEXTURE0`` instead of a cube at
      // ``WR_GLSL_LAYOUT_UNIFORM_TEXTURE_CUBE0``.  Used by the
      // atmospheric-sky pipeline to project the 2D sky-view LUT onto
      // a cubemap that the existing IBL specular bake can consume.
      glm::mat4 captureProjection = glm::perspective(glm::radians(90.0f), 1.0f, 0.1f, 10.0f);
      glm::mat4 captureViews[] = {
        glm::lookAt(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)),
        glm::lookAt(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)),
      ...
      unsigned int outputGlName = Texture::generateNewTexture();
      TextureCubeMap *outputCube = TextureCubeMap::createTextureCubeMap();
      outputCube->setGlName(outputGlName);
      outputCube->setTextureUnit(13);
      glstate::activateTextureUnit(13);
      outputCube->setSize(size, size);
      glBindTexture(GL_TEXTURE_CUBE_MAP, outputGlName);
      ...
      shader->setCustomUniformValue("projection", captureProjection);

      input2D->setTextureUnit(WR_GLSL_LAYOUT_UNIFORM_TEXTURE0);
      input2D->bind(Texture::DEFAULT_USAGE_PARAMS);

      glstate::bindProgram(shader->glName());
      glViewport(0, 0, size, size);
      glstate::bindFrameBuffer(captureFBO);
      glUniform1i(shader->uniformLocation(WR_GLSL_LAYOUT_UNIFORM_TEXTURE0), WR_GLSL_LAYOUT_UNIFORM_TEXTURE0);
      for (unsigned int i = 0; i < 6; ++i) {
        shader->setCustomUniformValue("view", captureViews[i]);
        shader->bind();
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, outputGlName, 0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        renderCube();
      }

      // Generate the mip chain so the downstream specular pre-filter
      // can sample lower-roughness lobes against a properly filtered
      // source.  Without this, low-roughness materials show aliasing
      // on the procedural sky.
      glBindTexture(GL_TEXTURE_CUBE_MAP, outputGlName);
      glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
...
WrTextureCubeMap *wr_texture_cubemap_bake_from_2d(WrTextureRtt *input2D, WrShaderProgram *shader, unsigned int size) {
  return reinterpret_cast<WrTextureCubeMap *>(wren::texturecubemapbaker::bakeFrom2D(
    reinterpret_cast<wren::TextureRtt *>(input2D), reinterpret_cast<wren::ShaderProgram *>(shader), size));
}
