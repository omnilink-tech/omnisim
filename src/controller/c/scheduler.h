...
// scheduler_init_* return values. ABI_MISMATCH is FATAL and non-retryable: the IPC
// handshake (messages.h) proved the engine and this libController are different builds --
// the caller must fail loudly (durable diagnostic + exit), never retry into a hang.
#define SCHEDULER_INIT_FAILURE 0       // could not connect; retryable
#define SCHEDULER_INIT_SUCCESS 1
#define SCHEDULER_INIT_ABI_MISMATCH (-1)
...
// Arm the handshake's launch-nonce cross-check with a nonce learned outside the environment
