...
#include <omnisim/display.h>  // used for WB_IMAGE_RGB constant
#include <omnisim/remote_control.h>
...
WbRequest *remote_control_handle_messages(WbRequest *r) {
  ...
  while (request_is_over(r) == 0) {
    ...
    // copy the request for all device types we still want to send to OmniSim.
