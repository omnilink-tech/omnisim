...
int g_pipe_receive_timeout(GPipe *p, char *data, int size, int timeout_ms) {
  int received = 0;
#ifdef _WIN32
  assert(p->handle);
  int elapsed_ms = 0;
  while (received < size) {
    DWORD available = 0;
    if (!PeekNamedPipe(p->handle, NULL, 0, NULL, &available, NULL))
    ...
    DWORD nb_read = 0;
    DWORD to_read = size - received;
    if (available < to_read)
      to_read = available;
    if (!ReadFile(p->handle, data + received, to_read, &nb_read, NULL) && GetLastError() != ERROR_MORE_DATA)
      return -1;
    received += (int)nb_read;
  }
#else
  const int fd = p->handle ? p->handle : p->fd[0];
  ...
  while (received < size) {
    ...
    if (ready == -1 && errno == EINTR)
      continue;  // retry with the (conservatively unchanged) remaining budget
    if (ready <= 0)
      return -1;  // timeout or select error
    const int n = read(fd, data + received, size - received);
    if (n <= 0 && errno != EINTR)
      return -1;
    if (n > 0)
      received += n;
