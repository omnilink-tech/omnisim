...
// solve_ik request (pending while non-NULL node ref) + last answer. The
// answer buffers are owned here and stay valid until the next call, matching
// wb_supervisor_node_get_contact_points()'s buffer-lifetime convention.
static WbNodeRef solve_ik_node_ref = NULL;
static int solve_ik_n_targets = 0;
static const double *solve_ik_targets = NULL;
static const double *solve_ik_rotations = NULL;    // NULL or 4 * n_targets
static const double *solve_ik_tool_offset = NULL;  // NULL or 3
static int solve_ik_iterations = 64;
static int solve_ik_status = -9;  // -9 = "the engine did not answer"
static int solve_ik_result_n_joints = 0;
static int solve_ik_result_n_targets = 0;
static int *solve_ik_result_joint_ids = NULL;
static double *solve_ik_result_angles = NULL;
static double *solve_ik_result_residuals = NULL;

static void free_solve_ik_results() {
  free(solve_ik_result_joint_ids);
  free(solve_ik_result_angles);
  free(solve_ik_result_residuals);
  solve_ik_result_joint_ids = NULL;
  solve_ik_result_angles = NULL;
  solve_ik_result_residuals = NULL;
  solve_ik_result_n_joints = 0;
...
static void supervisor_write_request(WbDevice *d, WbRequest *r) {
  ...
  if (node_id >= 0) {
    ...
    while (request) {
      ...
      if (request->type != GET) {
        ...
        // request is required when getting back the answer from OmniSim
  ...
  if (solve_ik_node_ref) {
    int i;
    request_write_uchar(r, C_SUPERVISOR_NODE_SOLVE_IK);
    request_write_uint32(r, solve_ik_node_ref->id);
    request_write_int32(r, solve_ik_n_targets);
    for (i = 0; i < 3 * solve_ik_n_targets; ++i)
      request_write_double(r, solve_ik_targets[i]);
    request_write_uchar(r, solve_ik_rotations ? 1 : 0);
    if (solve_ik_rotations)
      for (i = 0; i < 4 * solve_ik_n_targets; ++i)
        request_write_double(r, solve_ik_rotations[i]);
    request_write_uchar(r, solve_ik_tool_offset ? 1 : 0);
    if (solve_ik_tool_offset)
      for (i = 0; i < 3; ++i)
...
static void supervisor_read_answer(WbDevice *d, WbRequest *r) {
  ...
  switch (request_read_uchar(r)) {
    ...
    case C_SUPERVISOR_NODE_GET_CONTACT_POINTS: {
      ...
      if (n_points > 0) {
        ...
        for (i = 0; i < n_points; i++) {
          ...
          points[i].depth = request_read_double(r);  // trailing field (2026-05-29)
        }
      }
    ...
    case C_SUPERVISOR_NODE_SOLVE_IK: {
      // Fixed shape: status, n_joints, n_targets, then (on success) joint
      // node ids, problem-major angles, per-target residuals. The engine
      // always answers a solve_ik request, failures included, so a stale
      // -9 "did not answer" can only mean the exchange itself died.
      solve_ik_status = request_read_int32(r);
      const int n_joints = request_read_int32(r);
      const int n_targets = request_read_int32(r);
      free_solve_ik_results();
      solve_ik_result_n_joints = n_joints;
      solve_ik_result_n_targets = n_targets;
      if (n_joints > 0) {
        solve_ik_result_joint_ids = malloc(n_joints * sizeof(int));
        for (i = 0; i < n_joints; i++)
          solve_ik_result_joint_ids[i] = request_read_int32(r);
      }
      if (n_joints > 0 && n_targets > 0) {
        solve_ik_result_angles = malloc((size_t)n_joints * n_targets * sizeof(double));
        for (i = 0; i < n_joints * n_targets; i++)
          solve_ik_result_angles[i] = request_read_double(r);
      }
      if (n_targets > 0) {
  ...
  // free requests previously sent to OmniSim
...
bool wb_supervisor_world_save(const char *filename) {
  ...
  // Dual-read/dual-write: '.omniworld' is the current world extension, '.wbt' the
  // legacy one. Both are accepted here so existing supervisors keep working.
  if (filename) {
    const char *ext = wb_file_get_extension(filename);
    if (strcmp("omniworld", ext) != 0 && strcmp("wbt", ext) != 0) {
      fprintf(stderr, "Error: the target file given to %s() should have the '.omniworld' (or legacy '.wbt') extension.\n",
              __FUNCTION__);
      return false;
    }
  }
...
WbNodeRef wb_supervisor_node_get_from_def(const char *def) {
  ...
  if (!result) {
    // otherwise: need to talk to OmniSim
    node_def_name = def;
    node_id = -1;
    wb_robot_flush_unlocked(__FUNCTION__);
    if (node_id >= 0)
...
int wb_supervisor_node_solve_ik(WbNodeRef node, int n_targets, const double *targets, const double *rotations,
  ...
  if (n_targets <= 0 || targets == NULL) {
    if (!robot_is_quitting())
      fprintf(stderr, "Error: %s() requires at least one [x,y,z] target.\n", __FUNCTION__);
    return -10;
  }

  if (n_targets > 4096) {
    // Mirror of the engine-side bounded read: past this the decode would
    // truncate and the answer's target count would no longer match ours.
    if (!robot_is_quitting())
      fprintf(stderr, "Error: %s() supports at most 4096 targets per call (got %d).\n", __FUNCTION__, n_targets);
    return -10;
  }
  ...
  robot_mutex_lock();
  solve_ik_node_ref = node;
  solve_ik_n_targets = n_targets;
  solve_ik_targets = targets;
  solve_ik_rotations = rotations;
  solve_ik_tool_offset = tool_offset;
  solve_ik_iterations = iterations;
  solve_ik_status = -9;  // overwritten by the answer; survives only if none arrives
  wb_robot_flush_unlocked(__FUNCTION__);
  solve_ik_node_ref = NULL;
  solve_ik_targets = NULL;
  solve_ik_rotations = NULL;
  solve_ik_tool_offset = NULL;
  const int status = solve_ik_status;
  if (status == 0) {
    if (n_joints)
      *n_joints = solve_ik_result_n_joints;
    if (joint_node_ids)
      *joint_node_ids = solve_ik_result_joint_ids;
    if (angles)
...
WbProtoRef wb_supervisor_node_get_proto(WbNodeRef node) {
  ...
  if (!is_proto_ref_valid(node->proto_info)) {
    // if we don't know the proto info yet, we need to talk to OmniSim
...
// The VirtualRealityHeadset node was retired together with the WREN renderer. These three symbols
// are deliberately KEPT and still exported so that already-compiled controllers keep linking and
// loading (the same precedent used when the Radio and Microphone nodes were deleted while wb_radio_*
// and wb_microphone_* stayed exported). They now answer "no headset in use" locally, without a round
// trip to the simulator. The return values are exactly what the pre-existing no-headset path
// returned: false, and `invalid_vector` -- a static all-NaN double[16], never NULL and never a
// dangling pointer. The robot_check_supervisor() guard is kept so a non-supervisor Robot still gets
// the same warning it always did.
...
WbProtoRef wb_supervisor_proto_get_parent(WbProtoRef proto) {
  ...
  if (!is_proto_ref_valid(proto->parent)) {
    // if we don't know the parent yet, we need to talk to OmniSim
    WbProtoRef proto_list_before = proto_list;
