...
//-----------------------------------------------------------
// Purpose:  OmniSim API for loading and playing .motion files
//-----------------------------------------------------------
...
#include <omnisim/utils/motion.h>
...
// ON-DISK FILE-FORMAT MAGIC -- NOT a naming leftover, do not "migrate" it.
// Every shipped .motion file starts with this token, and the motion editor that WRITES them
// (resources/projects/libraries/qt_utils/motion_editor/Motion.cpp) emits and re-validates the
// same string. Renaming it here alone would make libController reject every existing .motion
// asset. A format rename has to be a dual-read change made on both sides at once.
