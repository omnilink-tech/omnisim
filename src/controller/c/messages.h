...
// ---- OmniSim IPC handshake (core-evolution-plan.md, Phase I1) ----
// Exchanged once per connection, immediately after transport connect and before
// any request/answer packet, on every transport (intern pipe, extern pipe, TCP).
// The engine writes its 16-byte hello as soon as it accepts the connection;
// libController validates it and replies with the same frame carrying its own
// protocol version; the engine validates the echo before entering the packet
// loop. Any mismatch or timeout is a FATAL, attributed error on both sides: a
// build-mismatched engine/libController pair must fail loudly at connect time,
// never reach the packet loop and hang at zero ticks (the stale-lib trap that
// `omnisim doctor` gates out-of-band, commit 6eea9d76).
//
// Frame layout, 16 bytes, little-endian:
//   [0..3]   magic "OMSH"
//   [4..5]   u16 protocol version (OMNISIM_IPC_VERSION)
//   [6..7]   u16 flags (reserved, must be 0)
//   [8..15]  u64 launch nonce (the engine PID; libController echoes it
//            verbatim, and an intern controller cross-checks it against
//            OMNISIM_IPC_NONCE to detect crossing onto a stale instance)
//
// Bump OMNISIM_IPC_VERSION on ANY change to the packet framing, the device
// (de)serialization, or the handshake itself.
#define OMNISIM_IPC_MAGIC "OMSH"
#define OMNISIM_IPC_VERSION 1
#define OMNISIM_IPC_HELLO_SIZE 16
// A valid controller can be delayed well beyond five seconds when a large world launches
...
// Batched IK preview against the live Newton model (internal parity plan, item W2.1).
// ctr -> sim: end-effector node id + Cartesian targets; sim -> ctr: per-slot
// joint node ids, angles and per-target FK residuals. APPEND-ONLY section:
