...
struct Sound {
  ...
  bool need_stop;
  bool is_playing;
  unsigned char *upload_data;  // sound data for uploading to OmniSim
...
typedef struct {
  Sound *sound_list;
  char *text;
  char engine[10];   // "microsoft" (Windows only)
  char language[6];  // e.g. "en-US", "fr-FR" — set of available codes depends on the host's Microsoft TTS install
  double text_volume;
  bool need_stop;
  bool need_stop_all;
    ...
    while (sound) {
      if (sound->need_update) {
        ...
        if (sound->upload_size) {
           ...
           * OmniSim will cache the data inside the device.
           */
...
#ifdef _WIN32
  if (strncmp(engine, "microsoft", 10) != 0)
    return false;
  strncpy(speaker->engine, "microsoft", sizeof(speaker->engine));
...
#ifdef _WIN32
  if (strncmp(speaker->engine, "microsoft", sizeof(speaker->engine)) == 0) {
