...
#include <omnisim/console.h>
#include <omnisim/joystick.h>
#include <omnisim/keyboard.h>
#include <omnisim/mouse.h>
...
#define OMNISIM_EXIT_FALSE 0
#define OMNISIM_EXIT_NOW 1
#define OMNISIM_EXIT_LATER 2
...
typedef struct {
  ...
  unsigned char omnisim_exit;  // OMNISIM_EXIT_FALSE, OMNISIM_EXIT_NOW or OMNISIM_EXIT_LATER
...
// Runtime env-var contract: OMNISIM_* is the ONLY spelling this runtime reads. The engine
// (OmController::setProcessEnvironment) and the launcher (omnisim_controller.c) write exactly
// these names and no longer write the legacy WEBOTS_* twins, so both halves moved together.
//
// Legacy-only is not silently ignored: a WEBOTS_* value with no OMNISIM_* counterpart means the
// caller (a user shell, an external tool, or a simulator binary older than the rename) is still
// on the retired name, and without a message the symptom is a 50-second wait ending in "Giving
// up" -- exactly the failure this warning exists to name. Warned once per variable per process.
//
// Same lifetime contract as wbu_system_getenv(): on Windows the returned pointer lives in one
// shared static buffer which the next wbu_system call overwrites. The legacy probe is safe
// because it is only reached on the path where the canonical lookup found nothing and we are
// therefore returning NULL -- it never clobbers a value this function is about to hand back.
static const char *getenv_omnisim(const char *omnisim_name, const char *legacy_name, bool *warned) {
  const char *value = wbu_system_getenv(omnisim_name);
  if (value && value[0])
    return value;
  if (!*warned) {
    const char *legacy = wbu_system_getenv(legacy_name);
    if (legacy && legacy[0]) {
      *warned = true;
      fprintf(stderr,
              "Warning: %s is set but %s is not. OmniSim reads only %s -- the %s name was retired. "
              "Rename it (or upgrade the tool that sets it); the legacy value is being ignored.\n",
              legacy_name, omnisim_name, omnisim_name, legacy_name);
    }
  }
  return NULL;
}

static bool warned_legacy_robot_name = false;
static bool warned_legacy_controller_url = false;

static const char *getenv_robot_name() {
  return getenv_omnisim("OMNISIM_ROBOT_NAME", "WEBOTS_ROBOT_NAME", &warned_legacy_robot_name);
}

static const char *getenv_controller_url() {
  return getenv_omnisim("OMNISIM_CONTROLLER_URL", "WEBOTS_CONTROLLER_URL", &warned_legacy_controller_url);
}

static int stream_pipe_create(int stream) {
  int fds[2];
#ifdef WIN32
  // 64 KB kernel pipe buffer (was 1024). A noisy controller can now write up
  // to ~64 KB between steps without blocking on a full pipe.
  _pipe(fds, 65536, O_TEXT);
#else
  if (pipe(fds) == -1) {
    fprintf(stderr, "Error: cannot create pipe for OMNISIM_STDOUT_REDIRECT.\n");
    exit(EXIT_FAILURE);
  }
...
// Drain whatever is queued on `fd` into a freshly allocated, NUL-terminated
// heap buffer (`*buffer`). Caller owns the buffer and must free it. If
// nothing is available, `*buffer` is set to NULL.
//
// Windows: anonymous pipes are blocking on read, so we use the same _eof
// non-data check as the original code, then issue ONE read into a 64-KB
// buffer (matched to the pipe size set in stream_pipe_create). This stays
// fast for small outputs (no extra Win32 API calls) while raising the
// per-step cap from ~1 KB to ~64 KB.
//
// POSIX: the read end is O_NONBLOCK, so we loop and grow the buffer until
// read() returns 0 (EOF) or -1 (EAGAIN/EWOULDBLOCK). Hard cap 4 MB.
...
#ifdef _WIN32
  // Match the kernel pipe size set in stream_pipe_create.
  const size_t cap = 65536;
  *buffer = malloc(cap);
  if (!*buffer)
    return;
  int len = eof(fd) ? 0 : read(fd, *buffer, (unsigned int)(cap - 1));
  if (len <= 0) {
    free(*buffer);
    *buffer = NULL;
    return;
  ...
  (*buffer)[len] = '\0';
#else
  const size_t hard_cap = 4 * 1024 * 1024;
  size_t cap = 4096;
  size_t len = 0;
  *buffer = malloc(cap);
  if (!*buffer)
  ...
  for (;;) {
    if (cap - len - 1 < 1024) {
      ...
      if (want > hard_cap)
        want = hard_cap;
      if (want == cap) {
        // Already at hard cap: stop draining; remaining bytes will arrive on
        // the next step. We do not silently drop them.
        fprintf(stderr,
                "Warning: controller stdout/stderr exceeded %zu bytes in one step; "
                "remaining bytes deferred to next step.\n",
                hard_cap);
        break;
      }
...
static void robot_quit() {  // called when OmniSim kills a controller
  WbDeviceTag tag;
...
static void robot_send_request(unsigned int step_duration) {
  ...
  if (wb_robot_get_mode() == WB_MODE_REMOTE_CONTROL) {
    // it returns a partial request copy to be sent to OmniSim.
...
static void robot_configure(WbRequest *r) {
  ...
  OMNISIM_VERSION = request_read_string(r);
  if (strlen(OMNISIM_VERSION) && (strncmp(OMNISIM_VERSION, LIBCONTROLLER_VERSION, 6)))
    fprintf(stderr,
            "Warning: OmniSim [%s] and libController [%s] versions are not the same for Robot '%s'! Different versions can lead "
            "to undefined behavior.\n",
            OMNISIM_VERSION, LIBCONTROLLER_VERSION, robot.device[0]->name);
...
void robot_read_answer(WbDevice *d, WbRequest *r) {
  ...
  switch (message) {
    ...
    case C_ROBOT_SIMULATION_CHANGE_MODE:
      ...
      break;
    case C_ROBOT_QUIT:
      robot.omnisim_exit = OMNISIM_EXIT_NOW;
      break;
...
void robot_toggle_remote(WbDevice *d, WbRequest *r) {
  ...
  if (wb_robot_get_mode() == WB_MODE_REMOTE_CONTROL)
    ...
    // in that case the request is not sent to OmniSim
...
WbDeviceTag wb_robot_get_device(const char *name) {
  ...
  if (!robot_init_was_done) {
    ...
    wb_robot_init();
    robot_abort("wb_robot_init() must be called before any other OmniSim function.\n");
  }
...
static int robot_step_begin(int duration) {
  ...
  if (robot.omnisim_exit == OMNISIM_EXIT_NOW) {
    robot_quit();
    robot_mutex_unlock();
    exit(EXIT_SUCCESS);
  } else if (robot.omnisim_exit == OMNISIM_EXIT_LATER) {
    robot.omnisim_exit = OMNISIM_EXIT_NOW;
    return -1;
  }
...
static int robot_step_end() {
  ...
  int e = -1;
  if (robot.omnisim_exit == OMNISIM_EXIT_FALSE)
...
WbUserInputEvent wb_robot_wait_for_user_input_event(WbUserInputEvent event_type, int timeout) {
  ...
  if (robot.omnisim_exit == OMNISIM_EXIT_LATER) {
    robot.omnisim_exit = OMNISIM_EXIT_NOW;
    robot_mutex_unlock();
...
void wb_robot_flush_unlocked(const char *function) {
  if (function && waiting_for_step_end) {
    fprintf(
      stderr,
      "Warning: %s(): functions with immediate requests to OmniSim cannot be implemented in-between wb_robot_step_begin() and "
      "wb_robot_step_end()!\n",
      function);
    return;
  }
  ...
  robot_read_data();
  if (robot.omnisim_exit == OMNISIM_EXIT_NOW)
    robot.omnisim_exit = OMNISIM_EXIT_LATER;
  robot.is_immediate_message = false;
}
...
#ifdef _WIN32
// Phase I3 (core-evolution-plan.md): an extern controller is user-launched with no inherited
// environment, so it cannot learn the launch nonce from OMNISIM_IPC_NONCE. The engine writes
// the nonce to an "ipc-nonce" rendezvous file in its port-salted instance dir; reading it lets
// the extern controller address the nonce-uniquified pipe (closing the stale-pipe race for
// externs) and arms the IPC handshake's nonce cross-check. Returns true when a nonce was read;
// false (legacy nonce-less pipe name) when the file is absent -- an older engine.
static bool read_instance_ipc_nonce(const char *instance_folder, char *nonce, int nonce_size) {
  const int length = strlen(instance_folder) + 11;  // "/ipc-nonce"
  char *path = malloc(length);
  snprintf(path, length, "%s/ipc-nonce", instance_folder);
  FILE *file = fopen(path, "r");
  free(path);
  if (!file)
    return false;
  const bool ok = fgets(nonce, nonce_size, file) != NULL && nonce[0] >= '0' && nonce[0] <= '9';
  fclose(file);
  if (ok) {
    char *newline = strchr(nonce, '\n');
    if (newline)
      *newline = '\0';
  }
  return ok;
...
// NOTE ON THE "webots-" LITERALS BELOW -- THEY ARE A WIRE CONTRACT, NOT A LEFTOVER NAME.
// The named pipe ("\\.\pipe\webots-<tmpId>[-<nonce>]-<robot>") and the instance tmp folder
// ("<TMP>/webots-<tmpId>", "<TMP>/webots/<user>/<tmpId>") are BUILT BY THE ENGINE with exactly
// these spellings -- OmController.cpp (serverName) and OmStandardPaths.cpp (webotsTmpPath).
// libController only has to find what the engine already created. Renaming them on this side
// alone desynchronises the rendezvous and every controller hangs at zero ticks with no error.
// If they are ever renamed it must be one atomic change across engine + libController + the
// Python/C++/Java bindings, which is out of scope for the naming migration.
static char *compute_socket_filename(char *error_buffer) {
  const char *OMNISIM_INSTANCE_PATH = wbu_system_webots_instance_path(true);
  char *robot_name = NULL;
  const char *OMNISIM_ROBOT_NAME = getenv_robot_name();
  char *socket_filename;
  if (OMNISIM_ROBOT_NAME && OMNISIM_ROBOT_NAME[0] && OMNISIM_INSTANCE_PATH && OMNISIM_INSTANCE_PATH[0]) {
    robot_name = encode_robot_name(OMNISIM_ROBOT_NAME, strlen(OMNISIM_INSTANCE_PATH) + strlen("ipc//intern"));
#ifndef _WIN32
    const int length = strlen(OMNISIM_INSTANCE_PATH) + strlen(robot_name) + 15;  // "%sintern/%s/socket"
    socket_filename = malloc(length);
    snprintf(socket_filename, length, "%sipc/%s/intern", OMNISIM_INSTANCE_PATH, robot_name);
#else
    int i = 0;
    int last = -1;
    while (OMNISIM_INSTANCE_PATH[i] != 0) {
      if (OMNISIM_INSTANCE_PATH[i++] == '-')
        last = i;
    }
    int number = -1;
    sscanf(&OMNISIM_INSTANCE_PATH[last], "%d", &number);
    assert(number != -1);
    // Per-launch IPC nonce (the simulator PID, exported by OmController::setProcessEnvironment): the
    // intern pipe name is uniquified as webots-<tmpId>-<nonce>-<robot> so back-to-back headless launches
    // reusing the same TCP port can't collide on an identical named pipe and cross onto a stale instance
    // (the residual launch-flake race, default-flip-plan.md §3.5). Absent (older simulator, or an extern
    // controller), fall back to the legacy webots-<tmpId>-<robot> name.
    const char *ipc_nonce = wbu_system_getenv("OMNISIM_IPC_NONCE");
    if (ipc_nonce && ipc_nonce[0]) {
      const int length = 30 + strlen(ipc_nonce) + strlen(robot_name);  // "\\.\pipe\webots-<num>-<nonce>-<robot>"
      socket_filename = malloc(length);
      snprintf(socket_filename, length, "\\\\.\\pipe\\webots-%d-%s-%s", number, ipc_nonce, robot_name);
    } else {
  ...
  // parse OMNISIM_CONTROLLER_URL to extract protocol, host, port and robot name
  const char *TMP_DIR = wbu_system_tmpdir();
  char *OMNISIM_CONTROLLER_URL = (char *)getenv_controller_url();
  // either the OMNISIM_CONTROLLER_URL is not defined, empty or contains only a robot name
  // default to the most recent /tmp/webots/username/* folder (/tmp/webots-* on Windows)
  if (OMNISIM_CONTROLLER_URL == NULL || OMNISIM_CONTROLLER_URL[0] == 0 || strstr(OMNISIM_CONTROLLER_URL, "://") == NULL) {
#ifndef _WIN32
    const int OMNISIM_TMP_DIR_length = strlen(TMP_DIR) + strlen(username) + 9;  // TMP_DIR + '/webots/' + username
    char *OMNISIM_TMP_DIR = malloc(OMNISIM_TMP_DIR_length);
    snprintf(OMNISIM_TMP_DIR, OMNISIM_TMP_DIR_length, "%s/webots/%s", TMP_DIR, username);
#else
    const int OMNISIM_TMP_DIR_length = strlen(TMP_DIR);
    char *OMNISIM_TMP_DIR = strdup(TMP_DIR);
#endif
    DIR *dr = opendir(OMNISIM_TMP_DIR);
    if (dr == NULL) {
      snprintf(error_buffer, ERROR_BUFFER_SIZE, "Cannot open directory %s", OMNISIM_TMP_DIR);
      free(OMNISIM_TMP_DIR);
      return NULL;
    }
...
#endif
        const int length = OMNISIM_TMP_DIR_length + strlen(de->d_name) + 2;
        char *filename = malloc(length);
        snprintf(filename, length, "%s/%s", OMNISIM_TMP_DIR, de->d_name);
    ...
    closedir(dr);
    free(OMNISIM_TMP_DIR);
    // No OmniSim instance has been started
    if (number == -1) {
      snprintf(error_buffer, ERROR_BUFFER_SIZE, "Cannot find any instance of OmniSim");
      return NULL;
    }
    if (OMNISIM_CONTROLLER_URL && OMNISIM_CONTROLLER_URL[0]) {  // only the robot name was provided in OMNISIM_CONTROLLER_URL
      const int length = 19 + strlen(OMNISIM_CONTROLLER_URL);
      char *tmp = malloc(length);
      snprintf(tmp, length, "ipc://%d/%s", number, OMNISIM_CONTROLLER_URL);
      OMNISIM_CONTROLLER_URL = tmp;
    } else {
      OMNISIM_CONTROLLER_URL = malloc(18);
      snprintf(OMNISIM_CONTROLLER_URL, 18, "ipc://%d", number);
    }
  } else
    OMNISIM_CONTROLLER_URL = strdup(OMNISIM_CONTROLLER_URL);  // it will be free
  if (strncmp(OMNISIM_CONTROLLER_URL, "ipc://", 6) != 0) {
    fprintf(stderr, "Error: unsupported protocol in OMNISIM_CONTROLLER_URL: %s\n", OMNISIM_CONTROLLER_URL);
    exit(EXIT_FAILURE);
  }
  // fprintf(stderr, "OMNISIM_CONTROLLER_URL=%s\n", OMNISIM_CONTROLLER_URL);
  int number = -1;
  sscanf(&OMNISIM_CONTROLLER_URL[6], "%d", &number);
  if (number == -1) {
    fprintf(stderr, "Error: invalid OMNISIM_CONTROLLER_URL: %s (missing or wrong port value)\n", OMNISIM_CONTROLLER_URL);
    exit(EXIT_FAILURE);
  }
#ifdef _WIN32
  const int omnisim_instance_path_length = strlen(TMP_DIR) + 20;  // TMP_DIR + "/webots-12345678901"
  const int ipc_path_length = omnisim_instance_path_length + 4;   // TMP_DIR + "/webots-12345678901/ipc"
#else
  const int omnisim_instance_path_length =
    strlen(TMP_DIR) + strlen(username) + 21;                    // TMP_DIR + '/webots/' + username + '/12345678901'
  const int ipc_path_length = omnisim_instance_path_length + 4;  // TMP_DIR + '/webots/' + username + '/12345678901/ipc'
#endif
  char *omnisim_instance_folder = malloc(omnisim_instance_path_length);
  char *ipc_folder = malloc(ipc_path_length);
#ifdef _WIN32
  snprintf(omnisim_instance_folder, omnisim_instance_path_length, "%s/webots-%d", TMP_DIR, number);
  snprintf(ipc_folder, ipc_path_length, "%s/ipc", omnisim_instance_folder);
#else
  snprintf(omnisim_instance_folder, omnisim_instance_path_length, "%s/webots/%s/%d", TMP_DIR, username, number);
  snprintf(ipc_folder, ipc_path_length, "%s/ipc", omnisim_instance_folder);
#endif

  // check if the OmniSim instance has been started
  DIR *dr = opendir(omnisim_instance_folder);
  if (dr == NULL) {
    snprintf(error_buffer, ERROR_BUFFER_SIZE, "Cannot connect to OmniSim instance");
    free(omnisim_instance_folder);
    free(ipc_folder);
    return NULL;
  }
  closedir(dr);
  size_t path_length = strlen(omnisim_instance_folder) + 9;
  // check if OmniSim is currently loading
  char *loading_file_path = malloc(strlen(omnisim_instance_folder) + 9);  // omnisim_instance_folder + '/loading'
  int written = snprintf(loading_file_path, path_length, "%s/loading", omnisim_instance_folder);
  ...
  if (loading_file) {
    fclose(loading_file);
    snprintf(error_buffer, ERROR_BUFFER_SIZE, "The OmniSim simulation world is not yet ready");
    free(omnisim_instance_folder);
    free(ipc_folder);
    free(loading_file_path);
    return NULL;
...
#ifdef _WIN32
  // Phase I3: learn this instance's launch nonce from its rendezvous file (see
  // read_instance_ipc_nonce above) so the extern pipe name below is nonce-protected.
  char instance_nonce[24] = "";
  const bool has_instance_nonce = read_instance_ipc_nonce(omnisim_instance_folder, instance_nonce, sizeof(instance_nonce));
  if (has_instance_nonce)
    scheduler_set_expected_ipc_nonce(strtoull(instance_nonce, NULL, 10));
#endif
  free(omnisim_instance_folder);
  free(loading_file_path);

  free(robot_name);
  const char *sub_string = strstr(&OMNISIM_CONTROLLER_URL[6], "/");
  ...
  if (robot_name) {
...
#else
    // "\\.\pipe\webots-<num>[-<nonce>]-robot_name" -- nonce-protected when the engine wrote the rendezvous file (I3)
    const int length = 30 + sizeof(instance_nonce) + strlen(robot_name);
    socket_filename = malloc(length);
    if (has_instance_nonce)
      snprintf(socket_filename, length, "\\\\.\\pipe\\webots-%d-%s-%s", number, instance_nonce, robot_name);
    else
      snprintf(socket_filename, length, "\\\\.\\pipe\\webots-%d-%s", number, robot_name);
#endif
    free(robot_name);
    ...
    if (dr == NULL) {  // the ipc folder was not yet created
      ...
      snprintf(error_buffer, ERROR_BUFFER_SIZE,
               "The OmniSim simulation has not yet started or there is no robot in the simulation");
      return NULL;
    }
    char **filenames = NULL;
    int count = 0;
    ...
    if (count == 0) {
      snprintf(error_buffer, ERROR_BUFFER_SIZE,
               "The OmniSim simulation has not yet started or there is no robot with an <extern> controller");
      socket_filename = NULL;
    } else if (count > 1) {  // more than one extern controller in the current instance of OmniSim
      fprintf(stderr,
              "No robot name provided, OmniSim instance %d should have exactly one robot set with an <extern> controller.\n",
              number);
...
#else
      // "\\.\pipe\webots-<num>[-<nonce>]-robot_name" -- nonce-protected when the engine wrote the rendezvous file (I3)
      const int l = 30 + sizeof(instance_nonce) + strlen(filenames[0]);
      socket_filename = malloc(l);
      if (has_instance_nonce)
        snprintf(socket_filename, l, "\\\\.\\pipe\\webots-%d-%s-%s", number, instance_nonce, filenames[0]);
      else
  ...
  free(ipc_folder);
  free(OMNISIM_CONTROLLER_URL);
  return socket_filename;
}

static void compute_remote_info(char **host, int *port, char **robot_name) {
  const char *OMNISIM_CONTROLLER_URL = getenv_controller_url();
  const char *url_suffix = strstr(&OMNISIM_CONTROLLER_URL[6], ":");

  if (url_suffix == NULL) {  // assuming only the IP address was provided
    fprintf(stderr, "Error: Missing port in OMNISIM_CONTROLLER_URL: %s\n", OMNISIM_CONTROLLER_URL);
    exit(EXIT_FAILURE);
  } else if (OMNISIM_CONTROLLER_URL[6] == ':') {
    fprintf(stderr, "Error: Missing IP address in OMNISIM_CONTROLLER_URL: %s\n", OMNISIM_CONTROLLER_URL);
    exit(EXIT_FAILURE);
  }
  const int host_length = strlen(&OMNISIM_CONTROLLER_URL[6]) - strlen(url_suffix) + 1;
  *host = malloc(host_length);
  snprintf(*host, host_length, "%s", &OMNISIM_CONTROLLER_URL[6]);
  sscanf(url_suffix, ":%d", port);
  const char *rn = strstr(url_suffix, "/");
  *robot_name = rn != NULL ? encode_robot_name(rn + 1, 0) : NULL;
}

// Record a controller-to-simulator connection failure to a durable sink.
//
// When the simulator launches a controller (an "intern" controller) but libController cannot open the
// controller's IPC pipe, the controller blocks in the retry loop below and eventually exits. That failure
// happens BEFORE the OMNISIM_STDOUT/STDERR_REDIRECT streams are installed (those streams run OVER the very
// IPC channel that failed), so the retry message reaches only a live attached console -- never the engine
// log. In a headless or CI run there is no console, so the hang is otherwise completely invisible.
//
// We therefore write the diagnostic ourselves, to a file next to the engine log. We deliberately do NOT
// append to the engine log itself: the simulator holds it as a single writer (WbLog falls back to a
// per-pid file when the path is busy), so a second writer would interleave and corrupt it. Instead we
// write a sibling "<log>.connect_error.txt", resolving the log location the same way the engine does
// (the OMNISIM_LOG_PATH override, else <OMNISIM_HOME>/omnisim_log.txt).
//
// On Windows wbu_system_getenv() returns a pointer into one shared static buffer that the next call
// frees, so every value we need is copied out (strdup / a stack buffer) before the following call.
static void write_connect_failure_diagnostic(const char *socket_filename, const char *reason) {
  char *log_path = NULL;
  const char *e = wbu_system_getenv("OMNISIM_LOG_PATH");
  if (e && e[0])
    log_path = strdup(e);
  if (log_path == NULL) {
    e = wbu_system_getenv("OMNISIM_HOME");  // the only install-root variable this runtime reads
    if (e && e[0]) {
      const int len = strlen(e) + strlen("/omnisim_log.txt") + 1;
      log_path = malloc(len);
      snprintf(log_path, len, "%s/omnisim_log.txt", e);
    }
  }
  if (log_path == NULL)
    return;  // no discoverable durable sink; the stderr message is the best we can do

  const char *suffix = ".connect_error.txt";
  const int path_length = strlen(log_path) + strlen(suffix) + 1;
  char *sidecar = malloc(path_length);
  snprintf(sidecar, path_length, "%s%s", log_path, suffix);
  free(log_path);

  // Tag the line with the simulator run id (OMNISIM_IPC_NONCE == the simulator PID, which the engine also
  // prints in its log header "=== OmniSim Log Started (pid=...) ==="), so a reader can tell this run's
  // entry apart from a stale one: this sibling file is append-only and is not truncated across runs.
  const char *nonce = wbu_system_getenv("OMNISIM_IPC_NONCE");
  char run_id[32];
  snprintf(run_id, sizeof(run_id), "%s", (nonce && nonce[0]) ? nonce : "unknown");

  FILE *file = fopen(sidecar, "a");
  if (file) {
    fprintf(file, "run=%s: %s (%s). Run 'python -m omnisim doctor' to check.\n", run_id, reason,
            socket_filename ? socket_filename : "unknown");
    fclose(file);
  }
  free(sidecar);
}

// the legacy pipe-open failure text, kept verbatim for log-scraping compatibility
static const char *CONNECT_DIAG_PIPE_OPEN =
...
// do not use any buffer for the standard streams
...
#endif
  // flush stdout and stderr to display messages printed before the robot initialization in the OmniSim console
  fflush(stdout);
  fflush(stderr);
  ...
  robot.omnisim_exit = OMNISIM_EXIT_FALSE;
  ...
  int retry = 0;
  // Distinguish an intern controller (one the simulator launched itself, which the engine gives a
  // OMNISIM_ROBOT_NAME + instance path -- see OmController::setProcessEnvironment) from an extern one, and
  // snapshot the verdict as a bool NOW: the strings live in wbu_system_getenv()'s shared static buffer,
  // which the repeated calls in the loop below overwrite. This only refines the intern case (an honest
  // failure message + a durable record); extern controllers legitimately wait here and are untouched.
  const char *intern_name_probe = getenv_robot_name();
  const bool intern_name_set = intern_name_probe && intern_name_probe[0];
  const char *intern_path_probe = wbu_system_webots_instance_path(true);
  const bool controller_is_intern = intern_name_set && intern_path_probe && intern_path_probe[0];
  bool wrote_connect_diag = false;
  while (true) {
    int init_status;
    bool is_simulation_loading;
    const char *OMNISIM_CONTROLLER_URL = getenv_controller_url();
    const char *OMNISIM_ROBOT_NAME = getenv_robot_name();
    const char *OMNISIM_TMP_PATH = wbu_system_webots_instance_path(true);
    char *error_message = malloc(ERROR_BUFFER_SIZE);
    char *socket_filename = NULL;
    if ((OMNISIM_CONTROLLER_URL != NULL) &&
        !(OMNISIM_ROBOT_NAME && OMNISIM_ROBOT_NAME[0] && OMNISIM_TMP_PATH && OMNISIM_TMP_PATH[0]) &&
        strncmp(OMNISIM_CONTROLLER_URL, "tcp://", 6) == 0) {  // TCP URL given and not an intern controller
      char *host, *robot_name;
      int port = -1;
      compute_remote_info(&host, &port, &robot_name);
      init_status = scheduler_init_remote(host, port, robot_name, error_message);
      is_simulation_loading = strncmp(error_message, "The OmniSim simulation world is not yet ready", 44) == 0;
      if (init_status == SCHEDULER_INIT_SUCCESS) {
        free(host);
        free(robot_name);
        free(error_message);
        break;
      ...
      init_status = socket_filename ? scheduler_init_local(socket_filename) : SCHEDULER_INIT_FAILURE;
      is_simulation_loading = strncmp(error_message, "The OmniSim simulation world is not yet ready", 44) == 0;
      if (init_status == SCHEDULER_INIT_SUCCESS) {
    ...
    // A handshake-proven ABI mismatch is permanent for this engine/libController pair: retrying
    ...
    // certainly a libController/simulator build mismatch: the engine creates the intern pipe (listen())
    // before it spawns the controller, so a matched build connects on the first try -- a persistent
    // failure means the two binaries derive the pipe name differently and never meet. Record it ONCE to a
    // durable sink (see write_connect_failure_diagnostic) so the hang is diagnosable in a headless/CI run
    // with no console. The 2-retry guard rules out a one-off startup transient; extern controllers, which
    // legitimately wait here for a user-launched simulator, are excluded.
    if (controller_is_intern && socket_filename && !wrote_connect_diag && retry >= 2 && !is_simulation_loading) {
      write_connect_failure_diagnostic(socket_filename, CONNECT_DIAG_PIPE_OPEN);
      wrote_connect_diag = true;
    }
    if (retry % 5 == 0 && retry != 50) {
      if (is_simulation_loading) {
        ...
        if (controller_is_intern)
          // The engine launched this controller; the legacy "<extern> controllers" wording is misleading
          // here and once sent an investigation down the wrong path. State the likely cause plainly.
          fprintf(stderr,
                  "Cannot open the intern controller's IPC pipe (at %s): the simulator launched this "
                  "controller but libController cannot find its pipe. This almost always means libController "
                  "and the simulator binary are built from different revisions (a build mismatch). Run "
                  "'python -m omnisim doctor' to check. Retrying for another %d seconds...\n",
  ...
  // OMNISIM_STD*_REDIRECT only: the launcher (omnisim_controller.c, --stdout-redirect /
  // --stderr-redirect) writes exactly these names and no longer writes the WEBOTS_* twins.
  if (getenv("OMNISIM_STDOUT_REDIRECT"))
    stdout_read = stream_pipe_create(1);
  if (getenv("OMNISIM_STDERR_REDIRECT"))
