...
// Global variables
static char *omnisim_home;  // resolved OmniSim installation root
...
// OMNISIM_CONTROLLER_URL is the whole contract with libController now: the legacy
// WEBOTS_CONTROLLER_URL twin is no longer exported, and robot.c no longer reads it.
static char *OMNISIM_CONTROLLER_URL;
...
// Gets and stores the path to the OmniSim installation folder from OMNISIM_HOME.
static bool get_omnisim_home() {
  // OMNISIM_HOME is the only install-root variable read; the legacy WEBOTS_HOME spelling was
  // retired. A setup still exporting only WEBOTS_HOME gets told exactly that, rather than the
  // bare "set OMNISIM_HOME" line, which reads as if nothing were configured at all.
  const char *home = getenv("OMNISIM_HOME");
  if (!home || !home[0]) {
    const char *legacy = getenv("WEBOTS_HOME");
    if (legacy && legacy[0])
      fprintf(stderr,
              "WEBOTS_HOME is set but OMNISIM_HOME is not. OmniSim reads only OMNISIM_HOME -- the WEBOTS_HOME name was "
              "retired. Rename it (OMNISIM_HOME=%s) and run this again.\n",
              legacy);
    else
      fprintf(stderr, "Set the path to your OmniSim installation folder in the OMNISIM_HOME environment variable.\n");
    return false;
  }
  omnisim_home = malloc(strlen(home) + 1);
  strcpy(omnisim_home, home);
  return true;
}

// Prints the command line options and their descriptions for the OmniSim controller launcher.
static void print_options() {
  printf(
    "Usage: omnisim-controller [options] [controller_file] [controller_args]\n\nOptions:\n\n  --help\n    Display this help "
    "message and exit.\n\n  --protocol=<ipc|tcp>\n    Define the protocol to use to communicate between the controller and "
    "OmniSim.\n    'ipc' is used by default. 'ipc' should be used when OmniSim is\n    running on the same machine as the extern "
    ...
    "connect.\n    1234 is used by default, as it is the default port for OmniSim.\n    This setting allows you to connect to a "
    "specific instance of OmniSim if\n    there are multiple instances running on the target machine.\n    The port of a OmniSim "
    "instance can be set at its launch.\n\n  --robot-name=<robot-name>\n    Target a specific robot by specifying its name in "
    "case multiple robots wait\n    for an extern controller in the OmniSim instance.\n\n  --stdout-redirect\n    "
    "Redirect the stdout of the controller to the OmniSim console.\n\n  --stderr-redirect\n    Redirect the stderr of the "
    "controller to the OmniSim console.\n\n");
}

// Parses the command line options for the OmniSim controller launcher and sets the controller URL environment variables.
static bool parse_options(int nb_arguments, char **arguments) {
  ...
  for (int i = 1; i < nb_arguments; i++) {
    if (option_arguments && arguments[i][0] == '-') {
      if (strncmp(arguments[i], "--protocol=", 11) == 0) {
        ...
        putenv("OMNISIM_STDOUT_REDIRECT=1");
      } else if (strncmp(arguments[i], "--stderr-redirect", 17) == 0) {
        putenv("OMNISIM_STDERR_REDIRECT=1");
  ...
  // Write OMNISIM_CONTROLLER_URL in function of given options.
  // This is the only name exported: libController (robot.c) reads exactly it, and the legacy
  // WEBOTS_CONTROLLER_URL twin was retired on both sides at once.
  const char *robot_separator = robot_name ? "/" : "";
  const char *robot_name_string = robot_name ? robot_name : "";
  if (strncmp(protocol, "tcp", 3) == 0) {
    if (!ip_address) {
      fprintf(stderr, "Specify the IP address of the OmniSim machine to connect to with '--ip-address=' option.\n");
      return false;
    }
    const size_t OMNISIM_CONTROLLER_URL_size = snprintf(NULL, 0, "OMNISIM_CONTROLLER_URL=%s://%s:%s%s%s", protocol, ip_address,
                                                        port, robot_separator, robot_name_string) +
                                               1;
    OMNISIM_CONTROLLER_URL = malloc(OMNISIM_CONTROLLER_URL_size);
    sprintf(OMNISIM_CONTROLLER_URL, "OMNISIM_CONTROLLER_URL=%s://%s:%s%s%s", protocol, ip_address, port, robot_separator,
            robot_name_string);
  } else if (strncmp(protocol, "ipc", 3) == 0) {
    if (ip_address)
      fprintf(stderr, "Skipping IP address for ipc protocol.\n");

    const size_t OMNISIM_CONTROLLER_URL_size =
      snprintf(NULL, 0, "OMNISIM_CONTROLLER_URL=%s://%s%s%s", protocol, port, robot_separator, robot_name_string) + 1;
    OMNISIM_CONTROLLER_URL = malloc(OMNISIM_CONTROLLER_URL_size);
    sprintf(OMNISIM_CONTROLLER_URL, "OMNISIM_CONTROLLER_URL=%s://%s%s%s", protocol, port, robot_separator, robot_name_string);
  } else {
    fprintf(stderr, "Only ipc and tcp protocols are supported.\n");
    return false;
  }
  putenv(OMNISIM_CONTROLLER_URL);

  // Show resulting target options to user
  const char *location = strncmp(protocol, "tcp", 3) == 0 ? "remote" : "local";
  printf("\nThe started controller targets a %s instance (%s protocol) of OmniSim with port number %s.", location, protocol,
         port);
  strncmp(protocol, "tcp", 3) == 0 ? printf(" The IP address of the remote OmniSim instance is '%s'.\n", ip_address) :
                                     printf("\n");
...
- For Windows it adds "<omnisim_home>\lib\controller;" to the Path environment variable.
- For Linux, it adds "<omnisim_home>/lib/controller:" to LD_LIBRARY_PATH.
- For macOS it adds "<omnisim_home>/Contents/lib/controller:" to DYLD_LIBRARY_PATH.
*/
static void add_lib_controller_to_path() {
  ...
  const size_t new_path_size =
    snprintf(NULL, 0, "%s=%s%s%s", path_env_variable, omnisim_home, lib_controller, getenv(path_env_variable)) + 1;
  new_path = malloc(new_path_size);
  sprintf(new_path, "%s=%s%s%s", path_env_variable, omnisim_home, lib_controller, getenv(path_env_variable));
  putenv(new_path);
}

/*
This function sets the environment variable for executable controllers (native or compiled).

On Windows, the function adds a new path to the "Path" environment variable by concatenating the OmniSim installation root, the
...
*/
static void exec_config_environment() {
#ifdef _WIN32
  const size_t new_windows_path_size = snprintf(NULL, 0, "Path=%s\\msys64\\mingw64\\bin;%s\\msys64\\mingw64\\bin\\cpp;%s",
                                                omnisim_home, omnisim_home, getenv("Path")) +
                                       1;
  new_windows_path = malloc(new_windows_path_size);
  sprintf(new_windows_path, "Path=%s\\msys64\\mingw64\\bin;%s\\msys64\\mingw64\\bin\\cpp;%s", omnisim_home, omnisim_home,
          getenv("Path"));
...
static void python_config_environment() {
  ...
  const size_t new_python_path_size =
    snprintf(NULL, 0, "PYTHONPATH=%s%s%s", omnisim_home, python_lib_controller, getenv("PYTHONPATH")) + 1;
  new_python_path = malloc(new_python_path_size);
  sprintf(new_python_path, "PYTHONPATH=%s%s%s", omnisim_home, python_lib_controller, getenv("PYTHONPATH"));
...
#ifdef _WIN32
  const size_t new_path_size = snprintf(NULL, 0, "Path=%s\\msys64\\mingw64\\bin\\cpp;%s", omnisim_home, getenv("Path")) + 1;
  new_path = malloc(new_path_size);
  sprintf(new_path, "Path=%s\\msys64\\mingw64\\bin\\cpp;%s", omnisim_home, getenv("Path"));
  putenv(new_path);
#elif defined __APPLE__
  const size_t new_path_size =
    snprintf(NULL, 0, "DYLD_LIBRARY_PATH=%s/Contents/lib/controller:%s", omnisim_home, getenv("DYLD_LIBRARY_PATH")) + 1;
  new_path = malloc(new_path_size);
  sprintf(new_path, "DYLD_LIBRARY_PATH=%s/Contents/lib/controller:%s", omnisim_home, getenv("DYLD_LIBRARY_PATH"));
  putenv(new_path);
#endif
}
    ...
    if (strncmp(runtime_ini_line, "[", 1) == 0) {
      if (strncmp(runtime_ini_line, "[environmentvariableswithpaths]", 31) == 0)
        ...
        fprintf(stderr,
                ...
                "https://github.com/omnilink-tech/omnisim/blob/main/docs/guide/controller-programming.md "
...
int main(int argc, char **argv) {
  // Check the OmniSim installation root and exit if empty
  if (!get_omnisim_home())
    exit(1);
  ...
  // Python controller
    ...
    fprintf(stderr,
            "The file extension '%s' is not supported by OmniSim. Supported file types are executable files and '.py'.\n",
