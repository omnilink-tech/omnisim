...
#ifndef OM_VIDEO_RECORDER_HPP
#define OM_VIDEO_RECORDER_HPP

//
// Description: class managing the make movie functionality
//
// LEGACY: GUI-driven movie recording inherited from upstream Webots. For new
// automation prefer the OmniSim HTTP capture service in scripts/capture/ —
// it drives deterministic shot lists and produces lossless PNG → ffmpeg
// renders independent of the GUI viewport. Kept for the desktop "Make Movie"
// workflow and existing call sites.
...
class OmVideoRecorder : public QObject {
  Q_OBJECT
public:
  // singleton: avoid multiple simultaneous recordings
  static OmVideoRecorder *instance();

  static void setMainWindow(OmMainWindow *mainWindow) { cMainWindow = mainWindow; }
  ...
  bool initRecording(OmSimulationView *view, double basicTimeStep);

  // initialize recording parameters
  // quality between 1 and 100,
  // winCompression: compression used in windows AVI creation
  bool initRecording(OmSimulationView *view, double basicTimeStep, const QSize &videoResolution, int quality, int codec = 0,
...
private:
  static OmVideoRecorder *cInstance;
  ...
  // P10 (WREN retirement): wgpu-arm frame sink. Takes the already-upright QImage returned by
  // OmView3D::grabWindowBufferNow() (deep-copied on this thread before the writer thread starts).
  void writeSnapshotImage(const QImage &frame);
