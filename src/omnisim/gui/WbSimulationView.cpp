...
WbSimulationView::WbSimulationView(QWidget *parent, const QString &toolBarAlign) :
  ...
  // R4 step-3b (opt-in via OMNISIM_VIEW3D_WGPU): embed a wgpu-rendered viewport
  // as a side-by-side pane next to the WREN view, so the two renders of the same
  // world + camera can be compared in-app. Default-off => layout byte-unchanged;
  // meaningful only on a wgpu-ON build. See docs/developer/rendering-arm-checklist.md (R4).
  if (qEnvironmentVariableIsSet("OMNISIM_VIEW3D_WGPU")) {
    mWgpuView = new WbWgpuView();
