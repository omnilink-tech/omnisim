...
bool OmNewVersionDialog::run() {
  OmNewVersionDialog dialog;
  return dialog.exec();
}

OmNewVersionDialog::OmNewVersionDialog() {
  style()->polish(this);
  QDir::addSearchPath("newVersionIconPath", OmStandardPaths::resourcesPath() + newVersionIconPath());
  style()->polish(this);

  const OmVersion &version = OmApplicationInfo::version();
  const QString &omniSimVersion = OmApplicationInfo::omniSimVersion();
  ...
  for (int i = 0; i < NUMBER_OF_THEMES; ++i)
    connect(mRadioButtons[i], &QRadioButton::toggled, this, &OmNewVersionDialog::updatePreview);
  ...
  connect(startButton, &QPushButton::clicked, this, &OmNewVersionDialog::startButtonPressed);
  updatePreview();
}

void OmNewVersionDialog::updatePreview() {
  for (int i = 0; i < NUMBER_OF_THEMES; ++i) {
    if (mRadioButtons[i]->isChecked()) {
      mPreviewLabel->setPixmap(
        QPixmap(OmStandardPaths::resourcesPath() + "images/themes/" + mRadioButtons[i]->objectName() + ".png"));
      break;
    }
  }
}

void OmNewVersionDialog::startButtonPressed() {
  QString theme;
  for (int i = 0; i < NUMBER_OF_THEMES; ++i) {
    if (mRadioButtons[i]->isChecked()) {
  ...
  OmPreferences::instance()->setValue("General/theme", theme);
  // force the sync in case we restart just after quitting the dialog (see #7662)
  OmPreferences::instance()->sync();
