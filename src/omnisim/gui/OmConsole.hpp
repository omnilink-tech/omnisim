...
#ifndef OM_CONSOLE_HPP
#define OM_CONSOLE_HPP

//
// Description: OmniSim console widget
//
...
class QAction;
class OmFindReplaceDialog;
class OmTextFind;
class OmSyntaxHighlighter;
...
private:
  OmSyntaxHighlighter *mSyntaxHighlighter;
...
class OmConsole : public OmDockWidget {
  Q_OBJECT
  // OmLog colors
...
public:
  explicit OmConsole(QWidget *parent = NULL, const QString &name = QString("Console"));
  virtual ~OmConsole() override {}
...
public slots:
  ...
  // append internal error message of OmniSim
  // the message color depends on the level
  void appendLog(OmLog::Level level, const QString &message, bool popup, const QString &logName);
...
private:
  ...
  void resetFormat();
  QString htmlSpan(const QString &s, OmLog::Level level) const;
  void handleCRAndLF(const QString &msg);
  void handlePossibleAnsiEscapeSequences(const QString &msg, OmLog::Level);
  QRegularExpression **createErrorMatchingPatterns() const;
  void updateTitle();

  void openFindDialog();
  OmFindReplaceDialog *mFindDialog;
