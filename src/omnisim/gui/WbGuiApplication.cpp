...
WbGuiApplication::WbGuiApplication(int &argc, char **argv) :
  ...
  setApplicationVersion(WbApplicationInfo::omniSimVersion());
...
bool WbGuiApplication::setup() {
  ...
  mMainWindow = new WbMainWindow(mShouldMinimize, mTcpServer, NULL, mShouldRunBackground);
#endif
  ...
  if (WbSysInfo::isRootUser() && WbSysInfo::environmentVariable("CI").isEmpty())
    WbLog::warning("It is not recommended to run OmniSim as root.");
