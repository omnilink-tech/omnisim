...
#ifndef OM_SIMULATION_VIEW_HPP
#define OM_SIMULATION_VIEW_HPP

//
// Description: central widget of OmMainWindow, contains all the actions for managing the simulation state
//

#include <QtWidgets/QWidget>

class OmDockTitleBar;
class OmRobot;
class OmSceneTree;
class OmSelection;
...
class OmSimulationView : public QWidget {
  Q_OBJECT
  Q_PROPERTY(int handleWidth MEMBER mHandleWidth READ handleWidth WRITE setHandleWidth)

public:
  OmSimulationView(QWidget *parent, const QString &toolBarAlign);
  virtual ~OmSimulationView();

  void prepareWorldLoading();
  void setWorld(OmSimulationWorld *w);

  OmView3D *view3D() const { return mView3D; }
  const OmSceneTree *sceneTree() const { return mSceneTree; }

  const OmSelection *selection() const { return mSelection; }
  OmRobot *selectedRobot() const;
...
private:
  ...
  OmSelection *mSelection;

  OmDockTitleBar *mTitleBar;
  QSplitter *mSplitter;
  OmSceneTree *mSceneTree;
  OmView3D *mView3D;
  QWidget *mView3DContainer;
  OmWgpuView *mWgpuView = nullptr;  // R4 step-3b: embedded wgpu pane (OMNISIM_VIEW3D_WGPU)
