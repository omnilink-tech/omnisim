...
OmNewProjectWizard::OmNewProjectWizard(QWidget *parent) : OmNewWorldWizard(parent) {
  ...
  mProject = new OmProject(path);
}

OmNewProjectWizard::~OmNewProjectWizard() {
}

QString OmNewProjectWizard::proposeNewProjectPath() const {
  QString path;
  // if current project is in OmniSim installation dir
  if (OmProject::current()->isReadOnly()) {  // propose a new project in user's home dir
    path = OmPreferences::instance()->value("Directories/projects").toString();
    if (!path.isEmpty()) {
      if (OmFileUtil::isDirectoryWritable(path))
        path += "my_project";
      else
        path = "";  // no valid default path found
    }
  } else {  // otherwise propose new project dir as sibling of current project
    QDir dir(OmProject::current()->path());
    if (!dir.cdUp())
      assert(false);
...
void OmNewProjectWizard::accept() {
  if (!mProject->createNewProjectFolders()) {
    OmMessageBox::warning(tr("Some directories could not be created."), this, tr("Directories creation failed"));
    mWorldEdit->setText("");
    QDialog::accept();
    return;
  }
  OmProject::setCurrent(mProject);
  createWorldFile();
...
void OmNewProjectWizard::updateUI() {
  updateWorldUI();
  mProject->setPath(mDirEdit->text());
  mFilesLabel->setText(
    QDir::toNativeSeparators(mProject->newProjectFiles().join("\n").replace(
      "empty" + OmWorldFileFormat::writeExtension(), mWorldEdit->text())));
}

bool OmNewProjectWizard::validateCurrentPage() {
  ...
  if (currentId() == directoryId()) {
    if (mDirEdit->text().isEmpty())
      return false;
    if (OmFileUtil::isLocatedInInstallationDirectory(mDirEdit->text())) {
      OmMessageBox::warning(tr("It is not allowed to create a new project inside the OmniSim installation directory.") + "\n" +
...
void OmNewProjectWizard::chooseDirectory() {
