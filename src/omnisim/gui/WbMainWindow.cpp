...
WbMainWindow::WbMainWindow(bool minimizedOnStart, WbTcpServer *tcpServer, QWidget *parent, bool runBackground) :
  QMainWindow(parent),
  mExitStatus(0),
  ...
  QAction *action = actionManager->action(WbAction::SHOW_ROBOT_WINDOW);
...
void WbMainWindow::createMainTools() {
  ...
  if (WbAgentHud::isEnabled()) {
    mAgentHud = new WbAgentHud(this);
...
QMenu *WbMainWindow::createHelpMenu() {
  ...
  action->setStatusTip(tr("Open the OmniSim update dialog."));
...
void WbMainWindow::showOnlineDocumentation(const QString &book, const QString &page) {
  ...
  const QString url = WbStandardPaths::omniSimDocsBaseUrl() + "/" + book + "/" + page + ".md";
  showDocument(url);
}
...
void WbMainWindow::setTheme(const QString &qssFile) {
  WbPreferences *prefs = WbPreferences::instance();
  if (prefs->value("General/theme").toString() == qssFile)
    return;
  prefs->setValue("General/theme", qssFile);
  prefs->sync();
  const bool restart = WbMessageBox::question(
...
void WbMainWindow::openWebotsUpdateDialogFromStartup() {
  ...
  // Compare the local OmniSim release label (e.g. "1.0.9") against the latest
  // GitHub release tag (e.g. "v1.0.10"). The Webots-format version() — "R2025a"
  // — would always compare as 2025 > 1.x.y and the dialog would never show.
  WbVersion currentVersion;
  if (!currentVersion.fromString(WbApplicationInfo::omniSimVersion()))
    return;
  if (currentVersion < webotsUpdateManager->targetVersion()) {
...
void WbMainWindow::updateWindowTitle() {
  const QString nameAndVersion("OmniSim " + WbApplicationInfo::omniSimVersion());
  QString title;
