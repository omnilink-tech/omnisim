...
public:
  ...
  // Public render trigger for verification harnesses: the wgpu parity self-check forces a fresh
  // WREN frame before grabbing it as the local golden — otherwise the grab can race the first
  // post-load repaint and capture the "Loading world" overlay (a black golden → spurious 0% gate).
  void renderImmediately(bool culling = true) { renderNow(culling); }

  // Deterministic FIXED-RESOLUTION offscreen render of the scene → QImage (RGB32), independent of
  // the window/layout: rebuilds the main framebuffer at w×h, renders offscreen, reads it back, then
  // restores the window-sized buffer. The wgpu parity self-check's golden — window grabs proved
  // layout-sensitive (pane bands, viewport/window mismatches swung within-tol 62%↔19% on identical
  // renderer code).
  QImage grabSceneOffscreen(int w, int h);
...
private:
  ...
  void recreateMainFrameBuffer(int w, int h);  // updateFrameBuffer's body, parametrized (grabSceneOffscreen)
