...
WbSplashScreen::WbSplashScreen() {
  ...
  const QString orbPath = WbStandardPaths::resourcesPath() + "branding/omnilink/orb/orb.png";
...
void WbSplashScreen::drawContents(QPainter *painter) {
  ...
  //    WbGuiApplication::addApplicationFont). Qt falls back gracefully if absent.
  {
  ...
  // 9. Version, bottom-right.
  {
    ...
    painter->drawText(versionRect, Qt::AlignRight | Qt::AlignVCenter, WbApplicationInfo::omniSimVersion());
  }
