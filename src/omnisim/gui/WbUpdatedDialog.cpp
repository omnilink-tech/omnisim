...
WbUpdatedDialog::WbUpdatedDialog(QWidget *parent) : QDialog(parent) {
  setWindowTitle(tr("Welcome to OmniSim %1").arg(WbApplicationInfo::omniSimVersion()));
  ...
  label->setText(tr("<b>Thank you for using OmniSim %1.</b>").arg(WbApplicationInfo::omniSimVersion()));
  ...
  label->setText(tr("Find out the new features, enhancements and bug fixes of OmniSim %1 in the "
                   ...
                   .arg(WbApplicationInfo::omniSimVersion()));
  label->setOpenExternalLinks(true);
  label->setAlignment(Qt::AlignLeading | Qt::AlignLeft | Qt::AlignTop);
  label->setWordWrap(true);
