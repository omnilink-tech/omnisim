...
namespace {
  ...
  bool directoryContainsWbtFile(QDir dir, const QString &wildcard) {
    if (dir.dirName() == "worlds") {
      ...
      dir.setNameFilters(OmWorldFileFormat::nameFilters());
      QStringList fileList = dir.entryList();
      const QDir projectsDir = QDir(OmStandardPaths::projectsPath());
...
OmOpenSampleWorldDialog::OmOpenSampleWorldDialog(QWidget *parent) : QDialog(parent) {
  setWindowTitle(tr("Open Sample World"));

  mFindLineEdit = new QLineEdit(this);
  mFindLineEdit->setClearButtonEnabled(true);
  connect(mFindLineEdit, &QLineEdit::textChanged, this, &OmOpenSampleWorldDialog::updateTree);
  ...
  mTreeView->setFocus();
  connect(mTreeView, &QTreeView::doubleClicked, this, &OmOpenSampleWorldDialog::handleDoubleClick);
  connect(mTreeView->selectionModel(), &QItemSelectionModel::selectionChanged, this,
          &OmOpenSampleWorldDialog::handleSelectedItemChanged);

  mButtonBox = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, Qt::Horizontal, this);
  mButtonBox->button(QDialogButtonBox::Ok)->setEnabled(false);
  connect(mButtonBox, &QDialogButtonBox::accepted, this, &OmOpenSampleWorldDialog::accept);
...
OmOpenSampleWorldDialog::~OmOpenSampleWorldDialog() {
}

QSize OmOpenSampleWorldDialog::sizeHint() const {
  return QSize(350, 500);
}

void OmOpenSampleWorldDialog::accept() {
...
void OmOpenSampleWorldDialog::handleDoubleClick(const QModelIndex &index) {
...
void OmOpenSampleWorldDialog::handleSelectedItemChanged(const QItemSelection &selected, const QItemSelection &deselected) {
...
void OmOpenSampleWorldDialog::updateTree(const QString &reg) {
  mModel->clear();
  mButtonBox->button(QDialogButtonBox::Ok)->setEnabled(false);
  ...
  int itemCounter = 0;
  populateTreeModel(itemCounter, parentItem, QDir(OmStandardPaths::projectsPath()), mFindLineEdit->text());
