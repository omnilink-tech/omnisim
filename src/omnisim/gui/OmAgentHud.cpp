...
OmAgentHud::OmAgentHud(QWidget *parent) :
  OmDockWidget(parent),
  mUrl(defaultUrl()),
  ...
  connect(btnSmaller, &QToolButton::clicked, this, &OmAgentHud::decreaseFont);
  ...
  connect(btnReset, &QToolButton::clicked, this, &OmAgentHud::resetFont);
  ...
  connect(btnBigger, &QToolButton::clicked, this, &OmAgentHud::increaseFont);
  ...
  connect(mChatSend, &QPushButton::clicked, this, &OmAgentHud::sendPrompt);
  ...
  // are scoped to this widget so they don't fight with OmniSim' global
  ...
  connect(scIncrease, &QShortcut::activated, this, &OmAgentHud::increaseFont);
  ...
  connect(scIncreasePlus, &QShortcut::activated, this, &OmAgentHud::increaseFont);
  ...
  connect(scDecrease, &QShortcut::activated, this, &OmAgentHud::decreaseFont);

  QShortcut *scReset = new QShortcut(QKeySequence(QStringLiteral("Ctrl+0")), this);
  scReset->setContext(Qt::WidgetWithChildrenShortcut);
  connect(scReset, &QShortcut::activated, this, &OmAgentHud::resetFont);

  mNet = new QNetworkAccessManager(this);
  connect(mNet, &QNetworkAccessManager::finished, this, &OmAgentHud::onReply);
  ...
  connect(mChatNet, &QNetworkAccessManager::finished, this, &OmAgentHud::onPromptReply);

  mTimer = new QTimer(this);
  mTimer->setInterval(mRefreshMs);
  connect(mTimer, &QTimer::timeout, this, &OmAgentHud::poll);
  mTimer->start();
  ...
  QTimer::singleShot(50, this, &OmAgentHud::poll);
}

void OmAgentHud::increaseFont() {
...
void OmAgentHud::appendChatLine(const QString &kind, const QString &text) {
  if (!mChatLog)
    return;
...
void OmAgentHud::onPromptReply(QNetworkReply *reply) {
  if (!reply)
    return;
  reply->deleteLater();
  if (mChatSend)
  ...
  if (reply->error() != QNetworkReply::NoError) {
    ...
    if (err.contains("refused", Qt::CaseInsensitive))
      appendChatLine("error",
                     tr("Bridge not reachable at %1. Is an omnilink_<robot>.omniworld demo "
...
void OmAgentHud::persistFontSize() const {
  QSettings settings;
  settings.beginGroup(QString::fromLatin1(kSettingsGroup));
...
OmAgentHud::~OmAgentHud() {
}

void OmAgentHud::poll() {
...
void OmAgentHud::onReply(QNetworkReply *reply) {
  reply->deleteLater();
  if (reply->error() != QNetworkReply::NoError) {
    ...
    const QString detail = defaultUrl
      ? tr("HUD is polling %1 every %2 ms. Open one of the "
           "<code>omnilink_&lt;robot&gt;.omniworld</code> demo worlds and this panel "
...
void OmAgentHud::renderError(const QString &shortReason, const QString &detail) {
...
void OmAgentHud::renderStatus(const QByteArray &json) {
