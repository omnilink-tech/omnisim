...
OmMultimediaStreamingLimiter::OmMultimediaStreamingLimiter(const QSize &resolution, int updateTimeStep) :
...
void OmMultimediaStreamingLimiter::resetResolution(const QSize &newSize) {
  mResolution = newSize;
  for (int i = mResolutionFactor; i > 1; i--)
    mResolution /= 2;
}

QSize OmMultimediaStreamingLimiter::fullResolution() const {
...
void OmMultimediaStreamingLimiter::recomputeStreamingLimits(int skippedImages) {
  mIsStopped = false;
  mResolutionChanged = false;
  ...
  // OmLog::info(QString("Streaming status: resolution: %1x%2, rate %3")
