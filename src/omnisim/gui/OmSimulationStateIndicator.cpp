...
OmSimulationStateIndicator::OmSimulationStateIndicator(QWidget *parent) : QFrame(parent) {
  ...
  connect(OmGuiRefreshOracle::instance(), &OmGuiRefreshOracle::canRefreshUpdated, this, &OmSimulationStateIndicator::update);
  connect(OmApplication::instance(), &OmApplication::postWorldLoaded, this, &OmSimulationStateIndicator::reset);

  mLastSpeedIndicatorTime = OmSimulationState::instance()->time();
...
OmSimulationStateIndicator::~OmSimulationStateIndicator() {
}

void OmSimulationStateIndicator::update() {
  ...
  if (OmGuiRefreshOracle::instance()->canRefreshNow()) {
    ...
    if ((OmSimulationState::instance()->isPaused() &&
         OmSimulationState::instance()->previousMode() == OmSimulationState::STEP) ||
        OmSimulationState::instance()->isStep())
      str = "N/A";
    else if (time < mLastSpeedIndicatorTime)
      str = "0.00x";
    else {
      // update speed indicator
      int elapsed = OmGuiRefreshOracle::instance()->elapsed();
...
void OmSimulationStateIndicator::reset() {
  mTimeLabel->setText("0:00:00:000");
  mSpeedLabel->setText("0.00x");
