...
OmPreferencesDialog::OmPreferencesDialog(QWidget *parent, const QString &defaultTab) : QDialog(parent) {
  ...
  connect(mButtonBox, &QDialogButtonBox::accepted, this, &OmPreferencesDialog::accept);
  connect(mButtonBox, &QDialogButtonBox::rejected, this, &OmPreferencesDialog::reject);
  ...
  connect(this, &OmPreferencesDialog::changedByUser, prefs, &OmPreferences::changedByUser);
...
OmPreferencesDialog::~OmPreferencesDialog() {
}

void OmPreferencesDialog::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_Enter || event->key() == Qt::Key_Return)
    return;
  QDialog::keyPressEvent(event);
}

void OmPreferencesDialog::accept() {
  OmPreferences *prefs = OmPreferences::instance();

  // Inform when restarting OmniSim is required
  bool willRestart = false;
  if (prefs->value("General/theme").toString() != mValidThemeFilenames.at(mThemeCombo->currentIndex()) ||
    ...
    willRestart = OmMessageBox::question(
                    tr("You have changed some settings which require OmniSim to be restarted. Restart OmniSim Now?"), this,
  ...
  if (!willRestart && prefs->value("OpenGL/textureQuality", 4).toInt() != mTextureQualityCombo->currentIndex())
    OmMessageBox::info(tr("The new texture quality will be applied next time the world is loaded."), this);
...
void OmPreferencesDialog::openFontDialog() {
  QFont initial;
...
void OmPreferencesDialog::clearCache() {
  OmNetwork::instance()->clearCache();
  OmMessageBox::info(tr("The cache has been cleared."), this);
  mCacheSizeLabel->setText(tr("Amount of cache used: %1 MB.").arg(OmNetwork::instance()->cacheSize() / (1024 * 1024)));
}

void OmPreferencesDialog::addNewIp() {
  bool ok;
  const QString text =
...
QWidget *OmPreferencesDialog::createGeneralTab() {
  ...
  for (int i = 0; i < mValidThemeFilenames.size(); ++i) {
    if (OmPreferences::instance()->value("General/theme").toString() == mValidThemeFilenames.at(i)) {
  ...
  mEditorFontEdit = new OmLineEdit(this);
  mExtraProjectPath = new OmLineEdit(this);
  ...
  // row 0
  layout->addWidget(new QLabel(tr("Theme:"), this), 0, 0);
  layout->addWidget(mThemeCombo, 0, 1);
  layout->addWidget(new QLabel(tr("(restart needed)"), this), 0, 2);

  // row 1
  layout->addWidget(new QLabel(tr("Startup mode:"), this), 1, 0);
  layout->addWidget(mStartupModeCombo, 1, 1);

  QPushButton *chooseFontButton = new QPushButton("...", this);
  connect(chooseFontButton, &QPushButton::pressed, this, &OmPreferencesDialog::openFontDialog);

  mRenderingCheckBox = new QCheckBox(tr("Rendering"), this);
  layout->addWidget(mRenderingCheckBox, 1, 2);

  // row 2
  layout->addWidget(new QLabel(tr("Editor font:"), this), 2, 0);
  layout->addWidget(mEditorFontEdit, 2, 1);
  layout->addWidget(chooseFontButton, 2, 2);

  // row 3
  layout->addWidget(new QLabel(tr("Number of threads:"), this), 3, 0);
  layout->addWidget(mNumberOfThreadsCombo, 3, 1);

  // row 4
  mBrowserProgram = new OmLineEdit(this);
  mBrowserProgram->setObjectName("defaultBrowser");
  mBrowserProgram->setText(OmPreferences::instance()->value("RobotWindow/browser").toString());
  if (OmPreferences::instance()->value("General/theme").toString() != "omnisim_classic.qss") {
  ...
  layout->addWidget(new QLabel(tr("Default robot window web browser:"), this), 4, 0);
  layout->addWidget(mBrowserProgram, 4, 1);
#ifdef __linux__
  ...
  layout->addWidget(mNewBrowserWindow, 5, 1);

  // row 6
  layout->addWidget(new QLabel(tr("Python command:"), this), 6, 0);
  if (OmSysInfo::isSnap()) {
    QLabel *label = new QLabel(
      tr("built-in python (snap), see <a href=\"https://github.com/omnilink-tech/omnisim/blob/main/docs/guide/"
         "running-extern-robot-controllers.md\">extern controllers</a> for alternatives."),
      this);
    layout->addWidget(label, 6, 1);
    connect(label, &QLabel::linkActivated, &OmDesktopServices::openUrl);
    mPythonCommand = NULL;
  } else
    layout->addWidget(mPythonCommand = new OmLineEdit(this), 6, 1);

  // row 7
  layout->addWidget(new QLabel(tr("Extra project path:"), this), 7, 0);
  layout->addWidget(mExtraProjectPath, 7, 1);
  ...
  mDisableSaveWarningCheckBox->setToolTip(
    tr("If this option is enabled, OmniSim will not display any warning when you quit, reload\nor load a new world after the "
  ...
  mThumnailCheckBox->setToolTip(tr("If this option is enabled, OmniSim will take and store a 768px by 432px (16:9)\nscreenshot "
                                   "of the world in .jpg format when the it is saved, shared or exported."));
  layout->addWidget(new QLabel(tr("Thumbnail:"), this), 10, 0);
  layout->addWidget(mThumnailCheckBox, 10, 1);
...
QWidget *OmPreferencesDialog::createOpenGLTab() {
  QWidget *widget = new QWidget(this);
  widget->setObjectName("preferenceWidget");
  QGridLayout *layout = new QGridLayout(widget);
...
QWidget *OmPreferencesDialog::createNetworkTab() {
  QWidget *widget = new QWidget(this);
  widget->setObjectName("preferenceWidget");
  ...
  // row 0
  mUploadUrl = new OmLineEdit(this);
  mUploadUrl->setText(OmPreferences::instance()->value("Network/uploadUrl").toString());
  ...
  mCacheSize->setValue(OmPreferences::instance()->value("Network/cacheSize", 1024).toInt());
  connect(mCacheSize, &QSpinBox::editingFinished, [=]() {
    if (mCacheSize->value() < 512) {
      mCacheSize->setValue(512);
      OmMessageBox::info(tr("At least 512 MB of cache are necessary."), this);
    }
  });
  ...
  connect(clearCacheButton, &QPushButton::pressed, this, &OmPreferencesDialog::clearCache);
  mCacheSizeLabel =
    new QLabel(tr("Amount of cache used: %1 MB.").arg(OmNetwork::instance()->cacheSize() / (1024 * 1024)), this);
  ...
  connect(addIpButton, &QPushButton::pressed, this, &OmPreferencesDialog::addNewIp);
  QPushButton *removeIpButton = new QPushButton(QString("-"), this);
  connect(removeIpButton, &QPushButton::pressed, this, &OmPreferencesDialog::removeSelectedIp);
...
QString OmPreferencesDialog::retrieveThemeName(const QString &filename) const {
  QFile inputFile(OmStandardPaths::resourcesPath() + filename);
