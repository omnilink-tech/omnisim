...
bool WbNewProtoWizard::generateProto() {
  ...
  if (mBaseNode != "") {
    if (mIsProtoNode) {
      ...
      externPath = QString("EXTERNPROTO \"%1\"\n").arg(url.replace(WbStandardPaths::webotsHomePath(), "omnisim://"));
      // define interface
          ...
          foreach (const QString &node, parser.protoNodeList()) {
            ...
            if (!prefix.isEmpty()) {
              if (!WbUrl::isWeb(nestedUrl)) {
                if (WbUrl::isLocalUrl(nestedUrl))  // replace the prefix (omnisim://) based on the parent's prefix
            ...
            const QString declaration =
              QString("EXTERNPROTO \"%1\"\n").arg(nestedUrl.replace(WbStandardPaths::webotsHomePath(), "omnisim://"));
      ...
      while (it.hasNext()) {
        ...
        if (match.hasMatch()) {
          ...
          if (!WbUrl::isWeb(asset) && QDir::isRelativePath(asset)) {
            ...
            interface.replace(QString("\"%1\"").arg(asset), newUrl.replace(WbStandardPaths::webotsHomePath(), "omnisim://"));
          }
        }
      }
...
void WbNewProtoWizard::updateNodeTree() {
  ...
  QTreeWidgetItem *const webotsProtosItem =
    new QTreeWidgetItem(QStringList(tr("PROTO nodes (OmniSim Projects)")), WbProtoManager::PROTO_WEBOTS);
