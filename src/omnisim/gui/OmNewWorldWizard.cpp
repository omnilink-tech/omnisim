...
OmNewWorldWizard::OmNewWorldWizard(QWidget *parent) : QWizard(parent) {
  setOption(QWizard::NoCancelButton, false);
  setOption(QWizard::CancelButtonOnLeft, true);
}

OmNewWorldWizard::~OmNewWorldWizard() {
}

int OmNewWorldWizard::exec() {
...
void OmNewWorldWizard::createWorldFile() {
  QFile file(OmProject::current()->worldsPath() + fileName());
  if (!file.open(QIODevice::WriteOnly)) {
    OmLog::error(tr("Could not create world file: '%1'.").arg(file.fileName()));
    return;
  }
  QByteArray worldContent;
  worldContent.append(QString("#OMNISIM %1 utf8\n").arg(OmApplicationInfo::version().toString(false)).toUtf8());
  ...
  foreach (const QString &protoModel, externProtoList) {
    const QString &modelPath = OmProtoManager::instance()->protoUrl(protoModel, OmProtoManager::PROTO_OMNISIM);
...
void OmNewWorldWizard::accept() {
  createWorldFile();
  QDialog::accept();
}

void OmNewWorldWizard::updateWorldUI() {
  if (!mWorldEdit->text().isEmpty() && !OmWorldFileFormat::isWorldFile(mWorldEdit->text()))
    mWorldEdit->setText(mWorldEdit->text().append(OmWorldFileFormat::writeExtension()));
}

void OmNewWorldWizard::updateUI() {
  updateWorldUI();
  mFilesLabel->setText(QDir::toNativeSeparators(OmProject::current()->worldsPath() + mWorldEdit->text()));
}

bool OmNewWorldWizard::validateWorldPage() {
  assert(currentId() == worldId());
  if (mWorldEdit->text().isEmpty()) {
    OmMessageBox::warning(tr("Please specify a world name."), this, tr("Invalid new world name"));
    return false;
  }
  if (QFileInfo(mWorldEdit->text()).dir() != QDir()) {
    // mWorldEdit->text() should not contain any directory
    OmMessageBox::warning(tr("Please specify a world name and not a path."), this, tr("Invalid new world name"));
    return false;
  }
  return true;
}

bool OmNewWorldWizard::validateCurrentPage() {
  if (currentId() == worldId()) {
    if (!validateWorldPage())
      return false;
    QString path = OmProject::current()->worldsPath() + mWorldEdit->text();
    if (!OmWorldFileFormat::isWorldFile(path))
      path += OmWorldFileFormat::writeExtension();
    if (QFile::exists(path)) {
      OmMessageBox::warning(tr("A world file with this name already exists, please choose a different name."), this,
...
QString OmNewWorldWizard::fileName() const {
  return mWorldEdit->text();
}

QWizardPage *OmNewWorldWizard::createIntroPage() {
...
QWizardPage *OmNewWorldWizard::createWorldPage() {
  QWizardPage *page = new QWizardPage(this);
  ...
  mWorldEdit = new OmLineEdit(page);
...
QWizardPage *OmNewWorldWizard::createConclusionPage() {
  QWizardPage *page = new QWizardPage(this);
