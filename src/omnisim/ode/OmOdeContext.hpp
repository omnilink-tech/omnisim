...
#ifndef OM_ODE_CONTEXT_HPP
#define OM_ODE_CONTEXT_HPP

// The real ODE headers are gone; the opaque handle typedefs are enough for this
// class shape (the class members were never guarded, only the includes were, so
// the moc output is unchanged from the pre-deletion build).
...
class OmOdeContext : public QObject {
  Q_OBJECT
...
public:
  ...
  static OmOdeContext *instance() { return cOdeContext; }
  ...
  // themselves via OmWorldInfo::resolveBroadphaseChoice when geometry-
  // aware selection is wanted).
  //
  // Safe to call only when the current space has zero geoms (typical
  // case: from OmWorldInfo::createOdeObjects at world-load time, before
  // any Solid has populated the space). If the space already has geoms
  // the requested switch is skipped with a single OmLog::warning and
...
private:
  ...
  static OmOdeContext *cOdeContext;
};
