...
WbContactSound::WbContactSound(const dGeomID &geom1, const dGeomID &geom2, const WbContactProperties *contactProperties) :
  ...
  mMass1(0.0),
  mMass2(0.0) {
  // P1.5 widening: scalar mass reads via dispatcher.
  WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
  if (mBody1)
    odeBackend->getBodyMass(static_cast<WbBodyHandle>(mBody1), &mMass1);
  if (mBody2)
    odeBackend->getBodyMass(static_cast<WbBodyHandle>(mBody2), &mMass2);
