...
OmContactSound::OmContactSound(const dGeomID &geom1, const dGeomID &geom2, const OmContactProperties *contactProperties) :
  mType(NONE),
  mGeom1(geom1),
  mGeom2(geom2),
  // ODE is gone, so contact sound never has bodies to listen to (the
  // producer, OmContactSoundManager::update over odeContacts(), sees an
  // eternally empty list -- this constructor is unreachable).
  mBody1(NULL),
  mBody2(NULL),
  mEnergy(0.0),
  ...
  mMass1(0.0),
  mMass2(0.0) {
  // ODE removed: the two body-mass reads that used to initialise mMass1/mMass2
  // are gone, so contact-sound MASS WEIGHTING IS UNIMPLEMENTED (both stay 0.0).
  // Moot while this whole class is unreachable -- see the mBody1/mBody2 note
  // above -- and both handles are NULL anyway, so the reads never ran.
  mSource = OmSoundEngine::createSource();
  ...
  mContactTime = 0.001 * OmSimulationState::instance()->time();
  mUpdateTime = 0.001 * OmSimulationState::instance()->time();
}

OmContactSound::~OmContactSound() {
  OmSoundEngine::deleteSource(mSource);
}

// cppcheck-suppress constParameter
bool OmContactSound::doesGeomsMatch(const dGeomID &geom1, const dGeomID &geom2) const {
  return ((geom1 == mGeom1 && geom2 == mGeom2) || (geom1 == mGeom2 && geom2 == mGeom1));
}

void OmContactSound::newOdeContact(const dContactGeom &c) {
  double deltaTime = 0.001 * OmSimulationState::instance()->time() - mUpdateTime;

  OmVector3 newContactPosition(c.pos[0], c.pos[1], c.pos[2]);

  if (deltaTime > 0) {  // avoid a division by 0
    OmVector3 newContactVelocity = (newContactPosition - mContactPosition) / deltaTime;
  ...
  double newEnergy = 0.5 * (mMass1 + mMass2) * (dv[0] * dv[0] + dv[1] * dv[1] + dv[2] * dv[2]);
  ...
  mContactTime = mUpdateTime = 0.001 * OmSimulationState::instance()->time();

  updateVelocities();
}

void OmContactSound::finalizeContactUpdate() {
  double deltaTime = 0.001 * OmSimulationState::instance()->time() - mUpdateTime;
...
void OmContactSound::updateSource() {
  ...
  if (bodiesRelativeVelocities > 0.01 && bodiesRelativeVelocities < (1.0 + MATCHING_VELOCITY_TOLERANCE) * contactVelocity &&
    ...
    OmVector3 projection =
      OmAffinePlane(mContactVelocity, mContactPosition).vectorProjection(mAngularVelocity1 + mAngularVelocity2);
