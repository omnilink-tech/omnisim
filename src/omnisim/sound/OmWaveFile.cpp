...
OmWaveFile::OmWaveFile(const QString &filename, QIODevice *device) :
  mFilename(filename),
  mDevice(device),
  mBuffer(NULL),
  mBufferSize(0),
...
OmWaveFile::OmWaveFile(qint16 *buffer, int bufferSize, int channelNumber, int bitsPerSample, int rate) :
  mDevice(NULL),
  mBuffer(buffer),
...
OmWaveFile::~OmWaveFile() {
  if (mOwnBuffer)
    free(mBuffer);
}

void OmWaveFile::loadConvertedFile(int side) {
  if (mBuffer != NULL)
    free(mBuffer);
...
void OmWaveFile::loadConvertedFile(int side, const QString &filename) {
  assert(mDevice == NULL);
...
void OmWaveFile::loadFromFile(const QString &extension, int side) {
  if (extension == "wav") {
    if (mDevice)
      loadConvertedFile(side);
    else
  ...
  static QString ffmpeg(QString("%1Contents/util/ffmpeg").arg(OmStandardPaths::omniSimHomePath()));
#else  // _WIN32
  static QString ffmpeg = "ffmpeg.exe";
#endif

  const QString outputFilename = OmStandardPaths::webotsTmpPath() + "output.wav";
  QString inputFilename;
  if (mDevice) {
    inputFilename = OmStandardPaths::webotsTmpPath() + "input." + extension;
...
void OmWaveFile::convertToMono(double balance) {
  if (mNChannels < 2)
    return;
