...
using namespace OmAction;

OmActionManager *OmActionManager::cInstance = NULL;

OmActionManager *OmActionManager::instance() {
  if (cInstance == NULL) {
    cInstance = new OmActionManager;
    qAddPostRoutine(OmActionManager::cleanup);
  }
  return cInstance;
}

void OmActionManager::cleanup() {
  delete cInstance;
  cInstance = NULL;
}

OmActionManager::OmActionManager() : QObject(), mFocusObject(NULL) {
  populateActions();
  connectActions();
}

OmActionManager::~OmActionManager() {
}

QAction *OmActionManager::action(OmActionKind kind) {
  return mActions[kind];
}

const QString OmActionManager::mapControlKey() {
#ifdef __APPLE__
  return "⌘";
#else
  return "Ctrl";
...
void OmActionManager::populateActions() {
  QAction *newAction;
  QIcon icon;
  ...
  connect(newAction, &QAction::triggered, this, &OmActionManager::dispatchUserCommand);
...
void OmActionManager::connectActions() {
  OmSimulationState *state = OmSimulationState::instance();

  connect(state, &OmSimulationState::enabledChanged, this, &OmActionManager::updateEnabled);
}

void OmActionManager::updateEnabled() {
  bool simulationEnabled = OmSimulationState::instance()->isEnabled();
...
void OmActionManager::setEnabled(OmActionKind kind, bool enabled) {
  mActions[kind]->setEnabled(enabled);
}

void OmActionManager::resetApplicationActionsState() {
...
void OmActionManager::enableTextEditActions(bool enabled, bool isReadOnly) {
...
void OmActionManager::updateRenderingButton() {
  QAction *rendering = action(OmAction::RENDERING);
...
void OmActionManager::forwardTransformToActionToSceneTree() {
...
void OmActionManager::dispatchUserCommand() {
  const QAction *senderAction = static_cast<QAction *>(sender());
  if (!senderAction)
    return;
  ...
  OmActionKind actionKind = (OmActionKind)senderAction->property("kind").toInt();
  if (focusObjectName.isEmpty())
    return;
  if (focusObjectName == "ConsoleEdit")
    // focus on console
...
void OmActionManager::setActionEnabledSilently(QAction *action, bool enabled) {
