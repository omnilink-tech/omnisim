...
#ifndef WB_CUDA_DISPATCH_CUH
#define WB_CUDA_DISPATCH_CUH
...
#error "WbCudaDispatch.cuh requires OMNISIM_WITH_CUDA to be defined."
#endif

#ifndef __CUDACC__
#error "WbCudaDispatch.cuh must be included from a CUDA (.cu) translation unit."
#endif
...
namespace WbCudaDispatch {
  ...
  // Throws WbCudaError if the launch itself fails — a kernel runtime error
  ...
  inline void parallelFor(std::size_t n, Kernel kernel, Args... args) {
    ...
    if (ctx == nullptr)
      throw WbCudaError(WbCudaCodes::NOT_AVAILABLE,
                        QObject::tr("WbCudaDispatch::parallelFor called without a CUDA context."));
    ...
    if (err != cudaSuccess)
      throw WbCudaError(WbCudaCodes::KERNEL_LAUNCH_FAILED,
