...
#ifndef WB_CUDA_CONTEXT_HPP
#define WB_CUDA_CONTEXT_HPP
...
public:
  ...
  // are cheap. Initialization failures emit WbLog::error with a CUDA_* code
  // and cause this to return null.
  static WbCudaContext *instance();
  ...
  // Block until the stream drains, mapping any error into WbLog::error with
...
private:
  WbCudaContext();
  ~WbCudaContext();
  WbCudaContext(const WbCudaContext &) = delete;
  WbCudaContext &operator=(const WbCudaContext &) = delete;
  ...
  static WbCudaContext *cInstance;
