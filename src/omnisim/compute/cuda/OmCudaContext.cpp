...
OmCudaContext *OmCudaContext::cInstance = nullptr;
bool OmCudaContext::cInitAttempted = false;

OmCudaContext::OmCudaContext() :
  mAvailable(false),
  mDeviceId(-1),
  mComputeMajor(0),
  mComputeMinor(0)
...
OmCudaContext *OmCudaContext::instance() {
  if (cInitAttempted)
    return cInstance ? (cInstance->mAvailable ? cInstance : nullptr) : nullptr;
  cInitAttempted = true;

  cInstance = new OmCudaContext();
  qAddPostRoutine([]() {
    delete cInstance;
    cInstance = nullptr;
  });
...
bool OmCudaContext::initialize() {
#ifndef OMNISIM_WITH_CUDA
  OmLog::diagnostic(OmCudaCodes::NOT_AVAILABLE,
  ...
  if (err != cudaSuccess || deviceCount <= 0) {
    const QString msg = QObject::tr("No CUDA-capable device found (cudaGetDeviceCount returned %1, count=%2).")
                          .arg(err)
                          .arg(deviceCount);
    OmLog::diagnostic(OmCudaCodes::NOT_AVAILABLE, msg);
    OmLog::warning(msg);
    return false;
  }

  if (requested >= deviceCount) {
    OmLog::warning(QObject::tr("OMNISIM_CUDA_DEVICE=%1 but only %2 device(s) present; falling back to device 0.")
                     .arg(requested)
                     .arg(deviceCount));
    requested = 0;
  }
  ...
  if (err != cudaSuccess) {
    ...
    OmLog::diagnostic(OmCudaCodes::DEVICE_INIT_FAILED, msg);
    OmLog::error(msg);
    return false;
  }
  ...
  if (props.major < kMinComputeMajor || (props.major == kMinComputeMajor && props.minor < kMinComputeMinor)) {
    ...
    OmLog::diagnostic(OmCudaCodes::COMPUTE_CAPABILITY_TOO_OLD, msg);
    OmLog::error(msg);
    return false;
  }
  ...
  OmLog::info(QObject::tr("CUDA initialized: device %1 '%2' (compute capability %3.%4, %5 MiB total).")
                .arg(mDeviceId)
                .arg(mDeviceName)
                .arg(mComputeMajor)
                .arg(mComputeMinor)
...
#ifdef OMNISIM_WITH_CUDA
bool OmCudaContext::synchronize(const QString &context) {
  if (!mAvailable)
    return false;
  ...
  if (err != cudaSuccess) {
    ...
    OmLog::diagnostic(OmCudaCodes::KERNEL_EXECUTION_ERROR, msg);
    OmLog::error(msg);
    return false;
  }
  return true;
