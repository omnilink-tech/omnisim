...
static const char *const gInfoLabels[] = {"loading",           "loadingParse",
                                          "loadingInstantiate","loadingDownload",
                                          "loadingFinalize",   "prePhysics",
                                          "physics",           "postPhysics",
...
OmPerformanceLog *OmPerformanceLog::cInstance = NULL;

void OmPerformanceLog::createInstance(const QString &fileName, int stepsCount) {
  if (cInstance)
    return;

  cInstance = new OmPerformanceLog(fileName, stepsCount);
}

void OmPerformanceLog::deleteInstance() {
  delete cInstance;
  cInstance = NULL;
}

void OmPerformanceLog::enableSystemInfoLog(bool enabled) {
  gLogSystemInfo = enabled;
}

OmPerformanceLog::OmPerformanceLog(const QString &fileName, int stepsCount) :
  mFileName(fileName),
  mFile(NULL),
...
OmPerformanceLog::~OmPerformanceLog() {
  for (int i = 0; i < INFO_COUNT; ++i) {
    delete mTimers[i];
    mTimers[i] = NULL;
  }
...
void OmPerformanceLog::worldClosed(const QString &worldName, const QString &worldMetrics) {
  if (mStepsCount == 0)
    return;
  ...
  if (logSysInfo) {
    // system info
    out << "System: " << OmSysInfo::sysInfo() << "\n";
    out << "Processor: " << OmSysInfo::processor() << "\n";
    out << "Number of cores: " << OmSysInfo::coreCount() << "\n";
    out << "OpenAL devices: " << OmSoundEngine::device() << "\n";
    out << "OpenGL vendor: " << OmSysInfo::openGLVendor() << "\n";
    out << "OpenGL renderer: " << OmSysInfo::openGLRenderer() << "\n";
    out << "OpenGL version: " << OmSysInfo::openGLVersion() << "\n";
    out << "\n";
  }
...
void OmPerformanceLog::writeTotalValues() {
  if (!openFile())
    return;
  QTextStream out(mFile);
...
void OmPerformanceLog::stepChanged() {
  if (mIsLogCompleted)
    return;
...
void OmPerformanceLog::startMeasure(InfoType type, const QString &object) {
  if (mIsLogCompleted)
    return;
...
void OmPerformanceLog::stopMeasure(InfoType type, const QString &object) {
  if (mIsLogCompleted)
    return;
...
void OmPerformanceLog::reportStepRenderingStats(int trianglesCount) {
  mValues[MAIN_TRIANGLES_COUNT] += trianglesCount;
  mValuesCount[MAIN_TRIANGLES_COUNT] += 1;
}

void OmPerformanceLog::writeLog(const QString &text) {
  if (!openFile())
    return;
  QTextStream out(mFile);
  out << text;
...
bool OmPerformanceLog::openFile() {
  return mFile->open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text);
}

void OmPerformanceLog::closeFile() {
  mFile->close();
}

QString OmPerformanceLog::justifiedNumber(double value, int size) {
