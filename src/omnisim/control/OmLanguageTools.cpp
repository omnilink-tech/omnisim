...
QString OmLanguageTools::pythonCommand(QString &shortVersion, const QString &command, QProcessEnvironment &env) {
  ...
  const QString advice =
    ...
    QObject::tr("To fix the problem, you should set the full path of your python command in "
                "OmniSim->preferences->python command.\n");
#else
    QObject::tr("OmniSim requires Python version 3.7 or newer in your current PATH.\n"
                "To fix the problem, you should:\n"
                "1. Check the Python command set in the OmniSim preferences.\n"
  ...
  if (pythonCommand == "!")
    OmLog::warning(QObject::tr("Python was not found.\n") + advice);
#else  // __linux__ and _WIN32
  shortVersion = checkIfPythonCommandExist(pythonCommand, env, false);
#ifndef _WIN32
  // Debian/Ubuntu ship no bare `python` executable (python3 only, unless the
  // optional python-is-python3 package is installed), and many controllers'
  // runtime.ini request `COMMAND = python`. Prefer `python3` when `python`
  // is absent instead of failing the controller launch.
  if (shortVersion.isEmpty() && pythonCommand == "python") {
    shortVersion = checkIfPythonCommandExist("python3", env, false);
    if (!shortVersion.isEmpty())
      pythonCommand = "python3";
  }
#endif
  if (shortVersion.isEmpty()) {
    OmLog::warning(QObject::tr("\"%1\" was not found.\n").arg(pythonCommand));
    pythonCommand = "!";
    OmLog::warning(QObject::tr("Python was not found.\n") + advice);
  } else {  // Python exists
    ...
    if (!processSucceeded || !output.startsWith("True")) {
      OmLog::warning(QObject::tr("\"%1\" 64 bit was not found, but the 32 bit version was found.\n").arg(pythonCommand) +
                     advice);
      pythonCommand = "!";
...
const QString OmLanguageTools::checkIfPythonCommandExist(const QString &pythonCommand, QProcessEnvironment &env, bool log) {
...
#ifdef __APPLE__
QString OmLanguageTools::findWorkingPythonPath(const QString &pythonVersion, QProcessEnvironment &env, bool log) {
  QString shortVersion;
...
const QStringList OmLanguageTools::pythonArguments() {
