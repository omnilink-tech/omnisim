...
void WbController::start() {
...
#else
  // A per-launch nonce (the simulator PID) uniquifies the INTERN pipe name so back-to-back headless
  ...
  // the pairing -> "no result". This is the residual launch-flake race (default-flip-plan.md §3.5). The
  // intern child receives the same nonce via OMNISIM_IPC_NONCE (setProcessEnvironment) and rebuilds the
  // identical name. EXTERN controllers are user-launched without the nonce, so their name stays the
  // legacy webots-<tmpId>-<robot> that libController's extern autodetect reconstructs.
  const QString serverName = "webots-" + QString::number(WbStandardPaths::webotsTmpPathId()) +
                             (mExtern ? QString() : "-" + QString::number(QCoreApplication::applicationPid())) +
                             "-" + mRobot->encodedName();
...
void WbController::reportControllerNotFound() {
  ...
  warn(tr("Expected either: %1, %2 or %3")
         .arg(name() + WbStandardPaths::executableExtension(), name() + ".py", "Dockerfile"));
