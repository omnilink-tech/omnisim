...
#ifndef OM_PERSPECTIVE_HPP
#define OM_PERSPECTIVE_HPP
...
public:
  ...
  explicit OmPerspective(const QString &worldPath);
  ...
  const QMap<OmAction::OmActionKind, bool> &disabledUserInteractionsMap() const { return mDisabledUserInteractionsMap; }
  bool isUserInteractionDisabled(OmAction::OmActionKind action) const {
  ...
  bool save() const;
  // The file this perspective is WRITTEN to: always the current extension.
  QString fileName() const;
  // The file this perspective is READ from: the current extension when it
  // exists, else the legacy sibling left by an older release. See
  // OmPerspectiveFileFormat for the dual-read / single-write policy.
  QString existingFileName() const;
...
private:
  ...
  static QString getActionName(OmAction::OmActionKind action);
  static OmAction::OmActionKind getActionFromString(const QString &actionString);
};
