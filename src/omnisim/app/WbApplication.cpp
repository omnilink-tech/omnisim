...
WbApplication::WbApplication() {
  ...
  WbPreferences::createInstance("OmniLink", "OmniSim", WbApplicationInfo::version());
  ...
  // needed for OmniSim controllers
...
bool WbApplication::isValidWorldFileName(const QString &worldName) {
  ...
  if (!worldNameInfo.exists() || !worldNameInfo.isFile() || !worldNameInfo.isReadable()) {
    WbLog::diagnostic("WORLD_FILE_NOT_FOUND", tr("Could not open file: '%1'.").arg(worldName), worldName);
    WbLog::error(tr("Could not open file: '%1'.").arg(worldName));
    return false;
  }
  if (QString::compare(worldNameInfo.suffix(), "wbt", Qt::CaseInsensitive) != 0) {
    WbLog::diagnostic("WORLD_WRONG_EXTENSION",
                      tr("Could not open file: '%1'. The world file extension must be '.wbt'.").arg(worldName), worldName);
...
void WbApplication::loadWorld(QString worldName, bool reloading, bool isLoadingAfterDownload) {
  ...
  if (errors > 0) {
    WbLog::diagnostic("WORLD_PARSE_INVALID_TOKENS", tr("'%1': Failed to load due to invalid token(s).").arg(worldName),
                      worldName);
  ...
  if (!parser.parseWorld(worldName, &updateParsingProgress)) {
    WbLog::diagnostic("WORLD_PARSE_SYNTAX_ERROR", tr("'%1': Failed to load due to syntax error(s).").arg(worldName),
                      worldName);
