...
#ifndef OM_SELECTION_HPP
#define OM_SELECTION_HPP
...
class OmSelection : public QObject {
  Q_OBJECT

public:
  // unique selection instance (can be NULL)
  static OmSelection *instance() { return cInstance; }

  OmSelection();
  virtual ~OmSelection();

  // the currently selected pose in the scene tree
  OmAbstractPose *selectedAbstractPose() const { return mSelectedAbstractPose; }

  // the currently selected solid in the scene tree
  OmSolid *selectedSolid() const;

  // the currently selected solid in the scene tree
  OmBaseNode *selectedNode() const { return mSelectedNode; }
...
public slots:
  ...
  void selectNodeFromSceneTree(OmBaseNode *node);

  // select another matter from the 3D view
  void selectPoseFromView3D(OmAbstractPose *p, bool handlesDisabled = false);
...
signals:
  ...
  void selectionChangedFromSceneTree(OmAbstractPose *t);

  // the selection has changed from the 3D view
  void selectionChangedFromView3D(OmAbstractPose *t);

  // the same OmAbstractPose was selected from the 3D view
  void selectionConfirmedFromView3D(OmAbstractPose *t);

  // the visible handles of the selected node have changed
  void visibleHandlesChanged();

private:
  static OmSelection *cInstance;
  OmAbstractPose *mSelectedAbstractPose;
  OmBaseNode *mSelectedNode;  // node selected in the scene tree
  bool mResizeHandlesEnabledFromSceneTree;

  // select another node
  void selectNode(OmBaseNode *n, bool handlesDisabled = false);

  // update matter selection
  // if mSelectedNode is not an instance of OmMatter, then select upper matter
