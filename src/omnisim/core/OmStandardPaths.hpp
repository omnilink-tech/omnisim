...
#ifndef OM_STANDARD_PATHS_HPP
#define OM_STANDARD_PATHS_HPP

class QString;

namespace OmStandardPaths {
  // directories, every path is terminated by a /
  const QString &omniSimHomePath();     // e.g. /usr/local/omnisim/
  const QString &omniSimLibPath();      // e.g. /usr/local/omnisim/lib/webots/ ("lib/webots" is the shipped on-disk directory name)
  const QString &controllerLibPath();  // e.g. /usr/local/omnisim/lib/controller/
#ifdef _WIN32
  const QString &omniSimMsys64Path();  // e.g. C:/Program Files/OmniSim/msys64/
#endif
  ...
  // Resolves a font *name* (as passed to wb_display_set_font() / wb_supervisor_set_label()) to the
  // absolute path of a TTF file shipped in fontsPath(). OmniSim ships only libre fonts (Liberation,
  // SIL OFL 1.1; DejaVu), so the legacy proprietary font names inherited from Webots (Arial,
  // Verdana, ...) are honoured as aliases onto their libre substitutes -- see fontAliases() below.
  // Returns an empty string when the name matches neither a shipped file nor a known alias.
  QString shippedFontFile(const QString &fontName);
  ...
  // urls
  const QString &omniSimDocsBaseUrl();   // https://github.com/omnilink-tech/omnisim/blob/main/docs
  const QString &githubRepositoryUrl();  // https://github.com/omnilink-tech/omnisim
