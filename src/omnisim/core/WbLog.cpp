...
void WbLog::initFileLog(const QString &logPath) {
  std::lock_guard<std::mutex> lock(gLogMutex);
  ...
  if (gLogFile.is_open()) {
    ...
    // PREVIOUS run. WbNewtonBackend::finalizeWorld() (re)writes "<log>.newton.json"
...
QString WbLog::logFilePath() {
  return gLogFilePath;
}

void WbLog::closeFileLog() {
  std::lock_guard<std::mutex> lock(gLogMutex);
  if (gLogFile.is_open()) {
...
void WbLog::fileLog(const QString &message) {
  fileLogWrite(message.toUtf8().constData());
}

void WbLog::diagnostic(const char *code, const QString &message, const QString &file, int line,
                       const char *severity) {
