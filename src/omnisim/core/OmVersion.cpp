...
OmVersion::OmVersion(int major, int minor, int revision, bool rStyle) :
  mMajor(major),
  mMinor(minor),
  mRevision(revision),
  mCommit(""),
...
bool OmVersion::fromString(const QString &text, const QString &prefix, const QString &suffix) {
  mMajor = 0;
  mMinor = 0;
  mRevision = 0;
  mCommit = "";
  ...
  // Check for version format R2018 or R2018a revision 1 or R2018a-rev1
...
QString OmVersion::toString(bool revision, bool digitsOnly, bool nightly) const {
  if (!digitsOnly && mIsRStyle) {
