...
static const char *WBT_OBJECTS =
  ...
  "DirectionalLight ElevationGrid Focus Fog Group HingeJoint "
  "HingeJointParameters Hinge2Joint ImageTexture "
...
static OmLanguage **gLanguages = NULL;

void OmLanguage::cleanup() {
  for (int i = 0; i < MAX; i++)
    delete gLanguages[i];
}

OmLanguage::OmLanguage(int code, const QString &name, const QString &defaultFileSuffix, const QString &commentPrefix,
                       bool isCompilable) :
  mCode(code),
  mName(name),
...
OmLanguage::~OmLanguage() {
}

void OmLanguage::addKeywords(const QString &words) {
  mKeywords += words.split(" ");
}

void OmLanguage::addApiWords(const QString &words) {
  mApiWords += words.split(" ");
}

void OmLanguage::addPreprocessorWords(const QString &words) {
  mPreprocessorWords += words.split(" ");
}

OmLanguage *OmLanguage::findByCode(int code) {
  assert(code >= 0 && code < MAX);

  if (gLanguages == NULL) {
    gLanguages = new OmLanguage *[MAX];
  ...
  OmLanguage *language = NULL;
  switch (code) {
    case PLAIN_TEXT:
      language = new OmLanguage(PLAIN_TEXT, "Plain text", "txt", "", false);
      break;
    case C:
      language = new OmLanguage(C, "C", "c", "//", true);
      ...
      break;
    case CPP:
      language = new OmLanguage(CPP, "C++", "cpp", "//", true);
      language->addKeywords(C_KEYWORDS);
      ...
      break;
    case PYTHON:
      language = new OmLanguage(PYTHON, "Python", "py", "#", false);
      language->addKeywords(PYTHON_KEYWORDS);
      language->addApiWords(API_CLASSES);
      break;
    case WBT:
      language = new OmLanguage(WBT, "OmniSim", "wbt", "#", false);
      language->addKeywords(WBT_KEYWORDS);
      language->addApiWords(API_CLASSES);
      language->addApiWords(WBT_OBJECTS);
      break;
    case PROTO:
      language = new OmLanguage(PROTO, "OmniSim", "proto", "#", false);
      language->addKeywords(WBT_KEYWORDS);
      language->addApiWords(API_CLASSES);
      language->addApiWords(WBT_OBJECTS);
      break;
    case MAKEFILE:
      language = new OmLanguage(MAKEFILE, "Makefile", "", "#", true);
...
OmLanguage *OmLanguage::findByFileName(const QString &fileName) {
...
const QStringList &OmLanguage::sourceFileExtensions() {
...
const QStringList &OmLanguage::headerFileExtensions() {
...
const QStringList &OmLanguage::dataFileExtensions() {
...
const QStringList &OmLanguage::exectuableExtensions() {
...
bool OmLanguage::isUnixLibraryExtension(const QString &extension) {
  return extension == ".so" || extension == ".dylib";
}

const QString &OmLanguage::compilationExtension() const {
