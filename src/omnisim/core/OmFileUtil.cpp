...
bool OmFileUtil::copyAndReplaceString(const QString &sourcePath, const QString &destinationPath, const QString &before,
                                      const QString &after) {
...
bool OmFileUtil::copyAndReplaceString(const QString &sourcePath, const QString &destinationPath,
...
bool OmFileUtil::forceCopy(const QString &source, const QString &target) {
...
int OmFileUtil::copyDir(const QString &sourcePath, const QString &destPath, bool recurse, bool merge, bool setWritable) {
...
bool OmFileUtil::areIdenticalFiles(const QString &fileAPath, const QString &fileBPath) {
...
bool OmFileUtil::isLocatedInDirectory(const QString &file, const QString &directory) {
#ifdef _WIN32
...
bool OmFileUtil::isLocatedInInstallationDirectory(const QString &file, bool ignoreAllowModify) {
  const bool inInstallTree = OmFileUtil::isLocatedInDirectory(file, OmStandardPaths::omniSimHomePath());
  if (!inInstallTree)
    return false;
  // Asset/PROTO resolution always needs to know whether a path is bundled, regardless of
  // whether the user is allowed to modify it.
  if (ignoreAllowModify)
    return true;
  // OmniSim is a developer- and agent-oriented distribution: its sample worlds and projects
  // live in the repository and are meant to be edited in place (and tracked in git). So the
  // upstream "installation directory is read-only" guard is OFF by default here -- saving a
  // bundled world overwrites it directly instead of forcing a project relocation/copy.
  // To restore the upstream read-only behaviour, set OMNISIM_LOCK_INSTALLATION=1.
  if (OmPreferences::booleanEnvironmentVariable("OMNISIM_LOCK_INSTALLATION"))
    return true;
  // Backwards compatibility: an explicit WEBOTS_ALLOW_MODIFY_INSTALLATION=0 still re-locks.
  if (!qEnvironmentVariableIsEmpty("WEBOTS_ALLOW_MODIFY_INSTALLATION"))
    return !OmPreferences::booleanEnvironmentVariable("WEBOTS_ALLOW_MODIFY_INSTALLATION");
  return false;
}

void OmFileUtil::searchDirectoryNameRecursively(QStringList &results, const QString &directoryName, const QString &root) {
  QDir dir(root);
  assert(dir.exists());
  QStringList subDirs = dir.entryList(QDir::AllDirs | QDir::NoDotAndDotDot | QDir::NoSymLinks);
  bool projectFolder = false;
  foreach (const QString &subDir, subDirs) {
    // we don't want to search below a OmniSim project folder to avoid very long and useless exploration
...
bool OmFileUtil::isDirectoryWritable(const QString &path) {
  QDir dir(path);
  ...
  QString fileName = dir.absolutePath() + "/omnisim_dummy.txt";
...
void OmFileUtil::revealInFileManager(const QString &file) {
#ifdef _WIN32
  ...
  if (output == "dolphin.desktop" || output == "org.kde.dolphin.desktop")
    ...
    OmDesktopServices::openUrl(QUrl::fromLocalFile(QFileInfo(file).path()).toString());
#endif
}
