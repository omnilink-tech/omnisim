...
OmPosixMemoryMappedFile::OmPosixMemoryMappedFile(const QString &name) : mName(name), mSize(0), mData(NULL) {
}

OmPosixMemoryMappedFile::~OmPosixMemoryMappedFile() {
  if (mData)
    munmap(mData, mSize);
  unlink(mName.toUtf8());
}

bool OmPosixMemoryMappedFile::create(int size) {
