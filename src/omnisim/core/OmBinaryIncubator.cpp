...
OmBinaryIncubator::Error OmBinaryIncubator::copyBinaryAndDependencies(const QString &filename) {
  ...
  QDir projectLibraryDir(OmProject::current()->path() + "libraries");
...
OmBinaryIncubator::Error OmBinaryIncubator::copyBinaryPath(const QString &path) {
  QFileInfo fi(path);
  assert(fi.isDir());
  ...
  if (isExecutablePath)
    binaryFullName += OmStandardPaths::executableExtension();
  else if (isLibraryPath)
    binaryFullName = OmStandardPaths::dynamicLibraryPrefix() + binaryFullName + OmStandardPaths::dynamicLibraryExtension();
  else
