...
#ifndef OM_LANGUAGE_HPP
#define OM_LANGUAGE_HPP

//
// Description: definition of the languages that can be edited in OmniSim text editor
//
...
public:
  ...
  static OmLanguage *findByFileName(const QString &fileName);

  // get the list of possible executable extensions, e.g. "", ".exe", ".so", ".dll", ...
  ...
  static const QStringList &sourceFileExtensions();  // ".c", ".cpp", ".c++", ".py", etc.
  ...
  // language enum codes
  enum { PLAIN_TEXT, C, CPP, PYTHON, MAKEFILE, WBT, PROTO, MAX };
  int code() const { return mCode; }
  static OmLanguage *findByCode(int code);
  ...
  // true for compiled languages (C, C++)
  ...
  // OmniSim API words, e.g. "Accelerometer", "wb_accelerometer_enable" ...
...
private:
  ...
  OmLanguage(int code, const QString &name, const QString &defaultFileSuffix, const QString &commentPrefix, bool isCompilable);
  ~OmLanguage();
