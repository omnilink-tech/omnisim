...
OmWindowsRegistry::OmWindowsRegistry(const QString &key) {
  HKEY baseKey = 0;
...
OmWindowsRegistry::~OmWindowsRegistry() {
  RegCloseKey(mCurrentKey);
}

QString OmWindowsRegistry::stringValue(const QString &name) const {
  WCHAR szBuffer[512];
...
QStringList OmWindowsRegistry::subKeys() const {
  QStringList keys;
