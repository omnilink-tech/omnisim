...
#ifndef OM_VERSION_HPP
#define OM_VERSION_HPP

//
// Description: store a generic version structure major.minor.revision
//              and support OmniSim specific version R2018a revision 1
//
...
class OmVersion {
public:
  explicit OmVersion(int major = 0, int minor = 0, int revision = 0, bool rStyle = false);
  ...
  // e.g. pattern "#OMNISIM R2025a utf8" (or legacy "#VRML_SIM V8.6 utf8") with prefix "^(?:OMNISIM|VRML_SIM) " -- keep any alternation NON-CAPTURING: the version regex owns groups 1-6
  ...
  // OmniSim version is a special case and the string will have the form
  ...
  OmVersion &operator=(const OmVersion &other) {
    ...
    mIsRStyle = other.mIsRStyle;
    mCommit = other.mCommit;
    mDate = other.mDate;
    return *this;
  }
  // comparison operators
  friend bool operator==(const OmVersion &l, const OmVersion &r) {
    return l.mMajor == r.mMajor && l.mMinor == r.mMinor && l.mRevision == r.mRevision;
  }
  friend bool operator!=(const OmVersion &l, const OmVersion &r) { return !(l == r); }
  friend bool operator<(const OmVersion &l, const OmVersion &r) {
  ...
  friend bool operator>(const OmVersion &l, const OmVersion &r) { return r < l; }
  friend bool operator<=(const OmVersion &l, const OmVersion &r) { return !(r < l); }
  friend bool operator>=(const OmVersion &l, const OmVersion &r) { return !(l < r); }
