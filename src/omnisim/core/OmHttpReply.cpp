...
QByteArray OmHttpReply::forge404Reply(const QString &url) {
  QByteArray reply;
  const QByteArray data =
    "<!DOCTYPE html>\n<html lang=\"en\"><head><meta charset=UTF-8><title>OmniSim: Not Found</title></head><body>"
...
QByteArray OmHttpReply::forgeFileReply(const QString &fileName, const QString &etag, const QString &host, const QString &url) {
  QByteArray reply;

  QFile file(fileName);
  if (!file.open(QIODevice::ReadOnly)) {
    OmLog::warning(QObject::tr("Cannot read '%1', sending \"404 Not Found\".").arg(fileName));
    return forge404Reply(url);
  }
  QByteArray data = file.readAll();
  QByteArray replace = "http://" + host.toUtf8() + "/";
  data.replace("omnisim://", replace);
  ...
  if (!etag.isEmpty() && hash.toHex().compare(etag.toLocal8Bit(), Qt::CaseSensitive) == 0) {
    ...
    const QString mimeTypeString = OmHttpReply::mimeType(fileName, true);
...
QString OmHttpReply::mimeType(const QString &url, bool generic) {
  ...
  if (extension == "png" || extension == "jpg" || extension == "jpeg" || extension == "ico")
    ...
    return "application/octet-stream";  // emscripten file-package bundle (e.g. wrenjs.data)
