...
const WbVersion &WbApplicationInfo::version() {
  ...
  if (firstCall) {
    ...
    if (!success)
      WbLog::fatal(QObject::tr("Internal error: the OmniSim version is not computable."));
    firstCall = false;
  }
  return webotsVersion;
}

const QString &WbApplicationInfo::omniSimVersion() {
  static const QString omniSimVersionString = "5.0.0";
