...
static OmMacAddress *gInstance = NULL;

OmMacAddress *OmMacAddress::instance() {
  if (gInstance == NULL)
    gInstance = new OmMacAddress();
  return gInstance;
}

bool OmMacAddress::check(const QString &macAddress) const {
  return macAddress.compare(address(), Qt::CaseInsensitive) == 0;
}

QString OmMacAddress::address() const {
...
OmMacAddress::OmMacAddress() {
  ...
  if (!found)
    mError = QObject::tr("No active network adapter found.") + "\n" +
             QObject::tr("An active network adapter is required to run OmniSim.");
}
