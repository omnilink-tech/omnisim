...
OmExternProtoEditor::OmExternProtoEditor(QWidget *parent) : OmValueEditor(parent) {
  connect(this, &OmExternProtoEditor::changed, OmActionManager::instance()->action(OmAction::SAVE_WORLD), &QAction::setEnabled);
}

OmExternProtoEditor::~OmExternProtoEditor() {
}

void OmExternProtoEditor::updateContents() {
  // clear layout
  clearLayout();
  ...
  connect(mInsertButton, &QPushButton::pressed, this, &OmExternProtoEditor::insertImportableExternProto);
  ...
  for (int i = 0; i < externProto.size(); ++i) {
    ...
    label->setToolTip(
      QString("EXTERNPROTO \"%1\"").arg(OmProtoManager::instance()->formatExternProtoPath(externProto[i]->url())));
    ...
    connect(removeButton, &QPushButton::pressed, this, &OmExternProtoEditor::removeImportableExternProto);
...
void OmExternProtoEditor::insertImportableExternProto() {
  OmInsertExternProtoDialog dialog(this);
...
void OmExternProtoEditor::removeImportableExternProto() {
  ...
  if (index > 1) {
    ...
    if (label) {
      ...
      OmProtoManager::instance()->removeImportableExternProto(proto, OmWorld::instance()->root());
      updateContents();  // regenerate panel
      emit changed(true);
    }
  }
...
void OmExternProtoEditor::stopEditing() {
  clearLayout();
}

void OmExternProtoEditor::clearLayout() {
