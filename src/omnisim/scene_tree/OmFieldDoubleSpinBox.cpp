...
OmFieldDoubleSpinBox::OmFieldDoubleSpinBox(QWidget *parent, int mode) : OmDoubleSpinBox(parent) {
  mDecimals = 0;
  mMode = mode;
  ...
  connect(lineEdit(), &QLineEdit::textEdited, this, &OmFieldDoubleSpinBox::findDecimals);

  if (mMode == RGB)
    setRange(0.0, 1.0);
}

OmFieldDoubleSpinBox::~OmFieldDoubleSpinBox() {
}

void OmFieldDoubleSpinBox::setValueNoSignals(double value) {
  if (value == this->value())
    return;
...
void OmFieldDoubleSpinBox::setMode(int mode) {
  mDecimals = 0;
  mMode = mode;
  if (mMode == RGB)
    setRange(0.0, 1.0);
...
void OmFieldDoubleSpinBox::stepBy(int steps) {
...
void OmFieldDoubleSpinBox::findDecimals(const QString &text) {
  if (mMode == RADIANS)
    return;
...
QString OmFieldDoubleSpinBox::textFromValue(double value) const {
  return QString::number(value);
}

void OmFieldDoubleSpinBox::keyPressEvent(QKeyEvent *event) {
  if (event->matches(QKeySequence::Undo))
    OmUndoStack::instance()->undo();
  else if (event->matches(QKeySequence::Redo))
    OmUndoStack::instance()->redo();
  else
    OmDoubleSpinBox::keyPressEvent(event);
}

void OmFieldDoubleSpinBox::keyReleaseEvent(QKeyEvent *event) {
...
void OmFieldDoubleSpinBox::focusOutEvent(QFocusEvent *event) {
