...
void WbAddNodeDialog::buildTree() {
  ...
  QTreeWidgetItem *const webotsProtosItem =
    new QTreeWidgetItem(QStringList("PROTO nodes (OmniSim Projects)"), WbProtoManager::PROTO_WEBOTS);
...
int WbAddNodeDialog::addProtosFromProtoList(QTreeWidgetItem *parentItem, int type, const QRegularExpression &regexp,
  ...
  while (it.hasNext()) {
    ...
    const QString cleanPath = path.replace("omnisim://", "").replace(re, "").replace(WbStandardPaths::webotsHomePath(), "");
