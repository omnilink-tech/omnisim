...
class OmGyro : public OmSolidDevice {
  Q_OBJECT

public:
  // constructors and destructor
  explicit OmGyro(OmTokenizer *tokenizer = NULL);
  OmGyro(const OmGyro &other);
  explicit OmGyro(const OmNode &other);
  virtual ~OmGyro() override;
...
private:
  ...
  OmNode *clone() const override { return new OmGyro(*this); }
  void init();
  void computeValue();
