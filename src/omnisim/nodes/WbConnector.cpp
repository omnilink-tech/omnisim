...
void WbConnector::rotateBodies(WbConnector *other, const dQuaternion q, const dBodyID b1, const dBodyID b2) {
  // P1.5 widening: the bookend get/set quaternion calls flow through
  // the dispatcher; the dQMultiply in between stays local ODE math
  // (it's a pure dReal[4]-on-dReal[4] arithmetic helper, not a body
  // op). This finishes WbConnector's migration off direct dBody*
  // calls -- snapOrigins, attachTo save-restore, and rotateBodies
  // all dispatch-routed now.
  WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
  double q1d[4], q2d[4];
  if (b1)
    odeBackend->getBodyQuaternion(static_cast<WbBodyHandle>(b1), q1d);
  if (b2)
    odeBackend->getBodyQuaternion(static_cast<WbBodyHandle>(b2), q2d);

  // multiply quaternions (rotation). Stage through dQuaternion temporaries
  // so dQMultiply gets the dReal[] it expects regardless of whether
  // dReal is float or double in this build.
  dQuaternion q1n, q2n;
  if (b1) {
    const dQuaternion q1Ode = {static_cast<dReal>(q1d[0]), static_cast<dReal>(q1d[1]),
                               static_cast<dReal>(q1d[2]), static_cast<dReal>(q1d[3])};
    dQMultiply0(q1n, q, q1Ode);  // q
  }
  if (b2) {
    const dQuaternion q2Ode = {static_cast<dReal>(q2d[0]), static_cast<dReal>(q2d[1]),
                               static_cast<dReal>(q2d[2]), static_cast<dReal>(q2d[3])};
    dQMultiply1(q2n, q, q2Ode);  // inverse of q
  }

  // change current bodies orientation
  if (b1) {
    const double q1Out[4] = {q1n[0], q1n[1], q1n[2], q1n[3]};
    odeBackend->setBodyQuaternion(static_cast<WbBodyHandle>(b1), q1Out);
  }
  if (b2) {
    const double q2Out[4] = {q2n[0], q2n[1], q2n[2], q2n[3]};
    odeBackend->setBodyQuaternion(static_cast<WbBodyHandle>(b2), q2Out);
  }
}
...
#endif
  // P1.5 of engine-migration-plan.md: ODE callsite routed through
  // WbPhysicsBackend. Both b1 and b2 are ODE bodies here (this
  // snapOrigins path runs only when ODE owns the connector's
  // ancestor), so the registry's ODE backend is the right dispatch
  // target. Both the get and set sides now flow through the
  // dispatcher; previous step landed get, this step lands set.
  WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
  // retrive current body positions and shift them
  if (b1) {
    double d1[3];
    odeBackend->getBodyPosition(static_cast<WbBodyHandle>(b1), d1);
    const double p1[3] = {d1[0] + h[0], d1[1] + h[1], d1[2] + h[2]};
    odeBackend->setBodyPosition(static_cast<WbBodyHandle>(b1), p1);
  }
  if (b2) {
    double d2[3];
    odeBackend->getBodyPosition(static_cast<WbBodyHandle>(b2), d2);
    const double p2[3] = {d2[0] - h[0], d2[1] - h[1], d2[2] - h[2]};
    odeBackend->setBodyPosition(static_cast<WbBodyHandle>(b2), p2);
  }
}
...
void WbConnector::attachTo(WbConnector *other) {
  ...
  if (mSnap->isTrue()) {
    // P1.5 widening: save-then-restore of both bodies' pose flows
    // through the dispatcher. b1 and b2 are ODE bodies (the connector
    // attachment path only fires on ODE-owned ancestors), so the ODE
    // backend is the right target. Same rationale as snapOrigins
    // above.
    WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
    ...
    double q1[4], q2[4];
    if (b1) {
      odeBackend->getBodyPosition(static_cast<WbBodyHandle>(b1), p1);
      odeBackend->getBodyQuaternion(static_cast<WbBodyHandle>(b1), q1);
    }
    if (b2) {
      odeBackend->getBodyPosition(static_cast<WbBodyHandle>(b2), p2);
      odeBackend->getBodyQuaternion(static_cast<WbBodyHandle>(b2), q2);
    }
    ...
    if (b1) {
      odeBackend->setBodyPosition(static_cast<WbBodyHandle>(b1), p1);
      odeBackend->setBodyQuaternion(static_cast<WbBodyHandle>(b1), q1);
    }
    if (b2) {
      odeBackend->setBodyPosition(static_cast<WbBodyHandle>(b2), p2);
      odeBackend->setBodyQuaternion(static_cast<WbBodyHandle>(b2), q2);
    }
  } else
    createFixedJoint(other, b1, b2);
}
