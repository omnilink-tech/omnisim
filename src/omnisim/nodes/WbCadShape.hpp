...
public:
  ...
  // R4 wgpu collection: expose the CAD submeshes (a .glb/.dae/.obj imports as N submeshes, each with a
  // WREN static mesh + its world transform + a PBRAppearance) so WbWgpuSceneRenderer can draw
  // URDF-imported robots — a mesh-visual arm ships CadShape visuals, not WbShape, and was otherwise skipped
  // by collectShapeDraws entirely (the "robot doesn't render" gap). wgpuTransform(i) is the world matrix
  // (parents included) via wr_transform_get_matrix.
  int wgpuMeshCount() const { return mWrenMeshes.size(); }
  WrStaticMesh *wgpuMesh(int i) const { return mWrenMeshes.value(i, nullptr); }
  WrTransform *wgpuTransform(int i) const { return mWrenTransforms.value(i, nullptr); }
  WbPbrAppearance *wgpuAppearance(int i) const { return mPbrAppearances.value(i, nullptr); }
  bool wgpuCastShadows() const;  // castShadows field (defined in .cpp; WbSFBool is incomplete here)
