...
void WbSliderJoint::applyToOdeMinAndMaxStop() {
  ...
  if (m != M) {
    // P1.6 slice 6: dispatcher-routed slider limits.
    ...
    ode->setJointSliderParam(h, WbPhysicsBackend::WB_JP_LO_STOP, m - mOdePositionOffset);
    ode->setJointSliderParam(h, WbPhysicsBackend::WB_JP_HI_STOP, M - mOdePositionOffset);
  }
}
...
void WbSliderJoint::applyToOdeSpringAndDampingConstants(dBodyID body, dBodyID parentBody) {
  ...
  // Stops + spring/damper config on the LMotor companion joint
  // (P1.6 slice 6: dispatcher-routed, axis 0).
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle lmh = reinterpret_cast<WbJointHandle>(mSpringAndDamperMotor);
  const double stop = -mOdePositionOffset;
  ode->setJointLMotorParam(lmh, 0, WbPhysicsBackend::WB_JP_LO_STOP, stop);
  ode->setJointLMotorParam(lmh, 0, WbPhysicsBackend::WB_JP_HI_STOP, stop);
  ode->setJointLMotorParam(lmh, 0, WbPhysicsBackend::WB_JP_STOP_CFM, cfm);
  ode->setJointLMotorParam(lmh, 0, WbPhysicsBackend::WB_JP_STOP_ERP, erp);
...
void WbSliderJoint::prePhysicsStep(double ms) {
  ...
  if (isEnabled()) {
    if (lm && lm->userControl()) {
      // user-defined force (P1.6 slice 3: dispatcher-routed; slider is
      // ODE-only today since Newton's arm doesn't wire prismatic joints).
      const double force = lm->rawInput();
      WbPhysicsBackendRegistry::odeBackend()->addJointSliderForce(
        reinterpret_cast<WbJointHandle>(mJoint), force);
      if (lm->hasMuscles())
      ...
      // routes through the dispatcher. Slider is ODE-only today.
      ...
      WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
      const WbJointHandle h = reinterpret_cast<WbJointHandle>(mJoint);
      ode->setJointSliderParam(h, WbPhysicsBackend::WB_JP_FMAX, fMax);
      // ODE convention reverses the slider axis -- the negation stays here.
      ode->setJointSliderParam(h, WbPhysicsBackend::WB_JP_VELOCITY, -currentVelocity);
    }
    if (mSpringAndDamperMotor) {
...
void WbSliderJoint::postPhysicsStep() {
  ...
  // prismatic. Prefer it when the joint is Newton-backed; otherwise read
  // the ODE slider position. Without this the finger sensors read frozen
  // and the gripper's "holding" detection never fired under Newton.
  bool fromNewton = false;
  if (mNewtonJointIndex >= 0) {
    ...
    if (newton != nullptr && newton->isAvailable()) {
      double pos = 0.0;
      if (newton->getJointHingeAngle(WbNewtonBackend::handleFromIndex(mNewtonJointIndex), &pos) == 0) {
        mPosition = pos;
        fromNewton = true;
      }
    }
  ...
  if (!fromNewton) {
    double position = 0.0;
    WbPhysicsBackendRegistry::odeBackend()->getJointSliderPosition(reinterpret_cast<WbJointHandle>(mJoint), &position);
    mPosition = position + mOdePositionOffset;
  }
  WbJointParameters *const p = parameters();
  if (p)
