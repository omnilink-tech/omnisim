...
#include <climits>
#include <cmath>
#include "../../../include/controller/c/omnisim/display.h"  // contains the definitions of the image format
...
public:
  OmDisplayImage(int id, int width, int height, unsigned int *image, bool isTransparent) :
    mId(id),
    mWidth(width),
    mHeight(height),
    mImage(image),
    mIsTransparent(isTransparent) {}
  OmDisplayImage(const OmDisplayImage &d) :
    mId(d.id()),
    mWidth(d.width()),
    mHeight(d.height()),
  ...
  virtual ~OmDisplayImage() { delete[] mImage; }
...
private:
  ...
  OmDisplayImage &operator=(const OmDisplayImage &);  // non copyable
};
...
void OmDisplay::init() {
  ...
  mDisplayFont = new OmDisplayFont();
  QString error = mDisplayFont->error();
  if (!error.isEmpty())
    warn(error);
  // libre monospace default; the legacy "Lucida Console" name still resolves here through
  // OmStandardPaths::shippedFontFile()
  setFont(const_cast<char *>("DejaVuSansMono"), 8);
}

OmDisplay::OmDisplay(OmTokenizer *tokenizer) : OmRenderingDevice("Display", tokenizer) {
  init();
}

OmDisplay::OmDisplay(const OmDisplay &other) : OmRenderingDevice(other) {
  init();
}

OmDisplay::OmDisplay(const OmNode &other) : OmRenderingDevice(other) {
  init();
}
...
void OmDisplay::clearImages() {
  for (int i = 0; i < mImages.size(); i++)
    delete mImages.at(i);
  mImages.clear();
}

void OmDisplay::preFinalize() {
  OmRenderingDevice::preFinalize();
  findImageTextures();
}

int OmDisplay::channelNumberFromPixelFormat(int pixelFormat) {
  switch (pixelFormat) {
    case WB_IMAGE_RGB:
      return 3;
    case WB_IMAGE_RGBA:
...
void OmDisplay::findImageTextures() {
  ...
  OmNode *firstChild = children().item(0);
  const OmShape *shape = dynamic_cast<OmShape *>(firstChild);
  if (shape) {
    const OmAppearance *appearance = shape->appearance();
    const OmPbrAppearance *pbrAppearance = shape->pbrAppearance();
    if (appearance) {
      // cppcheck-suppress constVariablePointer
      OmImageTexture *theTexture = appearance->texture();
      if (theTexture)
        mImageTextures.push_back(theTexture);
    } else if (pbrAppearance) {
      // cppcheck-suppress constVariablePointer
      OmImageTexture *theTexture = pbrAppearance->baseColorMap();
      if (theTexture)
        mImageTextures.push_back(theTexture);
    ...
    OmGroup *group = dynamic_cast<OmGroup *>(firstChild);
    if (group)
      findImageTextures(group);
  }

  for (int i = 0; i < mImageTextures.size(); ++i)
    connect(mImageTextures.at(i), &QObject::destroyed, this, &OmDisplay::removeImageTexture);

  // debug code - print the found materials
  // foreach (OmImageTexture *texture, mImageTextures)
  //   parsingWarn(QString("found image texture %1").arg(texture->usefulName()));
}

void OmDisplay::removeImageTexture(QObject *object) {
  mImageTextures.removeAll(static_cast<OmImageTexture *>(object));
}

void OmDisplay::clearImageTextures() {
  removeExternalTextures();
  mImageTextures.clear();
}

void OmDisplay::findImageTextures(OmGroup *group) {
  OmMFNode::Iterator i(group->children());
  while (i.hasNext()) {
    OmNode *node = i.next();
    const OmShape *shape = dynamic_cast<OmShape *>(node);
    if (shape) {
      const OmAppearance *appearance = shape->appearance();
      const OmPbrAppearance *pbrAppearance = shape->pbrAppearance();
      if (appearance) {
...
QString OmDisplay::pixelInfo(int x, int y) const {
  return QString::asprintf("pixel(%d,%d)=#%02X%02X%02X%02X", x, y, red(x, y), green(x, y), blue(x, y), alpha(x, y));
}

void OmDisplay::handleMessage(QDataStream &stream) {
  short size, x, y, w, h;
  int *px = NULL, *py = NULL;
  int id, channel;
  bool blend = false;
  ...
  switch (command) {
    ...
    case C_DISPLAY_IMAGE_COPY: {
      ...
      if (data != NULL)
        mImages.push_back(new OmDisplayImage(id, w, h, data, mIsTextureTransparent));
      break;
    }
    ...
    case C_DISPLAY_IMAGE_SAVE: {
      ...
      if (id == 0) {
        ...
        mSaveOrders.push_back(new OmDisplayImage(0, w, h, data, mIsTextureTransparent));
        break;
      }
      const OmDisplayImage *di = imageFind(id);
      if (di)
        mSaveOrders.push_back(new OmDisplayImage(*di));
      break;
    }
...
void OmDisplay::writeAnswer(OmDataStream &stream) {
  if (mRequestImages) {
    mRequestImages = false;
    ...
    for (unsigned i = 0; i < number; i++) {
      const OmDisplayImage *di = mImages.at(i);
      stream << (qint32)di->id();
  ...
  while (!mSaveOrders.empty()) {
    OmDisplayImage *di = mSaveOrders.back();
    stream << tag();
...
void OmDisplay::writeConfigure(OmDataStream &stream) {
  setup();
...
void OmDisplay::drawPixel(int x, int y) {
...
void OmDisplay::fastDrawPixel(int offset) {
  if (mAlpha != 0xFF)
    setTransparentTextureIfNeeded();
...
void OmDisplay::drawLine(int x0, int y0, int x1, int y1) {
...
void OmDisplay::drawRectangle(int x, int y, int w, int h, bool fill) {
  if (mAlpha != 0xFF)
    setTransparentTextureIfNeeded();
...
void OmDisplay::drawOval(int cx, int cy, int a, int b, bool fill) {
...
void OmDisplay::drawText(const char *txt, int x, int y) {
  if (x >= width() || y >= height())
...
int OmDisplay::drawChar(unsigned long c, int x, int y) {
...
void OmDisplay::setFont(char *font, unsigned int size) {
  ...
  QString filename = OmStandardPaths::shippedFontFile(QString(font));
  if (!filename.isEmpty())
    fileFound = true;
  else {
    filename = OmProject::current()->path() + "fonts/" + QString(font) + ".ttf";
...
void OmDisplay::drawPolygon(const int *px, const int *py, int size, bool fill) {
...
unsigned int *OmDisplay::imageCopy(short int x, short int y, short int &w, short int &h) {
...
void OmDisplay::imagePaste(int id, int x, int y, bool blend) {
  if (x >= width() || y >= height())
    return;
  const OmDisplayImage *subImage = NULL;
  for (int i = 0; i < mImages.size(); i++)
...
void OmDisplay::imageLoad(int id, int w, int h, void *data, int format) {
  ...
  mImages.push_back(new OmDisplayImage(id, w, h, clippedImage, isTransparent));
  // cppcheck-suppress memleak
}

void OmDisplay::imageDelete(int id) {
  for (int i = 0; i < mImages.size(); i++) {
    OmDisplayImage *di = mImages.at(i);
    if (di->id() == id) {
      mImages.remove(i);
      delete di;
      return;
...
OmDisplayImage *OmDisplay::imageFind(int id) {
  for (int i = 0; i < mImages.size(); i++)
    if (mImages.at(i)->id() == id)
      return mImages.at(i);
  return NULL;
...
void OmDisplay::createWrenObjects() {
  OmRenderingDevice::createWrenObjects();
  createWrenOverlay();
}

void OmDisplay::createWrenOverlay() {
  ...
  mNeedToSetExternalTextures = true;
  mOverlay = new OmWrenTextureOverlay(mImage, width(), height(), OmWrenTextureOverlay::TEXTURE_TYPE_BGRA,
                                      OmWrenTextureOverlay::OVERLAY_TYPE_DISPLAY, 1.0, mIsTextureTransparent);
  connect(mOverlay, &OmWrenTextureOverlay::textureUpdated, this, &OmRenderingDevice::textureUpdated);
  connect(mOverlay, &QObject::destroyed, this, &OmDisplay::removeExternalTextures);
...
void OmDisplay::removeExternalTextures() {
...
void OmDisplay::setTransparentTextureIfNeeded() {
...
void OmDisplay::attachCamera(WbDeviceTag cameraTag) {
  OmDevice *device = robot()->findDevice(cameraTag);
  assert(device);
  OmCamera *camera = static_cast<OmCamera *>(device);
  assert(camera);
  if (mAttachedCamera != camera) {
    if (isWindowActive()) {
      if (mAttachedCamera)
    ...
    connect(mAttachedCamera, &OmCamera::destroyed, this, &OmDisplay::detachCamera);
    // D1.4: the WREN background-texture compositing (camera image drawn behind
    // the display's own pixels) is retired with the renderer. The attachment
    // state and the transparent-pixel convention are kept: default-black pixels
    // become transparent so a compositor that can source the camera image still
    // has the correct alpha mask in the CPU buffer.
...
void OmDisplay::postPhysicsStep() {
  ...
  if (mNeedToSetExternalTextures) {
    mNeedToSetExternalTextures = false;
    // D1.4: the external texture is now handed over as the CPU pixel buffer
    // (the WREN texture object and its power-of-two padding are gone, so the
    // display/texture size ratio is exactly 1).
    foreach (OmImageTexture *texture, mImageTextures)
      texture->setExternalTexture(reinterpret_cast<unsigned char *>(mImage), width(), height(), 1.0, 1.0);
  }
...
void OmDisplay::reset(const QString &id) {
