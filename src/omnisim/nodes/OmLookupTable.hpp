...
#ifndef OM_LOOKUP_TABLE_HH
#define OM_LOOKUP_TABLE_HH

class OmMFVector3;

class OmLookupTable {
public:
  explicit OmLookupTable(const OmMFVector3 &lookupTable);
  ~OmLookupTable();
...
private:
  ...
  OmLookupTable(const OmLookupTable &);             // non constructor-copyable
  OmLookupTable &operator=(const OmLookupTable &);  // non copyable
};
