...
bool WbBallJoint::setJoint() {
  ...
  // P1.6 slice 5: control-motor enable/disable through the dispatcher.
  const bool enable = !(parentBody == NULL && body == NULL);
  WbPhysicsBackendRegistry::odeBackend()->setJointEnabled(
    reinterpret_cast<WbJointHandle>(mControlMotor), enable);
...
void WbBallJoint::applyToOdeSpringAndDampingConstants(dBodyID body, dBodyID parentBody) {
  ...
  // P1.6 slice 6: AMotor spring/damper stops + CFM/ERP through the
  // dispatcher. Axis indexing replaces ODE's per-axis param constant
  // suffixes (dParamLoStop2/3 etc.).
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle amh = reinterpret_cast<WbJointHandle>(mSpringAndDamperMotor);
  ode->setJointAMotorParam(amh, 0, WbPhysicsBackend::WB_JP_LO_STOP, clamped);
  ode->setJointAMotorParam(amh, 0, WbPhysicsBackend::WB_JP_HI_STOP, clamped);
  ode->setJointAMotorParam(amh, 0, WbPhysicsBackend::WB_JP_STOP_CFM, cfm);
  ode->setJointAMotorParam(amh, 0, WbPhysicsBackend::WB_JP_STOP_ERP, erp);
  ...
  ode->setJointAMotorParam(amh, 1, WbPhysicsBackend::WB_JP_LO_STOP, clamped2);
  ode->setJointAMotorParam(amh, 1, WbPhysicsBackend::WB_JP_HI_STOP, clamped2);
  ode->setJointAMotorParam(amh, 1, WbPhysicsBackend::WB_JP_STOP_CFM, cfm2);
  ode->setJointAMotorParam(amh, 1, WbPhysicsBackend::WB_JP_STOP_ERP, erp2);
  ...
  ode->setJointAMotorParam(amh, 2, WbPhysicsBackend::WB_JP_LO_STOP, clamped3);
  ode->setJointAMotorParam(amh, 2, WbPhysicsBackend::WB_JP_HI_STOP, clamped3);
  ode->setJointAMotorParam(amh, 2, WbPhysicsBackend::WB_JP_STOP_CFM, cfm3);
  ode->setJointAMotorParam(amh, 2, WbPhysicsBackend::WB_JP_STOP_ERP, erp3);
}
...
void WbBallJoint::prePhysicsStep(double ms) {
  ...
  if (isEnabled()) {
    // P1.6 slice 3: user-defined-torque writes route through the
    // dispatcher. Ball joints are ODE-only today (Newton's arm wires
    // only revolute hinges). `mJoint` here is the AMotor, dispatched
    // as a WbJointHandle reinterpreting to dJointID.
    WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
    const WbJointHandle motor = reinterpret_cast<WbJointHandle>(mJoint);

    // ODE control motor is a separate joint from the ball joint itself;
    // wrap it once for the per-axis velocity-control writes below.
    const WbJointHandle ctrl = reinterpret_cast<WbJointHandle>(mControlMotor);

    if (rm && rm->userControl())
      ode->addJointAMotorTorques(motor, -rm->rawInput(), 0.0, 0.0);
    else {
      // ODE motor velocity-control (axis 0). P1.6 slice 4 routes the
      // FMax + Vel pair through the dispatcher.
      ...
      ode->setJointAMotorParam(ctrl, 0, WbPhysicsBackend::WB_JP_FMAX, fMax);
      ode->setJointAMotorParam(ctrl, 0, WbPhysicsBackend::WB_JP_VELOCITY, currentVelocity);
    }

    if (rm2 && rm2->userControl())
      ode->addJointAMotorTorques(motor, 0.0, -rm2->rawInput(), 0.0);
    else {
      ...
      ode->setJointAMotorParam(ctrl, 1, WbPhysicsBackend::WB_JP_FMAX, fMax2);
      ode->setJointAMotorParam(ctrl, 1, WbPhysicsBackend::WB_JP_VELOCITY, currentVelocity2);
    }

    if (rm3 && rm3->userControl())
      ode->addJointAMotorTorques(motor, 0.0, 0.0, -rm3->rawInput());
    else {
      ...
      ode->setJointAMotorParam(ctrl, 2, WbPhysicsBackend::WB_JP_FMAX, fMax3);
      ode->setJointAMotorParam(ctrl, 2, WbPhysicsBackend::WB_JP_VELOCITY, currentVelocity3);
    }
...
void WbBallJoint::postPhysicsStep() {
  assert(mJoint);
  // P1.6 dispatcher routing: AMotor angle/rate reads route through
  // WbOdeBackend (ball joints are ODE-only -- Newton's arm wires only
  // revolute hinges today, no 3-DoF ball primitive). Same per-axis
  // angle-rate decorrelation + normalizeAngle dance as before, just
  // pulling the raw values through the abstract surface instead of
  // calling dJointGetAMotor* directly. WbJointHandle reinterprets to
  // dJointID under the hood.
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle motor = reinterpret_cast<WbJointHandle>(mControlMotor);
  double rate = 0.0, angle = 0.0;

  // First update the position roughly based on the angular rate of the joint so that it is within pi radians...
  ode->getJointAMotorAngleRate(motor, 0, &rate);
  mPosition -= rate * mTimeStep / 1000.0;
  // ...then refine the update to correspond to the actual measured angle (which is normalized to [-pi,pi])
  ode->getJointAMotorAngle(motor, 0, &angle);
  mPosition = WbMathsUtilities::normalizeAngle(-angle + mOdePositionOffset, mPosition);
  WbJointParameters *const p = parameters();
  if (p)
    p->setPositionFromOde(mPosition);

  ode->getJointAMotorAngleRate(motor, 1, &rate);
  mPosition2 -= rate * mTimeStep / 1000.0;
  ode->getJointAMotorAngle(motor, 1, &angle);
  mPosition2 = WbMathsUtilities::normalizeAngle(-angle + mOdePositionOffset2, mPosition2);
  WbJointParameters *const p2 = parameters2();
  if (p2)
    p2->setPositionFromOde(mPosition2);

  ode->getJointAMotorAngleRate(motor, 2, &rate);
  mPosition3 -= rate * mTimeStep / 1000.0;
  ode->getJointAMotorAngle(motor, 2, &angle);
  mPosition3 = WbMathsUtilities::normalizeAngle(-angle + mOdePositionOffset3, mPosition3);
  WbJointParameters *const p3 = parameters3();
  if (p3)
...
void WbBallJoint::applyToOdeMinAndMaxStop() {
  ...
  // place hard stops if defined (P1.6 slice 6: dispatcher-routed AMotor
  // limits per axis).
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle ctrl = reinterpret_cast<WbJointHandle>(mControlMotor);
  const WbJointParameters *const p = parameters();
  ...
  if (m != M) {
    ...
    ode->setJointAMotorParam(ctrl, 0, WbPhysicsBackend::WB_JP_LO_STOP, min);
    ode->setJointAMotorParam(ctrl, 0, WbPhysicsBackend::WB_JP_HI_STOP, max);
  }
  ...
  if (m != M) {
    ...
    WbMathsUtilities::clampAngles(min, max);
    ode->setJointAMotorParam(ctrl, 1, WbPhysicsBackend::WB_JP_LO_STOP, min);
    ode->setJointAMotorParam(ctrl, 1, WbPhysicsBackend::WB_JP_HI_STOP, max);
  }
  ...
  if (m != M) {
    ...
    WbMathsUtilities::clampAngles(min, max);
    ode->setJointAMotorParam(ctrl, 2, WbPhysicsBackend::WB_JP_LO_STOP, min);
    ode->setJointAMotorParam(ctrl, 2, WbPhysicsBackend::WB_JP_HI_STOP, max);
  }
}
