...
void WbPropeller::prePhysicsStep(double ms) {
  ...
  if (absoluteVelocity > 0.0) {
    ...
    // P1.5 widening: point-velocity at the thrust center via dispatcher.
    double vp[3];
    const double cotPos[3] = {cot.x(), cot.y(), cot.z()};
    WbPhysicsBackendRegistry::odeBackend()->getBodyPointVel(static_cast<WbBodyHandle>(b), cotPos, vp);
    ...
    // P1.5 widening: thrust + torque application via the dispatcher.
    // b is ODE-owned (propeller bodies are always on the canonical
    // backend), so the registry resolves to WbOdeBackend.
    ...
    // W3.1 (newton-ode-replacement-plan.md): if the thrust body is Newton-backed, deliver the wrench to
    // NEWTON -- its ODE body is disabled for Newton-backed Solids, so the ODE path is a no-op and the craft
    // gets no lift (the mavic freefalls). applyExternalForceNewton routes thrust@cot + reaction torque to
    // Newton's body_f and returns true; otherwise fall back to the canonical ODE path.
    if (!us->applyExternalForceNewton(thrustVector, cot, torqueVector)) {
      const double thrust[3] = {thrustVector.x(), thrustVector.y(), thrustVector.z()};
      // cotPos was computed above for the point-velocity read; reuse it.
      odeBackend->addBodyForceAtPos(static_cast<WbBodyHandle>(b), thrust, cotPos);
      const double torque[3] = {torqueVector.x(), torqueVector.y(), torqueVector.z()};
      odeBackend->addBodyTorque(static_cast<WbBodyHandle>(b), torque);
    }
