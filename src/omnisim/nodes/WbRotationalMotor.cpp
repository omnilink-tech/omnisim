...
void WbRotationalMotor::turnOffMotor() {
  ...
  // P1.6 slice 6: Hinge / Ball branches route through the dispatcher.
  // Hinge2 branch stays direct until slice 7 widens dJointSetHinge2Param.
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle h = reinterpret_cast<WbJointHandle>(jID);
  if (j->nodeType() == WB_NODE_HINGE_JOINT) {
    ...
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_FMAX, p ? p->staticFriction() : 0.0);
  } else if (j->nodeType() == WB_NODE_HINGE_2_JOINT) {
    if (this == j->motor()) {
      const WbJointParameters *const p = j->parameters();
      ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_FMAX, p ? p->staticFriction() : 0.0);
    } else if (this == j->motor2()) {
      const WbJointParameters *const p2 = j->parameters2();
      ode->setJointHinge2Param(h, 1, WbPhysicsBackend::WB_JP_FMAX, p2 ? p2->staticFriction() : 0.0);
    } else
      assert(false);
  } else if (j->nodeType() == WB_NODE_BALL_JOINT) {
    if (this == j->motor()) {
      const WbJointParameters *const p = j->parameters();
      ode->setJointAMotorParam(h, 0, WbPhysicsBackend::WB_JP_FMAX, p ? p->staticFriction() : 0.0);
    } else if (this == j->motor2()) {
      const WbJointParameters *const p2 = j->parameters2();
      ode->setJointAMotorParam(h, 1, WbPhysicsBackend::WB_JP_FMAX, p2 ? p2->staticFriction() : 0.0);
    } else if (this == j->motor3()) {
      const WbJointParameters *const p3 = j->parameters3();
      ode->setJointAMotorParam(h, 2, WbPhysicsBackend::WB_JP_FMAX, p3 ? p3->staticFriction() : 0.0);
    } else
      assert(false);
  } else
    assert(false);
