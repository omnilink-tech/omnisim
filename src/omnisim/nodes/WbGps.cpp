...
bool WbGps::refreshSensorIfNeeded() {
  ...
  WbSolid *const us = upperSolid();
  const WbBodyHandle upperHandle = us ? us->bodyHandle() : nullptr;
  if (upperHandle) {
    double newVelocity[3];
    const WbVector3 &p = position();
    const double pt[3] = {p.x(), p.y(), p.z()};
    us->physicsBackend()->getBodyPointVel(upperHandle, pt, newVelocity);
    mSpeedVector = WbVector3(newVelocity[0], newVelocity[1], newVelocity[2]);
