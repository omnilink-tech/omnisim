...
void WbTrack::prePhysicsStep(double ms) {
  ...
  if (mLinearMotor && mBodyID) {
    if (mLinearMotor->userControl()) {
      // force control. P1.5 widening: scalar mass read via dispatcher.
      double massScalar = 0.0;
      WbPhysicsBackendRegistry::odeBackend()->getBodyMass(static_cast<WbBodyHandle>(mBodyID), &massScalar);
      double force = mLinearMotor->rawInput();
      if (mBrake)
        force -= mBrake->getBrakingDampingConstant() * mSurfaceVelocity;
      mSurfaceVelocity = force * sec / massScalar;
    } else
