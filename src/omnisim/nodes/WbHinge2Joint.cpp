...
void WbHinge2Joint::applyToOdeMinAndMaxStop() {
  ...
  // P1.6 slice 7: Hinge2 world-load limits through the dispatcher.
  ...
  if (m != M) {
    ...
    ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_LO_STOP, min);
    ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_HI_STOP, max);
  }
  ...
  if (m != M) {
    ...
    WbMathsUtilities::clampAngles(min, max);
    ode->setJointHinge2Param(h, 1, WbPhysicsBackend::WB_JP_LO_STOP, min);
    ode->setJointHinge2Param(h, 1, WbPhysicsBackend::WB_JP_HI_STOP, max);
  }
}
...
void WbHinge2Joint::prePhysicsStep(double ms) {
  ...
  if (isEnabled()) {
    // P1.6 slice 7: per-step Hinge2 torque + velocity-control writes
    // through the dispatcher.
    WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
    const WbJointHandle h = reinterpret_cast<WbJointHandle>(mJoint);

    if (rm && rm->userControl()) {
      ode->addJointHinge2Torques(h, -rm->rawInput(), 0.0);
      if (rm->hasMuscles())
      ...
      // ODE motor velocity-control (axis 0)
      ...
      ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_FMAX, fMax);
      ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_VELOCITY, currentVelocity);
    }

    if (rm2 && rm2->userControl()) {
      ode->addJointHinge2Torques(h, 0.0, -rm2->rawInput());
      if (rm2->hasMuscles())
        // force is directly applied to the bodies and not included in joint motor feedback
        emit updateMuscleStretch(rm2->rawInput() / rm2->maxForceOrTorque(), false, 2);
    } else {
      // ODE motor velocity-control (axis 1)
      ...
      ode->setJointHinge2Param(h, 1, WbPhysicsBackend::WB_JP_FMAX, fMax2);
      ode->setJointHinge2Param(h, 1, WbPhysicsBackend::WB_JP_VELOCITY, currentVelocity2);
    }
