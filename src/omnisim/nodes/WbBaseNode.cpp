...
QString WbBaseNode::documentationUrl() const {
  ...
  if (!bookAndPage.isEmpty())
    return QString("%1/%2/%3.md").arg(WbStandardPaths::omniSimDocsBaseUrl()).arg(bookAndPage[0]).arg(bookAndPage[1]);
  return QString();
}
...
bool WbBaseNode::exportNodeHeader(WbWriter &writer) const {
  ...
  if (!bookAndPage.isEmpty())
    writer
      << QString(" docUrl=\'%1/%2/%3.md\'").arg(WbStandardPaths::omniSimDocsBaseUrl()).arg(bookAndPage[0]).arg(bookAndPage[1]);
