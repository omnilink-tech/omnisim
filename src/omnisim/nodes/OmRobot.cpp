...
#include "OmKinematicDifferentialWheels.hpp"
...
#include "../../../include/controller/c/omnisim/keyboard.h"
#include "../../../include/controller/c/omnisim/robot.h"
...
int OmRobot::mapSpecialKey(int qtKey) {
...
void OmRobot::init() {
  ...
  mPin = false;
  mPinTranslation = OmVector3();
  mPinRotation = OmRotation();
  ...
  OmSFString *data = findSFString("data");
...
OmRobot::OmRobot(OmTokenizer *tokenizer) : OmSolid("Robot", tokenizer) {
  init();
}

OmRobot::OmRobot(const OmRobot &other) : OmSolid(other) {
  init();
}

OmRobot::OmRobot(const OmNode &other) : OmSolid(other) {
  init();
}

OmRobot::OmRobot(const QString &modelName, OmTokenizer *tokenizer) : OmSolid(modelName, tokenizer) {
  init();
}
...
OmRobot::~OmRobot() {
  ...
  if (mMouse)
    OmMouse::destroy(mMouse);
  delete mSupervisorUtilities;
  mPressedKeys.clear();
  OmWorld::instance()->removeRobotIfPresent(this);
}

void OmRobot::preFinalize() {
  OmSolid::preFinalize();
  mBatterySensor = new OmSensor();
  mKeyboardSensor = new OmSensor();
  mJoystickSensor = new OmSensor();

  if ((OmTokenizer::worldFileVersion() < OmVersion(2020, 1, 0) ||
       (proto() && proto()->fileVersion() < OmVersion(2020, 1, 0))) &&
    ...
    QString message;
    if (OmNodeUtilities::isTemplateRegeneratorField(
  ...
  OmWorld::instance()->addRobotIfNotAlreadyPresent(this);
}

void OmRobot::postFinalize() {
  OmSolid::postFinalize();
  mKinematicDifferentialWheels = OmKinematicDifferentialWheels::createKinematicDifferentialWheelsIfNeeded(this);

  connect(mController, &OmSFString::changed, this, &OmRobot::updateControllerDir);
  connect(mWindow, &OmSFString::changed, this, &OmRobot::updateWindow);
  connect(mRemoteControl, &OmSFString::changed, this, &OmRobot::updateRemoteControl);
  connect(mCustomData, &OmSFString::changed, this, &OmRobot::updateData);
  connect(mSupervisor, &OmSFString::changed, this, &OmRobot::updateSupervisor);
  connect(this, &OmMatter::matterModelChanged, this, &OmRobot::updateModel);
  connect(OmSimulationState::instance(), &OmSimulationState::modeChanged, this, &OmRobot::updateSimulationMode);
  connect(mBattery, &OmMFDouble::itemInserted, this, [this]() { this->updateBattery(true); });
  connect(mBattery, &OmMFDouble::itemRemoved, this, [this]() { this->updateBattery(false); });
}

void OmRobot::reset(const QString &id) {
  OmSolid::reset(id);
  mPreviousTime = -1.0;
...
void OmRobot::save(const QString &id) {
  OmSolid::save(id);
  mBatteryInitialValues[id] = (mBattery->size() > CURRENT_ENERGY) ? mBattery->item(CURRENT_ENERGY) : -1.0;
}

void OmRobot::addDevices(OmNode *node) {
  if (!node)
    return;

  if (node != this && dynamic_cast<const OmRobot *>(node))
    return;  // do not recurse through child robots, their devices are hidden

  OmGroup *group = dynamic_cast<OmGroup *>(node);
  if (group) {
    // cppcheck-suppress constVariablePointer
    OmSolidDevice *solidDevice = dynamic_cast<OmSolidDevice *>(node);
    if (solidDevice) {
      mDevices.append(solidDevice);
      connect(static_cast<OmSolid *>(node), &OmSolid::destroyed, this, &OmRobot::updateDevicesAfterDestruction,
              Qt::UniqueConnection);

      OmRenderingDevice *renderingDevice = dynamic_cast<OmRenderingDevice *>(node);
      if (renderingDevice) {
        connect(renderingDevice, &OmNode::isBeingDestroyed, this, &OmRobot::removeRenderingDevice, Qt::UniqueConnection);
        mRenderingDevices.append(renderingDevice);
        OmAbstractCamera *camera = dynamic_cast<OmAbstractCamera *>(renderingDevice);
        if (camera) {
          connect(camera, &OmAbstractCamera::enabled, this, &OmRobot::updateActiveCameras, Qt::UniqueConnection);
          if (camera->isEnabled())
    ...
    const OmTrack *const track = dynamic_cast<OmTrack *>(group);
    if (track) {
      const QVector<OmLogicalDevice *> trackDevices = track->devices();
      for (int i = 0; i < trackDevices.size(); ++i) {
        mDevices.append(trackDevices[i]);
        connect(static_cast<OmBaseNode *>(trackDevices[i]), &OmBaseNode::destroyed, this,
                &OmRobot::updateDevicesAfterDestruction, Qt::UniqueConnection);
      }
    }
  ...
  const OmSlot *const slot = dynamic_cast<OmSlot *>(node);
  if (slot) {
    addDevices(slot->endPoint());
    return;
  }

  OmBasicJoint *const basicJoint = dynamic_cast<OmBasicJoint *>(node);
  if (basicJoint) {
    const OmJoint *const joint = dynamic_cast<OmJoint *>(basicJoint);
    if (joint) {
      const QVector<OmLogicalDevice *> &jointDevices = joint->devices();
      foreach (OmLogicalDevice *const jointDevice, jointDevices) {
        if (jointDevice == NULL)
          continue;
        mDevices.append(jointDevice);
        connect(static_cast<OmBaseNode *>(jointDevice), &OmBaseNode::destroyed, this, &OmRobot::updateDevicesAfterDestruction,
                Qt::UniqueConnection);
      }
    }
  ...
  OmPropeller *const propeller = dynamic_cast<OmPropeller *>(node);
  if (propeller) {
    OmLogicalDevice *const propellerDevice = propeller->device();
    if (propellerDevice) {
      mDevices.append(propellerDevice);
      connect(static_cast<OmBaseNode *>(propellerDevice), &OmBaseNode::destroyed, this, &OmRobot::updateDevicesAfterDestruction,
              Qt::UniqueConnection);
    }
    return;
  }
  ...
  if (dynamic_cast<const OmRobot *>(node)) {  // top node
    QStringList displayedWarnings;
    foreach (const OmDevice *deviceA, mDevices) {
      foreach (const OmDevice *deviceB, mDevices) {
...
void OmRobot::clearDevices() {
  mDevices.clear();
  mRenderingDevices.clear();
  mActiveCameras.clear();
}

void OmRobot::updateControllerStatusInDevices() {
  if (isBeingDeleted())
    return;
  foreach (OmDevice *const d, mDevices)
    d->setIsControllerRunning(mControllerStarted && !mControllerTerminated);
}

void OmRobot::updateDevicesAfterDestruction() {
  if (isBeingDeleted())
    return;
  clearDevices();
  addDevices(this);
...
void OmRobot::updateDevicesAfterInsertion() {
  clearDevices();
  addDevices(this);
  updateControllerStatusInDevices();
...
void OmRobot::pinToStaticEnvironment(bool pin) {
...
QString OmRobot::protoModelProjectPath() const {
  if (isProtoInstance())
...
QString OmRobot::searchDynamicLibraryAbsolutePath(const QString &key, const QString &pluginSubdirectory) {
  QString libBasename = OmStandardPaths::dynamicLibraryPrefix() + key + OmStandardPaths::dynamicLibraryExtension();

  if (!key.isEmpty()) {
    QString regularPath =
      OmProject::current()->pluginsPath() + pluginSubdirectory + "/" + key + "/build/release/" + libBasename;
    ...
    if (isProtoInstance()) {
      ...
      const OmProtoModel *protoModel = proto();
      while (protoModel) {
    ...
    OmControllerPlugin::Type t = OmControllerPlugin::pluginSubDirectoryToType(pluginSubdirectory);
    const QStringList &list = OmControllerPlugin::defaultList(t);
...
void OmRobot::updateWindow() {
  ...
  if (mAbsoluteWindowFilename.isEmpty()) {
    mAbsoluteWindowFilename = OmStandardPaths::resourcesRobotWindowsPluginsPath() + "generic/" +
                              OmStandardPaths::dynamicLibraryPrefix() + "generic" + OmStandardPaths::dynamicLibraryExtension();
    if (!QFile::exists(mAbsoluteWindowFilename))
      warn(tr("The <generic> robot window is not found. Please check your OmniSim installation."));
  }

  if (!mAbsoluteWindowFilename.isEmpty())
    OmBinaryIncubator::copyBinaryAndDependencies(mAbsoluteWindowFilename);
}

void OmRobot::updateRemoteControl() {
  ...
  if (!mAbsoluteRemoteControlFilename.isEmpty())
    OmBinaryIncubator::copyBinaryAndDependencies(mAbsoluteRemoteControlFilename);
}

void OmRobot::updateControllerDir() {
  if (controllerName().isEmpty()) {
    warn("The controller has not been set.");
    mControllerDir = "";
  } else if (controllerName() == "<generic>") {
    mControllerDir = OmStandardPaths::resourcesControllersPath() + "generic/";
  } else if (controllerName() != "<none>" && controllerName() != "<extern>") {
    QStringList path;
    path << OmProject::current()->controllersPath() + controllerName() + '/';
    const OmProtoModel *const protoModel = proto();
    if (protoModel)
    ...
    path << OmProject::defaultProject()->controllersPath() + controllerName() + '/';
    path << OmProject::system()->controllersPath() + controllerName() + '/';
  ...
  if (isPostFinalizedCalled()) {
    ...
    if (controllerName() != "<none>") {
      foreach (OmRenderingDevice *renderingDevice, mRenderingDevices) {
        OmAbstractCamera *ac = dynamic_cast<OmAbstractCamera *>(renderingDevice);
        if (ac)
...
const QString &OmRobot::controllerDir() {
...
void OmRobot::restartController() {
  mControllerStarted = false;
  emit controllerChanged();

  foreach (OmDevice *deviceObject, mDevices) {
    OmAbstractCamera *ac = dynamic_cast<OmAbstractCamera *>(deviceObject);
    if (ac)
...
bool OmRobot::isWaitingForUserInputEvent() const {
  return mMonitoredUserInputEventTypes != -1;
}

void OmRobot::setWaitingForWindow(bool waiting) {
...
void OmRobot::updateData() {
  mDataNeedToWriteAnswer = true;
}

void OmRobot::updateSupervisor() {
  mSupervisorNeedToWriteAnswer = true;
  if (mConfigureRequest) {
    delete mSupervisorUtilities;
    mSupervisorUtilities = supervisor() ? new OmSupervisorUtilities(this) : NULL;
  } else
    mSupervisorUtilitiesNeedUpdate = true;
}

void OmRobot::updateModel() {
  mModelNeedToWriteAnswer = true;
}

void OmRobot::updateBattery(bool itemInserted) {
  ...
  foreach (OmDevice *const d, mDevices) {
    ...
    OmMotor *motor = dynamic_cast<OmMotor *>(d);
    if (motor)
      motor->setupJointFeedback();
  }
}

void OmRobot::removeRenderingDevice() {
  mRenderingDevices.removeOne(dynamic_cast<OmRenderingDevice *>(sender()));
}

void OmRobot::assignDeviceTags(bool reset) {
...
double OmRobot::currentEnergy() const {
  return (mBattery->size() > CURRENT_ENERGY) ? mBattery->item(CURRENT_ENERGY) : -1.0;
}

void OmRobot::setCurrentEnergy(double e) {
  assert(e >= 0.0);
  if (!mPowerOn && e > 0)
    powerOn(true);
...
void OmRobot::postPhysicsStep() {
  ...
  if (energy > 0.0) {
    double s = OmWorld::instance()->basicTimeStep() * 0.001;
...
OmDevice *OmRobot::findDevice(WbDeviceTag tag) const {
  foreach (OmDevice *const d, mDevices)
    if (d->tag() == tag)
      return d;
...
void OmRobot::powerOn(bool e) {
  mPowerOn = e;
  foreach (OmDevice *const d, mDevices)
    d->powerOn(e);
}

void OmRobot::keyPressed(int key, int modifiers) {
...
void OmRobot::keyReleased(int key) {
  bool resetKey = true;
...
void OmRobot::writeDeviceConfigure(QList<OmDevice *> devices, OmDataStream &stream) const {
  QListIterator<OmDevice *> it(devices);
  while (it.hasNext()) {
    const OmDevice *d = it.next();
    stream << (short int)d->deviceNodeType();
    QByteArray n(d->deviceName().toUtf8());
    stream.writeRawData(n.constData(), n.size() + 1);
    const OmSolidDevice *solidDevice = dynamic_cast<const OmSolidDevice *>(d);
    if (solidDevice)
...
void OmRobot::writeConfigure(OmDataStream &stream) {
  mBatterySensor->connectToRobotSignal(this);
  stream << (short unsigned int)0;
  stream << (unsigned char)C_CONFIGURE;
  QByteArray n = name().toUtf8();
  stream.writeRawData(n.constData(), n.size() + 1);
  n = OmApplicationInfo::version().toString().toUtf8();
  stream.writeRawData(n.constData(), n.size() + 1);
  ...
  stream << (double)0.001 * OmSimulationState::instance()->time();

  writeDeviceConfigure(mDevices, stream);

  stream << (double)OmWorld::instance()->basicTimeStep();

  QString projectPath = OmProject::current()->path();
  ...
  QString worldPath = OmWorld::instance()->fileName();
  n = worldPath.toUtf8();
  stream.writeRawData(n.constData(), n.size() + 1);
...
void OmRobot::dispatchMessage(QDataStream &stream) {
  while (!stream.atEnd()) {
    WbDeviceTag tag;
    stream >> tag;
    int size;
    ...
    if (tag == 0) {
      ...
      OmDevice *const d = findDevice(tag);
      if (d) {
        const int end = stream.device()->pos() + size;
...
void OmRobot::handleMessage(QDataStream &stream) {
  ...
  switch (byte) {
    ...
    case C_ROBOT_SET_JOYSTICK_SAMPLING_PERIOD: {
      ...
      if (rate > 0) {
        if (!mJoystickInterface) {
          mJoystickInterface = new OmJoystickInterface();
          if (mJoystickInterface->isCorrectlyInitialized()) {
            mJoystickTimer = new QTimer(this);
            mJoystickTimer->setSingleShot(false);
            connect(mJoystickTimer, &QTimer::timeout, mJoystickInterface, &OmJoystickInterface::step);
        ...
        mJoystickTimer->stop();
        disconnect(mJoystickTimer, &QTimer::timeout, mJoystickInterface, &OmJoystickInterface::step);
        delete mJoystickTimer;
        delete mJoystickInterface;
        mJoystickTimer = NULL;
        mJoystickInterface = NULL;
    ...
    case C_ROBOT_WAIT_FOR_USER_INPUT_EVENT: {
      ...
      if (mMonitoredUserInputEventTypes >= 0) {
        if (!mUserInputEventTimer) {
          ...
          connect(mUserInputEventTimer, &QTimer::timeout, this, &OmRobot::userInputEventNeedUpdate);
        }
        if (((WB_EVENT_MOUSE_CLICK | WB_EVENT_MOUSE_MOVE) & mMonitoredUserInputEventTypes) && mMouse) {
          ...
          connect(mMouse, &OmMouse::changed, this, &OmRobot::handleMouseChange);
        }
        if ((WB_EVENT_KEYBOARD & mMonitoredUserInputEventTypes) && mKeyboardSensor->isEnabled()) {
          mKeyboardHasChanged = false;
          connect(this, &OmRobot::keyboardChanged, this, &OmRobot::userInputEventNeedUpdate);
        }
        if (((WB_EVENT_JOYSTICK_BUTTON | WB_EVENT_JOYSTICK_AXIS | WB_EVENT_JOYSTICK_POV) & mMonitoredUserInputEventTypes) &&
          ...
          connect(mJoystickInterface, &OmJoystickInterface::changed, this, &OmRobot::handleJoystickChange);
        }
    ...
    default:
      // if it was not catched, then this message is apparently for a subclass of OmRobot
...
void OmRobot::dispatchAnswer(OmDataStream &stream, bool includeDevices) {
  if (mConfigureRequest) {
...
QString OmRobot::encodedName() const {
  ...
  const QString &tmpPath = OmStandardPaths::webotsTmpPath();
...
void OmRobot::writeAnswer(OmDataStream &stream) {
  const double time = 0.001 * OmSimulationState::instance()->time();
  ...
  if (!mNewlyAddedDevices.isEmpty()) {
    ...
    QListIterator<OmDevice *> it(mNewlyAddedDevices);
    while (it.hasNext()) {
      OmDevice *deviceObject = it.next();
  ...
  if (mMonitoredUserInputEventTypes >= 0 && mNeedToWriteUserInputEventAnswer) {
    ...
    if (mMouse && (mMonitoredUserInputEventTypes & (WB_EVENT_MOUSE_CLICK | WB_EVENT_MOUSE_MOVE))) {
      ...
      disconnect(mMouse, &OmMouse::changed, this, &OmRobot::handleMouseChange);
    }
    if (mMonitoredUserInputEventTypes & WB_EVENT_KEYBOARD) {
      if (mKeyboardHasChanged)
        userInputEvents |= WB_EVENT_KEYBOARD;
      disconnect(this, &OmRobot::keyboardChanged, this, &OmRobot::userInputEventNeedUpdate);
    }
    if (mJoystickInterface &&
      ...
      disconnect(mJoystickInterface, &OmJoystickInterface::changed, this, &OmRobot::handleJoystickChange);
    }
  }
  ...
  if (mNeedToWriteUrdf) {
    ...
    QString urdfContent;
    OmWriter writer(&urdfContent, modelName() + ".urdf");
...
bool OmRobot::hasImmediateAnswer() const {
  if (mConfigureRequest)
    return false;
  return mShowWindowMessage || mUpdateWindowMessage || mMessageFromWwi;
}

void OmRobot::writeImmediateAnswer(OmDataStream &stream) {
  if (mConfigureRequest)
    return;
...
void OmRobot::handleMouseChange() {
  assert(mMouse);
...
void OmRobot::handleJoystickChange() {
...
QString OmRobot::windowFile(const QString &extension) const {
  if (window().isEmpty() || window() == "<generic>")
    return OmStandardPaths::resourcesRobotWindowsPluginsPath() + "generic/generic." + extension;

  const QString fileName = window() + "/" + window() + "." + extension;
  QString path = OmProject::current()->robotWindowPluginsPath() + fileName;
  QFileInfo file(path);
...
void OmRobot::showWindow() {  // show the Qt-based controller robot window (to be deprecated)
...
void OmRobot::updateControllerWindow() {  // run a html window step with duration 0 ms
  mUpdateWindowMessage = true;
  emit immediateMessageAdded();
}

void OmRobot::processImmediateMessages() {
  if (mSupervisorUtilities)
    mSupervisorUtilities->processImmediateMessages();
}

void OmRobot::startController() {
  emit startControllerRequest(this);
}

void OmRobot::receiveFromJavascript(const QByteArray &message) {
  if (mMessageFromWwi) {
...
void OmRobot::updateSimulationMode() {
  mSimulationModeRequested = true;
  emit immediateMessageAdded();
}

void OmRobot::descendantNodeInserted(OmBaseNode *decendant) {
  if (isPreFinalizedCalled())
    updateDevicesAfterInsertion();
}

void OmRobot::updateActiveCameras(OmAbstractCamera *camera, bool isActive) {
  if (isActive) {
...
bool OmRobot::refreshBatterySensorIfNeeded() {
...
bool OmRobot::refreshKeyboardSensorIfNeeded() {
...
bool OmRobot::refreshJoyStickSensorIfNeeded() {
  if ((mJoystickInterface && mJoystickInterface->isCorrectlyInitialized()) &&
...
void OmRobot::fixMissingResources() const {
  if (controllerName()[0] != '<' && mControllerDir != (OmProject::current()->controllersPath() + controllerName() + "/")) {
    mController->setValue("<generic>");
    OmLog::info(tr("The 'controller' field of the robot has been changed to \"<generic>\"."));
  }

  if (window()[0] != '<' &&
      windowFile() != (OmProject::current()->robotWindowPluginsPath() + window() + "/" + window() + ".html")) {
    mWindow->blockSignals(true);
    mWindow->setValue("<generic>");
    mWindow->blockSignals(false);
    OmLog::info(tr("The 'window' field of the robot has been changed to \"<generic>\"."));
  }
}

const QString OmRobot::urdfName() const {
  return getUrdfPrefix() + QString("base_link");
}

int OmRobot::computeSimulationMode() {
...
void OmRobot::notifyExternControllerChanged() {
  foreach (OmRenderingDevice *renderingDevice, mRenderingDevices) {
    OmAbstractCamera *ac = dynamic_cast<OmAbstractCamera *>(renderingDevice);
    if (ac)
...
void OmRobot::newRemoteExternController() {
  foreach (OmRenderingDevice *renderingDevice, mRenderingDevices) {
    OmAbstractCamera *ac = dynamic_cast<OmAbstractCamera *>(renderingDevice);
    if (ac)
...
void OmRobot::removeRemoteExternController() {
  foreach (OmRenderingDevice *renderingDevice, mRenderingDevices) {
    OmAbstractCamera *ac = dynamic_cast<OmAbstractCamera *>(renderingDevice);
    if (ac)
