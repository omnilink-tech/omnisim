...
WbWorld::WbWorld(WbTokenizer *tokenizer) :
  ...
  if (tokenizer) {
    ...
    // read/create nodes
    if (log)
      log->startMeasure(WbPerformanceLog::LOADING_PARSE);
    WbNodeReader reader;
    ...
    if (log)
      log->stopMeasure(WbPerformanceLog::LOADING_PARSE);
    if (WbApplication::instance()->wasWorldLoadingCanceled()) {
      mWorldLoadingCanceled = true;
      return;
    }
    if (log)
      log->startMeasure(WbPerformanceLog::LOADING_INSTANTIATE);
    ...
    foreach (WbNode *node, nodes) {
      ...
      if (WbApplication::instance()->wasWorldLoadingCanceled()) {
        mWorldLoadingCanceled = true;
        if (log)
          log->stopMeasure(WbPerformanceLog::LOADING_INSTANTIATE);
        return;
      }
...
void WbWorld::finalize() {
  ...
  // authoritative (the REAL WbSolid resolution, not a Python re-derivation that would drift) and COMPLETE
