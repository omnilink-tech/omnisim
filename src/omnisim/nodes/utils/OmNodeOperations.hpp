...
#ifndef OM_NODE_OPERATIONS_HPP
#define OM_NODE_OPERATIONS_HPP
...
class OmNodeOperations : public QObject {
  Q_OBJECT

public:
  static OmNodeOperations *instance();
  static void cleanup();
  ...
  OperationResult importNode(OmNode *parentNode, OmField *field, int itemIndex, ImportType origin,
                             const QString &nodeString = "", bool avoidIntersections = false);

  OperationResult initNewNode(OmNode *newNode, OmNode *parentNode, OmField *field, int newNodeIndex = -1,
                              bool subscribe = false, bool finalize = true);

  bool deleteNode(OmNode *node, bool fromSupervisor = false);

  void notifyNodeAdded(OmNode *node);
  void notifyNodeDeleted(OmNode *node);
  void notifyNodeRegenerated();

  bool updateDictionary(bool load, OmBaseNode *protoRoot);  // called after every modification of the Scene Tree
...
public slots:
  ...
  void requestUpdateSceneDictionary(OmNode *node, bool fromUseToDef);

  // add missing EXTERNPROTO declarations after modifying parameters
  void updateExternProtoDeclarations(OmField *modifiedField);

signals:
  void nodeAdded(OmNode *node);
  void nodeDeleted(OmNode *node);

private:
  static OmNodeOperations *cInstance;
  OmNodeOperations();
  ~OmNodeOperations() {}
...
private slots:
  void resolveSolidNameClashIfNeeded(OmNode *node) const;
};
