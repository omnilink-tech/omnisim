...
void WbSupervisorUtilities::handleMessage(QDataStream &stream) {
  ...
  switch (byte) {
    ...
    case C_SUPERVISOR_SET_LABEL: {
      ...
      QString filename = WbStandardPaths::shippedFontFile(font);
      if (!filename.isEmpty())
        fileFound = true;
      else {
      ...
      if (!fileFound) {
        ...
        filename = WbStandardPaths::fontsPath() + "LiberationSans-Regular.ttf";
      }
    ...
    case C_SUPERVISOR_NODE_ADD_FORCE: {
      ...
      if (solid) {
        ...
        if (body) {
          // Newton-backed Solids can't accept direct body force; the
          // dBodyID here is just a collision bridge proxy. Warn the
          // user and skip rather than apply impotent force to the
          // proxy.
          if (solid->physicsBackend()->kind() != WbPhysicsBackendKind::Ode) {
            // W3.1: route to Newton's external-force path -- force at the node's global COM, no extra torque.
            if (!solid->applyExternalForceNewton(force, solid->computedGlobalCenterOfMass(), WbVector3()))
            ...
            WbPhysicsBackend *const backend = solid->physicsBackend();
            backend->addBodyForceAtRelPos(static_cast<WbBodyHandle>(body), f, p);
            backend->setBodyEnabled(static_cast<WbBodyHandle>(body), true);
          }
        } else
    ...
    case C_SUPERVISOR_NODE_ADD_FORCE_WITH_OFFSET: {
      ...
      if (solid) {
        ...
        if (body) {
          if (solid->physicsBackend()->kind() != WbPhysicsBackendKind::Ode) {
            // W3.1: route to Newton -- force at the world offset point (applyExternalForceNewton turns it
            // into the equivalent COM force + offset torque).
            const WbMatrix4 &solidMatrixN = solid->matrix();
            const WbVector3 offsetW = solidMatrixN * WbVector3(ox, oy, oz);
            WbVector3 forceW(fx, fy, fz);
            if (relative == 1)
              forceW = solidMatrixN.extracted3x3Matrix() * forceW;
            if (!solid->applyExternalForceNewton(forceW, offsetW, WbVector3()))
            ...
            backend->setBodyEnabled(static_cast<WbBodyHandle>(body), true);
            const double f[3] = {force.x(), force.y(), force.z()};
            const double o[3] = {offset.x(), offset.y(), offset.z()};
            backend->addBodyForceAtPos(static_cast<WbBodyHandle>(body), f, o);
          }
        } else
    ...
    case C_SUPERVISOR_NODE_ADD_TORQUE: {
      ...
      if (solid) {
        ...
        // Polymorphic dispatch via WbSolid::bodyHandle. ODE-backed
        ...
        const WbBodyHandle h = solid->bodyHandle();
        if (h) {
          WbPhysicsBackend *const backend = solid->physicsBackend();
          backend->setBodyEnabled(h, true);
          // W3.1: a pure torque -> Newton's external-wrench path (force=0, so the reference point is moot).
          // For non-Newton Solids applyExternalForceNewton returns false and we keep the ODE/dispatch path
          // (which is byte-identical for ODE, and warns for any other backend that lacks body torque).
          if (!solid->applyExternalForceNewton(WbVector3(), solid->matrix().translation(), torque)) {
