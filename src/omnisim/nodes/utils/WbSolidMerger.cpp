...
void WbSolidMerger::setOdeDamping() {
  ...
  odeBackend->setBodyMaxAngularSpeed(static_cast<WbBodyHandle>(mBody), MAX_ANGULAR_SPEED);
  ...
  if (damping) {
    odeBackend->setBodyDamping(static_cast<WbBodyHandle>(mBody), l, a);
    return;
  }

  // global (world) damping default
  odeBackend->setBodyDampingDefaults(static_cast<WbBodyHandle>(mBody));
}

void WbSolidMerger::setGeomAndBodyPositions(bool zeroVelocities, bool resetJoints) {
  updateCenterOfMass();
  // P1.5 widening: position + rotation set via the dispatcher. Rotation
  // arrives as a dMatrix3 (3x4 row-major) -- we convert to quaternion
  // via dQfromR so the dispatcher stays single-form (the abstract
  // interface uses quaternions, since they're more compact and what
  // the eventual Newton/Vulkan paths expect). dReal=double in the
  // OmniSim default build makes this a lossless conversion.
  WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
  const double pos[3] = {mAbsoluteCenterOfMass.x(), mAbsoluteCenterOfMass.y(), mAbsoluteCenterOfMass.z()};
  odeBackend->setBodyPosition(static_cast<WbBodyHandle>(mBody), pos);
  ...
  dQuaternion qOde;
  dQfromR(qOde, m);
  const double q[4] = {qOde[0], qOde[1], qOde[2], qOde[3]};
  odeBackend->setBodyQuaternion(static_cast<WbBodyHandle>(mBody), q);
  // Sets the offset position (with respect to the ODE dBody) of every ODE dGeom in the boundingObject
  setGeomOffsetPositions();

  // Wake up (P1.5 widening: through the dispatcher).
  if (!mBodyArtificiallyDisabled)
    odeBackend->setBodyEnabled(static_cast<WbBodyHandle>(mBody), true);

  // Resets velocities as if the object was moved gently (P1.5 widening:
  // through the function-scope odeBackend from above).
  if (zeroVelocities) {
    const double zero[3] = {0.0, 0.0, 0.0};
    odeBackend->setBodyLinearVel(static_cast<WbBodyHandle>(mBody), zero);
    odeBackend->setBodyAngularVel(static_cast<WbBodyHandle>(mBody), zero);
  }
...
void WbSolidMerger::setupOdeBody() {
  ...
  setGeomAndBodyPositions();
  // P1.5 widening: velocity set flows through the dispatcher. The Solid
  // wraps WbVector3; pass through stack arrays into the backend API.
  WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
  const WbVector3 &l = mSolid->linearVelocity();
  const double linVel[3] = {l.x(), l.y(), l.z()};
  odeBackend->setBodyLinearVel(static_cast<WbBodyHandle>(mBody), linVel);
  const WbVector3 &a = mSolid->angularVelocity();
  const double angVel[3] = {a.x(), a.y(), a.z()};
  odeBackend->setBodyAngularVel(static_cast<WbBodyHandle>(mBody), angVel);
}
...
void WbSolidMerger::setOdeAutoDisable() {
  ...
  if (t > 0.0) {
    odeBackend->setBodyAutoDisableFlag(static_cast<WbBodyHandle>(mBody), true);
    odeBackend->setBodyAutoDisableLinearThreshold(static_cast<WbBodyHandle>(mBody),
                                                  wi->physicsDisableLinearThreshold());
    odeBackend->setBodyAutoDisableAngularThreshold(static_cast<WbBodyHandle>(mBody),
                                                   wi->physicsDisableAngularThreshold());
    odeBackend->setBodyAutoDisableTime(static_cast<WbBodyHandle>(mBody), t);
  } else
    odeBackend->setBodyAutoDisableFlag(static_cast<WbBodyHandle>(mBody), false);
}
...
void WbSolidMerger::setBodyArtificiallyDisabled(bool disabled) {
  ...
  WbPhysicsBackend *const odeBackend = WbPhysicsBackendRegistry::odeBackend();
  if (disabled) {
    odeBackend->setBodyEnabled(static_cast<WbBodyHandle>(mBody), false);
    dGeomID g = mSolid->odeGeom();
    if (g) {
