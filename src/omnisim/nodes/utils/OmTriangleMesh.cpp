...
#include "OmRay.hpp"
#include "OmTesselator.hpp"

#include <cassert>
#include <limits>

OmTriangleMesh::OmTriangleMesh() {
  cleanup();
}
...
void OmTriangleMesh::cleanupTmpArrays() {
...
QString OmTriangleMesh::init(const OmMFVector3 *coord, const OmMFInt *coordIndex, const OmMFVector3 *normal,
                             const OmMFInt *normalIndex, const OmMFVector2 *texCoord, const OmMFInt *texCoordIndex,
...
QString OmTriangleMesh::init(const double *coord, const double *normal, const double *texCoord, const unsigned int *index,
...
void OmTriangleMesh::indicesPass(const OmMFVector3 *coord, const OmMFInt *coordIndex, const OmMFInt *normalIndex,
                                 const OmMFInt *texCoordIndex) {
  ...
  for (int i = 0; i < coordIndexSize; ++i) {
    ...
    if (id == -1 || i == coordIndexSize - 1) {
      ...
      if (currentFaceValidity) {
        ...
        QList<OmVector3> tesselatorVectorInput;
        ...
        const QString warning(OmTesselator::tesselate(currentFaceIndices, tesselatorVectorInput, tesselatorOutput));
        if (!warning.isEmpty())
        ...
        // left by the now-missing triangle back together to re-close the polygon
          ...
          for (int j = 0; j < toSize; j += 3) {
            const OmVector3 &a = coord->item(tesselatorOutput[j][0]);
            const OmVector3 &b = coord->item(tesselatorOutput[j + 1][0]);
            const OmVector3 &c = coord->item(tesselatorOutput[j + 2][0]);

            // check for colinear edges, and discard this triangle if found
            const OmVector3 d = b - a;
            const OmVector3 e = c - b;
            // don't append if edges are co-linear
            if (d.cross(e).almostEquals(OmVector3(), 0.00001))
              continue;
...
QList<QVector<int>> OmTriangleMesh::cutTriangleIfNeeded(const OmMFVector3 *coord, const QList<QVector<int>> &tesselatedPolygon,
  ...
  const OmVector3 &firstEdgeStart = coord->item(firstVertexIndex);
  const OmVector3 &firstEdgeEnd = coord->item(secondVertexIndex);

  const OmVector3 &secondEdgeStart = coord->item(secondVertexIndex);
  const OmVector3 &secondEdgeEnd = coord->item(thirdVertexIndex);

  const OmVector3 &thirdEdgeStart = coord->item(thirdVertexIndex);
  const OmVector3 &thirdEdgeEnd = coord->item(firstVertexIndex);
...
QString OmTriangleMesh::tmpNormalsPass(const OmMFVector3 *coord, const OmMFVector3 *normal) {
  ...
  for (int i = 0; i < mNTriangles; ++i) {
    if (mNormalsValid)
      ...
      const OmVector3 &posA = coord->item(indexA);
      const OmVector3 &posB = coord->item(indexB);
      const OmVector3 &posC = coord->item(indexC);

      const OmVector3 &v1 = posB - posA;
      const OmVector3 &v2 = posC - posA;
      OmVector3 n(v1.cross(v2));
...
void OmTriangleMesh::setDefaultTextureCoordinates(const OmMFVector3 *coord) {
  const OmVector3 minBound(min(X), min(Y), min(Z));
  const OmVector3 maxBound(max(X), max(Y), max(Z));
  const OmVector3 size(maxBound - minBound);
  ...
  for (int t = 0; t < mNTriangles; ++t) {  // foreach triangle
    ...
    const OmVector3 edge1(vertices[1] - vertices[0]);
    const OmVector3 edge2(vertices[2] - vertices[0]);
    OmVector3 normalVector(edge1.cross(edge2));
    normalVector.normalize();
    const OmVector3 origin((vertices[0] + vertices[1] + vertices[2]) / 3.0);

    // compute intersection with the bounding box
    const OmRay faceNormal(origin, normalVector);
    double tmin, tmax;
    const std::pair<bool, double> result = faceNormal.intersects(minBound, maxBound, tmin, tmax);
    assert(result.first);
    const int faceIndex = OmBox::findIntersectedFace(minBound, maxBound, origin + result.second * normalVector);
    ...
    for (int v = 0; v < 3; ++v) {  // foreach vertex
      ...
      const OmVector2 uv(OmBox::computeTextureCoordinate(minBound, maxBound, vertices[v], true, faceIndex));
...
void OmTriangleMesh::finalPass(const OmMFVector3 *coord, const OmMFVector3 *normal, const OmMFVector2 *texCoord,
                               double creaseAngle) {
  ...
  OmVector3 vertexVector = coord->item(0);
  ...
  for (int t = 0; t < mNTriangles; ++t) {  // foreach triangle
    ...
    for (int v = 0; v < 3; ++v) {  // foreach vertex
      ...
      if (!mNormalsValid || !mNormalPerVertex) {
        OmVector3 triangleNormal;
        const OmVector3 &faceNormal = mTmpTriangleNormals[t];
        ...
        const OmVector3 **linkedTriangleNormals = new const OmVector3 *[ltSize];
        ...
        for (int j = 0; j < ltSize; ++j) {
          ...
          if (linkedTriangleIndex >= 0 && linkedTriangleIndex < mNTriangles) {
            const OmVector3 &linkedTriangleNormal = mTmpTriangleNormals[linkedTriangleIndex];
            ...
            if (faceNormal.angle(linkedTriangleNormal) < creaseAngle) {
              ...
              for (int lN = 0; lN < linkedTriangleNormalsIndex; ++lN) {
                const OmVector3 *currentLinkedTriangleNormal = linkedTriangleNormals[lN];
        ...
        if (indexNormal >= 0 && indexNormal < normalSize) {
          const OmVector3 nor(normal->item(indexNormal));
      ...
      if (mTextureCoordinatesValid) {
        ...
        if (indexTex >= 0 && indexTex < texCoordSize) {
          const OmVector2 tex(texCoord->item(indexTex));
...
// ]
int OmTriangleMesh::estimateNumberOfTriangles(const OmMFInt *coordIndex) {
  assert(coordIndex);

  OmMFInt::Iterator coordIndexIt(coordIndex);
  int nTriangles = 0;
...
void OmTriangleMesh::updateScaledCoordinates(double x, double y, double z) {
