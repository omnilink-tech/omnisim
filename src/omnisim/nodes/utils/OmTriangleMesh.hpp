...
#ifndef OM_TRIANGLE_MESH_HPP
#define OM_TRIANGLE_MESH_HPP
...
public:
  OmTriangleMesh();
  virtual ~OmTriangleMesh();

  // to be initialized from a OmIndexedFaceSet
  QString init(const OmMFVector3 *coord, const OmMFInt *coordIndex, const OmMFVector3 *normal, const OmMFInt *normalIndex,
               const OmMFVector2 *texCoord, const OmMFInt *texCoordIndex, double creaseAngle, bool normalPerVertex);
  // to be initialized from a OmMesh
...
private:
  static int estimateNumberOfTriangles(const OmMFInt *coordIndex);
  ...
  void indicesPass(const OmMFVector3 *coord, const OmMFInt *coordIndex, const OmMFInt *normalIndex,
  ...
  QString tmpNormalsPass(const OmMFVector3 *coord, const OmMFVector3 *normal);
  // contains normal vectors for each triangle (match with the mTmpCoordIndices order)
  QVarLengthArray<OmVector3, 1> mTmpTriangleNormals;
  ...
  void finalPass(const OmMFVector3 *coord, const OmMFVector3 *normal, const OmMFVector2 *texCoord, double creaseAngle);
  // populate mTextureCoordinates with default values
  void setDefaultTextureCoordinates(const OmMFVector3 *coord);
  ...
  QList<QVector<int>> cutTriangleIfNeeded(const OmMFVector3 *coord, const QList<QVector<int>> &tesselatedPolygon,
