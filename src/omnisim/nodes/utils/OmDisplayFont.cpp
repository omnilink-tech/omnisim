...
void OmDisplayFont::loadFallbackFaces(unsigned int size) {
  static QStringList fontFilenames;
  if (fontFilenames.isEmpty()) {
    QString fallbackFontsFilename = OmStandardPaths::fontsPath() + "fallback_fonts.txt";
    ...
    if (fallbackFontsFile.open(QIODevice::ReadOnly)) {
      ...
      while (!in.atEnd()) {
        ...
        if (!line.startsWith("#") && !line.isEmpty()) {
          QString fontPath = OmStandardPaths::fontsPath() + QString(line) + ".ttf";
...
void OmDisplayFont::cleanupFallbackFaces() {
  foreach (const FT_Face face, mFallbackFaces)
    FT_Done_Face(face);
  mFallbackFaces.clear();
}

OmDisplayFont::OmDisplayFont() : mFallbackFacesSize(0), mFaceIsInitialized(false), mError(""), mFontSize(0) {
  if (FT_Init_FreeType(&mLibrary))
    mError = "An error occurred while initializing the freetype library.";
}

OmDisplayFont::~OmDisplayFont() {
  FT_Done_Face(mFace);
  cleanupFallbackFaces();
  FT_Done_FreeType(mLibrary);
}

unsigned char *OmDisplayFont::generateCharBuffer(unsigned long character, bool antiAliasing, int *width, int *rows,
...
void OmDisplayFont::setFont(const QString &filename, unsigned int size) {
  mFontSize = size;
...
void OmDisplayFont::loadFace(FT_Face *face, const QString &filename, unsigned int size) {
  mError = "";
  mFontSize = size;
...
unsigned int OmDisplayFont::verticalSpace() const {
