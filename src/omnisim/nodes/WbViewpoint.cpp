...
// Viewpoint's draw submission through WbRenderBackendRegistry::resolve.
// Mirrors WbSolid::physicsBackendName / physicsBackend exactly.
const QString &WbViewpoint::renderBackendName() const {
...
WbRenderBackend *WbViewpoint::renderBackend() const {
  ...
  // explicit per-node value ("wren"/"wgpu") is used as-is. Inert today: the node default is the
  // concrete "wren", so this branch is only taken once a node opts into the "auto"/empty sentinel.
