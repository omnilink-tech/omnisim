...
protected:
  ...
  // Virtual so subclasses (WbCamera) can intercept the main-image
  // copy path and route through an alternate backend (R3.3b: wgpu).
  // The default impl is the legacy WREN copy. Subclasses are
  // responsible for distinguishing the main camera from secondary
  // cameras (e.g. segmentation) by inspecting the `camera` arg.
  virtual void copyImageToMemoryMappedFile(WbWrenCamera *camera, unsigned char *data);
