...
#ifndef OM_PHYSICS_HPP
#define OM_PHYSICS_HPP
...
class OmPhysics : public OmBaseNode {
  Q_OBJECT
...
public:
  ...
  explicit OmPhysics(OmTokenizer *tokenizer = NULL);
  OmPhysics(const OmPhysics &other);
  explicit OmPhysics(const OmNode &other);
  virtual ~OmPhysics() override;
...
private:
  ...
  OmSFDouble *mDensity;
  OmSFDouble *mMass;
  OmMFVector3 *mCenterOfMass;
  OmMFVector3 *mInertiaMatrix;
  OmSFNode *mDamping;

  OmPhysics &operator=(const OmPhysics &);  // non copyable
  OmNode *clone() const override { return new OmPhysics(*this); }
  void init();
  bool mSkipUpdate;
