...
OmLookupTable::OmLookupTable(const OmMFVector3 &lookupTable) {
  ...
  if (mSize) {
    ...
    int j = 0;
    OmMFVector3::Iterator it(lookupTable);
    while (it.hasNext()) {
      const OmVector3 &v = it.next();
      mInputs[j] = v.x();
      if (j > 0 && mInputs[j - 1] >= mInputs[j])
        OmLog::warning(QObject::tr("The input values of a lookup table must be sorted in increasing order."), false,
                       OmLog::PARSING);
      mOutputs[j] = v.y();
      mNoises[j] = v.z();
      j++;
...
OmLookupTable::~OmLookupTable() {
  delete[] mInputs;
  delete[] mOutputs;
  delete[] mNoises;
}

double OmLookupTable::lookup(double value) const {
  ...
  if (noise != 0.0)
    return v1 * (1.0 + noise * OmRandom::nextGaussian());
  else
    return v1;
}
