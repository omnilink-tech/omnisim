...
void WbBackground::activate() {
  ...
  connect(mAtmosphericSky, &WbSFString::changed, this, &WbBackground::updateCubemap);
...
void WbBackground::applySkyBoxToWren() {
  ...
  // Hillaire 2020 atmospheric path.  When the ``atmosphericSky`` field
  // is non-empty AND the preset name is recognised, replace the
  // cubemap skybox with the procedural sky pipeline and skip the rest
  // of this function entirely.  Falls back to the cubemap path on any
  // failure so worlds that mistype the preset still get a visible sky.
  const QString preset = mAtmosphericSky ? mAtmosphericSky->value().trimmed() : QString();
  if (!preset.isEmpty()) {
    if (!mAtmosphericSkyImpl)
      mAtmosphericSkyImpl = new WbWrenAtmosphericSky();
    if (mAtmosphericSkyImpl->setPreset(preset)) {
      // Pick a sun direction AND a sun illuminance from the first
      // DirectionalLight.  The shader expects TOWARD-sun, while
      // WbDirectionalLight.direction is the direction light travels —
      // so we negate.  The light's `color` is also lifted into
      // sunIlluminance so the user gets a single intuitive control:
      // set DirectionalLight.color (1, 1, 1) → neutral-white sun and
      // a neutral-white sky-IBL contribution; set (1.0, 0.6, 0.3) →
      // warm sunset tint propagated through the entire atmospheric
      // pipeline.  Magnitude is anchored to the per-preset reference
      // illuminance (Earth ≈ 22, Mars ≈ 9) so existing worlds that
      // didn't author a custom DL colour keep the same overall sky
      // brightness as before.  If the world has no DirectionalLight,
      // the impl's preset defaults stand.
      //
      // Also wire signal connections so any subsequent edit to the
      // DirectionalLight's direction, colour, or intensity triggers
      // an automatic re-bake of the IBL cube — no need to toggle
      // atmosphericSky by hand.  Qt::UniqueConnection deduplicates so
      // re-entering applySkyBoxToWren doesn't stack connections.
      const QList<const WbLight *> lights = WbLight::lights();
      for (const WbLight *light : lights) {
        const WbDirectionalLight *dirLight = dynamic_cast<const WbDirectionalLight *>(light);
        if (dirLight && light->isOn()) {
          const WbVector3 &d = dirLight->direction();
          mAtmosphericSkyImpl->setSunDirection(QVector3D(-d.x(), -d.y(), -d.z()));
          const WbRgb &c = dirLight->color();
          const float refLuma = (preset == "mars") ? 9.0f : 22.0f;
          mAtmosphericSkyImpl->setSunIlluminance(QVector3D(
            static_cast<float>(c.red())   * refLuma,
            static_cast<float>(c.green()) * refLuma,
            static_cast<float>(c.blue())  * refLuma));

          connect(dirLight, &WbLight::directionChanged, this, &WbBackground::updateCubemap,
                  Qt::UniqueConnection);
          connect(dirLight, &WbLight::colorChanged, this, &WbBackground::updateCubemap,
                  Qt::UniqueConnection);
          connect(dirLight, &WbLight::intensityChanged, this, &WbBackground::updateCubemap,
                  Qt::UniqueConnection);
          break;
        }
      }
      WbWrenOpenGlContext::makeWrenCurrent();
      mAtmosphericSkyImpl->update();
      // Procedural PBR irradiance: bake the sky-view LUT into a cube
      // and pre-filter it.  WbPbrAppearance picks this up via
      // ``WbBackground::irradianceCubeTexture()`` so every PBR
      // material in the scene reflects the procedural sky rather
      // than a static cubemap.  Ownership transfers from the impl to
      // this WbBackground, so the regular ``destroySkyBox()``
      // lifecycle frees it on next rebuild.  See T1.3 session 5 in
      // docs/developer/rendering-roadmap.md §7.2.
      //
      // Procedural PBR irradiance.  After fixing sky_view.frag to
      // drop the multi-scattering term (which was blue-shifting Mars
      // because Mars's strong Mie scattering kills red in the sun
      // transmittance through the atmosphere column), the sky-view
      // LUT is warm-toned, so the cubemap baked from it is also
      // warm — rocks reflect Mars-rust instead of olive-green.
      // Bake BEFORE bindForSkybox so the cubemap baker's raw-GL
      // texture-unit shuffling doesn't pollute the sky-view LUT's
      // binding on the skybox material.
      mIrradianceCubeTexture = mAtmosphericSkyImpl->bakeAndReleaseIrradianceCubeMap();
      if (mIrradianceCubeTexture)
        wr_texture_cubemap_disable_automatic_mip_map_generation(mIrradianceCubeTexture);
      mAtmosphericSkyImpl->bindForSkybox(mSkyboxMaterial);
      emit cubemapChanged();
    ...
    // Unknown preset: drop the impl and fall through to the cubemap
    // path so the world still renders, with a one-time warning.
    delete mAtmosphericSkyImpl;
    mAtmosphericSkyImpl = NULL;
    static bool warned = false;
    if (!warned) {
      WbLog::instance()->warning(
        tr("Background.atmosphericSky: unrecognised preset '%1'; falling back to cubemap skybox.").arg(preset));
      warned = true;
    }
  } else if (mAtmosphericSkyImpl) {
    // Field cleared at runtime: tear down the impl so memory frees.
    delete mAtmosphericSkyImpl;
    mAtmosphericSkyImpl = NULL;
    wr_material_set_default_program(mSkyboxMaterial, mSkyboxShaderProgram);
  }
