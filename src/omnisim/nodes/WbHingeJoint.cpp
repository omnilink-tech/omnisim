...
void WbHingeJoint::applyToOdeMinAndMaxStop() {
  ...
  if (m != M) {
    ...
    // P1.6 slice 6: Hinge LoStop/HiStop through the dispatcher.
    ...
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_LO_STOP, min);
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_HI_STOP, max);
  }
}
...
void WbHingeJoint::applyToOdeStopErp() {
  ...
  // P1.6 slices 6/7: both Hinge and Hinge2 routes go through the dispatcher.
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle h = reinterpret_cast<WbJointHandle>(mJoint);
  if (nodeType() == WB_NODE_HINGE_JOINT)
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_STOP_ERP, erp);
  if (nodeType() == WB_NODE_HINGE_2_JOINT)
    ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_STOP_ERP, erp);
}
...
void WbHingeJoint::applyToOdeStopCfm() {
  ...
  const WbJointHandle h = reinterpret_cast<WbJointHandle>(mJoint);
  if (nodeType() == WB_NODE_HINGE_JOINT)
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_STOP_CFM, cfm);
  if (nodeType() == WB_NODE_HINGE_2_JOINT)
    ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_STOP_CFM, cfm);
}
...
void WbHingeJoint::applyToOdeSpringAndDampingConstants(dBodyID body, dBodyID parentBody) {
  ...
  // Stops + spring/damper config on the AMotor companion joint
  // (P1.6 slice 6: dispatcher-routed).
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
...
void WbHingeJoint::applyToOdeSuspension() {
  ...
  // P1.6 slices 6/7: Hinge + Hinge2 suspension params through the dispatcher.
  WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
  const WbJointHandle h = reinterpret_cast<WbJointHandle>(mJoint);
  if (nodeType() == WB_NODE_HINGE_2_JOINT) {
    ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_SUSPENSION_ERP, erp);
    ode->setJointHinge2Param(h, 0, WbPhysicsBackend::WB_JP_SUSPENSION_CFM, cfm);
  } else {
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_SUSPENSION_ERP, erp);
    ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_SUSPENSION_CFM, cfm);
  }
...
void WbHingeJoint::prePhysicsStep(double ms) {
  ...
  if (isEnabled()) {
    if (rm && rm->userControl()) {
      // user-defined torque (dispatcher P1.6 slice 3: routes through
      // WbOdeBackend::addJointHingeTorque; Newton-backed joints don't
      // reach this prePhysicsStep because WbBasicJoint takes over their
      // motor control upstream).
      const double torque = rm->rawInput();
      WbPhysicsBackendRegistry::odeBackend()->addJointHingeTorque(
        reinterpret_cast<WbJointHandle>(mJoint), mIsReverseJoint ? torque : -torque);
      if (rm->hasMuscles())
      ...
      // ODE motor velocity-control (FMax + Vel pair). P1.6 slice 4:
      // routes through the dispatcher's setJointHingeParam. Newton-
      // backed joints never reach prePhysicsStep (WbBasicJoint owns
      // their motor control), so this is ODE-only by control flow.
      ...
      WbPhysicsBackend *const ode = WbPhysicsBackendRegistry::odeBackend();
      const WbJointHandle h = reinterpret_cast<WbJointHandle>(mJoint);
      ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_FMAX, fMax);
      ode->setJointHingeParam(h, WbPhysicsBackend::WB_JP_VELOCITY, currentVelocity);
    }
...
void WbHingeJoint::postPhysicsStep() {
  ...
  // Newton-backed joints have their own truth for the joint angle that
  // ODE's hinge doesn't reflect (parallel sims diverge). Without this
  // override, the scene tree renders the kinematic chain at ODE's
  // angles while the Solid's mTranslation is at Newton's pose -- the
  // mismatch makes the visual meshes detach from each other ("body
  // parts falling off"). Query Newton's angle first; fall through to
  // ODE only when no Newton joint is registered.
  //
  // P1.6 of the engine master plan formalises this routing through
  // WbPhysicsBackend's getJointHingeAngle: Newton's override packs the
  // integer joint index into the abstract joint handle and ODE's
  // override calls dJointGetHingeAngle on the dJointID handle. The
  // ODE-side angle-rate correction stays inline because that
  // normalisation is ODE-specific (Newton's joint_q is already wrapped)
  // -- the dispatcher exposes the raw rate via getJointHingeAngleRate
  // and the wrap-then-renormalise stays a per-callsite concern.
  bool angleFromNewton = false;
  if (mNewtonJointIndex >= 0) {
    WbPhysicsBackend *const newton = WbPhysicsBackendRegistry::newtonBackend();
    if (newton != nullptr && newton->isAvailable()) {
      double angle = 0.0;
      if (newton->getJointHingeAngle(WbNewtonBackend::handleFromIndex(mNewtonJointIndex), &angle) == 0) {
        mPosition = angle;
        angleFromNewton = true;
      }
    }
  }
  if (!angleFromNewton) {
    ...
    ode->getJointHingeAngleRate(reinterpret_cast<WbJointHandle>(mJoint), &angleRate);
    if (mIsReverseJoint)
      angleRate = -angleRate;
    ...
    double angle = 0.0;
    ode->getJointHingeAngle(reinterpret_cast<WbJointHandle>(mJoint), &angle);
    if (!mIsReverseJoint)
      angle = -angle;
