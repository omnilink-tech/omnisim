...
void WbSimulationCluster::odeNearCallback(void *data, dGeomID o1, dGeomID o2) {
  ...
  if (!s1 || !s2) {
    ...
    if ((s1 && f2) || (s2 && f1)) {
      ...
      // Mutex IS necessary: ODE dispatches per-pair contact callbacks across
      // multiple threads when WbOdeContext::setNumberOfThreads(n>=2) calls
      // dToggleODE_MT(n). dImmersionLinkGroup is not thread-safe; this lock
      // guards dImmersionLinkCreate against concurrent writes from sibling
      // callbacks. (TODO resolved 2026-05-27.)
      cJointCreationMutex->lock();
  ...
  // P1.5 widening: enable/disable + read state all flow through the
  // dispatcher. b1/b2 are ODE bodies (this is the ODE collision callback);
  // the registry resolves to WbOdeBackend.
  ...
  if (odeGeomData1->lastChangeTime() > 0.0) {
    if (odeGeomData1->lastChangeTime() >= WbSimulationState::instance()->time()) {
      if (b2)
        odeBackend->setBodyEnabled(static_cast<WbBodyHandle>(b2), true);
    } else
  ...
  if (odeGeomData2->lastChangeTime() > 0.0) {
    if (odeGeomData2->lastChangeTime() >= WbSimulationState::instance()->time()) {
      if (b1)
        odeBackend->setBodyEnabled(static_cast<WbBodyHandle>(b1), true);
    } else
  ...
  const bool b1Disabled = b1 && odeBackend->isBodyEnabled(static_cast<WbBodyHandle>(b1)) == 0;
  const bool b2Disabled = b2 && odeBackend->isBodyEnabled(static_cast<WbBodyHandle>(b2)) == 0;
