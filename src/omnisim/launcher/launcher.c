...
// Description:  This is the source code of webots.exe and webotsw.exe, the binary launchers of omnisim-bin.exe (Windows only).
//               It adds paths to the PATH environment variable to ensure that the correct libraries will be found and starts
//               omnisim-bin.exe, passing all the command line arguments, wait until the completion of omnisim-bin.exe and
//               returns the exit status code of omnisim-bin.exe.
...
int main(int argc, char *argv[]) {
  ...
  // Replace the launcher filename (webots.exe / webotsw.exe) with the
  // canonical binary name omnisim-bin.exe. The launcher's own filename
  // remains webots.exe / webotsw.exe so the PATH-construction math
  // (`\webots.exe` = 11 chars) below stays correct.
  const int last_slash = l - 11;  // points to '\' before "webots.exe"
  wcsncpy(command_line, module_path, last_slash + 1);
  command_line[last_slash + 1] = L'\0';
  ...
  while (1) {
    ...
    // omnisim-bin.exe should be killed whenever its parent (webots.exe or webotsw.exe) terminates.
