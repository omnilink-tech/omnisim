...
void OmRotation::fromQuaternion(const OmQuaternion &q) {
  ...
  if (q.w() >= 1.0)
    ...
    mAngle = 2.0 * acos(q.w());
  if (mAngle < OmPrecision::DOUBLE_EQUALITY_TOLERANCE) {
...
void OmRotation::fromMatrix3(const OmMatrix3 &M) {
  // Reference: https://www.geometrictools.com/Documentation/RotationRepresentations.pdf
  const double theta = OmMathsUtilities::clampedAcos((M(0, 0) + M(1, 1) + M(2, 2) - 1) / 2);
  if (theta < OmPrecision::DOUBLE_EQUALITY_TOLERANCE) {  // If `theta == 0`
    mX = 1;
    mY = 0;
    mZ = 0;
    mAngle = 0;
    return;
  } else if (M_PI - theta < OmPrecision::DOUBLE_EQUALITY_TOLERANCE) {  // If `theta == pi`
...
void OmRotation::fromBasisVectors(const OmVector3 &vx, const OmVector3 &vy, const OmVector3 &vz) {
...
void OmRotation::toFloatArray(float *rotation) const {
...
OmVector3 OmRotation::direction() const {
  const double c = cos(mAngle), s = sin(mAngle), t = 1 - c;
  const double tTimesX = t * mX;
  return OmVector3(tTimesX * mX + c, tTimesX * mY + s * mZ, tTimesX * mZ - s * mY);
}

OmVector3 OmRotation::right() const {
  const double c = cos(mAngle), s = sin(mAngle), t = 1 - c;
  const double tTimesY = t * mY;
  return OmVector3(tTimesY * mX - s * mZ, tTimesY * mY + c, tTimesY * mZ + s * mX);
}

OmVector3 OmRotation::up() const {
  const double c = cos(mAngle), s = sin(mAngle), t = 1 - c;
  const double tTimesZ = t * mZ;
  return OmVector3(tTimesZ * mX + s * mY, tTimesZ * mY - s * mX, tTimesZ * mZ + c);
}
