...
#define ORDER(k, l)                                                   \
  ...
  void straightInsertionSort(std::vector<int> &indices, const std::vector<double> &cosinus, const std::vector<double> &distance) {
  ...
  // not
  bool rightTurn(const OmVector2 &A, const OmVector2 &B, const OmVector2 &C) {
...
bool OmMathsUtilities::isPowerOf2(unsigned int n) {
  return (n != 0) && ((n & (n - 1)) == 0);
}

unsigned int OmMathsUtilities::nextPowerOf2(unsigned int n) {
  n--;
  n |= n >> 1;
  n |= n >> 2;
  n |= n >> 4;
...
int OmMathsUtilities::round(double n) {
  return floor(n + 0.5);
}
...
int OmMathsUtilities::convexHull(const std::vector<OmVector2> &points, std::vector<int> &hullIndices) {
...
#pragma GCC diagnostic pop
  ...
  std::vector<double> cosinus(size);
...
int OmMathsUtilities::twoStepsConvexHull(const std::vector<OmVector2> &points, std::vector<int> &hullIndices) {
  std::vector<int> filteredIndices(points.size());
  ...
  std::vector<OmVector2> rotatedPoints(size);
  // We rotate the convex hull of +pi/2 before performing another Graham scan
  for (int i = 0; i < size; ++i) {
    const OmVector2 &v = points.at(filteredIndices.at(i));
...
bool OmMathsUtilities::isConvex(const OmPolygon &p) {
...
void OmMathsUtilities::orthoBasis(const OmVector3 &vY, OmVector3 b[3]) {
  b[Y] = vY.normalized();
...
bool OmMathsUtilities::isZeroAngle(double angle) {
...
void OmMathsUtilities::clampAngles(double &min, double &max) {
  normalizeAngle(min);
  normalizeAngle(max);
  if (min > max) {  // swap
    const double m = max;
...
bool OmMathsUtilities::isZeroVector3(const double *v) {
  return v[0] == 0.0 && v[1] == 0.0 && v[2] == 0.0;
}

void OmMathsUtilities::printVector3(const char *str, const double *v) {
  printf("%s %g %g %g\n", str, v[0], v[1], v[2]);
}

void OmMathsUtilities::printMatrix3x4(const char *str, const double *m) {
  printf("%s:\n", str);
  printf("%g %g %g %g\n", m[0], m[1], m[2], m[3]);
  printf("%g %g %g %g\n", m[4], m[5], m[6], m[7]);
  printf("%g %g %g %g\n", m[8], m[9], m[10], m[11]);
...
////////////////////////////////////////////////////////////////////////
bool OmMathsUtilities::computeRationalApproximation(double value, int maxDenominator, int &numerator, int &denominator) {
  double x = value;
  long ai = 0;
