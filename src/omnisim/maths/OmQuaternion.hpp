...
#ifndef OM_QUATERNION_HPP
#define OM_QUATERNION_HPP
...
public:
  ...
  OmQuaternion() : mW(1.0), mX(0.0), mY(0.0), mZ(0.0) {}

  // construct from other quaternion
  OmQuaternion(const OmQuaternion &other) : mW(other.mW), mX(other.mX), mY(other.mY), mZ(other.mZ) {}
  OmQuaternion(double w, double x, double y, double z) : mW(w), mX(x), mY(y), mZ(z) {}
  explicit OmQuaternion(const double q[4]) : mW(q[0]), mX(q[1]), mY(q[2]), mZ(q[3]) {}
  OmQuaternion(double w, const double q[3]) : mW(w), mX(q[0]), mY(q[1]), mZ(q[2]) {}
  OmQuaternion(double w, const OmVector3 &v) {
    mW = w;
    mX = v.x();
    mY = v.y();
    mZ = v.z();
  }
  OmQuaternion(const OmVector3 &vx, const OmVector3 &vy, const OmVector3 &vz) { fromBasisVectors(vx, vy, vz); }

  // construct from Euler axis and angle
  OmQuaternion(const OmVector3 &r, double angle) { fromAxisAngle(r.x(), r.y(), r.z(), angle); }
  ...
  double dot(const OmQuaternion &other) const { return mW * other.w() + mX * other.x() + mY * other.y() + mZ * other.z(); }
  ...
  static OmQuaternion slerp(const OmQuaternion &a, const OmQuaternion &b, double slerpAmount);

  // assignment: Q1 = Q2
  OmQuaternion &operator=(const OmQuaternion &q) {
    mW = q.mW;
    mX = q.mX;
    mY = q.mY;
    mZ = q.mZ;
  ...
  OmQuaternion operator*(const OmQuaternion &q) const;
  OmQuaternion &operator*=(const OmQuaternion &q) {
    *this = *this * q;
    return *this;
  }
  ...
  OmQuaternion conjugated() const { return OmQuaternion(mW, -mX, -mY, -mZ); }

  // assign from other types of rotation
  void fromAxisAngle(double rx, double ry, double rz, double angle);
  void fromBasisVectors(const OmVector3 &vx, const OmVector3 &vy, const OmVector3 &vz);
  void fromEulerAngles(double yaw, double pitch, double roll);

  // comparison operators
  bool operator==(const OmQuaternion &q) const { return mW == q.mW && mX == q.mX && mY == q.mY && mZ == q.mZ; }
  bool operator!=(const OmQuaternion &q) const { return mW != q.mW || mX != q.mX || mY != q.mY || mZ != q.mZ; }
  ...
  friend QTextStream &operator<<(QTextStream &stream, const OmQuaternion &q);

private:
  double mW, mX, mY, mZ;
};

inline void OmQuaternion::fromAxisAngle(double rx, double ry, double rz, double angle) {
...
inline void OmQuaternion::fromBasisVectors(const OmVector3 &vx, const OmVector3 &vy, const OmVector3 &vz) {
...
inline QTextStream &operator<<(QTextStream &stream, const OmQuaternion &q) {
  stream << q.toString(OmPrecision::DOUBLE_MAX);
  return stream;
}

inline OmQuaternion OmQuaternion::operator*(const OmQuaternion &q) const {
  return OmQuaternion(mW * q.mW - mX * q.mX - mY * q.mY - mZ * q.mZ, mW * q.mX + mX * q.mW + mY * q.mZ - mZ * q.mY,
                      mW * q.mY + mY * q.mW + mZ * q.mX - mX * q.mZ, mW * q.mZ + mZ * q.mW + mX * q.mY - mY * q.mX);
}

inline OmVector3 OmQuaternion::operator*(const OmVector3 &v) const {
  ...
  return OmVector3(m00 * v.x() + m01 * v.y() + m02 * v.z(), m10 * v.x() + m11 * v.y() + m12 * v.z(),
                   m20 * v.x() + m21 * v.y() + m22 * v.z());
}

inline void OmQuaternion::normalize() {
...
inline OmQuaternion OmQuaternion::slerp(const OmQuaternion &a, const OmQuaternion &b, double slerpAmount) {
  double alpha;
  double beta;
  ...
  if (dotProduct > 0.999999f) {
    ...
    double theta = OmMathsUtilities::clampedAcos(dotProduct);
  ...
  return OmQuaternion(newW, newX, newY, newZ);
}
