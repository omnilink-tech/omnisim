...
void OmRandom::setSeed(unsigned int s) {
  gW = s;
  gZ = 362436069;  // default value
}

unsigned int OmRandom::getSeed() {
  return gW;
}

unsigned int OmRandom::nextUInt() {
  gZ = 36969 * (gZ & 65535) + (gZ >> 16);
  gW = 18000 * (gW & 65535) + (gW >> 16);
  return (gZ << 16) + gW;
}

double OmRandom::nextUniform() {
