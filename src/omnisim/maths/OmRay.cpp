...
std::pair<bool, double> OmRay::intersects(const OmAffinePlane &p, bool testCull) const {
...
std::pair<bool, double> OmRay::intersects(const OmVector3 &vert0, const OmVector3 &vert1, const OmVector3 &vert2, bool testCull,
                                          double &u, double &v) const {
  // find vectors for two eges sharing vert0
  OmVector3 edge1 = vert1 - vert0;
  OmVector3 edge2 = vert2 - vert0;

  // begin calculating determinant - also used to calculate U parameter
  OmVector3 pvec = direction().cross(edge2);
  ...
  if (testCull) {
    ...
    OmVector3 qvec = tvec.cross(edge1);
...
std::pair<bool, double> OmRay::intersects(const OmVector3 &center, double radius, bool testCull) const {
  // calculate distance
  OmVector3 q = origin() - center;
...
std::pair<bool, double> OmRay::intersects(const OmVector3 &minBound, const OmVector3 &maxBound, double &tMin,
                                          double &tMax) const {
  OmVector3 bounds[2];
  bounds[0] = minBound;
  bounds[1] = maxBound;
  OmVector3 invDirection(1.0 / mDirection.x(), 1.0 / mDirection.y(), 1.0 / mDirection.z());
...
OmRay OmRay::transformed(const OmMatrix4 &matrix) const {
  return OmRay(matrix * mOrigin, matrix.sub3x3MatrixDot(mDirection));
}
