...
#ifndef OM_WREN_RENDERING_CONTEXT_HPP
#define OM_WREN_RENDERING_CONTEXT_HPP
...
class OmWrenRenderingContext : public QObject {
  Q_OBJECT
...
public:
  ...
  enum {
    ...
    VM_MAIN = 0xFFFFFFFE,
    VM_OMNISIM_CAMERA = 0xFFFA0000,       // mask for OmCamera (all WREN MovableObjects)
    VM_OMNISIM_RANGE_CAMERA = 0xFFF80000  // mask for OmRangeFinder and OmLidar (all WREN MovableObjects except laser beam)
  };
  ...
  static OmWrenRenderingContext *instance() {
    if (!cRenderingContext)
      cRenderingContext = new OmWrenRenderingContext(0, 0);
...
private:
  OmWrenRenderingContext(int width, int height);
  ~OmWrenRenderingContext();

  OmWrenRenderingContext(const OmWrenRenderingContext &);             // Prevent copy-construction
  OmWrenRenderingContext &operator=(const OmWrenRenderingContext &);  // Prevent assignment

  static OmWrenRenderingContext *cRenderingContext;
