...
#ifndef OM_WGPU_SHADERS_HPP
#define OM_WGPU_SHADERS_HPP

//
// OmWgpuShaders — R3.4 of engine-migration-plan.md §14.3.
//
...
// OmWgpuRenderTarget::clearAndDrawTriangle path works end-to-end:
...
// the layout OmWgpuMeshCache uploads (pos3 + norm3 + uv2 = 32 bytes
...
// OmWgpuMeshCache uploads and WREN's mesh-adapter (R3.2b) will
...
extern const char *kSkyScatterLut;
...
// as kSkyAtmosphere, but sampling a texture_cube with WREN's (x, y, -z) convention instead of the
// analytic palette. Drawn when the Background carries a complete image cubemap and no recognised
// atmosphericSky preset (preset > cubemap > flat clear, mirroring OmBackground::applySkyBoxToWren).
extern const char *kSkyCubemap;
...
// GTAO (WREN-parity AO): horizon-based estimate + depth-aware denoise, sampling the scene's
// own MSAA depth attachment (no dedicated prepass; normals derived from depth). kSsaoEstimate
// above remains the legacy curvature kernel, selected by OMNISIM_WGPU_GTAO=0 (exact-revert hatch).
extern const char *kSsaoGtao;
...
// P7 (WREN retirement) -- HUD QUAD: the device-inset / label / status compositing primitive for
// the wgpu MAIN VIEW. Same NDC sub-rect idea as kTexturedQuad, but with the three things a real
// HUD needs and that one does not have: a per-quad TINT/alpha (so one pipeline draws a camera
// inset, a translucent label background and a solid border), PREMULTIPLIED output so the caller
// can alpha-blend One/OneMinusSrcAlpha, and a dynamic-offset uniform so N quads are N draws
// against one buffer instead of N bind groups. The texture is sampled with a NON-filtering
// sampler on purpose: a camera inset is device pixels and bilinear smearing would misrepresent
// what the controller actually read.
extern const char *kHudQuad;
...
// cursor, and decode the picked draw. The wgpu analog of OmWrenPicker's "picking"
...
// identical (golden-image-gated); opt-in via OmCamera OMNISIM_CAMERA_AGX=1.
...
// OmWgpuSceneRenderer::buildCascadeLightViewProjs. Carries an array<mat4x4,4> of per-cascade
...
extern const char *kSolidClipDepthSharedVP;
