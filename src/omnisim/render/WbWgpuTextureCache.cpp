...
WbWgpuTextureCache::WbWgpuTextureCache(WbVulkanBackend *owner) : mOwner(owner) {
}

WbWgpuTextureCache::~WbWgpuTextureCache() {
#ifdef OMNISIM_WITH_VULKAN
#  ifdef WB_WGPU_NATIVE_AVAILABLE
...
void WbWgpuTextureCache::evictToBudget() {
#ifdef OMNISIM_WITH_VULKAN
#  ifdef WB_WGPU_NATIVE_AVAILABLE
...
bool WbWgpuTextureCache::tryGet(uint64_t textureId, WbWgpuTextureHandle &out) {
...
WbWgpuTextureHandle WbWgpuTextureCache::acquire(uint64_t textureId, uint32_t width, uint32_t height,
                                                const void *rgba8, size_t rgba8Len) {
  auto it = mEntries.find(textureId);
  if (it != mEntries.end()) {
    it->second.lastUsed = ++mClock;  // LRU: mark hot
    const Entry &e = it->second;
    return {e.texture, e.view, e.width, e.height};
  }
...
#  ifdef WB_WGPU_NATIVE_AVAILABLE
  ...
  if (!rgba8 || rgba8Len < static_cast<size_t>(width) * height * 4u || width == 0 || height == 0) {
    WbLog::info(QString("[WbWgpuTextureCache] acquire(%1) bad args; refusing").arg(textureId));
    return {nullptr, nullptr, 0, 0};
  }
  ...
  if (!tex) {
    WbLog::info(QString("[WbWgpuTextureCache] CreateTexture failed for id %1").arg(textureId));
    return {nullptr, nullptr, 0, 0};
  }
  ...
  if (!view) {
    WbLog::info(QString("[WbWgpuTextureCache] CreateView failed for id %1").arg(textureId));
  ...
  // count climb without bound (one upload per WbImageTexture instance) → VRAM OOM. The running
  // `entries`/`totalMB` show the LRU budget holding. Gated by OMNISIM_WGPU_TEXLOG=<file>; inert
  // otherwise. See WbWgpuSceneRenderer::stableTexId.
...
void WbWgpuTextureCache::release(uint64_t textureId) {
  auto it = mEntries.find(textureId);
  if (it == mEntries.end())
    return;
#ifdef OMNISIM_WITH_VULKAN
