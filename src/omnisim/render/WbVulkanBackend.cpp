...
namespace {
  ...
  // windows-subsystem GUI binary). Logs via WbLog AND, if OMNISIM_WGPU_ERRLOG is set,
  ...
  void onUncapturedError(WGPUDevice const * /*device*/, WGPUErrorType type, WGPUStringView message,
    ...
    const QString msg = QString("[WbWgpuBackend] WGPU-ERROR type=%1: %2")
...
#  ifdef WB_WGPU_NATIVE_AVAILABLE
  ...
  // reached to that file (the WbLog lines below are invisible on a windows-
  ...
  if (!instance) {
    ...
    WbLog::info("[WbWgpuBackend] wgpuCreateInstance returned NULL; falling back to WREN");
    return;
  }
  note("instance OK");
  ...
  // SILENTLY (WbLog::info is invisible on the windows-subsystem binary).
  ...
  for (int attempt = 0; attempt < kMaxAttempts && !mAvailable; ++attempt) {
    ...
    // ~WbVulkanBackend). Queue fetched eagerly -- every upload/submit needs it.
    mInstance = instance;
    mAdapter = aCap.adapter;
    mDevice = dCap.device;
    mQueue = wgpuDeviceGetQueue(dCap.device);
    WbLog::info("[WbWgpuBackend] wgpu-native init OK (instance + adapter + device + queue)");
  ...
  if (!mAvailable) {
    ...
    // the windows-subsystem binary (where WbLog::info is not).
    fprintf(stderr,
            "[WbWgpuBackend] wgpu init FAILED after %d attempts; falling back to WREN. "
...
WbVulkanBackend::~WbVulkanBackend() {
#ifdef OMNISIM_WITH_VULKAN
#  ifdef WB_WGPU_NATIVE_AVAILABLE
