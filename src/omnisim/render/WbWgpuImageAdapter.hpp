...
#ifndef WB_WGPU_IMAGE_ADAPTER_HPP
#define WB_WGPU_IMAGE_ADAPTER_HPP

//
// WbWgpuImageAdapter — R3.5b of engine-migration-plan.md §14.3.
//
// Glue layer between Qt's QImage (which is where OmniSim's
// WbImageTexture keeps the original RGBA pixel data — `mImage`
// stays alive after wren::Texture2d destroys its own staging copy
// at GPU upload time) and WbWgpuTextureCache (which wants tightly
// packed RGBA8Unorm bytes).
//
// Source of truth: WbImageTexture loads ARGB32 (Format_ARGB32) at
// `WbImageTexture::createImage` and we hold a `QImage *mImage` for
// the rest of the node's lifetime. The adapter converts to
// Format_RGBA8888 (tight memory-order RGBA on all endians) and
// hands the bytes to WbWgpuTextureCache::acquire.
//
...
// QImage* or the WbImageTexture* as uint64_t for stability.
//
