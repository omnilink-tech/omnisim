...
struct UrdfInertial {
  ...
  // Webots' Physics.inertiaMatrix (2-line MFVector3: [Ixx Iyy Izz], [Ixy Ixz Iyz])
...
// a child Solid currently crashes Webots' device registration mid-startup
...
void warnUnsupportedGeometry(const QString &linkName, const QString &context, const UrdfGeometry &geometry) {
  if (geometry.kind == "mesh" && geometry.meshPath.isEmpty()) {
    WbLog::warning(QObject::tr("URDF link '%1': %2 mesh '%3' could not be resolved and will be skipped.")
  ...
  if (geometry.kind == "unsupported") {
    WbLog::warning(QObject::tr("URDF link '%1': unsupported %2 geometry <%3>; this element will be skipped.")
                     .arg(linkName, context, geometry.detail),
                   false, WbLog::PARSING);
  }
}
...
// with forward slashes, suitable for inlining into a Webots Mesh { url "..." }
...
UrdfLink parseLink(const QDomElement &elem, QHash<QString, UrdfMaterial> &materials, const QString &urdfDir) {
  ...
  // Multiple <collision> elements are combined into a single Webots
  ...
  if (!inertEl.isNull()) {
    ...
    if (!inertiaEl.isNull()) {
      ...
      // Only pass the tensor through if it's PD; Webots would reject it
      ...
      if (inertiaIsPositiveDefinite(ixx, ixy, ixz, iyy, iyz, izz)) {
        ...
        WbLog::warning(QObject::tr("URDF link '%1': inertia tensor is not positive definite "
...
UrdfJoint parseJoint(const QDomElement &elem) {
  ...
  // position (overrides Webots' default-to-midpoint when 0 is outside
...
bool parseRobotXml(const QByteArray &xml, const QString &sourceLabel, const QString &urdfDir, UrdfRobot &robot) {
  ...
  if (!doc.setContent(xml, &errorMsg, &errorLine)) {
    WbLog::error(QObject::tr("URDF parse error in '%1' at line %2: %3")
                   .arg(sourceLabel)
                   .arg(errorLine)
                   .arg(errorMsg));
    return false;
  ...
  if (root.tagName() != "robot") {
    WbLog::error(QObject::tr("URDF '%1': expected <robot> root, got <%2>")
                   .arg(sourceLabel)
                   .arg(root.tagName()));
    return false;
  }
...
QString emitVisual(const UrdfVisual &v, const QString &indent, const QString &linkName) {
  ...
  // 25k+ triangles); Webots can't cast shadows above ~21k triangles and
  ...
  out += innerIndent + "    roughness 0.5\n";
  out += innerIndent + "    metalness 0\n";
  out += innerIndent + "  }\n";
...
// Webots does not honour scale on a Transform inside a boundingObject. If a
...
void warnMeshCollisionScale(const QString &linkName, const UrdfGeometry &g) {
  if (meshHasNonUnitScale(g)) {
    WbLog::warning(QObject::tr("URDF link '%1': collision mesh '%2' has scale %3 %4 %5 which cannot be applied "
...
// collision origin is identity. Webots' bare-Cylinder boundingObject path on
...
// projects/samples/devices/worlds/imu.wbt. This is the only structure
// observed to load reliably: bare WbSolidDevice nodes as direct children of
...
QString emitLinkPhysics(const UrdfLink &link, const QString &indent, bool allowSyntheticPhysics = true) {
  ...
  if (supported.size() == 1) {
    ...
    // Webots needs a boundingObject to compute an inertia matrix; a link with
  ...
  // Webots treats a Solid with boundingObject but no Physics as unable to
  ...
  if (link.inertial.present || needsSyntheticPhysics) {
    ...
    // Webots refuses the custom inertiaMatrix unless mass > 0 AND a single-line
    ...
    // and let Webots derive inertia from the bounding object.
    ...
    if (hasUrdfInertia) {
      // Webots inertiaMatrix layout: line 1 = [Ixx Iyy Izz], line 2 = [Ixy Ixz Iyz].
...
void appendJointPhysicsParameters(QString &out, const UrdfJoint &joint, const QString &indent) {
  ...
  // limits by definition. Webots also requires the stops to be strictly
  ...
  if (joint.type == "revolute" && joint.hasLower && joint.hasUpper) {
    ...
    if (!fullRange) {
      ...
      if (hi > lo) {
        ...
        // Webots defaults the initial position to 0. If the clamped range
        // excludes 0, the joint starts outside its stops and Webots warns
...
void logJointDebugInfo(const UrdfJoint &joint, double parentAx, double parentAy, double parentAz) {
  ...
  WbLog::info(message, false, WbLog::PARSING);
}
...
QString emitJoint(const UrdfJoint &j, const QString &indent,
  ...
  if (j.type == "revolute" || j.type == "continuous") {
    ...
    // skipped by appendJointPhysicsParameters (Webots requires stops strictly
    // inside (-pi, pi)). Under Newton that left them with NO limit signal, so
    // the ke/kd discriminator (WbBasicJoint::flushPendingNewtonRegistrations)
...
QString emitRobot(const UrdfRobot &robot) {
  ...
  if (rootName.isEmpty()) {
    WbLog::error(QObject::tr("URDF '%1': no root link found").arg(robot.name));
    return QString();
  }
  if (rootCandidates.size() > 1) {
    WbLog::warning(QObject::tr("URDF '%1': multiple root links found (%2). Using '%3'.")
                     .arg(robot.name)
  ...
  if (rerooted && urdfDebugEnabled()) {
    WbLog::info(QObject::tr("URDF_DEBUG robot '%1': re-rooted to '%2' (skipping empty frame links)")
  ...
  // Webots requires the Robot root to have Physics if any descendant Solid
  ...
  // cleanly with Webots' device manager). Non-fixed joints break the chain
  ...
  for (const UrdfSensor &s : robot.sensors) {
    if (!robot.links.contains(s.linkName)) {
      WbLog::warning(QObject::tr("URDF '%1': sensor '%2' references unknown link '%3'; skipping.")
  ...
  if (urdfDebugEnabled() && !robot.sensors.isEmpty()) {
    ...
    WbLog::info(QObject::tr("URDF_DEBUG robot '%1': sensors=[%2]")
  ...
  // insertion path appears to corrupt something Webots reads downstream
...
QString WbUrdfImporter::convertContent(const QByteArray &xmlContent, const QString &sourceLabel) {
  UrdfRobot robot;
  ...
  if (xacroVar.match(QString::fromUtf8(processed)).hasMatch()) {
    ...
    WbLog::warning(QObject::tr("URDF '%1': stripped unexpanded xacro ${...} "
...
QString WbUrdfImporter::convertFile(const QString &urdfPath) {
  QFile file(urdfPath);
  if (!file.open(QIODevice::ReadOnly)) {
    WbLog::error(QObject::tr("Cannot open URDF file '%1'").arg(urdfPath));
    return QString();
  }
...
QByteArray WbUrdfImporter::expandUrdfRobotBlocks(const QByteArray &worldContents,
  ...
  while (true) {
    ...
    if (urlField.isEmpty()) {
      WbLog::error(QObject::tr("URDFRobot block at offset %1 has no 'url' field").arg(markerPos));
      searchStart = q + 1;
      continue;
    }
    ...
    QString vrml = WbUrdfImporter::convertFile(urdfPath);
    if (vrml.isEmpty()) {
      searchStart = q + 1;
      continue;
    }
    ...
    if (!controllerArgsField.isEmpty()) {
      ...
      // emitted Robot block from WbUrdfImporter::convertFile() never
    ...
    // WbSolid::effectivePhysicsBackendName then propagates the field
    ...
    // Webots reads a Robot with no physics field as a kinematic root, which is
    ...
    if (urdfDebugEnabled()) {
      const QString dumpPath = QDir::temp().filePath(
        QString("omnisim_urdf_%1.wbt.snippet").arg(QFileInfo(urdfPath).baseName()));
      QFile dumpFile(dumpPath);
      if (dumpFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        dumpFile.write(vrml.toUtf8());
        dumpFile.close();
        WbLog::info(QObject::tr("URDF_DEBUG: wrote expanded VRML for '%1' to %2").arg(urdfPath, dumpPath));
      }
    }
  }
  ...
  if (urdfDebugEnabled()) {
    const QString dumpPath = QDir::temp().filePath("omnisim_world_postexpand.wbt");
    QFile dumpFile(dumpPath);
    if (dumpFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
      dumpFile.write(result);
      dumpFile.close();
      WbLog::info(QObject::tr("URDF_DEBUG: wrote post-expansion world to %1").arg(dumpPath));
    }
  }
  return result;
}
