...
#ifndef OM_TOKENIZER_HPP
#define OM_TOKENIZER_HPP

//
// OmTokenizer: provides the functionality to split a text file into VRML-based tokens
//
...
public:
  ...
  // in `contents` by splicing the included file's body inline. Paths
  // resolve relative to `currentFilePath` (or absolute, or use the
  // omnisim:// scheme). The first-pass `#VRML_SIM ...utf8` header of
  // each included file is stripped before splicing. Tracks already-
  // included canonical paths in `visited` to break circular includes;
  // hard cap of 16 recursion levels. Returns the expanded byte array.
  // Used by tokenize() before any other content massaging.
  static QByteArray expandWorldIncludes(const QByteArray &contents,
  ...
  static const OmVersion &worldFileVersion();
  ...
  OmToken *peekToken() const { return mVector.at(mIndex); }
  ...
  // report an error to OmLog about the last token consumed with nextToken() or nextWord()
