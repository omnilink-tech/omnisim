...
void OmSFRotation::readSFRotation(OmTokenizer *tokenizer, const QString &worldPath) {
  try {
...
void OmSFRotation::setValue(const OmRotation &v) {
  if (mValue == v)
    return;
...
void OmSFRotation::setValue(double x, double y, double z, double angle) {
...
void OmSFRotation::setValueByUser(const OmRotation &v, bool changedFromSupervisor) {
  if (mValue == v)
    return;
...
double OmSFRotation::component(int index) const {
  switch (index) {
    case 0:
      return mValue.x();
    case 1:
...
void OmSFRotation::setComponent(int index, double d) {
...
OmSFRotation &OmSFRotation::operator=(const OmSFRotation &other) {
...
bool OmSFRotation::equals(const OmValue *other) const {
  const OmSFRotation *that = dynamic_cast<const OmSFRotation *>(other);
  return that && *this == *that;
}

void OmSFRotation::copyFrom(const OmValue *other) {
  const OmSFRotation *that = dynamic_cast<const OmSFRotation *>(other);
  *this = *that;
}
