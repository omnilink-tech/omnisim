...
#ifndef OM_PROTO_MANAGER_HPP
#define OM_PROTO_MANAGER_HPP
...
class OmExternProto {
public:
  OmExternProto(const QString &name, const QString &url, bool isImportable) :
    mName(name),
    mUrl(url),
...
class OmProtoInfo {
public:
  OmProtoInfo(const QString &url, const QString &baseType, const QString &license, const QString &licenseUrl,
  ...
  // copy constructor
  OmProtoInfo(const OmProtoInfo &other) :
    OmProtoInfo(other.mUrl, other.mBaseType, other.mLicense, other.mLicenseUrl, other.mDocumentationUrl, other.mDescription,
...
class OmProtoManager : public QObject {
  Q_OBJECT
public:
  static OmProtoManager *instance();

  // used to reference the different PROTO lists and by the dialog windows (OmAddNodeDialog, OmNewProtoWizard,
  // OmInsertExternProtoDialog) to categorize their respective PROTO
  enum { BASE_NODE = 10001, PROTO_WORLD = 10002, PROTO_PROJECT = 10003, PROTO_EXTRA = 10004, PROTO_OMNISIM = 10005 };
  ...
  const QList<OmProtoModel *> &models() const { return mModels; }
  ...
  // 2. If the first fails, it searches among the known official OmniSim PROTO (proto-list.xml)
  OmProtoModel *findModel(const QString &modelName, const QString &worldPath, const QString &parentFilePath,
                          const QStringList &baseTypeList = QStringList());

  OmProtoModel *readModel(const QString &url, const QString &worldPath, const QString &prefix = QString(),
  ...
  const QMap<QString, OmProtoInfo *> &omniSimProtoList() { return mOmniSimProtoList; };

  // generates meta info from a PROTO file (license, tags, ...)
  OmProtoInfo *generateInfoFromProtoFile(const QString &protoFileName);
  ...
  const QMap<QString, OmProtoInfo *> &protoInfoMap(int category) const;
  const OmProtoInfo *protoInfo(const QString &protoName, int category);
  ...
  const QVector<OmExternProto *> &externProto() const { return mExternProto; };
  ...
  QString externProtoUrl(const OmNode *node, bool formatted = false) const;
  QString removeProtoUrl(const OmNode *node, bool formatted = false) const;

  QString findExternProtoDeclarationInFile(const QString &url, const QString &modelName);
  void removeImportableExternProto(const QString &protoName, OmNode *root);

  bool isImportableExternProtoDeclared(const QString &protoName);

  // EXTERNPROTO stored after copying or cutting an inserted node
  const QList<OmExternProto *> &externProtoClipboardBuffer() const { return mExternProtoClipboardBuffer; };
  void saveToExternProtoClipboardBuffer(const QList<const OmNode *> &nodes);
...
private:
  ...
  Q_DISABLE_COPY(OmProtoManager)

  QList<OmProtoModel *> mModels;
  // Fingerprints for parsed immutable PROTO models.  Unchanged local models
  // survive a world reload; edited files are evicted before the new parse.
  QHash<const OmProtoModel *, QByteArray> mModelFingerprints;
  ...
  QVector<OmExternProto *> mExternProto;

  // mExternProtoClipboardBuffer: contains the externProto reference of the last copies or cut instance
  QList<OmExternProto *> mExternProtoClipboardBuffer;

  // stores PROTO metadata
  QMap<QString, OmProtoInfo *> mOmniSimProtoList;     // loaded from proto-list.xml
  QMap<QString, OmProtoInfo *> mWorldFileProtoList;  // compiled from EXTERNPROTO referenced in .wbt
  QMap<QString, OmProtoInfo *> mProjectProtoList;    // compiled from PROTO in current project directory
  QMap<QString, OmProtoInfo *> mExtraProtoList;      // compiled from PROTO in extra project directories
  ...
  OmVersion checkProtoVersion(const QString &protoUrl, bool *foundProtoVersion);
  void cleanup();
};
