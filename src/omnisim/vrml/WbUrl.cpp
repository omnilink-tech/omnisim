    ...
    if (protoNode) {
      ...
      if (protoNode->proto()->isDerived()) {
        ...
        if (isWeb(protoUrl)) {
          ...
          QRegularExpression re(remoteWebotsAssetRegex(false));
          QString localPath = protoUrl;
          localPath.replace(re, WbStandardPaths::webotsHomePath());
...
QString WbUrl::resolveUrl(const QString &rawUrl) {
  ...
  if (isLocalUrl(url))
    return QDir::cleanPath(url.replace("omnisim://", WbStandardPaths::webotsHomePath()));
...
bool WbUrl::isLocalUrl(const QString &url) {
  return url.startsWith("omnisim://") ||
         WbFileUtil::isLocatedInInstallationDirectory(url, true);
}

QString WbUrl::computeLocalAssetUrl(QString url, bool isW3d) {
  if (!isW3d)
    return url.replace(WbStandardPaths::webotsHomePath(), "omnisim://");
  ...
  if (!WbApplicationInfo::repo().isEmpty() && !WbApplicationInfo::branch().isEmpty()) {
    ...
    return url.replace("omnisim://", prefix).replace(WbStandardPaths::webotsHomePath(), prefix);
  }
...
const QString &WbUrl::remoteWebotsAssetPrefix() {
  ...
  if (url.isEmpty()) {
    ...
    url = WbStandardPaths::webotsHomePath();
  }
  return url;
}
...
QString WbUrl::combinePaths(const QString &rawUrl, const QString &rawParentUrl) {
  ...
  if (WbUrl::isLocalUrl(url)) {
    // URL fall-back mechanism: only trigger if the parent is a world file (.wbt), and the file (omnisim://) does not exist
    if (parentUrl.endsWith(".wbt", Qt::CaseInsensitive) &&
        !QFileInfo(QDir::cleanPath(url.replace("omnisim://", WbStandardPaths::webotsHomePath()))).exists()) {
      WbLog::error(QObject::tr("URL '%1' changed by fallback mechanism. Ensure you are opening the correct world.").arg(url));
      return url.replace("omnisim://", WbUrl::remoteWebotsAssetPrefix());
    }
  ...
  if (QDir::isRelativePath(url)) {
    ...
    if (WbUrl::isWeb(parentUrl) || QDir::isAbsolutePath(parentUrl) || WbUrl::isLocalUrl(parentUrl)) {
      ...
      if (WbUrl::isLocalUrl(parentUrl))
        parentUrl.replace("omnisim://", WbStandardPaths::webotsHomePath());
