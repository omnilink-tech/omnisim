// Copyright 1996-2024 Cyberbotics Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
//
// Modifications copyright 2026 OmniLink, licensed under the Apache License, Version 2.0.

#include "OmNodeModel.hpp"

#include "OmFieldModel.hpp"
#include "OmLog.hpp"
#include "OmParser.hpp"
#include "OmStandardPaths.hpp"
#include "OmTokenizer.hpp"

#include <QtCore/QCoreApplication>
#include <QtCore/QDir>
#include <QtCore/QMap>
#include <QtCore/QRegularExpression>
#include <QtCore/QTextStream>

QMap<QString, OmNodeModel *> OmNodeModel::cModels;

void OmNodeModel::cleanup() {
  QMapIterator<QString, OmNodeModel *> it(cModels);
  while (it.hasNext())
    delete it.next().value();
}

OmNodeModel::OmNodeModel(OmTokenizer *tokenizer) :
  mInfo(tokenizer->info()),
  mName(tokenizer->nextWord()),
  mParentName(tokenizer->parent()),
  mParentModel(NULL) {
  tokenizer->skipToken("{");

  while (tokenizer->peekWord() != "}") {
    // cppcheck-suppress constVariablePointer
    OmFieldModel *fieldModel = new OmFieldModel(tokenizer, "");
    fieldModel->ref();
    mFieldModels.append(fieldModel);
  }

  tokenizer->skipToken("}");
}

OmNodeModel::~OmNodeModel() {
  foreach (const OmFieldModel *fieldModel, mFieldModels)
    fieldModel->unref();
  mFieldModels.clear();
}

OmNodeModel *OmNodeModel::readModel(const QString &fileName) {
  OmTokenizer tokenizer;
  int errors = tokenizer.tokenize(fileName);
  if (errors > 0)
    return NULL;

  // parsing the .wrl files is not absolutely necessary:
  // we can assume that their syntax is correct
  OmParser parser(&tokenizer);
  bool ok = parser.parseNodeModel();
  if (!ok)
    return NULL;

  tokenizer.rewind();
  return new OmNodeModel(&tokenizer);
}

void OmNodeModel::readAllModels() {
  QString path = OmStandardPaths::resourcesPath() + "nodes/";
  QStringList list = QDir(path, "*.wrl").entryList();
  foreach (const QString &modelName, list) {
    // cppcheck-suppress constVariablePointer
    OmNodeModel *model = readModel(path + modelName);
    if (model)
      cModels.insert(model->name(), model);
  }

  // Now that all the models are loaded, populate the ancestry tree
  foreach (QString baseModelName, baseModelNames()) {
    OmNodeModel *baseModel = findModel(baseModelName);
    if (baseModel)
      baseModel->mParentModel = findModel(baseModel->mParentName);
  }

  qAddPostRoutine(OmNodeModel::cleanup);
}

QStringList OmNodeModel::baseModelNames() {
  if (cModels.isEmpty())
    readAllModels();

  return cModels.keys();
}

bool OmNodeModel::isBaseModelName(const QString &modelName) {
  if (cModels.isEmpty())
    readAllModels();

  return cModels.contains(modelName);
}

QString OmNodeModel::compatibleNodeName(const QString &modelName) {
  if (modelName == "CameraFocus")
    return "Focus";
  if (modelName == "CameraLensDistortion")
    return "Lens";
  if (modelName == "CameraZoom")
    return "Zoom";
  return modelName;
}

OmNodeModel *OmNodeModel::findModel(const QString &modelName) {
  if (cModels.isEmpty())
    readAllModels();

  return cModels.value(modelName, NULL);
}

OmFieldModel *OmNodeModel::findFieldModel(const QString &fieldName) const {
  foreach (OmFieldModel *fieldModel, mFieldModels)
    if (fieldModel->name() == fieldName)
      return fieldModel;

  return NULL;  // not found
}

bool OmNodeModel::fuzzyParseNode(const QString &fileName, QString &nodeInfo) {
  QFile input(OmStandardPaths::resourcesPath() + "nodes/" + fileName + ".wrl");
  if (!input.open(QIODevice::ReadOnly)) {
    OmLog::warning(fileName + ": could not open file", false, OmLog::PARSING);
    return false;
  }

  nodeInfo.clear();
  QTextStream stream(&input);
  QString line;
  while (!stream.atEnd()) {
    line = stream.readLine();
    if (line.startsWith("#")) {
      if (line.contains(QRegularExpression("#\\s*VRML(_...|) V(\\d+).(\\d+)")))
        continue;
      line = line.mid(1).trimmed();  // clean line
      if (!line.isEmpty())
        nodeInfo.append(line + "\n");
    } else {
      nodeInfo.chop(1);  // remove last newline
      break;
    }
  }

  input.close();
  return true;
}

QStringList OmNodeModel::fieldNames() const {
  QStringList names;
  foreach (const OmFieldModel *fieldModel, mFieldModels)
    names.append(fieldModel->name());
  return names;
}
