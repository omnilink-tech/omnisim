...
Webots subprocess for each prop, which is expensive enough that the
...
The output rotation is in Webots axis-angle form
...
def _align_up_to_normal(
    ...
    """Return a Webots axis-angle rotation that sends +Z to ``normal``,
    ...
    # For Webots' ``Solid.rotation`` we can get away with a *single*
...
def ground_fit_on_heightmap(
    ...
    *,
    ...
    Webots axis-angle rotation, and the diagnostic slope in degrees.
    """
    wx, wy = world_xy
