...
- Discover the OmniSim binary: ``OMNIWORLD_WEBOTS_BIN`` env var wins;
  otherwise we try ``$OMNISIM_HOME/msys64/mingw64/bin/omnisim-bin.exe``
  (the OmniSim bundled layout) and ``omnisim-bin`` on PATH.
- Launch with a short timeout (Webots does not auto-exit, so we
...
def run_headless(
    ...
    *,
    ...
    """Launch Webots headless on ``world_path`` and decide whether
    the world loads cleanly.

    Arguments:
        world_path: path to the ``.wbt`` to load.
        webots_bin: override the discovered Webots binary path.
        ...
        load_time_budget_s: maximum acceptable wall-clock time from
            launch to "Webots has accepted the world" (proxy: wall
            clock at process exit).
        strict_warnings: treat any ``WARNING:`` line as a failure.

    Raises:
        FileNotFoundError: if no Webots binary can be discovered.
    """
    path = Path(world_path).resolve()
    if not path.exists():
        raise FileNotFoundError(f"world file does not exist: {path}")

    binary = Path(webots_bin) if webots_bin else discover_webots_bin()
    if binary is None:
        raise FileNotFoundError(
            "could not locate omnisim binary; set OMNIWORLD_WEBOTS_BIN or "
            "OMNISIM_HOME, or put omnisim-bin on PATH"
        )
    ...
    # Expose the repo-local msys64 runtime on PATH so Webots finds its
    # DLLs (matches what launch.bat does). Harmless on platforms where
    # that directory does not exist.
    env = os.environ.copy()
    if "WEBOTS_HOME" in env:
        bundled = Path(env["WEBOTS_HOME"]) / "msys64" / "mingw64" / "bin"
        if bundled.exists():
    ...
    try:
        ...
        # A timeout is expected — Webots does not self-exit. We treat
    ...
    # Pass criterion: zero ERROR: lines. Webots does not self-exit, so
...
__all__ = ["HeadlessResult", "discover_webots_bin", "run_headless"]
