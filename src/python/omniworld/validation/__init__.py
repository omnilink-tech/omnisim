...
``omniworld validate <world.wbt>`` runs a set of checks against a
...
Three checks are in-process (cheap, always on): asset locality, prop
overlap, spawn reachability.

One check is headless-simulator (T1.7, opt-in via ``with_sim=True``
because it spawns a Webots process): load success + warning count.
...
from .headless import HeadlessResult, discover_webots_bin, run_headless
