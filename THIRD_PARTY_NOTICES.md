...
| Lite3, X30, M20, M20S, M20 + Piper, DR02 Standard, DR02 Pro | Deep Robotics | [DeepRoboticsLab/deep_robotics_model](https://github.com/DeepRoboticsLab/deep_robotics_model) (the `urdf/` tree of each robot, commit `549eb9ac`, 2026-09-08) | BSD 3-Clause — © 2024 DeepRoboticsLab. The M20 Piper's arm links are AgileX Piper geometry redistributed by Deep Robotics under the same licence. Only the low-resolution URDF + STL set was taken; the MJCF / USD trees and the Google-Drive high-resolution meshes were not. Every edit (mesh paths, standing rest poses, foot inertias, one full-width-period typo) is listed in `projects/robots/deep_robotics/PROVENANCE.md` and applied by `make_urdfs.py`, never by hand. | `projects/robots/deep_robotics/LICENSE.upstream`, `projects/robots/deep_robotics/PROVENANCE.md` |
...
| SO-101 (Standard Open Arm) | The Robot Studio, with Hugging Face | [TheRobotStudio/SO-ARM100](https://github.com/TheRobotStudio/SO-ARM100) (the `Simulation/SO101/` tree — two URDFs + 15 STL meshes, commit `eecbe3e0`, 2026-09-06) | Apache-2.0. ⚠️ **No copyright holder or year is asserted, and none is known** — upstream's LICENSE carries the Apache-2.0 body with its appendix left as the unfilled template `Copyright [yyyy] [name of copyright owner]`, so there is no notice to retain. Attribution is instead made through upstream's own `CITATION.cff`, redistributed here as `CITATION.upstream.cff`, which names the work "Standard Open SO-100 & SO-101 Arms" (released 2024) and its authors: Rob Knight, Pepijn Kooijmans, Remi Cadene, Simon Alibert, Michel Aractingi, Dana Aubakirova, Adil Zouitine, Russi Martino, Steven Palma, Caroline Pascal, Thomas Wolf. A holder is **not** invented. Only the simulation assets were taken — the `STL/` and `STEP/` 3D-print CAD, the SO-100 model, the `Mini/` and `Optional/` variants and the MJCF files were not. The two local edits (`gripper_frame_link` mass and inertia, clearing a singular mass matrix) are listed in `projects/robots/therobotstudio/so101/PROVENANCE.md`. | `projects/robots/therobotstudio/so101/LICENSE.upstream`, `projects/robots/therobotstudio/so101/PROVENANCE.md`, `projects/robots/therobotstudio/so101/CITATION.upstream.cff` |
...
#### Brand fonts (`resources/branding/omnisim/fonts/`)

| Font | Licence |
|---|---|
| Space Grotesk (variable, weights 300-700) | SIL Open Font License 1.1. © 2020 The Space Grotesk Project Authors ([floriankarsten/space-grotesk](https://github.com/floriankarsten/space-grotesk)); designed by Florian Karsten; the OFL header declares no Reserved Font Name. Text: `resources/branding/omnisim/fonts/OFL-SpaceGrotesk.txt` |
