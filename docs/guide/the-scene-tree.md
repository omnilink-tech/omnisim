...
The scene tree of OmniSim is structured like a VRML97 file.
...
This section describes the user interface of the Scene Tree, and gives an overview of the VRML97 nodes and OmniSim nodes.
...
> **Note**:
We recommend to use the Scene Tree to write OmniSim world files. However, because the nodes and fields are stored in a human readable form, it is also possible to edit world files with a regular text editor.
Some search and replace operations may actually be easier that way.
Please refer to OmniSim [Reference Manual](../reference/omnisim-world-files.md) for more info on the available nodes and the world file format.
...
This list is normally compiled automatically by OmniSim when a world is saved.
...
This allows OmniSim to download the PROTO, if not already available, prior to it being summoned into the world.
