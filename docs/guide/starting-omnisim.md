...
**Note:** The bundled `webots.exe` / `webotsw.exe` launchers also work (they're thin wrappers that spawn `omnisim-bin.exe`); the only difference is `webotsw.exe` runs in the background and returns immediately.
...
To do this, simply set the environment variable `WEBOTS_SAFE_MODE` in the environment running OmniSim.
