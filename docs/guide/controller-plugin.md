...
Whatever its language, a controller executable is linked with the OmniSim controller library (libController) at startup.
...
The IPC between libController and OmniSim is a pipe (On Windows this is a named pipe, and otherwise a local domain socket).
...
All the entities (the controller, the remote control library and the robot window library) should only call the libController interface (OmniSim API) functions.
...
`OMNISIM_HOME/resources/Makefile.include`
%figure "Controller plugin overview"
%chart
graph LR
  OmniSim[OmniSim] -.-> libController[libController]
...
To create a robot window for your project follow these steps:
...
The HTML robot windows can communicate with controller programs written using any of the supported programming languages, i.e. C, C++ and Python.
...
A remote-control plugin allows to simply and efficiently create an interface using the OmniSim API to communicate with a real robot.
The main purpose of a remote-control library is to wrap all the OmniSim API functions used by the robot with a protocol communicating to the real robot.
...
As mentioned above, all the OmniSim API functionalities that should work with the real robot have to be wrapped into the remote-control library.
To achieve this:
...
`OMNISIM_HOME/include/controller/c/omnisim/remote_control.h`

> ⚠️ **The legacy `webots/remote_control.h` spelling no longer compiles.** This page said it
> "still compiles" until 2026-08-16; the 91 one-line forwarder headers under
> `include/controller/{c,cpp}/webots/` were deleted and that directory no longer exists.
> Port a plugin by rewriting `webots/` to `omnisim/` in its include lines.
...
A complete sample of a remote-control plugin lives under `OMNISIM_HOME/tests/api/plugins/remote_controls/remote_control/`; its `remote_control.c` implements the `wbr_*` entry points and the `WbrInterface` mapping described above.
