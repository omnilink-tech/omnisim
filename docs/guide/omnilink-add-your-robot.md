...
- a clear answer to one question — which **bridge class** does your robot belong to? It determines which bridge you'll piggy-back on. (Quadrupeds also have a bridge, but it's Spot-specific.)
...
| Quadruped | `omnilink_quadruped_bridge` | Spot | hard-coded |
...
```bash
cp projects/samples/demos/worlds/chat/omnilink_husky.wbt \
