...
The Robot Window for a [Robot](../reference/robot.md) is opened from the right-click context menu (see [Context Menu](#context-menu) below).
...
#### Keyboard Free-fly (WASD)

When the 3D window has keyboard focus, you can fly the camera freely with the keyboard, in the style of a first-person video game or the Unreal Engine editor:

- <kbd>W</kbd> / <kbd>S</kbd> — move forward / backward along the camera's look direction
- <kbd>A</kbd> / <kbd>D</kbd> — strafe left / right
- <kbd>E</kbd> / <kbd>Q</kbd> — ascend / descend along the world up axis
- Hold <kbd>⇧ shift</kbd> to sprint (3× speed)
- Hold the **left mouse button** while pressing any of the keys above to look around with the mouse, FPS-style — the mouse aims the view and the keys move the camera. The cursor is hidden and re-centred while looking; release the left button to leave look mode. A simple left-click without dragging still selects objects, and left-button drag with no fly key held still orbits the camera, as before.

#### View Snap Shortcuts

You can snap the camera to the canonical world axes from the keyboard. The mapping mirrors Blender so users coming from a DCC tool feel at home:

- <kbd>Numpad 7</kbd> — top view (look straight down) &nbsp; · &nbsp; <kbd>Ctrl</kbd>+<kbd>Numpad 7</kbd> — bottom view
- <kbd>Numpad 1</kbd> — north view (front) &nbsp; · &nbsp; <kbd>Ctrl</kbd>+<kbd>Numpad 1</kbd> — south view (back)
- <kbd>Numpad 3</kbd> — east view (right) &nbsp; · &nbsp; <kbd>Ctrl</kbd>+<kbd>Numpad 3</kbd> — west view (left)

The same actions are also reachable from the menu under **View → Change View**, and there's a **View → Align View to Object** submenu that snaps the camera onto the local axes of whichever solid is currently selected.

#### Follow Selected Object (F)

Click on a robot (or any solid) to select it, then press <kbd>F</kbd> to make the camera follow it. The current camera offset (distance, height, angle) is preserved as the object moves — like a chase camera in a video game. Press <kbd>F</kbd> again to stop.

This is the same action as **View → Follow Object → Tracking Shot** (also bound to <kbd>F5</kbd>) and the **Follow Object → Tracking Shot** entry in the right-click context menu.

Speed adapts to the scene scale: motion is slower when the camera is zoomed in on a small object and faster when it's far from the scene, so the same keys feel right in both a tabletop world and a planetary terrain.

If a robot controller is also using these keys (some teleoperation demos do), the keys are still forwarded to the controller — the camera will move alongside the robot. To suppress camera motion in that case, enable **View → Lock Viewpoint**.
...
Currently OmniSim provides two different ways to move solid objects: axis-aligned handles and keyboard shortcuts.
