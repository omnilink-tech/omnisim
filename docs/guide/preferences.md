...
The OmniSim preferences can be modified by a dialog box which can be open from the `OmniSim / Preferences` menu item on Mac, and from the `Tools / Preferences` menu item on the other operating systems.
...
- The **Language** option allows you to choose the language of OmniSim user interface (restart needed).
- The **Startup mode** allows you to choose the state of the simulation when OmniSim is started (pause, realtime, run, fast; see the `Simulation` menu).
- The **Console font** defines the font used in the Console.
It is recommended to select a fixed width font for better log readability.
The default value of this preference is "Consolas,10" on Windows, "Courier,14" on Mac and "Monospace" on linux.
- The **Number of threads** defines how many threads can be created by OmniSim at maximum.
...
- The **Python command** defines which Python command is invoked by OmniSim when starting a Python controller.
The default value is `python` on Windows and `python3` on Linux/macOS.
...
- The **Extra project path** defines the paths to user folders similar to the `OMNISIM_HOME/projects` folder.
...
Alternatively, the environment variable `OMNISIM_EXTRA_PROJECT_PATH` can be used. (The legacy `WEBOTS_EXTRA_PROJECT_PATH` spelling is still honoured when the canonical name is unset — see [`OmProject.cpp:90-93`](../../src/omnisim/core/OmProject.cpp#L90) — but write the canonical name.)
If the `Extra project path` parameter and the `OMNISIM_EXTRA_PROJECT_PATH` environment variable are set, then both will be considered, with the priority given to the paths set in the `Preferences` setting.
Duplicated PROTO nodes will be disabled in the `Add node` menu.
These paths have the priority over the other search paths.
- The **Warnings: Display save warning only for scene tree edit** checkbox prevents OmniSim from displaying any warning dialog window when you quit, reload or load a new world after the current world was modified by either changing the viewpoint, dragging, rotating, applying a force or torque to an object, or modifying the world from a controller.
It will however still display a warning if the world was modified from the scene tree.
- The **Thumbnail: Capture thumbnail on world save or share** checkbox allows a 768px by 432px (16:9) screenshot of the [3D Window](the-3d-window.md) to be taken when the world is saved or exported. All optional renderings, overlays and selections are hidden for the thumbnail. It can be viewed locally and is used during the loading process of the OmniSim [Web Interface](web-interface.md).
- The **Update policy: Check for OmniSim updates on startup** checkbox allows OmniSim to check if a new version is available for download at every startup.
...
The anti-aliasing algorithm used by OmniSim is [SMAA 1x](http://www.iryoku.com/smaa/).
...
The **Proxy** section allows you to manually configure a HTTP proxy that OmniSim will use to access its license server over the Internet.
...
After changing the proxy configuration, it is recommended to restart OmniSim to ensure the changes are properly taken into account.
If you clear the **Hostname** field, OmniSim will try to retrieve the default system proxy on the next launch.
...
- The **simulation upload service** field allows you to set your own simulation upload server URL. OmniSim does not operate an upload service, so the field is **empty by default** and no upload destination is configured; set it to your own server if you want to upload there.
...
The **Disk Cache** section allows you to set the maximum size of the cache used by OmniSim to store the assets (textures, meshes and sounds) downloaded from the Internet and to clear the cache content.
...
`C:/Users/<USER>/AppData/Local/OmniLink/OmniSim/cache`
...
`~/Library/Caches/OmniLink/OmniSim`
