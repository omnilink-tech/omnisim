...
In the dialog choose the `MyController.exe` process. (This page named a `MyController.exe_webots` process until 2026-08-16; no such artifact is produced — a controller builds to `$(NAME)$(EXE_EXTENSION)`, i.e. plain `MyController.exe`, see [`resources/Makefile.include`](../../resources/Makefile.include) `MAIN_TARGET`.)
...
In the `Settings` window, select the `Project` / `Project Structure` tab, then, the `Add Content Root` button can be used to add a new folder to the path, select the `OMNISIM_HOME/lib/controller/python/omnisim` folder.
...
To allow [PyCharm](https://www.jetbrains.com/pycharm) to start the controller instead of OmniSim, set the controller of the robot to `<extern>` and set the environment variables as explained in the [Running Extern Robot Controllers](running-extern-robot-controllers.md) chapter.
