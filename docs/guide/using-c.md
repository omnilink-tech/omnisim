...
This is the low level interface with the OmniSim simulator; all other APIs are built over the C API.
A majority of OmniSim controller examples are written in C, therefore the C API is OmniSim de facto standard API.
...
OmniSim will need principally the `gcc` (GNU C Compiler) and `make` commands of Xcode.
...
For compiling C controllers, OmniSim will need the GNU C Compiler and GNU Make utility.
