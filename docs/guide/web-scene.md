...
- Once removed, it is not possible to recreate a `omnisim-view` element on the same page. However there is some workarounds:
    ...
    let omnisimView;
    function create() {
      if (typeof omnisimView === 'undefined') {
        omnisimView = document.createElement('omnisim-view');
        omnisimView.style = "height:80%; display:block;"
      }
      document.body.appendChild(omnisimView);

      omnisimView.loadScene("scene.w3d");
    }

    function remove() {
      document.body.removeChild(omnisimView);
    }
    ```
