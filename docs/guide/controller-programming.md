...
These header files must be included using statements like `#include <webots/xyz.h>` where `xyz` represents the name of an OmniSim node in lowercase.
...
The related C header is located at "[OMNISIM\_HOME/include/controller/c/webots/utils/ansi\_codes.h]({{ url.github_tree }}/include/controller/c/webots/utils/ansi_codes.h)", it contains some useful macros on top of constants, to use it:
...
Moreover the main Makefile ("[OMNISIM\_HOME/resources/Makefile.include]({{ url.github_tree }}/resources/Makefile.include)") used to compile OmniSim controllers is able to create shared libraries and to link easily with the Controller libraries, ODE or the Qt framework.
