...
The following page describes how to write controller code. Though originally focused on C, most relevant and non-language-specific details have been translated to C++ and Python.
To have a more in-depth look for equivalent functions/methods in other languages, see [Nodes and API functions](../reference/nodes-and-api-functions.md) and [C++ and Python](cpp-python.md).

### Hello World Example

The tradition in computer science is to start with a "Hello World!" example.
So here is a "Hello World!" example for an OmniSim controller:
...
This code repeatedly prints `"Hello World!"` to the standard output stream which is redirected to OmniSim console.
The standard output and error streams are automatically redirected to OmniSim console for all OmniSim supported languages.

OmniSim C API (Application Programming Interface) is provided by regular C header files.
These header files must be included using statements like `#include <omnisim/xyz.h>` where `xyz` represents the name of an OmniSim node in lowercase.
...
This function initializes the communication between the controller and OmniSim.
The `wb_robot_cleanup` function does the opposite: it closes the communication between the controller and OmniSim to terminate the controller smoothly.
...
This function will indeed return `-1` when OmniSim terminates the controller (see [Controller Termination](#controller-termination)).
Therefore, in this example, the control loop will run as long as the simulation runs.
When the loop exists, no further communication with OmniSim is possible and the only option is to confirm to OmniSim to close the communication by calling the `wb_robot_cleanup` function.
...
%tab "Python"
```python
from omnisim import Robot, DistanceSensor
...
%tab "C++"
```cpp
const double *omnisim::GPS::getValues() const;
const double *omnisim::Accelerometer::getValues() const;
const double *omnisim::Gyro::getValues() const;
```
%tab-end
...
Just like sensors, each OmniSim actuator must be identified by a `WbDeviceTag` returned by the `wb_robot_get_device` function.
...
%tab "Python"
```python
from omnisim import Robot, Motor
...
OmniSim uses two different time steps:
...
OmniSim API functions are executed but they are applied to the simulation world only when calling the `wb_robot_step` function, that is when the controller process communicates with OmniSim process.
...
OmniSim and each robot controller are executed in separate processes.
For example, if a simulation involves two robots, there will be three processes in total: one for OmniSim and two for the two robots.
Each controller process exchanges sensors and actuators data with the OmniSim process during the `wb_robot_step` function calls.
So for example, the `wb_motor_set_position` function does not immediately send the data to OmniSim.
...
Usually a controller process runs in an endless loop until it is terminated by OmniSim on one of the following events:
...
From this point, OmniSim will not communicate with the controller any more.
Therefore, new print statements executed by the controller on `stdout` or `stderr` will no longer appear in the OmniSim console.
After one second (real time), if the controller has not terminated by itself, OmniSim will kill it (`SIGKILL`).
That leaves a limited amount of time to the controller to save important data, close files, etc. before it is actually killed by OmniSim.
...
int main() {
  ...
  // OmniSim triggered termination detected!
  // Past this point, new printf statements will no longer be
  // displayed in the OmniSim console
...
# OmniSim triggered termination detected!
# Past this point, new print statements will no longer be displayed in the OmniSim console
...
Note that the exit status of an OmniSim controller is ignored by OmniSim.

### Console Output

As mentioned earlier, printing to `stdout` or `stderr` from a controller will be redirected by default to the OmniSim console.
Note however, that the OmniSim console doesn't support `stdin` input.
...
The related C header is located at "[OMNISIM\_HOME/include/controller/c/omnisim/utils/ansi\_codes.h]({{ url.github_tree }}/include/controller/c/omnisim/utils/ansi_codes.h)", it contains some useful macros on top of constants, to use it:
...
```c
#include <omnisim/utils/ansi_codes.h>
...
```cpp
#include <omnisim/utils/AnsiCodes.hpp>
...
Moreover the main Makefile ("[OMNISIM\_HOME/resources/Makefile.include]({{ url.github_tree }}/resources/Makefile.include)") used to compile OmniSim controllers is able to create shared libraries and to link easily with the Controller libraries or the Qt framework. (It used to offer an ODE link target as well; ODE was deleted on 2026-08-08 and there is no physics library for a controller to link against — talk to the physics through the controller API, or from a `Supervisor`.)

A good example of this is the Qt utility library located there: "[OMNISIM\_HOME/resources/projects/libraries/qt\_utils]({{ url.github_tree }}/resources/projects/libraries/qt_utils)".

If for some reason shared libraries cannot be in the `libraries` directory, the `OMNISIM_LIBRARY_PATH` environment variable will be very helpful.
...
OmniSim offers an elegant solution to this.
...
The OmniSim "runtime.ini" supports 7 sections:
...
[environment variables with paths]
OMNISIM_LIBRARY_PATH = lib:$(OMNISIM_LIBRARY_PATH):../../library
...
The "runtime.ini" file may also contain language specific sections, named `[java]` and `[python]`.
...
In the example above, the resulting command issued by OmniSim will be: `/opt/local/bin/python3.8 -m package.name.given my_controller.py` possibly followed by the value of the `controllerArgs` field of the corresponding [Robot](../reference/robot.md) node.
