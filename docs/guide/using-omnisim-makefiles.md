...
The C++ API is a set of C++ classes provided by C++ header files, e.g. `#include <omnisim/Robot.hpp>` (the legacy `#include <webots/Robot.hpp>` path still compiles as a forwarding shim).
If you prefer, C++ controllers can use OmniSim C API instead.
The C API is a set of C functions starting with the `wb` prefix and provided by C header files, e.g. `#include <omnisim/robot.h>` (the legacy `#include <webots/robot.h>` path still compiles as a forwarding shim).
