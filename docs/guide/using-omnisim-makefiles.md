## Using OmniSim Makefiles

### What are Makefiles

The compilation of OmniSim C/C++ controllers can be configured in the provided Makefiles.
...
Note that Python is an interpreted language and therefore it doesn't need a Makefile.
So if you are using any of these programming languages or Visual Studio then you can ignore this section.

When using C/C++, the presence of a Makefile in the controller directory is necessary.
If the Makefile is missing OmniSim will automatically propose to create one.
...
The global "Makefile.include" file is stored in "OMNISIM\_HOME/resources/" directory (the Makefile also accepts WEBOTS\_HOME for backwards-compatibility); it contains the effective build rules and may vary with the OmniSim version.
Note that OmniSim Makefiles are platform and language independent.
...
%figure "OmniSim Makefile Sources Variables"
...
%figure "OmniSim Makefile Compiler Variables"
...
OmniSim C/C++ controllers are regular binary executable files that can easily be compiled and linked with external libraries.
...
#### Using OmniSim C API in a C++ Controller

Normally, C++ controllers use OmniSim C++ API.
The C++ API is a set of C++ classes provided by C++ header files, e.g. `#include <omnisim/Robot.hpp>` (the legacy `#include <omnisim/Robot.hpp>` path still compiles as a forwarding shim).
If you prefer, C++ controllers can use OmniSim C API instead.
The C API is a set of C functions starting with the `wb` prefix and provided by C header files, e.g. `#include <omnisim/robot.h>` (the legacy `#include <omnisim/robot.h>` path still compiles as a forwarding shim).
