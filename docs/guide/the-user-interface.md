...
OmniSim GUI is composed of three principal windows: the *3D window* that displays and allows you to interact with the 3D simulation, the *Scene tree* which is a hierarchical representation of the current world, and the *Console* that displays controller outputs. Edit controller and PROTO source code in your preferred external editor.
...
The GUI has seven menus: `File, Edit, View, Simulation, Overlays, Tools` and `Help`.
...
- The **New** submenu allows you to create new simulation files:
  ...
  A project directory contains several subdirectories that are used to store the files related to a particular OmniSim project, i.e., world files, controller files, data files, plugins, etc. OmniSim remembers the current project directory and automatically opens and saves any type of file from the corresponding subdirectory of the current project directory.
  ...
  You will first be prompted to choose between a C, C++, or Python controller.
  If you choose C or C++ on Windows, OmniSim will offer you the possibility to create a Makefile / gcc project or a Visual Studio project.
  Then, OmniSim will ask you to enter the name of your controller and finally it will create all the necessary files (including a template source code file) in your current project directory.
  - The **New Physics Plugin...** menu item will let you create a new physics plugin for your project.
  OmniSim asks you to choose a programming language (C or C++) and a name for the new physics plugin.
  ...
  OmniSim asks you to define a name for the PROTO node, the tags that should be included (if any), and the base node from which the PROTO itself will inherit.
  Based on the choice of base node, OmniSim will ask you which among its fields should be exposed (i.e., visible from the scene tree) and will create the necessary parameters accordingly.
...
- ![](images/screenshot-button.png =26x26) The **Take Screenshot...** item allows you to take a screenshot of the current view in OmniSim.
...
However, frames are only captured during OmniSim steps and not when the simulation is paused.
...
For both options, the result is saved locally. It can also be uploaded automatically to a share service, but OmniSim does not operate one — no upload destination is configured by default, and you must set your own server URL in *Preferences > Web Services* for the upload option to do anything.
...
- **Quit** terminates the current simulation and closes OmniSim.

> **Note:** on Windows, "Exit" is used instead of "Quit".

### Edit Menu

The **Edit** menu provides usual text-edition functions (Copy, Paste, Cut, Find, etc.) that operate on the currently focused widget — for example, the *Console* or the *Scene tree*.
...
- The **Rendering** radio button group allows you to choose between the **Plain Rendering** (default) and the **Wireframe** modes for OmniSim simulation window.
...
These options can be used to improve interacting with the 3D scene by disabling some unneeded functionalities, or to prevent some interactions for example when streaming a simulation over the web.
  ...
  This is particularly useful during web streaming, where the context menu is opened directly on client web interface and it is not needed to open in on the OmniSim server instance.
...
The **Tools** menu allows you to open various OmniSim windows.
...
- The **Documentation** menu item shows or hides the offline OmniSim documentation window.
...
- The **OmniSim Guided Tour...** menu item starts a guided tour that demonstrates OmniSim capabilies through a series of examples.

- The **Check for updates...** item pops up a window informing if the OmniSim version in use is the latest one and providing the link to download the latest version if needed.
...
This window allows you to insert nodes or objects from the vast OmniSim library into your world.
