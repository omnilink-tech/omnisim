...
There are several approaches to using optimization algorithms in OmniSim.
...
It is possible, and recommended, to use OmniSim [Emitters](../reference/emitter.md) and [Receivers](../reference/receiver.md) to exchange information between the [Supervisor](../reference/supervisor.md) controller and the other controllers.
...
The motor positions should be reset using the `wb_motor_set_position` function and the robot controller should be reset by sending a message from the supervisor process to the robot controller process (using OmniSim [Emitter](../reference/emitter.md) / [Receiver](../reference/receiver.md) communication system).
...
#### By Starting and Quitting OmniSim

Finally, the last method is to start and quit the OmniSim program for each parameter evaluation.
This may sound like an overhead, but in fact OmniSim startup time is usually very short compared to the time necessary to evaluate a controller, so this approach makes perfectly sense.

For example, OmniSim can be called from a shell script or from any type of program suitable for running the optimization algorithm.
Starting OmniSim each time does clearly reload the world completely, so each robot will start from the same initial state.
The drawback of this method is that the optimization algorithm has to be programmed outside of OmniSim.
This external program can be written in any programming language, e.g. shell script, C, PHP, perl, etc., provided that there is a way to call OmniSim and wait for its termination, e.g. like the C standard `system` function does.
On the contrary, the parameter evaluation must be implemented in an OmniSim controller.
...
For example, the optimization algorithm can write the genotypes values into a text file then call OmniSim.
When OmniSim starts, the robot controller reads the genotype file and carries out the parameter evaluation.
When the robot controller finishes the evaluation, it writes the fitness result into another text file and then it calls the `wb_supervisor_simulation_quit` function to terminate OmniSim.
