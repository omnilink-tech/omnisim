...
OmniSim can record a simulation to an interactive 3D `HTML` page.
...
The [figure below](#screenshot-of-a-web-animation-page-generated-by-omnisim) shows an example of an animation page generated by OmniSim. This animation is available the upstream Webots web animation example.

%figure "Screenshot of a Web animation page generated by OmniSim"
...
Select `Record HTML5 Animation...` from the `File` menu, then choose a local file in the export dialog to start the recording.
Click `Stop HTML5 Animation...` to finish the recording and save the animation.
OmniSim will ask to playback the resulting file in the default Web browser (from the OS settings).
...
The web animation is played by a web component from the [OmnisimView.js] package called `omnisim-view`.
...
The attributes of `omnisim-view` are only evaluated once: when the page is loaded. If the `data-scene` and the `data-animation` attributes are set, the `omnisim-view` web-component will automatically try to load an animation .

For more complex interaction with the web component, the following functions are available:
* `close()`: close the current animation. Note that if the `omnisim-view` element is removed from the HTML page or `loadScene`, `loadAnimation` or `connect` is called, `close` will be automatically called.
* `getNode(id)`: return the web-viewer node with the corresponding `id` if it exists or undefined.
...
* `updateNode(nodeId, field, value, render)`: Update the value of a web-viewer node. The value is updated only on the web side, do not use with the simulation.
  ...
  * `value`: the new value to be set, in `OmVector3.js` or `OmVector4.js` format.
...
To enable it, you must set the `showCustomWindow` attribute of the `omnisim-view` element to `true` before loading the animation.
...
- `TextureTransform.translation` (only for the Track (archived 2026-09-02, see [docs/ARCHIVE.md](../ARCHIVE.md)) node)
- `Pose.rotation`
- `Pose.translation`
- `Light.color`
- `Light.on`
