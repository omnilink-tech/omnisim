...
```text
#include "relative/path/from/this/world.omniworld"
...
[`projects/robot_combat/orc/worlds/include_example/world.omniworld`](../../projects/robot_combat/orc/worlds/include_example/world.omniworld)
is a minimal world that exercises the feature — it declares
`WorldInfo` + `Viewpoint`, then `#include`s a sibling
`_base_env.omniworld` that supplies sky + sun + floor.

## Implementation note

The expansion is done in `OmTokenizer::expandWorldIncludes()` in
[`src/omnisim/vrml/OmTokenizer.cpp`](../../src/omnisim/vrml/OmTokenizer.cpp).
