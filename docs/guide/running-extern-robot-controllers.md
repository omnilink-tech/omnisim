...
If you are running against an **upstream Webots snap** install rather than an OmniSim one, please refer to the corresponding section: [Running Extern Robot Controller against an Upstream Webots Snap Install](#running-extern-robot-controller-against-an-upstream-webots-snap-install). (OmniSim itself ships no snap package.)
...
### Running Extern Robot Controller against an Upstream Webots Snap Install

> ⚠️ **This section is about an upstream Webots snap install, not an OmniSim one.** **OmniSim ships
> no snap package** — there is no `snapcraft.yaml` in this tree, and the one snap-aware branch that
> survives in libController exists solely to keep an upstream-packaged install working
> ([`system.c:139-141`](../../src/controller/c/system.c#L139)). This page described it as "the snap
> version of OmniSim" until 2026-08-16, which sent readers looking for a distribution channel that
> does not exist. For an ordinary OmniSim install, use the instructions above.

In order to compile and execute extern controllers against such an install, the following environment variables should be set:
```
export OMNISIM_HOME=/snap/webots/current/usr/share/webots
export LD_LIBRARY_PATH=$OMNISIM_HOME/lib/controller
```

Because of the snap sand-boxing system, a special temporary folder is used to share information with robot controllers.
When the snap launcher starts, it computes the temporary-folder variable if it is not already set.
This variable is computed from the `SNAP_USER_COMMON` environment variable which typically points to `/home/username/snap/webots/common`, a folder accessible by both the simulator and your own programs.
Similarly, the libController will automatically check this folder and its contents to determine if it should use it to communicate with the simulator.
It is recommended that you do not override it, unless you want to experiment with a different mechanism.

> **Variable name**: the canonical spelling is **`OMNISIM_TMPDIR`**; `WEBOTS_TMPDIR` is still read
> as a legacy alias when the canonical name is unset
> ([`OmStandardPaths.cpp:277-280`](../../src/omnisim/core/OmStandardPaths.cpp#L277)), and the Linux
> launcher exports both ([`omnisim-linux.sh:78-95`](../../src/omnisim/launcher/omnisim-linux.sh#L78)).
> Write `OMNISIM_TMPDIR` in anything new.
...
2. Open a world that contains a robot with a known controller, e.g. `projects/samples/demos/worlds/showcase/jackal_drive.omniworld`.
...
7. You should see the robot in `jackal_drive.omniworld` (or whichever world you chose) being controlled by the extern process you just started.
