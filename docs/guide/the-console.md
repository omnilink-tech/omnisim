...
The OmniSim console displays the logs coming from OmniSim (such as parsing warnings or errors when opening a world, compilation outputs, physics-backend warnings, etc.) and the outputs of the controllers.
To guarantee that the log printed in the OmniSim console is deterministic, the output coming from the controllers is not printed immediately when receiving it but it is grouped by controller and printed at the end of the simulation step.
...
By default, the console displays all the OmniSim logs and controller outputs.
...
The `Filter` sub-menu allows the user to define which kind of OmniSim logs and which robot controllers outputs should be displayed.

The `Level` sub-menu allows the user to define which type of OmniSim logs (classified by severity) and which type (stdout or stderr) of robot controller outputs should be displayed.
