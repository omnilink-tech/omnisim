...
OmniSim can be used as a Web streaming server, i.e., to stream a simulation to several interactive 3D `HTML` pages, as shown in the [figure below](web-streaming.md#screenshot-of-omnisim-streaming-server).
...
- `mjpeg`: the OmniSim scene rendered on the server is directly sent as an `MJPEG` image to the web browser client.
...
| page loading time       | -     | +       |
| high fidelity to OmniSim | +     | +       |

%figure "Screenshot of OmniSim streaming server"
...
- ![](images/web_interface_quit.png =25x25) **Quit**: close the OmniSim web view.
...
- ![](images/web_interface_real_time.png =26x26) **Play**: play the OmniSim simulation.
- ![](images/web_interface_pause.png =26x26) **Pause**: pause the OmniSim simulation.
...
### How to Stream an OmniSim Simulation

Start OmniSim with the streaming server using the `--stream` option.
Please refer to the [command line documentation](starting-omnisim.md#command-line-arguments) for details about the sub-options.
...
Use the buttons on the top of the page to connect to the OmniSim streaming server.
...
Connecting with the default setting from the machine were OmniSim is running can be done via:
```
...
The OmniSim streaming server is simply running on the local computer on a given port (`1234` by default, but it can be modified from the [command line arguments](starting-omnisim.md#command-line-arguments)).
...
Similarly to [this section](web-scene.md#how-to-embed-a-web-scene-in-your-website), to embed the simulation it is enough to instantiate a `omnisim-view` web component from the [OmnisimView.js] package.

### Programming Interface

This is the API of the `omnisim-view` web component:
* `connect(server, mode, broadcast, mobileDevice, timeout, thumbnail) `: function instantiating the simulation web interface and taking as argument:
  * `server`: The URL of the server. Different URL formats are supported:
      * URL to a session server (here the upstream Webots session server, as an example of the URL format): "https://webots.cloud/ajax/server/session.php?url=https://github.com/cyberbotics/webots/projects/languages/python/worlds/example.wbt"
...
* `close()`: close the simulation web scene. Note that if the `omnisim-view` element is removed from the HTML page or `loadScene`, `connect` or `loadAnimation` is called, `close` will be automatically called.
...
* `setWebotsMessageCallback(callback)`: define a function that will be called every time a message is sent by OmniSim.
...
* `setWebotsErrorMessageCallback(callback)`: define a function that will be called every time an error is send by OmniSim.
...
The attributes of `omnisim-view` are only evaluated once: when the page is loaded. If the `data-server` attribute is set, the `omnisim-view` web-component will automatically connect to the `server`.
