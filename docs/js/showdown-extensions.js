...
showdown.extension('wbVariables', function() {
  ...
  const branch = (typeof setup !== 'undefined' && typeof setup.branch !== 'undefined' && setup.branch !== '') ? setup.branch : 'main';
  var vars = {
    // NOTE: the `webots.*` key name is kept because it is referenced by name from
    // ~130 inherited doc pages. The VALUES are OmniSim's — the repository URLs must
    // point at OmniSim, not upstream, or every '{{ url.github_tree }}' link in the
    // guide/reference books 404s on any OmniSim-only path.
    webots: {
      version: {
        major: 'v7.0.0',
    ...
    url: {
      github_tree: `https://github.com/omnilink-tech/omnisim/tree/` + branch,
      github_blob: `https://github.com/omnilink-tech/omnisim/blob/` + branch,
      github_raw: `https://raw.githubusercontent.com/omnilink-tech/omnisim/` + branch
    }
  };
      ...
      filter: function(text, converter, options) {
        text = text.replace(/%robot\s+([^ \n]+)\s+([^ \n]+)/gi, function(match, robot, imageFallback) {
          ...
          if (location.href.startsWith('file:')) {
            // offline documentation embedded in OmniSim.
            ...
            replacement =
              ...
              '    <div id="%ROBOT%-robot-omnisim-view" class="robot-omnisim-view">\n' +
              '    </div>\n' +
