...
class Books:
    ...
    def __init__(self):
        ...
        self._books = []
        # docs/blog and docs/discord were deleted in 3d2ea7854. Book() calls
        # os.listdir() in its constructor, so a stale name here raised
        # FileNotFoundError in setUp and took EVERY test in this suite with
        # it. Skip a book that is not on disk rather than naming it here, so
        # the next deletion degrades instead of detonating.
        for bookName in ["guide", "reference"]:
