...
Likewise, a [LinearMotor](linearmotor.md) can power a [SliderJoint](sliderjoint.md), producing a sliding motion along its axis.

> ✅ **A MOTOR ON A `Hinge2Joint` OR A `BallJoint` NOW MOVES. THIS NOTE USED TO SAY IT DID NOT.**
> All four actuated joint types — [HingeJoint](hingejoint.md), [SliderJoint](sliderjoint.md),
> [Hinge2Joint](hinge2joint.md) and [BallJoint](balljoint.md) — are driven by the Newton backend as of
> 2026-08-17. The old text was accurate when written: the multi-DoF motors were accepted and silently
> ignored, and axis 1 read 0.0000 rad when commanded 0.4. The defect was upstream, in newton's
> d6 → MuJoCo actuator mapping for multi-DoF position control, and it was measured against the
> then-vendored **newton 1.2.0**. The `b56be84a0` upgrade to **newton 1.5.0** fixed it;
> `OMNISIM_NEWTON_BALL_HINGE2` now defaults ON (`=0` is the exact-revert hatch) and
> `tests/test_newton_ball_hinge2.py` pins it from both sides. Samples unblocked: `motor2`, `motor3`,
> `muscle`, `gyro`, and the api joint worlds.
>
> ⚠️ **One caveat:** a `BallJoint`'s per-axis `minStop` / `maxStop` are still **not enforced** (the
> ball element reaches MuJoCo with `limited: False`). `Hinge2Joint` axes are limited normally.
...
In OmniSim, position control is carried out in three stages, as depicted in [this figure](#motor-control).
...
%chart
graph LR
  node["Motor node (VRML)"] -->|"wb_motor_set_position(Pt)<br/>wb_motor_set_velocity(Vd)<br/>wb_motor_set_acceleration(A)<br/>wb_motor_set_available_force(F)<br/>wb_motor_set_control_pid(P,I,D)"| PID["2.motor PID-controller (OmniSim)"]
  robot["1. robot controller (user-code)"] -->|"controlPID(P,I,D)<br/>maxVelocity(Vmax)<br/>acceleration(A)<br/>maxForce(Fmax)"| PID
    PID -->|"motor force/torque(F)</br>motor velocity(Vc)"| SOLVER["3. physics simulator (Newton/MuJoCo)"]
  robot -->|wb_motor_set_force<br/>wb_motor_set_torque| SOLVER
%end
%end
...
The position control described above is performed by the OmniSim PID-controller feeding the Newton/MuJoCo solver's joint actuation.
As an alternative, OmniSim also allows the user to directly specify the amount of force (resp. torque) that must be applied by a [Motor](#motor).
...
| ------------------------------------------------------------------------------ | ------------------------------------------------ | ------------------------------------------------------------- | -------------------------------------------- |
| uses PID-controller                                                            | yes (PID is applied on the velocity)             | no (the solver's own control loop is applied on the force/torque) | no                                       |
...
This function returns immediately (asynchronous) while the actual motion is carried out in the background by OmniSim.
...
> ⚠️ **`wb_motor_get_force_feedback` AND `wb_motor_get_torque_feedback` RETURN A HARDCODED `0.0`
> ON THE CURRENT ENGINE — there is no force feedback.** The measurement came from ODE's
> `dJointFeedback`, and ODE was deleted on 2026-08-08 (commit `bdc02139`).
> `OmRotationalMotor::computeFeedback` / `OmLinearMotor::computeFeedback` now end at
> `return 0.0;  // unreachable (jID is always null now); the dJointFeedback math this replaced went
> with ODE`. **The one exception** is a [RotationalMotor](rotationalmotor.md) whose parent is a
> [Propeller](propeller.md): that case is computed from the propeller's own thrust and torque and
> returns a real number.
>
> `enable`, the sampling period and the getters all still behave normally, so the failure is a
> plausible-looking zero rather than an error — which is why it is flagged here.
>
> **Two consequences that bite elsewhere:**
> * The **energy model** below multiplies this feedback by `consumptionFactor`, so joint motors
>   now draw **zero** power and never drain a [Robot](robot.md)'s `battery`.
> * A controller that closed a loop on measured joint effort (contact detection, compliance,
>   stall detection) silently reads zero. Detect contact with
>   [TouchSensor](touchsensor.md) `bumper` type or `/sim/contacts` instead, and infer effort from
>   the position error against your own PD gains.
>
> The description below is the intended semantics, kept because the API is unchanged.
...
- The spring and damping forces that apply when the `springConstant` or `dampingConstant` fields are non-zero. (⚠️ Those forces are not applied at all on the current engine — see [JointParameters](jointparameters.md#springs-and-dampers).)
...
This function bypasses the PID-controller and the solver's own joint actuation; it adds the force to the physics simulation directly.
...
```python
from omnisim import Motor, Brake, PositionSensor
