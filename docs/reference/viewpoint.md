...
- The `renderBackend` field selects the renderer used to draw this [Viewpoint](#viewpoint).
The default value `"wren"` uses the WREN OpenGL renderer.
Setting it to `"vulkan"` (the alias `"wgpu"` is also accepted) opts the [Viewpoint](#viewpoint) into the GPU-driven wgpu-native renderer.
