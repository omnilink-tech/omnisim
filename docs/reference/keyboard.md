...
> **Note** [Python]: In C++ and Python the keyboard functions are in a dedicated class called `Keyboard`.
...
These functions allow a controller program to read keys pressed on the computer keyboard when the 3D window of OmniSim is selected and the simulation is running.
