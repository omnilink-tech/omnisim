...
If the `ico` field is TRUE and the `subdivision` field is 1 (default value), then the sphere is rendered as an icosahedron whose each face is subdivided into 4 faces, resulting in a 80 faces icosahedron.
...
By default [Sphere](#sphere) nodes created in OmniSim are icosahedrons with `subdivision` value of 1, whereas sphere geometries imported from VRML are UV spheres with a `subdivision` value of 24.
