...
Lidar {
  ...
  SFString renderBackend        "wren"        # {"wren", "wgpu"}
...
```python
from omnisim import LidarPoint
...
A value of 1.0 corresponds to a gaussian noise having a standard deviation of **1 metre**. The noise is ADDITIVE and in ABSOLUTE METRES: `range_noise.frag` computes `depth + noise * gaussian(...)`, and the authored value reaches it unscaled (`OmAbstractCamera::setRangeNoise(mNoise->value())`). ⚠️ This line previously said the deviation was `maxRange` metres. That is true only when `maxRange` is 1.0 — which is this node's default, which is probably how the error survived. A device with `maxRange 8` and `noise 0.1` gets σ = 0.10 m, not 0.8 m.
...
- The `renderBackend` field selects the renderer that produces this lidar's range image: `"wren"` (the default) or `"wgpu"` (`"vulkan"` is an accepted alias), the same contract as the [Camera](camera.md)'s field. On the wgpu arm `noise` and `resolution` are applied as a CPU post-process with the same formulas as the WREN shaders (additive gaussian, sigma in absolute metres; round-half-up quantization) — statistically equivalent, not bit-identical. With `noise` greater than 0.0, values outside the open interval (`minRange`, `maxRange`) — including the background at exactly `maxRange` — read infinity on both backends. `OMNISIM_LIDAR_WGPU=1` selects wgpu regardless of this field; `OMNISIM_WGPU_SENSOR_POSTFX=0` disables the wgpu-side post-process (the device then warns that the authored effect is dropped).
