...
- The `renderBackend` field is a **warned no-op**: every lidar renders its range image through wgpu-native (WREN was deleted on 2026-08-23), the same contract as the [Camera](camera.md)'s field. `"wren"`, `"wgpu"` and the `"vulkan"` alias still parse (an undeclared field is a load error) and all select wgpu. `noise` and `resolution` are applied as a CPU post-process (additive gaussian, sigma in absolute metres; round-half-up quantization). With `noise` greater than 0.0, values outside the open interval (`minRange`, `maxRange`) — including the background at exactly `maxRange` — read infinity. `OMNISIM_WGPU_SENSOR_POSTFX=0` disables the post-process (the device then warns that the authored effect is dropped).
