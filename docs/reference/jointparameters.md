...
The joint hard limits are enforced by the active physics backend: the ODE backend maps them to ODE joint stops (for more information see the ODE documentation on `dParamLoStop` and `dParamHiStop`), while the Newton backend enforces them independently as a post-step joint-state clamp.
