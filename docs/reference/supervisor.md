...
This occurs when closing a world and quitting its controllers, i.e. reloading the current world, opening a new world, or closing OmniSim.
...
```cpp
#include <omnisim/Supervisor.hpp>
...
The `wb_supervisor_node_get_root` function returns a handle to the root node which is actually a [Group](group.md) node containing all the nodes visible at the top level in the scene tree window of OmniSim.
...
An example of such a usage is provided in the "supervisor.omniworld" sample worlds (located in the "projects/samples/devices/worlds" directory of OmniSim.
...
namespace omnisim {
  class Node {
    typedef enum {
      NO_NODE,
      // 3D rendering
      APPEARANCE, BACKGROUND, BILLBOARD, BOX, CAD_SHAPE, CAPSULE, COLOR, CONE, COORDINATE,
...
A list of all node types is provided in the "omnisim/nodes.h" include file.
Node types include `WB_NODE_APPEARANCE`, `WB_NODE_LIGHT_SENSOR`, etc.

> ⚠️ **`WB_NODE_FLUID` and `WB_NODE_IMMERSION_PROPERTIES` are dead enumerators.** They are still
> present in `omnisim/nodes.h` — removing an enumerator would renumber every value after it and
> break the controller ABI — but the `Fluid` and `ImmersionProperties` nodes themselves were removed
> with the ODE backend on 2026-08-08 (commit `bdc02139`), so no scene can contain one and
> `wb_supervisor_node_get_type` will never return either value. The same applies to their `Node`
> class equivalents (`FLUID`, `IMMERSION_PROPERTIES`) in C++ and Python. Do not branch on them, and
> do not try to import one.
...
> **Note** [C++ and Python]: In the oriented-object APIs, the `WB_NODE_*` constants are available as static integers of the `Node` class (for example, Node::ROBOT).
...
The `wb_supervisor_node_remove` function removes the node specified as an argument from the OmniSim scene tree.
...
A file with the equivalent content can be produced in the OmniSim user interface by selecting the node in the scene tree window and using the `Export` button.
...
If a field is defined for the base (OmniSim) type of a node but does not appear in the scene tree due to being an internal field of a PROTO, the `wb_supervisor_node_get_base_node_field` function can be used instead.
...
namespace omnisim {
  class Node {
    ...
    void enablePoseTracking(int samplingPeriod, const Node *fromNode = nullptr) const;
...
The `wb_supervisor_node_enable_pose_tracking` function forces OmniSim to stream poses to the controller.
...
Both the `node` argument (or the node instance for C++ and Python) and the `from_node` argument must be a [Pose](pose.md) node (or a derived node).
...
The `wb_supervisor_node_enable_contact_points_tracking` function forces OmniSim to stream contact points data to the controller.
...
The "[OMNISIM\_HOME/projects/samples/demos/worlds/misc/cylinder\_stack.wbt]({{ url.github_tree }}/projects/samples/demos/worlds/misc/cylinder_stack.omniworld)" project shows how to use this function.
...
> ⚠️ **RESET-PHYSICS NO LONGER STOPS A BODY ON THE CURRENT ENGINE.** The velocity zeroing was
> ODE's `dBodySetLinearVel` / `dBodySetAngularVel`, and ODE was deleted on 2026-08-08
> (commit `bdc02139`). `OmSolid::resetSingleSolidPhysics` now clears only the *display* fields:
> "zeroing the SOLVER-side velocity/force/torque of the merged body is now UNIMPLEMENTED … a
> Newton body keeps whatever `body_qd` / `body_f` it had." Joint motors are still reset, and
> `Solid.linearVelocity` / `angularVelocity` still read back as zero — so the call *looks* like it
> worked while the body keeps moving.
>
> **What to do instead:** to put a scene back, use `wb_supervisor_simulation_reset` or
> `wb_supervisor_node_load_state("__init__")`, which restore state rather than trying to cancel
> momentum. To stop one body, write its velocity directly with
> `wb_supervisor_node_set_velocity(node, {0,0,0,0,0,0})`, which does route to Newton.
...
Additionally, the visibility of [Propeller](propeller.md) helices may be overridden internally by OmniSim when running the simulation or selecting them from the scene tree.
...
The `wb_supervisor_set_label` function displays a text label overlaying the 3D scene in OmniSim' main window.
...
OmniSim ships only libre fonts (Liberation, [SIL OFL 1.1](https://openfontlicense.org/); DejaVu).
The Microsoft core fonts that Webots used to bundle (Arial, Times New Roman, Verdana, ...) are
proprietary and cannot be redistributed under OmniSim's Apache-2.0 license, so they were removed.
Their names are still accepted as **aliases** onto the libre replacements, so controllers written
for Webots keep working:

| legacy name | renders as | metric-compatible? |
|---|---|---|
| `Arial` | Liberation Sans | yes — identical advance widths |
| `Ariali` | Liberation Sans Italic | yes |
| `Arial Black` | Liberation Sans Bold | no — approximation |
| `Times New Roman` | Liberation Serif | yes |
| `Courier New` | Liberation Mono | yes |
| `Lucida Console` | DejaVu Sans Mono | no — monospace substitute |
| `Lucida Sans Unicode`, `Tahoma`, `Verdana` | DejaVu Sans | no — substitute |
| `Trebuchet MS` | Liberation Sans | no — substitute |
| `Georgia`, `Palatino Linotype` | DejaVu Serif | no — substitute |
| `Comic Sans MS`, `Impact` | *(unavailable)* | no libre substitute exists |
...
```c
wb_supervisor_set_label(1,"hello OmniSim",0,0.1,0.1,0x00ff00,0.5,"Impact");
```

This will display the label "hello OmniSim" in semi-transparent green, just below.
...
The `wb_supervisor_simulator_quit` function quits OmniSim, as if one was using the `File / Quit` menu item.
This function makes it easier to invoke an OmniSim simulation from a script because it allows to terminate the simulation automatically, without human intervention.
...
The POSIX exit status returned by OmniSim can be defined by the status `status` parameter.
...
int main(int argc, char *argv[]) {
  ...
  saveExperimentsData();
  wb_supervisor_simulation_quit(EXIT_SUCCESS); // ask OmniSim to terminate
...
int main(int argc, char *argv[]) {
  ...
  controller->simulationQuit(EXIT_SUCCESS);  // ask OmniSim to terminate
...
Calling `wb_robot_step(0)` could be useful to send information to OmniSim when it is paused (such as modifying, moving, adding or deleting objects).
...
The current simulation mode can also be modified by the OmniSim user, when he's clicking on the corresponding buttons in the user interface.
...
The reset process is explained in detail in the [User Guide](../guide/the-user-interface.md#file-menu), the only difference is that the supervisor and robot controllers are not restarted, if needed, they have to be restarted with the `wb_supervisor_node_restart_controller` function.
...
> **Note** [C++ and Python]: Only for the C API it is required to pass NULL in order to perform a simple save operation.
...
The `wb_supervisor_export_image` function saves the current image of OmniSim main window into a JPEG file as specified by the `filename` parameter.
...
Field types are defined in "omnisim/supervisor.h" and include for example: `WB_SF_FLOAT`, `WB_MF_NODE`, `WB_SF_STRING`, etc.
...
The `wb_supervisor_field_enable_sf_tracking` function forces OmniSim to stream field data to the controller.
It improves the performance as the controller by default uses a request-response pattern to get data from the field.
...
> **Note** [C++ and Python]: In the oriented-object APIs, the SF\_* and MF\_* constants are available as static integers of the `Field` class (for example, Field::SF\_BOOL).
...
> **Note**: If a `wb_supervisor_field_set_*` function is executed just before a corresponding `wb_supervisor_field_get_*` function, in the same time step, the controller library will not send the query to OmniSim, but answer directly with the value that has just been set before.
...
The set operations are received by OmniSim from possibly several supervisors running concurrently.
...
> **Note**: Since OmniSim 7.4.4, the inertia of a solid is no longer automatically reset when changing its translation or rotation using `wb_supervisor_field_set_sf_vec2f` and `wb_supervisor_field_set_sf_rotation` functions.
If needed, the user has to explicitly call [this section](#wb_supervisor_node_reset_physics) function.

##### Examples

The "warehouse_husky.omniworld" world, located in the "projects/samples/demos/worlds/showcase" directory, provides a simple example for getting and setting fields with the above described functions.
...
The `wb_supervisor_field_import_mf_node_from_string` and `wb_supervisor_field_import_sf_node_from_string` functions import an OmniSim node into an `MF_NODE` or `SF_NODE` field.
...
```c
#include <omnisim/contact_point.h>
...
```python
from omnisim import ContactPoint
