...
OmniSim v5.0.0 — for the authoritative version and release notes, see the root [`CHANGELOG.md`](https://github.com/omnilink-tech/omnisim/blob/main/CHANGELOG.md).

%figure
![ImageData](images/1234web.png)
%end

This is the reference manual for OmniSim — a robotics simulator built to be driven by AI coding agents. It specifies the nodes and fields of the `.wbt` world description language and the API functions available to controller programs.

OmniSim is a fork of Webots and inherits its world-file format, controller API, and PROTO system. Pages in this book may still use Webots terminology where the behaviour is identical; OmniSim-specific divergences will be called out inline as the documentation is brought current.

For a contributor-facing view of the simulator internals (architecture, build/iteration, performance, OmniWorld procedural generation), see the [developer book](../developer/README.md).

Copyright &copy; {{ date.year }} the OmniSim contributors. Inherited content is Copyright &copy; Cyberbotics Ltd. and licensed under the Apache 2.0 license.
