...
The plugin code must contain user-implemented functions that will be called by OmniSim during the simulation.
...
Since OmniSim 7.2.0, the ODE physics library used in OmniSim is multi-threaded.
This allows OmniSim to run some physics simulation much faster than before on multi-core CPUs.
...
Before calling this function, OmniSim sets the current directory to where the plugin's ".dll", ".so" or ".dylib" was found.
...
Reinitializing the generator with a constant seed allows OmniSim to run reproducible (deterministic) simulations.
If you don't need deterministic behavior you should initialize the `srand` function with the current time: `srand(time(NULL))`.
OmniSim itself does not invoke the `srand` function; however, it uses the `rand` function, for example to add noise to sensor measurements.
...
If you don't wish to handle a particular collision you should return 0 to inform OmniSim that the default collision handling code must be used.
...
Finally, the `webots_physics_collide` function should return either 1 or 2 to inform OmniSim that this collision was handled.
If the value 2 is returned, OmniSim will moreover notify graphically that a collision occurred by changing the color of the corresponding boundingObject Geometry in the 3D view.

Since OmniSim 7.2.0, a multi-threaded version of ODE is used.
...
This callback is declared in `physics.h` but is **deprecated** and should not be implemented in new plugins.
If a plugin defines it, OmniSim logs a deprecation warning when the plugin is loaded.
