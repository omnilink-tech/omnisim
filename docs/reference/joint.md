...
In this case OmniSim keeps track of the initial positions of [Joint](#joint) nodes by means of hidden position fields.
...
For [HingeJoint](hingejoint.md) and [SliderJoint](sliderjoint.md) nodes containing no [JointParameters](jointparameters.md), OmniSim uses the hidden field named `position`.
For a [Hinge2Joint](hinge2joint.md) node, an additional hidden field named `position2` is used to store the joint position with respect to the second axis.
