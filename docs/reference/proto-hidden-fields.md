...
Still, this is not true for all field values, since OmniSim saves for you hidden PROTO fields which are bound to change over simulation time.
...
⚠️ **Velocities are no longer SAVED.** The hidden-field save of a Solid's `linearVelocity` / `angularVelocity` is UNIMPLEMENTED since the ODE deletion (2026-08-08, commit `bdc02139`) — both values stay NULL, so a world saved by the current engine contains no `linearVelocity_N` / `angularVelocity_N` lines at all, and the worked example further down this page shows output you will not get. Hidden `translation`, `rotation` and joint `position` fields are unaffected and still round-trip. Velocities that are already *written* in a file are still **read** and applied, so hand-authored initial velocities keep working; it is only the save side that is gone.

As in the case of non-PROTO objects, initial velocities of physical subparts of a PROTO were saved and could be subsequently restored when reloading your world file.
Like the other hidden fields, velocities were saved in the field scope of every top-level PROTO.
...
As an example, the excerpt below is representative of what a saved `.wbt` looks like after one simulation step (using a Pioneer 3-AT robot as the illustrative object).
