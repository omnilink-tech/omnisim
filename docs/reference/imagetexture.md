...
If the width or the height of a texture image is not a power of two, then an internal upscaling is performed and a warning is displayed in the OmniSim console.
...
```
i = current_url_index
generic_textures_path = "$OMNISIM_HOME/projects/default/worlds"
if is_absolute(url[i]) then
  ...
  else if defined_in_a_omnisim_PROTO(current_node) and exists(PROTO_path + url[i])
    return PROTO_path + url[i]
