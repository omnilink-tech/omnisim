...
- `snap`: ⚠️ **not implemented on the current engine — no alignment happens.** `OmConnector`'s `rotateBodies` and `snapOrigins` are empty stubs (both the ODE body handles they used are permanently NULL since the 2026-08-08 deletion), and the Newton weld path in `createFixedJoint` attaches the two bodies **wherever they currently are**. So a lock still forms and still holds — you just get it at the docking pose your approach produced, not a snapped-flush one. Approach accurately, or accept the offset. The description below is the intended semantics.
When TRUE: the two connectors do automatically snap (align, adjust, etc.) when they become docked.
...
A [Connector](#connector)'s axis system is displayed by OmniSim when the corresponding robot is selected or when *Display Axes* is checked in OmniSim *Preferences*.
