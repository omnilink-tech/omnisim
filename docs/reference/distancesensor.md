...
> ⚠️ **On `WorldInfo.newtonSolver "mujoco_warp"` the rays see the scene FROZEN AT ITS BUILD-TIME POSE — the rule there is STATIC TARGETS ONLY.**
> Rays are answered natively by an `mj_ray` service (default on), which serves off the CPU-side
> `mj_data` — and on the GPU `mujoco_warp` path newton steps `mjw_data` instead and never writes
> `mj_data` back. The ray's *origin* is unaffected (the engine builds it from its own scene-graph
> transform, fresh on both paths), so a **moving sensor against static geometry reads correctly**
> — measured 2026-08-12 to within **2e-05 m** on a solver-driven rover — while a sensor whose
> **target moves reads that target's authored pose for ever** (measured during raycast bring-up:
> pinned at 0.75 m while the CPU control tracked 0.75 → 3.9 → 1.3). Both results follow from one
> frozen scene, because a static geom's t=0 pose *is* its pose at every t. The decisive
> differential — a moving target under `mujoco_warp` — has not been run.
> The default CPU `mj_step` solver is unaffected. Detail, with line references:
> [newton-physics-backend.md](../guide/newton-physics-backend.md).
>
> ⚠️ Separately, and on **both** solvers: a node **deleted at runtime is never removed from the
> collision model**, so its geometry keeps blocking rays (and keeps producing contacts). Reload the
> world after removing collidable nodes.
...
There is no upper limit on the number of rays; however, OmniSim' performance drops as the number of rays increases.
...
A lookup table indicates how the value measured by OmniSim must be mapped to response values returned by the sensor (the distance returned by the `wb_distance_sensor_get_value` function in case of [DistanceSensor](#distancesensor)).
...
```python
from omnisim import DistanceSensor
