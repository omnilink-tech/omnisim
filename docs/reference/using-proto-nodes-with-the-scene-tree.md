...
Several PROTO examples are packaged with OmniSim.
...
In order to make a PROTO available to OmniSim, the complete PROTO definition must be placed in a ".proto" file.
...
In addition to the current project directory, OmniSim does also manage a *default* project directory.
This directory is structurally similar to the current project directory (see above) but it is located inside OmniSim distribution.
In the default project directory there is also a "protos" subdirectory that provides OmniSim standard PROTO nodes.
...
In the dialog, the PROTO nodes are organized using the same directory hierarchy found in the project's and OmniSim's "protos" folders.
...
Note that it is possible to create the ".png" files directly with OmniSim's menu `File / Take Screenshot...`.
