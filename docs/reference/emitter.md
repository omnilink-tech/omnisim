...
Channel 0 (the default) used to be reserved for communicating with a physics plugin; physics plugins were removed from OmniSim, so channel 0 is now an ordinary channel.
...
> **Note** [Python]: The `send` function accepts a `str`, a `bytes` object, or a list of `float`/`int`/`bool` values.
For sending primitive data types as binary, the *struct* module can be used.
This module performs conversions between Python values and C structs represented as `bytes` objects, which is the correct type for packed binary data.
Here is an example:
...
> **Note** [C++ and Python]: In the oriented-object APIs, the WB\_CHANNEL\_BROADCAST constant is available as static integer of the [Emitter](#emitter) class (Emitter::CHANNEL\_BROADCAST).
