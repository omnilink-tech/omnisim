...
Solid {
  ...
  SFString physicsBackend       "auto"       # {"auto", "ode", "newton"}
...
- `physicsBackend`: OmniSim-specific field selecting which physics backend simulates this [Solid](#solid). One of `"auto"` (the default — picks the Newton backend when it is available, otherwise falls back to ODE), `"ode"` (explicitly use ODE), or `"newton"` (explicitly use Newton, silently falling back to ODE if the Newton runtime is not reachable).
...
- `linearVelocity` and `angularVelocity`: these fields, which aren't visible from the Scene Tree, are used by OmniSim when saving a world file to store the initial linear and angular velocities of a [Solid](#solid) with a non-NULL [Physics](physics.md) node.
