...
Solid {
  ...
  SFString physicsBackend       "auto"       # {"auto", "ode", "newton"} -- "ode" no longer selects a working solver
  SFNode   boundingObject      NULL          # {node, PROTO}
  SFNode   physics             NULL          # {Physics, PROTO}
  SFBool   locked              FALSE         # {TRUE, FALSE}
  SFFloat  radarCrossSection   0.0           # [0, inf)
...
- `physicsBackend`: OmniSim-specific field naming which physics backend simulates this [Solid](#solid). Leave it out (or write `"auto"`, the default) and it resolves to Newton; `"newton"` says the same thing explicitly.
  ...
  > ⚠️ **`"ode"` still parses but no longer selects a working solver — do not use it.** ODE was
  > deleted on 2026-08-08 (commit `bdc02139`) and Newton/MuJoCo is the only backend. Reading the
  > engine source: `OmOdeBackend::isAvailable()` is now `false` and every one of its operations
  > returns `-1`, and `OmSolid::flushPendingNewtonRegistrations` **skips** any Solid whose effective
  > backend resolves to `"ode"` — so such a Solid is registered with no solver and **is not
  > simulated**. The value is still accepted only because an *undeclared* field is a parse ERROR
  > that would break every legacy world that sets it, and 21 tracked worlds still carry it.
  >
  > **The engine warns once per such Solid** (added 2026-08-08), but only for a Solid that declares
  > a `boundingObject` or `physics` — a collider-less visual node was never simulated anyway, so
  > warning about it would be noise. An *older log* has no such line, so a body that never falls or
  > a joint that never moves in an older world is still worth checking for `physicsBackend "ode"`
  > first.
  >
  > ⚠️ **It is also used deliberately as a "do not simulate this" switch, and that use is
  > load-bearing.** The 16 G1 ghost worlds pin their hologram robot to `"ode"` so its ~30
  > collider-less links stay out of the solver entirely; the robot is posed by its controller
  > instead. Removing those pins is not a cleanup — measured, it puts every link on the kinematic
  > mocap path and cost the flagship demo enough wall clock to miss its budget (t=8721 vs ~9700 of
  > a 900 s limit), while the walk itself stayed bit-identical, so a correctness check alone will
  > not catch the regression. If the idiom is ever retired it needs its own field first.

  The field **cascades**: an explicit `"newton"` or `"ode"` on any ancestor [Solid](#solid) — e.g. an outer `URDFRobot` — governs that whole articulation, because one coupled multibody system must resolve to a single solver. The world-level counterpart is `WorldInfo.defaultPhysicsBackend`, which supplies the backend for any Solid still on `"auto"`.
⚠ Writing `physicsBackend` inside a `WorldInfo` is an *error*, not an ignorable typo: the engine logs `Skipped unknown 'physicsBackend' field in WorldInfo node` and that ERROR line takes a headless run's exit code to 1. Background: [Physics (Newton)](../guide/newton-physics-backend.md).

- ⚠️ `immersionProperties`: **this field no longer exists.** It held `ImmersionProperties` nodes describing a Solid's interaction with `Fluid` nodes; both node types, and this field, were removed with the ODE backend, and buoyancy/drag is not simulated. Unlike the retired-but-parsed fields elsewhere in this manual, this one was deleted from the schema outright — so a legacy world still declaring `immersionProperties` gets a `Skipped unknown field` **ERROR**, which takes a headless run's exit code to 1. Delete the field from the world.
...
If the `physics` field is NULL then OmniSim simulates this object in *kinematics* mode.
...
- `linearVelocity` and `angularVelocity`: these fields, which aren't visible from the Scene Tree, store the initial linear and angular velocities of a [Solid](#solid) with a non-NULL [Physics](physics.md) node.
⚠️ **The SAVE half no longer works: OmniSim does not write these fields when saving a world.** `OmSolid`'s hidden-velocity save is UNIMPLEMENTED since the ODE deletion (2026-08-08, commit `bdc02139`) and both values stay NULL, so a saved world silently loses the dynamic state it used to carry. **Reading them still works** — a world that declares `linearVelocity` / `angularVelocity` does get those initial velocities applied to the Newton body — so hand-written initial velocities are unaffected, and only the save-and-restore round trip is broken.
...
The color of the `boundingObject` outline indicates the status of the object: usually the outline is represented by white lines, and these lines turn pink if the solid is colliding with another object.
⚠️ The **blue "idle" colour no longer appears.** It reported ODE's auto-disable, and Newton has no body sleep — `OmSolid::isSleeping()` unconditionally returns `false`, so nothing ever renders blue. Do not read a non-blue outline as "this body is active"; it says nothing either way.
...
For this reason since Webots R2018a (a baseline inherited by OmniSim), the user is encouraged to set unique names for sibling [Solid](#solid) nodes and a warning is printed in the console if this rule is not respected.
...
To smooth the creation of worlds, when a new [Solid](#solid) node is inserted from the GUI or from a [Supervisor](robot.md) controller, OmniSim checks the name of the just inserted node and automatically computes a unique name if needed.
