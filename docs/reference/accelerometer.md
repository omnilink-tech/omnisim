...
For example, if `zAxis ` is FALSE, then third element of the array returned by the `wb_accelerometer_get_values` function will always be *NaN*.
...
```python
from omnisim import Accelerometer
