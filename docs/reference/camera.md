...
Camera {
  ...
  SFString renderBackend          "wren"   # {"wren", "vulkan"}
}
```
...
If the `noiseMaskUrl` value starts with `http://` or `https://`, OmniSim will get the file from the web.
...
If both types of noise are enabled, the noise mask is applied **after** the gaussian noise. ⚠️ This line said "before" until 2026-08-22; the post-processing passes run in ascending `drawingIndex` (`src/wren/Viewport.cpp:95-102`) and `PP_NOISE_MASK` sorts after `PP_COLOR_NOISE` (`OmWrenRenderingContext.hpp`), so the code is unambiguous and the sentence was inverted.
...
- The `renderBackend` field selects the renderer used for this camera's images: `"wren"` (the default) or `"vulkan"`, which opts this camera into the GPU-driven renderer.
It falls back to WREN on builds or hosts without Vulkan support.
On the wgpu arm, `noise` and `motionBlur` are applied as a CPU post-process with the same formulas as the WREN shaders (per-channel additive gaussian with clamp; `mix(scene, last, 0.005^(rate/motionBlur))`) — statistically equivalent, not bit-identical, and the motion blur blends the final tonemapped bytes where WREN blends pre-tonemap HDR.
The `lens` distortion, the `focus` depth-of-field blur, the `lensFlare` effect and the `noiseMaskUrl` overlay are retired on wgpu: the fields keep parsing and the device warns by name that the effect is absent (a `lens` camera produces an undistorted image; the controller-facing focus API keeps working).
`OMNISIM_WGPU_SENSOR_POSTFX=0` disables the wgpu-side post-process (the device then warns that the authored `noise`/`motionBlur` is dropped).
...
In OmniSim, a camera can see as far as needed.
...
If the mouse cursor is over the overlay image and the simulation is paused, the RGB value of the selected pixel is displayed in the status bar at the bottom of the OmniSim window.
...
```c
#include <omnisim/camera_recognition_object.h>
...
```python
from omnisim import CameraRecognitionObject
