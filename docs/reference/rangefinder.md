...
- The `renderBackend` field selects the renderer that produces this range-finder's depth image: `"wren"` (the default) or `"wgpu"` (`"vulkan"` is an accepted alias), the same contract as the [Camera](camera.md)'s field. On the wgpu arm `noise`, `resolution` and `motionBlur` are applied as a CPU post-process with the same formulas as the WREN shaders (statistically equivalent, not bit-identical); a declared `lens` is NOT rendered there — the device warns that it produces an undistorted image. `OMNISIM_RANGEFINDER_WGPU=1` selects wgpu regardless of this field; `OMNISIM_WGPU_SENSOR_POSTFX=0` disables the wgpu-side post-process (the device then warns that the authored effect is dropped).
...
```cpp
#include <omnisim/RangeFinder.hpp>
...
```python
from omnisim import RangeFinder
