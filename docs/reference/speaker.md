...
If the file path is accessible by the robot controller either absolutely or relative to its working directory, it is loaded by the controller and its contents are streamed to OmniSim.
If the file is not found by the controller, the given path is passed to OmniSim.
OmniSim will first search for the path relative to the robot controller folder.
If the file is not found relative to the controller folder and the robot is a PROTO, OmniSim will search for the sound file relative to the folder of the PROTO file.
...
The value of the `text` parameter is converted into sound by OmniSim using the engine specified by the `wb_speaker_set_engine` function and the language specified by the `wb_speaker_set_language` function.
