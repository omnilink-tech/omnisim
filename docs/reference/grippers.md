...
- World friction matters — ⚠️ **but not through `ContactProperties`.** That node
  is retired and **not read**: a world declaring `coulombFriction 5` still runs
  at μ = 1.0, and `softCFM` is a deleted ODE concept. This exact confusion has
  cost real debugging time twice, and five shipped OmniArm 6 grasp demos were found
  declaring the `coulombFriction 5` a pinch needs while running at 1.0.
  Set friction on [WorldInfo](worldinfo.md) instead:
  `newtonGroundMu 3` (measured: what a two-finger friction pinch needs; above
  ~6 the floor itself destabilises), a firmer `newtonContactKe` (~8000, so the
  pads stop **at** the part's face instead of burying into it), and
  `basicTimeStep 8`. For a pinch that must not spin about the contact normal,
  add `newtonCondim 4`. Full recipe: [friction-grasp.md](../guide/friction-grasp.md).

Verified on a 6-DOF arm with this setup: cube lifted to z≈0.27 m and placed
within ~1 cm of the target, by friction alone.

⚠️ **The gripper palm usually has NO collider, and that is an engine
mechanism, not a modelling slip.** When a URDF link is joined to its parent by a
**fixed** joint, the engine folds it into the merge leader — the merged body
carries the summed mass, but **only the leader's own `boundingObject` registers**
(`src/omnisim/nodes/OmSolid.cpp:2742`). A gripper base link attached by a fixed
joint therefore contributes its mass and no collision geometry, so the palm
cannot collide with anything. `TouchSensor` and `VacuumGripper` were given
explicit un-fold gates for exactly this reason (`OmSolid.cpp:2664, 2699-2711`) —
a gripper palm link was not. Consequences: any collision-free approach validated
in simulation is validated against a tool that is missing a solid block at the
palm, in the optimistic direction, so cell clearances and "will the palm hit the
fixture" questions are unanswered by the sim; and a **palm wedge** (a part too
tall for the pads, jammed against the gripper body and carried while the fingers
do nothing) is structurally impossible in sim and therefore untestable there.
Confirm the geometry the solver actually built with
`OMNISIM_NEWTON_DUMP_MJMODEL`.
