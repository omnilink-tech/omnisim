...
OmniSim cannot load procedural PROTO nodes directly, therefore the procedural PROTO is first translated to pure JavaScript and the resulting script is evaluated by a JavaScript engine, in OmniSim's case using QJSEngine, and the result of this process yields a VRML97 compatible PROTO file which is then loaded by OmniSim as any other non-procedural would.
...
- JavaScript standard output and error streams are redirected on the OmniSim console (written respectively in regular and in red colors).
...
| webots\_version         | object representing the version of OmniSim with which the PROTO is currently used (keys: "major" and "revision")                                                                                                                |
| webots\_home            | absolute path to the OmniSim installation directory                                                                                                                                                                                        |
| temporary\_files\_path  | absolute path to the temporary folder currently used by OmniSim                                                                                                                 |
...
> ⛔ **`webots_version` and `webots_home` are the literal key names, and they must NOT be
> "rebranded".** They are emitted verbatim into the template context by
> [`OmProtoTemplateEngine.cpp:76`](../../src/omnisim/vrml/OmProtoTemplateEngine.cpp#L76) and `:84`,
> and mirrored by the web PROTO visualiser
> ([`TemplateEngine.js:58-59`](../../resources/web/wwi/protoVisualizer/TemplateEngine.js#L58)).
> Renaming them in this table would make the table false; renaming them in code would silently
> break every procedural PROTO that reads them.
...
]
{
  ...
  %<
    ...
    // print a welcoming message to the OmniSim console
