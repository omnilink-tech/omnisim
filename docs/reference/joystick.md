...
The [Joystick](#joystick) API doesn't correspond to an OmniSim node.
...
> **Note**: In C++ and Python the joystick functions are in a dedicated class called `Joystick`.
...
The OmniSim window must be selected and the simulation must be running.
All the buttons pressed can be read by calling the `wb_joystick_get_pressed_button` function repeatedly until this function returns -1.
...
Calling the `wb_joystick_get_pressed_button` function a second time will return either -1 or the button code of another button which is currently simultaneously pressed.
