...
> **Note**: In older OmniSim versions, `coulombFriction, bounce, bounceVelocity` and `forceDependentSlip` fields used to be specified in [Physics](#physics) nodes.
These fields have since been removed from the [Physics](#physics) node and now live exclusively in [ContactProperties](contactproperties.md) nodes, where these values must be specified.
