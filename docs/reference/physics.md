...
[Physics](#physics) nodes are used in most OmniSim worlds with the exception of some purely kinematics-based simulations.
...
Note that OmniSim ignores if the geometrical primitives intersect or not, the volume of each primitive is simply added to the total volume and finally multiplied by the density.
...
If this vector is different from [0 0 0], then the center of mass is depicted as a dark red/green/blue cross in OmniSim 3D-window.
...
If this field is empty, OmniSim will compute the inertia matrix automatically according to the position and orientation of the geometrical primitives in `boundingObject`.
    ...
    *products of inertia*. Together these values form a 3x3 inertia matrix:
    ...
    the [Solid](solid.md). These 6 values are passed unchanged to the physics
    backend. Since commit `fe35be64` the mass/inertia integrator is native
    (it replaced ODE's `dMassSetParameters` path when ODE was deleted), and the
    inertia tensors it computes for primitive `boundingObject`s are **bitwise
    identical** to the ODE values they replaced — those values are frozen in
    `tests/goldens/ode_oracle_goldens.json`.

    ⚠️ Leaving `inertiaMatrix` empty is the normal case, but be aware of what
    it used to cost: until `e7b9fb11`, every `Solid` with no explicit
    `inertiaMatrix` inherited a **husky-wheel-shaped inertia preset** on the
...
It is usually not necessary to specify all the [Physics](#physics) nodes in an OmniSim world.
...
That's because OmniSim uses a special *kinematics* algorithm for this kind of robots without [Physics](#physics).
However, if the [Physics](#physics) nodes are present then OmniSim uses the regular *physics* simulation algorithms.
...
If a [Solid](solid.md) child in the above sense is not related to its [Solid](solid.md) parent by a joint while both have a [Physics](#physics) node, they are *merged at the physics engine level*: the solver is given only one body to represent both parent and child.
...
> **Note**: When solid merging applies, only the highest ancestor of the rigid assembly has a body, and that body holds the physical properties of the whole assembly.
This may impact the way you address the assembly from a `Supervisor` controller.
