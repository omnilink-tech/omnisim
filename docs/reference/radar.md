...
Radar {
  ...
  SFFloat minRadialSpeed         0.0     # [0, maxRadialSpeed]
  SFFloat maxRadialSpeed         0.0     # {-1, [minRadialSpeed, inf)}
  ...
  SFFloat transmittedPower       20.0    # (-inf, inf)
...
```python
from omnisim import RadarTarget

class RadarTarget:
    @property
    distance, receiver_power, speed, azimuth
```
...
if (target created) {
  if (cellSpeed <= 0 and target is closer to another target than cellDistance)
    merge both targets
  else if (cellSpeed > 0 and speed difference between the two target < cellSpeed) {
