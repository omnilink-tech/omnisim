...
This is illustrated in the "husky_fleet_arena.wbt" sample demo, where the ten Husky robots run the same controller code but adopt different roles because their names ("husky_01" through "husky_10") are tested to determine their behaviour.
...
The message is sent using the `webots.window("<robot window name>").send` method of the OmniSim JavaScript API.
...
The message is received using the `webots.window("<robot window name>").receive` method of the OmniSim JavaScript API.
