...
The corresponding command line instruction will have the form:
    ...
    In case of C, C++ and Python controller programs the values are passed as arguments of the `main` function or method.
...
⚠️ **In practice joint motors no longer drain the battery.** Motor power is `force/torque feedback × consumptionFactor`, and that feedback returns a hardcoded `0.0` since the ODE deletion (see [Motor](motor.md)) — so unless something else in your model consumes energy, the level never falls and this cut-off never fires. A [Propeller](propeller.md)-driven RotationalMotor is the one motor that still reports real consumption.
...
The search algorithm works as following: Let $(VALUE) be the value of the `window` field, let $(EXT) be the shared library file extension of the OS (".so", ".dll" or ".dylib"), let $(PREFIX) be the shared library file prefix of the OS ("" on windows and "lib" on other OS), let $(PROJECT) be the current project path, let $(OMNISIM) be the OmniSim installation path, and let $(...) be a recursive search, then the first existing file will be used as absolute path:

    $(PROJECT)/plugins/robot\_windows/$(VALUE)/$(PREFIX)$(VALUE)$(EXT)

    $(OMNISIM)/resources/$(...)/plugins/robot\_windows/$(VALUE)/$(PREFIX)$(VALUE)$(EXT)
...
So for example if the simulation step (`WorldInfo.basicTimeStep`) is 16 ms and the control step is 64 ms, then OmniSim will always execute precisely 4 simulation steps during one control step.
After the 4th simulation step, OmniSim will wait for the controller's next control step (call to `wb_robot_step(64)`).

If `synchronization` is `FALSE`, the simulator will run as fast a possible without waiting for the control step.
So for example, with the same simulation step (16 ms) and control step (64 ms) as before, if the simulator has finished the 4th simulation step but the controller has not yet reached the call to `wb_robot_step(64)`, then OmniSim will not wait; instead it will continue the simulation using the latest actuation commands.
...
This function synchronizes the sensor and actuator data between OmniSim and the controllers.
If the `wb_robot_step` function is not called then there will be no actuation in OmniSim and no update of the sensors in the controller.
...
If this function returns -1, this indicates that OmniSim is about to terminate the controller.
This happens when the user hits the `Reload` button or quits OmniSim.
...
The controller termination cannot be vetoed: one second later the controller is killed by OmniSim.
...
When using `wb_robot_step`, the controller code is executed sequentially with the OmniSim simulation step, i.e., not in parallel.
This is due to the fact that a typical controller reads sensor information, makes some computation, orders motor commands and calls `wb_robot_step` which actually sends the motor commands and sensor requests and waits until OmniSim completes a simulation step, which may take some time depending on the complexity of the simulation.
During this time, the controller is idle, waiting for OmniSim to complete its simulation step.
On the other hand, prior to starting a new step, OmniSim waits for all the controllers to send their `wb_robot_step` messages which may induce some idle waiting time in OmniSim if a controller doesn't send quickly enough its `wb_robot_step` message because it is busy with some computation.
If the two computational processes (OmniSim and controller) are slow, it may be interesting to parallelize them.
`wb_robot_step_begin` and `wb_robot_step_end` allow you to achieve such an implementation.
They correspond to a split version of `wb_robot_step`, with the particularity that the code written between the two function calls is executed in parallel with the OmniSim simulation step.
`wb_robot_step_begin` and `wb_robot_step_end` both return -1 if the simulation is terminated.
Note that some OmniSim API functions cannot be called between `wb_robot_step_begin` and `wb_robot_step_end`, as they require immediate response from OmniSim using a request-response pattern.
...
OmniSim will warn you in case you call one of these functions between `wb_robot_step_begin` and `wb_robot_step_end`.
...
`wb_supervisor_field_enable_sf_tracking`, `wb_supervisor_node_enable_pose_tracking` and `wb_supervisor_node_enable_contact_points_tracking` force OmniSim to continuously stream the requested information to the controller.
By enabling the tracking, the corresponding supervisor functions can be called between `wb_robot_step_begin` and `wb_robot_step_end`, because their value will be queried to OmniSim during `wb_robot_step_begin` and received during `wb_robot_step_end`.
...
There is no equivalent of the `wb_robot_init` and `wb_robot_cleanup` functions in the C++ and Python APIs.
In these languages the necessary initialization and cleanup of the controller library is done automatically.

The `wb_robot_init` function is used to initialize the OmniSim controller library and enable the communication with the OmniSim simulator.
Note that the `wb_robot_init` function must be called before any other OmniSim API function.

Calling the `wb_robot_cleanup` function is the clean way to terminate a C controller.
This function frees the various resources allocated by OmniSim on the controller side.
In addition the `wb_robot_cleanup` function signals the termination of the controller to the simulator.
As a consequence, OmniSim removes the controller from the simulation which can continue normally with the execution of the other controllers (if any).
If a C controller exits without calling the `wb_robot_cleanup` function, then its termination will not be signaled to OmniSim.
...
Any subsequent OmniSim API function call will give unpredictable results.
...
int main() {
  /* initialize the OmniSim controller library */
  wb_robot_init();
...
```cpp
#include <omnisim/Robot.h>
#include <omnisim/DistanceSensor.h>
...
int main() {
  ...
  do {
    ...
    /* begin simulation step computation: send command values to OmniSim for update */
    /* leave the loop when the simulation is over */
    if (wb_robot_step_begin(TIME_STEP) == -1)
      break;

    /* the following code (until wb_robot_step_end) is executed in parallel with the OmniSim simulation step */
    ...
    /* end simulation step computation: retrieve new sensor values from OmniSim */
    /* leave the loop when the simulation is over */
...
If the specified device is not found, the function returns 0 in C or `None` in Python.

In C++, users should use the device specific typed methods, for example `getDistanceSensor`.
...
This function blocks the simulation and will return:
  ...
  - when OmniSim is about to terminate the controller (in that case `WB_EVENT_QUIT` is returned).
...
| Event                       | Purpose                                                   |
| --------------------------- | --------------------------------------------------------- |
| `WB_EVENT_QUIT`             | returned when OmniSim is about to terminate the controller |
...
This is illustrated in the "husky_fleet_arena.omniworld" sample demo, where the ten Husky robots run the same controller code but adopt different roles because their names ("husky_01" through "husky_10") are tested to determine their behaviour.
This sample world is located in the "projects/samples/demos/worlds/showcase" directory of OmniSim.
...
The OmniSim controller API is thread safe, however, some API functions use or return pointers to data structures which are not protected outside the function against asynchronous access from a different thread.
...
OmniSim nodes are squashed into a single URDF link node whenever possible to simplify the exported robot model.
In case you want an OmniSim node to be shown as a separate URDF link it is enough to define its `name` field.
URDF links inherit the `name` field from OmniSim node except when there is no `name` field defined or there are two or more OmniSim nodes with the name.
URDF joints are named after position sensor in the corresponding OmniSim joints.
...
```c
#include <omnisim/plugins/robot_window/default.h>
...
Such a window is embedded as a dockable sub-window in the OmniSim user interface.
...
The message is sent using the `send` method of the window's `RobotWindow` object.

The `wb_robot_wwi_send` and `wb_robot_wwi_send_text` functions allow a robot controller to send a message to a JavaScript function running in the HTML robot window.
The message is received by assigning to the `receive` method of that same `RobotWindow` object.

> **Note**: this page described both halves as `webots.window("<robot window name>").send` /
> `.receive` until 2026-08-16. That was wrong twice over. The canonical JavaScript global is
> `omnisim` — `webots` survives only as a deprecated alias bound to the *same* object
> ([`omnisim.js:374-377`](../../resources/web/wwi/omnisim.js#L374)) — and no `window(name)`
> accessor exists on either spelling anywhere in the tree. The shipped robot windows import
> the class directly and instantiate it:
>
> ```JavaScript
...
>   window.robotWindow.receive = (message, robot) => { /* handle it */ };
>   window.robotWindow.send('a message for the controller');
> };
> ```
>
> See [`resources/projects/plugins/robot_windows/generic/generic.js`](../../resources/projects/plugins/robot_windows/generic/generic.js) for a complete working example.
...
> **note**: `wb_robot_wwi_receive` and `wb_robot_wwi_send` functions are not available in the Python API.
