...
Lights have two purposes in OmniSim: (1) the are used to graphically illuminate objects and (2) they determine the quantity of light perceived by [LightSensor](lightsensor.md) nodes.
...
When activated, sharp shadows are casted from and received by any renderable object except for the semi-transparent objects, and the [IndexedLineSet](indexedlineset.md) and [PointSet](pointset.md) primitives.
...
- Build OmniSim from sources (instructions are available [here](../developer/quickstart.md)).

- Modify the number of lights of `gMaxActiveDirectionalLights`, `gMaxActivePointLights` and `gMaxActiveSpotLights` in `OMNISIM_HOME/src/wren/Constants.hpp`.

- Modify the number of lights of `maxDirectionalLights`, `maxPointLights` and `maxSpotLights` in the following shaders:
  - `OMNISIM_HOME/resources/wren/shaders/pbr.frag`
  - `OMNISIM_HOME/resources/wren/shaders/pbr_stencil_diffuse_specular.frag`
  - `OMNISIM_HOME/resources/wren/shaders/phong.frag`
  - `OMNISIM_HOME/resources/wren/shaders/phong_stencil_ambient_emissive.frag`
  - `OMNISIM_HOME/resources/wren/shaders/phong_stencil_diffuse_specular.frag`
  - `OMNISIM_HOME/resources/wren/shaders/shadow_volume.vert`

- Compile OmniSim again.
