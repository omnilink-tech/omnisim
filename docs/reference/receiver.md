...
> **Note**: OmniSim' Emitter/Receiver API guarantees that:
...
The *data size* is always equal to the *size* argument of the corresponding `wb_emitter_send` function call.
...
If the packet has no emitter, the returned value will be positive infinity.
...
If the packet has no emitter, the returned values will be NaN (Not a Number).
