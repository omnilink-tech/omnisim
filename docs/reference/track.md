...
> ⚠️ **TRACK PROPULSION DOES NOT REACH THE PHYSICS ON THE CURRENT ENGINE — a `Track` will not
> drive a robot.** The belt was simulated by writing a *contact surface velocity* into ODE's
> contact `motion1` parameter, and ODE was deleted on 2026-08-08 (commit `bdc02139`). Two
> independent breaks remain in `OmTrack`: the body handle the surface-velocity update needs is
> permanently NULL (`OmSolidMerger`'s body is only ever set to `NULL`), so the update never runs
> and `mSurfaceVelocity` stays 0; and even if it did, `contactSurfaceVelocity()` now has **no
> consumers** — Newton has no equivalent of ODE's `motion1`. Force control on a track motor is
> explicitly UNIMPLEMENTED as well.
>
> The `Track` still renders, its `animatedGeometry` still animates from the motor command, and
> its `boundingObject` still collides — so a tracked robot sits on the ground and looks right,
> and simply does not move. That silence is why this is flagged at the top of the page.
>
> **What to do instead:** model a tracked vehicle as a skid-steer with wheels (the pattern every
> shipped mobile robot in this tree uses), and keep the track geometry as a visual child with no
> `boundingObject`.
