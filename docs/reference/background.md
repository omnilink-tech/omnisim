...
If a `*Url` value starts with `http://` or `https://`, OmniSim will get the file from the web.
...
This [set of image tools]({{ url.github_tree }}/scripts/image_tools) provides basic conversion tools to transform your background resources to OmniSim ones.
...
The `atmosphericSky` field (an OmniSim addition) replaces the cubemap skybox with a Hillaire 2020 procedural atmospheric sky when set to one of the recognized presets `"earth"` or `"mars"`.
The default empty value `""` leaves the cubemap path untouched.
When a preset is active, the sun direction and colour are taken from the first `DirectionalLight` in the scene, and the procedural sky also drives the PBR irradiance used for reflections.
An unrecognized preset emits a warning and falls back to the cubemap skybox.
