...
```cpp
#include <omnisim/utils/Motion.hpp>
...
If an error occurred, an error message is printed to OmniSim' console, and `NULL` is returned.
...
> **Note** [C++ and Python]: The constructor and destructor of the Motion class are used instead of the `wbu_motion_new` and `wbu_motion_delete` functions.
...
In this case OmniSim computes intermediate joint positions by linear interpolation.
