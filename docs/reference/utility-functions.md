...
It is highly recommended to test for NULL returned values, because passing a NULL dBodyID to an ODE function is illegal and will crash the plugin and OmniSim.
...
The dGeomID returned by OmniSim corresponds to the boundingObject of the [Solid](solid.md).
...
This function searches the Scene Tree recursively therefore it is recommended to store the result rather than calling it at each step.
It is highly recommended to test for NULL returned values, because passing a NULL dGeomID to an ODE function is illegal and will crash the plugin and OmniSim.
...
This is an alternative to calling the `dWebotsGetBodyFromDEF` function described above.
...
This corresponds to the time displayed in the bottom right corner of the main OmniSim window.
...
This function prints a line of formatted text to the OmniSim console.
