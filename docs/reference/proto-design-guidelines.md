...
This ensures that your new PROTO nodes will be well designed, easy-to-use and consistent with the existing PROTO nodes provided in OmniSim.
...
It should be the following: `#OMNISIM R2025a utf8`.
`R2025a` is the version of OmniSim needed to run this PROTO; every PROTO shipped in this tree carries it.

> **Note**: this page told authors to write `#VRML_SIM {{ webots.version.major }} utf8` until
> 2026-08-16. The header token migrated `#VRML_SIM` → `#OMNISIM` under the same **dual-read,
> single-write** rule as the world-file extension: the tokenizer accepts either spelling forever
> ([`OmTokenizer.cpp:156`](../../src/omnisim/vrml/OmTokenizer.cpp#L156) matches
> `^(?:OMNISIM|VRML_SIM)`), so existing PROTOs keep loading — but nothing should **write**
> `#VRML_SIM` any more, and the new-PROTO wizard does not
> ([`OmNewProtoWizard.cpp:270`](../../src/omnisim/gui/OmNewProtoWizard.cpp#L270) emits
> `#OMNISIM %release%`). The `{{ … }}` form was a documentation placeholder, never a template
> expression the header line evaluates.
It should be followed by a number of comments describing the PROTO.
These comments should be formatted as described in the [example](#example) below to be displayed properly by OmniSim.
...
- `deprecated` means this PROTO should not be used any more in new simulations, but is kept for backwards compatibility. When using a deprecated PROTO, OmniSim will display a warning message about it.
- `hidden` tells OmniSim not to display this PROTO in the Add Node dialog when the user wants to insert a new PROTO.
...
- `nonDeterministic` tells OmniSim that this PROTO may yield a different result with each execution.
...
- `no3dView` tells the asset catalog service not to load the 3D view when displaying the page of this PROTO.
...
The `keywords:` comment should be added to classify the proto correctly on the asset catalog service.
...
Be careful to avoid as much as possible `Solid.name` values which are automatically generated by OmniSim, such as `solid(1)`.
