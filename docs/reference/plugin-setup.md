...
You can add a new plugin by using the `File > New > New Physics Plugin...` menu item.
...
The extension will be added by OmniSim depending on the platform: it will be ".so" (Linux), ".dll" (Windows) or ".dylib" (macOS) for the binary file.
...
If OmniSim does not find the file there, it will also look in the "[OMNISIM\_HOME/resources/projects/plugins/physics]({{ url.github_tree }}/resources/projects/plugins/physics)" directory.
