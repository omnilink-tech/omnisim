...
The look is the same one shipped with the Spot demo at [`projects/robots/boston_dynamics/spot/worlds/spot.wbt`](../projects/robots/boston_dynamics/spot/worlds/spot.wbt) — that's the reference world.
...
Webots PROTOs are single-rooted, which is why the recipe is three nodes rather than one — `Background`, `DirectionalLight`, and `Robot` can't share a parent.
...
[`projects/robots/boston_dynamics/spot/worlds/spot.wbt`](../projects/robots/boston_dynamics/spot/worlds/spot.wbt) is the canonical recipe consumer. When in doubt, copy its lighting block.
