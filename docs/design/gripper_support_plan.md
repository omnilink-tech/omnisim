...
- No gripper *type*, no width/force control, no feedback (holding?
  object_present?).
- A Webots `VacuumGripper` device exists but is not wired into the bridge.
...
- ROADMAP notes "gripper sim is shaky"; the early arm assembly demo used a
  Supervisor-teleported fake gripper because real Robotiq PROTOs hung Webots.
...
- `VacuumEffector` — wraps the Webots `VacuumGripper` device
