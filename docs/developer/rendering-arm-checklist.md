...
[WbView3D.cpp:1548](../../src/omnisim/gui/WbView3D.cpp)) is now UN-GATED for any
...
- [x] WbCamera migrated onto the shared `WbWgpuSceneRenderer`.
...
- [x] T1.2 CSM **multi-cascade receiver shader** — `kSolidLitCsm` ([WbWgpuShaders.cpp](../../src/omnisim/render/WbWgpuShaders.cpp)):
      ...
      **NEXT (on-GPU half, needs a wgpu engine relink):** render N depth-map layers (drive
      ...
      pipeline/bind-group in `WbWgpuRenderTarget` + A/B a cascade-split render.
- [x] T1.2 CSM **end-to-end render proof** — [docs/developer/csm_render_prototype.py](csm_render_prototype.py):
      ...
      **Engine-wiring caveat (coordination):** the single-cascade runtime probe entry lives in
      `WbCamera.cpp` (`OMNISIM_CAMERA_SHADOW(PROBE)`), which is NOT an L2-owned file, so wiring an
      in-engine CSM probe to runtime-verify the `WbWgpuRenderTarget` translation needs an
...
- [x] T1.2 CSM **on-GPU engine wiring + in-engine verification** — the multi-cascade design is now LIVE
      in the engine (no longer Python-only). `WbWgpuRenderTarget` gains three render-layer methods
      ([render/WbWgpuRenderTarget.{cpp,hpp}](../../src/omnisim/render/WbWgpuRenderTarget.cpp)):
      ...
      maths/nodes up-dependency — `WB_RENDER_INCLUDE` omits `-Imaths`/`-Inodes` by design); the WbMatrix4 +
      cascade math lives in `WbWgpuSceneRenderer::csmSelfTest` (nodes/, which owns both). **Resolved the
      engine-wiring caveat** (the old `OMNISIM_CAMERA_SHADOW` trigger was in non-L2 `WbCamera.cpp`): a new
...
- [x] T1.2 CSM **main-view wiring (L2→L3 seam, gated)** — `clearAndDrawSceneCsm` is now wired into the
      live main-view render (`renderMainFrameViaWgpu` in L3-owned [gui/WbView3D.cpp](../../src/omnisim/gui/WbView3D.cpp))
...
- [x] T1.2 CSM **full-material × multi-cascade merge (L2) + main-view upgrade (L3)** — the gated
      ...
      New L2 shader `kSolidLitTexturedCsm` ([render/WbWgpuShaders.cpp](../../src/omnisim/render/WbWgpuShaders.cpp))
      ...
      material bind groups). The gated main-view branch ([gui/WbView3D.cpp](../../src/omnisim/gui/WbView3D.cpp))
...
- [x] **3c** — two-pass orchestration wired into WbCamera
...
- [x] **3d** — cross-object cast shadow onto a SEPARATE receiver. **DONE +
      ...
      light, differing only by the caster: `camera_wgpu_shadow_cast.wbt` (has an
      elevated caster) and `camera_wgpu_shadow_nocaster.wbt` (control, caster
...
- [~] **T1.3 fog** — the analytic distance-fog FOUNDATION is now in-engine + verified (full volumetric
      ...
      ([render/WbWgpuShaders.cpp](../../src/omnisim/render/WbWgpuShaders.cpp)) — a fullscreen post pass
      ...
      transmittance `1-exp(-density·dist)` — plus `WbWgpuRenderTarget::resolveFog` (4-entry layout:
...
- [~] **T1.4 TAA** — the temporal-RESOLVE pass is now in-engine + verified (history reproject + 3×3
      ...
      ([render/WbWgpuShaders.cpp](../../src/omnisim/render/WbWgpuShaders.cpp), the WGSL port of
      taa-preview.html's resolveFS) + `WbWgpuRenderTarget::resolveTaa` (a fullscreen post pass, 4-entry
      ...
      verified** — `WbWgpuRenderTarget::accumulateTaa` (two RGBA8 RenderAttachment|TextureBinding|CopySrc
      ...
      landed + verified** — `WbWgpuSceneRenderer::haltonJitter` (Halton(2,3) 8-frame ±amp px) +
...
- [x] **R4-step-1: on-screen swapchain/surface bound to a real Win32 window —
      ...
      `WbWgpuSurface` (`src/omnisim/render/WbWgpuSurface.{hpp,cpp}`) creates a
      ...
      per frame. Driven by `WbWgpuView` (`src/omnisim/gui/WbWgpuView.{hpp,cpp}`), a
      ...
      `OMNISIM_VIEW3D_WGPU=1` (spawned from `WbView3D` ctor); default-off ⇒ shipped
      GUI byte-unchanged. Added `instance()`/`adapter()` accessors to
      `WbVulkanBackend`. **Evidence:** with the flag set, a window titled
...
- [x] **R4-step-2: lit, depth-tested 3D rendered into the surface — ✅ VISUALLY +
      NUMERICALLY VERIFIED 2026-06-04 (wgpu-ON build, commit pending).** `WbWgpuSurface`
      ...
      `WbWgpuView` renders a mimosa unit box spinning about +Z, fixed camera at
      (-2.5,0,0) via `WbWgpuSceneRenderer::buildViewProj`. **Numerical self-check
...
- [x] **R4-step-3a: live world from the live Viewpoint camera — ✅ NUMERICALLY VERIFIED
      2026-06-04 (panda.wbt, commit pending).** `WbWgpuView::drawWorld` builds a camera
      matrix from the live main `WbViewpoint` (`buildViewpointCamera`) and renders
      `WbWgpuSceneRenderer::collectWorldDraws` each frame, so the wgpu window mirrors AND
      ...
      `WbRotation::direction()` is the toward-scene look vector, so `buildViewProj` wants
...
- [x] **R4-step-3b: wgpu view composited INTO the main window — ✅ 2026-06-04 (commit
      pending).** `WbSimulationView` creates `WbWgpuView` and embeds it via
...
- [x] **R4-step-3b polish: FOV matches WREN — ✅ 2026-06-04 (commit pending).** Replicated
      `WbViewpoint::updateFieldOfViewY`'s VRML rule (fieldOfView taken on the LARGEST
...
- [ ] **R4-step-3:** composite the wgpu surface into the main Qt layout / drive the
      real `WbView3D` viewport (vs the current standalone sibling window). This is the
