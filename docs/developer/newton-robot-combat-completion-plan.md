...
> **⚠ 2026-08-08 — THE VERIFICATION STRATEGY IN THIS DOC IS NO LONGER RUNNABLE.** `bdc02139`
> deleted the vendored ODE library; Newton with `SolverMuJoCo` is the only physics backend.
> Every gate below that works by *comparing Newton against ODE* is dead — the P0 "newton vs ode
> trajectories diverge" proof, the `husky_head_on_ode.wbt` golden world, the P4 "within 10× of
> the ODE golden" exit criterion, the `drive_test.py ode` arm. There is no second backend to
> compare against — and worse, a world still pinning `physicsBackend "ode"` **still loads and
> still runs, with NO PHYSICS AT ALL, SILENTLY**: an explicit `"ode"` still wins
> (`OmSolid::effectivePhysicsBackendName`) and is handed an inert stub, with no FATAL, no ERROR
> and no warning. Nothing moves and nothing collides, and the run looks like a result.
> The dated measurements and commit hashes below stand as history and are preserved verbatim;
> what is lost is the ability to re-derive them or to re-run the parity checks — a
> **Newton-only baseline must be re-captured** before any damage-parity number here can be
> re-asserted. And the silent-wrong-backend failure mode has **not** gone away, it changed shape:
> an *installed-but-broken* Newton runtime is now a hard FATAL, but an **ABSENT** runtime is still
> silent and lands on the inert stub — i.e. no physics, no warning. **Set `OMNISIM_REQUIRE_NEWTON=1`
> on any combat run whose result you intend to trust, and prove the backend from the `.newton.json`
> sidecar — never from the exit code.** Record:
...
The only surviving `ode` pin was `projects/robot_combat/worlds/tests/husky_head_on_ode.wbt`, kept **deliberately**
as the ODE-golden parity reference. ⚠ **Since `bdc02139` that world is a TRAP, not a reference.** It still
loads and still runs — an explicit `"ode"` pin still wins and is handed the inert stub — so it produces a
run with **no physics at all, silently**: no FATAL, no ERROR, no warning, no bot movement, no collisions.
A capture against it will still emit a JSONL, of a world where nothing happened. **The parity reference no
longer exists, and the world that used to hold it now yields a plausible-looking fake.**
...
confirmation that the prior "verified on Newton" sign-off was not a silent ODE fallback. ⚠ That last item is
**historical only**: after `bdc02139` there is no ODE backend to fall back to, so the failure mode it guarded against
cannot recur — but neither can the check that detected it be re-run.

## What is already fixed (do not re-solve)

| Symptom | Status | Commit / mechanism |
|---|---|---|
| Child-rotation drop (wheels/weapon spawn ~90° off, then tumble) | **FIXED** | `98409b28` (2026-06-14) — `OmBasicJoint.cpp:377` bakes `R_child^T·R_parent` into the revolute `child_xform`; `OmNewtonBackend.cpp:1008` |
...
and the Newton wheel-lock issue ("UNFIXED", `drive_test.omniworld`/`drive_demo` — neither exists in the tree).
...
1. **🔴 Confirm Newton is actually running.** `_scratch/traj_battlebox_duel.wbt_{newton,ode}.txt` are **byte-for-byte
   ...
   makes the embedded interpreter miss `warp` → silent ODE fallback" trap (`OmPhysicsBackend.cpp:325`), the entire prior
   ...
   ⚠ **2026-08-08 — closed by deletion, not by measurement.** The two-trajectory divergence test has no ODE arm any
   more, so it cannot be re-run; the byte-identity observation above is a preserved historical datum. The failure mode
   it detected has **changed shape, not disappeared**: an installed-but-broken Newton runtime now FATALs, but an
   *absent* one silently lands on the inert no-physics stub. Read the engine's `.newton.json` verdict sidecar for
   the backend attribution, and set `OMNISIM_REQUIRE_NEWTON=1` to make the absent case loud.
...
3. **🟠 No combat world sets the Newton runtime flags** (`grep` confirms zero set `newtonRobotColliders` /
   ...
   - **Chassis = 1mm placeholder collider** → ramming produces no chassis contacts → the core mechanic deals **zero
     damage**. Every Robot wrapper (inlined BattleBots *and* URDF huskies — both are `OmRobot`, both get the placeholder)
...
7. **⚪ Deferred (needs native rebuild):** clean contact-impulse/depth API (retire the velocity proxy); true cylinder
   ...
   (`OmSolid.cpp` `addNewtonPrimitive` still emits `addShapeSphere("was cylinder")`).
...
- From **PowerShell** (not bash), launch `battlebox_duel.omniworld` with the 2026-06-18 `omnisim-bin.exe`.
- Assert: (a) `[OmNewtonBackend]` "world opened" + per-hinge registration + "registered N dynamic + M static bodies"
  census lines; (b) newton vs ode trajectories **diverge** — ⚠ **unrunnable since `bdc02139`**; the only backend
  attribution available now is the `.newton.json` verdict sidecar (`{backend, solver, finalised, degraded}`);
  (c) a single bot's chassis translates several meters under sustained drive (not spin-in-place).
- **Exit:** Newton confirmed live (sidecar present, `degraded: false`) + bots drive. The old "if silently ODE, stop and
  re-scope" arm has a successor, not a retirement: run under `OMNISIM_REQUIRE_NEWTON=1`, because an absent Newton
  runtime silently lands the world on the inert no-physics stub and every bot simply stands still.
...
- Bake `OMNISIM_NEWTON_SUBSTEPS` + `OMNISIM_DAMAGE_VEL_SMOOTH` into `scripts/battlebox_tournament.sh` /
  `scripts/combat_run.sh`. ⚠ **The ODE-golden baseline (`husky_head_on_ode.wbt`) can no longer be re-captured** —
  no ODE backend exists to produce it. Worse: the capture command **still runs and still writes a JSONL**,
  because the `"ode"`-pinned world loads onto the inert stub and simulates nothing. Do not mistake that
  file for a golden. Re-tune per-world HP/threshold `customData` if needed.
- **Exit:** ⚠ **the "within 10× of the ODE golden" criterion has no reference side any more.** It must be replaced by a
  **Newton-only baseline** — a fresh dated damage-event capture on the surviving Newton head-on world, checked in as the
  reference, with the tolerance re-derived against it. Until that exists, P4 has no exit test.
...
- **Biggest unknown:** whether the prior migration's "verified on Newton" evidence is real or a silent ODE fallback
  (P0 settled this in the affirmative on 2026-06-20 — it *had* been ODE). ⚠ **Retired as a risk since `bdc02139`**: no
  fallback backend exists, so nothing can silently run on ODE; the compensating loss is that the divergence check that
  would re-prove it is unrunnable.
...
- Run from **PowerShell**, not MSYS bash (bash → the embedded interpreter misses `warp`). ⚠ The *consequence* changed
  with `bdc02139` and got **worse, not better**: this used to degrade silently to ODE and still simulate; now there is
  nothing to degrade to, so it degrades silently to the **inert stub and simulates nothing at all** — same silence,
  emptier result. **`OMNISIM_REQUIRE_NEWTON=1` is what makes it loud.** The PowerShell advice stands unchanged.
...
- Engine: `src/omnisim/nodes/OmSolid.cpp` (collider decision 3105-3146, statics harvester 2984-3001),
  `src/omnisim/nodes/OmWorldInfo.cpp` (flag parsing 48-51),
  `src/omnisim/physics/OmNewtonBackend.cpp`, `src/omnisim/nodes/OmBasicJoint.cpp`
- Launch scripts: `scripts/battlebox_tournament.sh`, `scripts/combat_run.sh`
- Reference world (template for flags): `projects/samples/demos/worlds/showcase/warehouse_husky.omniworld`
...
predated `newtonRobotColliders`). Confirmed at `OmSolid.cpp:3105-3146` that *every* Robot wrapper —
...
- The bash launch scripts (`battlebox_tournament.sh`, `combat_run.sh`) silently run ODE from bash
  ...
  ⚠ **Since `bdc02139` they no longer silently run ODE — they silently run NOTHING.** A warp-less bash launch lands on
  the inert stub: no physics, no warning, a scorecard of a match that never moved. Unless the run sets
  `OMNISIM_REQUIRE_NEWTON=1`, nothing distinguishes it from a real draw. `combat_match.py` is still the recommended
  runner.
...
1. **Flap = wheel collider orientation.** Each BattleBot wheel Solid carries `rotation 1 0 0 1.5708`
   ...
   (`OmNewtonBackend.cpp add_shape_cylinder`) rotates the capsule −90° about X assuming a body-Y
...
2. **Lock = XPBD cannot drive a multi-wheel rover.** Newton's default **XPBD** solver locks
   **laterally-paired (left/right) wheels** — proven in pure Python: a 1-wheel rover drives 6 m, a
   ...
   unlock it. **MuJoCo and ODE drive every case** (measured 2026-06-20, when both still existed — the ODE half of that
   comparison is now unrepeatable). This hits *every* 4-wheel robot under XPBD. ⚠ Moot since `94f04222` removed XPBD and
   `7b431e81` made `SolverMuJoCo` the default and now only solver.
...
`orc_*`, `newton_husky_combat_2`). Verified: `drive_test.omniworld` rolls 11 m straight; `battlebox_duel`
...
**Regression + tooling:** `projects/robot_combat/worlds/tests/drive_test.omniworld` (two rovers: current
"flapper" authoring vs husky "roller") + `projects/robot_combat/controllers/drive_demo`, run via
`scripts/dev/drive_test.py newton` (⚠ the `ode` argument is **invalid since `bdc02139`** — it selects nothing, and
the run still completes on the inert stub with no physics, so it reports a rover that "did not drive" rather
than failing). Pure-Python repros: `scripts/dev/wheel_lock_repro.py`,
