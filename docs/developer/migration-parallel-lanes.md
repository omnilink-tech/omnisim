...
"spine" files that a whole arm funnels through (`WbNewtonBackend.cpp` on physics;
`WbWgpuShaders.cpp` / `WbWgpuRenderTarget.cpp` / `WbWgpuSceneRenderer.cpp` on
...
|---|------|------------------|-------------|--------------|
| **L1** | Newton physics core | `physics/WbNewtonBackend.{cpp,hpp}`, `nodes/WbSolid.{cpp,hpp}`, `nodes/WbBasicJoint.{cpp,hpp}` | newton-ode-replacement-plan.md | ✅ fully |
| **L2** | wgpu render fidelity | `render/WbWgpuShaders.{cpp,hpp}`, `render/WbWgpuRenderTarget.{cpp,hpp}`, `nodes/WbWgpuSceneRenderer.{cpp,hpp}` | rendering-arm-checklist.md | ⚠️ interface-seam with L3 |
| **L3** | wgpu viewport + surface | `gui/WbView3D.{cpp,hpp}` (wgpu path only), `gui/WbWgpuView.{cpp,hpp}`, `gui/WbSimulationView.{cpp,hpp}`, `render/WbWgpuSurface.{cpp,hpp}` | r4-completion-checklist.md, r4-step3c-plan.md | ⚠️ interface-seam with L2 |
...
- **The architectural lever (R3.7 GPU interop)** — Newton GPU body buffer read directly by the wgpu vertex shader. Touches L1's `WbNewtonBackend` *and* L2's `WbWgpuRenderTarget`, so it **cannot** be disjoint. It runs **last, as a convergence step**, after L1 + L2 reach their checkpoints — not in parallel.
...
2. **Frozen — nobody edits these during parallel work:**
   - `physics/WbPhysicsBackend.{cpp,hpp}` — dispatcher surface, signed off as final
     ([dispatcher-surface-signoff.md](dispatcher-surface-signoff.md)). No new virtuals.
   - `physics/WbOdeBackend.*`, `render/WbWrenBackend.*` — legacy, must stay byte-identical (migration non-negotiables #1/#2).
   - `render/WbRenderBackend.*`, `render/WbVulkanBackend.*`, the render adapters/caches (`WbWgpuMesh*`, `WbWgpuTexture*`, `WbWgpuImageAdapter`, `WbWgpuGlBlit`) — stable plumbing.
   - The WREN code path inside `gui/WbView3D.cpp` (L3 edits the **wgpu** region only; the WREN render path stays byte-identical).
...
4. **The two interface seams** (the only sanctioned cross-lane coupling):
   - **L2 ↔ L3:** L3 *calls* `WbWgpuRenderTarget` + `WbWgpuSceneRenderer` public
   ...
   - **L1 ↔ L2/L3:** `WbWgpuSceneRenderer` (L2) reads `WbSolid`'s (L1) public
...
**Owns:** `src/omnisim/physics/WbNewtonBackend.{cpp,hpp}` (incl. the embedded
Python runtime source), `src/omnisim/nodes/WbSolid.{cpp,hpp}`,
`src/omnisim/nodes/WbBasicJoint.{cpp,hpp}`.
...
- **Engine-side legged dynamic fidelity** (the Spot/Newton roll-instability root cause,
  if it lands in `WbNewtonBackend`) — see boundary with L5.
...
**Definition of done:** coverage meter ≥ ~99% faithful on the corpus with ODE as the documented residual.

---

## L2 · wgpu render fidelity (offscreen pipelines + shaders)

**Owns:** `src/omnisim/render/WbWgpuShaders.{cpp,hpp}`,
`src/omnisim/render/WbWgpuRenderTarget.{cpp,hpp}`,
`src/omnisim/nodes/WbWgpuSceneRenderer.{cpp,hpp}` (draw collection + material harvest).
...
**Seam:** keep `WbWgpuRenderTarget.hpp` + `WbWgpuSceneRenderer.hpp` public signatures
...
**Owns:** `src/omnisim/gui/WbView3D.{cpp,hpp}` (**wgpu region only** —
`renderMainFrameViaWgpu` and below; the WREN path is frozen),
`src/omnisim/gui/WbWgpuView.{cpp,hpp}`, `src/omnisim/gui/WbSimulationView.{cpp,hpp}`,
`src/omnisim/render/WbWgpuSurface.{cpp,hpp}`.
...
- **Harden the on-screen surface** (`WbWgpuSurface`) — resize, multi-monitor, context-loss, robustness — so the wgpu pane can be *the* main view.
...
per-contact data — `cp.depth` (ODE wire, already landed) and the native Newton
contact source (already wired under `OMNISIM_NEWTON_NATIVE_CONTACTS`) — and retire
...
**Verify:** head-on damage capture, ODE vs Newton event-count parity.
...
Spot roll-instability if it turns out to be a `WbNewtonBackend` control-bridge bug)
...
`WbNewtonBackend`. Pure-Python/controller work → disjoint from the engine.
...
**Scope:** **bundle the Newton runtime** (`warp` + `newton`) so a stock install
resolves `auto`→Newton instead of silently falling back to ODE (today it is NOT
bundled); **ship `wgpu_native`** in the release build matrix across platforms. This
...
**Caveat:** has a few integration points with L1 (`WbSolid`/scene throttling for
`granularMode`) — keep those minimal and coordinate via the tracker; if it needs
real `WbNewtonBackend`/`WbSolid` edits, sequence behind L1 rather than racing it.
...
- **The two default flips** (`physicsBackend`→newton once L1+L6 are ready; `renderBackend`→wgpu once L2+L3+L7 hit parity) — human-gated, done by the user / integrator, not inside a lane.
...
- **L1 (Newton core):** "Take lane L1 (Newton physics core) from `migration-parallel-lanes.md`. Owned files: `physics/WbNewtonBackend.{cpp,hpp}`, `nodes/WbSolid.{cpp,hpp}`, `nodes/WbBasicJoint.{cpp,hpp}`. Work the W-series in `newton-ode-replacement-plan.md` (PAL writes, joints, control ops, contact impulse under XPBD, W4.3). Verify with `physics_oracle.py --gate --require-newton` + `newton_coverage.py`. [+ shared contract]"
- **L2 (Render fidelity):** "Take lane L2 (wgpu render fidelity). Owned files: `render/WbWgpuShaders.{cpp,hpp}`, `render/WbWgpuRenderTarget.{cpp,hpp}`, `nodes/WbWgpuSceneRenderer.{cpp,hpp}`. Work the fidelity ladder in `rendering-arm-checklist.md` (full CSM, TAA, fog, T2–T5, 75→100% parity). Keep public `.hpp` signatures stable for L3. Verify on the offscreen golden path (`render_oracle.py` + `OMNISIM_PROBE_*`). [+ shared contract]"
- **L3 (Render viewport):** "Take lane L3 (wgpu viewport + surface). Owned files: `gui/WbView3D.{cpp,hpp}` (wgpu region only), `gui/WbWgpuView.{cpp,hpp}`, `gui/WbSimulationView.{cpp,hpp}`, `render/WbWgpuSurface.{cpp,hpp}`. Work `r4-completion-checklist.md` / `r4-step3c-plan.md` (harden surface, 3c-C, default-flip prep). Call L2's render APIs, don't edit them. Verify with `OMNISIM_VIEW3D_WGPU_SELFCHECK`. [+ shared contract]"
...
- **L8 (Granular/CUDA, optional):** "Take lane L8 (granular/CUDA, research). Create NEW files only (`GranularGroup` PROTO, CUDA solver TUs). Granular Tier 1–3 + CUDA M2 per `engine-migration-plan.md` §13.7. Keep `WbSolid`/`WbNewtonBackend` integration minimal and sequence it behind L1. [+ shared contract]"
