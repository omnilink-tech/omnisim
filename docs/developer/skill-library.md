...
pipeline, and CLI. Code lives in [`projects/policies/skills/`](../../projects/policies/skills/).

If a doc and the code disagree, the code wins — the library's own `verify-demos`
self-test is the tie-breaker for the demo bundles.
...
| # | artifact | lived in |
|---|---|---|
| 1 | the **ghost** (achievable reference) | a `ghost_*_lut.json` under `controllers/g1_ghost/` |
...
projects/policies/skills/
  ...
  humanoid/
    walk/skill.json       carry_box/skill.json   stand_rl/skill.json
    turn_in_place/skill.json   climb_stairs/skill.json
    balance_two_legs/skill.json   arm_motion/skill.json   # deterministic overlays
  sequences/
...
| **validate (structure)** | `python skill_lib.py ghost <lut>` | typed schema round-trip: shapes, widths, motion class |
...
| **preview** | `python skill_lib.py preview <skill>` | the `g1_ghost` hologram beside the robot — **design → show maintainer → agree, before training** |
| **train** | `python skill_lib.py train <skill>` | assemble the recipe trainer env from the manifest, launch `run_walk_rl.sh … train` |
| **run** | `python skill_lib.py run <skill>` | deploy one skill solo |
| **verify** | `python skill_lib.py verify-demos` | prove the assembled env == the reproduced shell scripts (regression guard) |
| **sequence** | `python skill_lib.py sequence <name>` | run a BATON demo (walk→turn→walk, box delivery, …) |
| **freeze** | `python skill_lib.py freeze <skill>` | promote a run's captured env lock into the manifest (`train.status: frozen`) |
| **adapt** | `python skill_lib.py adapt <skill> --to-nb N` | resample a skill's ghost to a target cadence (cross-cadence blend) |
| **blendable** | `python skill_lib.py blendable <a> <b>` | report whether two skills can element-wise blend + what's needed |
| **handover** | `python skill_lib.py handover <seq>` | show the resolved per-edge warm/cold plan |
...
> ground truth; the library reproduces them. `verify-demos` is green today for **all four**
> sequences. Only once a manifest path is proven equivalent should a script be retired.

---

## Current catalogue (2026-07-10)

| skill | robot | kind | method | status | motion | notes |
|---|---|---|---|---|---|---|
| `walk` | g1 | rl | shadowing | verified | cyclic | the BATON primary; heading-aware; `wr_navigator.pt` |
| `carry_box` | g1 | rl | shadowing | verified | cyclic | 1.5 kg payload; `wr_carrier.pt` |
| `stand_rl` | g1 | rl | shadowing | verified | static | dead-still BATON pause; `wr_stander.pt` |
| `turn_in_place` | g1 | rl | shadowing | experimental | sequence | 90° footwork turn (`wtz=0` — no crane yaw); needs `OMNISIM_FOOT_TORSION`; solo-swap. ⭐ **In-sequence 90° turn is SOLVED** (`72a7bb19`: TURN-LOOP, 90.6–95.6° actual, 3/3, 0 falls) — the manifest still reads `experimental` because the *solo* decel-stop path (`turn_solo`) has not been re-verified. |
| `climb_stairs` | g1 | rl | shadowing | **open** | sequence | the **7 cm climb-ghost** skill: walk-in + ascend a 5-tread staircase (riser 0.07). **OPEN** — plateaus at ~2 of 5 steps; live riser refusal. ⚠️ **Not the same artifact as the shipped stair DEMO** — see the note below. |
| `h1_walk` | h1 | rl | unitree-rehost | verified | cyclic | official H1 re-host; `deploy.run: world`; ghost ready for a native train |
| `balance_two_legs` | g1/h1/valkyrie | deterministic | overlay | verified | static | the balance core |
| `arm_motion` | g1 | deterministic | overlay | verified | static | arms in 3-D while balancing |
| `go2_walk` | go2 | rl | shadowing | verified | cyclic | residual PPO trot; `deploy.run: world`; +86 m |
| `spot_walk` | spot | rl | shadowing | verified | cyclic | residual PPO trot (+vc/rough variants); **47.8 m, 0 falls** |

> ⛔ **`climb_stairs` (the SKILL) and the stair-climb DEMO are DIFFERENT ARTIFACTS.** They look like
...
**Sequences — four, all green under `verify-demos`:**

| sequence | chain | status | reproduces |
|---|---|---|---|
| `box_delivery` | walk → stand → pick → carry → **real 90° corner** → place → walk → stand | verified, 0 falls | `demos/run_box_delivery.sh` |
...
All four run on the weight-bearing balance harness (`HARNESS_LAM0=0.9`, `HARNESS_KZ=2000` — up to
~700 N of upward support, ≈2× the 34 kg G1's weight, plus ±350 N·m of attitude authority). That is
part of the shipped configuration, not a hidden crutch — but the **rotation** in the turns is the
robot's own footwork (`wtz = 0`, zero crane yaw torque, measured per tick). The **stair-climb demo is
the one exception**: it runs `HARNESS_KZ=0` so the legs do all the vertical lift.
...
1. **Design the ghost** and get it green:
   `python skill_lib.py ghost <lut>` (structure) **and**
...
2. **Preview it** and get maintainer sign-off: `python skill_lib.py preview <skill>`
   (author the manifest with `ghost.preview_world` first). Ghost-first is a hard rule.
3. **Train** it: `python skill_lib.py train <skill>` (the manifest's `train.recipe_env`
...
5. **Write** `<class>/<skill>/skill.json` (copy a sibling), declaring the `baton`
   ...
   sequence). Add a `registry.json` line (`python skill_lib.py index` regenerates it).
...
4. **Cross-robot** — `h1_walk` (Unitree re-host) and `go2_walk` / `spot_walk` (residual
...
`climb_stairs` skill** (see the catalogue note above), and — at the *thesis* level —
