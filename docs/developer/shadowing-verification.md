...
| **C2** | Robot-*agnostic* generator: interface is strictly `(model, intent) → ghost` | **VERIFIED (structurally)** | One `GhostGenerator` class drives go2 / b2 / omniquad / g1 / arm generators; only the MJCF + `Intent` differ. Go2 re-rolled fresh; b2/omniquad/g1/arm ship stored ghosts. |
...
| **E4** | Sim-to-deploy: G1 walk 200 m+, OmniQuad/Go2 walk, arm toss — *all deployed* | **MIXED** | Quadrupeds ✅: **Go2 +111.45 m @ t=293 s, 0 falls** and **OmniQuad +170.8 m @ t=426 s, 0 falls**, both dead-straight, upright, Newton/mjwarp confirmed. **G1 walk ❌ does NOT reach 200 m** — see §4. Arm toss landing not re-run. |
...
2. **The present, deployable G1 walk policy topples in ~1.3 s.** I ran the policy the standard
   ...
   (`[OmNewtonBackend] warp + newton imports OK`, 14 dynamic bodies). Result:
...
and OmniQuad walk **+170.8 m**, both 0 falls, dead-straight, upright, Newton/mjwarp confirmed
...
(OmniQuad **+30 m** / Go2 **+66 m** / B2 **+95 m**, 0 falls). Where a single headline number is
...
pipeline crossed the deploy gap for quadrupeds (Go2 + OmniQuad verified here, B2 per canonical
...
**missed OmniQuad entirely** (OmniQuad has no `foot`-named body, so the name-matched foot set was empty).
...
*all* robot collision geoms against a terrain-aware ground (covers belly/knee/elbow support, OmniQuad,
...
| get-up | omniquad / b2 | **PASS** (judged on reconstructed leg support) |
| crouch / sit | omniquad crouch / g1 sit-stand (chair) | **PASS** |
| hill | b2 hill6 / omniquad hill8 | **PASS** (against the injected terrain model) |
| jump | omniquad pronk | **FAIL → honest boundary** (see below) |
