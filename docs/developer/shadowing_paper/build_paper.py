...
def build():
    ...
    A(Paragraph("Technical Report &mdash; June 2026", st_affil))
    A(Spacer(1, 5))
    ...
    A(Paragraph(
        ...
        "(the Unitree G1 humanoid, a Boston-Dynamics-class Spot, the Unitree Go2 and B2 "
        "quadrupeds, and a fixed-base 6-DOF manipulator) and five motion classes. The "
        "<b>quadrupeds cross the deployment gap durably</b>&mdash;Spot walks "
        ...
        "beyond the arm's kinematic reach</b>. The <b>biped is where we state the limit "
        "plainly</b>: <b>no from-scratch G1 policy walks durably free-standing in our deploy "
        "engine</b>, and the best G1 gait quality we can show is produced on a "
    ...
    A(P("<b>Contributions.</b> (i) The Shadowing architecture&mdash;generator&rarr;verifier"
        ...
        "feasibility program is classical [3,4,5], and we do not improve it. What is new, "
        ...
        "the one prior RL-independent gate we found, KungFuBot/PBHC [11]). "
        ...
        "and five motion classes. (vi) <b>Two negative results</b> that delineate the "
    ...
    A(P("where the exponential tracking errors <i>e<sub>i</sub></i> are, in turn, the "
        ...
        "lean <i>&theta;&minus;&theta;<sub>ref</sub></i>, with weights "
        "<i>w<sub>i</sub></i> and sharpnesses <i>k<sub>i</sub></i> in Table&nbsp;3's "
        "companion (height sharpness 20&ndash;25 dominates, keeping the pelvis on the "
        "reference)."))
    ...
    A(P("All experiments run in <b>OmniSim</b>. Training uses a GPU-parallel MuJoCo-warp "
        ...
        "environments on a laptop RTX 3060.", st_body0))
    ...
    A(P("We evaluate on five robots: the Unitree <b>G1</b> humanoid (23 DOF, 34&nbsp;kg), a "
        "Boston-Dynamics-class <b>Spot</b> quadruped, the Unitree <b>Go2</b> (16&nbsp;kg) "
    ...
    A(P("The same generator&rarr;verifier&rarr;tracker machinery, changing only the intent "
        "and the robot model, produces deployable policies across five morphologies and five "
        "motion classes (Table 6, Figures 5&ndash;7). The results split cleanly along one "
        "line, and we report both sides of it.", st_body0))
    A(P("<b>Quadrupeds cross the deployment gap.</b> Spot walks <b>47.8&nbsp;m</b> "
    ...
    A(figure("fig_sitstand.png", CW*0.52,
        ...
        "35&ndash;47&deg;). The <i>body</i> of this motion tracks to 2.5&nbsp;cm; the "
        "dead-seated <i>launch</i> is never learned (&sect;5.5)."))

    A(figrow3("omni_spot_walk.png", "omni_go2_walk.png", "omni_b2_hill.png", CW*0.315,
        ...
        "<b>Left:</b> a Boston-Dynamics-class Spot trotting&mdash;<i>deployed</i>, "
    ...
    A(P("The pipeline also spans non-legged manipulation. A fixed-base 6-DOF arm "
        ...
        "lands it <b>1.5&nbsp;cm from the bin centre</b>. Here the verifier "
    ...
    A(P("Beyond walking, the generator <i>discovers</i> contact-rich motions that have no "
        ...
        "both Spot and B2 (Figure 8), and hill traversals (Figure 9). Both get-up ghosts "
    ...
    A(figure("fig_getup.png", CW*0.485,
        ...
        "Spot and B2: base height rising from a collapsed, belly-flat start to a stand, a "
    ...
    E.extend(make_table(
        ...
        [["G1", "human-gait walk<br/><i>(free-standing)</i>",
         ...
         ["Spot", "straight walk (vel.-cond.)", "47.8 m, 0.32 m/s", "0", "deployed"],
         ["Go2", "walk", "86.7 m, 0.38 m/s", "0", "deployed"],
         ["B2", "walk", "110.7 m, 0.49 m/s", "0", "deployed"],
         ["B2", "get-up (lying&rarr;stand)", "rises to a stand", "0", "deployed"],
         ["B2 / Spot", "hill walk", "tracker stalls at the flat&rarr;ramp transition", "&mdash;",
          "blocked (tracker)"],
    ...
    A(P("We presented Shadowing, a recipe that makes the dynamic feasibility of a reference "
        ...
        "tracker carries certified ghosts across a sim-to-sim deploy gap on five robots and "
        "five motion classes&mdash;durably on the quadrupeds (Spot 47.8&nbsp;m, Go2 "
        "86.7&nbsp;m, B2 110.7&nbsp;m, zero falls) and on the manipulator. We claim "
