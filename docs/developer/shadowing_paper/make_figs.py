...
# ----------------------------------------------------------------------------
# Fig 6 — Spot leap  [REMOVED 2026-07-11 — DO NOT RESURRECT WITHOUT SOURCES]
#
# The former fig_jump() plotted the spot_jump ghost against a hard-coded
...
#      certificate table records `spot jump -> FAIL (boundary)` — the explosive
...
# ----------------------------------------------------------------------------
# Fig 7 — Get-up rise curves (Spot + B2), MPPI-discovered contact-rich motions
# ----------------------------------------------------------------------------
def fig_getup():
    fig, ax = plt.subplots(figsize=(3.5, 2.7))
    peak = 0.0
    for fn, col, lab in [("spot_getup_ghost.npz", C["blue"], "Spot get-up"),
    ...
    # NOTE: the old hard-coded ylim of 0.62 CLIPPED both curves (Spot peaks at
...
#   * Spot model+residual ~80k env-steps/s, RTX 5070 -- performance-comparison.md:191
...
def fig_generality():
    ...
    # ---- (a) durable deploy distances: quadrupeds only, and that is the truth ----
    rows = [
        ("Spot straight walk", 47.8, 0.32, C["green"]),    # gpu_spot_walk_vc_main
        ("Go2 walk",           86.7, 0.38, C["red"]),      # gpu_go2_walk_main
        ("B2 walk",           110.7, 0.49, C["orange"]),   # gpu_b2_walk_main
    ]
    rows = sorted(rows, key=lambda r: r[1])
    yi = np.arange(len(rows))
    ax.barh(yi, [r[1] for r in rows], color=[r[3] for r in rows],
            edgecolor="black", lw=0.5)
    for y, r in zip(yi, rows):
        ax.text(r[1] + 2.5, y, f"{r[1]:.1f} m · {r[2]:.2f} m/s", va="center", fontsize=6.2)
    ax.set_yticks(yi); ax.set_yticklabels([r[0] for r in rows], fontsize=7)
    ax.set_xlabel("deploy distance (m), zero falls")
    ax.set_title("(a) Durable deploy walks", fontsize=9.5)
    ax.set_xlim(0, 168)
    ...
    cert_rows = [
        ...
        ("Hill walk (B2 / Spot)", "PASS  5.8% mg", False, "tracker stalls"),
