...
> TRACKING). Session arc 0.581 → 0.868 (commits b135a8e7, 68ced9e7). Open thread: live stride
> below trainer (~0.68× hip amplitude, under investigation). The WBMATCH2-era numbers below
...
> each skill (walk / turn / carry / stand / climb, + H1 / Go2 / Spot) is ONE versioned manifest
...
> - Ghost lineage in [`controllers/g1_ghost/`](../../projects/policies/controllers/g1_ghost/):
...
> world `g1_walk_ghost_wide.wbt`, `GHOST_LUT_JSON=ghost_hop1v11_lut.json` (control),
...
> world `g1_climb_stairs_demo3.wbt`): the G1 walks in on the flat and **climbs a 5-tread staircase
...
>   ([`skills/humanoid/climb_stairs/skill.json`](../../projects/policies/skills/humanoid/climb_stairs/skill.json)):
...
> walks to cart A, takes a real 1.5 kg ODE box with a proximity-gated two-phase lift (it can never
...
> corner, walks away, ends standing. **Verified 2026-07-10: 3/3 headless, min base-z 0.659, 0 falls.**
...
>
> ### 4 · IN-ENGINE QUAD TRAINER — Go2 / Spot / B2, train == deploy (`a824e564`, `101864a4`)
...
> parity gap (the standalone trainers' MJCF re-parse). One recipe, three quads (`QUAD_ROBOT=go2|spot|b2`),
...
>   fell over 48 s, 16.6 m mean (max 20.7 m), 0.357 m·s⁻¹**. **Spot** (30-iter *smoke*, 1024 envs) →
...
>   Spot +47.8 m, B2 +110.7 m, 0 falls — Table A in the historical block); the in-engine champions
...
> results below ARE real and verified — OmniSim-trained **quadruped** walks (Spot/Go2/B2,
...
> the RL quad walk with deterministic control was run and measured on real Newton (Spot,
> `spot_walk_deploy.wbt`): RL residual = **0.40 m/s, dead straight, 0 falls, 327 s+ (131 m)**;
...
> (`spot_raibert_walk`) propels (0.16–0.31 m/s) but has an unsolved **roll instability** that
...
> flat-trained residual walk is blind to terrain (limit ≈ gait foot-clearance: Spot trips at a
...
> untouched) fixes it: terrain-trained **Spot clears 18 cm bumps + a rubble field, 0 falls,
...
> **same ~1.3–1.4 s forward-tip wall** in deploy. **Quadrupeds (Spot/Go2/B2) DO walk
...
> | **Spot** | `gpu_spot_walk_vc_main` | **OmniSim-trained** (residual on trot) | **47.8 m, 0 falls**, bz 0.553, 0.32 m/s, y −0.55 m |
...
> | **G1** | "SOLVED, holds indefinitely" | ❌ **REFUTED — tips forward, FALL@~1.38 s** (steady-state collapse for 93 s; matches the `g1_stand_deploy.wbt` world-header's own "MARGINAL, tips forward ~1.4 s on 6/7 runs"). *Caveat: deploy is a cold first-load; not re-tested warm — but corroborated by the world header and by the identical ~1.4 s wall hit by all four G1 deterministic controllers.* |
...
> 2. **Spot's residual is NOT a passenger** — measured **+44 m with the policy vs max +0.13 m /
...
>    docs or the CI extract:** doc-vs-reality drift (G1 stand, Spot passenger, the missing
...
> *not* in-tree; a static audit — no live runs); Spot/Atlas sections date to
> 2026-05-29.
>
...
[h1-walk-rl-journey.md](h1-walk-rl-journey.md); the Spot/Newton-deploy row reconciled
to the 2026-06-23 "Spot walks on Newton" finding; G1 sections re-verified 2026-06-19;
...
[spot-residual-rl.md](spot-residual-rl.md). **This doc carries the honest "is it
...
|--|--|--|--|
| **Spot** (quadruped) | model+residual walks straight; velocity-conditioned walker (`gpu_spot_walk_vc_main`) trained on Newton-MuJoCo | **Newton: WALKS** — `gpu_spot_walk_vc_main` walks **+30 m straight, 0 falls** (chassis `bz`≈0.55) under Newton-MuJoCo via `run_spot_walk_vc_deploy.ps1` (2026-06-23). The earlier "~3 s collapse" was **largely an init bug** (a writable-stdio crash that silently fell back to ODE), now fixed — not a Newton dynamic-fidelity wall. Forced-ODE also walks straight (+1.43 m/8 s, z≈0.67). | **Walks on Newton now.** On the *older* model+residual policy the *learned residual is a passenger* (+4.87 m with policy vs +5.03 m with no policy at all; the hand-coded gait does the work). The newer VC walker is the working Newton deploy; the 2026-06-08 "W6 roll-collapse" characterization predates both the VC walker (2026-06-21) and the stdio fix (see Spot — detail). |
...
the original Spot/G1/Atlas table. They follow the **Shadowing** (Ghost) recipe — a
...
2. **Train-in-the-deploy-solver** (the "faithful trainer") — the honest answer
   ...
   **not the lever it was hypothesized to be**; the lever is aligning the
   ...
   it is still unwritten. The Spot-only
...
3. **The generic `--robot/--backend` registry**
   ...
   practice **it only fully runs Spot**: `sb3_cpu.py` hard-raises
   `NotImplementedError` for non-Spot robots, no `robot_spec.json` sidecars
...
and the *native-Newton-joint build pattern* (`spot_native.py` →
...
> for the rest of the run (steady-state, not a transient). The `g1_stand_deploy.wbt`
...
- **Canonical launcher:** [`g1_deploy_launch.py`](../../projects/policies/research/runners/g1_deploy_launch.py)
  (spec-driven; default `ft_pdoff_clamp` + `g1_walk_arms_deploy_mjwarp.wbt`; `--print-env`).
...
**BUT on the real `g1_stand_deploy.wbt` (zero policy), ke=400 only gets 0.51 s →
...
a **23-body** articulation that stands; the deploy's `WbNewtonBackend` import builds a
...
([`atlas_robot_spec.py`](../../projects/policies/research/backends/atlas_robot_spec.py)) is a
...
> **UPDATE 2026-06-23 — Spot DOES walk on Newton** (the TL;DR row above now reflects this;
> the original "Newton: COLLAPSES" wording was reconciled 2026-06-24).
> Verified on the velocity-conditioned walker (`gpu_spot_walk_vc_main`) under Newton-MuJoCo
...
> `WbNewtonBackend.cpp` (writable-stdio guard before the warp import); Newton now initializes
...
> the current build.** Running `run_spot_walk_deploy.ps1` with the policy vs `-Bare` (no
...
> different gait operating point). (The VC walker `gpu_spot_walk_vc_main` walks +47.8 m,
...
keep on Spot is, as of today, unproven.
...
- **Net learned value delivered to deploy is small-to-negative** across all robots
  (Spot residual is a *passenger*; Atlas policy *== baseline*; G1 **stand** is pure
  ...
  **load-bearing** is the **G1 walk** (analytic balance off → the residual does the
  ...
  The one clean learned win remains the Spot *ODE* residual.
- **"Passenger" vs. "saboteur" — a complete baseline on a *marginal static* task makes
  ...
  demo). A residual on a *statically-stable* task with redundancy (Spot walk, 4 contacts)
...
- **The "standard method" is documentation, not infrastructure** (see method
  §3). The registry runs Spot; humanoid policies came from standalone scripts.
...
[`WbNewtonBackend.cpp`](../../src/omnisim/physics/WbNewtonBackend.cpp) only
...
**Fix added (this pass):** a **base-divergence guard** in `WbNewtonBackend`'s
...
on [`g1_stand_deploy.wbt`](../../projects/policies/research/worlds/g1_stand_deploy.wbt)) to
...
1. **(done this pass)** Stale "G1 stands forever / 44+ s" headlines corrected
   ...
   `spot-residual-rl.md`; `sim-to-deploy-rl-recipe.md` given a scope caveat.
...
4. **Reproducibility holes:** commit `g1_full.mjcf.xml` (or its dump inputs) so
   ...
   document that generic deploy is Spot-only.
...
- **Unverifiable-from-artifact numbers:** the G1 "98.6 % / 2466-of-2500" eval,
  the Spot "+4.87 m straight" deploy, the Spot from-scratch v3/v4 failures, and
...
- [spot-residual-rl.md](spot-residual-rl.md) — the quadruped model+residual
