...
driver. Root cause: `OmVulkanBackend.o` was a stale STUB (built without
...
OmVulkanBackend.cpp fixed it** (`dda1da31`); `OMNISIM_WGPU_INITLOG` now traces the
init stage. Lesson: when building
wgpu-ON, ALWAYS touch OmVulkanBackend.cpp; backend-unavailable-at-runtime ≠ driver
...
- [x] **Picking** — pick pass `7ebb29c9` **RUNTIME-VERIFIED** (probe PASS, `dda1da31`).
      ...
      `OmWgpuView::mousePressEvent` → `OmSelection::selectPoseFromView3D` in `cc50902b`
...
- [x] **Manipulator handles** — translate/rotate/scale gizmos in wgpu; hit-test via the
  ...
  - [x] **Translate-gizmo rendering** (`f5f77094`) — 3 world-axis lines (R/G/B) at the selected
        solid's origin, always-on-top; live pane (at OmSelection) + verified offscreen on
        camera.omniworld (7 gizmos, 971 px). The visual handle.
  ...
  - [x] **Rotate-gizmo rings** (`e08614b8`) — 3 R/G/B rings (about X/Y/Z) per solid, distance-
        scaled, always-on-top. VERIFIED on camera.omniworld (rings render, 5066 px). Render-only.
  - [x] **Scale-gizmo boxes** (`a0e20367`) — small R/G/B cubes at the axis ends, distance-scaled.
        VERIFIED on camera.omniworld (2057 px). **Gizmo-RENDER set complete (translate/rotate/scale).**
  - [x] **Rotate drag** (`e90748ff`) — `pickRotateRing` (verified) + mousePress grab / mouseMove
        compose-rotation-about-world-axis (`OmMatrix3(axis,Δ)·startRot`) / mouseRelease. Manipulator
        INTERACTION now complete (translate + rotate drag). **Scale drag is N/A for solids** —
        `setScale` is on `OmTransform`, `OmSolid` is not one (no scale field; physics bodies aren't
...
- [x] **Optional renderings** (ADDITIVE-safe) — gated on `OmWrenRenderingContext` `VF_*`
  ...
  - [x] **Joint axes** (`VF_JOINT_AXES`) — thin cyan bars through each joint anchor along
        ...
        public `OmJoint::axisRepresentation()` (additive core-API). **VERIFIED 2026-06-04**:
  ...
  - [x] **Line/wireframe pipeline** (foundation, `24f668c9`) — `OmWgpuRenderTarget::`
  ...
  - [x] **Camera/sensor frustums** (`VF_CAMERA_FRUSTUMS`, `d82fa84e`) — near+far rect + 4
        connectors per `OmAbstractCamera`, +X-forward/horizontal-FOV convention, world-
        transformed by `camera->matrix()`, always-on-top. VERIFIED + visually confirmed on
        camera.omniworld (1 camera → proper truncated-pyramid frustum). (Per-type RangeFinder gating =
        small refinement.)
  - [x] **Surface normals** (`VF_NORMALS`, `90f66cf2`) — outward lines on primitive geometries
        (Box/Sphere/Cylinder/Capsule), always-on-top. VERIFIED on camera.omniworld (80 segments).
  ...
  - [x] **Lidar ray paths** (`VF_LIDAR_RAYS_PATHS`, `dc661f13`) — subsampled ray fan per
        `OmLidar` (32×≤6) out to maxRange, +X-forward convention. VERIFIED + visually confirmed
        on lidar.omniworld (198 rays). Lidar/range-finder FRUSTUMS already covered (OmLidar is-a
        OmAbstractCamera). **Optional-renderings family now 7/8.**
  - [x] **Per-type RangeFinder frustum gating** (`d6d61889`) — frustum overlay now gates a
        ...
        new dual-sensor fixture (`tests/physics/fixtures/wgpu_dual_sensor.omniworld`): camera-only=1,
  ...
  - [x] **Support polygon** (`43b71292`) — per top-solid convex hull of ground contacts on the
        horizontal plane (mirrors `OmSolid::supportPolygon()` via `twoStepsConvexHull`, computed
        cold from `computedContactPoints()`). UNBLOCKED by a new resting-contact fixture
        (`tests/physics/fixtures/wgpu_overlay_contacts.omniworld`, `f0aa9eb2`) + running `--mode=fast`
  ...
  - [x] **OmWgpuSurface line method** (`2fead888`) — `presentScene` line-overlay pass; bounding
...
- [~] **Overlays** — device-output texture overlays (camera/display/range-finder) +
  ...
  - [~] device-output texture insets (camera/display/range-finder image as a corner HUD).
        **Rendering DONE** — `kTexturedQuad` + `drawTexturedInset` primitive (`935101cc`, PROBE_INSET-
        ...
        (camera.omniworld, fast+realtime, retried to mPhase>40): the controller PROCESS spawns but the
...
- [~] **Screenshots / video**
  ...
  - [ ] wire into the screenshot menu action + `OmVideoRecorder` when wgpu is the source
...
- [x] **Material fidelity** — real `PBRAppearance` so the pane matches WREN, not flat. The full
  ...
  - [x] **Albedo textures** (`fd58d669` fetch + `1632a0c0` render) — `baseColorMap` uploaded
        (`OmImageTexture::image()` → `OmWgpuImageAdapter` → cache, with a `tryGet` fast-path
  ...
  - [x] **metalnessMap + normalMap** (`ea231e23`) — kSolidLitTextured extended 4→6 bindings both
        ...
        (`tests/physics/fixtures/wgpu_pbr_maps.omniworld`, `91089acf`) using stock metal PROTOs that ship
  ...
  - [~] **Lighting/shadow convergence to WREN** (residual parity gap; target wgpu brightness 87→WREN 41).
        ...
        **Increment 4 (convergence) DEFINITIVELY CHARACTERIZED** (`ffb073c8`): the residual is
        ...
        IMPROVING** (`0775723c`): harvested the real `OmniSimSun` (a top-level OmDirectionalLight, found
        ...
        **HEMISPHERE-IBL ambient implemented + a CORRECTED measurement methodology** (`d15a977f` + the
        ...
        parity gate harvests the scene sky colour + `OmWorldInfo::upVector`). PROBE_TEXSHADOW PASS
...
- [~] **Robustness**
  ...
  - [x] resize / DPI correctness — offscreen render targets verified at 64/256/480/640 px
        ...
        window 640×480 → 1400×900 (→ `OmWgpuSurface::resize` swapchain reconfigure), and every
...
- [x] **`OmView3D::renderNow` backend-dispatch SEAM (`5ef0e329`, increment 1).** The single main-view
      frame funnel now consults the active Viewpoint's `renderBackend()` via a new `renderMainFrameViaWgpu()`
      helper; returns false → the UNCHANGED `OmWrenWindow::renderNow()` runs (WREN byte-identical; no shipped
      world sets `renderBackend "wgpu"`, so the branch is unreachable for them). Verified: builds clean,
      WREN-default panda.wbt renders + holds a live window, no crash. (The seam lives at OmView3D, not
      OmWrenWindow's default path — the per-backend init/grab dispatch is naturally subsumed here.)
- [x] **Backend-selectable surface — solved by OFFSCREEN render → GL blit (increment 1b).** OmView3D keeps
      ...
      blitted into the GL window via a tiny non-Qt TU `render/OmWgpuGlBlit.cpp` (isolates glad from Qt; Y-flip
      via `glBlitFramebuffer`). New OmView3D members: lazy wgpu backend + mesh/texture caches + a cached
...
- [x] **STABILITY HARDENING — ✅ FIXED (`a4fec74b`, 2026-06-06). It was an APP-LEVEL texture-cache key
      ...
      the wgpu texture cache on the `OmImageTexture*` pointer, but a scene PROTO makes a separate
      `OmImageTexture` per use, so many shapes sharing one texture FILE (one `Plaster.jpg` across the factory
      ...
      source file path (`OmWgpuSceneRenderer::stableTexId`). **Verified:** creations 507 → 63, 0 wgpu errors,
      ...
      per-draw texId (the `OmImageTexture*`) changes every frame (panda PROTO appearance churn), so the
      ...
      (2) LRU VRAM cap on `OmWgpuTextureCache` (512 MB, evict LRU); (3) confirmed `ensureScenePipeline` is
      ...
      (`OmWgpuView`, `presentScene` → SURFACE) renders the same world per-frame and NEVER crashes — because it
      does NOT do the offscreen `copyTextureToBuffer`+`mapAsync` readback. So the readback is the culprit, and the
      un-gate path is to **avoid the per-frame readback**, two options: (A) the WINDOW-SWAP — show `OmWgpuView`
      (its own Vulkan surface, no readback) as the main view when wgpu is selected (needs OmSimulationView
      per-world view switching + OmWgpuView lacks OmView3D's full editor interaction); (B) wgpu↔GL SHARED-TEXTURE
      ...
      **FEASIBILITY CHECKED — Option B (interop) is INFEASIBLE with the pinned wgpu-native** (`_scratch/wgpu-native`
      ...
      narrows to: **A (window-swap to `OmWgpuView`'s surface)** — un-gates but the wgpu main view gets only the
      pane's PARTIAL interaction (pick/select + translate/rotate manipulators I built), NOT OmView3D's full editor,
...
- [x] Human checkpoint before touching `OmWrenWindow`'s default path — **GIVEN** (and the seam deliberately
      lives at OmView3D, leaving `OmWrenWindow`'s default path untouched).
...
- [x] **R5 sensor pipeline (Lidar/RangeFinder/Camera/Recognition) — parity CONFIRMED, regression GREEN
      ...
      for every display caller; >0.5→linear) + a `clearAndDrawScene(srgbEncode=true)` param that only `OmCamera`
