...
│       └── spot_rl_deploy.py
├── worlds/
│   ├── spot_rl.omniworld            # training world (per-env via SPOT_RL_PORT env)
│   └── spot_rl_deploy.omniworld     # deploy world — loads ONNX policy
├── training/
...
./msys64/mingw64/bin/omnisim-bin.exe projects/policies/worlds/spot_rl_deploy.omniworld
```
...
```bash
# A) env var override
SPOT_POLICY_ONNX=path/to/policy.onnx omnisim-bin projects/policies/worlds/spot_rl_deploy.omniworld
...
omnisim-bin projects/policies/worlds/spot_rl_deploy.omniworld
```
