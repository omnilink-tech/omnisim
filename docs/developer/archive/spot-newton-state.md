...
now lives in [spot-residual-rl.md](../spot-residual-rl.md#newton-port--same-recipe-different-physics-walks-straight-in-84-seconds).
...
|---|---|---|
| Articulation builder (leaf-first joint feed safe) | `WbNewtonBackend.cpp` `kNewtonRuntimeSource` | 13 Spot bodies + 12 hinges finalise cleanly |
| Position-target bridge from `motor->setPosition` | `WbBasicJoint::pushNewtonMotorTargets` | wired through `control.joint_target_pos` |
| Live joint-angle readback | `WbNewtonBackend::getJointAngle` | reads `state.joint_q` |
| Newton overwrite of Solid pose | `WbSolid::postPhysicsStep` | uses `setValueFromOde` + manual `setMatrixNeedUpdate` to break the reset-hook feedback loop |
| Per-shape transforms (Pose `<origin>`) | `WbSolid::flushPendingNewtonRegistrations` | foot sphere registers `at (0,0,-0.32)` matching URDF |
...
| URDF inertia tensors (`OMNISIM_URDF_USE_INERTIA=1`) | `WbSolid::flushPendingNewtonRegistrations` | chassis I=(0.20, 1.14, 1.17), legs match URDF |
| Newton-aware Supervisor reset | `WbSolid::syncNewtonPoseFromFields` | warps `body_q` + reset `joint_q` on field writes |
| Parent-child collision filter | `_add_revolute_to_builder` | `collision_filter_parent=True` |
| SpotEnv training under Newton | `SpotEnv` + `spot_rl_newton.wbt` + `train_spot_newton.py` | 570 steps/s smoke under MuJoCo CPU |
...
[Newton residual walker section in spot-residual-rl.md](../spot-residual-rl.md#newton-port--same-recipe-different-physics-walks-straight-in-84-seconds)
...
2. Runs `projects/policies/research/tools/_eval_newton.py` against `spot_newton_demo.wbt`.
...
[spot-residual-rl.md](../spot-residual-rl.md#newton-port--same-recipe-different-physics-walks-straight-in-84-seconds)
