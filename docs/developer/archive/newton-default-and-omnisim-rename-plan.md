...
> ## ⛔ [2026-07-26] PHASE A'S EDITORIAL DIRECTIVE IS REVOKED — DO NOT APPLY IT TO ANY DOC
>
> This is an archive plan, but **Phase A below is written as instructions to a doc
> author**, and two of those instructions are now measurably wrong. Both are struck in
> place (§Phase A, and the Phase E note under "ODE is documented as 'legacy mode'"):
>
> | Directive in Phase A | Status |
> |---|---|
> | *lead with "Newton is 30–100× faster than ODE" as the headline* | **REVOKED.** That ratio is a **cross-harness artifact**: the ODE side was whole-engine GUI fps (ODE + WREN rendering + the Python damage tracker, itself ~2× whole-sim cost), the Newton side a bare solver probe. Measured in **one** harness (OmniBench lane 1, dt=4 ms, post-fix), **ODE is *cheaper* per step than Newton** on these scenes — T1 bounce **0.131 ms/step (ODE) vs 3.169 (Newton)** on machine `9722d23d12a3`. Newton's real case is **batching** (thousands of parallel envs on GPU), not per-step cost. |
> | *frame ODE numbers as the legacy baseline* | **REVOKED.** "Fallback" describes ODE's **role** (CPU path, no GPU required) and is fine. "Legacy" implies worse physics and is false: ODE is the **correctness star** of OmniBench lane 1 — best or tied-best on 6 of 7 scenes at dt=4 ms, linear momentum exactly zero to double precision, bitwise determinism on both machines. |
>
> **Replacement guidance for any doc that would have followed Phase A.** Say: *"Newton is
> OmniSim's GPU/batched physics path and the default where its runtime is present; ODE is
> the CPU path and the permanent, fully supported fallback — and the accuracy reference of
> the benchmark suite."* Cite measured numbers per machine id from
> [omnibench-2026-07-24.md](../../benchmarks/omnibench-2026-07-24.md); the reasoning behind
> the retraction is in the header box and §3.5 of
> [performance-comparison.md](../../benchmarks/performance-comparison.md).
...
- **The Wb* class prefix stays** — `OmWorld`, `OmSolid`, etc. remain. Stack traces still show the Webots heritage. The source files themselves still carry the Cyberbotics copyright header where applicable.
...
2. Newton works end-to-end in the binary now. ~~The 30–100× speedup vs ODE is measured, not theoretical.~~ *(⛔ 2026-07-26: it was cross-harness, not measured — see the banner at the top. The durable part of this bullet is just "Newton works end-to-end in the binary now.")*
...
TRACK 1 — Newton-as-default messaging + auto mode
  ...
  Phase B   add physicsBackend "auto" + OmWorldInfo resolution
...
Single-pass edit across user-facing docs to flip the positioning. The narrative changes from "Newton is a power feature, ODE is default" to ~~"Newton is OmniSim's solver, ODE is the legacy fallback."~~ **"Newton is OmniSim's GPU/batched path and the default where its runtime is present; ODE is the CPU path and the permanent, fully supported fallback."** *(⛔ 2026-07-26: "legacy" struck — see the banner at the top of this file. ODE is the correctness star of OmniBench lane 1, not a deprecated tier.)*
...
- `docs/developer/migration-perf-comparison.md` (since archived) — ~~lead with "Newton is 30–100× faster than ODE" as the headline, frame ODE numbers as the legacy baseline.~~
  > ⛔ **[REVOKED 2026-07-26 — this line is the single worst directive in this plan; it
  > instructed future docs to propagate two errors, and they did.]** The 30–100× is a
  > **cross-harness artifact** (whole-engine ODE GUI fps ÷ a bare Newton solver probe), and
  > "legacy baseline" misdescribes the backend that scores best in the suite. **Replacement
  > directive:** do *not* lead any doc with a Newton-vs-ODE speedup ratio at all — none is
  > defensible. Lead instead with the two same-harness facts: **(1) Newton is the batched
  > GPU path — 129,431 control-env-steps/s @4096 on an RTX 3060 Laptop (`9722d23d12a3`),
  > 650,487 @8192 on an RTX 4090 (`65dd6587d5c9`), contacts ON, both sides CUDA-graphed —
  > where ODE has no batched path at all; (2) ODE is the accuracy reference and is cheaper
  > per step on single-scene work** (T1 bounce 0.131 ms/step vs Newton's 3.169 on
  > `9722d23d12a3`). The archived doc itself now carries a superseded banner:
  > [archive/migration-perf-comparison.md](migration-perf-comparison.md).
...
Adds a third value to the field. Mirrors the `broadphase "auto"` pattern landed in `OmWorldInfo`. Resolution:
...
- `src/omnisim/nodes/OmSolid.cpp` — extend `effectivePhysicsBackendName()` to handle `"auto"` by calling a new `resolveAutoBackend()` helper.
- `resources/nodes/Solid.wrl` — extend the field's enum: `SFString{"ode", "newton", "auto"}`.
- `resources/nodes/Robot.wrl` — same.
- New: a runtime probe in `OmPhysicsBackendRegistry` that reports Newton availability + scene-size cliff status to the resolver.
...
- `resources/projects/worlds/empty.omniworld` — add `physicsBackend "newton"` to the default empty world template.
...
> ⛔ **[2026-07-26] The Phase E documentation step is revoked; the rest of Phase E was
> never committed to anyway.** "ODE is documented as 'legacy mode'" cannot be written now:
> OmniBench lane 1 measures ODE as the **most accurate engine in the suite** (best or
> tied-best on 6 of 7 analytic scenes at dt=4 ms, linear momentum exactly zero to double
> precision, bitwise determinism on both machines), and it is the **cheaper** backend per
> step on single-scene work. Whatever happens to the *default*, ODE is documented as
> **"the CPU backend and the permanent fallback"** — never as lower fidelity. See the
...
| Newton not available on dev box, "auto" mode silently falls back to ODE confusingly | B–D | Single `OmLog::info` per world on resolution decision, naming which backend won and why |
| Body-index-30 cliff hits an "auto" world mid-session and Newton breaks correctness | D | Per the "auto" gating — Phase D doesn't land until the cliff is fixed upstream. Newton's `OmNewtonBackend::beginWorld` already returns -1 on failure; the auto-resolver treats that as "fall back to ODE." |
...
- **Deprecate ODE** — kept as the ~~legacy~~ **fully supported CPU** fallback indefinitely. *(2026-07-26: the intent was always right; only the adjective was wrong — see the banner at the top.)*
