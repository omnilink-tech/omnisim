...
Three candidates considered. The R3 layer goes between `OmVulkanBackend` and the GPU; the dispatcher abstraction means we can swap the choice later without changing call sites, but the cost of swapping grows once R3.2+ work assumes a specific API surface.
...
WREN's `wren::Mesh` cache already keys on geometry hash with ref-counting (per `src/wren/Scene.cpp` and `src/wren/StaticMesh.cpp` — deleted with WREN on 2026-08-23, `976b9449d`). Plan: keep that cache as the source of truth; add a parallel bgfx-side cache keyed on the same `Mesh::sortingId()`.
...
The gnarliest piece. WREN has a shader set under `resources/wren/shaders/` (deleted with WREN, `976b9449d`) — GLSL 330. bgfx wants its own shader format (GLSL + `$input`/`$output` annotations) compiled to backend-specific bytecode by shaderc.
...
The `OmVulkanBackend` constructor (currently at [src/omnisim/render/OmVulkanBackend.cpp](../../../src/omnisim/render/OmVulkanBackend.cpp)) is the probe site. With bgfx:
...
A single `OmLog::warning` per world load when fall-back fires. World still loads, every Camera/Viewpoint still renders through WREN.
...
|---|---|---|---|
| R3.1 | bgfx init + isAvailable() probe. Build flag actually does something. No rendering yet. | `OmVulkanBackend.cpp`, Makefile (bgfx dep) | 2–3 days |
...
| R3.3 | Single-Camera render-to-texture: a Camera with `renderBackend "vulkan"` routes its frame through bgfx. Solid-color shader only; no scene geometry yet. | `OmCamera.cpp`, new `WbVulkanCamera.cpp` | 1–2 weeks |
