...
`WbGuiRefreshOracle` throttles GUI refreshes while the simulation is running. It only allows refresh roughly every 300 ms when the simulator is not paused.
...
`WbSceneTreeModel` also mixes fine-grained `dataChanged(...)` notifications with broader `layoutChanged()` emissions. Broad layout changes are much more disruptive because they can invalidate more UI state and trigger more view work than a targeted data update.
