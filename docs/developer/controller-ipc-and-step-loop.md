...
2. `WbController` sends step and immediate packets, flushing the socket after each write
3. `WbController::readRequest()` reconstructs Webots packets from a possibly fragmented byte stream
4. `WbControlledWorld::step()` coordinates controller startup, waiting, retries, and when the main simulation step may execute
5. `WbSimulationWorld::step()` only runs once controller-side conditions are satisfied
...
`WbControlledWorld` currently handles:
...
`WbController` has to know about:
...
`WbController` already starts and stops `WbPerformanceLog::CONTROLLER` measurements per controller name.
...
`WbControlledWorld` contains logic that updates sensors regularly even when controller scheduling differs.
