...
`~OmSimulationWorld` never told it the world was gone. `running` stayed
...
`OmNewtonBackend::teardownWorld()` called from the world destructor.
...
`g1_walk_ghost_demo.omniworld` + `run_g1_walk_ghost_demo.ps1`: a second,
...
- Startup flake (rare, under heavy concurrent sim load): newton FFI smoke
  fails with `[Errno 9]`. **Historically** that put the whole process on ODE,
  logged by the existing warning; unfixed. ⚠ **2026-08-08 (`bdc02139`): the
  failure mode changed shape.** With ODE deleted there is nothing to degrade
  to, so the same flake now **aborts** — a Newton runtime that will not come up
  is a hard failure and the run does not produce a wrong-backend result. The
  open item is therefore no longer "silently wrong physics" but "a rare
  load-time abort under concurrent sim load"; still unfixed, and now visible
  rather than silent.
