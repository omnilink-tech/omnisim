...
- ~~important failures still surface as broad log text rather than stable machine-readable diagnostics~~ — **partially addressed by the [agent-facing validation harness](../../scripts/harness/README.md)**: `POST /world/load` returns structured diagnostic codes (`PROTO_NAME_MISMATCH`, `WORLD_PARSE_SYNTAX_ERROR`, `TEXTURE_READ_FAILED`, … 33 in all, enumerated in [`scripts/harness/diagnostic_codes.py`](../../scripts/harness/diagnostic_codes.py)) instead of free-text stderr
