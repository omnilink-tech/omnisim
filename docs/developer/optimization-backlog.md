...
3. **Core loop O(n²) per tick.** `src/omnisim/engine/OmSimulationWorld.cpp:227`
...
4. **Newton contact extraction walks the whole scene.**
   `src/omnisim/nodes/OmSolid.cpp:3881` calls `OmWorld::findSolids()` (full
   O(n) traversal) per contact-extraction. Fix: cache a
   `newtonBodyIndex → OmSolid*` map at finalize, maintained on (un)registration.

5. **Scene-tree full relayout on every cell edit.**
   `src/omnisim/scene_tree/OmSceneTreeModel.cpp:251` emits `layoutChanged()`
...
6. **wgpu renderer** (`src/omnisim/render/OmWgpuRenderTarget.cpp`, 8.4k lines):
