...
> deleted ~4.7M lines of Webots we didn't need — so the repo today is **~3.1M
> lines leaner** than the raw import, while ~23M lines of total change have moved
> through it since the fork.
...
from Webots we didn't need. Overall net diff import→HEAD:
**+1.58M added / −4.72M deleted**, so **the tree today is ~3.14M lines *smaller*
