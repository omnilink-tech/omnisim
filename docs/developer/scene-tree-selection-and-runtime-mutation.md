...
- `src/omnisim/scene_tree/OmSceneTree.cpp`
...
`OmGuiRefreshOracle` only allows refresh roughly every 300 ms while the simulation is running.
...
In `OmSceneTreeModel::updateData()`:
...
`OmSceneTree.cpp` checks whether a deleted node is a DEF node or has a descendant DEF node on which USE nodes depend.
