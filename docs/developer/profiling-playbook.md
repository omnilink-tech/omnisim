...
Average FPS is part of the log format and is populated by the WREN-side hook in `src/omnisim/gui/WbView3D.cpp`: `logWrenStatistics()` accumulates frames during the main render path (`++mRenderedFrameCount`) and calls `WbPerformanceLog::setAvgFPS(...)`. It is flushed from `WbMainWindow.cpp` on world unload and on shutdown.
...
```bash
python scripts/dev/omnisim_dev.py profile-world tests/physics/worlds/contact_points.wbt --log tests/benchmarks/logs/contact_points-local.log
```
...
- `resources/projects/worlds/empty.wbt` for startup
- `tests/rendering/worlds/normals.wbt` for a sensor-heavy-frame proxy
- `tests/physics/worlds/contact_points.wbt` for steady-state physics
- `projects/robot_combat/worlds/tests/newton_husky_head_on.wbt` for a contact-heavy physics step
- `tests/protos/worlds/template_deterministic.wbt` for world-load behavior
...
The ODE step and related physics work. This is the most important bucket for throughput regressions in physics-heavy worlds.
