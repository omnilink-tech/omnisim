...
2. `src/omnisim/nodes/utils/WbWorld.*` and `src/omnisim/vrml/*` load the world model
3. `src/omnisim/engine/WbSimulationWorld.*` owns world startup, mode transitions, and the main simulation step
4. `src/omnisim/control/WbControlledWorld.*` and `src/omnisim/control/WbController.*` synchronize controller traffic with stepping
...
`WbControlledWorld` is still driven directly by controller request flow, and `WbController` still mixes:
