...
The deploy's `OmNewtonBackend` (C++ + embedded Python,
`src/omnisim/physics/OmNewtonBackend.cpp`) builds the Newton articulation
...
native code (`OmNewtonBackend.cpp`), so it requires a **native rebuild**, and the
...
- **The post-step joint clamp** (clamp qpos to URDF limits, qvel to
  ...
  (`OmNewtonBackend.cpp`, mirrored by the trainer's `--train-joint-clamp`). It is
