...
**Owns:** the granular subsystem — `src/omnisim/nodes/OmGranularGroup.{cpp,hpp}`,
...
> `OmGranularGroup.hpp` cites two plan docs that were never written
...
(`kPhysicsKernelSrc`) inside `OmGranularGroup.cpp`. NVRTC (not a `.cu` file) is a
...
1. **Newton coupling — now a HARD BREAK on every world (L1 seam, do NOT fix in
   L8).** ⚠ **2026-08-08 (`bdc02139`, ODE backend deleted):** two-way coupling
   used to be *ODE-only*; with ODE gone it is **broken everywhere**, because both
   halves of it still address the ODE path. `OmGranularGroup::collectColliders`
   filters on `solid->body()` (an ODE `dBody`), which now returns `NULL` for
   every Solid — nothing is ever collected as a collider. And
   `onPhysicsStepStarted` pushes the reverse forces through
   `OmSolid::addForceAtPosition`, which despite its backend-neutral name still
   routes to the no-op ODE backend stub (`OmPhysicsBackendRegistry::odeBackend()
   ->addBodyForceAtPos`). One-way coupling (robot pushes particles) is
   unaffected — the kernel reads collider poses, not bodies — but **balls no
   longer push the robot back in any world**, and there is no world flag that
   restores it. ⚠ Do **not** reach for the old `physicsBackend "ode"` pin as a
   workaround — it is worse than an error. Code-verified in
   `OmSolid::effectivePhysicsBackendName`: an explicit `"ode"` still parses and
   still "wins", so the world **loads without complaint** and the Solid is simply
   handed the inert `OmOdeBackend` stub — i.e. **no physics at all**, silently.
   **The fix is a Newton-side force-application path**: the machinery already
   exists as `OmSolid::applyExternalForceNewton` (used by `OmSupervisorUtilities`
   and `OmPropeller`), so the work is (a) replacing the `body()` presence filter
   with a Newton body-presence query and (b) routing the summed per-collider
   force through the Newton call. **L8 must not edit `OmSolid`/`OmNewtonBackend`**
   — flag it here and hand to L1 via the tracker (parallel-lane protocol #3/#4).
   Note also that `OmGranularGroup.cpp`'s kernel comment still says the summed
   forces are "pushed to ODE via `dBodyAddForceAtPos`" — that comment is stale
   and describes code that no longer exists; L1 should correct it with the fix.
...
3. **M3 dynamic-state serialization.** `OmGranularGroup.hpp` notes the static
...
owned files; **never** touch `OmSolid`/`OmNewtonBackend` (L1) or the render
