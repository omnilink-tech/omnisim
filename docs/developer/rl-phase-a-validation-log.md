...
and the deploy-side `joint_f` write in `WbNewtonBackend` (an L1 file) is a
...
- **Legs-only model** (23 bodies, ~28 kg, no arms). The real deploy
  `g1_stand_deploy.wbt` is the full G1 (~34 kg, arms raise the CoM → harder); the
...
### 6. Deploy verification (real `g1_stand_deploy.wbt`, full G1) — partial transfer, NOT 30 s
Ran the canonical [`g1_stand_deploy.wbt`](../../projects/policies/research/worlds/g1_stand_deploy.wbt)
**zero-policy** (`G1_BALANCE_FALLBACK=1`) through `headless_runner.py` on the
existing **06-08 binary** (Newton confirmed active in the sim log: `[WbNewtonBackend]
...
([WbNewtonBackend.cpp:968](../../src/omnisim/physics/WbNewtonBackend.cpp)) — no
...
Caveats: 06-08 binary (misses today's L1 ball-joint/Spot commits — both irrelevant
...
fidelity issue** (the OmniSim `motor.setPosition`→`WbNewtonBackend` actuation bridge,
...
- **Deploy forward-drift / bridge fidelity:** a pure-NOMINAL zero-policy G1 holds
  ...
  `g1_stand_deploy.wbt` + `G1_BALANCE_FALLBACK=1 OMNISIM_NEWTON_TARGET_KE=400
...
- **(reiterated) joint_f deploy writer:** with the bridge gap closed, the harness's
  *torque*-mode capture-point law (which outperformed position mode) could deploy —
  but only once `WbNewtonBackend` exposes a `joint_f` write (live under MuJoCo).
...
- **Reframing (honest):** the Phase-A *gate as literally written* ("≥30 s zero-policy
  ...
  *solver*.** What remains unmet is **Phase A′** ("the same hold in the real
  `g1_stand_deploy.wbt`"), blocked by the L1 motor→Newton bridge forward-drift.
...
today's L1 commits W2.2-ball-joint / Spot-W6 are not in it — those are gated on push):
...
- **`faithful_check` (robot corpus):** **5/8 FAITHFUL** (panda 0.000, rosbot 0.003, rosbot_xl 0.005, the two cobot-arm worlds 0.000) — **identical to the §8.1 baseline**; the 3 non-faithful are all documented non-regressions (ur_arms drift 0.062 m; spot W6 collapse; mavic = uncontrolled drone).
...
**but the difference was friction**: `WbNewtonBackend` defaults `OMNISIM_NEWTON_GROUND_MU=1.0`,
and its own in-file friction-probe comment ([WbNewtonBackend.cpp:128-135](../../src/omnisim/physics/WbNewtonBackend.cpp))
...
- **Run from PowerShell, not MSYS2 bash** (the embedded/standalone interpreter
  needs `warp` from user site-packages; bash launch → silent ODE fallback). Verified
...
- **Caveat — fragile + unproven on the real deploy.** The ke window is narrow
  (300/500 fail) and the result is legs-only; the full-robot `g1_stand_deploy.wbt`
...
- **Layer 2 (residual beats baseline under push): not started** — still the
  independent, cheap-on-Spot validation; now has a standing G1 base to sit on.
...
1. **[PRIME, L1/deploy hand-off] Test joint stiffness on the REAL deploy.** Set
   ...
   and run the canonical [`g1_stand_deploy.wbt`](../../projects/policies/research/worlds/g1_stand_deploy.wbt)
...
4. **Then Layer 2:** Spot push-recovery A/B
