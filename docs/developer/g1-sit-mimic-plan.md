...
| [projects/policies/research/worlds/g1_sit_mimic.omniworld](../../projects/policies/research/worlds/g1_sit_mimic.omniworld) | The scene: two visual-only chairs, real + ghost, default sky/sun. |
...
| [projects/policies/research/worlds/g1_sit_mimic_shot.omniworld](../../projects/policies/research/worlds/g1_sit_mimic_shot.omniworld) + [g1_sit_shot](../../projects/policies/research/controllers/g1_sit_shot/g1_sit_shot.py) | Screenshot variant: a CAMBOT photographs the scene to `_scratch/g1_sit_shots/`. |
...
- ✅ **Looks right + rests ON the chair.** The chair seat has a real collision
  ...
  it. Render (`g1_sit_mimic_shot.omniworld` + `g1_sit_shot`) confirms it; PNGs in
...
1. **Base wouldn't pin.** `staticBase TRUE` strips only the **Webots root**
   `Physics` block ([OmUrdfImporter.cpp](../../src/omnisim/vrml/OmUrdfImporter.cpp)).
   ...
   [OmSolid::flushPendingNewtonRegistrations](../../src/omnisim/nodes/OmSolid.cpp)
...
the unreachable bolt-upright idealization. Because the ghost replays a pose the robot
**already achieved**, tracking it is a *control* result, not a reproducibility one:
**ROBOT vs ACHIEVABLE GHOST = 100%** (0.00° joint dev, height + rock matched) — measured
on **one rollout**, not repeated. The robot genuinely actively RL-balances (unpinned,
leaning on the chair back) and does the full wave. `run_g1_sit_mimic.ps1 -Policy`.

⚠️ **An earlier edition of this line read "OmniSim is deterministic, so the live robot
reproduces it exactly" — struck.** Determinism is never claimed unscoped in this repo, and
in any case it is not the cause of the match: run-to-run reproducibility would explain a
run reproducing *itself*, not a robot tracking a reference. (For the record the deploy
world does pin `newtonSolver "mujoco"` — the CPU `mj_step` path, which *is* in the
bitwise-reproducible set — but that is a separate fact from the 100%, and the GPU
`mujoco_warp` path is refuted. Solver-by-solver scope:
...
& msys64/mingw64/bin/omnisim-bin.exe projects/policies/research/worlds/g1_sit_mimic_shot.omniworld --mode=fast --minimize --stdout --stderr
```

Run from **PowerShell**, not MSYS2 bash (Newton needs the right interpreter — a bash
launch makes `warp` invisible to it). ⚠ 2026-08-08: the consequence is no longer a silent
ODE fallback, because ODE was deleted (`bdc02139`) — but it is not automatically loud
either. Code-verified: a runtime that is **installed but will not come up** is a FATAL
(`OmNewtonBackend::refuseIfBrokenAndNewtonWanted`), while a runtime that is merely **not
importable** — the bash-launch case — still falls through to the inert `OmOdeBackend` stub
and runs with **no physics at all, silently**, unless `OMNISIM_REQUIRE_NEWTON=1` is set.
**Add `$env:OMNISIM_REQUIRE_NEWTON=1` to these runs.** A headless `PASS` only means it loaded + ran — check the
