...
- `tests/api/worlds/accelerometer.wbt`
- `tests/physics/worlds/contact_points.wbt`
- `tests/rendering/worlds/normals.wbt` (currently `"skip": true` in `tests/smoke/smoke_worlds.json` — does not run; see its `skip_reason` for the offscreen-Camera framebuffer bug)
- `tests/protos/worlds/template_deterministic.wbt`
...
python tests/test_suite.py tests/protos/worlds/template_deterministic.wbt
```
...
```bash
python scripts/dev/omnisim_dev.py build gui
python scripts/dev/omnisim_dev.py test-world tests/protos/worlds/template_deterministic.wbt --nomake
python scripts/dev/omnisim_dev.py test-group protos --nomake
```
...
```bash
python scripts/dev/omnisim_dev.py build gui
python scripts/dev/omnisim_dev.py run-world resources/projects/worlds/empty.wbt
```
