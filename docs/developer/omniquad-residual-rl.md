# OmniQuad residual RL: the model walker + tiny policy
...
> ⚠ **2026-08-08 — THE ODE DEPLOY TARGET NO LONGER EXISTS.** `bdc02139` deleted the
> vendored ODE library; Newton with `SolverMuJoCo` (CPU `mj_step` / batched-GPU
> `mujoco_warp`) is the only physics backend, and `physicsBackend "ode"` /
> `OMNISIM_FORCE_ODE` / `OMNISIM_LEGACY` select nothing. **Every measurement in this
> document taken against "OmniSim-ODE deploy" — including the whole `kp = 20` /
> `ground_z = -0.56` / `lateral_y = 0.30` ODE-settle operating point, the ODE model-walker
> baseline (+4.77 m / -0.62 m / -15.6°) and the ODE column of the comparison table — is
> HISTORICAL and UNREPEATABLE.** It is preserved verbatim below because it is the record
> of how the recipe was derived, not because it can be re-run. **The shipped path is
> Newton**, whose measured result is the `Newton residual (CPU)` column
> (+4.87 m / 30 s, the 2026-06-11 port). Consequence: the ODE↔Newton
> comparisons below can never be re-measured, so the Newton numbers stand on their own
> and are **not** retroactively validated by the ODE baseline they beat.
...
> OmniQuad's recipe, fell over" to a robust G1 stand (≈98 % in the trainer; the OmniSim
...
|                            | omniquad_walk_v12_200k (old) | omniquad_residual_main (new) |
| --                         | --                       | --                       |
...
Production quadrupeds (OmniLink, ANYbotics, Unitree) don't train end-to-end from raw state. They have a hand-coded controller that does the structural work — gait pattern, foot trajectories, inverse kinematics, body stabilization — and learning sits on top to refine the parts that are hard to model.
...
### Layer 1: gait engine — `projects/policies/control/omniquad_gait.py`
...
### Layer 2: inverse kinematics — `projects/policies/control/omniquad_kinematics.py`

Closed-form analytic IK for OmniQuad's 3-DoF legs (hip_x, hip_y, knee). Given a target foot position in the body frame, solves for the three joint angles in three closed-form steps:
...
Dimensions extracted from `projects/robots/omnisim/omniquad/urdf/omniquad.urdf`: L1 = 0.3205 (thigh), L2 = 0.32 (shank), knee x-offset = 0.025, hip_y offset = 0.110945, hip_x mount = ±0.29785, ±0.055. Round-trip `fk(ik(target)) == target` to 1e-16 across 200 random feasible targets per leg — the IK is exact, not numerical.

### Layer 3: balance PD — `projects/policies/control/omniquad_balance.py`
...
It's worth saying clearly: the model walker is what does the heavy lifting. With **zero neural network**, just gait + IK + balance, OmniQuad walks 4.77 m forward / -0.62 m lateral / -15.6° yaw over 30 s. That alone beats v12_200k on every axis. The 20k-step residual policy adds another +0.78 m forward, halves the lateral drift, and pulls the yaw down inside the STRAIGHT threshold. The policy is a meaningful refinement, but the model layer is doing most of the work — and that's the point.

**Under Newton this becomes even sharper: the policy is essentially a passenger.** The Newton model walker alone walks +5.03m / +0.03m lat / +2.1° yaw over 30 s — already STRAIGHT. The 50k-step residual policy on top deploys to +4.87m / +0.13m lat / +3.2° yaw — marginally *worse*. The visual demo you can watch in `omniquad_residual_deploy_newton.omniworld` is, functionally, the hand-coded gait engine + Newton physics. The trained ONNX is being called every tick, but its output is small noise around an already-near-perfect baseline.

This is not a failure of the recipe — it's a *feature*. It tells us when the analytical model has converged on the right answer for the task. Under Newton's stiffer joints, planted feet (μ=2), and URDF-derived inertia tensors, the gait engineer's assumptions hold cleanly, and the gait equation is "the answer." Under ODE+kp=20 the assumptions held less well — softer contacts, more sag, less foot-floor friction — so the policy had compensation work to do. (ODE was deleted 2026-08-08; that contrast is now a historical observation, not a configuration you can select.)
...
```bash
./msys64/mingw64/bin/omnisim-bin.exe projects/policies/research/worlds/omniquad_residual_deploy.omniworld
```

The world points at `omniquad_residual_deploy`, which loads `projects/policies/research/policies/omniquad_residual_main/policy.onnx` by default. Override with `OMNIQUAD_POLICY_ONNX=<path>` to deploy a different residual policy.

### Verify a residual policy

```bash
python projects/policies/research/tools/verify_straight_walk.py \
  --policy projects/policies/research/policies/omniquad_residual_main/policy.onnx \
  --world projects/policies/research/worlds/omniquad_residual_deploy.omniworld \
  --duration 30 --vx 0.5
```
...
```bash
OMNIQUAD_FIXED_VX=0.5 python projects/policies/research/training/train_residual.py \
  --run-name omniquad_residual_v2 \
  --steps 50000 \
  --ckpt-every 5000 \
...
All passed via env vars; defaults in `omniquad_residual_agent.py`:

- `OMNIQUAD_RES_R_VX_WT` (2.0) — weight on the exp(-err) forward-velocity tracking term.
- `OMNIQUAD_RES_R_VX_SCALE` (0.10) — width of the tracking Gaussian (smaller = sharper).
- `OMNIQUAD_RES_R_HEADING_WT` (-1.0) — penalty on (yaw - spawn_yaw)².
- `OMNIQUAD_RES_R_LATERAL_WT` (-0.5) — penalty on (y - spawn_y)².
- `OMNIQUAD_RES_R_ACTION_WT` (-0.05) — penalty on ||action||² (smoothness).
- `OMNIQUAD_RES_R_ALIVE` (0.05), `OMNIQUAD_RES_R_TERM` (-10.0) — alive / termination shaping.
...
The production self-righting is purely scripted, in `projects/policies/control/omniquad_recovery.py`:
...
- `righting_joint_targets(orientation, joint_order)` — returns 12 joint angles per fallen class. LEFT_SIDE extends the left legs (push the ground, lift the chassis) and tucks the right; RIGHT_SIDE mirrors; ON_BACK folds all legs OVER the body (requires the widened URDF `hip_y` limit ≥ 2.5 rad, see `projects/robots/omnisim/omniquad/urdf/omniquad.urdf`); FACE_PLANT extends the rear legs to push the body back over its CoM; REAR_PLANT mirrors.
...
The deploy controller (`projects/policies/research/controllers/omniquad_residual_deploy/omniquad_residual_deploy.py`, FSM constructed at lines 222-225, stepped every tick at lines 520-558) runs the FSM every tick, and when it returns `RIGHTING` it commands the scripted pose and skips gait + IK + policy for that tick. No supervisor teleport. Disable with `OMNIQUAD_RECOVERY=0` if you want to see the robot just lie there.

### What was tried for the residual layer, and why it stalled

`projects/policies/research/controllers/omniquad_recovery_agent/`, `projects/policies/research/envs/omniquad_recovery_env.py`, and `projects/policies/research/training/train_recovery.py` are the parked RL stack. Three iterations of reward + curriculum + observation-space tweaks (commits 2f638404, a93a2865, 06982614, 4bd560d3, 7a9a8589) all converged to "no learning signal" — return curves flat at random-policy level across 100k+ steps. Three root causes diagnosed but not fixed:
...
The parked code is correct as a *training scaffold* — the env, controller, and reward plumbing all work, and the `OMNIQUAD_ENV_STARTUP_S` override on `omniquad_recovery_env.py` is now in sync with the residual walker. If you revisit this, fix the obs first; tuning the reward or curriculum without doing that won't help.

## Build-time gotchas worth knowing

- **Don't match the gait's foot z to the agent's nominal-pose FK.** ⚠ *Historical (ODE deleted 2026-08-08 — this operating point cannot be reproduced; re-measure the settle height under Newton before reusing these constants.)* OmniQuad's body equilibrium under ODE + kp = 20 sat at ~0.67 m even with the agent's NOMINAL_POSE, higher than the IK-computed foot z of -0.559. If you commanded the foot to body-z = -0.559, it floated 11 cm above the ground. `ground_z = -0.56` with `lateral_y = 0.30` was empirically the only combo that kept feet engaging the ground without stiffening the PD to the point of physics explosions.
- **MuJoCo `ground_z` ≠ OmniSim `ground_z`.** Same kp = 20 in MuJoCo (URDF-derived MJCF) settles OmniQuad's chassis at ~0.11 m, not OmniSim's ~0.67 m. The GPU mjwarp residual trainer treadmills with the OmniSim-tuned `-0.56` because the foot targets push deep into the ground and the IK saturates. Stiffer kp explodes in MuJoCo at dt = 0.016. Run `python projects/policies/research/tools/calibrate_ground_z.py [--mjcf <path>]` to measure the actual settle height for your MJCF and pass the recommended value via `--ground-z` to the trainer (or replace the MJCF with the deploy-faithful one exported by OmNewtonBackend, which has different mass/inertia than the URDF derivation).
...
The CPU SB3+OmniSim trainer takes 52 s wall and is plenty for a single-policy run, but multi-policy sweeps (reward grids, scale sweeps, curriculum search) want a faster loop. The intended use of `projects/policies/research/training/gpu_mjwarp_residual_trainer.py` is to take the SAME residual recipe — gait engine + IK + tiny policy — and run it at ~80 k env-steps/s in batched NVIDIA Warp on mujoco_warp physics. 1024 parallel envs × 500 PPO iters in ~157 s on an RTX 5070.

It now produces a policy that walks forward at ~0.34 m/s in its own MuJoCo physics (`projects/policies/research/training/runs/gpu_omniquad_residual_v3/policy_main.pt`, exported to `projects/policies/research/inference/policies/gpu_omniquad_residual_main/policy.onnx`). It did NOT deploy cleanly into OmniSim as measured — the policy was trained against MuJoCo contacts and kp = 250 position-velocity actuators, while the OmniSim deploy of the day was ODE contacts and kp = 20. Both contact model and motor authority differed. ⚠ *2026-08-08: that specific mismatch is gone — OmniSim deploy is now Newton/`SolverMuJoCo`, i.e. the same family the policy was trained against, so the transfer is **untested under the current backend**, not known-bad. It has not been re-measured; do not read this as a fixed transfer.* Use the CPU pipeline + `omniquad_residual_main` for anything you actually need to deploy.
...
3. **GaitParams defaults were ODE-tuned, not mjwarp-tuned.** `ground_z = -0.56`, `lateral_y = 0.30` is the empirical sweet spot for OmniSim+ODE settle (body z ≈ 0.67) — in mjwarp the body settles at ≈ 0.60 with feet floating 11 cm above the floor at neutral. New trainer defaults `ground_z = -0.62`, `lateral_y = 0.344`, `front_x = 0.322`, `rear_x = -0.274` match the FK of the canonical stand pose and seat the feet on the floor.
...
⚠ **HISTORICAL SECTION (ODE backend deleted 2026-08-08, `bdc02139`).** The deploy target
described below no longer exists, so **none of this table can be re-measured** and the
"cleaner fix" options are scoped to a comparison that cannot be re-run. Deploy is now
Newton/`SolverMuJoCo` — the same solver family as the mjwarp trainer — which removes the
contact-model half of the mismatch **on paper**; the motor-authority half (`kp = 20`
deploy vs `kp = 250` training) is a controller/world setting and is unaffected by the
deletion. Nothing here has been re-measured under Newton. Retained verbatim as the record
of the diagnosis.

The trained policy was committed and a parallel deploy controller written (`projects/policies/research/controllers/omniquad_gpu_residual_deploy/`, `projects/policies/research/worlds/omniquad_gpu_residual_deploy.omniworld`) that mirrors the trainer's 49-dim observation, joint-space ±0.15 rad residual, and MJCF-tuned gait. The mismatch between training and deploy physics is:

| Quantity                        | mjwarp training        | OmniSim-ODE deploy *(historical — backend deleted)* |
| --                              | --                     | --                     |
...
- **Retrain a foot-offset residual on the fixed mjwarp env.** Use `RES_SCALE = 0.03` and rely on the existing `omniquad_residual_deploy.py` controller. This is closer to the CPU recipe and has a small chance of transferring.
- **Skip the cross-engine deploy entirely** — use the GPU trainer as a reward / curriculum search tool, then port any successful reward shape back to the CPU SB3+OmniSim trainer for the deployable policy.
...
- `projects/policies/research/training/view_gpu_omniquad_residual.py` — live MuJoCo viewer (Python-only, no OmniSim needed); shows what the policy actually does in its own physics.
- `projects/policies/research/inference/policies/gpu_omniquad_residual_main/policy.onnx` — exported policy (ONNX round-trip vs torch = 1.8e-7).
- `projects/policies/research/controllers/omniquad_gpu_residual_deploy/omniquad_gpu_residual_deploy.py` + `projects/policies/research/worlds/omniquad_gpu_residual_deploy.omniworld` — parallel deploy controller and world for the joint-space residual policy. Loads, runs, doesn't walk well (see "Why sim-to-sim ODE deploy doesn't transfer cleanly" above).

## Resolved: the "drop into a sit" regression was URDF vs binary, not P1.5

On 2026-05-25 launching `projects/policies/research/worlds/omniquad_residual_deploy.omniworld` made OmniQuad drop into a sit instead of walking. The first hypothesis (recorded in this section's earlier version) blamed the P1.5 engine widening — commits `da10b10f` / `b2a94e10` routing `getBodyPosition` / `setBodyPosition` through `OmPhysicsBackend`. Following the prescribed diagnostic (test the model walker alone), the model walker also failed under both ODE and Newton with the same `min_bz=0.27m` signature. But the local binary (`msys64/mingw64/bin/omnisim-bin.exe`, May 18) **predates** every P1.5 commit (all landed May 25 12:13+), so the engine couldn't be the cause.

The actual cause is commit `c9e35da0` (May 24 20:00, "urdf: <rest> tag extension + widen OmniQuad hip_y for self-righting"). That commit did three coupled things:

- Added `<rest>` XML tags to every joint in `omniquad.urdf` to encode the standing pose so the URDF importer can seed it.
- Widened `hip_y` from `[0.001, 0.60]` to `[-0.50, 3.20]` so the leg can fold over the body for on-back recovery.
- Added C++ in `src/omnisim/vrml/OmUrdfImporter.cpp` to actually parse `<rest>` and apply the seed.

The May 18 binary doesn't have the parser change. It silently ignores `<rest>` tags, so joints spawn at angle 0 (straight legs) instead of the standing pose (0.30, 0.30, -0.60). The body then collapses in ~0.7s regardless of stiffness, which is exactly the failure mode the `OmNewtonBackend` SEED_POSE comment documents.

The fix without rebuilding is to point the worlds at the pre-widen URDF, which is preserved as `projects/robots/omnisim/omniquad/urdf/omniquad.classic.urdf`. The `omniquad_residual_train_newton.omniworld` and `omniquad_residual_deploy_newton.omniworld` worlds reference it directly. After the next OmniSim rebuild (which will include the `<rest>` parser), `omniquad.classic.urdf` can be retired and the widened `omniquad.urdf` will work everywhere.

The `omniquad_residual_main` policy is fine — running it through `omniquad_residual_deploy.omniworld` after pointing that world at `omniquad.classic.urdf` reproduces +5.33m / +0.22m lateral / +1.6° yaw STRAIGHT, matching the doc's original +5.55m / -0.30m / -9.4° verification within noise.
...
> OmniQuad's chassis hull overlap its four upper-leg hulls in MuJoCo's own collision, and the
> permanent internal wrench tipped every walker at ~1.2 s. Fixed bridge-side with OmniSim
...
> - OmniQuad STANDS indefinitely at `OMNISIM_NEWTON_TARGET_KE=500 KD=60..200` (250 is too soft
...
>   `OMNIQUAD_GAIT_STEP_HEIGHT=0.09` restores FORWARD walking (+2.7 m / 30 s, zero falls).
> - **HEADING SOLVED (2026-06-11 evening).** Two more root causes found and fixed:
>   (1) `omniquad_gait.foot_targets` "steered" by rotating each foot's neutral by a CONSTANT
...
>   body (+½ → −½ of the per-stance yaw, the step_x analogue). (2) `OmniQuadResidualEnv`'s
>   agent port (`env_port+200`) was EXACTLY its own simulator's `--port`
>   (`omnisim_extern_port+100`) — the trainer's TCP connect raced two listeners and ~50%
>   of training launches deadlocked at startup; ports now live in disjoint sub-bands.
>   With the fixed gait + heading lock (`OMNIQUAD_HEADING_LOCK=1 KP=1.0 CLIP=0.20
...
>   One-command repro: `projects/policies/research/runners/run_omniquad_walk_newton.ps1`.
...
|                    | ODE residual (CPU) *(historical — backend deleted 2026-08-08, column unrepeatable)* | **Newton residual (CPU)** — the shipped path |
| --                 | --                           | --                                |
...
That's already STRAIGHT, and it was measured **slightly better than the ODE model walker baseline (+4.77m / -0.62m / -15.6°)** — ⚠ *a baseline that can no longer be reproduced (ODE deleted 2026-08-08), so the comparison stands as a dated observation and cannot be re-run; the Newton figure above stands on its own and is not validated by it.* At the time, Newton's stiffer joints, planted feet (μ=2 instead of μ=1), and URDF-derived inertia tensors were a better match for the analytical gait than ODE+kp=20's soft compliance. The policy on top adds ~5% noise around an already-near-perfect baseline; it's not the load-bearing piece here.
...
- `OMNIQUAD_TRAIN_WORLD=projects/policies/research/worlds/omniquad_residual_train_newton.omniworld`
...
`OmniQuadResidualEnv` honors `OMNIQUAD_TRAIN_WORLD` (added alongside the existing same-pattern hook in `OmniQuadEnv`), so no Python changes are needed in the residual env itself for the Newton variant. The same `omniquad_residual_agent` controller runs against either physics backend.
...
```bash
OMNIQUAD_POLICY_ONNX=projects/policies/research/policies/omniquad_residual_newton_main/policy.onnx \
...
OMNIQUAD_VX=0.5 \
./msys64/mingw64/bin/omnisim-bin.exe projects/policies/research/worlds/omniquad_residual_deploy_newton.omniworld
```
...
- Independent stress test in [tests/engine/joint_limits/](../../tests/engine/joint_limits/): spawns OmniQuad, pelts it with 1 kg cubes at 5 m/s, 5 kg cubes at 10 m/s, then a 20 kg anvil dropped from 2.5 m. Asserts no joint ever exceeds URDF limits. With clamp off: 185 violations in the same run. With clamp on: 0.

**The build trap that blocked us for an afternoon** *(historical — the mechanism has changed twice since)*. After `00280fe0` landed I rebuilt several times without setting `OMNISIM_WITH_NEWTON=ON`. At the time the Makefile defaulted to `OFF` — `#ifdef OMNISIM_WITH_NEWTON` blocks then compiled to stubs, the entire Newton backend (including the new clamp) was silently absent, and worlds with `physicsBackend "newton"` fell back to ODE without any runtime warning. My env-var tuning (armature / limit_ke / KD=500) was acting on an ODE binary the whole time. Diagnostic prints inside the embedded Python source never executed because the source itself was never compiled in.

⚠ **2026-08-08 — both halves of that trap are gone, and the failure is now louder.** `src/omnisim/Makefile` has `OMNISIM_WITH_NEWTON ?= ON`, so you no longer have to pass it; and there is no fallback to fall back to — with ODE deleted (`bdc02139`), an `OMNISIM_WITH_NEWTON=OFF` build produces a binary with **no working physics backend at all** (every Solid lands on the inert `OmOdeBackend` stub). The modern version of this trap is therefore "no physics", not "secretly ODE" — and note the two runtime cases differ: an *installed but broken* Newton runtime FATALs, while an *absent* one still degrades silently to the inert stub unless you set `OMNISIM_REQUIRE_NEWTON=1`. Set it.
...
**Empirical result with the clamp in a correctly-built binary (no env-var tuning needed):** headless run of [omniquad_residual_deploy_perturb_newton.omniworld](../../projects/policies/research/worlds/omniquad_residual_deploy_perturb_newton.omniworld) — OmniQuad walking with cubes thrown every 3 s for ~100 sim-seconds — analyzed per-tick:
...
Every "glitch" the user reported pre-fix was either (a) the env-var-tuned ODE binary masquerading as Newton (a confusion that is no longer possible — there is no ODE binary to build) — i.e., none of the supposed Newton fixes were actually applied — or (b) the recovery FSM commanding the OVER pose (`hip_y=2.50`) into the narrow `omniquad.classic.urdf` (`hip_y` capped at 0.60) and joints saturating against the limit. The clamp now makes that saturation behave like a real end-stop. The deploy world now uses the widened `omniquad.urdf` so the recovery FSM has the range it needs.

**Tooling note** for future debugging:
- [`projects/policies/research/tools/analyze_joints.py`](../../projects/policies/research/tools/analyze_joints.py) consumes `OMNIQUAD_DEBUG_JOINTS_CSV` output and flags any unrealistic motion category (out-of-range positions, over-velocity, big jumps, acceleration spikes, direction reversals). Run after a deploy headless capture; 0-everything = simulator behaving.
- Embedded-Python stderr does NOT reach the host process reliably (see the embedded-interpreter stdio handling at [OmNewtonBackend.cpp:2482](../../src/omnisim/physics/OmNewtonBackend.cpp#L2482)). For diagnostics inside the Newton helper module, write to a file. The Newton backend's own solver-init log under `.build_tmp/newton_solver.log` confirms the helper is parsed — if that log is missing after a run, `OMNISIM_WITH_NEWTON=ON` wasn't set during the build.
...
Production quadrupeds (OmniLink, ANYbotics, Unitree) all build this way: hand-coded controller doing the structural work, learning sitting on top to refine what's hard to model. We're following the same pattern.
...
2. **Training launchers go through `train_residual_*.py`, not `train_robot.py` / `train_omniquad.py`.** Those legacy launchers stay for backward compatibility and for the rare task that genuinely needs from-scratch, but they aren't the default.
...
⚠ **2026-08-08 label note:** the "CPU/ODE" labels below describe how these files were
*originally built and tuned*, not what they run on. ODE is deleted, so **everything here
that still loads runs on Newton/`SolverMuJoCo`.** Verified while relabelling:
`omniquad_residual_train.omniworld`, `omniquad_residual_deploy.omniworld` and `omniquad_gpu_residual_deploy.omniworld`
all already carry `physicsBackend "newton"` + `newtonSolver "mujoco"` (the 2026-06-11
migration), so none of them fails to load on the ODE deletion. The ODE-era **policies and
tuning** are a different matter: they were trained against physics that no longer exists
and have **not** been re-validated on Newton.

- `projects/policies/control/omniquad_kinematics.py` — analytic FK + IK.
- `projects/policies/control/omniquad_gait.py` — trot foot-trajectory generator.
- `projects/policies/control/omniquad_balance.py` — body-pose PD.
- `projects/policies/control/omniquad_recovery.py` — scripted self-righting FSM (shipped).
- `projects/policies/research/controllers/omniquad_model_walk/` — model-only walker (no policy).
- `projects/policies/research/controllers/omniquad_residual_agent/` — training-side controller (TCP env).
- `projects/policies/research/controllers/omniquad_residual_deploy/` — CPU deploy controller, originally ODE-tuned (loads ONNX; runs scripted recovery on fall). Backend-agnostic controller code — it drives motors, not a backend — so it runs on Newton. Works in either OmniSim worlds if the URDF matches the running binary — see "Resolved" above.
- `projects/policies/research/controllers/omniquad_gpu_residual_deploy/` — joint-space-residual deploy controller for the mjwarp-trained policy. Sim-to-sim transfer *to ODE* was unreliable; ran but didn't walk well — see "Why sim-to-sim ODE deploy doesn't transfer cleanly" (now a historical section; the ODE transfer cannot be re-measured, and the Newton transfer has not been measured).
- `projects/policies/research/controllers/omniquad_recovery_agent/` — **parked** residual-RL recovery agent (see "Applying the recipe to self-righting" above).
- `projects/policies/control/omniquad_gait_np.py`, `projects/policies/control/omniquad_kinematics_np.py` — vectorized numpy versions of gait + IK used by the GPU trainer (round-trip the scalar references to 1e-6).
- `projects/policies/research/envs/omniquad_residual_env.py` — gym env.
- `projects/policies/research/envs/omniquad_recovery_env.py` — **parked** gym env for recovery RL.
- `projects/policies/research/training/train_residual.py` — tiny PPO trainer (CPU, SB3, in-engine physics — the shipped recipe). Written against OmniSim-ODE physics; it trains through whatever backend the world selects, which is now always Newton.
...
- `projects/policies/research/training/view_gpu_omniquad_residual.py` — live MuJoCo viewer for the GPU-trained policy.
...
- `projects/policies/research/worlds/omniquad_residual_train.omniworld` — the original ODE-era training world; **now Newton** (`physicsBackend "newton"`, `newtonSolver "mujoco"`) — loads fine.
- `projects/policies/research/worlds/omniquad_residual_train_newton.omniworld` — training world (Newton MuJoCo CPU).
- `projects/policies/research/worlds/omniquad_residual_deploy.omniworld` — the original CPU/ODE deploy world; **now Newton** (`physicsBackend "newton"`, `newtonSolver "mujoco"`) — loads fine.
- `projects/policies/research/worlds/omniquad_residual_deploy_newton.omniworld` — Newton deploy world.
- `projects/policies/research/worlds/omniquad_gpu_residual_deploy.omniworld` — sim-to-sim deploy world for the mjwarp policy.
- `projects/policies/research/training/train_residual.py` — tiny PPO trainer (CPU, SB3; originally OmniSim-ODE physics, now Newton — duplicate entry, see above).
...
- `projects/policies/research/policies/omniquad_residual_main/policy.onnx` — the ODE-trained policy (**historical artifact**: the physics it was trained in no longer exists; not re-validated on Newton). Prefer the Newton policy below.
- `projects/policies/research/policies/omniquad_residual_newton_main/policy.onnx` — shipped Newton policy.
- `projects/policies/research/inference/policies/gpu_omniquad_residual_main/policy.onnx` — GPU/MuJoCo policy (not deployable yet).
- `projects/robots/omnisim/omniquad/urdf/omniquad.classic.urdf` — pre-widen URDF (no `<rest>` tags, narrow hip_y limits). Used by the Newton training/deploy worlds until the binary is rebuilt with `<rest>` parser support.
