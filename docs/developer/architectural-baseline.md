...
1. **Both backends are first-class and dispatched.** Every physics body op and every
   render/sensor op flows through a single backend-agnostic surface (`WbPhysicsBackend`,
   `WbRenderBackend`); selecting a backend is a per-node field + a runtime resolve, never
...
- [x] **`WbPhysicsBackend` dispatcher** — symmetric PAL, ODE canonical / Newton opt-in
...
- [x] **`WbRenderBackend` dispatcher** — symmetric to physics, WREN canonical / wgpu opt-in.
- [x] **Runtime resolve + fallback** — `OMNISIM_FORCE_WREN` + `OMNISIM_LEGACY` (`459a2a21`).
- [x] **Main-view dispatch seam** — `WbView3D::renderNow → renderMainFrameViaWgpu`
      (`5ef0e329`); WREN default byte-identical.
- [x] **On-screen present path** — offscreen wgpu render → GL blit (`render/WbWgpuGlBlit.cpp`);
...
- [x] **Surface abstraction** — `WbWgpuSurface` (Win32-HWND); Metal/Linux are *impls within
...
>   B4 (physics gate cell + CI reconciliation), C2 (`WbPhysicsBackend` surface sign-off).
...
>   ~~C2 (`WbRenderBackend` surface sign-off)~~ ✅, ~~B4 render cell (render gate in the pre-push
...
- [x] **A1 — Missing-geometry bug FIXED (2026-06-07, `14107285`).** *The original
      ...
      **without making the WREN GL context current**. `collectWorldDraws → acquireFromWren →
      ...
      persistent first-frame cache did not. **Fix:** one `WbWrenOpenGlContext::makeWrenCurrent()`
      ...
      - *Known separate gap (NOT A1, pre-existing):* `geometric_primitives.wbt` collects 0
...
- [x] **A2 — Real scene sun + cast shadows, DONE (2026-06-07, `14107285`).**
      `renderMainFrameViaWgpu` now harvests the world's first `WbDirectionalLight` (e.g. the
...
- [x] **B1 — Dual-backend oracle, one tool. ✅ DONE — 🟦 physics arm + unified front-end (2026-06-07);
      ...
      - **Physics arm (done):** delegates to `physics_oracle.py` — ODE-vs-Newton per-body divergence
        table (report) or the invariant gate (`--gate`). Verified this session on `oracle_drop.wbt`:
...
- [x] **B3 — Physics determinism gate, standing. 🟦 ✅ DONE + VERIFIED (2026-06-07).** The gate is
      ...
      diverges from forced-ODE by 0.275 m); verified PASS this session (INV3 max-diff 0 m ⇒ Newton is
...
- [~] **B4 — CI runs the gates. 🟦 physics cell reconciled (2026-06-07); 🟥 render cell pending.**
      **Correction:** `.github/workflows/physics-gates.yml` does **not** exist (claimed "authored" — it
      ...
      - 🟥 **Render cell (render session) ✅ wired into the pre-push hook (2026-06-07).** `.githooks/pre-push`
        ...
        `WbWgpu*`, `WbView3D`/`WbWgpuView`, `render_oracle.py`), since the wgpu self-check needs a brief
...
- [~] **C2 — Confirm both dispatcher surfaces are FINAL.** Quick audit: no future op should
      require *adding* a virtual to `WbPhysicsBackend` / `WbRenderBackend` (Newton overriding
      ...
      [dispatcher-surface-signoff.md §2](dispatcher-surface-signoff.md): `WbRenderBackend` is a THIN
      ...
      adapter`). The render operations live in a shared CONCRETE layer (`WbWgpuSceneRenderer`/
      `WbWgpuSurface`, shared by main-view + sensors), dispatched select-then-concrete — so ordinary
...
- **Cross-platform surfaces** (Metal/Linux): needs hardware not present here; same
  `WbWgpuSurface`/`WbWindingSurface` abstraction, new platform impl.
...
1. **Phase B-AUDIT (½ day) — close the surfaces (C2) + scope the oracle (B1).** Confirm no
   ...
   framing.* — 🟦 **physics side LANDED (2026-06-07):** `WbPhysicsBackend` signed off
   ...
   side (C2 `WbRenderBackend` sign-off + the render arm) is the render session's.
