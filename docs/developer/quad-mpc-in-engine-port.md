...
`src/omnisim/physics/WbNewtonBackend.cpp` is free of concurrent edits.
...
PER-ROBOT heading (YAW_W): go2 (12,4) -> dead straight ; spot/b2 (0,0) -> straighter
...
- **Angular-rate damping** is what bought durability (Spot 8 s fall → 14 s upright).
- Heading weights are **per-robot** (small/light go2 wants strong yaw; heavier
  spot/b2 don't).

**Offline results** (mujoco_warp, ~0.4 m/s target): Spot 14 s upright @ ~0.27 m/s
...
bound is negative (`jnt_range[1] < 0`, true for spot/go2/b2); else `hip_y`. Map to
...
## 4. Exact additive patch points in `WbNewtonBackend.cpp`
...
- **Coordinate** off `WbNewtonBackend.cpp` with the concurrent session before editing;
