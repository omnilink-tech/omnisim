...
inertia rollup in `WbSolid.cpp`); the other found the **simpler, more general, lower-risk** cause
...
`rolledUpComInertia()` in [`src/omnisim/nodes/WbSolid.cpp`](../../src/omnisim/nodes/WbSolid.cpp)
...
- A gated arm-hinge stiffness scope (`OMNISIM_NEWTON_ARM_KE`/`ARM_KD`, effort-thresholded so it
  ...
  [`WbNewtonBackend.cpp`](../../src/omnisim/physics/WbNewtonBackend.cpp) for cases where a *soft*
