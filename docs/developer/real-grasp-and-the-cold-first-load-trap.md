...
> (`OmNewtonBackend::finalizeWorld`): cold now builds **MuJoCo** instead of falling back to XPBD, and
...
2. **A position-servo finger is limp on a thin wall.** Grip force = ke·error capped at
   ...
   **finger-only** knob, `OMNISIM_NEWTON_FINGER_KE` / `_FINGER_KD`, applied only to
   prismatic/slider joints (`OmNewtonBackend.cpp`). The arm keeps its safe gains.
...
plumbed from `OmSolid`'s Newton flush and can arrive *after* the solver is built. The
