...
> ## ⚠️ HISTORICAL — Atlas was REMOVED from OmniSim on 2026-07-17
>
> **This document is retained for its negative result only. The robot it describes is no longer
> in the tree**, and every path it names below has been deleted — the model, the trainer, the
> controllers, the worlds and the checkpoints. **The Atlas package ships as REMOVED in v5.3.0**,
> so the paths below are written as plain `code`, not links — none of them resolve in this tree.
> Nothing here is runnable.
>
> It is kept because the finding is worth more than the code: **PPO could not extract a residual
> that beat the analytic baseline at 30 DOF**, and that cost real compute to establish. Read this
> before proposing that Atlas (or a comparably heavy, high-DOF biped) be re-imported and retrained
> on this recipe — the conclusion below is the reason it was dropped, not an invitation to retry it.
>
> Atlas was removed for two independently sufficient reasons: this negative result (nothing shipped
> depended on it) and an **unclear upstream chain of title** for the OmniLink v5 geometry
> (see the internal `docs/developer/third-party-licenses.md`, blocker #9 — that doc is held from
> the public snapshot; the public licensing record is
> [`THIRD_PARTY_NOTICES.md`](../../THIRD_PARTY_NOTICES.md)). **Re-importing the
> geometry would reopen the licensing blocker** — resolve that first, regardless of the RL question.
> The deleted code is recoverable from git history prior to the removal commit.

Companion to [`g1-stand-rl-playbook.md`](g1-stand-rl-playbook.md) and [`sim-to-deploy-rl-recipe.md`](sim-to-deploy-rl-recipe.md). What changes when you move the same recipe from a 13-DOF G1-legs trainer to the 30-DOF OmniLink Atlas.
...
| Artifact | Path |
|--|--|
| GPU mujoco_warp trainer (30 DOF, curriculum support via `--init-from`) | `projects/policies/research/training/gpu_mjwarp_atlas_stand_trainer.py` |
| Newton-MJCF → readable-MJCF importer (arm-collision strip + njmax bump) | `projects/policies/research/training/import_newton_mjcf_atlas.py` |
| Deploy controller (mirrors trainer baseline + loads policy.onnx) | `projects/policies/research/controllers/atlas_stand_deploy/atlas_stand_deploy.py` |
| Deploy world (z=0.95 spawn for new NOMINAL) | `projects/policies/research/worlds/atlas_stand_deploy.wbt` |
...
Fix in `import_newton_mjcf_atlas.py`: walk the body tree and set `contype="0" conaffinity="0"` on all geoms whose body is in `{l_clav, l_scap, l_uarm, l_larm, l_ufarm, l_lfarm, l_hand, head, …r-side mirror…}`. After that, the only contacts left are the two feet on the ground.
...
- [`omniquad-residual-rl.md`](omniquad-residual-rl.md) — the quadruped path.
