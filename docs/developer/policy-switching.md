...
for Legged Robots** ("Policy Switching" is the plain technical name; the maintainer may rename either.)

> ## ⭐ BATON IS A LIBRARY NOW, NOT A HUMANOID DEPLOY FUNCTION (2026-07-13)
>
> It used to be ~170 lines living **inside** `g1_walk_recipe.py`'s in-engine deploy hook, written
> out **twice** (a course arbiter and a schedule arbiter) plus a fossil third copy in the legacy
> recipe. Every assumption in it was a humanoid one: it blended an `arm` and an `elb` channel, it
> reached into `world._wr_*`, it ran torch nets, and it only let a specialist go at **double
> support**. The duplication was not merely ugly — it was **hiding a live bug**: `BATON_COLD_HIDDEN`
> was implemented only in the schedule copy, so all four box demos set it and **it never once fired**.
>
> It is now [`omnisim/policy/baton.py`](../../omnisim/policy/baton.py) — one
> arbiter, and its executable code contains **zero** references to `torch`, `world`, arms, elbows or
> cranes. The entire robot-specific surface is an injected **host**:
>
> | BATON asks | the host answers |
> |---|---|
> | which reference channels exist? | `host.channels` (humanoid: glut/arm/elb/att/ref · **quadruped: glut/ref/ffdq**) |
> | when may a specialist let go? | `gate(phase)` — biped: calibrated double support; quadruped: a **host-injected gait-model support window** (trot/pace/crawl differ) |
> | how is a policy evaluated? | `host.act(...)` — **torch** in-engine, **ONNX** in a controller |
>
> **The falsifier ran.** [`go2_baton_deploy`](../../projects/policies/research/controllers/go2_baton_deploy/)
> drives a **Go2** through `walk → stand → walk` using this library **unchanged**, in a host that
> breaks every original assumption at once: a Webots **controller** (no `world`), **ONNX** (no torch),
> **no arms**, **no crane**, and a gait-specific four-foot-support gate rather than the biped
> double-support rule. Verified live on a RunPod 4090
> (Newton/mujoco_warp), `ONNX loaded:` asserted:
>
> | run | result |
> |---|---|
> | Go2 `walk:10,stand:6,walk:10` | 2 switches, **0 falls**. On the hold: `vx → 0.00`, `gmatch 0.998` |
> | Go2 `walk:8,stand:5,walk:8,stand:5,walk:10` | **4 switches, 0 falls**, 391 m over 960 s sim |
> | Go2 walk segment **under BATON** | gmatch **0.865** |
> | Go2 walk **without BATON** (control) | gmatch **0.864** — *the arbiter costs the walk nothing* |
> | G1 `box_delivery` (course arbiter) | **9 switches**, 0 falls, min pelvis z 0.659, ends x=10.69 (pre-refactor: x=10.70) |
> | G1 `walk_turn_walk` (schedule arbiter) | stand+walk handovers + the 90° solo-turn, ends yaw **86.6°**, upright |
>
> **A quadruped `stand` needs no policy at all.** Four feet on the ground is statically stable, so the
> hold is a BATON specialist with `policy=None` — zero residual, tracking a nominal-stance ghost
> ([`go2_stand`](../../projects/policies/skills/quadruped/go2_stand/skill.json)). It required **no
> training**. The G1's stand is a *trained* specialist on a weight-bearing crane. **The library does
...
>
> ### ⛔ THIS IS NOT A PAPER YET — say so out loud (2026-07-12)
>
> There is **one** OmniSim paper ([`shadowing_paper/`](shadowing_paper/), built, 18 pages) and
> **this scaffold**. This file has no abstract, no figures, no bibliography and no build script.
> Do not describe BATON to anyone as "our second paper" — describe it as a **mechanism that ships
> and a hypothesis we have not yet tested**. That framing is defensible to a robotics partner;
> the other one evaporates the moment they ask for the plot.
>
> **What was attempted on 2026-07-12, and why it produced no result.** The cheapest decisive arm of
> step 1 below needs no training at all: BATON's *engineered* handover vs a *naive* one, on the same
> specialists, at growing horizon. The knob exists (`BATON_COLD_HIDDEN`, the stand-attractor lock),
> so `box_delivery` was run with it on (shipped) and off (naive warm hidden). **Both runs ended
> upright near the final waypoint, and the demo emits no per-segment success signal** — so the two
> arms could not be scored against each other, and no claim is made here. That is the real blocker,
> and it is smaller than it looks:
>
> **`baton_metrics.py` must log per-cycle success vs cycle index** (the scaffold has said so all
> along, step 1 below). Until a run emits "cycle *k*: completed / fell / stalled", every BATON
> comparison is eyeballed from a trajectory dump, which is exactly how a rushed ablation turns into
> a result nobody can defend. Instrument first, then run the three arms.
...
- **Specialist B (stander)**: trained in the SAME era/arch/obs family (LSTM, REF_OBS,
  ...
  (`ghosts/g1/ghost_stand_v1_lut.json`: unitree-default pose = the walker's
...
- 2026-07-06 (evening 3): **THE REAL SEQUENTIAL DEMO — a physical box through the full
  ...
  - ⛔ **THE BACKEND LESSON (measured twice, the day's most expensive discovery):** a
    ...
    face-plant at the place swap). **Fix (as shipped 2026-07-06, historical): props
    that supervisors move were `physicsBackend "ode"` Robot nodes** (the ghost-hologram
    precedent) — they lived outside the Newton state entirely, so writes were safe and
    cheap while gravity and contact stayed real. Mixed-backend caveat at the time: no
    box↔robot contact across backends (the carried pose keeps hand clearance by design;
    no runtime weld exists in the engine, so the carried phase is kinematic-honest and
    the payload dynamics are the carry policy's trained plant).
    - ⚠ **2026-08-08 (`bdc02139`, ODE backend deleted): the fix above no longer works, and
      it fails SILENTLY.** Code-verified in `OmSolid::effectivePhysicsBackendName`: an
      explicit `physicsBackend "ode"` still parses and still "wins", so such a world
      **loads without complaint** — the prop is just handed the inert `OmOdeBackend` stub
      and gets **no physics at all** (no gravity, no contact, no rest). The pin selects
      nothing. That escape hatch is gone. **The reasoning still holds unchanged** — a
      supervisor-moved prop must live outside Newton's *dynamic* state or every write
      dirties the solver. **The Newton-native answer is a physics-less node**: a Solid /
      Robot with **no `Physics`** child, which Newton registers as a fixed-root MuJoCo
      **mocap body** (`add_kinematic_body`). Its pose is written straight into
      `mj_data.mocap_pos`/`mocap_quat` (`OmNewtonBackend::setKinematicPose`), effective
      on the next step, with no re-import and no state invalidation — it is never
      integrated, so it is outside the solver's dynamic state exactly as the ODE prop
      was. The trade changes shape rather than disappearing, and it is a **worse** trade
      in one respect: a mocap prop's geoms are collision-tested against dynamic bodies
      every step (so it pushes the robot, and contact is not lost across backends the way
      it was) but it is never pushed back, never integrates, and **has no gravity** — so
      the "REAL box that genuinely RESTS on the cart" property of the 2026-07-06 delivery
      is *not* what a mocap prop reproduces. Recovering that needs a dynamic Newton body
      plus a write discipline that never writes it per tick (or the weld path). The
      7/7-switch / zero-fall numbers above were measured on the ODE-prop configuration
      and are **not reproducible** as recorded.
...
- 2026-07-10: ⭐ **THE 90° FOOTWORK TURN IS FINALIZED — `walk_turn_walk` ships (`72a7bb19`; deploy
  ...
  - **Verified 3/3 headless** (telemetry, not screenshots): the turn lands **90.6 / 91.5 / 95.6°
    ...
    `verify-demos` MATCHes (the `walk_turn_walk` manifest reproduces the script's 40 env keys).
