...
| **OmniQuad** (12-DOF quadruped) | **4.5e-7 rad** (→1e-8 by end) | ✅ PASS | machine precision **after the self-collision fix** (was hip_x ~3.4° DIFF — see below) |

Takeaway: the engine + compiled model are parity-clean **in the binary** for all 6
legged robots probed — machine precision for G1/H1/OmniQuad, ~0.0001–0.02° for Go2/B2,
...
### The OmniQuad hip_x gap — root-caused + fixed (self-collision)

OmniQuad first showed a localized hip_x (ab/adduction) divergence of ~0.06 rad (3.4°)
...
the trainer's full-mesh OmniQuad legs touched the body at the crouch and got shoved
(hip_x most); the deploy (`OmNewtonBackend`) **filters intra-robot self-collision**,
...
the deploy. OmniQuad then matches to ~4.5e-7 rad (PASS); H1/Go2/B2 are unchanged (they
...
| Deploy worlds (welded + free base) | [`g1_parity_probe.omniworld`](../../projects/policies/research/worlds/g1_parity_probe.omniworld) / `g1_parity_probe_free.omniworld` |
...
1. **`staticBase` weld-at-origin defect — FOUND by this probe and FIXED.**
   ...
   (`add_joint_fixed(parent=-1, child=root)` in `OmNewtonBackend.cpp`'s embedded
   ...
   **Fix:** pin the weld at `body_q[root]` (mirroring the static-collider weld),
   in `OmNewtonBackend.cpp` + the regenerated `g1_deploy_runtime.py` extract. After
