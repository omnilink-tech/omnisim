...
>    warp import** in `WbNewtonBackend.cpp`. Newton now reliably *engages* wherever
>    the runtime is present. **The real "Newton is active" signal is the
>    `[WbNewtonBackend] world finalised (solver=...)` line — NOT an earlier
...
>    when set, a Newton-init failure becomes `WbLog::fatal` (loud + non-zero exit)
...
| **C. Runtime resolve** (the dispatcher + fallback) | `WbPhysicsBackendRegistry::resolve()` — Newton-when-available, else ODE; honors `OMNISIM_FORCE_ODE` | `WbRenderBackendRegistry::resolve()` — Vulkan-when-available, else WREN; **no global force** |
...
- **Keep** `OMNISIM_FORCE_ODE` (read in `WbPhysicsBackend.cpp:272`, short-circuits `resolve()`).
- **Add** `OMNISIM_FORCE_WREN` — mirror it in `WbRenderBackendRegistry::resolve()`
  (`WbRenderBackend.cpp:108`): when set, return `wrenBackend()` for every kind, ignoring the field.
...
every Solid/Viewpoint. Add to `WbWorldInfo`:
...
- **Physics oracle** — ✅ BUILT: `scripts/dev/physics_oracle.py` runs the same world twice (ODE via
  ...
  (`tests/physics/controllers/oracle_dumper/`) on `tests/physics/worlds/oracle_drop.wbt`, then reports
...
- **Render oracle** — render the same Viewpoint under WREN and wgpu and image-diff them. This is the
  ...
  `WbWgpuView`'s self-check / `grabWindowBufferNow`) is the metric. **Valid only on a static scene
...
- **Physics determinism gate** — exists: `tests/smoke/run_smoke.py` + `smoke_worlds.json` (empty,
  ...
  `OMNISIM_FORCE_ODE=1` (the legacy baseline).
- **Newton pass/fail gate** — ✅ BUILT (commit fdb10589, **Option C: physical invariants**). The design
  ...
  violation. Verified both ways: PASS on the settled drop world (and INV3 max-diff 0m ⇒ Newton is
  bit-deterministic on this box); FAIL (exit 1) when bodies are still mid-fall, so it's not a rubber
  stamp. Extend later with more worlds (per-arm completeness, Stages 1–2).
...
- **RESOLVED at the source (2026-07-05) — per-launch IPC pipe nonce.** The residual race was the
  ...
  both sides (`WbController::start` + libController `robot.c compute_socket_filename`); extern names
...
**Status (2026-06-06):** `WbSolid::articulationNewtonCapable()` (root-resolved, recomputed per query,
consulted in `effectivePhysicsBackendName()`) ships and is verified on BOTH tiers:
- **Tier B (mesh):** verified — oracle_mesh.wbt: mesh faller → ODE, box faller → Newton.
- **Tier A (joints):** verified — oracle_mixedjoint.wbt (ROOT -hinge-> MID -ball-> TIP): the gate fires
...
Three real bugs were found + fixed to make Tier A actually work (807419f8): (1) `WbBallJoint` inherits
`WbHingeJoint`, so `dynamic_cast<WbHingeJoint*>` wrongly accepted ball joints — now checks exact
...
- **Tier A — correctness (gate unconditionally):** non-Hinge/non-Slider joints. The Newton joint
  registration only handles `WbHingeJoint`/`WbSliderJoint` (WbBasicJoint.cpp:181-182); a ball/hinge2
...
- **Tier B — fidelity (policy call):** mesh collision. It is NOT unsupported — the code already
  AABB-fits mesh boundingObjects for Newton (`computeBoundingObjectMeshAabb`, WbSolid.cpp:2390,
...
- **Detection + caching:** correctness needs *articulation-level* analysis (walk from the articulation
  root over the subtree's joints + boundingObjects) to avoid the per-link mixing that caused the Spot
...
- **N1 (slider/prismatic) — ✅ DONE.** `WbNewtonBackend::addJointPrismatic` registers sliders on Newton
...
- **N3 (stand without env babysitting) — ✅ DONE.** `OMNISIM_NEWTON_SUBSTEPS`/`STATICS` folded into
  ...
  real `[WbNewtonBackend] world finalised (solver=...)` line before attributing a fall to physics, and use
...
- **N5 (RL transfer parity) — DOCUMENTED LIMITATION.** Spot walks (model+residual); G1 deploy loses balance
...
- **N1 — non-revolute joints** (prismatic at least): widen `WbNewtonBackend` past the `-1` inherit so
...
**Mechanism — packaging-only, no engine/C++ change.** `WbNewtonBackend` brings
the interpreter up with a bare `Py_InitializeEx(0)` (no `Py_SetPythonHome`, no
`PyConfig`, no `sys.path` edits — verified in `WbNewtonBackend.cpp`), then
...
> per-submit leak. Root cause: `WbWgpuSceneRenderer::collectShapeDraws` keyed the wgpu texture
> cache on the `WbImageTexture*` **pointer**, but a scene PROTO instantiates a separate
> `WbImageTexture` node per use — so dozens of shapes sharing one texture FILE (one `Plaster.jpg`
...
The 3c-B seam (`WbView3D::renderNow` → `renderMainFrameViaWgpu`, gated by `OMNISIM_WGPU_MAINVIEW`)
...
- **Option A — window-swap: ❌ RULED OUT (2026-06-06).** The premise was "the surface path has no
  ...
  `WbWgpuView` pane, whose `renderTick`→`drawWorld(nullptr)`→`presentScene(rgbaOut=nullptr)` is pure
  ...
  Former scope notes:
  `WbSimulationView` has **84** `WbView3D`/`mView3D` references and calls many `WbView3D`-specific
  ...
  `WbWgpuView : QWindow` is NOT a `WbView3D` subclass (no shared interface). So Option A requires either
  giving `WbWgpuView` the full `WbView3D` interface (methods + signals + the editor interactions) or
...
not an app leak:
- **Texture IDs are stable** — `WbWgpuSceneRenderer::collectShapeDraws` keys the texture cache on the
  `WbImageTexture*` (`reinterpret_cast<uint64_t>(map)`), which is a stable scene-graph pointer across
...
- **Per-frame draw resources ARE released** — the per-draw bind groups and uniform buffers in
  `WbWgpuRenderTarget`'s scene/textured draw paths are `wgpuBindGroupRelease`/`wgpuBufferRelease`'d each
...
**Attempt-2 (Option A, surface path) TESTED + RULED OUT:** ran the live `WbWgpuView` surface pane
...
Only Win32-HWND surface exists (`WbWgpuSurface`). Metal (macOS) and Vulkan-surface (Linux) are
...
Flip the schema defaults (Viewpoint.wrl:17, Camera.wrl:41) and the `WbWrenWindow`/`WbView3D` dispatch
