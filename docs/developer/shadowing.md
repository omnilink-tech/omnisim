...
4. **Empirical results** — feasibility predicts learnability + sim-to-deploy success;
   generality across motions and robots (Spot walk **47.8 m / 0 falls**, Go2 walk, B2 walk, arm
...
1. **Robust support-contact reconstruction** (`support_contacts`). Instead of named feet at a
   ...
   to the local ground) — this finds belly/knee/elbow support in a get-up and Spot's foot spheres
   (Spot has no foot-*named* body; a kinematic-leaf-body fallback selects geoms 4/7/10/13); (b) a
...
support frames are judged — both regression-guarded in the self-test (`spot getup+shake → INDET`,
...
- **E3 — Generality:** one tracker, many generated ghosts → G1 {walk, sit-stand, reach-
  while-standing}; second robot (Spot). Robot-/motion-agnostic generator + tracker. **Non-legged
...
- **E4 — Sim-to-deploy:** deploy success/robustness of policies shadowing certified ghosts.
  **Quadrupeds crossed the gap** (Spot walk 47.8 m / 0 falls, Go2/B2 walk, arm toss — verified in
...
| spot getup | spot_newton_fixed2 | getup | **PASS** | 0.05 | judged on reconstructed leg support (93 stance frames) |
| b2 getup | b2_planner | getup | **PASS** | 0.40 | reconstructed leg support; b2 has no trunk collider (model gap) |
| spot jump | spot_newton_fixed2 | jump | **FAIL → boundary** | 0.00 | flight clean (8.6% mg) but the kinematic ghost's explosive *takeoff* is not contact-consistent (33% mg loading) — flagged, not rubber-stamped |
| b2 hill6 | b2_planner (+terrain) | hill | **PASS** | 0.06 | base p95 5.8% mg, 1317/1317 stance |
| spot hill8 | spot_newton_fixed2 (+terrain) | hill | **PASS** | 0.05 | base p95 0% mg, 0 bad-nocontact |
...
- **cert PASS & deploy-FAIL** (the E5 zone): 4 — **g1 walk** (the crux row), b2 hill6, spot hill8,
  g1 sitstand.
...
(rl-current-state.md flags the G1 deploy numbers as such; the B2-hill / spot-get-up verdicts come from
...
was changed after the last build — always diff `omnisim-bin.exe` vs `WbNewtonBackend.cpp`
