...
> ## ⚠️ SUPERSEDED 2026-08-08 — THE REPLACEMENT COMPLETED, BUT NOT BY THIS PLAN
>
> **Read [ode-retirement-campaign.md](ode-retirement-campaign.md) instead.**
> `bdc02139` deleted the ODE backend. Newton with `SolverMuJoCo` is the only physics
> backend, so "complete ODE replacement" is done — **by deletion, not by finishing the
> coverage climb this document describes.** The distinction matters:
>
> - The W0–W7 coverage plan was **~60% complete** and its faithfulness figure was
>   **~35–40%** against a ≥99% bar, measured on 76 worlds out of a live 701. That
>   climb was never completed on the meter's terms.
> - What replaced it was a **load-and-step** corpus sweep (486 worlds, 444 PASS, zero
>   Newton-attributable failures). ⚠ **That is a robustness result, not a fidelity
>   result**, and the campaign proved how much such a sweep can miss: `tests/api`
>   scored 140/140 while all 140 worlds had gravity projected to exactly zero.
> - So the ≥99%-faithful question this plan existed to answer **was never answered**,
>   and with ODE gone there is no oracle left to answer it against.
>
> The non-negotiables below are also retired. In particular *"ODE remains the permanent
> one-switch fallback (`OMNISIM_LEGACY=1` / `OMNISIM_FORCE_ODE=1`) and the verification
> oracle"* was a **written promise that the retirement campaign deliberately broke**. It
> is preserved here as the record of what was promised, not as current policy.
>
> Everything below is the working record. Its coverage numbers are kept as written.
...
— body-space force injection (W3.1: propeller + supervisor, verified — ⛔ but it REGRESSED to a total no-op
between 2026-08-08 and 2026-08-22 and its revert-lever A/B could not see it; read the W3.1 bullet's correction
block before quoting any number from it) and velocity sets (W3.2, verified) both
...
**Where it lives:** [OmSolid.cpp](../../src/omnisim/nodes/OmSolid.cpp) `articulationNewtonCapable()` /
...
- **Tier A (correctness):** any joint whose `nodeType()` is not `WB_NODE_HINGE_JOINT` /
  `WB_NODE_SLIDER_JOINT` → ODE. (Deliberately `nodeType()`, not `dynamic_cast`, because `OmBallJoint` /
  `OmHinge2Joint` derive from `OmHingeJoint`.)
...
*Acceptance for the phase:* met — `OMNISIM_PROBE_GATE` (a post-finalize sweep in `OmWorld::finalize()`
...
| metric | value |
|---|---|
| worlds entirely Newton-eligible | **5 / 8** (omniquad, mavic, rosbot, + the two cobot-arm worlds) |
| mixed (some articulation ODE-gated) | 3 (panda, rosbot_xl, ur_arms) |
| top-level articulations Newton-**eligible** | **124 / 129 (~96%)** |
| capability-gate gaps | **mesh ×3 worlds** (omniquad, rosbot_xl, ur_arms) · **joint ×1** (panda's platform carts) |
...
2. **"Eligible" ≠ "faithful" — ~96% MUST NOT be read as "Newton already runs 96% of robots well."** The
   ...
   regardless of whether Newton *simulates it faithfully*. Worse, `omniquad` reads as "3/3 eligible" only
...
wheels, [OmSolid.cpp](../../src/omnisim/nodes/OmSolid.cpp) ~2788–2791), and **plane / mesh / heightfield are
absent**.
- **W1.1 Plane** (static half-space ground). ✅ **LANDED 2026-06-07** — the faithful-check (W5.4) finding:
  ...
  was wired. Wired `addShapePlane` → native `add_shape_plane` (local normal +Z, the `OmPlane` convention;
  the body's transform orients it); node-side detects a `OmPlane` boundingObject. **Verified:** panda's
...
- **W1.2 Native cylinder narrow-phase. ✅ LANDED 2026-06-08 (capsule fit; metric-neutral, honest).** Native
  ...
  onto the Webots cylinder's body-local-Y axis (verified in isolation: Z→(0,1,0)); OmSolid's cylinder branch
...
- **W1.3 Mesh / trimesh narrow-phase — THE single biggest unlock. ✅ LANDED 2026-06-07.** GO/NO-GO
  ...
  Implemented: `OmNewtonBackend::addShapeMesh` (marshals the flat vertex + triangle-index arrays as PyLists)
  → Python `add_shape_mesh` building a `newton.Mesh(compute_inertia=False, is_solid=True)`; the OmSolid
  AABB-box fallback in `attachNewtonShapeFromBoundingObject` REPLACED with real mesh registration (verts +
  indices from `OmTriangleMesh::coordinatesData()`/`indicesData()`); Tier-B mesh gate narrowed (mesh allowed
  ...
  worlds (100%) eligible, 124/129 → 135/135 articulations, 4 mesh+2 joint gaps → 0** (with W2); omniquad + panda
...
- **W1.4 Heightfield** (terrain worlds) — `add_shape_heightfield` exists natively too (same pattern as mesh).
  *Confirmed exercised: `UnevenTerrain.proto` and `husky_rocks_traverse.omniworld` both `boundingObject USE` an
...
- **W2.2 Ball joint** (3-DoF) — common in legged/soft rigs. ✅ **LANDED 2026-06-09.** Native Newton
  ...
  triple); `OmNewtonBackend::addJointBall` + the embedded-Python `add_joint_ball` queue → `_add_revolute_to_builder`
  `kind=="ball"` finalize branch; registered in `OmBasicJoint::flushPendingNewtonRegistrations` in a dedicated
  branch BEFORE the Hinge2 *and* Hinge casts (`OmBallJoint` is-a `OmHinge2Joint` is-a `OmHingeJoint`, so either
  cast would capture it and register only 1–2 of its 3 DoF — `nodeType()` discriminates exactly); gate Tier-A
  allow-list widened (`OmSolid::solidSubtreeNewtonCapable`). **PASSIVE**: a *motorised* ball still needs MuJoCo
  ...
  + `JointType.BALL` confirmed on the installed newton 1.2.0; (2) **coverage meter** — `oracle_mixedjoint.omniworld`
  3/3 + `ball_joint_vs_hinge_joints.omniworld` 9/9 Newton-eligible, **0 joint gaps** (panda 77/77, omniquad 3/3 unchanged
  → no regression from the new branch); (3) **behavioral** — `physics_oracle.py --gate --require-newton` on
  `oracle_mixedjoint.omniworld` (a Hinge→Ball chain): INV1/INV2/INV3 PASS (finite/settled/reproducible) and the
...
- **W2.3 Hinge2 / universal** (2-DoF) — steered+rolling wheels (cars). ✅ **LANDED 2026-06-07** (it was
  ...
  angular `JointDofConfig` axes (no phantom body); `OmNewtonBackend::addJointHinge2` + the Python
  `add_joint_hinge2` build branch; registered in `OmBasicJoint::flushPendingNewtonRegistrations` (a
  dedicated branch BEFORE the OmHingeJoint cast, since OmHinge2Joint is-a OmHingeJoint); gate Tier-A
...
Newton overrides ~11 of ~45 `OmPhysicsBackend` virtuals (reads + joint targets + reset) and inherits the
...
- **W3.1 Force/torque injection** (`addBodyForce/Torque`) — the headline. **✅ GO + CORE LANDED 2026-06-08.**
  **Go/no-go resolved GO, NOT class-E blocked:** Newton's `state.body_f` IS a writable per-body external
  ...
  the tick (ODE addBodyForce semantics); C++ `OmNewtonBackend::addBodyForce`; `OmSolid::applyExternalForceNewton`
  (force-at-world-pos + torque → COM wrench via the body's pose-readback origin); `OmPropeller` routes thrust
  ...
  fires for `propeller.omniworld`'s helicopters with `newtonIdx` 0/1/2; (3) the **decisive** supervisor fixture
  (W3.1 supervisor entry below): a +20 N push lifts a 1 kg Newton box to the analytic height, zero lift
  without injection. **HONEST CORRECTION to commit c9a28347:** that commit cited `propeller.omniworld` "force OFF →
  ...
  **ON == OFF == 0.102 m drift every time** — that demo's propellers thrust only ~2 mN (far too small to move
  the faithful number, which is a husky-like contact/settling offset), so `propeller.omniworld` is NOT a valid
  force-injection A/B. The routing still fires (diagnostic), but the supervisor fixture is the real proof.

  > ⛔ **ALL THREE OF THOSE VERIFICATIONS SILENTLY REGRESSED ON 2026-08-08 AND WERE ONLY REPAIRED ON
  > 2026-08-22. Read every W3.1 number in this bullet as measured on a pre-`bdc02139` binary.** The ODE
  > deletion left `OmSolidMerger::mBody` permanently `NULL` (set in the constructor, assigned nowhere),
  > and both `C_SUPERVISOR_NODE_ADD_FORCE` and `C_SUPERVISOR_NODE_ADD_FORCE_WITH_OFFSET` still tested
  > *that* handle rather than the Newton one — so they took their "can't be used with a kinematic Solid"
  > branch and dropped the wrench on bodies Newton had registered as DYNAMIC.
  > `OmPropeller::prePhysicsStep` carried the identical dead gate. Only `C_SUPERVISOR_NODE_ADD_TORQUE`
  > had been migrated, which is precisely why the regression stayed invisible: this paragraph, and the
  > comment then sitting in `OmSolid.cpp`, both generalised from the one verb that worked.
  >
  > **What that costs this document specifically.** The "decisive" fixture — *+20 N up on a 1 kg box
  > lifts it 0.50 → 5.44 m in 1.0 s WITH injection and not at all without it* — would, re-run any day in
  > that window, have measured **"not at all" in BOTH arms**, because the revert lever
  > `OMNISIM_NEWTON_NO_EXT_FORCE=1` was **inert**: `applyExternalForceNewton`, where the lever is read,
  > was never reached. A revert lever that cannot change the result is not an A/B, and neither arm of
  > this fixture would have said so. Verification (2), the `propeller.omniworld` routing diagnostic, was
  > dead for the same reason — `newtonIdx 0/1/2` never printed, because `OmPropeller` returned before the
  > call. Verification (1), the in-isolation `state.body_f` work, is unaffected: that measured Newton,
  > not our gate.
  >
  > **Re-measured on the repair** (OmniBench lane 4, machine `9722d23d12a3`, CPU `mj_step`, binary
  > `dd7ae9921b6629ba`): `phenomenon.supervisor_external_force` `broken → works` — 10 N on a 2 kg body
  > accelerates it at **5.000076 m/s²** against an analytic 5.0 (ratio 1.0000151) over 700 per-tick
  > applications, and the 5 cm-offset arm sweeps **9.388 rad**, so the moment survives too.
  > `device.propeller_thrust` `broken → works` at **10.000155 m/s²** against an analytic 10.0. The lever
  > is honest again, and that was checked rather than assumed: with `OMNISIM_NEWTON_NO_EXT_FORCE=1` the
  > three `propeller.omniworld` helicopters stay at their spawn pose for **15,360 steps (491.5 s of
  > simulated time)**, x/y/z unchanged to 3 decimals, and fly without it.
  >
  > **The generalisable lesson is the one this bullet already half-learned above.** A verification that
  > names a fixture but is not *executed* by anything cannot notice its own subject being deleted; and a
  > revert-hatch A/B is only evidence while the hatch sits on a live code path — when the feature dies
  > upstream of the hatch, both arms agree and the A/B reports a clean, meaningless PASS. What caught it
  > is an executed probe with a red arm (lane 4 `phenomenon.supervisor_external_force` /
  > `device.propeller_thrust`), not a re-reading of this document.

  *Note: the corpus `mavic_2_pro.omniworld` is `controller "<none>"` (URDF, no Propeller nodes) so its freefall is
  ...
  (`control.joint_f` at the joint's `qd_start` DoF, zero-and-rewrite each tick) + `OmNewtonBackend::setJointForce`
  + a `motor->userControl()` branch in `OmBasicJoint`'s Newton motor loop, then VERIFIED in-binary on a
...
- **W3.2 Mid-step velocity sets** (`setBodyLinearVel/AngularVel`). **✅ LANDED + VERIFIED 2026-06-08.** Newton
  ...
  tick force accumulator). `OmNewtonBackend::setBodyVel` + `OmSolid::setNewtonBodyVel`; `OmSolid::setLinear/
...
- **W4.2** Wire the damage tracker + ray/range sensors to Newton contacts. **Integration point found
  ...
  → **`OmSolid::extractContactPoints()`**, which iterates `world->odeContacts()`. **CORRECTED 2026-06-08 by
  ...
  *pure-Newton* bodies — a box-on-floor fixture polled `getContactPoints` and got **ODE_pts=0 vs native_pts=8**
  ...
  Newton bodies. **Done so far:** (a) ✅ C++ `getContacts(std::vector<OmNewtonContact>&)` accessor + struct +
  ...
  map (`OmWorld::findSolids()` per poll — no risky persistent state) + (c) ✅ `extractContactPoints` now ADDS
...
- **W5.4 The parity bar / faithful-match layer. ✅ FIRST CUT LANDED 2026-06-07.** The eligibility meter says
  ...
  (a per-step articulation-root world-position dump in `OmSolid::postPhysicsStep`, both solvers sampled at
...
- **W5.4 update — the robot/prop split. ✅ LANDED 2026-06-08.** The "first reading" above was *misleading*:
  ...
  So the meter now **classifies each root** (`OmSolid::postPhysicsStep` tags the probe line `A` for a
  `OmRobot`, `R` for a free-prop/static root — *not* "has a joint", which mis-tags an articulated prop like a
  ...
  **omniquad** is `NO DATA` (the known `omniquad.urdf <rest>` parse collapse → use `omniquad.classic.urdf`). *Honest
...
built, trainer unwritten); Atlas never deployed; learned residual ≈ a passenger on OmniQuad. Sequence **last**,
...
**OmniQuad deploy collapse — characterized 2026-06-08 (regression check, easy causes ruled out).** A headless
run of `omniquad_residual_deploy_newton.omniworld` (classic URDF) shows OmniQuad **spawns standing (z=0.70) and collapses to
...
inert for OmniQuad (0 cylinders → W1.2 inert; native contacts opt-in-off; probe read-only), and the native-config
run uses effectively-baseline physics yet still collapses; (b) NOT torque control — OmniQuad uses position control
...
`ke=800`/`kd=500`). Yet OmniQuad **still collapses to z≈0.11 at ke=8000** — i.e. the legs are at (nearly) the
...
the W6 OmniQuad gap is lateral/roll dynamic fidelity** — leg forces + foot-contact friction don't supply enough
...
verdict).** Decisive A/B: the SAME OmniQuad model (the May-22 Newton-dumped MJCF `C:\tmp\omniquad_newton_fixed.xml`,
...
walker, `projects/policies/research/tools/omniquad_newton_knob_sweep.py`) all fail identically: substeps 4, statics, base-guard on/off,
...
walk). Note G1 walks through the same bridge (different gains kp100/kd5, MJWARP), so it is a OmniQuad-shaped
...
W1's native mesh registration (1adbcb69, Jun 7 — the day before the collapse was observed) turned OmniQuad's 9
URDF collision MESHES into real MuJoCo geoms for the first time, and OmniQuad's chassis hull overlaps all four
...
SolverMuJoCo via its graph-coloring into contype/conaffinity → verified `nexclude=78` for OmniQuad's 13 bodies).
Opt-out: `OMNISIM_NEWTON_SELF_COLLISION=1`. Verified after the fix: OmniQuad STANDS 100% upright / no falls
...
[rl-current-state.md](rl-current-state.md) still records the **OmniQuad RL deploy as COLLAPSING under Newton**
...
PASS. Remaining W6 work is RE-TUNING, not a bug: every pre-Jun-7 OmniQuad walking policy/gait was implicitly
...
Newton dynamic-fidelity wall.** Root cause (fixed in `6a459f84`, [OmNewtonBackend.cpp](../../src/omnisim/physics/OmNewtonBackend.cpp)):
...
where the runtime is present**. With Newton actually initialized, **OmniQuad/Go2/B2 WALK on Newton-MuJoCo**
(OmniQuad +30 m, Go2 +66 m, B2 +95 m, 0 falls). The real "Newton drove it" signal is the
`[OmNewtonBackend] world finalised (solver=...)` log line — NOT `imports OK`. The W1 self-collision fix above
...
(opt-in fail-loud guard, `cfb11d06`): `OmLog::fatal` + non-zero exit when Newton init fails, instead of
silently degrading to ODE. *(Note: the [rl-current-state.md](rl-current-state.md) OmniQuad-deploy headline should
...
- **Catch silent fallback:** wire `OMNISIM_REQUIRE_NEWTON` (the `cfb11d06` fail-loud guard) into the
  ...
  `[OmNewtonBackend] world finalised (solver=...)` log line, not `imports OK`.
