...
[`scripts/dev/make_omnisim_header_forwarders.py`](../../scripts/dev/make_omnisim_header_forwarders.py)
(the `include/controller/*/webots/` shims replace upstream files in place, so they
carry the Cyberbotics notice) and `projects/omni_quest/tools/gen_*.py`.
