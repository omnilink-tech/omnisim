...
%OMNISIM_HOME%\msys64\mingw64\bin\omnisim-bin.exe path\to\world.wbt --stdout --stderr
```
...
- Individual COLLADA (`.dae`) meshes may crash Webots' mesh loader at world load. Most DAE files load fine (Husky uses many), but specific ones do not — TurtleBot3 waffle's `meshes/sensors/r200.dae` crashes the loader, while every other TB3 mesh loads. Strip the offending `<visual>` block from the URDF if you hit this. Reproduction: load `projects/samples/demos/worlds/turtlebot3_waffle_only.wbt` (no longer in tree — recreate by pointing `URDFRobot` at `turtlebot3_waffle.urdf` alone) and it crashes; remove the `<visual>` containing `r200.dae` and it loads.
...
Tensors below that threshold are silently dropped — Webots/ODE's `dMassSetParameters` is observed to crash with `ACCESS_VIOLATION` on values smaller than that even when the tensor satisfies positive-definiteness and the triangle inequality (Jackal wheels at 0.0013 are right on the threshold; many URDFs use 1e-9 placeholders on sensor-mount frames). Links that hit the clamp fall back to bounding-object-derived inertia.

Verified end-to-end: `OMNISIM_URDF_USE_INERTIA=1 python scripts/dev/headless_runner.py projects/samples/demos/worlds/showcase/jackal_drive.wbt --duration 20` drives the Jackal 125+ m with the chassis tensor in effect.

### `OMNISIM_URDF_USE_SENSORS=1` — emit Webots devices from `<gazebo><sensor>` and `<plugin>` blocks **(works for short runs; long-run crash is in Webots, not the importer)**

What it does: maps `<gazebo reference="LINK"><sensor type="imu/gps/camera/ray">` and legacy `<plugin filename="libhector_gazebo_ros_*.so">` patterns into `InertialUnit`/`Gyro`/`Accelerometer`/`GPS`/`Camera`/`Lidar` nodes. Devices are wrapped in a per-cluster carrier `Solid` (the pattern from `projects/samples/devices/worlds/imu.wbt`) which is emitted inline as a child of the sensor's URDF link. Carrier has a tiny `Box` bounding object + `Physics`; SolidMerger absorbs it into the parent body.
...
- **The stock `projects/samples/devices/worlds/imu.wbt` sample world — Cyberbotics' own hand-authored example with `Accelerometer`/`Gyro`/`Compass`/`InertialUnit` siblings inside a `Solid` — crashes at the same ~5-8 second mark with the same exit code**, both in `--mode=fast` and `--mode=realtime`. Reproduce with `python scripts/dev/headless_runner.py projects/samples/devices/worlds/imu.wbt --duration 25`.
...
- Plain `Robot { children [ Solid {} ] }` runs 10s+ clean. **The bug is specifically in the WbSolidDevice path**, not in nested-Solid handling.
- `PositionSensor` (extends `WbJointDevice`) and `Motor`/`LinearMotor` worlds run clean. The crash is exclusive to subclasses of `WbSolidDevice` (Accelerometer, Camera, GPS, IMU, Gyro, Compass, TouchSensor, Lidar, Display, Connector, Emitter, Receiver, LED, Speaker, Pen, Radar, Radio, RangeFinder, VacuumGripper, etc.).
...
- Code that runs per-step for `WbSolidDevice` but **not** for plain `WbSolid` or `WbJointDevice`. The most likely suspects are in `WbRobot::dispatchAnswer` / `writeAnswer` (foreach device → `d->writeAnswer(stream)` runs even when controller is `<none>` since the robot is still serializing state), the ODE space registration done by `WbSolid::createOdeObjects` for a device-Solid that has no physics body, and any sensor-update side path triggered by `WbSimulationCluster::step` for the dual-inheritance WbDevice+WbSolid intersection.
...
$proc = Start-Process -FilePath "msys64\mingw64\bin\omnisim-bin.exe" -ArgumentList "<world.wbt>","--mode=fast","--batch","--minimize" -PassThru
for ($i = 0; $i -lt 10; $i++) {
...
Verified: `python scripts/dev/headless_runner.py projects/samples/demos/worlds/showcase/turtlebot3_drive.wbt --duration 30` drives all three TB3 variants — burger 11+ m, waffle 6+ m, waffle_pi 4+ m, no crashes.
...
msys64\mingw64\bin\omnisim-bin.exe projects\samples\demos\worlds\showcase\jackal_drive.wbt --mode=realtime
```
