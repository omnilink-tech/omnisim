...
> **⚠ 2026-08-08 — THE MECHANISM THIS DOC DOCUMENTS IS DELETED.** `bdc02139` removed the
> vendored ODE library, and with it the **entire ODE per-contact wire path** described below:
> `dContactGeom.depth`, `OmOdeContact`, `odeNearCallback`, `dJointSetFeedback`, the disabled-ODE-proxy
> collision for Newton Solids, and `OmSimulationCluster`'s surface-parameter fill. The **native
> Newton contact source** (`OmNewtonBackend::getContacts()`) is now the **only** source of
> `ContactPoint.depth` and of contact impulse, and it is **default-ON** since 2026-08-07
> (`OMNISIM_NEWTON_NATIVE_CONTACTS=0` reverts). What survives unchanged and is still worth
> reading: the **wire-format layout** (the 40-byte `WbContactPoint`, the six-layer append, the
> consumer inventory, the atomicity hazard) — that contract is backend-independent and still
> governs any future change. What does **not** survive: every "ODE computes it, we just forward
> it" premise, and every ODE-side verification. Record:
> [ode-retirement-campaign.md](ode-retirement-campaign.md).

> **Update 2026-06-08 — read first.** This doc covers the **ODE** per-contact
> `depth` wire API (landed 2026-05-29, below; ⚠ **that path is deleted** — see the 2026-08-08
> banner above). Since then Newton grew a **native**
> contact source — `OmNewtonBackend::getContacts()`
> ([OmNewtonBackend.hpp:175](../../src/omnisim/physics/OmNewtonBackend.hpp)) — which
> feeds the same damage / contact-points supervisor lists when
> `OMNISIM_NEWTON_NATIVE_CONTACTS` is set (⚠ **default-ON since 2026-08-07**; `=0` reverts) (see
...
> the **MuJoCo** solver — ⚠ which is now the *only* solver (`94f04222` removed XPBD,
> `7b431e81` made `SolverMuJoCo` the default), so the XPBD caveat is historical. So treat §2's
> "ODE still runs collision for Newton bodies" as **superseded for ALL Solids** — ⚠ not just
> pure-Newton ones: since `bdc02139` there is no ODE collision pass at all, for any Solid.
...
carried the real ODE `dContactGeom.depth` per contact (⚠ **that producer is deleted**; the
field is now filled from the native Newton contact source). Landed as one atomic
...
stayed green. ⚠ **That smoke set no longer runs on ODE** (`bdc02139`): the verification above is
a dated 2026-05-29 result on a deleted backend and **cannot be re-run as stated**; re-basing
those three worlds on Newton is outstanding work, and until then the depth wire has no
green determinism gate. The original "NOT landed / coordinate carefully" caution below is
...
deltas. On ODE that proxy was fine (rolling/resting Δv ≈ 0) — ⚠ ODE is deleted, so that half of
the comparison is historical; on Newton the
...
> **⚠ 2026-08-08 — §2's MECHANISM IS OBSOLETE; kept as the design record.** The first two
> bullets below describe the deleted ODE collision path (`bdc02139`): `dContactGeom.depth`,
> `OmOdeContact`, `enableForContactPoint`, `odeNearCallback`, and above all *"Newton drives the
> pose; ODE still runs collision"* — **there is no ODE collision pass any more, for any Solid.**
> Depth and impulse now come from `OmNewtonBackend::getContacts()` (default-ON since
> 2026-08-07). The **third bullet onward is still current**: the wire is still unversioned and
> position-based, the 40-byte `WbContactPoint` layout still holds in three independent places,
> the `padding` slot must still not be reused, and the consumer list is still the recompile
> checklist. Read §2 for the *wire* facts, not the *producer* facts.

- `dContactGeom.depth` is computed by ODE and **already survives intact
  into `OmOdeContact`** (which stores the whole `dContactGeom` by value).
  It is simply dropped at `OmSolid::extractContactPoints`, which keeps only
  `cg.pos`.
...
- The wire protocol is **unversioned, position-based binary** (no
  ...
  - C++ serializer `OmSupervisorUtilities::pushContactPointsToStream`
    (writes `3 doubles + 1 int` per point).
  - C reader `src/controller/c/.../supervisor.c` (reads into a malloc'd
    `WbContactPoint[]`).
  - Python binding `lib/controller/python/omnisim/node.py`
...
| # | Layer | File | Change |
|---|---|---|---|
| 1 | extraction | `OmSolid.cpp::extractContactPoints` (~3067-3104) | alongside `mListOfContactPoints.append(v)`, append `cg.depth` to a new parallel `QVector<double> mContactPointDepths` (+ global variant); declare in `OmSolid.hpp` near `mListOfContactPoints`; reset in `resetContactPoints()`/`resetContactPointsAndSupportPolygon()`. Add `computedContactPointDepths(includeDescendants)` accessor. |
| 2 | serialize | `OmSupervisorUtilities.cpp::pushContactPointsToStream` (~2027-2044) | after `stream << (int)uniqueId()` in the per-point loop, append `stream << (double)depths.at(i);` |
...
| 5 | Python | `lib/controller/python/omnisim/node.py` (`ContactPoint`, `getContactPoints` ~24-33/169-177) | **alignment-sensitive** — `3 doubles + int + double` is NOT `'3did'` (the trailing double forces 8-byte alignment → a 4-byte gap after `node_id`). Use `'3dixxxxd'` OR (safer) a `ctypes.Structure` with `.from_buffer_copy` so the compiler's padding rules — not a hand-counted format — define the layout. Set `format_size = 40`. Expose `cp.depth`. |
...
(~882-905, mirroring `OmMotor::setupJointFeedback`), store the feedback on
`OmOdeContact`, and serialize `|fb->f1|` as a second new field. Heavier;
depth alone is enough for the damage threshold.
⚠ **2026-08-08: Layer 1b as written is DEAD.** `dJointSetFeedback`, `odeNearCallback` and
`OmOdeContact` were all deleted with the ODE library (`bdc02139`). Real contact impulse must now
come from the native Newton contact source, which populates `forceMag` under `SolverMuJoCo` —
i.e. it is a *readback plumbing* task on `OmNewtonBackend::getContacts()`, not an ODE
joint-feedback task.
...
- The C reader (`supervisor.c` ~1270) is **stream-based** (`request_read_double`
  ...
  The serializer (`OmSupervisorUtilities.cpp` ~2037) is likewise field-by-field;
...
- Engine `OmSolid` side: store `cg.depth` in parallel `QVector<double>
...
| `src/omnisim/core/OmLanguage.cpp` | engine (API registration only, not a wire consumer) | rebuilt with the engine anyway |
...
`projects/samples/demos/worlds/misc/contact_self_check.omniworld` +
...
> **⚠ 2026-08-08 — §7's EVIDENCE BASE WAS ODE, AND IT IS UNREPEATABLE.** Every validation in
> this section ran on the deleted backend: the `husky_head_on_ode.wbt` capture (the world no
> longer loads), the "provably unchanged on the ODE battlebot games" guarantee, the
> `depth OFF/ON` comparison table, and the closing note that depth mode was *"validated under ODE
> (depth populated) and confirmed inert under XPBD"*. All numbers below are **preserved as dated
> historical measurements**; none can be re-derived, and the depth path's only *positive*
> validation was the ODE one — so **the depth scoring mode is now effectively unvalidated on the
> shipping configuration** (native Newton contacts + `SolverMuJoCo`), which is exactly the
> "Newton+MuJoCo capture" that the last open item below says was never run. That capture is now
> the *only* way to validate it, not an optional extra.
...
- **`OMNISIM_DAMAGE_VEL_SMOOTH` not retired.** Under the default **XPBD** Newton
  ...
  L1**, recorded here, not an L4 edit to `OmNewtonBackend`.
