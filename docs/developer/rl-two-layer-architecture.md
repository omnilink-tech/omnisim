...
that generalize across the whole zoo (G1, Spot, Atlas, and any robot we
add next):
...
Spot found the stand-still local optimum (zero reward, zero risk) after 500 k
...
has little left to do.** Spot's hand-coded gait+IK+balance walks **+5.03 m**
...
> The Spot-walk result (§7.1) is the gentle version of that: the residual is a
...
|---|---|---|
| **Shadow** | The physics-free *kinematic gait model* — the ideal trajectory the robot should produce. Foot-space planning + closed-form IK; never falls. | `projects/policies/control/gait/g1_human_gait.py`, `spot_trot_gait.py` |
| **Ghost** | The shadow *displayed* as a translucent hologram robot (staticBase, no physics) beside the real one, phase-locked to its forward progress. A debugging/demo lens: the gap = the error the mimic must close. | `projects/policies/controllers/g1_ghost/`, `spot_ghost/` |
| **Mimic** | The controller on the *physical* robot that tracks the shadow: deterministic baseline + bounded learned residual. | `projects/policies/research/controllers/g1_walk_deploy/`, `spot_residual_deploy/` |
| **Baseline (Layer 1)** | The deterministic part of the mimic: shadow feedforward + IK + a *feedback* balance law. Ships on its own. | `projects/policies/control/g1_brain.py`, `spot_gait.py`+`spot_balance.py`+`spot_kinematics.py` |
...
Vanilla 500 k-step PPO on Spot converged to *stand still* — the zero-reward,
...
3. **No redundancy to absorb the error → passenger becomes saboteur.** This is
   the variable that decides *which* end of law (c) you land on. **Spot walking**
...
`g1_human_gait.py` / `spot_trot_gait.py` already do this: each ships
...
Spot composes at the **foot-target** level (`foot = shadow + balance_dz +
...
- **Tight bound.** Keep `ACT_SCALE` / `RES_SCALE` small (Spot ±3 cm; for G1 the
...
- **Anchor 3 — a KL-to-zero / L2-on-residual penalty** so the policy provably
  ...
  [spot-residual-rl.md](spot-residual-rl.md); add it.)
...
- **Position-mode is the default deploy path; a torque sink now EXISTS but is
  opt-in (updated 2026-06-19).** By default `WbNewtonBackend` writes only
...
- **Deploy environment (required).**
  `OMNISIM_NEWTON_FORCE_MUJOCO=1` (use `SolverMuJoCo`, not XPBD),
  `OMNISIM_NEWTON_MJWARP=1` (GPU mjwarp, matches the trainer),
  `OMNISIM_NEWTON_STATICS=1`, `OMNISIM_NEWTON_SUBSTEPS=4` (a single 16 ms contact
...
| **Spot** (12-DOF quadruped) | `spot_trot_gait` (the g1_human_gait port) | `spot_gait` + `spot_balance` + `spot_kinematics` + heading-hold + recovery FSM | ±3 cm foot-offset residual | Walks under ODE; under Newton the residual is a **passenger** (§7.1). Reference implementation of the contract |
...
### 7.1 Spot in detail — the reference, and its honest gap

Spot is the cleanest, oldest instance of the contract and is worth studying
first ([`spot_residual_deploy.py`](../../projects/policies/research/controllers/spot_residual_deploy/spot_residual_deploy.py)):
...
**The conformance gap (the same as G1's):** there are two Spot gait paths. The
**canonical** one — `spot_trot_gait` (a direct port of `g1_human_gait`, single
numpy+torch source) — is used by the **ghost** and the **GPU trainer**. The
**shipped Newton deploy** still rides the older ODE-tuned `control/spot_gait` +
heading-hold, because the GPU-trained residual **does not transfer to Newton**
(`spot_gpu_residual_deploy`) — the same MuJoCo→Newton engine gap that flipped
G1's distilled walker. Fully converging Spot's deploy onto `spot_trot_gait` +
an in-solver-trained residual is **gated on closing that gap** (§8); until then
the deploy keeps the working ODE-tuned baseline, and the ghost shows the
canonical shadow. This is documented, not hidden.
...
`SolverMuJoCo.step()` (MuJoCo wrapped in Newton's XPBD articulation, LCP rigid
contacts + ~1-tick control latency + gain-scaling drift) **differ enough to
flip a marginal biped.** Spot (4 contacts, statically stable) absorbs the drift
...
2. **Render the ghost** in `controllers/<robot>_ghost/`: mirror
   [`g1_ghost.py`](../../projects/policies/controllers/g1_ghost/g1_ghost.py) — hologram,
...
[`spot_trot_gait.py`](../../projects/policies/control/gait/spot_trot_gait.py) ·
ghosts — [`g1_ghost.py`](../../projects/policies/controllers/g1_ghost/g1_ghost.py),
[`spot_ghost.py`](../../projects/policies/research/controllers/spot_ghost/spot_ghost.py) ·
baselines — [`g1_brain.py`](../../projects/policies/control/g1_brain.py),
[`spot_gait.py`](../../projects/policies/control/spot_gait.py),
[`spot_balance.py`](../../projects/policies/control/spot_balance.py) ·
...
[`spot_residual_deploy.py`](../../projects/policies/research/controllers/spot_residual_deploy/spot_residual_deploy.py) ·
