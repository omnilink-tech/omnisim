...
```
                ┌─────────────┐
                │   CREATED   │  WbController constructed, not yet started
                └──────┬──────┘
                       │ start()
                       ▼
         ┌─────────────────────────────┐
...
### WbController (`src/omnisim/control/WbController.*`)
...
### WbControlledWorld (`src/omnisim/control/WbControlledWorld.*`)
...
## Controller Lists in WbControlledWorld
...
3. Simulation advances by the world's `basicTimeStep` via `WbControlledWorld::step()`
...
## Step Execution Flow in WbControlledWorld::step()
...
4. Call WbSimulationWorld::step() to advance physics
...
- Socket path: `$WEBOTS_TMP/ipc/<robot_encoded_name>/intern` or `extern`
...
- The `<simulatorPid>` nonce on the intern name (exported to the child as
  `OMNISIM_IPC_NONCE`, appended on both sides — `WbController::start` and libController's
...
- Connected via `WbTcpServer` on the configured port (default: 1234)
...
| File | Role |
|------|------|
| `src/omnisim/control/WbController.*` | Single controller lifecycle and protocol |
| `src/omnisim/control/WbControlledWorld.*` | Multi-controller orchestration and step sync |
...
| `src/controller/launcher/webots_controller.c` | Standalone extern controller launcher |
| `src/omnisim/gui/WbTcpServer.*` | TCP server for remote extern controllers |
