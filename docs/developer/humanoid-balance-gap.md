...
why the Spot residual recipe doesn't port to bipeds out of the box — but
...
blocks the model+residual recipe from porting from Spot to humanoids.
...
The model+residual RL recipe ([spot-residual-rl.md](spot-residual-rl.md))
assumes the analytical layer **almost walks at zero policy.** For Spot,
that's true: gait engine + IK + balance PD walks straight 5 m in 30 s
without any neural network. The policy adds ~5% refinement.

For Spot this is trivial because trot has 2 feet on the ground at all
...
(Spot doesn't hit this Newton bug because its URDF defines feet as
...
- **PPO residual on top of walking** — should be ~1 hour of plumbing
  once the analytical stack works, mirroring the Spot residual stack.
...
| G1 (23-DOF) | ✓ (foot patched) | ✓ | g1_smoke.wbt, g1_model_walk_newton_demo.wbt, g1_pose_sweep_newton.wbt | ✗ (active balance needed) | ✗ |
...
- Not the Newton physics — the body responds to commands, joints
  ...
  for Spot's push-recovery experiment).
