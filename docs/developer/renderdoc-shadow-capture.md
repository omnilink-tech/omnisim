...
**Why this doc exists:** the wgpu **cast-shadow** render (`OmWgpuRenderTarget::clearAndDrawSceneTexturedShadowed`)
...
2. **Executable**: `msys64\mingw64\bin\omnisim-bin.exe`
   ...
   **Command-line args**: `"projects\robots\omnisim\omniquad\worlds\omniquad.omniworld" --mode=pause`
...
2. **In-app capture harness** (in `OmWgpuView::runSelfCheck`, gated by `OMNISIM_RDC_CAPTURE`, compiled only when
