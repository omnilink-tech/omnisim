...
> `WbNewtonBackend::addStaticBody`
> ([WbNewtonBackend.hpp:124](../../src/omnisim/physics/WbNewtonBackend.hpp)),
...
Reading `WbSolid::flushPendingNewtonRegistrations` (the gate that
...
### 4.1 — Python helper (`WbNewtonBackend.cpp::kNewtonRuntimeSource`)
...
### 4.2 — C++ side (`WbNewtonBackend.cpp/.hpp`)
...
### 4.3 — Dispatch on `WbSolid::flushPendingNewtonRegistrations`
...
- ✅ `add_static_body` Python helper in `WbNewtonBackend.cpp::kNewtonRuntimeSource`
  (builder.add_link with mass=0 + zero inertia; reuses the existing
  `add_shape_*` methods to attach geometry).
- ✅ `WbNewtonBackend::addStaticBody(x, y, z, qx, qy, qz, qw)` C++
  binding + matching NEWTON=OFF stub.
- ✅ Newton-direct smoke probe at
  [`scripts/xpbd_probes/probe_p8_static_body_smoke.py`](../../scripts/xpbd_probes/probe_p8_static_body_smoke.py)
  — builds the load-bearing geometry directly via the underlying
...
assumption is sound; the P8.2 WbSolid dispatch can layer on top.

**Deferred from this slice:** an in-OmniSim .wbt smoke world.
Without P8.2's WbSolid dispatch there's no way for a .wbt to opt a
Solid into the static-body path — the Newton helper is only
reachable through `WbSolid::flushPendingNewtonRegistrations`. The
Newton-direct probe covers the technical-bet half of "smoke"; the
OmniSim-world half rides along with P8.2.

### P8.2 — WbSolid dispatch for static Solids (1-2 weeks)
...
- New `WbSolid::mNewtonBodyIsStatic` flag: static bodies skip the
...
- **Gated opt-in via `OMNISIM_NEWTON_STATICS`.** Default-off keeps
  ...
  is met.
- New smoke world
  [`newton_static_collider_smoke.wbt`](../../projects/samples/demos/worlds/physics/newton_static_collider_smoke.wbt)
...
resolved; `newton_husky_head_on.wbt`, the arena is a no-op static here
...
anywhere in `WbSimulationCluster.cpp`. The actual mechanism:

- A Newton-backed Solid keeps its ODE `dBody` + geom, but the body is
  **artificially disabled** (`WbSolidMerger::setBodyArtificiallyDisabled(true)`
...
- The only real per-step ODE work is the single fused
  ...
  `WbSimulationCluster::step()`, which does (a) collision broad+narrow
...
| URDF "fixed-joint decoration" semantics drift | Spot's hip_motor / shoulder_motor links currently merge into the leg; if P8 registers them as separate Newton statics they become collidable, which may add unwanted self-collisions | P8.2 keeps the `isFixedChild` check intact for inside-robot statics; only top-level scene statics (arena floor/walls/obstacles) become Newton statics |
...
- **Mesh collision for statics.** URDF static meshes (Spot's terrain
...
|---|---|---|
| `newton_husky_smoke_test.wbt` | works | works (no behavioural change; ground was already on Newton as `addGroundPlane`) |
| `newton_husky_head_on_damage.wbt` (4v4) | works through bridge | works without bridge; damage events parity within rounding |
| `newton_husky_20v20.wbt` | works through bridge | works without bridge; ms/step drops by ≥ 5% |
| `husky_head_on.wbt` (ODE) | works (pure ODE) | works (unchanged; no Newton-backed Solids) |
| `husky_damage_arena.wbt` (ODE) | works | works |
| Mixed (`mixed_husky_10.wbt`: 9 Newton + 1 ODE) | works | works through bridge (the 1 ODE husky still needs proxy contacts against Newton statics, which is the bridge's job) |
...
- P8.2 WbSolid dispatch: 1-2 weeks (the hard part; matching Newton
