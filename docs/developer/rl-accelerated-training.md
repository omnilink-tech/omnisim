...
python projects/policies/research/training/train_omniquad.py --backend list
```

## sb3 backend — the default

Uses our existing OmniSim subprocess env (`OmniQuadEnv` in
`projects/policies/research/envs/omniquad_env.py`). One OmniSim process per training env,
TCP/IP between them. Works everywhere SB3 works (i.e. any machine
with Python). Slow but always-correct baseline.

```bash
python projects/policies/research/training/train_omniquad.py --backend sb3 \
    --envs 4 --steps 500000
```
...
python projects/policies/research/training/train_omniquad.py --backend mjx \
...
OMNIQUAD_R_LIN_VEL_WT=2.0 \
OMNIQUAD_R_ALIVE_BONUS=0.02 \
python projects/policies/research/training/train_omniquad.py --backend mjx --envs 1024 ...
```
...
* `<dynamics damping="..."/>` on each joint (~0.05 N·m·s/rad for
  OmniQuad-class motors) — RL discovers self-oscillation exploits without
  it;
* realistic `effort` and `velocity` limits on each joint — the policy
  learns around real actuator constraints.

The OmniQuad URDF demonstrates all three; see commit `6333f075` and
`projects/policies/research/tools/patch_omniquad_urdf_for_rl.py` for the exact edits.
...
ROBOTS = {
    "omniquad":     ("projects.policies.research.backends.omniquad_robot_spec", "OMNIQUAD"),
...
OMNISIM_POLICY_ONNX=projects/policies/research/inference/policies/my_biped_v1/policy.onnx \
   omnisim-bin.exe projects/policies/research/worlds/rl_deploy.omniworld
```

The generic `rl_deploy` world has OmniQuad as its default URDF; for your
robot, copy `rl_deploy.omniworld` to `my_biped_rl_deploy.wbt` and swap the
...
```bash
OMNIQUAD_R_LIN_VEL_WT=2.0 OMNIQUAD_R_ALIVE_BONUS=0.02 \
...
|---|---|
| `sb3`   | Currently OmniQuad-only (gym env is OmniQuad-specific). Generalising means copying `omniquad_env.py` per robot — `~half-day` per robot. |
| `mjx`   | Fully robot-agnostic via `RobotSpec` + `obs_recipe`. No code edits needed. |
| `isaac` | Currently scaffolded for legged_gym-style OmniQuad. Other robots need an Isaac task definition tweak in `isaac_remote.py`. |
...
imported from `omniquad_robot_spec.py` or your robot's equivalent.
