...
numbers on our four-legged walkers (OmniQuad/Go2/B2), which all walk stably under Shadowing.

## What the pipeline already does, and the one lever it lacks

The quadruped walk trainers (`gpu_mjwarp_{omniquad,go2,b2}_walk_trainer.py`) already implement **two of
...
An opt-in **ghost-tube ET** in `gpu_mjwarp_omniquad_walk_trainer.py` (default-off ⇒ byte-identical),
...
- `--tube-prog 0.8` — **forward-progress lag tube**: integrate the velocity deficit `(v* − vx)` after
  ...
  | quantity (OmniQuad bare gait) | healthy p99 | healthy max | fall gate |
  |---|---|---|---|
...
## Measured results — OmniQuad walk, 3 seeds, from-scratch, 17.2M env-steps (RTX 5070 Ti)
...
The prototype was opt-in flags on `gpu_mjwarp_omniquad_walk_trainer.py` (`--tube-rp/--tube-prog/--tube-q/
