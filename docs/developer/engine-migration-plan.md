...
canonical renderer, with the legacy ODE+WREN stack remaining as a fallback
forever. Both arms move on the same sequencing because the rendering
...
|---|---|---|---|
| 2-husky head-on physics | 98 ms/step | < 5 ms/step | **~20×** |
...
- P1.6 (joint-op widening) **complete** 2026-05-28 across 7 commits
  (`750d84f9` → `27be352a`). 17 virtuals on `WbPhysicsBackend` cover
...
Phase γ  (scaffolds landed 2026-05-28; ✅ RUNTIME-VERIFIED 2026-05-29
   ...
   Rendering: R3.1 ✓ wgpu-native init + isAvailable() probe
              ...
              R3.4 ✓ shader port — steps 1+2+3+4+5+5b
                    ...
                    = step-4 scene-walk); camera_wgpu_scene_smoke.wbt
...
Phase δ  (mostly code-landed 2026-05-28; end-to-end demo outstanding)
   ...
   Outstanding: real Newton bodies driving the wgpu storage buffer
              ...
              2026-05-29:** the WbCamera δ hook (`OMNISIM_PROBE_DELTA=1`)
              snapshots the live husky bodies and draws them via
              `clearAndDrawInstanced` — `[WbCamera δ] 5 LIVE Newton bodies
...
Phase ε  (3–6 months)
   Rendering: R4 (main viewport on wgpu)
              ...
              ── **R5 RangeFinder (depth) — ✅ IMPLEMENTED + RUNTIME-VERIFIED
                 ...
                 (reuses the lit bind-group layout + depth view); WbCamera
              ...
              ── **R5b RangeFinder (REAL-METERS / R32Float) — ✅ IMPLEMENTED +
                 ...
                 float *outMeters)`; WbCamera `OMNISIM_CAMERA_DEPTH=2` drives it
              ...
              ── **R5c real WbRangeFinder DEVICE node — ✅ IMPLEMENTED +
                 ...
                 shared `WbWgpuSceneRenderer` module (nodes/) holding the scene
                 walk + viewProj + target-management factored out of WbCamera:
                 `ensureTarget()`, `collectWorldDraws()`, `buildViewProj()`.
                 `WbRangeFinder::copyImageToMemoryMappedFile` (opt-in
                 ...
                 `WbCamera` is untouched (its object is bit-for-bit the R5b build).
              ── **R5c-follow-up: WbCamera migrated onto WbWgpuSceneRenderer —
                 ✅ DONE + RE-VERIFIED 2026-05-29.** The temporary duplication is
                 gone: WbCamera::ensureWgpuTarget now delegates to
                 `WbWgpuSceneRenderer::ensureTarget`, and its scene walk + viewProj
                 are `collectWorldDraws` + `buildViewProj`. The Camera-specific
                 bits stay in WbCamera (δ probe, DEPTH=1/2 modes, lightDirAmbient,
              ...
              ── **R5d Lidar RADIAL-range foundation — ✅ IMPLEMENTED +
                 ...
                 `WbWgpuRenderTarget::{ensureSceneRangeF32Pipeline,
                 clearAndDrawSceneRangeF32}` (renders into the same R32Float
                 target), and `WbWgpuSceneRenderer::buildView` (the view matrix on
                 its own; `buildViewProj` is now `perspective * buildView`).
                 WbCamera `OMNISIM_CAMERA_DEPTH=3` drives it. **Verified** on
                 ...
                 can't span ~180°+), plus the WbLidar device-node wiring.
                 ── WbLidar SCOPING (read 2026-05-29, confirms it's multi-session,
                    NOT a RangeFinder-style drop-in): WbLidar does **not** override
                    ...
                    then Recognition.
                 ── WbLidar IMPLEMENTATION SPEC (read copyAllLayersToMemoryMapped-
              ...
              ── **R5f WbLidar MULTI-LAYER — ✅ IMPLEMENTED + RUNTIME-VERIFIED
                 ...
                 **above_hits=3 > below_hits=1** (the box-extends-up asymmetry,
                 ...
                 heads + tilt.
              ── **R5g WbLidar WIDE-FOV (multi-frustum) + azimuth-sign fix — ✅
                 ...
                 (`camK = worldMatrix · WbMatrix4(0,0,0, 0,0,1, θ_centre_k)`) and
                 ...
                 rotating heads + tilt.
              ── **R5h WbLidar MULTI-LAYER × WIDE-FOV — ✅ IMPLEMENTED +
              ...
              ── **R5j WbLidar ROTATING HEAD (single-layer) — ✅ IMPLEMENTED +
              ...
              ── **R5k WbLidar MULTI-LAYER ROTATING — ✅ IMPLEMENTED + VERIFIED
                 ...
                 PASS.
              ── **R5l WbLidar WIDE-FOV ROTATING — ✅ IMPLEMENTED + VERIFIED
...
1. **`OMNISIM_WITH_NEWTON=OFF` and `OMNISIM_WITH_VULKAN=OFF` builds
   ...
   ζ cleanup.)
2. **Defaults preserve behavior.** `physicsBackend` defaults to `"ode"`.
   `renderBackend` defaults to `"wren"`. Worlds without these fields
   are byte-equivalent across phases until Phase D (physics) or Phase ζ
   (rendering) explicitly flip them.
3. **Per-leaf dispatch.** Migration is opt-in *per-Solid* and *per-
   ...
   Newton.
4. **Runtime fall-through.** If a world asks for a backend that isn't
   available (no CUDA, no Vulkan, library missing), the registry
   silently falls back to the canonical path with one warning per
   world.
...
>   no attribute 'write'`, the `WbNewtonBackend` ctor caught it, and the engine
...
>   [WbNewtonBackend.cpp](../../src/omnisim/physics/WbNewtonBackend.cpp). **The
>   real signal that Newton actually drove a world is the
>   `[WbNewtonBackend] world finalised (solver=…)` log line — NOT the earlier
>   `… imports OK` line, which prints even on a fallback.**
> - **`OMNISIM_REQUIRE_NEWTON` (`cfb11d06`) — new deploy/CI assert.** Opt-in
>   (default off); when set and Newton init fails, the engine `WbLog::fatal`s (FATAL
...
>   the earlier "Spot collapses on Newton" reading was largely the *fallback*, not
>   a physics wall: **quadrupeds WALK on Newton-MuJoCo** — Spot +30 m, Go2 +66 m,
...
- Runtime defaults **unchanged on purpose**: `renderBackend` defaults to
  `"wren"` ([WbRenderBackend.cpp](../../src/omnisim/render/WbRenderBackend.cpp));
  `physicsBackend` defaults to `"auto"`, which resolves to **Newton when the
  warp/newton runtime is importable, else ODE**
  ([WbPhysicsBackend.cpp](../../src/omnisim/physics/WbPhysicsBackend.cpp)). The
...
- **Main viewport (R4) is no longer 0% — the audit's "biggest gap" is closed to
  "functional and gate-passing, not parity-complete."** The live `WbView3D`
  frame dispatches `renderMainFrameViaWgpu`
  ([WbView3D.cpp:1509](../../src/omnisim/gui/WbView3D.cpp),
  [:1542](../../src/omnisim/gui/WbView3D.cpp)) — an offscreen wgpu render
  ...
  **deterministic** city within-tol is **~37%** against a **30%** advisory floor;
  ...
  ([WbWgpuSurface.cpp](../../src/omnisim/render/WbWgpuSurface.cpp): create /
  configure / getCurrentTexture / present; picking + selection + overlays in
  [WbWgpuView.cpp](../../src/omnisim/gui/WbWgpuView.cpp)).
...
- Newton native **collision shapes** now cover sphere / box / cylinder
  ...
  [WbNewtonBackend.hpp](../../src/omnisim/physics/WbNewtonBackend.hpp). A static-base
...
- **Native contact readback is built + wired** (reverses the audit's "real
  ...
  ([WbNewtonBackend.hpp:175](../../src/omnisim/physics/WbNewtonBackend.hpp))
  snapshots the narrow-phase cache and feeds the damage / contact-points
  supervisor lists ([WbSolid.cpp](../../src/omnisim/nodes/WbSolid.cpp), gated by
...
- **Measured coverage** (rigorous, from `scripts/dev/newton_coverage.py` +
  ...
  uncontrolled drone, spot hits the `spot.urdf <rest>` parse collapse). The
  ...
  its own index API
  ([WbNewtonBackend.hpp:287-330](../../src/omnisim/physics/WbNewtonBackend.hpp)).
  **Legged standing still falls ~t=1.55s** and needs `STATICS=1` / `SUBSTEPS=4`
  ...
  Newton-MuJoCo deploys: quadrupeds (Spot/Go2/B2) **walk** 0-falls; humanoids
...
- **Default engines (the load-bearing reality):** on a stock build, *zero*
  ...
  defaults to `"wren"` ([WbRenderBackend.cpp:26-28](../../src/omnisim/render/WbRenderBackend.cpp)); `physicsBackend` schema-defaults to
...
- **Main viewport (R4) = 0% — the biggest gap.** [`WbView3D.cpp`](../../src/omnisim/gui/WbView3D.cpp) (the actual 3D
  view) contains no wgpu/swapchain/surface/present code; there is no on-screen
  presentation path anywhere in `render/` — `WbWgpuRenderTarget` is offscreen
...
- **Fidelity (Tier 1-5) ≈ 5% in-engine (was 0% at audit time; T1.1 AgX now
  ...
  wired + runtime-verified** — `kSolidLitAgX` in [`WbWgpuShaders.cpp`](../../src/omnisim/render/WbWgpuShaders.cpp)
  (3×3 inset → log2 → 6th-order contrast → outset, ported from the preview),
  driven by `WbWgpuRenderTarget::ensureSceneAgxPipeline()` + a default-false
  ...
  **Still 0% in-engine:** T1.2 CSM + T1.4 TAA (those two remain WebGL2 HTML
  ...
  - **T1.1 follow-up — emissive HDR source DONE (commit b0a2e8d5):** the
    ...
    harvested in `collectShapeDraws` into `WbWgpuSolidDraw.emissiveRGB`, packed
  ...
  - **T1.1 follow-up — specular highlight DONE (commits 717fc735 → 55182c7d):**
    ...
    (V from camPos−worldPos); WbCamera passes camPos; A/B demo worlds + guard.
...
- **Testing/parity is largely manual + not gated.** Golden-image harness exists
  ...
  *magenta-clear* image — it would not catch a geometry/shading regression. No
  ...
  and the shared `WbWgpuSceneRenderer` / `WbAbstractCamera` refactor now feeding
...
on-screen path (needs a wgpu surface/swapchain + `WbView3D` integration — the
...
work. Verified by reading the code (`WbCamera.cpp`, byte-identical to pre-session
`196547b2`):

- **Object recognition is renderer-agnostic CPU geometry.**
  `computeRecognizedObjects()` ([WbCamera.cpp:1013](../../src/omnisim/nodes/WbCamera.cpp))
...
- **Only segmentation (the per-pixel object-ID image) is renderer-coupled**, via
  a second `WbWrenCamera` (1218/1400). That is the genuine wgpu gap, and §7
...
| Newton API churn (young codebase) | Breaking changes between Newton releases | Pin to specific Newton release; bump deliberately; the dispatcher discipline isolates churn to `WbNewtonBackend` |
...
regression and hardware-less CI need it. WREN remains a shipping
...
- ✅ **Newton silent-ODE-fallback INIT BUG — FIXED 2026-06-23 (`6a459f84`); the
  ...
  the `WbNewtonBackend` ctor caught it, and the engine **silently fell back to
  ODE** — so Newton-tuned worlds collapsed for reasons that masqueraded as physics
  regressions (this accounts for much of the earlier "Spot collapses on Newton"
  history). Fixed via a writable-stdio guard installed *before* the warp import in
  `WbNewtonBackend.cpp`. **Newton now reliably ENGAGES wherever the runtime is
  present.** Two corollaries: (a) the real "Newton actually drove this world"
  signal is the `[WbNewtonBackend] world finalised (solver=…)` log line — **NOT**
  ...
  **`OMNISIM_REQUIRE_NEWTON`** (`cfb11d06`, opt-in/default-off) turns an init
  failure into a `WbLog::fatal` (FATAL + non-zero exit) instead of a silent ODE
  ...
  quadrupeds **walk** 0-falls (Spot +30 m, Go2 +66 m, B2 +95 m); humanoids **stand**
...
- ✅ **Phase-D hybrid-backend-split regression — FIXED 2026-06-01**
  (`WbSolid::effectivePhysicsBackendName`): the third independent fallout
  ...
  *default* `"auto"` overrode the robot's *explicit* `"ode"`. Result: an
  ...
  Spot's hip_y + knee, but not hip_x) registered as Newton bodies/joints.
  ODE actuated those joints correctly, but `WbHingeJoint::postPhysicsStep`
  ...
  canonical ODE residual walker (`spot_residual_deploy.wbt`,
  ...
  restores the documented `WbUrdfImporter` inheritance contract
...
- **Physics arm**: P1.6 ✅, P5 ✅ (20v20 load + finalize verified
  ...
  the canonical `newton_husky_smoke_test.wbt` husky now DRIVES
  ...
  *dynamic* Newton root bodies; their per-step `changed` pose signal
  ...
  Solids in `WbSolid::flushPendingNewtonRegistrations`
...
- ✅ **Phase-D vs test-matrix regression — FIXED 2026-05-29** (`c92d81e4`):
  ...
  **`OMNISIM_FORCE_ODE`** env knob — `WbPhysicsBackendRegistry::resolve()`
...
- ⚠️ **Build hygiene 2026-05-29**: the in-tree `omnisim-bin.exe` was
  ...
  `0/500`. Always confirm `nm WbNewtonBackend.o | grep snapshotBody`
...
- **Rendering arm**: **WREN gets bug fixes only** (locked 2026-05-28,
  ...
  **wgpu build tooling REPAIRED + γ reproducibly RE-VERIFIED 2026-05-29:**
  ...
  `wgpu-native init OK`, RTT `clearAndRead PASS`, and `camera_wgpu_scene_smoke.wbt`
  ...
  failed build sequence), NOT a real bug. `WbCamera::ensureWgpuTarget()` guards
...
1. **ke/kd wiring** (`46f2d9ba`, 2026-05-28): env defaults 20/3
   ...
   WbBasicJoint. Correct + necessary, but **not sufficient** — the
...
2. **Static furniture as dynamic Newton bodies** (the real freeze):
   ...
   `WbSolid::syncNewtonPoseFromFields` supervisor-write bridge fired
   ...
   **Fix**: `WbSolid::flushPendingNewtonRegistrations` now skips Solids
...
`newton_husky_smoke_test.wbt` chassis drives -7.0 → +1.09 m at
...
✅ FIXED (`2ea86d3f`):** `WbBasicJoint::flushPendingNewtonRegistrations`
...
husky/jackal/rover/Spot/Atlas unaffected. <br>(2) **floor-contact
...
Newton path:** the canonical `newton_husky_smoke_test.wbt` chassis trace is
...
re-eval'd: the G1/Spot/Atlas deploy *policies*, which are sensitive to deploy
...
```
                +-----------------------------------+
                |       WbApplication / WbWorld     |
                +---+---------------------------+---+
                    |                           |
                    |  per-step physics tick    |
                    v                           v
            +-------------------+      +-------------------+
            |   WbOdeBackend    |      |  WbNewtonBackend  |
...
**Abstract surface** (`WbPhysicsBackend`): 24 virtual methods grouped
into Pose / Force / Sleep / Config families. Each takes an opaque
`WbBodyHandle` (`void *`). `WbOdeBackend` reinterprets as `dBodyID`;
`WbNewtonBackend` packs as `(void*)(uintptr_t)(idx+1)` so 0 is reserved
...
| P1 / P1.5 | ✅ COMPLETE | `WbPhysicsBackend` abstraction + 24-virtual body-op widening; 12 ODE-dispatched files; `WbNewtonBackend` 5 read overrides + sensor/supervisor write-path dispatch (warn-and-skip Newton-backed). |
| P1.6 | ✅ COMPLETE | Joint-op widening (slices 1–7 landed 2026-05-28; `750d84f9` → `27be352a`, with closure doc `0b6699c1`). Final surface: `WbJointHandle` opaque-pointer typedef + `WbJointParam` enum (8 entries: FMax / Velocity / LoStop / HiStop / StopCFM / StopERP / SuspensionERP / SuspensionCFM) + **17 virtuals** on `WbPhysicsBackend`. Reads: `getJointHingeAngle/Rate`, `getJointSliderPosition`, `getJointAMotorAngle/Rate` (5). Per-step write torque/force: `addJointHingeTorque`, `addJointSliderForce`, `addJointAMotorTorques`, `addJointHinge2Torques` (4). setParam family: `setJointHingeParam`, `setJointSliderParam`, `setJointAMotorParam(axis,…)`, `setJointLMotorParam(axis,…)`, `setJointHinge2Param(axis,…)`, `setJointBallParam(axis,…)` (6). Lifecycle: `setJointEnabled`, `isJointEnabled` (2). ODE overrides forward to the matching `dJoint*` calls on dJointID-packed handles; `WbJointParam → dParam*` mapping lives in `WbOdeBackend.cpp` to keep the abstract header ODE-free. Multi-axis joints (AMotor / LMotor / Hinge2 / Ball) use ODE's `dParamGroup = 0x100` offset for axis indexing. Newton overrides `getJointHingeAngle` (routes to existing integer-index `getJointAngle(idx)`); everything else inherits Newton's -1 default. Every per-tick + world-load joint callsite across `WbHingeJoint`, `WbSliderJoint`, `WbBallJoint`, `WbHinge2Joint`, `WbBasicJoint`, `WbRotationalMotor`, `WbLinearMotor`, `WbSolid` routes through the dispatcher. Smoke verified per slice on Newton + Hinge + Slider + Ball + Hinge2 worlds, 0 errors each. <br><br>**Explicitly carved out** (genuinely ODE-internal, not "joint-op widening"; can stay direct or grow into a separate "P1.7 lifecycle widening" if needed): joint feedback buffer pattern (`dJointGetFeedback`/`SetFeedback` with the ODE-specific `dJointFeedback` struct), lifecycle (`dJointCreate*`/`Destroy`/`Attach`/`Group*` — Newton handles via its own `addJointRevolute` API, not through this dispatcher), world-load anchor/axis setup, ODE-internal spring/damper companion-joint mechanics (`dJointSetAMotorAngle` in spring/damper paths — no Newton equivalent since Newton integrates spring/damper through the solver, no companion needed), and ODE-structural queries (`dJointGetType`, `dJointTypeContact`). |
| P2 | ✅ COMPLETE | `WbNewtonBackend` (~1500 lines), embedded CPython driving Newton + warp-lang. `newton_smoke_test.wbt` golden. |
| P3 | ✅ COMPLETE | URDF importer routes `physicsBackend "newton"` URDFRobots through Newton body/joint creation. `newton_husky_smoke_test.wbt`. |
...
(`8030a85e`): **Spot walks +4.87 m / 30 s / STRAIGHT verdict / 100%
upright**, trained in 84 seconds wall under Newton MuJoCo CPU. See
[spot-residual-rl.md](spot-residual-rl.md) for the reproducible recipe.

> ⚠️ **Scope caveat (don't over-generalize this to "Spot stands on Newton").**
> This is a *bench* result for one model+residual policy under Newton MuJoCo
> **CPU** with specific gains. The earlier reading that the broader **Spot RL
...
> 0-falls (Spot +30 m, Go2 +66 m, B2 +95 m). **Still honest, still open:** these
...
These ride on the physics arm's CUDA infrastructure (`WbCudaContext`,
`WbCudaBuffer<T>`, `WbCudaDispatch`).

| Subsystem | Status | Note |
|---|---|---|
| CUDA infrastructure M0 | ✅ SHIPPED | `src/omnisim/compute/cuda/` core primitives + smoke test. |
| CUDA M1 (GL/CUDA interop) | 🟡 DEFERRED 2-3 wk | `WbCudaBuffer::registerGlBuffer`; touches WREN ctx on Windows. **Once rendering arm moves to wgpu (Phase δ), this becomes wgpu/CUDA interop and the M1 design changes substantially.** Recommend deferring M1 until after Phase δ. |
...
+ FBO + WbWrenWindow integration, then a port to WGSL + wgpu
...
| Phase | Status | What landed / Exit criterion |
|---|---|---|
| R0 | ✅ LANDED (`4a6ec769`) | `WbRenderBackend.hpp/cpp` + `WbWrenBackend.hpp` — abstract interface, registry singletons, no behavior change. |
| R1 | ✅ LANDED (`4eccc53d`) | `OMNISIM_WITH_VULKAN=OFF` build flag (kept name for historical reasons; backend is actually wgpu post-refresh), `WbVulkanBackend` stub, registry wire-up. |
...
| R3.1 | ✅ LANDED + ✅ RUNTIME-VERIFIED 2026-05-28 | wgpu-native init + `isAvailable()` probe. With `WGPU_NATIVE_HOME` set on the build host, `WbVulkanBackend` (kept name pending Phase ζ rename) calls `wgpuCreateInstance` → `wgpuInstanceRequestAdapter` → `wgpuAdapterRequestDevice` → `wgpuDeviceGetQueue` in its constructor and flips `mAvailable` to `true` on success. Without `WGPU_NATIVE_HOME` the constructor stays in the R1-style unavailable branch (no compile-time / link-time dep on wgpu-native), so the `Newton-ON, Vulkan-OFF` and `Newton-ON, Vulkan-ON-without-dep` build matrix cells are both green for contributors who don't have wgpu-native installed. **Runtime evidence:** `OMNISIM_PROBE_WGPU=1 msys64/mingw64/bin/omnisim-bin.exe --help` (with `WGPU_NATIVE_HOME=…/_scratch/wgpu-native` at build time + `wgpu_native.dll` next to `omnisim-bin.exe`) emits the `[WbWgpuBackend] wgpu-native init OK (instance + adapter + device + queue)` log line. wgpu-native-specific gotcha discovered + fixed: v29's `wgpuInstanceWaitAny` panics on `unimplemented.rs:233`; the working sync pattern is `WGPUCallbackMode_AllowSpontaneous` + a `wgpuInstanceProcessEvents` busy-poll. Build hook + detection in `src/omnisim/Makefile`; install recipe in [`wgpu-native-setup.md`](wgpu-native-setup.md). **Unblocks R3.2** (mesh cache) — the device handle the probe opens is the seam R3.2 hangs the mesh cache off of. |
| R3.2 | 🟢 SCAFFOLDED + ✅ RUNTIME-VERIFIED 2026-05-28 (R3.2b ✅ DONE same day) | `WbWgpuMeshCache` (`src/omnisim/render/WbWgpuMeshCache.hpp/cpp`) lands the GPU-resident byte-stream cache: `acquire(meshId, vertBytes, idxBytes) -> {WGPUBuffer, WGPUBuffer, idxCount}` with upload-on-first-use semantics, USAGE_VERTEX/INDEX/COPY_DST descriptors, `wgpuQueueWriteBuffer` upload, and matching `release()` for explicit drop. Device + queue handles are now exposed from `WbVulkanBackend::device()` / `::queue()`; destructor properly releases instance → adapter → device → queue. **R3.2b WREN-mesh adapter (✅ DONE):** `WbWgpuMeshAdapter::acquireFromWren(cache, WrStaticMesh*)` in `src/omnisim/render/WbWgpuMeshAdapter.{hpp,cpp}` calls `wr_static_mesh_read_data` to pull WREN's coord/normal/uv/index arrays, interleaves into the 32-byte stride layout, keys cache by `reinterpret_cast<uint64_t>(wrenMesh)`. **Runtime evidence (probes 4 + 8):** byte-perfect identical readback `(0,0,255,255)` for hand-crafted vs adapter-translated uploads of the same +Z-normal triangle. |
| R3.3 | 🟢 SCAFFOLDED + ✅ RUNTIME-VERIFIED 2026-05-28 (R3.3b ✅ DONE same day) | `WbWgpuRenderTarget` (`src/omnisim/render/WbWgpuRenderTarget.{hpp,cpp}`) owns the RGBA8Unorm color attachment + texture view + readback buffer for a single off-screen render. `clearAndRead(clearColor, rgba8)` encodes a one-pass clear, copies the texture to a row-stride-aligned (256-byte multiple) buffer via `CopyTextureToBuffer`, polls the device via `wgpuDevicePoll` until `wgpuBufferMapAsync` resolves, and memcpys the unpadded pixels into the caller buffer. **R3.3b Camera-side wiring (✅ DONE):** `WbAbstractCamera::copyImageToMemoryMappedFile` is now `virtual`; `WbCamera::copyImageToMemoryMappedFile` override routes through `mWgpuTarget->clearAndRead` when `renderBackend()->kind() == Vulkan`. `WbCamera` owns a per-instance `WbWgpuRenderTarget *`, lazy-built via `ensureWgpuTarget()` at `width()*height()`, rebuilt on resize. The render-backend string parser also accepts `"wgpu"` as an alias for `"vulkan"` so world authors can write the natural name. **Runtime evidence:** `projects/samples/demos/worlds/rendering/camera_wgpu_smoke.wbt` boots through `headless_runner.py`; `camera_wgpu_smoke.py` controller reads `cam.getImage()` after 3 steps and writes `BGRA=(255,0,255,255) PASS magenta (wgpu path)` to `_scratch/r33b_smoke.txt`. The chain is: wgpu clear → CopyTextureToBuffer → BufferMapAsync → memcpy into `mImageData` → memory-mapped file → `Camera.getImage()` → Python bytes. |
| R3.4 | ✅ STEPS 1+2+3+4+5+5b RUNTIME-VERIFIED 2026-05-29 on a wgpu-ON build | **✅ RUNTIME VERIFICATION 2026-05-29:** built wgpu-ON (`OMNISIM_WITH_VULKAN=ON WGPU_NATIVE_HOME=_scratch/wgpu-native`, SDK already present so no download needed; linked to a scratch binary via `make ... TARGET=…` to avoid the concurrent session's lock on `omnisim-bin.exe`). `OMNISIM_PROBE_WGPU=11` emits all PASS lines: init OK; probe2 clearAndRead magenta; probe3 triangle bary; probe4 mesh; probe5 mesh+MVP; probe6 textured; **probe 11 = clearAndDrawScene = step-4 scene-walk** `pixel(4,4)=(255,0,0,255) PASS`; probe10 QImage. The **`camera_wgpu_scene_smoke.wbt` integration test** (the step-4 blocker) now reads back **non-clear pixels: 3072 of 3072** cyan (center BGRA=(0,141,141) = the box's lambertian-shaded `baseColor`) — vs the prior `0 of 3072`. The WREN-mesh-readback blocker is resolved by the R3.5 primitive codegen (Box drawn via `acquirePrimitive`). **R3.4-step-1: WGSL triangle path.** `WbWgpuShaders::kTriangleClipSpace` is the first hand-written WGSL — fixed-position triangle, vertex-shaded RGB, no buffers/bind-groups/uniforms. Runtime evidence (probe=3): `pixel(4,4)=(54,94,108,255) ~bary blend PASS`. **R3.4-step-2: vertex-buffer triangle.** `WbWgpuShaders::kTriangleVertexBuffer` is the production-layout WGSL — `@location(0..2)` pos3/norm3/uv2, stride 32 bytes, color = `max(normal, 0)`. `WbWgpuRenderTarget::clearAndDrawMesh(vb, ib, n)` consumes any `WGPUBuffer` pair (typically from `WbWgpuMeshCache::acquire`). Runtime evidence (probe=4): `pixel(4,4)=(0,0,255,255) PASS`. **R3.4-step-3: MVP-uniform + bind group.** `WbWgpuShaders::kTriangleMVP` adds `@group(0) @binding(0) var<uniform> u : Uniforms { viewProj : mat4x4<f32> }`. `WbWgpuRenderTarget::clearAndDrawMeshMVP(viewProj16, vb, ib, n)` lazy-builds the full stack. Runtime evidence (probe=5): `pixel(4,4)=(0,0,255,255) PASS`. **R3.4-step-4: scene-walk + multi-draw Lambertian shader — SCAFFOLDED but pixel-output BLOCKED.** Three pieces landed: (a) `WbWgpuShaders::kSolidLit` (per-draw uniform with viewProj + model + baseColor + light, single-binding 192-byte block + 64-byte align pad, Lambertian fragment). (b) `WbWgpuRenderTarget::clearAndDrawScene(viewProj, lightDirAmbient, draws[], n, rgba8)` builds one dynamic-offset uniform buffer of `slotCount * 256 B`, depth attachment, one bind-group rebind per draw via dynamic offset. **Runtime-verified standalone (probe=11)**: the same `+Z-normal` triangle from probe=4, fed through `clearAndDrawScene` with `baseColor=(1,0,0)`, reads back `pixel(4,4)=(255,0,0,255) PASS`. (c) `WbCamera::copyImageToMemoryMappedFile` walks `WbWorld::topSolids()` recursively, harvests each Shape's WrStaticMesh through `WbWgpuMeshAdapter::acquireFromWren`, builds a per-Solid model matrix from `WbGeometry::matrix()` + a column-major basis-swap (Webots `+X-forward, +Z-up` → wgpu `-Z-forward, +Y-up`), composes `viewProj = perspective(vertFov, aspect, near, far) * basisSwap * inverse(cameraMatrix())`. With the camera_wgpu_scene_smoke.wbt test world (a cyan PBRAppearance Box at +X 1m, camera at origin), the scene walk fires correctly — diagnostic log shows `[WbCamera scene] first draw: idx=36 baseColor=(0.00,1.00,1.00) trans=(1.000,0.000,0.000)` and `[WbCamera] 'cam_wgpu' rendered through wgpu (64x48, 1 draw)`. **The blocker:** the readback shows ZERO non-clear pixels — the cube renders to nothing. Root cause diagnosed but not fixed: `wren::StaticMesh::prepareGl` clears `mCoords/mIndices/mNormals/mTexCoords` after uploading them to GL (see `src/wren/StaticMesh.cpp:1916`), and `wren::StaticMesh::readData` then falls back to `glGetBufferSubData` (line 1991+). The OmniSim headless harness runs with `--no-rendering` so there's no GL context, so `readData` returns garbage (no zero-init either) for any mesh built through `wr_static_mesh_unit_*_new` (cache-backed primitives — Box, Sphere, Cylinder, …). `wr_static_mesh_new(...)` from raw arrays (the probe=8 path) keeps `mCoords` populated so it works fine. **Fix paths for next contributor:** (A) — make `prepareGl` keep `mCoords/mIndices` populated alongside the GL buffer (~ +50 MB on a heavy scene, but cleanest); (B) — add a primitive-aware codegen in `WbWgpuMeshAdapter` that generates pos3+norm3+uv2 directly for `WbBox` / `WbSphere` / `WbCylinder` / `WbCapsule` / `WbPlane` from their geometry parameters, bypassing `readData`; (C) — make the wgpu Camera path opt-in via `WbApplication` running with a hidden GL context so `glGetBufferSubData` works. Path B is probably the right call (avoids the memory hit, isolates wgpu-side from WREN-side mesh storage). Diagnostic env knob `OMNISIM_R34_IDENTITY_VP=1` bypasses the viewProj math (passes identity) so contributors can isolate projection-math bugs from mesh-readback bugs. **R3.4-step-5: primitive codegen unblocks step-4 (Fix Path B).** `WbWgpuMeshAdapter::acquirePrimitive(cache, meshId, PrimitiveKind)` (in `src/omnisim/render/WbWgpuMeshAdapter.{hpp,cpp}`) builds the pos3+norm3+uv2+idx stream CPU-side for the four common cache-backed primitives — `Box` (24 verts / 36 idx matching `wren::createUnitBox`), `Plane` (4 verts / 6 idx matching `createUnitRectangle`), `UVSphere` (subdivision-24 lat/long), `Cylinder` (subdivision-24 side + 2 caps). Mesh-id stays the `WrStaticMesh *` bit pattern so caching is shared across the WREN-side and wgpu-side pointers. `WbCamera::collectShapeDraws` switches on `geom->nodeType()` to choose `acquirePrimitive` for those four kinds (falling back to `acquireFromWren` for Capsule / IndexedFaceSet / Mesh / CadShape, which keep `mCoords` populated in WREN — Capsule via `wr_static_mesh_capsule_new` baking radius+height; IFS via raw arrays through `wr_static_mesh_new`). The geometry's local scale (`size` for Box / Plane, `radius` for Sphere, `(radius, radius, height)` for Cylinder — mirroring WREN's `wr_transform_set_scale` choice for each) is composed into the model matrix via `WbMatrix4::scale`. **Build status 2026-05-28 (workflow audit):** all changed sources compile + link clean via `bash scripts/dev/build_with_cd.sh` (`omnisim-bin.exe` rewritten at 7,277,568 bytes; zero errors / zero warnings). The earlier "link blocked by user's running omnisim-bin.exe" caveat no longer applies. **Runtime verification status:** the default-built `omnisim-bin.exe` is `WB_WGPU_NATIVE_AVAILABLE=0` — probe 1 logs `[WbWgpuBackend] wgpu-native unavailable (build flag off, dep missing, or runtime init failed). Probe stops here.` So the `OMNISIM_PROBE_WGPU=N` PASS lines for probes 3-11 + the `camera_wgpu_scene_smoke.wbt` integration test are **NOT reproducible on the shipped binary by design** (no behavior change for ODE/Newton-only users). To re-verify on this checkout, run `scripts/dev/setup_wgpu_native.sh` then rebuild with the wgpu flag on (recipe in [wgpu-native-setup.md](wgpu-native-setup.md)). On a wgpu-enabled build the smoke world should read back ~half the pixels as the box's cyan `baseColor` (lambertian-shaded against `(0.3, 0.4, -0.85)` + 0.25 ambient); the diagnostic prints `[WbCamera scene] first draw` + `[WbCamera] 'cam_wgpu' rendered through wgpu` remain in place + the integration signal is `non-clear pixels: 0 of 3072` ⇒ `> 0`. **One-shot setup script** [`scripts/dev/setup_wgpu_native.sh`](../../scripts/dev/setup_wgpu_native.sh) lands 2026-05-28. **⚠️ It was BROKEN until 2026-05-29** (so the original "RUNTIME-VERIFIED 2026-05-29" above used a *manually-present* SDK, not this script): it pinned `WGPU_NATIVE_VERSION=v24.0.5.1`, a **non-existent tag (404)**, and downloaded the **msvc** variant while the MinGW build + the script's own `.dll.a` check require the **gnu** variant. **FIXED 2026-05-29** (`15817ae5`): pinned `v29.0.0.0` (the code is "API tracked against wgpu-native v29.0.0+" per `WbVulkanBackend.cpp`) + `wgpu-windows-x86_64-gnu-release.zip`. Separately, [`build_with_cd.sh`](../../scripts/dev/build_with_cd.sh) was fixed (`d57c0002`) to forward `OMNISIM_WITH_VULKAN`/`WGPU_NATIVE_HOME` to make (it silently dropped them, compiling the stub). **Reproducibly RE-VERIFIED 2026-05-29** on a freshly built v29 wgpu-ON binary: `OMNISIM_PROBE_WGPU=2` → `wgpu-native init OK (instance+adapter+device+queue)` + RTT `clearAndRead OK pixel(0,0)=(255,0,255,255) PASS`; `camera_wgpu_scene_smoke.wbt` → `rendered through wgpu (64x48)` + **non-clear pixels: 3072 of 3072** cyan. The download is still a human-opt-in (auto-mode blocks third-party-binary installs), and — note for in-tool builds — flags must be passed as direct `make` CLI args because this harness's Bash tool strips exported env from child scripts. |
| R3.5 | 🟢 SCAFFOLDED + ✅ STATIC-VERIFIED 2026-05-28 (R3.5b code-landed same day; probe 6 / 10 PASS pixel-values reproducible only on wgpu-enabled rebuild — NOT on shipped default-build binary) | Texture bridge. `WbWgpuTextureCache` (`src/omnisim/render/WbWgpuTextureCache.{hpp,cpp}`) is the texture analog of `WbWgpuMeshCache`: `acquire(textureId, width, height, rgba8bytes)` → `{WGPUTexture, WGPUTextureView}` with `wgpuQueueWriteTexture` upload (handles the 256-byte row alignment internally so callers don't need to). Format pinned at RGBA8Unorm + single mip + sample-count 1. `WbWgpuShaders::kTriangleTextured` is the matching WGSL — uniform at binding 0, `texture_2d<f32>` at binding 1, `sampler` at binding 2. **R3.5b QImage adapter (✅ DONE):** `WbWgpuImageAdapter::acquireFromQImage(cache, textureId, QImage)` in `src/omnisim/render/WbWgpuImageAdapter.{hpp,cpp}`. Approach: pull pixels from `WbImageTexture::mImage` (a `QImage *` that survives wren::Texture2d's mData-destroy-after-GPU-upload because it's owned by the OmniSim node, not WREN). Convert to `Format_RGBA8888` (memory-order RGBA on all endians; ARGB32 is little-endian BGRA which is the wrong byte order for `RGBA8Unorm`). Repack to tight stride if `bytesPerLine != w*4`. **Runtime evidence (probes 6 + 10):** a 2×2 solid-red `QImage(Format_ARGB32)` round-tripped through the adapter renders identical `pixel(4,4)=(255,0,0,255)` to the hand-crafted byte upload of probe=6. |
...
`wr_scene_get_last_render_counts` and logged from `WbWrenWindow`
...
- [spot-residual-rl.md](spot-residual-rl.md) — model+residual RL recipe under Newton
