...
2-link IK. The **ghost** (`projects/policies/controllers/ghost_hologram/ghost_hologram.py`) plays it with
...
✅ **RETIRED 2026-08-15** (`dbb0513c6`). The scratch
`projects/policies/control/brain/` package (923 LOC, 4 files) is deleted; the
canonical `control/g1_brain.py` is the only brain. Verified before removal: the
package was reachable ONLY via `G1_BRAIN_MODULE`, and the tree's two assignments
of that variable both name the root module — `g1_walk_deploy.py:207` (the
default) and `run_g1_brain_deploy.ps1:36`. `run_g1_brain.ps1` looked like a third
referrer but names it in a header comment only and never sets the variable, so it
has always run the root brain; its header is now corrected. Verified after: no
import, `importlib` target or dispatch value anywhere names `control.brain`.
