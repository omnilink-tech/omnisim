...
- `src/omnisim/vrml/OmNode.hpp`
- `src/omnisim/nodes/OmBaseNode.hpp`
- `src/omnisim/nodes/utils/OmWorld.hpp`
- `src/omnisim/nodes/OmSolid.hpp`
- `src/omnisim/engine/OmSimulationWorld.hpp`
- `src/omnisim/control/OmController.hpp`
...
`OmImageTexture` and related classes sit near a bad boundary today: node semantics, asset loading, and renderer upload behavior are tightly coupled.
...
Measure the configured dependency graph directly before and after broad-header work:

```bash
python scripts/dev/include_fanout.py OmNode.hpp OmBaseNode.hpp OmSolid.hpp OmWorld.hpp
```

The command reads `src/omnisim/build/release/.deps/*.d`, so its counts are the
translation units the current make configuration will actually invalidate, not
a source-text approximation. Rebuild once before measuring if those dependency
files are stale.
