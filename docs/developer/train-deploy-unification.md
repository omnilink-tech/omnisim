...
| | **Training** | **Deployment** |
|---|---|---|
| What it is | thin Python harness driving batched `mujoco_warp` on GPU | full C++ OmniSim engine ([`WbNewtonBackend.cpp`](../../src/omnisim/physics/WbNewtonBackend.cpp)) wrapping one Newton world |
...
```
  ...
  Layer C  STEP CORE (the World.step) .. g1_deploy_runtime.py  (← WbNewtonBackend.cpp)      [SHARED in Python; C++ flip gated — Phase 2]
```
...
Newton physics (`WbNewtonBackend.cpp`'s `kNewtonRuntimeSource` string). The
...
- **Rejected: runtime load (option B).** Reading the `.py` from disk removes
  the rebuild requirement but breaks self-containment and adds a silent-ODE-
  fallback failure mode (missing file / `OMNISIM_HOME` skew). Not worth it.
...
> "the change lives in compiled native code (`WbNewtonBackend.cpp`), so it
...
4. **Generalize** `g1_env_core` beyond G1 (H1, Spot, Go2 share the same obs/IC
