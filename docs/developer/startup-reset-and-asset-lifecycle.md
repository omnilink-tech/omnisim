...
2. syntax-parse it
3. instantiate nodes through `WbNodeReader`
...
`WbSimulationWorld` explicitly emits a `"Downloading assets"` status, resets the download manager, asks the root node to download assets, and then waits in a loop until download progress reaches 100.
...
`WbDownloadManager::createDownloader()` calls:

- `WbSimulationState::instance()->pauseSimulation()`

And once all downloads complete, `WbDownloadManager` calls:
...
`WbSimulationWorld::reset()` pauses simulation, resets time, blocks regeneration, resets runtime state, and only resumes simulation immediately if download progress is already 100.
...
`WbDownloader::finished()` saves successful primary downloads to disk through `WbNetwork::instance()->save(...)`.
...
- `python scripts/dev/omnisim_dev.py profile-world resources/projects/worlds/empty.wbt`
