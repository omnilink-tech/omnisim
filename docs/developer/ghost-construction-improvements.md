...
| Kinematic FK replay on terrain ([hill_terrain.py](../../projects/policies/research/shadowing/hill_terrain.py)) | Analytic gait evaluated on an incline | hill-walk (B2/Go2/Spot) | FK check only |
...
4. **Deploy runs a MuJoCo solver — gated by config, not XPBD (CORRECTED 2026-06-23).** Generator
   ...
   "long-running G1 deploy gap", [WbNewtonBackend.cpp:356-363](../../src/omnisim/physics/WbNewtonBackend.cpp#L356-L363));
...
2. **[DONE] Deploy-solver config guard + world pins (REFRAMED 2026-06-23).** *Not* a foreign-engine
   ...
   world" migration was never done) and pinned them all. Verified: `g1_smoke.wbt` now logs
...
- **Runtime guard** `OMNISIM_REQUIRE_MUJOCO_SOLVER=1` — raises at world load if the Newton solver resolves
  ...
  [WbNewtonBackend.cpp](../../src/omnisim/physics/WbNewtonBackend.cpp) and re-extracted to
