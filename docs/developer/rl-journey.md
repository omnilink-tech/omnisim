...
([omniquad-residual-rl](omniquad-residual-rl.md),
...
> (OmniQuad/Atlas sections date to 2026-05-29). **Where this journey and that file
...
## 1. Origins — residual RL on OmniQuad (the paradigm is born)
...
- **Model layer:** analytic FK/IK (`omniquad_kinematics.py`, round-trips to machine
  epsilon), a trot foot-trajectory generator (`omniquad_gait.py`), and a body-pose
  balance PD (`omniquad_balance.py`).
...
This produced the first **STRAIGHT-verified** OmniQuad walker, and — more durably —
...
**2026-05-28.** The OmniQuad recipe was ported to the **Unitree G1** (a 13-DOF
...
## 5. OmniQuad walks straight — the G1 recipe ported back (2026-06-12)

The mature G1 walking recipe (foot-space model + IK + residual RL) was ported
*back* to OmniQuad, superseding the original residual stack and an intermediate
Raibert layer ([project omniquad-walk-g1-recipe]). `gpu_omniquad_walk_main` deploys
...
This closed a long-standing item: earlier OmniQuad work *collapsed under Newton*
(a W6 lateral/roll fidelity gap, traced to W1-mesh self-collision and fixed
in-bridge). The G1 recipe is what made OmniQuad a *real* Newton deploy.
...
cubes/tubes **12/12** and sorts a 21-part, 3-bin conveyor line 21/21
...
- **All RL worlds on Newton** ([project rl-newton-migration]): the last 11 ODE RL
  worlds were flipped to Newton; a OmniQuad self-collision regression was fixed
  in-bridge.
...
10. **A correction layer on a *complete* baseline is a passenger at best and a
    ...
    *nothing*; the sign of "nothing" depends on the task's stability margin. On a
    statically-stable, redundant task (OmniQuad walk) the residual is a harmless
...
|--|--|--|--|
| **OmniQuad** walk | model+residual, straight | **+119 m / 298 s, 0 falls, straight, `wz` steering** | ✅ shipped (`gpu_omniquad_walk_main`) |
...
(OmniQuad, G1) with the *same* two-layer recipe, the deploy gap is now a
...
| Doc | What it covers |
|--|--|
| [omniquad-residual-rl.md](omniquad-residual-rl.md) | the original quadruped model+residual recipe; the "passenger" finding |
...
| [rl-current-state.md](rl-current-state.md) | the canonical "is it done?" status; supersedes this doc for current-state claims (G1/locomotion re-verified 2026-06-19; OmniQuad/Atlas date to 2026-05-29) |
