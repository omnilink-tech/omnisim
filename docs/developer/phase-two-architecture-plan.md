...
> ⚠ **2026-08-08 — this plan's physics guardrails have been overtaken by events.**
> Its architecture snapshot and its "do not replace ODE" guardrail both predate the
> ODE retirement: `bdc02139` deleted `src/ode` + `include/ode` (106,283 lines) and
> `a0ed801c` deleted the physics-plugin C ABI, so Newton with `SolverMuJoCo` is now
> the only physics backend and there are no physics plugins. Read the physics
> statements below as a record of what the plan committed to, not as current state.
> Campaign record: [ode-retirement-campaign.md](ode-retirement-campaign.md).
...
- `src/omnisim/gui/OmGuiApplication.*` owns task dispatch such as normal run, conversion, sysinfo, version, and update flows.
...
The world pipeline is centered on the VRML/PROTO stack and the `OmWorld` object model.

Key pieces:
- `src/omnisim/vrml/OmTokenizer.*`: token stream and file/token error reporting
- `src/omnisim/vrml/OmParser.*`: syntax parsing
- `src/omnisim/vrml/OmNodeReader.*`: node creation during world loading
- `src/omnisim/vrml/OmNode.*`: base node class, field reading, cloning, export, reset, PROTO awareness
- `src/omnisim/vrml/OmProtoManager.*`: EXTERNPROTO / PROTO registry and lookup
- `src/omnisim/nodes/utils/OmTemplateManager.*`: template regeneration control
- `src/omnisim/nodes/utils/OmWorld.*`: world loading, root creation, saving, reset, robot registry, W3D export
...
- `OmWorld` is already the effective simulation state root, but it is still product-aware
...
- `src/omnisim/nodes/OmRobot.*`
- `src/omnisim/nodes/OmWorldInfo.*`

The current split is roughly:
- `OmWorld`: world graph, load/save/reset, root ownership
- `OmSimulationWorld`: simulation timer, physics stepping, camera render triggers, reset behavior
- `OmControlledWorld`: controller lifecycle, synchronization, request/answer flow, extern controller connection state

Important details visible in the current code:
- assets are downloaded during world setup before normal simulation flow
- physics plugins were loaded per world (⚠ no longer: `a0ed801c` deleted the physics-plugin C ABI)
...
The split today is:
- `src/omnisim/control/OmController.*`: process launch, IPC/TCP server setup, request parsing, answers, stdout/stderr forwarding, extern controller lifecycle
...
- `src/omnisim/gui/OmView3D.*` and related classes tie rendering into the desktop UI
...
The closest thing to a core runtime exists, but it is not presented as a stable layer. The effective core is spread across `OmWorld`, `OmSimulationWorld`, `OmControlledWorld`, controller protocol code, and node internals.
...
`OmControlledWorld` and `OmController` jointly manage synchronization, process startup, request buffering, extern connection state, and parts of runtime retry logic.
...
- Make `OmSimulationWorld` and `OmControlledWorld` the initial implementation behind that interface.
...
- reduce direct dependence on `OmApplication::instance()` from load/parsing paths
...
- Do not replace ODE, Wren, or the controller APIs in this phase. (⚠ **OVERTAKEN BY EVENTS 2026-08-08** — recorded here as what the plan committed to. ODE *was* replaced, outside this phase: `bdc02139` deleted it and Newton/`SolverMuJoCo` is now the only physics backend. Wren and the controller APIs still stand.)
