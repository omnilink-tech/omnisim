...
- `src/omnisim/nodes/utils/WbTemplateManager.cpp`
- `src/omnisim/nodes/utils/WbDictionary.cpp`
...
`WbTemplateManager::blockRegeneration(bool block)` flips a global manager flag.
...
`WbTemplateManager::subscribe(...)` and `recursiveFieldSubscribeToRegenerateNode(...)` walk node fields and descendants to attach regeneration behavior where needed.
...
`WbDictionary` contains asserts stating that nested dictionary updates should be avoided.
...
There are at least two important signs of incompleteness in `WbDictionary.cpp`:
...
`WbDictionary` creates local dictionaries for USE nodes and nested PROTO situations.
...
- `python scripts/dev/omnisim_dev.py test-world tests/protos/worlds/template_deterministic.wbt`
