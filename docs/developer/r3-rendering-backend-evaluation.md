...
Four candidates considered. The R3 layer goes between `OmRenderBackend`
...
existing `OmWrenCamera` code is sized for a few cameras, not dozens.
...
The hottest piece. WREN had ~20 GLSL 330 shaders under `resources/wren/shaders/`
(deleted with WREN on 2026-08-23, `976b9449d`; read them at
`git show 976b9449d^ -- resources/wren/shaders`). wgpu wants WGSL.
...
|---|---|---|---|
| **R3.1** | wgpu-native init + `isAvailable()` probe. Build flag does something. No rendering. | `OmVulkanBackend.cpp` → renamed `WbWgpuBackend.cpp`; Makefile (wgpu-native dep + Rust toolchain detection) | 3–4 days |
| **R3.2** | wgpu-side mesh cache. Upload-on-first-use from WREN's mesh data. Smoke test verifies upload, no rendering. | new `src/omnisim/render/OmWgpuMeshCache.{hpp,cpp}` | 1 week |
| **R3.3** | Single-Camera render-to-texture: a Camera with `renderBackend "wgpu"` routes its frame through wgpu. Solid-color shader, no scene geometry. | `OmCamera.cpp`, new `WbWgpuCamera.cpp` | 1.5 weeks |
...
| **R3.5** | Texture bridge. Cameras with textured materials render correctly. | `OmWgpuTextureCache.{hpp,cpp}` | 1 week |
...
| **R3.7 (new)** | Newton interop probe: wire Newton's body buffer to a wgpu storage buffer; verify a single instanced draw reads positions directly. **Gated on Newton P5 + Newton-canonical (§16.2 #1).** | bridge code between `OmNewtonBackend` and `WbWgpuBackend` | 1.5 weeks |
...
| Probe code site | `OmRenderBackend::isAvailable()` for Kind::WGPU | Bound to backend choice |
