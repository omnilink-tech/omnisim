# Spot motions via the shadowing pipeline

Three Boston Dynamics Spot motions, each produced the same way — a **feasible
...
[shadowing.md](shadowing.md)) applied to Spot; the per-robot tracker lives in
`projects/policies/research/training`.

| Motion | Ghost source (Component 1) | Tracker (Component 3) | Deploy | Launch |
|---|---|---|---|---|
| **Walk / stop / walk** | analytic foot-space trot model (`spot_trot_gait.py`) | velocity-conditioned residual (`gpu_spot_walk_vc_main`) | RL the whole time | `run_spot_walk_vc_ghost_demo.ps1` |
| **Crouch–recover** | MPPI `ghost_generator` | none — **feedforward** | replays the ghost open-loop | `run_spot_crouch_deploy.ps1` |
| **Get-up from the ground** | MPPI `ghost_generator` | reference-tracking residual + heavy DR (`gpu_spot_getup_main`) | RL rise → faded hold | `run_spot_getup_ghost_demo.ps1` |
...
## The pipeline, concretely for Spot
...
- **Component 1 — the ghost.** For a *gait*, the analytic trot model is the feasible
  ...
  *non-gait* motions (crouch, get-up) the **MPPI receding-horizon `ghost_generator`**
  discovers a feasible trajectory over Spot's real dynamics. Spot's dump has
  ...
  **override the GhostGenerator mappings on the instance** (pos actuators at even ctrl
  idx; foot bodies 4/7/10/13) — see `projects/policies/research/tools/generate_spot_{crouch,getup}.py`.
- **Component 2 — the verifier.** `ghost_verifier` re-sims the ghost: the **open-loop
  ...
  accepted G1 ghost too). `projects/policies/research/tools/verify_spot_walk_ghost.py` certified the
  walk ghost; the get-up generator runs the gate inline.
- **Component 3 — the tracker.** `gpu_mjwarp_spot_getup_trainer.py` (the walk trainer
  ...
  `gpu_mjwarp_spot_getup_trainer.py` (run locally).
- **Component D — deploy.** A mimic controller replays the certified ghost's position
  targets + the RL residual on the URDF Spot in Newton.
...
1. **Velocity conditioning (walk/stop).** One policy, commandable forward speed incl.
   0 = stand (`SPOT_VX_CMD_MAX`); the gait scales toward the standing pose. Bias the
...
5. **Fade the residual at the end of a one-shot motion.** The get-up rose then
   *collapsed* the hold ~2 s later — the rise-trained residual over-actuates a now-
   ...
   baseline to the proven trot stand** (`SPOT_GETUP_HOLD_FADE_S`) → crisp upright stand
...
- **C1 works:** MPPI + a **launch-velocity task objective** (the toss mechanism, driving
  ...
  four feet airborne**, +0.75 m, landed upright in the ghost. `generate_spot_jump.py`.
...
- **Deploy — the wall, and how far we pushed it:**
  ...
  3. **Height ceiling (the deep wall):** the deploy **under-jumps** and *saturates* at
     ...
     **launch-velocity reward** (`--rw-launchvel`, gpu_spot_jump_lv) → 0.693.
- **Diagnosis:** the ghost reaches 1.05 m with the *same* kp500 actuators in mujoco,
  ...
  Policy `gpu_spot_jump_main`; `run_spot_jump_deploy.ps1` / `run_spot_jump_ghost_demo.ps1`.
...
- Gait model: `projects/policies/control/gait/spot_trot_gait.py` (`speed_scale` = velocity conditioning).
- Generators (C1): `projects/policies/research/tools/generate_spot_crouch.py`, `generate_spot_getup.py`.
- Verifier helper (C2): `projects/policies/research/tools/verify_spot_walk_ghost.py`.
- Trainer (C3): `projects/policies/research/training/gpu_mjwarp_spot_getup_trainer.py` (`--track-ref`).
- Certified ghosts: `projects/policies/research/shadowing/ghosts/spot_{crouch,getup}_ghost.npz`.
- Deploy controllers/worlds: `projects/policies/controllers/spot_{walk_deploy,crouch_mimic,getup_mimic,ghost,getup_ghost}/`, `projects/policies/worlds/spot_*_deploy.wbt` + `*_ghost_demo.wbt`.
- Policies: `projects/policies/research/inference/policies/gpu_spot_walk_vc_main/`, `gpu_spot_getup_main/`.
- Launch scripts: `scripts/dev/run_spot_{walk_vc_ghost_demo,crouch_deploy,getup_deploy,getup_ghost_demo}.ps1`.
...
- [spot-residual-rl.md](spot-residual-rl.md) — the earlier Spot residual-RL walk work.
