...
`WbView3D : WbWrenWindow : QWindow` is hardcoded to WREN/OpenGL. The `WbRenderBackend`
...
parity in the **additive** wgpu pane first, or (b) introduces a seam into `WbWrenWindow`,
which edits code every user runs.

The HWND surface type is fixed in `WbWrenWindow`'s ctor: `setSurfaceType(QWindow::OpenGLSurface)`
+ `WbWrenOpenGlContext::init(...)`. On Windows an HWND's pixel format is set once — **flipping
...
|---|---|---|---|---|---|
| Per-frame submission | `WbWrenWindow::renderNow` (`wr_scene_render` → blit → `swapBuffers`) | total | **SHARED** | L | breaks the view for **all** users, every world |
| Picking / selection | `WbWrenPicker` (`wr_scene_render_to_viewports` "picking" pass → `wr_frame_buffer_copy_pixel` GL readback); `WbSelection` | total | **SHARED** | M | click-to-select dead |
| Manipulator handles | `WbWrenAbstractManipulator`, resize manips, drag-event classes (WREN renderables + handle-ID picking) | total | **SHARED** | M | drag-to-move/rotate dead |
| Overlays | `WbWrenFullScreenOverlay`, `WbWrenTextureOverlay` (camera/display device output), `wr_viewport_attach_overlay` | total | **SHARED** | M | device overlays + status messages gone |
| Video / screenshots | `WbWrenWindow::grabWindowBufferNow` (`readPixels` GL), PBO path (`wr_scene_init_frame_capture`/`map_pixel_buffer`) | total | **SHARED** | M | recording/screenshots broken |
| Optional renderings | `WbWrenRenderingContext` visibility-mask + per-node renderables (bounding objs, contact pts, joint axes, COM, rays, normals…) | mask is **decoupled** | **ADDITIVE** | S | (none — read the mask, render in parallel) |
| Backend-dispatch seam | none exists; `WbWrenWindow` ctor + `renderNow` | — | **SHARED** | L | architecture-defining |

Only the optional-renderings **visibility mask** is genuinely decoupled (a wgpu renderer can
read `WbWrenRenderingContext::optionalRenderingsMask()` and filter its own draws). Everything
...
*pane* (`WbWgpuView`/`WbWgpuSurface`) is already a self-contained, opt-in render surface.
...
- **Phase 3c-A — additive parity (breaks nothing).** All new code lives in the wgpu path,
  gated on `OMNISIM_VIEW3D_WGPU`; WREN's `WbView3D`/`WbWrenWindow` are not edited.
  - Wire mouse input on `WbWgpuView` → a **wgpu picking pass** (render IDs to an offscreen
    target, read back the clicked texel) → update the shared `WbSelection`.
...
- **Phase 3c-B — the flip (gated + reversible; the only risky step).** Introduce a
  `WbMainViewport` seam (or extend `WbRenderBackend`) so `WbWrenWindow::initialize()` /
...
- **Phase 3c-B: possible, and this is where the risk concentrates.** Editing
  `WbWrenWindow`'s ctor (surface type) and `renderNow` is code every user runs. The danger
...
**3c-A.1: wgpu picking in the pane.** Wire `WbWgpuView` mouse-press → an offscreen wgpu
ID-render pass → readback → set `WbSelection`. It's additive (zero WREN-path edits),
