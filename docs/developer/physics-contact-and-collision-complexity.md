...
- `src/omnisim/engine/WbSimulationCluster.cpp`
- `src/omnisim/nodes/utils/WbMassChecker.cpp`
- `src/omnisim/nodes/WbWorldInfo.cpp`
...
`WbSimulationCluster` contains a warning path for when the number of contact points exceeds the allowed number of contact joints between two materials.
...
`fillSurfaceParameters(...)` in `WbSimulationCluster` applies a fairly large amount of per-contact policy:
...
`WbMassChecker` warns when the ratio between the heaviest and lightest dynamic solids exceeds `1e5`.
...
`WbWorldInfo.cpp` warns that simulation replicability is not guaranteed when multithreading is enabled.
...
- a contact-heavy world such as `tests/physics/worlds/contact_points.wbt`
