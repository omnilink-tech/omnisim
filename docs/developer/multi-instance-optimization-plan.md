...
Log path defaults to the **shared install-root file** `<OMNISIM_HOME>/omnisim_log.txt` (`main.cpp` passes `omnisimDirPath + "/omnisim_log.txt"` to `WbLog::initFileLog`, which uses it verbatim). Port-isolation is **opt-in per child via the `OMNISIM_LOG_PATH` env override** — set it to a unique path per instance, or parallel children all clobber the one shared file (the per-`webotsTmpPath()` default was never landed). `WbLog.cpp` `initFileLog` reads `OMNISIM_LOG_PATH` first and only falls back to the passed path; on a busy/locked file it drops to a per-pid `omnisim_log.<pid>.txt` sibling.
...
- ⚠️ The K=1-vs-K=1 cross-run loading delta (1651 ms → 315 ms, "−80.9%") is dominated by OS disk cache, not by the WbLog change. **Cross-run absolute numbers should not be compared**; only intra-run K-scaling ratios are clean. The next baseline should be captured cold to make future deltas comparable.
...
- [src/omnisim/nodes/WbImageTexture.cpp](../../src/omnisim/nodes/WbImageTexture.cpp) — `gImagesMap`
- [src/omnisim/nodes/WbTriangleMeshGeometry.cpp](../../src/omnisim/nodes/WbTriangleMeshGeometry.cpp) — `cTriangleMeshMap`
- [src/omnisim/nodes/WbCamera.cpp](../../src/omnisim/nodes/WbCamera.cpp) — `mInvalidRecognizedObjects` not cleared on `reset()`
- [src/omnisim/nodes/WbSolidDevice.cpp](../../src/omnisim/nodes/WbSolidDevice.cpp) — `cDirtySensors` not cleared on world reset
- [src/omnisim/core/WbLog.cpp](../../src/omnisim/core/WbLog.cpp) — `mPendingConsoleMessages` only cleared at process exit

**Change:** Add a single "world-reset" hook (or extend the existing one in `WbControlledWorld::reset`) that walks these caches and evicts entries belonging to nodes that no longer exist. Map-style caches get an `evictUnused()` method; the queues get a `clear()` on reset.
...
- [src/omnisim/gui/WbView3D.cpp](../../src/omnisim/gui/WbView3D.cpp) — WREN init still runs under `--no-rendering`
- New CLI flag in [src/omnisim/gui/WbGuiApplication.cpp](../../src/omnisim/gui/WbGuiApplication.cpp), e.g. `--no-sensors` (or fold into `--no-rendering=full`)
...
**Acceptance:** Per-process RSS drop of ≥30% on a no-sensor world. K=4 parallel headless instances of `newton_husky_swarm_drive.wbt` complete the same step count in ≤1.3× single-instance wall time (currently GPU contention bites earlier).
...
**File:** [src/omnisim/control/WbController.cpp](../../src/omnisim/control/WbController.cpp), the `mRequest += readAll()` / `.left(n)` / `.remove(0, n)` loop in `readRequest()` around lines 1048–1135.
...
**File:** [src/omnisim/control/WbControlledWorld.cpp](../../src/omnisim/control/WbControlledWorld.cpp), the per-controller serial loop around lines 448–458.
...
**Files:** [src/omnisim/nodes/WbCamera.cpp](../../src/omnisim/nodes/WbCamera.cpp), [src/omnisim/nodes/WbLidar.cpp](../../src/omnisim/nodes/WbLidar.cpp), [src/omnisim/nodes/WbRangeFinder.cpp](../../src/omnisim/nodes/WbRangeFinder.cpp), and the per-sensor `WbWrenCamera` instances.
...
**File:** [src/omnisim/nodes/WbCamera.cpp](../../src/omnisim/nodes/WbCamera.cpp) — recognition object detection (`~lines 720–737`) and overlay update path (`~lines 381–403`).
...
**File:** [src/omnisim/wren/WbWrenTextureOverlay.cpp](../../src/omnisim/wren/WbWrenTextureOverlay.cpp) (already backlog item #18) — `~lines 296–316`, depth float→int conversion.
...
- ⏸ Item 3 — pure-physics `--no-sensors` mode. RSS sizing data above shows the realistic ceiling is ~100-150 MB per omnisim-bin (= ~30 % of omnisim-bin RSS, ~10 % of total per-instance RSS). Architecture (unconditional `WbMainWindow` → `WbView3D` → `WbWrenWindow`) requires its own milestone.
